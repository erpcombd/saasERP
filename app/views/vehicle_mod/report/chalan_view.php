 ...
 .print-header-wrap{
  ...
  <p class="text-center">Cell :<?php echo find_a_field('project_info','proj_phone','1')?>.Web:<?php echo find_a_field('project_info','proj_email','1')?>  </p>
            ...
            <div class="text-center">
              <button class="btn btn-default outline border rounded-pill border border-dark">Challan</button>
            </div>
            ...
            <div class="row">
              <div class="col-6 row mb-2 mb-sm-3">
                ...
                <div class="col-sm-10">
                  <input type="text" readonly class="form-control" id="no" value="<?php echo $chalan_no;?>">
                </div>
              </div>
              <div class="col-6 row mb-2 mb-sm-3">
                <label for="date" class="col-sm-2 col-form-label">Date</label>
                <div class="col-sm-10">
                  <input type="text" readonly class="form-control" id="no" value="<?=date("j-M-Y",strtotime($chalan_date));?>">
                </div>
              </div>
            </div>
            <div class="row">
              <div class="col-6 row mb-2 mb-sm-3">
                ...
                <div class="col-sm-10">
                  <input type="text" readonly class="form-control" id="name" value="<?php echo ($dealer->propritor_name_b=='')?$dealer->propritor_name_e:$dealer->propritor_name_b;?>">
                </div>
              </div>
            ...
            <div class="row">
              <div class="col-sm-6 row mb-3">
                ...
                <div class="col-sm-10  mr-3">
                  <input type="text"  readonly class="form-control" id="address" value="Shantahar Road,Nurani more,Bogura">
                </div>
              </div>
            </div>
              ...
              <table class="table table-bordered border-primary">
                <thead>
                  <tr>
					...
					<th colspan="2">bag/Ctn</th>
					<th colspan="2">Delivery Kg</th>
					<th colspan="2">Payable Amt</th>
                  </tr>
                </thead>
                <tbody>
...
<tr>
   <td colspan="1"><?=$i+1?></td>
  <td colspan="3"><? echo $items->item_name.'-'.$items->finish_goods_code; if($unit_price[$i]<1) echo ' <b>[Gift Item]</b>';?>
...
while($qdata=mysql_fetch_object($gquery)){if($g==0) echo '<b>  [Offer-'.$qdata->offer_name; else echo '/'.$qdata->offer_name; $g++; }if($g>0) echo ']</b>';?></td>
   <td colspan="2"><?=number_format($unit_price[$i]*$fit_size,2);?></td>
   <td colspan="2"><?=number_format($ord_unit[$i],2);?></td>
	<td colspan="2"><? $pkg_ctn=number_format(($ord_unit[$i]/$items->pakg_ctn_size),2);echo ($unit_price[$i]!=0)?$pkg_ctn:''?></td>
    <td colspan="2"><? echo number_format($item_ex[$i],2);$tttttd = $tttttd + $item_ex[$i];?></td>
	<td colspan="2"><? $ttt =($unit_price[$i]*$fit_size)*$item_ex[$i]; echo number_format($ttt,2); $tot = $tot + $ttt; ?></td>
                  </tr> 
				    <? }?>
					...
					<tr>
					 ...
					 <td colspan="2"><?=$tttttd?></td>
					  <td colspan="2"><?=number_format($tot,2)?></td>
					</tr> 
					<tr>
					 <td colspan="8"></td>
					 <td colspan="2"><b>Commission</b></td>
					 <td colspan="2"><?= $cash_discountm?>%</td>
					 <td colspan="8"><?php $dis=number_format((($tot*$cash_discountm)/100),2); echo $dis;?></td>

					 </tr>
					 <tr>
					 <td colspan="10"></td>
					 <td colspan="2"><b>Net Payable Amount :</b></td>
					 <td colspan="2"><?=number_format($tot-(($tot*$cash_discountm)/100),2)?></td>
