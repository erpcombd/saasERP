...
case 1:
		...
		$report="Delivery Challan Summary ";
...
case 353465346:
		...
		$report="Sales Order Maturity Report";
    ...
    <thead>
      ...
      <tr>
        ...
        <td style="border:0px;" colspan="10"><div class="header">
            ...
            <h1>ZISAN FOOD AND BEVERAGE LIMITED</h1>
...
while($dataw=mysql_fetch_object($queryw)){
...
$queryaa=mysql_query($sqleaa);

while($dataa=mysql_fetch_object($queryaa)){
...
$querya=mysql_query($sqlea);

while($datad=mysql_fetch_object($querya)){?>
  ...
  <table width="100%" cellspacing="0" cellpadding="2" border="0">
    ...
    <thead>
      ...
      <tr>
        ...
        <td style="border:0px;" colspan="8"><?=$str?>
...
<?php 
 ...
 $sql='select * from sale_do_chalan where chalan_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'" and status="COMPLETED" group by chalan_no ';
$query=mysql_query($sql);
while($data=mysql_fetch_object($query)){
?>
<tr>
	<td><?=++$i?></td>
	<td><a href="chalan_bill_corporate1.php?v_no=<?=$data->chalan_no?>" target="_blank"><?=$data->chalan_no_another?></a></td>
	<td><?=$data->do_no?></td>
	...
	<td style="text-align:right;"><?=number_format($vat=find_a_field('secondary_journal','sum(cr_amt)',' jv_date between  "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'" and ledger_id="2002000200000000" and tr_from="Sales" and tr_no="'.$data->chalan_no.'"'),2);?></td>
	<td style="text-align:right;"><?=number_format($transport=find_a_field('secondary_journal','sum(cr_amt)',' jv_date between  "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'" and ledger_id="4017002900000000" and tr_from="Sales" and tr_no="'.$data->chalan_no.'"'),2);?></td>
...
<?php 
 ...
 $sql='select * from sale_do_chalan where bill_maturity_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'" and status="COMPLETED" group by chalan_no ';
$query=mysql_query($sql);
while($data=mysql_fetch_object($query)){
?>
<tr>
...
if(isset($area_id)) 		{$area_con=' and d.area_code="'.$area_id.'"';}
   ...
   $sqls="select c.chalan_no,m.do_no,c.driver_name as serial_no,c.chalan_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,a.AREA_NAME as area,    z.ZONE_NAME as zone, r.BRANCH_NAME as region, w.warehouse_name as depot,sum(c.total_amt) as total_amt from sale_do_master m,sale_do_chalan c,dealer_info d  , warehouse w, area a, zon z, branch r where  m.do_no=c.do_no and m.dealer_code=d.dealer_code and w.warehouse_id=c.depot_id   ".$depot_con.$date_con.$pg_con.$dealer_con.$dealer_type_con.$area_con.$zone_con.$region_con." group by chalan_no order by c.chalan_no";
...
<thead><tr><td style="border:0px;" colspan="13">'.$str.'</td></tr><tr><th>S/L</th><th>Challan No</th><th>SO No</th><th>Driver Name</th><th>Challan Date</th><th>Customer Name</th><th>Divition</th><th>Depot</th><th>Sales Total</th><th>Dis</th><th>Actual Sales</th></tr></thead>
...
$sqld1 = 'select sum(total_amt) as total_amount from sale_do_chalan  where chalan_no='.$data->chalan_no.' and unit_price<0 ';
...
$tot_chalan_amount=find_a_field('sale_do_chalan','sum(total_amt)','chalan_no='.$data->chalan_no);
...
?>
  ...
  <tr>
    ...
    <td><a href="chalan_bill_corporate1.php?v_no=<?=$data->chalan_no?>" target="_blank">
    ...
    <td style="text-align:right;"><?=number_format($tot_chalan_amount,2)?></td>
...
$gr_tot_ch+=$tot_chalan_amount;
$gr_tot_ch2+=$tot_chalan_amount;
}
...
?>
  ...
  <tr class="footer">
    ...
    <td style="text-align:right;" ><?=number_format($gr_tot_ch,2)?></td>

    <td></td>

    <td style="text-align:right;" ><?=number_format($gr_tot_ch2,2)?></td>
...
if(isset($area_id)) 		{$area_con=' and d.area_code="'.$area_id.'"';}
 ...
 $sqls="select c.chalan_no,m.do_no,c.memo_no as memo_no,c.chalan_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name, a.AREA_NAME as area,w.warehouse_name as depot,sum(total_amt) as total_amt from 
...
<thead><tr><td style="border:0px;" colspan="11">'.$str.'</td></tr><tr><th>S/L</th><th>Chalan No</th><th>Do No</th><th>Memo No</th><th>Chalan Date</th><th>Dealer Name</th><th>Area</th><th>Depot</th><th>Sales Total</th><th>Dis</th><th>Actual Sales</th></tr></thead>
