...
$f_date=${'f_mons'.$i};
		...
		$report="Yearly Sales Report (Item Wise in KG)";
...
$f_date=${'f_mons'.$i};
		...
		$report="Yearly Sales Report (Item Wise in TK)";
...
$sqlmon0 = mysql_fetch_row(mysql_query("select sum(c.total_unit), c.unit_price from sale_do_chalan c, dealer_info d, warehouse w where d.dealer_code=c.dealer_code and c.depot_id=w.warehouse_id  and c.unit_price>0 and c.chalan_date between '".$f_mons0."' and '".$t_date."' and c.item_id=".$item->item_id.$pg_con.$depot_con));



$sqlmon1 = mysql_fetch_row(mysql_query("select sum(c.total_unit), c.unit_price from sale_do_chalan c, dealer_info d, warehouse w where d.dealer_code=c.dealer_code and c.depot_id=w.warehouse_id and c.unit_price>0 and c.chalan_date between '".$f_mons1."' and '".$f_mone1."' and c.item_id=".$item->item_id.$pg_con.$depot_con));



$sqlmon2 = mysql_fetch_row(mysql_query("select sum(c.total_unit), c.unit_price from sale_do_chalan c, dealer_info d, warehouse w where d.dealer_code=c.dealer_code and c.depot_id=w.warehouse_id and  c.unit_price>0 and c.chalan_date between '".$f_mons2."' and '".$f_mone2."' and c.item_id=".$item->item_id.$pg_con.$depot_con));



$sqlmon3 = mysql_fetch_row(mysql_query("select sum(c.total_unit), c.unit_price from sale_do_chalan c, dealer_info d, warehouse w where d.dealer_code=c.dealer_code  and c.depot_id=w.warehouse_id and c.unit_price>0 and c.chalan_date between '".$f_mons3."' and '".$f_mone3."' and c.item_id=".$item->item_id.$pg_con.$depot_con));
...
$sql = "select item_id, item_name from item_info where product_nature='Salable'  order by item_id";
...
$sqql = "select sum(c.total_unit) from sale_do_chalan c,dealer_info d,area a, zon z, branch r where c.chalan_date between '".${'f_mons'.$i}."' and '".${'f_mone'.$i}."' and c.dealer_code=d.dealer_code and d.dealer_type='Distributor' and c.unit_price>0  and d.area_code=a.AREA_CODE and z.ZONE_CODE=a.ZONE_ID and r.BRANCH_ID=z.REGION_ID and c.item_id='".$item_id."'".$con;
...
?>
    ...
    <td bgcolor="#FFFF99"><div align="right"><strong>
        ...
        <? $totalallr= $totalallr + ${'totalr'.$item_id};echo number_format(${'totalr'.$item_id},2)?>
...
$sqql = "select sum(c.total_amt) from sale_do_chalan c,dealer_info d,area a, zon z, branch r where c.chalan_date between '".${'f_mons'.$i}."' and '".${'f_mone'.$i}."' and c.dealer_code=d.dealer_code and d.dealer_type='Distributor' and c.unit_price>0  and d.area_code=a.AREA_CODE and z.ZONE_CODE=a.ZONE_ID and r.BRANCH_ID=z.REGION_ID and c.item_id='".$item_id."'".$con;
