      ...
      <div class="oe_view_manager_view_list">
        <div class="oe_list oe_view">
          <table width="100%" height="35px;" border="0" cellpadding="0" cellspacing="0" bordercolor="#000000">
            <tbody>
              <tr>
                <td width="8%" bordercolor="#333333" bgcolor="#99CCCC"><div align="right" style="font-size:14px;">Date Interval:</div></td>
                <td width="39%" bordercolor="#333333" bgcolor="#99CCCC"><input name="from_date" type="text" id="from_date" value="<?=($_REQUEST['from_date']=='')?date('Y-m-01'):$_REQUEST['from_date'];?>" />
                  &nbsp;--&nbsp;
                  <input name="to_date" type="text" id="to_date" value="<?=($_REQUEST['to_date']=='')?date('Y-m-d'):$_REQUEST['to_date'];?>" /></td>
                <td width="8%" bordercolor="#333333" bgcolor="#99CCCC"><div align="right" style="font-size:14px;">Team Name:&nbsp;</div></td>
                <td width="11%" bordercolor="#333333" bgcolor="#99CCCC"><select name="team_name"id="team_name">
                    <option value="">Select Team</option>
                    <?php foreign_relation('teams','team_name','team_name',$_POST['team_name']);?>
                  </select></td>
                <!--<td width="8%" bordercolor="#333333" bgcolor="#99CCCC"><div align="right" style="font-size:14px;">Region Name: </div></td>
                <td width="12%" bordercolor="#333333" bgcolor="#99CCCC"><select name="select"id="select">
                    <option value="">Select Region</option>
                    <?php foreign_relation('branch','BRANCH_ID','BRANCH_NAME',$_POST['BRANCH_NAME']);?>
                  </select></td>-->
                <td width="14%" bordercolor="#333333" bgcolor="#99CCCC"><input name="show" type="submit" class="style4" id="show" style="width:150px; height:30px; color:#339966; font-weight:bold; font-size:15px;" value="Show Report" /></td>
              </tr>
            </tbody>
          </table>
        </div>
...
while ($region = mysql_fetch_object($data)) {
$region_net_sales=0;
$region_order=0;
$region_ims=0;
$region_memo=0;
...
$sql1 = "select * from zon where REGION_ID=".$region->BRANCH_ID;
$data1 = mysql_query($sql1);			
while ($zone = mysql_fetch_object($data1)) {
///////////////////////////////////////////////////////////////////////
...
while ($area = mysql_fetch_object($data2)) {
...
$memo_data=mysql_fetch_object(mysql_query($memo_sql));
...
$order_sale_data=mysql_fetch_object(mysql_query($order_sale_sql));
...
$sales_return_data=mysql_fetch_object(mysql_query($sales_return_sql));
...
$sales_remitence_data=mysql_fetch_object(mysql_query($sales_remitence_sql));
...
//////////////////////////////////////////////////////////////////////
      ...
      <tr align="center" valign="middle" bgcolor="#0099FF">
        <td><?=$pi?></td>
        <td>REGION:
          <?=$region_incharge->PBI_NAME?></td>
        <td><?=$region_designation?></td>
        <td><strong>REGION:</strong>
          <?=$region->BRANCH_NAME?></td>
        <td>&nbsp;</td>
        <td><?=$info2->order_no?></td>
        <td><?=$region_order?></td>
        <td><?=$region_ims?></td>
