...
if($_REQUEST['vehicle']!='') 		$vehicle=$_REQUEST['vehicle'];
...
switch ($_REQUEST['report']) {
    case 1:
	$report="Driver Attendance Report";
	break;
...
$report="Delivery Order Brief Report (Region Wise)";
	break;
		case 6:
	    $report = 'Vehicle Log Book';
	    break;
		
		case 7:
	    $report = 'Fuel/Gasoline Continous Cost Calculation Sheet';
	    break;
		
		case 6:
	    $report = 'Fuel/Gasoline Continues Cost Calculation Sheet';
	    break;
...
<?
		...
		if(isset($vehicle)) 
		$str 	.= '<h3>Vehicle: '.find_a_field('vehicle_info','vehicle_name','vehicle_id='.$vehicle).'</h3>';
		if(isset($user_id)) 
		$str 	.= '<h3>Name: '.find_a_field('user_activity_management','fname','user_id='.$user_id).'</h3>';				
		if(isset($dealer_type)) 
...
if($_POST['f_date']!='') $date_con = ' and a.access_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'"';
if($_POST['user_id']!='') $user_con = ' and a.user_id = "'.$_POST['user_id'].'"';
$sql='select a.*,u.fname from ss_location_log a, user_activity_management u where a.user_id=u.user_id and a.user_id = "'.$_POST['user_id'].'" '.$date_con.' group by a.access_date,a.user_id';
$query = db_query($sql); ?>
...
$in_time = find_a_field('ss_location_log','access_time','user_id="'.$data->user_id.'" and access_date="'.$data->access_date.'" and attendance_type="IN TIME"');
$out_time = find_a_field('ss_location_log','access_time','user_id="'.$data->user_id.'" and access_date="'.$data->access_date.'" and attendance_type="OUT TIME"');

$from_time = date('H:i:s',strtotime($in_time));
$to_time = date('H:i:s',strtotime($out_time));

$time1 = new DateTime($from_time);
$time2 = new DateTime($to_time);
...
<td><?=date('H:i:s',strtotime($out_time))?></td>
<td><?=$diff?></td>
<td>10:00</td>
<td><?=$overtime?></td>
<td><?=$overtime?></td>
...
<tr>
  <td colspan="6"><strong>Total Overtime</strong></td>
  <td><strong><?=$tot_overtime?></strong></td>
  <td></td>
  <td></td>
</tr>
</tbody>
...
<?
if($_POST['f_date']!='') $date_con = ' and m.expense_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'"';
$sql='select m.*,d.*,i.expense_name,u.fname from fuel_expense m, fuel_expense_detail d, expense_head i, user_activity_management u where m.entry_by=u.user_id and i.id=d.item_id and m.expense_no=d.expense_no '.$date_con.'';
$query = db_query($sql);
...
<tr>
<td><?=$s?></td>
<td><?='Purchased '.$data->expense_name.' From '.$data->vendor?></td>
<td><?=$data->expense_date?></td>
<td><?=$data->amount;$total_amt +=$data->amount?></td>
<td><?=$data->fname?></td>
<td><a href="../../../vehicle_mod/<?=$data->att_file?>" target="_blank">View</a></td>
</tr>
<? } ?>
<tr>
...
<?
//@b := @b + d.mileage - d.mileage AS balance
//(SELECT @b := 0.0) AS dummy CROSS JOIN
if($_POST['user_id']>0) $driver_con = ' and d.entry_by="'.$_POST['user_id'].'"';
if($_POST['vehicle']>0) $vehicle_con = ' and d.vehicle="'.$_POST['vehicle'].'"';
if($_POST['f_date']!='') $date_con = ' and d.expense_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'"';
$sql='select m.*,d.*,i.expense_name,u.fname from fuel_expense_detail d,fuel_expense m, expense_head i, user_activity_management u where m.entry_by=u.user_id and i.id=d.item_id and m.expense_no=d.expense_no '.$date_con.$driver_con.$vehicle_con.' order by d.id asc';
$query = db_query($sql);

while($data=mysqli_fetch_object($query)){$s++;
$start_km = find_a_field('fuel_expense_detail','mileage','vehicle="'.$data->vehicle.'" and id<'.$data->id.' order by id desc');
$per_km = $data->mileage-$start_km;
?>
<tr>
<td><?=$s?></td>
<td><?=$data->expense_date?></td>
...
<td><?=$start_km;?></td>
<td><?=$data->mileage;?></td>
<td><?=$per_km;$tot_km+=$per_km;?></td>
<td><?=number_format($data->amount/$per_km,2)?></td>
<td></td>
<td><a href="../../../vehicle_mod/<?=$data->att_file?>" target="_blank">View</a></td>
</tr>
<? } ?>
...
if($_POST['vehicle']>0) $vehicle_con = ' and d.vehicle="'.$_POST['vehicle'].'"';
if($_POST['f_date']!='') $date_con = ' and d.expense_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'"';
$sql='select d.*,t.driver_name,v.vehicle_name from tool_maintenance_detail d,tools t,vehicle_info v where d.item_id=t.driver_id and d.vehicle=v.vehicle_id';
$query = db_query($sql);
...
<th colspan="2"><div align="center">Meter Reading (KM)</div></th>
<th rowspan="2">Net KM</th>
<th rowspan="2">Fuel/Gasoline</th>
<th colspan="3"><div align="center">Duty</div></th>
<th rowspan="2">User Name</th>
<th rowspan="2">User Sign</th>
<th rowspan="2">Driver's Sign</th>
<th rowspan="2">Authorized Sign</th>
...
<?
if($_POST['f_date']!='') $date_con = ' and d.log_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'"';
if($_POST['user_id']>0) $driver_con = ' and d.entry_by="'.$_POST['user_id'].'"';
if($_POST['vehicle']>0) $driver_con .= ' and d.vehicle="'.$_POST['vehicle'].'"';
 $sql='select d.*,u.fname,v.vehicle_name from vehicle_log_book d left join vehicle_info v on v.vehicle_id=d.vehicle left join user_activity_management u on u.user_id=d.vehicle_user where 1 '.$date_con.$driver_con.' order by d.log_date asc';
$query = db_query($sql);

while($data=mysqli_fetch_object($query)){$s++;
$total_km +=$data->net_km;
$in_time = find_a_field('ss_location_log','access_time','user_id="'.$data->entry_by.'" and access_date="'.$data->log_date.'" and attendance_type="IN TIME"');
$out_time = find_a_field('ss_location_log','access_time','user_id="'.$data->entry_by.'" and access_date="'.$data->log_date.'" and attendance_type="OUT TIME"');
$in_time = date('H:i:s',strtotime($in_time));
$out_time = date('H:i:s',strtotime($out_time));


$time2 = new DateTime($out_time);
$time3 = new DateTime($in_time);
$interval = $time2->diff($time3);
$diff = $interval->format('%h');
$overtime = abs($diff-10);

//Faijlami
$take_date = $data->log_date;
if($take_date!=$took_date){
$out_time = '';
$overtime = '';
$diff = '';
}else{
...
elseif($_REQUEST['report']==7) {
 ?>
<table width="100%" cellspacing="0" cellpadding="2" border="0">
<thead><tr><td style="border:0px;" colspan="15"><?=$str?></td></tr>
<tr>
<th>S/L</th>
<th>Date(Fuel/Gasoline Refil)</th>
<th>Fuel Bill</th>
<th>Fuel Ltr.</th>
<th>CMT Ltr</th>
<th>Start KM</th>
...
<th>Used KM</th>
<th>Used CMT KM</th>
<th>Fuel Cost P/KM</th>
<th>Per Ltr Vs KM</th>
<th>CMT P/Ltr Vs. KM</th>
<th>Toll</th>
<th>Other</th>
...
<td><?=$data->mileage?></td>
<td></td>
<td><?=$used_km=$data->mileage-$start_km;$total_km +=$used_km;?></td>
<td></td>
<td><?=number_format($data->amount/$used_km,2)?></td>
<td></td>
<td></td>
<td></td>
<td></td>
...
<?
if($_POST['f_date']!='') $date_con = ' and m.expense_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'"';
$sql='select m.*,d.*,u.fname from office_expense m, office_expense_detail d, user_activity_management u where m.entry_by=u.user_id and m.expense_no=d.expense_no '.$date_con.'';
$query = db_query($sql);
...
{
$sql="select m.do_no,m.do_date,m.entry_time,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,(select AREA_NAME from area where AREA_CODE=d.area_code) as area,w.warehouse_name as depot,d.product_group as grp, m.rcv_amt,concat(m.payment_by,':',m.bank,':',m.remarks) as Payment_Details 
...
$sqlmon0 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons0."' and '".$t_date."' and c.item_id=".$item->item_id.$pg_con));

$sqlmon1 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons1."' and '".$f_mone1."' and c.item_id=".$item->item_id.$pg_con));

$sqlmon2 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons2."' and '".$f_mone2."' and c.item_id=".$item->item_id.$pg_con));

$sqlmon3 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons3."' and '".$f_mone3."' and c.item_id=".$item->item_id.$pg_con));
...
where i.item_brand!='Promotional' and i.sales_item_type!='' ".$item_con.$item_brand_con.$pg_con.' group by i.finish_goods_code';
	...
	$query = db_query($sql);
	while($item=mysqli_fetch_object($query)){
$sqlmon0 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons0."' and '".$t_date."' and c.item_id=".$item->item_id.$pg_con));
$sqlmon1 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons1."' and '".$f_mone1."' and c.item_id=".$item->item_id.$pg_con));
$sqlmon2 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons2."' and '".$f_mone2."' and c.item_id=".$item->item_id.$pg_con));
$sqlmon3 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c, dealer_info d where d.dealer_code=c.dealer_code and c.chalan_date between '".$f_mons3."' and '".$f_mone3."' and c.item_id=".$item->item_id.$pg_con));
...
$sqlmon0 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c,dealer_info d, area a where c.chalan_date between '".$f_mons0."' and '".$t_date."' and c.dealer_code=d.dealer_code and d.area_code=a.AREA_CODE and a.ZONE_ID='".$zone_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon1 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c,dealer_info d, area a where c.chalan_date between '".$f_mons1."' and '".$f_mone1."' and c.dealer_code=d.dealer_code and d.area_code=a.AREA_CODE and a.ZONE_ID='".$zone_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon2 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c,dealer_info d, area a where c.chalan_date between '".$f_mons2."' and '".$f_mone2."' and c.dealer_code=d.dealer_code and d.area_code=a.AREA_CODE and a.ZONE_ID='".$zone_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon3 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c,dealer_info d, area a where c.chalan_date between '".$f_mons3."' and '".$f_mone3."' and c.dealer_code=d.dealer_code and d.area_code=a.AREA_CODE and a.ZONE_ID='".$zone_code."' and c.item_id=".$item_info->item_id.$pg_con));
...
$sql = "select * from zon where REGION_ID='".$region_id."' order by ZONE_NAME";
	...
	while($item=mysqli_fetch_object($query)){
 $zone_code = $item->ZONE_CODE;
$sqlmon0 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c,dealer_info d, area a where c.chalan_date between '".$f_mons0."' and '".$t_date."' and c.dealer_code=d.dealer_code and d.area_code=a.AREA_CODE and a.ZONE_ID='".$zone_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon1 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c,dealer_info d, area a where c.chalan_date between '".$f_mons1."' and '".$f_mone1."' and c.dealer_code=d.dealer_code and d.area_code=a.AREA_CODE and a.ZONE_ID='".$zone_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon2 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c,dealer_info d, area a where c.chalan_date between '".$f_mons2."' and '".$f_mone2."' and c.dealer_code=d.dealer_code and d.area_code=a.AREA_CODE and a.ZONE_ID='".$zone_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon3 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c,dealer_info d, area a where c.chalan_date between '".$f_mons3."' and '".$f_mone3."' and c.dealer_code=d.dealer_code and d.area_code=a.AREA_CODE and a.ZONE_ID='".$zone_code."' and c.item_id=".$item_info->item_id.$pg_con));
...
$sql = "select d.dealer_code, d.dealer_name_e, a.area_name from dealer_info d, area a where d.area_code=a.AREA_CODE and ZONE_ID='".$zone_id."' order by d.dealer_name_e";
	...
	while($item=mysqli_fetch_object($query)){
 $zone_code = $item->ZONE_CODE;
$sqlmon0 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c,dealer_info d where c.chalan_date between '".$f_mons0."' and '".$t_date."' and c.dealer_code=d.dealer_code and d.dealer_code='".$item->dealer_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon1 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c,dealer_info d where c.chalan_date between '".$f_mons1."' and '".$f_mone1."' and c.dealer_code=d.dealer_code and d.dealer_code='".$item->dealer_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon2 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c,dealer_info d where c.chalan_date between '".$f_mons2."' and '".$f_mone2."' and c.dealer_code=d.dealer_code and d.dealer_code='".$item->dealer_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon3 = mysql_fetch_row(db_query("select sum(c.pkt_unit) from sale_do_chalan c,dealer_info d where c.chalan_date between '".$f_mons3."' and '".$f_mone3."' and c.dealer_code=d.dealer_code and d.dealer_code='".$item->dealer_code."' and c.item_id=".$item_info->item_id.$pg_con));
...
$zone_code = $item->ZONE_CODE;
$sqlmon0 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c,dealer_info d where c.chalan_date between '".$f_mons0."' and '".$t_date."' and c.dealer_code=d.dealer_code and d.dealer_code='".$item->dealer_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon1 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c,dealer_info d where c.chalan_date between '".$f_mons1."' and '".$f_mone1."' and c.dealer_code=d.dealer_code and d.dealer_code='".$item->dealer_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon2 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c,dealer_info d where c.chalan_date between '".$f_mons2."' and '".$f_mone2."' and c.dealer_code=d.dealer_code and d.dealer_code='".$item->dealer_code."' and c.item_id=".$item_info->item_id.$pg_con));

$sqlmon3 = mysql_fetch_row(db_query("select sum(c.total_amt) from sale_do_chalan c,dealer_info d where c.chalan_date between '".$f_mons3."' and '".$f_mone3."' and c.dealer_code=d.dealer_code and d.dealer_code='".$item->dealer_code."' and c.item_id=".$item_info->item_id.$pg_con));
...
${'sqlmon'.$i} = mysql_fetch_row(db_query($sqql));
...
${'sqlmonc'.$i} = mysql_fetch_row(db_query($sqql));
...
${'sqlmons'.$i} = mysql_fetch_row(db_query($sqql));
...
$sql = "select z.*,b.BRANCH_NAME from zon z,branch b where 1 and BRANCH_ID=REGION_ID ".$region_con." order by BRANCH_NAME,ZONE_NAME";
	...
	while($item=mysqli_fetch_object($query)){
 $zone_code = $item->ZONE_CODE;
$sqlmon = mysql_fetch_row(db_query("select sum(c.total_amt),sum(c.total_unit),c.pkt_size from sale_do_chalan c,dealer_info d, area a where c.chalan_date between '".$f_date."' and '".$t_date."' and d.dealer_type='Distributor' and c.dealer_code=d.dealer_code and d.area_code=a.AREA_CODE and a.ZONE_ID='".$zone_code."' and c.item_id=".$item_id));
