...
$con.=' and a.mobile_no = "'.$_POST['mobile_no'].'"';
         ...
         $sql="select dealer_code as Customer_code,account_code,dealer_name_e as customer_name,mobile_no as mobile,zone_name as zone,region_name as region,area_name as area from dealer_info where 1";
		break;
...
<form action="" method="post">
  ...
  <div class="main">
    <?
	$proj_name= find_a_field('project_info','proj_name','1');
		//echo $sql;
		$str 	.= '<div class="header">';
		if(isset($_SESSION['company_name'])) 
		$str 	.= '<h2 style="font-size:24px;">'.$proj_name.'</h2>';
...
<?
$s=0;
while($datas=mysql_fetch_object($query)){$s++;
?>
...
<table width="100%" cellspacing="0" cellpadding="2" border="0">
      ...
      <?php 
	  $result=mysql_query("Select distinct DEPT_ID,DEPT_DESC from department order by custom_serial");
	  while($row=mysql_fetch_array($result)){
		  


$results = mysql_query("SELECT SUM(gross_salary) AS gross_salary FROM salary_attendence where PBI_DEPARTMENT='$row[DEPT_ID]'"); 
$rows = mysql_fetch_array($results); 
$sum = $rows['gross_salary'];
$grosstot=$grosstot+$sum;

$resultss = mysql_query("SELECT SUM(total_payable) AS total_payable FROM salary_attendence where PBI_DEPARTMENT='$row[DEPT_ID]'"); 
$rowss = mysql_fetch_array($resultss); 
$sums = $rowss['total_payable'];
 
$resulttax = mysql_query("SELECT SUM(income_tax) AS income_tax FROM salary_attendence where PBI_DEPARTMENT='$row[DEPT_ID]'"); 
$taxrow = mysql_fetch_array($resulttax);
$totalincometax=$totalincometax+$taxrow[income_tax];

$resultadvance = mysql_query("SELECT SUM(advance_install) AS advance_install FROM salary_attendence where PBI_DEPARTMENT='$row[DEPT_ID]'"); 
$advancerow = mysql_fetch_array($resultadvance);	
...
$ttotals=$ttotals+$ttotal;		  
    ...
    <table width="100%" cellspacing="0" cellpadding="2" border="0">
      ...
      <?php 
	  $result=mysql_query("Select * from essential_info where ESS_BANK_ACC_NO='' order by PBI_ID");
	  while($row=mysql_fetch_array($result)){
		  


$perresult = mysql_query("SELECT * from personnel_basic_info where PBI_ID='$row[PBI_ID]'"); 
$prerow = mysql_fetch_array($perresult); 


$results = mysql_query("SELECT SUM(gross_salary) AS gross_salary FROM salary_attendence where PBI_ID='$row[PBI_ID]'"); 
$rows = mysql_fetch_array($results); 
$sum = $rows['gross_salary'];
$grosstot=$grosstot+$sum;

$resultss = mysql_query("SELECT SUM(total_payable) AS total_payable FROM salary_attendence where PBI_ID='$row[PBI_ID]'"); 
$rowss = mysql_fetch_array($resultss); 
$sums = $rowss['total_payable'];
 
$resulttax = mysql_query("SELECT SUM(income_tax) AS income_tax FROM salary_attendence where PBI_ID='$row[PBI_ID]'"); 
$taxrow = mysql_fetch_array($resulttax);
$totalincometax=$totalincometax+$taxrow[income_tax];

$resultadvance = mysql_query("SELECT SUM(advance_install) AS advance_install FROM salary_attendence where PBI_ID='$row[PBI_ID]'"); 
$advancerow = mysql_fetch_array($resultadvance);	
$totaladvance=$totaladvance+$taxrow[advance_install];	  
