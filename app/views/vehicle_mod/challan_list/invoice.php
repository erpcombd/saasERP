...
<!--<h4 class="text-center"><u>Invoice</u></h4>-->
  ...
  <tbody>
  <?php
 $sql='SELECT s.chalan_no,s.item_id,s.dealer_code,s.unit_price,s.total_unit,s.total_amt,s.chalan_date,s.dealer_code,i.item_name,i.unit_name,s.transport_cost,s.chalan_no_another,s.vat_amount,s.do_no 
...
<td style="font-weight:bold;text-align:right"><?php echo number_format($all_amt=$vat_amt+$total1+$cost,2); ?></td>
<td></td>
</tr>
...
<div class="row">
	...
	$appro_sta=find_a_field('sale_do_chalan','approve_status','chalan_no="'.$challan.'"');
				...
				if($check_sta!='yes'){
					if($_SESSION['user']['id']==10095 || $_SESSION['user']['id']==10103 || $_SESSION['user']['id']==1){
				?>
				<input class="btn btn-info" style="color:white;"  type="submit" name="check" value="Check">
				<?php } } 
				else{
				if($appro_sta!='yes'){
				if($_SESSION['user']['id']==10100 || $_SESSION['user']['id']==10140 || $_SESSION['user']['id']==1){
				?>
