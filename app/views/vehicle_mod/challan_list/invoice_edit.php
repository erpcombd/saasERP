...
while($row2=mysql_fetch_object($queryc)){
$item_idc=$_POST['item_id_'.$row2->id];
$unit_pricec=$_POST['unit_price_'.$row2->id];
$tot_amtc=$_POST['tot_amt_id_'.$row2->id];
$update_chsql='update sale_do_chalan set unit_price="'.$unit_pricec.'",total_amt="'.$tot_amtc.'",vat_amount="'.$vat.'",transport_cost="'.$transport.'" where item_id="'.$item_idc.'" and chalan_no="'.$row2->chalan_no.'"';
mysql_query($update_chsql);
///update journal item.//
...
<form action="" method="post">
  ...
  <tbody>
  <?php
 $sql='SELECT s.id,s.chalan_no,s.item_id,s.dealer_code,s.unit_price,s.total_unit,s.total_amt,s.chalan_date,s.dealer_code,i.item_name,i.unit_name,s.transport_cost,s.chalan_no_another,s.vat_amount 
