...
$title='Unchecked Fuel Expense List';
do_calander('#fdate');
do_calander('#tdate');
...
$target_url = '../fuel_expense/fuel_expense_checking.php';
...
//where m.status in ('PROCESSING') and d.area_code=a.AREA_CODE  and m.dealer_code=d.dealer_code ".$con." and d.dealer_type='Distributor' order by m.do_date,d.dealer_name_e";
 ...
 $res="select c.* from fuel_expense_detail c where c.status='UNCHECKED' ".$con." group by expense_no";
$query = db_query($res);
