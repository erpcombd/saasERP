...
do_calander('#delivery_req_date');
do_calander('#po_date');



do_calander('#do_date');
do_calander('#due_pay_date');
...
if($dealer_code<1){
header("Location:select_dealer_do.php");
}
}
...

{
	...
	$so_file="so-".$_POST['do_no'];
		$file_exten=explode(".",$_FILES['do_file']['name']);
	
$img_file_name=$so_file.".".end($file_exten);
$img_file_name_temp=$_FILES['do_file']['tmp_name'];
move_uploaded_file($img_file_name_temp,"../../../../files_doc/so/$img_file_name");
$_POST['do_file']=$img_file_name;
...
getData2('do_ajax_s.php', 'do',document.getElementById("item").value,document.getElementById("depot_id").value);
}
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
  ...
  <tr>
    ...
    <div>
      ...
      <label style="width:175px;">SO No : </label>
	...
	<div>
			    ...
			    <label style="width:175px;">SO Date : </label>
    ...
    <div>
      ...
      <label style="width:175px;">Customer : </label>
      ...
      <div>
        ...
        <label style="width:175px;">Area : </label>
      ...
      <div>
        ...
        <label style="width:175px;">Zone : </label>
        ...
        <label style="width:175px;">Region : </label>
       ...
       <div>
              <label style="width:175px;"> Corporate Address: </label>
              <input name="address_e" type="text" id="address_e"  value="<? if($address_e!='') echo $address_e; else echo $dealer->address_e?>" />
            </div>
...
<div>
              <label style="width:175px;">File Upload </label>
              <input name="do_file" type="file" id="do_file"  value="<?=$do_file?>" autocomplete="off"/>
            </div>
			<div>
              <label style="width:175px;">View PDF </label>
    ...
    <td width="50%">
			...
			<div>
              <label style="width:175px;">PO No: </label>
              <input name="po_no" type="text" id="po_no"  value="<?=$po_no?>" />
            </div>
			<div>
              <label style="width:175px;">PO Date: </label>
              <input name="po_date" type="text" id="po_date"  value="<?=$po_date?>" autocomplete="off"/>
            </div>
			    ...
			    <label style="width:175px;">Delivery Schedule Date : </label>
			...
			<?php /*?><div>
			  ...
			  <label style="width:162px;">Available Amt : </label>
		...
		<div>
          <label style="width:175px;">Depot : </label>
          <select  id="depot_id" name="depot_id" readonly="readonly">
            <option value="<?=find_a_field('warehouse','warehouse_id','warehouse_id=1')?>">
              <?=find_a_field('warehouse','warehouse_name','warehouse_id=1')?>
              </option>
          </select>
		 <!--<input style="width:155px;"  name="wo_detail" type="text" id="wo_detail" value="<?=$depot_id?>" readonly="readonly"/>-->
			...
			<div>
              <label style="width:175px;">Delivery Address: </label>
              <input name="delivery_address" type="text" id="delivery_address"  value="<? if($delivery_address!='') echo $delivery_address; else echo $dealer->delivery_address?>" />
            </div>
			
			
			<div>
       			 <label style="width:175px;">Advance: </label>
        ...
        <label style="width:175px;">Payment By: </label>
		...
		<option value="Bkash" <?=($payment_by=='Bkash')?'selected':''?>>Bkash</option>
		<option value="Nagad" <?=($payment_by=='Nagad')?'selected':''?>>Nagad</option>
		<option value="Rocket" <?=($payment_by=='Rocket')?'selected':''?>>Rocket</option>
		<option value="Upay" <?=($payment_by=='Upay')?'selected':''?>>Upay</option>
		<option value="Mcash" <?=($payment_by=='Mcash')?'selected':''?>>mCash</option>
		<option value="Scash" <?=($payment_by=='Scash')?'selected':''?>>SureCash</option>
		<option value="Tcash" <?=($payment_by=='Tcash')?'selected':''?>>T-Cash</option>
		<option value="Ok" <?=($payment_by=='Ok')?'selected':''?>>Ok Wallet </option>
		<option value="Due" <?=($payment_by=='Credit')?'selected':''?>>Credit </option>
	   		...
	   		<!--<div>
			    ...
			    <label style="width:162px;">Due Payment Date :</label>

			    <input name="due_pay_date" type="text" id="due_pay_date" value="<?=($due_pay_date!='')?$due_pay_date:date('Y-m-d')?>" />
  ...
  <tr>
	...
	<? if($dealer->canceled=='Yes'|| $dealer->canceled=='' ){?>



		<div class="buttonrow text-center" ><span class="buttonrow" >
		  <? if($$unique_master>0) {?>
...
<table   border="1" align="left"  style="border-collapse:collapse; border:1px solid #caf5a5;width:1100px;" cellpadding="0" cellspacing="2">
...
<input name="depot_id" type="hidden" id="depot_id" value="1"/>
<input name="flag" id="flag" type="hidden" value="1" />
</span>
...
<td bgcolor="#CCCCCC">
 ...
 <tr>
  ...
  <tr>
<td><input name="item2" type="text" class="input3" id="item2"  style="width:230px;margin-right: -50px;" required="required" tabindex="3" value="<?=$item_all->item_name?>" onfocus="focuson('dist_unit')"/></td>
    ...
    <tr>
     <input name="pkt_unit" type="hidden" value="0" readonly class="input3" id="pkt_unit" style="width:55px;" onkeyup="count()" required="required"  tabindex="4"/>
	  ...
	  <td><input name="dope_qty" type="text" value="" class="input3" id="dope_qty" style="width:55px;" readonly /></td>
	   <td><input name="sec_unit" type="text" value="" class="input3" id="sec_unit" style="width:51px;" readonly /></td>
...
<? 
$res='select a.id,b.finish_goods_code as code,b.item_name,a.in_stock, b.unit_name as unit,a.dist_unit as qty ,a.unit_price as price,a.dope_qty as sec_qty,a.sec_unit as sec_unit,a.total_amt,"X"
...
<table style="cursor:pointer" width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td><div class="tabledesign2">
	     <span id="codzList">
        <? 
             echo link_report_add_del_auto($res,'',10);
		?>
         </span>
      </div></td>
...
<form action="select_dealer_do.php" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
	...
	<td class="text-right"  width="80%">VAT Amount(Tk)</td>
	<td ><input style="width:80%" class="text-right" type="text" name="vat_amt_tk" id="vat_amt_tk" /></td>
	</tr>
...
<table style="margin:0 auto;"  width="80%" border="0">
  <tr>
      <td >
      <input name="delete"  type="submit" class="btn btn-danger" value="DELETE DO" style="width:270px;height:37px;" />
     <input  name="do_no" type="hidden" id="do_no" value="<?=$$unique_master?>"/></td><td align="right" style="text-align:right">
      <input name="confirm" type="submit" class="btn btn-success" value="CONFIRM AND SEND WORK ORDER" style="width:270px;height:37px; " />
      </td>
