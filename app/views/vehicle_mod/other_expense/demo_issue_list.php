...
$target_url = '../other_issue/demo_view.php';
...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and c.oi_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
...
//where m.status in ('PROCESSING') and d.area_code=a.AREA_CODE  and m.dealer_code=d.dealer_code ".$con." and d.dealer_type='Distributor' order by m.do_date,d.dealer_name_e";
 ...
 $res="select c.* from warehouse_other_issue c where c.issue_type='Damage Issue' ".$con."";
$query = mysql_query($res);
while($data = mysql_fetch_object($query))
{
?>
<tr <?=($data->oi_no>0)?'style="background-color:#FFCCFF"':'';?>>
<td><?=++$i?></td>
<td onClick="custom(<?=$data->oi_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->oi_no;?></td>
<td onClick="custom(<?=$data->oi_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->oi_date;?></td>
<td onClick="custom(<?=$data->oi_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=find_a_field('warehouse','warehouse_name','warehouse_id="'.$data->warehouse_id.'"')?></td>
<td onClick="custom(<?=$data->oi_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->status;?></td>
<td onClick="custom(<?=$data->oi_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=find_a_field('user_activity_management','fname','user_id="'.$data->entry_by.'"');?></td>
<td onClick="custom(<?=$data->oi_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->entry_at;?></td>
