...
$title='Fuel Expense Status';
do_calander('#fdate');
do_calander('#tdate');
$table_master='expense_status.php';
$unique_master='expense_no';

$table_detail='fuel_expense_detail';
$unique_detail='id';

$table_chalan='fuel_expense_detail';
$unique_chalan='id';
...
$target_url = '../tools/fuel_expense_print_view.php';
...
if($_POST['status']!='') $con .=' and m.status="'.$_POST['status'].'"';
...
$res="select c.*,m.*,c.expense_date as expense_date from tool_maintenance_detail c, tool_maintenance m where m.expense_no=c.expense_no ".$con." group by c.expense_no";
$query = db_query($res);
...
<td  <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=find_a_field('user_activity_management','fname','user_id="'.$data->checked_by.'"');?></td>
<td  <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->checked_at;?></td>
<td  <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=find_a_field('user_activity_management','fname','user_id="'.$data->approved_by.'"');?></td>
<td  <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->approved_at;?></td>
