...
$title='Approve Tools Maintenance List';
do_calander('#fdate');
do_calander('#tdate');
...
$target_url = '../tools/fuel_expense_approving.php';
...
//where m.status in ('PROCESSING') and d.area_code=a.AREA_CODE  and m.dealer_code=d.dealer_code ".$con." and d.dealer_type='Distributor' order by m.do_date,d.dealer_name_e";
 ...
 $res="select c.* from tool_maintenance_detail c where c.status='CHECKED' ".$con." group by expense_no";
$query = db_query($res);
