...
do_calander('#fdate');
do_calander('#tdate');
$title='Vehicle Requisition Status';
...
$con=' and r.from_date= "'.$_POST['fdate'].'"';
}
  $sql='select r.*,t.type,u.fname from vehicle_requisition r,vehicle_type t,user_activity_management u where r.vehicle_type=t.id and r.entry_by=u.user_id '.$con.' order by r.from_date DESC';
$query=db_query($sql);
while($data=mysqli_fetch_object($query)){
?>
                      <tr onclick='window.location.href="requisition_details.php?id=<?=$data->req_no?>"'>
                        <td><?=$data->req_no ?></td>
                        <td><?=$data->type ?></td>
