...
<?php 
	$day_check=find_a_field('sale_do_chalan','count(id)','chalan_no="'.$data->chalan_no.'" and bill_maturity_date>0');
		if($day_check>0){
?>
<a target="_blank" href="due_maturity_edit.php?v_no=<?=$data->chalan_no ?>" style="color:#FFFFFF;"><button class="btn btn-danger btn-sm">Update Done</button></a>
<?php }  else { ?>
<a target="_blank" href="due_maturity_edit.php?v_no=<?=$data->chalan_no ?>" style="color:#FFFFFF;"><button class="btn btn-success btn-sm">Update Due Maturity</button></a>
<?php } ?>
