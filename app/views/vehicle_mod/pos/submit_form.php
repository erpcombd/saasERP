...
if(isset($_POST['date'])){
	...
	$done_i = mysql_query($iquery);
	
	$latest_pos_info = mysql_fetch_object(mysql_query("select * from sale_pos_master where entry_by  = '".$entry_by."' order by pos_id desc limit 0, 1"));
	...
	while($data = mysql_fetch_assoc($rr_query )){
		extract($data);
		$ji_date = $pos_date;
		$item_in = 0;
		$item_ex = $qty;
