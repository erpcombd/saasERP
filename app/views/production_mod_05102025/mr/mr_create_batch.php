...
$title='New STR Create (BATCH Wise)';
...
$batch_info=find_all_field('batch_master',' ','batch_no='.$_POST['bom_no']);
$_POST['req_qty']=$batch_info->batch_qty;
$_POST['group_for']=$batch_info->group_for;

        //$_POST['bom_no']=$_POST['bom_no'];
...
$bom_item= "SELECT id,item_id,batch_cal_qty FROM batch_raw_material WHERE batch_no='".$_POST['bom_no']."'";
$q=db_query($bom_item);			  
while($r=mysqli_fetch_object($q)){

$stock=find_a_field('journal_item','sum(item_in-item_ex)','warehouse_id='.$_POST['req_for'].' and item_id='.$r->item_id.' ');
$rate = find_a_field('journal_item','final_price',' final_price > 0 and item_id="'.$r->item_id.'" order by id desc');
$item_all= find_all_field('item_info','','item_id="'.$r->item_id.'"');

$item_qty= $r->batch_cal_qty;

$master_details=" INSERT INTO `master_requisition_details`(`id`, `req_no`, `exp_date`, `remarks`, `warehouse_id`, `item_id`, `qoh`, `order_qty`, `qty`, `unit_name`, `rate`, `purchase_qty`, `purchase_unit_name`, `last_p_qty`, `last_p_date`, `status`, `entry_at`, `entry_by`, `edit_at`, `edit_by`, `for`, `floor`, `purpose`, `journal`) VALUES ('','".$_POST['req_no']."','".$_POST['need_by']."','".$_POST['req_note']."','".$_POST['warehouse_id']."','".$r->item_id."','".$stock."','".$item_qty."','".$item_qty."','".$item_all->unit_name."','".$rate."','','','','','PENDING','".$_POST['entry_at']."','".$_POST['entry_by']."','".$_POST['edit_at']."','".$_POST['edit_by']."','','','','')";

$query = db_query($master_details);
...
//echo $msg;
		...
		echo '<script> alert(" Please Receive your Product First.");
			...
			window.location = "mr_create_batch.php";
...
if(isset($_POST['confirmm'])){
		...
		// Details insert start
			 ...
			 $res='select a.id,a.item_id,concat(b.item_name) as item_name,a.qoh as Floor_qty,a.exp_date,a.remarks,a.qty,"x" 
			 from master_requisition_details a,item_info b 
			 where b.item_id=a.item_id and  a.req_no='.$$unique;
		...
		echo '<script> window.location.href = "mr_create_batch.php"; </script>';
...
function sub_group_function(id){
	...
	window.location.href = "../mr/mr_create_batch.php?sub_group=" + id;
...
<div class="form-container_large">
	...
	<form action="mr_create_batch.php" method="post" name="codz" id="codz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
		  ...
		  <div class="container n-form2">
            ...
            <div class="form-group pb-1 row m-0">
			 ...
			 <div class="form-group pb-1 row m-0">
              ...
              <label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Need By :</label>
 ...
 <div class="form-group pb-1 row m-0">
    ...
    <label for="<?=$bom_no?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Batch :</label>

              <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">

			<select name="bom_no" id="bom_no" required>

					<option></option>

					<?php foreign_relation('batch_master b,item_info i','b.batch_no','concat(b.batch_no,"-",i.item_name," (",b.batch_date,")")',$bom_no,'b.status="MANUAL" and i.item_id=b.fg_item_id and b.group_for in ('.$user_group_define.')'); ?>
		  ...
		  <div class="container n-form2">
			...
			<select name="<?=$field?>" id="<?=$field?>">	
			 ...
			 <div class="form-group pb-1 row m-0">
              ...
              <label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Batch :</label>
...
<table  class="table1  table-striped table-bordered table-hover table-sm">
          ...
          <tbody class="tbody1">
					...
					<td align="center">
                        	...
                        	<!--<select name="sub_group" id="sub_group" style="width:170px;" onChange="sub_group_function(this.value)">
								...
								if($req_type=='DSTR'){ $con1 .= ' AND status !=1 and item_ledger !=0';}
                         ...
                         echo $a2="select sub_group_id, sub_group_name from item_sub_group where sub_group_id in ('200010000','700010000') ".$con1."";
 ...
 <? //i.sub_group_id like "%'.$get_data.'%"
  ...
  foreign_relation('item_info i,item_sub_group s','concat(i.item_name,"#>",i.item_id,"#>",i.item_id)','i.item_name',$item_id,' i.sub_group_id in ("200010000","700010000")  and i.sub_group_id=s.sub_group_id ');?>
...
/*$res='select a.id,a.item_id,concat(b.item_name) as item_name,a.qoh as Floor_qty,a.exp_date,a.remarks,b.sub_group_id,a.qty,"x" from bom_raw_material a,item_info b where b.item_id=a.item_id and  a.bom_no='.$bom_no.' order by a.item_id';*/
...
$(document).ready(function(){
    $("#group_for").change(function(){
		//document.getElementById('fg_item_id').value="";

        var group_for = $(this).val();
//$("#fg_item_id").html('<option value="">Select Product</option>');

        if(group_for != ""){
            $.ajax({
                url: "../../mis_mod/common/group_wise_filtering_ajax.php",
                type: "POST",
                data: { 
                    group_for: group_for, 
                    type: "Issue_WH"   
