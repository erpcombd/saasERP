...
if(isset($_POST['new'])){
 ...
 $bom_item= "SELECT id,item_id,total_unit FROM bom_raw_material WHERE bom_no='".$_POST['bom_no']."'";
$q=db_query($bom_item);
...
$rate = find_a_field('journal_item','final_price',' final_price > 0 and item_id="'.$r->item_id.'" and tr_from in ("Purchase","Local Purchase","Production Receive","Opening") order by id desc');
$item_all= find_all_field('item_info','','item_id="'.$r->item_id.'"');
$item_qty= $_POST['req_qty']*$r->total_unit;
...
$query = db_query($master_details);
		...
		echo '<script> alert(" Please Receive your Product First.");
			...
			window.location = "mr_create_bom.php";
...
if(isset($_POST['confirmm'])){
		...
		echo '<script> window.location.href = "mr_create_bom.php"; </script>';
...
function sub_group_function(id){
	...
	window.location.href = "../mr/mr_create_bom.php?sub_group=" + id;
...
<div class="form-container_large">
	...
	<form action="mr_create_bom.php" method="post" name="codz" id="codz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
		  ...
		  <div class="container n-form2">
			...
			<select name="warehouse_id" id="warehouse_id">
			 ...
			 <div class="form-group pb-1 row m-0">
			    ...
			    <? $field='bom_no'; $table='bom_master';$get_field='bom_no';$show_field='bom_no';?>

      <label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">BOM :</label>
			...
			<select name="bom_no" id="bom_no" required>
					...
					<?php foreign_relation('bom_master','bom_no','bom_no',$bom_no,' 1'); ?>
