...
("'.$pr_no.'", "'.$YR.'", "'.$cy_id_batch.'", "'.$inv_type.'", "'.$pr_date.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$batch_no.'", "'.$fg_item_id.'", "'.$unit_name.'", "'.$pr_qty.'", "'.$pr_qty.'", "'.$status.'", "'.$entry_at.'", "'.$entry_by.'")';
...
<div class="box" style="width:100%;">
    ...
    <td width="9%" align="right" ><strong> Qc No: </strong></td>
	...
	<a href="../fg_from_pl/invoice_print_view.php?bom_no=<?=$batch_no;?>" target="_blank"><input name="batch" type="text" id="batch" style="width:90%; height:32px;" value="<?=$batch_data->inv_type;?><?=$batch_data->batch_no;?>" readonly="" required tabindex="1" /></a>	</td>
	 ...
	 <td width="13%"  style="text-align:right"><strong>  Product Description: </strong></td>
	 ...
	 <td width="9%" style="text-align:right"><strong>FG Qty: </strong></td>

	 <td width="13%" ><input name="batch_tot_qty" type="text" id="batch_tot_qty" style="width:90%; height:32px;" value="<?=$batch_data->batch_qty-find_a_field('production_receive_detail','sum(total_unit)','batch_no="'.$batch_no.'"');?> <?=$batch_data->unit_name;?> " required readonly="" tabindex="1" /></td>
	 ...
	 <td width="17%" rowspan="8" align="center" ><strong>
      ...
      <input type="submit" name="master_data" id="master_data" value="Initiate Entry" style="width:180px; text-align:center; font-weight:bold; font-size:12px; height:30px; color:#090"/>
								  ...
								  <tr>
								    ...
								    <td align="right" ><strong> QC Date: </strong></td>
								    ...
								    <td style="text-align:right"><strong> Plant Name : </strong></td>
					...
					<option>Extrusion Plant 05</option>
								    ...
								    <td style="text-align:right"><strong>QC Qty:</strong>  </td>
...
<? //if($po_no[$data->po_no]==0) { } ?>
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
	...
	<td bgcolor="#99CCFF"><input name="stock_<?=$datafg->id?>" type="text" id="stock_<?=$datafg->id?>" value="<?=$sstk; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " /></td>

	

    <td bgcolor="#99CCFF"><input name="unit_price_<?=$datafg->id?>" type="text" id="unit_price_<?=$datafg->id?>" value="<?=$final_price[$datafg->item_id]; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " /></td>
	...
	<input name="total_unit_<?=$datafg->id?>" type="text" id="total_unit_<?=$datafg->id?>" value="<?=$total_qty_bom=$datafg->batch_qty; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " /></td>

	 <td bgcolor="#99CCFF"><input name="actual_unit_<?=$datafg->id?>" type="text" id="actual_unit_<?=$datafg->id?>" value="<?=$total_qty ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " /></td>

    <td bgcolor="#99CCFF"><input name="total_amt_<?=$datafg->id?>" type="text" id="total_amt_<?=$datafg->id?>" value="<?=$total_amt=$total_qty*$final_price[$datafg->item_id]; $grand_total_amt +=$total_amt; ?>" style="width:80px; height:30px; " /></td>
...
//		$do_con .= ' and m.do_no in ('.$_POST['do_no'].') ';
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
	...
	<input name="total_unit_by_<?=$databy->id?>" type="text" id="total_unit_by_<?=$databy->id?>" value="<?=$total_qty_bom=$databy->bom_qty; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " /></td>
