...
<div class="box" style="width:100%;">
 ...
 <tr>
    ...
    <td style="background-color: yellow;" width="16%" ><?=find_a_field('production_receive_detail','sum(total_unit)','batch_no="'.$batch_no.'"');?></td>
 </tr>
...
WHERE id = (
    ...
    WHERE ji1.item_id = ji2.item_id
		...
		$sql = "select  item_id, sum(item_in-item_ex) as stock from journal_item where warehouse_id=".$ms_data->section_id."  group by item_id   order by id desc ";
...
<? //if($po_no[$data->po_no]==0) { } ?>
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
    ...
    <td bgcolor="#99CCFF"><input name="unit_price_<?=$datafg->id?>" type="text" id="unit_price_<?=$datafg->id?>" value="<?=$final_price[$datafg->item_id]; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " />
