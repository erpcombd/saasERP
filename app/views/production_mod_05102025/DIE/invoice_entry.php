...
("'.$batch_no.'", "'.$YR.'", "'.$cy_id.'", "'.$inv_type.'", "'.$group_for.'", "'.$bom_no.'", "'.$bom_qty.'", "'.$batch_date.'", "'.$fg_item_id.'", "'.$unit_name.'", "'.$batch_qty.'", "'.$warehouse_id.'", "'.$status.'", "'.$entry_at.'", "'.$entry_by.'","'.$section_id.'")';
...
$sql3 = 'select * from bom_by_product where bom_no="'.$bom_no.'" order by id';
		...
		$batch_cal_qty=$data3->total_unit;
...
$cquery=db_query($csql);
while($crow=mysqli_fetch_object($cquery)){
$pl_ledger[$crow->warehouse_id]=$crow->ledger_id;
}
...
$bsql='select b.item_id,b.batch_no,b.bom_no,b.batch_date,b.final_qty,b.section_id,b.warehouse_id,i.item_id,i.sub_group_id,s.sub_group_id,s.item_ledger from item_info i,item_sub_group s,batch_raw_material b where b.item_id=i.item_id and i.sub_group_id=s.sub_group_id and b.batch_no="'.$batch_no.'"';
$bquery=db_query($bsql);
while($brow=mysqli_fetch_object($bquery)){
$cost_amt=($brow->final_qty*$cost_rate[$brow->item_id]);
add_to_sec_journal('CloudMVC', $jv, $brow->batch_date, $brow->item_ledger,'Batch issue', '0', $cost_amt,'Issue', $batch_no,'','',$cc_code,$_SESSION['user']['group'],$_SESSION['user']['id'],'');

journal_item_control($brow->item_id ,$brow->section_id,$brow->batch_date,0,$brow->final_qty,'Issue',$batch_no,$cost_rate[$brow->item_id],$brow->warehouse_id,$batch_no,'','',$_SESSION['user']['group'],'');
journal_item_control($brow->item_id ,$brow->warehouse_id,$brow->batch_date,$brow->final_qty,0,'Issue',$batch_no,$cost_rate[$brow->item_id],$brow->section_id,$batch_no,'','',$_SESSION['user']['group'],'');
$gr_cost_amt+=$cost_amt;
}
add_to_sec_journal('CloudMVC', $jv, $batch_all->batch_date, $pl_ledger[$batch_all->warehouse_id],'Batch issue', $gr_cost_amt, '0','Issue', $batch_no,'','',$cc_code,$_SESSION['user']['group'],$_SESSION['user']['id'],'');
 sec_journal_journal($jv,$jv,'Issue');
...
<div class="box" style="width:100%;">
    ...
    <td style="width:10%; text-align: right" ><strong> BOM No: </strong></td>
	...
	<input name="bom_no" type="hidden" id="bom_no" style="width:90%; height:32px;" value="<?=$bom_no;?>" readonly="" required  />



	<input name="bom_qty" type="hidden" id="bom_qty" style="width:90%; height:32px;" value="<?=$bom_data->quantity;?>" readonly="" required  />



	<input name="unit_name" type="hidden" id="unit_name" style="width:90%; height:32px;" value="<?=$bom_data->unit_name;?>" readonly="" required />



	<input name="group_for" type="hidden" id="group_for" style="width:90%; height:32px;" value="<?=$bom_data->group_for;?>" readonly="" required />



	<a href="../BOM/invoice_print_view.php?bom_no=<?=$bom_no;?>" target="_blank"><input name="bom" type="text" id="bom" style="width:90%; height:32px;" value="<?=$bom_data->inv_type;?><?=$bom_data->bom_no;?>" readonly="" required /></a>	</td>
	 ...
	 <td style="width:11%" ><strong>  Product: </strong></td>
	 ...
	 <td style="width:10%" ><strong>BOM Qty: </strong></td>



	 <td style="width:12%" ><input name="bom_qty" type="text" id="bom_qty" style="width:90%; height:32px;" value="<?=$bom_data->quantity;?> <?=$bom_data->unit_name;?> " required readonly="" /></td>



	 <td style="width:20%; text-align: center" rowspan="8" ><strong>
      ...
      <input type="submit" name="master_data" id="master_data" value="Initiate Entry" style="width:180px; text-align:center; font-weight:bold; font-size:12px; height:30px; color:#090"/>
								  ...
								  <tr>
								    ...
								    <td style="text-align: right" ><strong> Batch Date: </strong></td>



								    <td ><input name="batch_date" type="text" id="batch_date" style="width:90%; height:32px;" value="<?=$batch_date;?>"  required autocomplete="off" /></td>
								    ...
								    <td ><input name="batch_qty" type="number" id="batch_qty" onblur="qtyLimit(this.value)" style="width:90%; height:32px;" value="<?=$batch_qty;?> " required autocomplete="off"/></td>
...
<div class="box" style="width:100%;">
    ...
    <td style="width:15%; text-align: right" ><strong> Batch No: </strong></td>
	...
	<input name="batch" type="text" id="batch" style="width:90%; height:32px;" value="<?=$ms_data->inv_type;?><?=$ms_data->batch_no;?>" readonly="" required /></td>
	 ...
	 <td style="width:12%; text-align: right"><strong>  Batch Date: </strong></td>



	 <td style="width:25%; text-align: right"><input name="batch_date" type="text" id="batch_date" style="width:90%; height:32px;" value="<?=$ms_data->batch_date?>" required readonly="" /></td>
	...
	value="<?=$bm_data->inv_type;?><?=$bm_data->bom_no;?>" readonly="" required /></a>	 </td>
								  ...
								  <tr>
								    ...
								    <td style="width:15%; text-align: right" ><strong> Production Line: </strong></td>
								    ...
								    <td style="width:10%; text-align: right"><strong>Batch Qty : </strong></td>



								    <td style="width:17%"><input name="quantity" type="text" id="quantity" style="width:90%; height:32px;" value="<?=$ms_data->batch_qty;?> <?=$ms_data->unit_name;?>" readonly="" required /></td>
...
<input name="foe_amt_<?=$data->ledger_id?>" type="text" id="foe_amt_<?=$data->ledger_id?>" value="<?=$foe_exp[$data->ledger_id]; $tot_foe_exp +=$foe_exp[$data->ledger_id];?>" onkeyup="SUMcalculation(<?=$data->ledger_id?>)"  style="width:120px; height:30px;" readonly="" /></td>
 ...
 <? } ?>
  ...
  <tr>
    ...
    <td class="text-center" style="background-color:#90EE90;"><div class="text-center"><strong>Total</strong></div></td>



    <td style="background-color:#90EE90; text-align: right"><input name="foe_amt" id="foe_amt" type="text" size="10"  value="<?=$tot_foe_exp;?>" style="width:120px; height:30px;" readonly="" /></td>
...
document.getElementById('fg_code').value);"  autocomplete="off" >
  ...
  <datalist id="item_name_list" style="font-size:16px;" >
     ...
     <? foreign_relation('item_info','concat(item_id)','concat(item_id,"-> ",item_name)',$fg_code,'1');?>
		                            ...
		                            <td style="width:10%"><input name="total_unit1" type="text" class="input3"   autocomplete="off"  value="" id="total_unit1" style="width:80%; height:30px;" onkeyup="count()" /></td>
...
<table border="0" class="text-center w-100" id="grp" style="font-size:12px; text-transform:uppercase;">
...
//		}
  ...
  <tr style="<?=($i%2)?'background-color:#E8F3FF':'background-color:#fff';?>">
	...
	<input name="total_unit_<?=$datafg->id?>" type="text" id="total_unit_<?=$datafg->id?>" value="<?=$datafg->final_unit; $tot_total_unit +=$datafg->total_unit;?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " /></td>
...
<h1><div class="w-100 text-center"><span style="background-color: yellow; color: red;font-size: 30px !important;">Check Item Stock</span></div></h1>
