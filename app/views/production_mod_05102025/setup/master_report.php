...
where m.status in ('CHECKED','COMPLETED') group by s.item_id";
 ...
 $stock_qty="select item_id,sum(item_in-item_ex) as stock_qty from journal_item where 1 group by item_id";
					$stock_query = db_query($stock_qty);
...
<thead>
<tr><td style="border:0px; text-align:center; font-size:18px; font-weight:200;" colspan="12"><?=$report?></td></tr>
...
<th rowspan="2">SO Qty</th>
<th rowspan="2">Challaned Qty</th>
<th rowspan="2">Pending Qty</th>
<th rowspan="2">Stock Qty</th>
<th rowspan="2">Shortage Qty</th>
<th rowspan="2" bgcolor="#5390F5"><div align="center">Production Qty</div></th>
...
<tr><td><?=$s?></td>
  ...
  <td><?=$datas[5]?></td>
  <td><?=$datas[6]?></td>
  <td><?=$so_qty=$details_qty_get[$datas[2]];?></td>
  <td><?=$ch_qty=$ch_qty_get[$datas[2]];?></td>
  <td><?=$remain=$so_qty-$ch_qty;?></td>
  <td><?=$stock_qty_get[$datas[2]];?></td>
  <td>
  <?=$datas[6]?>
  ...
  <input type="hidden" name="item_id#<?=$datas[2]?>" id="item_id#<?=$datas[2]?>" value="<?=$datas[2]?>" /> 
  <td><div id="po<?=$datas[2]?>"><input type="button" name="Change" value="Change" onclick="getData2('<?=$ajax_page?>', 'po<?=$datas[2]?>',<?=$datas[2]?>,
