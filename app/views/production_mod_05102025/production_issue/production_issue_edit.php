...
$line_id = $_SESSION['line_id'] = find_a_field('production_floor_issue_detail','warehouse_from',$unique_master.'='.$$unique_master);
...
db_delete_all('production_floor_issue_detail','pi_no="'.$_POST['pi_no'].'" and item_id="'.$data->item_id.'"');
db_delete_all('journal_item',' tr_from="Consumption" and sr_no="'.$_POST['pi_no'].'" and item_id="'.$data->item_id.'"');
...
('".$_POST['pi_no']."', '".$_POST['pi_date']."', '".$data->item_id."', '".$line_id."', '0', '".$_POST['total_unit_'.$data->item_id]."', '".$data->cost_price."', '".$total_amt."')";
db_query($do);
...
?>
  <tr>
    ...
    <td align="center" bgcolor="#CCCCCC"><input name="total_unit_<?=$data->item_id?>" type="text" class="input3" id="total_unit_<?=$data->item_id?>"  maxlength="100" style="width:67px;" required="required" value="<?=(find_a_field('production_floor_issue_detail','total_unit','pi_no="'.$$unique_master.'" and item_id = "'.$data->item_id.'"')*1)?>"/></td>
    </tr>
<? }?>
</table>
<table  width="80%" border="1" align="center"  style="border-collapse:collapse; border:1px solid #caf5a5;" cellpadding="2" cellspacing="2">
...
<table width="100%" border="0">
  <tr>
      <td align="center">
        <div align="center">
          <input  name="pi_no2" type="hidden" id="pi_no2" value="<?=$$unique_master?>"/>        
