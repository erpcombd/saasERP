...
if (prevent_multi_submit()) {
	...
	if (isset($_POST['master_data'])) {
		...
		$alter_quantity = $_POST['alter_quantity'];
		$inv_type = 'BATCH-';
		...
		if ($fg_item_id > 0) {
			...
			$ins_sql = 'INSERT INTO batch_master (batch_no, year, inv_id, inv_type, group_for, bom_no, bom_qty, batch_date, fg_item_id, unit_name, batch_qty, alter_quantity, warehouse_id, status, entry_at, entry_by,section_id) VALUES
("' . $batch_no . '", "' . $YR . '", "' . $cy_id . '", "' . $inv_type . '", "' . $group_for . '", "' . $bom_no . '", "' . $bom_qty . '", "' . $batch_date . '", "' . $fg_item_id . '", "' . $unit_name . '", 
"' . $batch_qty . '", "' . $alter_quantity . '", "' . $warehouse_id . '", "' . $status . '", "' . $entry_at . '", "' . $entry_by . '","' . $section_id . '")';

			db_query($ins_sql);
		}

		$bomQty = find_a_field('bom_master', 'quantity', 'bom_no="' . $bom_no . '"');
		$sql1 = 'select * from bom_factory_overhead where bom_no="' . $bom_no . '" order by id';
		$query1 = db_query($sql1);
		while ($data1 = mysqli_fetch_object($query1)) {
			$primeAmt = $data1->amount / $bomQty;
			$batch_amt = $primeAmt * $batch_qty;

			$foe_batch = 'INSERT INTO batch_factory_overhead (batch_no, bom_no, group_for, warehouse_id, batch_date, fg_item_id, ledger_id, bom_amt, batch_amt, final_amt, status, entry_at, entry_by, batch_qty, foe_rate,section_id)

 VALUES("' . $batch_no . '", "' . $bom_no . '", "' . $group_for . '", "' . $warehouse_id . '", "' . $batch_date . '", "' . $fg_item_id . '", "' . $data1->ledger_id . '", "' . $data1->amount . '", "' . $batch_amt . '", "' . $batch_amt . '", "' . $status . '", "' . $entry_at . '", "' . $entry_by . '", "' . $batch_qty . '", "' . $data1->amount . '","' . $section_id . '")';
		...
		while ($data2 = mysqli_fetch_object($query2)) {
			...
			if($data2->cal_with=='Alter Qty'){
				$batch_cal_qty = $data2->total_unit * $alter_quantity;
			}else{
				$primeAmt2 = $data2->total_unit / $bomQty;

				$batch_cal_qty = $primeAmt2 * $batch_qty;
			}

			$rm_batch = 'INSERT INTO batch_raw_material (batch_no, bom_no, group_for, warehouse_id, batch_date, fg_item_id, item_id, unit_name, batch_qty, bom_qty, batch_cal_qty, final_unit, final_qty, status, entry_at, entry_by,section_id,cal_with)
VALUES("' . $batch_no . '", "' . $bom_no . '", "' . $group_for . '", "' . $warehouse_id . '", "' . $batch_date . '", "' . $fg_item_id . '", "' . $data2->item_id . '", "' . $data2->unit_name . '", "' . $batch_qty . '", "' . $data2->total_unit . '", "' . $batch_cal_qty . '", "' . $data2->total_unit . '", "' . $batch_cal_qty . '", "' . $status . '", "' . $entry_at . '", "' . $entry_by . '","' . $section_id . '","' . $data2->cal_with . '")';
		...
		while ($data3 = mysqli_fetch_object($query3)) {
			...
			$primeAmt3 = $data3->total_unit / $bomQty;
			$batch_cal_qty = $primeAmt3 * $batch_qty;

			$rm_batch2 = 'INSERT INTO batch_by_product (batch_no, bom_no, group_for, warehouse_id, batch_date, fg_item_id, item_id, unit_name,type,rate_ratio,batch_qty, bom_qty, batch_cal_qty, final_unit, final_qty, status, entry_at, entry_by,section_id)
VALUES("' . $batch_no . '", "' . $bom_no . '", "' . $group_for . '", "' . $warehouse_id . '", "' . $batch_date . '", "' . $fg_item_id . '", "' . $data3->item_id . '", "' . $data3->unit_name . '","' . $data3->type . '","' . $data3->rate_ratio . '" ,"' . $batch_qty . '", "' . $data3->total_unit . '", "' . $batch_cal_qty . '", "' . $data3->total_unit . '", "' . $batch_cal_qty . '", "' . $status . '", "' . $entry_at . '", "' . $entry_by . '","' . $section_id . '")';
...
?>
		...
		<script language="javascript">
			window.location.href = "invoice_entry_primitek.php?request_no=<?= $batch_no ?>";
		</script>
	...
	if (isset($_POST['create'])) {
		...
		$final_unit = floatval($_POST['total_unit']) / floatval($batch_qty);



		$stkQty = find_a_field('journal_item', 'sum(item_in-item_ex)', 'warehouse_id="' . $section_id . '" and item_id="' . $item_id . '"');
		...
		if ($item_id > 0) {
			...
			if ($total_unit <= $stkQty) {
				...
				$ins_sql = 'INSERT INTO batch_raw_material (batch_no, bom_no, group_for, warehouse_id, batch_date, fg_item_id, item_id, unit_name, batch_qty, bom_qty, batch_cal_qty, final_unit, final_qty,status, entry_at, entry_by,section_id)



 

  VALUES("' . $batch_no . '", "' . $bom_no . '", "' . $group_for . '", "' . $warehouse_id . '", "' . $batch_date . '", "' . $fg_item_id . '", "' . $item_id . '", "' . $unit_name . '", "' . $batch_qty . '", "0", "' . $total_unit . '", "' . $final_unit . '", "' . $total_unit . '", "' . $status . '", "' . $entry_at . '", "' . $entry_by . '","' . $section_id . '")';
  ...
  VALUES("' . $batch_no . '", "' . $bom_no . '", "' . $group_for . '", "' . $warehouse_id . '", "' . $batch_date . '", "' . $fg_item_id . '", "' . $item_id . '", "' . $unit_name . '","' . $type . '","' . $rate_ratio . '" ,"' . $batch_qty . '", "' . $total_unit . '", "' . $total_unit . '", "' . $total_unit . '", "' . $total_unit . '", "' . $status . '", "' . $entry_at . '", "' . $entry_by . '")';
	...
	if (isset($_POST['confirm'])) {
		...
		//$csql='SELECT ji1.item_id, ji1.final_price
		//FROM journal_item ji1
		//JOIN (
		//    SELECT item_id, MAX(id) AS max_id
		//    FROM journal_item
		//    GROUP BY item_id
		//) ji2
		//ON ji1.item_id = ji2.item_id AND ji1.id = ji2.max_id';
		//$cquery=db_query($csql);
		//while($crow=mysqli_fetch_object($cquery)){
		//$cost_rate[$crow->item_id]=$crow->final_price;
		//}

		$csql = 'SELECT item_id,SUM(item_in*item_price)/sum(item_in) as final_price from journal_item WHERE warehouse_id="' . $batch_all->section_id . '" and lot_no="' . $batch_no . '" group by item_id';
		$cquery = db_query($csql);
		while ($crow = mysqli_fetch_object($cquery)) {
			$cost_rate[$crow->item_id] = $crow->final_price;
		}
		$csql = 'select * from warehouse  where 1 group by warehouse_id ';
		$cquery = db_query($csql);
		while ($crow = mysqli_fetch_object($cquery)) {
			$pl_sub_ledger[$crow->warehouse_id] = $crow->ledger_id;
		}
		...
		//$jv = next_journal_sec_voucher_id('', 'Issue', $_SESSION['user']['depot']);

		$bsql = 'select b.item_id,b.batch_no,b.bom_no,b.batch_date,b.final_qty,b.section_id,b.warehouse_id,i.item_id,i.sub_group_id,s.sub_group_id,s.item_ledger,i.sub_ledger_id from item_info i,item_sub_group s,batch_raw_material b 
where b.item_id=i.item_id and i.sub_group_id=s.sub_group_id and b.batch_no="' . $batch_no . '"';
		//$bquery = db_query($bsql);
		//while ($brow = mysqli_fetch_object($bquery)) {
		//	$cost_amt = ($brow->final_qty * $cost_rate[$brow->item_id]);

		//	add_to_sec_journal($_SESSION['proj_id'], $jv, $brow->batch_date, $brow->item_ledger, 'Batch issue', '0', $cost_amt, 'Batch issue', $batch_no, $brow->sub_ledger_id, '', $cc_code, $batch_all->group_for, $_SESSION['user']['id'], '');

		//	journal_item_control($brow->item_id, $brow->section_id, $brow->batch_date, 0, $brow->final_qty, 'Batch issue', $batch_no, $cost_rate[$brow->item_id], $brow->warehouse_id, $batch_no, '', '', $batch_all->group_for, '', '', '', $batch_no);

		//	journal_item_control($brow->item_id, $brow->warehouse_id, $brow->batch_date, $brow->final_qty, 0, 'Batch issue', $batch_no, $cost_rate[$brow->item_id], $brow->section_id, $batch_no, '', '', $batch_all->group_for, '', '', '', $batch_no);


		//	$gr_cost_amt += $cost_amt;
	//	}

		//add_to_sec_journal($_SESSION['proj_id'], $jv, $batch_all->batch_date, $pl_main_ledger[$pl_sub_ledger[$batch_all->warehouse_id]], 'Batch issue', $gr_cost_amt, '0', 'Batch issue', $batch_no, $pl_sub_ledger[$batch_all->warehouse_id], '', $cc_code, $batch_all->group_for, $_SESSION['user']['id'], '');

		//sec_journal_journal($jv, $jv, 'Batch issue');
	...
	if (isset($_POST['delete'])) {
		...
		$found_journal_item = find_a_field('journal_item', 'count(id)', 'lot_no=' . $batch_no);
		if ($found_journal_item > 0) {
		?>
			<script language="javascript">
				alert("Stock is Already Received to the section. Can not delete the Batch");
				window.location.href = "invoice_entry_primitek.php?request_no=<?= $batch_no ?>";
			</script>
		<?
		} else {
			...
			$req_no = find_a_field('master_requisition_master', 'req_no', 'bom_no=' . $batch_no);
			$pi_no = find_a_field('production_issue_master', 'pi_no', 'req_no=' . $req_no);

			$master_sql = "DELETE FROM `master_requisition_master` WHERE req_no=" . $req_no . "";
			db_query($master_sql);
			$detail_sql = "DELETE FROM `master_requisition_details` WHERE req_no=" . $req_no . "";
			db_query($detail_sql);
			$sql2 = "DELETE FROM `production_issue_master` WHERE pi_no=" . $pi_no . "";
			db_query($sql2);
			$sql3 = "DELETE FROM `production_issue_detail` WHERE pi_no=" . $pi_no . "";
			db_query($sql3);
			...
			echo $msg = "Deleted Requisition no :- '" . $req_no . "',Issue No:-'" . $pi_no . "',Batch No:-'" . $batch_no . "'";
		?>
...
<script>
	...
	function update_value(order_no) {
		...
		var total_unit = parseFloat(document.getElementById('total_unit_' + order_no).value);
		var floor_qty = parseFloat(document.getElementById('floor_qty_' + order_no).value);
		//alert(floor_qty);
		...
		if (floor_qty >= total_unit) {
			...
			alert('The floor stocks are currently insufficient: ' + floor_qty + ' < ' + total_unit);
			document.getElementById('total_unit_' + order_no).value = 0;
		}
	}
	...
	function update_value2(order_no) {
		...
		var total_unit = parseFloat(document.getElementById('total_unit_by_' + order_no).value);
		//alert(floor_qty);
		//var amount=(document.getElementById('amount_'+order_no).value);
		var flag = (document.getElementById('flag2_' + order_no).value);


		var strURL = "item_revise_by_ajax.php?order_no=" + order_no + "&item_id=" + item_id + "&batch_qty=" + batch_qty + "&total_unit=" + total_unit + "&flag=" + flag;
		...
		if (req) {
			req.onreadystatechange = function() {
				if (req.readyState == 4) {
					// only if "OK"
					if (req.status == 200) {
						document.getElementById('divi2_' + order_no).style.display = 'inline';
						document.getElementById('divi2_' + order_no).innerHTML = req.responseText;
					} else {
						alert("There was a problem while using XMLHTTP:\n" + req.statusText);
					}
				}
...
<div class="form-container_large">
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<td width="14%">
							...
							<a href="../BOM_PRIMITEK/invoice_print_view.php?bom_no=<?= $bom_no; ?>" target="_blank"><input name="bom" type="text" id="bom" style="width:90%; height:32px;" value="<?= $bom_data->inv_type; ?><?= $bom_data->bom_no; ?>" readonly="" required tabindex="1" /></a>
						</td>
					...
					<tr>
						...
						<td>
							<select name="warehouse_id" id="warehouse_id" style="width:200px;" required>
								<option></option>
								<? foreign_relation('warehouse', 'warehouse_id', 'warehouse_name', $_POST['warehouse_id'], 'use_type="SC"'); ?>
							</select>
						</td>
					...
					<tr>
						...
						<td><input name="alter_quantity" type="text" id="alter_quantity" style="width:90%; height:32px;" value="<?= $alter_quantity; ?>" required tabindex="1" /></td>
					</tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<tr>
						...
						<td width="21%">
							...
							<input name="batch" type="text" id="batch" style="width:90%; height:32px;" value="<?= $ms_data->inv_type; ?><?= $ms_data->batch_no; ?>" readonly="" required tabindex="1" />
						</td>
						...
						<td width="17%">
							...
							<a href="../BOM/invoice_print_view.php?bom_no=<?= $ms_data->bom_no; ?>" target="_blank">
								...
								<input name="bom" type="text" id="bom" style="width:90%; height:32px;"
									...
									value="<?= $bm_data->inv_type; ?><?= $bm_data->bom_no; ?>" readonly="" required tabindex="1" /></a>
						</td>
					...
					<tr>
						...
						<td align="right" width="15%"><strong> Production Line: </strong></td>
					...
					<tr>
						<td align="right">Alter Quantity: </td>
						<td><input name="alter_quantity" type="text" id="alter_quantity" style="width:90%; height:32px;" value="<?= $ms_data->alter_quantity; ?>" required tabindex="1" /></td>
					</tr>
	...
	from ledger_group l, accounts_ledger a where l.group_id=a.ledger_group_id and a.ledger_group_id=416003
	...
	order by l.group_id, a.ledger_id";
					...
					?>
						...
						<tr bgcolor="<?= ($i % 2) ? '#E8F3FF' : '#fff'; ?>">
							...
							<td style="background-color:#99CCFF;" align="right">
								...
								<input name="foe_amt_<?= $data->ledger_id ?>" type="text" id="foe_amt_<?= $data->ledger_id ?>" value="<?= $foe_exp[$data->ledger_id];
								$tot_foe_exp += $foe_exp[$data->ledger_id]; ?>" onkeyup="SUMcalculation(<?= $data->ledger_id ?>)" style="width:120px; height:30px;" readonly="" />
							</td>
						...
						<tr>
							...
							<td align="center" style="background-color:#90EE90;">
		...
		<? if ($data_found > 0) { ?>
			...
			<div class="box text-center">
				...
				<b class="display-5">RAW MATERIAL</b>
...
<td width="10%">
<input name="total_unit1" type="text" class="input3" autocomplete="off" value="" id="total_unit1" style="width:80%; height:30px;" onkeyup="count()" />
</td>
...
where i.item_id=a.item_id  and i.sub_group_id=s.sub_group_id and a.batch_no='" . $batch_no . "' order by a.id ";
					...
					while ($datafg = mysqli_fetch_object($queryfg)) {
						...
						$sstk = find_a_field('journal_item', 'sum(item_in-item_ex)', ' warehouse_id = ' . $datafg->warehouse_id . ' and lot_no=' . $batch_no . ' and item_id=' . $datafg->item_id);

						

						$ip_qty = find_a_field('batch_raw_material', 'final_qty', 'batch_no="' . $batch_no . '" and item_id="' . $datafg->item_id . '"');

						$checkStock[] = ($sstk > 0 && $sstk >= $ip_qty) ? 1 : 0;
					...
					?>
						...
						<tr style="background-color: <?= ($i % 2) ? '#E8F3FF' : '#fff'; ?>;">
							...
							<td style="background-color:#99CCFF;" class="text-center">
								<input type="text" name="floor_qty_<?= $datafg->id ?>" id="floor_qty_<?= $datafg->id ?>" value="<?= $sstk ?>" readonly />
							</td>
							...
							<td style="background-color:#99CCFF;">
								...
								<input name="total_unit_<?= $datafg->id ?>" type="text" id="total_unit_<?= $datafg->id ?>" value="<?= $datafg->final_qty; ?>" onkeyup="TRcalculationpo(<?= $data->id ?>)" style="width:80px; height:30px; " />
							</td>
...
<? }?>
		...
		<? if ($data_found > 0) { ?>
			...
			<div class="box text-center">
				...
				<b class="display-5">BY PRODUCT</b>
...
document.getElementById('fg_code').value);" autocomplete="off">
						...
						<td width="10%"><input name="total_unit" type="number" class="input3" step="any" autocomplete="off" value="" id="total_unit" style="width:80%; height:30px;" /></td>
 ...
 where i.item_id=a.item_id  and i.sub_group_id=s.sub_group_id and a.batch_no='" . $batch_no . "' order by a.id ";
					...
					while ($databy = mysqli_fetch_object($queryfg)) {
						$i++;
					...
					?>
						...
						<tr style="background-color: <?= ($i % 2) ? '#E8F3FF' : '#fff'; ?>;">
							...
							<td style="background-color:#99CCFF;"><?= $databy->type ?></td>

							<td style="background-color:#99CCFF;"><?= $databy->rate_ratio ?></td>

							<td style="background-color:#99CCFF;">

								<input name="order_no_by_<?= $databy->id ?>" type="hidden" id="order_no_by_<?= $databy->id ?>" value="<?= $databy->id ?>" />
								...
								<input name="total_unit_by_<?= $databy->id ?>" type="text" id="total_unit_by_<?= $databy->id ?>" value="<?= $databy->final_qty;
																																		$tot_total_unit += $databy->total_unit; ?>" onkeyup="TRcalculationpo(<?= $data->id ?>)" style="width:80px; height:30px; " />
							</td>
							...
							<td>
								...
								<? } else { ?>
									...
									<span id="divi2_<?= $databy->id ?>">



										<input name="flag2_<?= $databy->id ?>" type="hidden" id="flag2_<?= $databy->id ?>" value="0" />
			...
			<table width="100%" border="0">
				...
				?>
						...
						<h1>
							<center><span style="color: red;font-size: 30px !important;">Some Material's Stock Less Then Required Qty! </span></center>
						</h1>
