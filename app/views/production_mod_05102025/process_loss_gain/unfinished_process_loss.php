    ...
    <form action="" method="post" name="codz" id="codz">
      ...
      <div class="container-fluid pt-5 p-0 ">
        ...
        <table class="table1  table-striped table-bordered table-hover table-sm">
          ...
          <tbody class="tbody1">
          <?
          if($_POST['fdate']!='') {$con=' and m.req_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';}else{$con=' and m.req_date between "'.date('Y-m-01').'"
and "'.date('Y-m-d').'"';}

          $res ='select m.*,w.warehouse_name

  from process_loss_gain_master m,warehouse w

  where m.warehouse_id=w.warehouse_id and m.status="MANUAL" and m.issue_type="PROCESS_LOSS" '.$con.' order by m.req_no desc';
          ...
          ?>
            <tr>
              ...
              <td class="text-center"><?php echo date('d-m-Y',strtotime($data->req_date));?></td>
              <td class="text-left"><?=$data->warehouse_name?></td>
			  <td class="text-left"><?=find_a_field('warehouse','warehouse_name','warehouse_id='.$data->to_warehouse_id)?></td>
              <td class="text-center">
                <a href="process_loss_entry.php?old_req=<?=$data->req_no?>">
                  <input type="button" value="Complete Process Loss"  class="btn1 btn1-submit-input"/>
                </a>
              </td>
            </tr>
