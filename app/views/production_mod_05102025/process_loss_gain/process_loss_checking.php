...
// Details insert start
$jv=next_journal_voucher_id('','PROCESS_LOSS');
...
journal_item_control($r->item_id, $masterAll->warehouse_id, $masterAll->req_date, 0, $r->qty, 'PROCESS_LOSS', $r->id, $r->rate, '', $$unique, '', '',0, $r->rate,'');
...
add_to_sec_journal($proj_id, $jv, $pdate, $r->item_ledger, 'Process Loss - '.$$unique,'0',($r->qty*$r->rate), 'PROCESS_LOSS', $$unique,'',$$unique,$cc_code);
...
add_to_sec_journal($proj_id, $jv, $masterAll->req_date, $config->process_loss_ledger, 'Process Loss - '.$$unique,$total_amt,'0', 'PROCESS_LOSS', $$unique,'',$$unique,$cc_code);

// Details insert end	
sec_journal_journal($jv,$jv,'PROCESS_LOSS');
unset($_POST);
...
echo '<script> window.location.href = "unchecked_process_loss.php?new=2"; </script>';
