...
$table_master='requisition_master_local';

$table_details='requisition_order_local';
...
auto_complete_from_db('item_info','item_name','concat(item_name,"#>#>",item_id,"#>",finish_goods_code)','product_nature != "Saleable" and sub_group_id like "%'.$get_data.'%"','item_id');
...
$select = "select req_no , req_for from requisition_master_local where 1 order by req_no";

$query=db_query($select);

while($sr=mysqli_fetch_object($query)){

$select2 = "select count(req_no) as co from requisition_master_local where req_no<".$sr->req_no." and req_for=".$sr->req_for;
...
$select3 = "update requisition_master_local set temp_id='".($req_count+1)."' where req_no=".$sr->req_no." and req_for=".$sr->req_for;
...
<td width="100" align="center" bgcolor="#0099FF"><strong>Sub Group <br />Item Name</strong></td>
...
<td align="center" bgcolor="#0099FF"  style="width: 70px;"><strong>P.QTY</strong></td>

<td align="center" bgcolor="#0099FF"  style="width: 70px;"><strong>AVG. Consumption</strong></td>
...
<td align="center" bgcolor="#0099FF"><strong>Remark</strong></td>
...
<input  name="item_id" type="text" id="item_id" value="<?=$item_id?>" style="width:100px;" required="required" onBlur="getData2('mr_ajax.php', 'mr', this.value, document.getElementById('warehouse_id').value+'#-#'+document.getElementById('req_for').value);"/>   
...
<td colspan="5" align="center" bgcolor="#CCCCCC"><span id="mr">
...
<td><input name="avgconsump" type="text" class="input3" id="avgconsump" style="width:70px;" onfocus="focuson('qty')" readonly/></td>
...
<td align="center" bgcolor="#CCCCCC"><input name="qty" type="text" class="input3" id="qty"  maxlength="100" style="width:50px;" required/></td>
...
$res='select a.id,concat(b.item_name," :: ",b.item_description) as item_name,a.qoh as stock_qty,a.exp_date,a.remarks,a.qty,"x" from requisition_order_local a,item_info b where b.item_id=a.item_id and a.req_no='.$req_no;
...
$req_count = find_a_field('requisition_order_local','count(1)','req_no="'.$$unique.'"');
...
<td align="center">
		<input name="reqtype" type="hidden" value="<?=$req_type?>"/>
