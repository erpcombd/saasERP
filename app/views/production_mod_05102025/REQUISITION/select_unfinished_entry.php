...
div.form-container_large input {
  ...
  <form action="" method="post" name="codz" id="codz">
    <table style="width:80%" border="0" align="center">
        <tr>
          <td style="width:320px; background-color:#FF9966;" ><strong> Date </strong></td>
          ...
          <td style="width:237px; background-color:#FF9966;" ><input type="text" name="tdate" id="tdate" value="<?=$_POST['tdate']?>" style="width:150px; font-weight:bold; font-size:12px; height:30px; color:#090"/></td>
          <td style="width:350px; background-color:#FF9966; text-align: center" rowspan="4"><strong>
...
$con .= ' and m.invoice_no in ('.$_POST['invoice_no'].') ';
   ...
   $res="select  m.* from  purchase_requisition_master m where m.status='MANUAL' ".$con." order by m.invoice_date, m.invoice_no";
...
<td onClick="custom(<?=$data->invoice_no;?>);" <?=(++$z%2)?'':'';?>><?= find_a_field('warehouse','warehouse_name','warehouse_id="'.$data->warehouse_id.'"');?></td>
