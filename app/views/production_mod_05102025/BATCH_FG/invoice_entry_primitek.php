...
if(isset($_POST['master_data'])){
	...
	$by_product_qty = $_POST['by_product_qty'];
...
//   	 $lot_no_generate='LOT'.$tr_no.''.$cy_id;
	 ...
	 if($fg_item_id>0) {
	  ...
	  $ins_sql1 = 'INSERT INTO production_receive_master (pr_no, year, inv_id, inv_type, pr_date, group_for, warehouse_id, batch_no, fg_item_id, unit_name, batch_qty, pr_qty,alter_quantity,by_product_qty, status, entry_at, entry_by,section_id) VALUES



("'.$pr_no.'", "'.$YR.'", "'.$cy_id_batch.'", "'.$inv_type.'", "'.$pr_date.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$batch_no.'", "'.$fg_item_id.'", "'.$unit_name.'", "'.$pr_qty.'", "'.$pr_qty.'", "'.$alter_quantity.'", "'.$by_product_qty.'" ,"'.$status.'", "'.$entry_at.'", "'.$entry_by.'", "'.$section_id.'")';
	...
	db_query($ins_sql1);
	 ...
	 $ins_sql2 = 'INSERT INTO production_receive_detail (pr_no, pr_date, batch_no, group_for, warehouse_id, item_id, total_unit, unit_price, total_amt, status, entry_at, entry_by,item_type) VALUES



("'.$pr_no.'", "'.$pr_date.'", "'.$batch_no.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$fg_item_id.'", "'.$pr_qty.'", "'.$unit_price.'", "'.$total_amt.'", "'.$status.'", "'.$entry_at.'", "'.$entry_by.'","FG")';
...
$sql_by_product = 'select item_id as by_product_item_id,sum(final_qty) as total_qty,unit_price from batch_by_product where batch_no="'.$batch_no.'"';
$by_product_query = db_query($sql_by_product);
while($data_by_product=mysqli_fetch_object($by_product_query))
{

$total_by_product_amt=$data_by_product->total_qty*$data_by_product->unit_price;

 $ins_sql3 = 'INSERT INTO production_receive_detail (pr_no, pr_date, batch_no, group_for, warehouse_id, item_id, total_unit, unit_price, total_amt, status, entry_at, entry_by,item_type) VALUES



("'.$pr_no.'", "'.$pr_date.'", "'.$batch_no.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$data_by_product->by_product_item_id.'", "'.$by_product_qty.'", "'.$data_by_product->unit_price.'", "'.$total_by_product_amt.'", "'.$status.'", "'.$entry_at.'", "'.$entry_by.'","By_Product")';
...
db_query($ins_sql3);
	 ...
	 $batchQty=find_a_field('batch_master','batch_qty','batch_no="'.$batch_no.'"');
		...
		$final_foe_amt=($primeAmt*($pr_qty+$by_product_qty))*1;
...
window.location.href = "invoice_entry_primitek.php?request_no=<?=$pr_no?>";
...
if(isset($_POST['confirm'])){
	...
	$tot_production_cost = (float)$tot_foe_cost+(float)$tot_rm_cost;
	...
	while($databy =mysqli_fetch_object($queryby)){
		...
		if($by_product_qty>0){
			...
			if($databy->type=="Ratio"){
				$rate = ((($databy->rate_ratio / 100) * $tot_production_cost)/$by_product_qty);
			}else{
				$rate = $databy->rate_ratio;
			}
...
$updateby_rec = "update  production_receive_detail  set unit_price = ".$rate.",total_unit=".$by_product_qty.",total_amt=".$rate*$by_product_qty.", status='".$status."'
			  where pr_no=".$pr_no." and item_id=".$databy->item_id." and item_type='By_Product'";

			db_query($updateby_rec);

			$amount_by += $by_product_qty* $rate;

			journal_item_control($databy->item_id ,$warehouse_id,$pr_date,$by_product_qty,0,'Production Receive',$pr_no,$rate,0,$pr_no,'','',$group_for,'','','',$batch_no);
...
journal_item_control($item_id,$warehouse_id,$pr_date,0,$total_unit,'Consumption',$data->id,$unit_price,0,$pr_no,'','',$group_for,'','','',$batch_no);
	...
	$con_sql1="UPDATE production_receive_master SET cost_price='".$pr_unit_price."', cost_amt='".$tot_production_cost."', status='".$status."',  checked_by='".$entry_by."', checked_at='".$entry_at."' WHERE pr_no = '".$pr_no."' ";
	 ...
	 $con_sql2="UPDATE production_receive_detail SET unit_price='".$pr_unit_price."', total_amt='".$tot_production_cost."', status='".$status."', checked_by='".$entry_by."', checked_at='".$entry_at."' WHERE batch_no = '".$batch_no."' and  pr_no = '".$pr_no."' and item_type='FG'";

	 db_query($con_sql2);

	 journal_item_control($fg_item_id ,$warehouse_id,$pr_date,$pr_qty,0,'Production Receive',$pr_no,$pr_unit_price,0,$pr_no,'','',$group_for,'','','',$batch_no);
   ...
   $oh_sql = "select l.group_name, a.ledger_id, a.ledger_name
	...
	from ledger_group l, accounts_ledger a where l.group_id=a.ledger_group_id and a.ledger_group_id in (416003)
	...
	group by a.ledger_id";
	 ...
	 $oh_sum +=$overhead_data;
	 	 ...
	 	 $oh_insert="INSERT INTO `secondary_journal` (`proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `entry_by`, `tr_id`, `group_for`, `entry_at`) VALUES ('".$_SESSION['proj_id']."', '".$jv."', '".$pr_date."', '".$oh_data->ledger_id."','', '0.00',".$overhead_data.", '".$tr_from."', '".$pr_no."', '', '0', '".$_SESSION['user']['id']."', '', '".$group_for."', '".date('Y-m-d H:i:s')."')";
	...
	$wip_ledger = find_a_field('config_group_class', 'wip_ledger', 'group_for=' . $all_pr->group_for);
...
$pl_main_ledger[$crow->sub_ledger_id]=$crow->ledger_id;
	...
	$sql_fg = "select * from production_receive_detail  where pr_no=".$pr_no."";
	$queryfg = db_query($sql_fg);
	while($datafg =mysqli_fetch_object($queryfg)){
	if($datafg->item_type=='By_Product')
	{ 
	$total_by_product_amt+=$datafg->total_amt;
	
	$by_product_sub_ledger = find_a_field('item_info','sub_ledger_id','item_id='.$datafg->item_id);
 	$by_product_ledger = find_a_field('item_info i,item_sub_group s','s.item_ledger',' i.sub_group_id=s.sub_group_id and i.item_id='.$datafg->item_id);

	 $by_product_insert="INSERT INTO `secondary_journal` (`proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `entry_by`, `tr_id`, `group_for`, `entry_at`) VALUES ('".$_SESSION['proj_id']."', '".$jv."', '".$pr_date."', '".$by_product_ledger."','',".$datafg->total_amt.",'0.00', '".$tr_from."', '".$pr_no."', '".$by_product_sub_ledger."', '0', '".$_SESSION['user']['id']."', '', '".$group_for."', '".date('Y-m-d H:i:s')."')";

	 db_query($by_product_insert);

	
	
	$fg_total_amt_without_by=($rm_sum+$oh_sum)-$total_by_product_amt;
	
	$fg_without_by_insert="INSERT INTO `secondary_journal` (`proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `entry_by`, `tr_id`, `group_for`, `entry_at`) VALUES ('".$_SESSION['proj_id']."', '".$jv."', '".$pr_date."', '".$fg_ledger."','',".$fg_total_amt_without_by.",'0.00', '".$tr_from."', '".$pr_no."', '".$fg_sub_ledger."', '0', '".$_SESSION['user']['id']."', '', '".$group_for."', '".date('Y-m-d H:i:s')."')";

	 db_query($fg_without_by_insert);

}
else{
	 //Item Sub Group Dr

	 $fg_insert="INSERT INTO `secondary_journal` (`proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `entry_by`, `tr_id`, `group_for`, `entry_at`) VALUES ('".$_SESSION['proj_id']."', '".$jv."', '".$pr_date."', '".$fg_ledger."','',".($rm_sum+$oh_sum).",'0.00', '".$tr_from."', '".$pr_no."', '".$fg_sub_ledger."', '0', '".$_SESSION['user']['id']."', '', '".$group_for."', '".date('Y-m-d H:i:s')."')";
...
window.location.href = "batch_fg_receive_primitek.php";
...
<div class="box" style="width:100%;">
 ...
 <tr>
     <td colspan="2" style="background-color: yellow;" width="" align="right" ><strong>Total Batch Qty: </strong></td>

    <td style="background-color: yellow;" width="" ><?=$batch_data->batch_qty?></td>
	<td  style="background-color: yellow;"></td>
	
	<td colspan="2" style="background-color: yellow;" width="" align="right" ><strong>Batch received Qty: </strong></td>

    <td style="background-color: yellow;" width="" >
...
<? foreign_relation('warehouse','warehouse_id','warehouse_name',$_POST['warehouse_id'],'warehouse_id="'.$batch_data->warehouse_id.'"');?>
								  ...
								  <tr>
									<td colspan=2></td>
						<td align="">Alter Quantity: </td>
						<td><input name="alter_quantity" type="text" id="alter_quantity" style="width:90%; height:32px;" value="" required tabindex="1" /></td>

						<td>By Product Qty:</td>
						<td><input name="by_product_qty" type="text" id="by_product_qty" style="width:90%; height:32px;" value="" required tabindex="1" /></td>
					</tr>
...
<div class="box" style="width:100%;">
	...
	value="<?=$bch_data->inv_type;?><?=$bch_data->batch_no;?>" readonly="" required tabindex="1" /></a>	 </td>
								  ...
								  <tr>
								    ...
								    <td width="17%"><input name="quantity" type="text" id="quantity" style="width:90%; height:32px;" 
									value="<?=$ms_data->pr_qty;?> <?=$ms_data->unit_name;?>" readonly="" required tabindex="1" />
									</td>
							      </tr>
						...
						<td>By Product Qty:</td>
						<td><input name="by_product_qty" type="text" id="by_product_qty" style="width:90%; height:32px;" value="<?=$ms_data->by_product_qty;?>" required tabindex="1" /></td>
					</tr>
...
$sql = 'SELECT item_id,SUM(item_in*item_price)/SUM(item_in) as final_price
from journal_item 
WHERE warehouse_id="'.$ms_data->warehouse_id.'" and lot_no="'.$ms_data->batch_no.'" group by item_id';
...
from batch_raw_material a, item_info i, item_sub_group s 
where i.item_id=a.item_id  and i.sub_group_id=s.sub_group_id and a.batch_no='".$ms_data->batch_no."' and a.final_qty>0 order by i.sub_group_id, i.item_name";
...
if($datafg->cal_with=='Main Qty'){
	$total_qty=($datafg->final_qty/$datafg->batch_qty)*($ms_data->pr_qty+$ms_data->by_product_qty);

}else{
	$total_qty=$datafg->final_unit*$ms_data->alter_quantity;
}
...
<? //if($po_no[$data->po_no]==0) { } ?>
   ...
   <tr style="background-color: <?= ($i % 2) ? '#E8F3FF' : '#fff'; ?>;">
	...
	<td style="background-color:#99CCFF;"><input name="stock_<?=$datafg->id?>" type="text" id="stock_<?=$datafg->id?>" value="<?=$sstk; ?>" 
	onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " readonly="" /></td>

	

    <td style="background-color:#99CCFF;"><input name="unit_price_<?=$datafg->id?>" type="text" id="unit_price_<?=$datafg->id?>" 
	value="<?=$final_price[$datafg->item_id]; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " readonly="" />
	<input name="item_id_<?=$datafg->id?>" type="hidden" id="item_id_<?=$datafg->id?>" value="<?=$datafg->item_id; ?>"  />
	</td>

	
    <td style="background-color:#99CCFF;"><input name="actual_unit_<?=$datafg->id?>" type="text" id="actual_unit_<?=$datafg->id?>" value="<?=$total_qty ?>" 
	onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " readonly="" /></td>

    <td style="background-color:#99CCFF;"><input name="total_amt_<?=$datafg->id?>" type="text" id="total_amt_<?=$datafg->id?>" 
	value="<?=$total_amt=$total_qty*$final_price[$datafg->item_id]; $grand_total_amt +=$total_amt; ?>" style="width:80px; height:30px; " readonly="" /></td>
    </tr>
  ...
  <tr style="background-color: <?= ($i % 2) ? '#E8F3FF' : '#fff'; ?>;">
    ...
    <td style="background-color:#99CCFF;" style="font-size:18px;">
...
<div class="tabledesign2" style="width:100%">
<div class="box text-center">
<b class="display-5">Rejected/By Product</b>
</div>
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0" style="font-size:12px; text-transform:uppercase;">
<tr>
<th width="19%">Category</th>
...
<input name="total_unit_by_<?=$databy->id?>" type="text" id="total_unit_by_<?=$databy->id?>" value="<?=$total_qty_bom=$ms_data->by_product_qty; ?>" 
onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; "  readonly="readonly" /></td>



<td bgcolor="#99CCFF"><input name="actual_unit_by_<?=$databy->id?>" type="text" id="actual_unit_by_<?=$databy->id?>" 
value="<?=$total_qty=$ms_data->by_product_qty; $grand_by_unit +=$total_qty;  ?>" onkeyup="TRcalculationpo(<?=$databy->id?>)" style="width:80px; height:30px; "  /></td>
...
<input type="button" name="Button" value="EDIT"  onclick="update_value2(<?=$databy->id?>)" 
...
<div class="box text-center">
	...
	<b class="display-5">Factory Overhead</b>
...
<!--<input name="delete" type="submit" class="btn1" value="DELETE" style="width:200px; font-weight:bold; float:left; font-size:12px; height:30px; background:#FF0000;color: #000000; border-radius:50px 20px;" />--></td>
