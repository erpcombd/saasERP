...
$batch_no=find_a_field('batch_close_master','batch_no','closing_no='.$_REQUEST['request_no']);
    ...
    $total_raw_remaining_value =$_POST['total_remaining_value'];
		 ...
		 $jv=next_journal_voucher_id('','Batch Closing',$group_for);
		 $wip_ledger = find_a_field('config_group_class','wip_ledger','id="'.$group_for.'"');

	$sql = 'select a.*, s.sub_group_name, i.finish_goods_code, i.item_name, i.unit_name,i.sub_ledger_id,s.item_ledger 
...
VALUES("'.$closing_no.'", "'.$pr_date.'", "'.$batch_no.'", "'.$group_for.'", "'.$section_id.'", "'.$fg_item_id.'", "'.$item_id.'","'.$rm_issue_unit.'","'.$rm_issue_price.'","'.$rm_issue_amt.'","'.$consumption_unit.'","'.$consumption_price.'","'.$consumption_amt.'","'.$remaining_unit.'","'.$remaining_price.'","'.$remaining_amt.'","'.$status.'", "'.$entry_at.'", "'.$entry_by.'")';
...
journal_item_control($item_id,$_POST['transfer_wh'],$pr_date,$remaining_unit,0,'Batch Closing',$data->id,$remaining_price,0,$closing_no,'','',$group_for,'','','','');

$item_insert="INSERT INTO `secondary_journal` (`proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `entry_by`, `tr_id`, `group_for`, `entry_at`) VALUES ('".$_SESSION['proj_id']."', '".$jv."', '".$pr_date."', '".$data->item_ledger."','',".$remaining_amt.",'0.00','Batch Closing', '".$closing_no."', '".$data->sub_ledger_id."', '0', '".$_SESSION['user']['id']."', '', '".$group_for."', '".date('Y-m-d H:i:s')."')";
db_query($item_insert);
$wip_insert="INSERT INTO `secondary_journal` (`proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `entry_by`, `tr_id`, `group_for`, `entry_at`) VALUES ('".$_SESSION['proj_id']."', '".$jv."', '".$pr_date."', '".$wip_ledger."','','0.00',".$remaining_amt.",'Batch Closing', '".$closing_no."', '".$data->sub_ledger_id."', '0', '".$_SESSION['user']['id']."', '', '".$group_for."', '".date('Y-m-d H:i:s')."')";
db_query($wip_insert);
 ...
 $update_transfer_wh = "update  batch_close_master  set transfer_wh = ".$_POST['transfer_wh'].",status='COMPLETED' where closing_no='".$closing_no."'";
db_query($update_transfer_wh);
 $update_batch_master = "update  batch_master  set status='CLOSED' where batch_no='".$batch_no."'";
db_query($update_batch_master);


}
sec_journal_journal($jv,$jv,'Batch Closing');
...
<div class="box" style="width:100%;">
 ...
 <tr>
     <td colspan="2" style="background-color: yellow;" width="15%" align="right" ><strong>Total Batch Qty: </strong></td>

    <td style="background-color: yellow;" width="16%" ><?=$batch_data->batch_qty?></td>
	
	     <td style="background-color: yellow;" width="15%" align="right" ><strong>Batch FG received Qty: </strong></td>

    <td style="background-color: yellow;" width="16%" >
	<?=find_a_field('production_receive_detail','sum(total_unit)','batch_no="'.$batch_no.'" and status not in ("MANUAL") and item_type="FG"');?>
	</td>
	<td style="background-color: yellow;" width="15%" align="right" ><strong>Batch BY received Qty: </strong></td>

    <td style="background-color: yellow;" width="16%" >
	<?=find_a_field('production_receive_detail','sum(total_unit)','batch_no="'.$batch_no.'" and status not in ("MANUAL") and item_type="By_Product"');?>
	</td>
 </tr>
<tr>
	...
	<a href="../fg_from_pl/invoice_print_view.php?bom_no=<?=$batch_no;?>" target="_blank"><input name="batch" type="text" id="batch" style="width:90%; height:32px;" value="<?=$batch_data->inv_type;?><?=$batch_data->batch_no;?>" readonly="" required tabindex="1" /></a>	</td>
	 ...
	 <td width="17%" rowspan="2" align="center" ><strong>
...
from journal_item 
WHERE warehouse_id="'.$closing_data->warehouse_id.'" and lot_no="'.$closing_data->batch_no.'" and tr_from="Issue" group by item_id';
...
<td bgcolor="#99CCFF"><?=$by_product_price[$databy->item_id];?></td>
<td bgcolor="#99CCFF"><input name="total_unit_by_<?=$databy->id?>" type="text" id="total_unit_by_<?=$databy->id?>" 
value="<?=$by_product_stock[$databy->item_id]; $total_qty_bom=$by_product_stock[$databy->item_id]; ?>" 
style="width:80px; height:30px; "  readonly="readonly" /></td>
<!--<td bgcolor="#99CCFF">
...
<td bgcolor="#99CCFF">&nbsp;</td>
<td bgcolor="#99CCFF">
<input name="total_qty_bom" id="total_qty_bom" type="text" size="10"  value="<?=$total_qty_bom;?>" style="width:120px; height:30px;" readonly="" /></td>
<!--<td bgcolor="#99CCFF"></td>-->

<td bgcolor="#99CCFF">
<input name="total_amt_bom" id="total_amt_bom" type="text" size="10"  value="<?=$total_amt_bom;?>" style="width:120px; height:30px;" readonly="" /></td>
</tr>
</table>
...
<td> <?=$databy->item_name?> </td>
<td bgcolor="#99CCFF"><?=$databy->unit_name?></td>
<td bgcolor="#99CCFF"><?=$fg_price[$databy->fg_item_id]?></td>
<td bgcolor="#99CCFF">
...
echo 'Issue Value='.$total_issue_value. '|| Consume Value='.$total_consume_value.'|| FG Value='.$total_amt.'|| By Product Value='.$total_amt_bom.'|| Factory Overhead='.$tot_foe_exp;
