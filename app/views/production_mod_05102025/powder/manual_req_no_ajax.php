<?php
session_start();
//require "../../support/inc.all.php";

 

 

require_once "../../../controllers/routing/default_values.php";
require_once SERVER_CORE."routing/layout.top.php";


@ini_set('error_reporting', E_ALL);


@ini_set('display_errors', 'Off');





 $str = $_POST['data'];


$data=explode('##',$str);


$pl_id=$data[0];
$manual_req_no=$data[1];
 $manual_req= find_a_field('master_requisition_master','manual_req_no','req_for="'.$pl_id.'" order by req_no desc');

$man = explode("-",$manual_req);
 $manual_req_no = $man[1] +1;



$short_code= find_a_field('warehouse','short_code','warehouse_id="'.$pl_id.'"');


if($manual_req_no==0)$manual_req_no=1;

if($short_code=="")$short_code="PL";
$line_req_no=$short_code."-".$manual_req_no;


$id= find_a_field('production_issue_master','entry_at','status="COMPLETE" and warehouse_to='.$pl_id);

		if($id !=''){
			$pending_date = date('Y-m-d',strtotime($id));
			}
			else {
				$pending_date ='';
			}
			
//$date = strtotime("+7 day");
//echo date('M d, Y', $date);
			

?>
<input  name="manual_req_no" type="text" id="manual_req_no" value="<?=$line_req_no?>"/>
<input  name="pending_date" type="hidden" id="pending_date" value="<?=$pending_date?>" />


