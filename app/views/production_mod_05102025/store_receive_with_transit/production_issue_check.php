...
<input name="so_no" type="text" id="so_no"  value="<?=$so_no;?>" />
        ...
        <input name="warehouse_from" type="hidden" id="warehouse_from"  value="<?=find_a_field('warehouse w, production_issue_master p','w.warehouse_name','p.warehouse_from=w.warehouse_id and p.pi_no='.$_GET['old_pi_no'])?>" />

  <input name="warehouse_from4" type="text" id="warehouse_from4" value="<?=find_a_field('warehouse w, production_issue_master p','w.warehouse_name','p.warehouse_from=w.warehouse_id and p.pi_no='.$_GET['old_pi_no'])?>" readonly/>
                </div>
              </div>
		...
		<div class="form-group row m-0 pb-1">
		                <label for="manual_req_no" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Transit Warehouse: </label>
                <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
				
				
				
                <input name="" type="text" id="" value="<?=find_a_field('warehouse w, production_issue_master p','w.warehouse_name','p.transit_warehouse=w.warehouse_id and p.pi_no='.$_GET['old_pi_no'])?>" readonly/>
                </div>
              </div>	  
...
<table id="grp" class="table1  table-striped table-bordered table-hover table-sm">
	...
	<tbody class="tbody1">
<? 
  $res='select w.id,i.item_name,w.item_id, w.pi_no,pm.pi_no,i.item_description,i.unit_name,w.req_qty,w.total_unit, pm.transit_warehouse 
 ...
 $stock_final = find_a_field('journal_item','sum(item_in-item_ex)','item_id="'.$row->item_id.'" and warehouse_id="'.$row->transit_warehouse.'"');
