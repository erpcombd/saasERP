...
$report="Undelivered Do Details Report";
 ...
 $sql_undel = "select m.do_no,m.do_date,o.item_id,concat(d.dealer_code,'- ',d.dealer_name_e)  as dealer_name,d.product_group as grp,w.warehouse_name as depot,concat(i.finish_goods_code,'- ',item_name) as item_name,o.pkt_unit as crt,o.dist_unit as pcs,o.total_unit,
o.total_amt,m.rcv_amt,m.payment_by as PB from 

sale_do_master m,sale_do_details o, item_info i,dealer_info d , warehouse w

where m.do_no=o.do_no and i.item_id=o.item_id and m.dealer_code=d.dealer_code and m.status in ('CHECKED') and w.warehouse_id=d.depot ".$date_con.$item_con.$depot_con.$dtype_con.$dealer_con.$item_brand_con;
...
<table width="70%" cellspacing="0" cellpadding="2" border="0" align="center">
  <thead>
    <tr>
      <td style="border:0px;" colspan="6" align="center"><h1><?=find_a_field('user_group','group_name','id="'.$_SESSION['user']['group'].'"')?></h1></td>
    </tr>

	 <tr>
      <td style="border:0px;" colspan="6" align="center"><h2>Sales Target Report At a Glance<br />Date Interval : <?=$_POST['f_date']?> To <?=$_POST['t_date']?> </h2></td>
    </tr>
	
    <tr>
      <th><div align="center">S/L</div></th>
	  <th>Sales Person</th>
      <th><div align="center">Total WO</div></th>
	  <th>Target amount</th>
...
if($_POST['PBI_NAME']!=''){
  ...
  $PBI_NAME = $_POST['PBI_NAME'];
  $d_con = " and  t.sales_person = '$PBI_NAME' ";
  $s_con = " and  m.marketing_person = '$PBI_NAME' ";
   ...
   $startDate = $_POST['f_date'];
   $endDate = $_POST['t_date'];
   $d_con .= " and  t.from_date between '$startDate' and '$endDate'";
   $s_con .= " and  m.do_date between '$startDate' and '$endDate'";
  }

    $sql = " select sum(t.target_amount) as target_amount, p.PBI_NAME,t.sales_person 
    from sales_target t, personnel_basic_info p 
    where t.sales_person=p.PBI_ID ".$d_con." group by t.sales_person";

 $query = db_query($sql);


while($data3=mysqli_fetch_object($query)){$s++;
  $total_wo = find_a_field('sale_do_master','count(do_no)','marketing_person='.$data3->sales_person.$s_con.' ');
  $total_sale_amount = find_a_field('sale_do_master m, sale_do_details d','sum(d.total_amt)','m.do_no=d.do_no and m.marketing_person='.$data3->sales_person.$s_con.' ');
...
?>
    ...
    <tr>
      <td><div align="center"><?=++$j?></div></td>
	   <td><?=$data3->PBI_NAME?></td>
      <td><div align="center"><?=$total_wo?></div></td>
      <td><div style="text-align:right"><?=number_format($data3->target_amount,0,'.',',')?></div></td>
    <td style="text-align:right"><?=number_format($total_sale_amount,0)?></td>
    <td style="text-align:right"><?=number_format($due = $data3->target_amount-$total_sale_amount,0)?></td>
    ...
    <?
	$tot_sale = $tot_sale+$total_sale_amount;
  $tot_target = $tot_target+$data3->target_amount;
  $tot_due = $tot_due+$due;
}
...
?>
    <tr class="footer">
	  ...
	  <td>&nbsp;</td>
      <td>&nbsp;</td>
      <td style="text-align:right"><?= number_format($tot_target,0,'.',',') ?></td>
      <td style="text-align:right"><?= number_format($tot_sale,0,'.',',') ?></td>
      <td style="text-align:right"><?= number_format($tot_due,0,'.',',') ?></td>
    </tr>
  </tbody>
</table>
<? 
...
<table width="70%" cellspacing="0" cellpadding="2" border="0" align="center">
  <thead>
    <tr>
	 ...
	 <tr>
      <td style="border:0px;" colspan="6" align="center"><h2>Production Target Report<br />Date Interval : <?=$_POST['f_date']?> To <?=$_POST['t_date']?> </h2></td>
    </tr>
  ...
  <tbody>
   ...
   $startDate = $_POST['f_date'];
   $endDate = $_POST['t_date'];
   $d_con .= " and  d.start_date between '$startDate' and '$endDate' ";
   $s_con .= " and  m.do_date between '$startDate' and '$endDate'";
  }

     $sql = "select d.*,i.item_name,w.warehouse_name as project_name from production_target d, item_info i, warehouse w where w.warehouse_id=d.project_id and i.item_id=d.item_id ".$d_con." ";
...
?>
    <tr>
      <td><div align="center"><?=++$j?></div></td>
	   <td><?=$data3->project_name?></td>
      <td><div align="center"><?=$data3->lot_no?></div></td>
	  <td><div align="left"><?=$data3->item_name;?></div></td>
      <td><div style="text-align:right"><?=$data3->qty; $total_qty = $total_qty+$data3->qty;?></div></td>
    <td style="text-align:right"><?=$data3->details;?></td>
...
<table width="100%" cellspacing="0" cellpadding="2" border="0" align="center">
  <thead>
    <tr>
      <td style="border:0px;" colspan="13" align="center"><h1><?=find_a_field('user_group','group_name','id="'.$_SESSION['user']['group'].'"')?></h1></td>
    </tr>

	 <tr>     
	  <td style="border:0px;" colspan="13" align="center"><h2>Undelivered Do Details Report<br />Date Interval : <?=$_POST['f_date']?> To <?=$_POST['t_date']?> </h2></td>
    </tr>
    ...
    <tr>
      <th><div align="center">S/L</div></th>
	  <th>Do No</th>
      <th><div align="center">Do Date</div></th>
	  <th>Dealer Name</th>
      <th>Depot</th>
      <th>Item Name</th>
	  <th>Crt</th>
...
//  $sql="select sum(d.total_amt) as total_amt,count(m.do_no) as total_do,m.marketing_person ,p.PBI_NAME, sum(t.target_amount) as target_amount
//  from sale_do_master m,sale_do_details d, sales_target t, personnel_basic_info p
//  where  m.do_no=d.do_no and t.sales_person=p.PBI_ID and p.PBI_ID = m.marketing_person  ".$d_con." group by t.sales_person";

    //$sql = " select sum(t.target_amount) as target_amount, p.PBI_NAME,t.sales_person 
//    from sales_target t, personnel_basic_info p 
//    where t.sales_person=p.PBI_ID ".$d_con." group by t.sales_person";

 $query = db_query($sql_undel);


while($data3=mysqli_fetch_object($query)){$s++;
 

 $delivered_qty = find_a_field('sale_do_chalan','sum(total_unit)','do_no='.$data3->do_no.' and  item_id='.$data3->item_id.' ');
 $un_delivered_qty = ($data3->total_unit-$delivered_qty);
?>
    ...
    <tr>
      <td><div align="center"><?=++$j?></div></td>
	   <td><?=$data3->do_no?></td>
	   <td><?=$data3->do_date;?></td>
      <td><div align="center"><?=$data3->dealer_name?></div></td>
      <td><div style="text-align:right"><?=$data3->depot?></div></td>
    <td style="text-align:left"><?=$data3->item_name;?></td>
    <td style="text-align:right"><?=$data3->crt;?></td>
	<td style="text-align:right"><?=number_format($pcs = $data3->pcs,0)?></td>
	<td style="text-align:right"><?=number_format($data3->total_unit,0)?></td>
	<td style="text-align:right"><?=number_format($delivered_qty,0)?></td> 
	<td style="text-align:right"><?=number_format($un_delivered_qty,0)?></td>
	<td style="text-align:right"><?=number_format($data3->total_amt,0)?></td>
	<td style="text-align:right"><?=number_format($data3->rcv_amt,0)?></td>
