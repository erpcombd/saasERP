...
$sql = 'select sum(cr_amt) as total_receive from journal where tr_from not in ("LFOpening") and ledger_id="'.$customer_ledger.'" and jv_date between "'.$fdate.'" and "'.$tdate.'"';
$qry = db_query($sql);
...
$sql = 'select ledger_id, sum(dr_amt) as opening from journal where tr_from="MCOpening" group by ledger_id';
$qry = db_query($sql);
...
$sql = 'select * from service_customer where  status="Active" '.$con.$con2.'';
$qry = db_query($sql);
...
<tr>
 <td><?=++$sl?></td>
 <td><?=$data->customer_name?></td>
 <td><?=($mc[$data->customer_id]>0)? number_format($mc[$data->customer_id],2):'';$total_receivable +=$mc[$data->customer_id];?></td>
...
<tr>
 <td></td>
 <td></td>
 <td align="right"><strong><?=number_format($grand_receivable,2)?></strong></td>
