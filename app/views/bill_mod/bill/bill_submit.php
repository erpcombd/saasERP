...
$sqql = 'select b.*,sum(b.service_charge) as total_service_charge,c.short_name from bill_assign b,service_customer c where b.customer=c.customer_id and b.billing_cycle<="'.$today.'" and b.cycle="monthly" and b.status="Active" group by b.customer order by b.customer';
...
$_POST['amount'] = $data->total_service_charge;
$_POST['net_receivable_amt'] = $data->total_service_charge;

$master_id=$crud->insert();

$ssqqll = 'select * from bill_assign where customer="'.$data->customer.'" and billing_cycle="'.$i.'" and cycle="monthly" order by customer';

$qqrryy=db_query($ssqqll);

while($details=mysqli_fetch_object($qqrryy)){

$insert = 'insert into bill_details(`bill_no`,`customer`,`service_type`,`service_charge`) value("'.$master_id.'","'.$details->customer.'","'.$details->service_id.'","'.$details->service_charge.'")';

db_query($insert);
$next_billing_cycle = date("Y-m-d", strtotime("+1 month", strtotime($i)));
$billing_cycle_update = 'update bill_assign set billing_cycle="'.$next_billing_cycle.'",bill_count="'.$max_count.'" where id="'.$details->id.'"';
...
$sqql = 'select b.*,c.short_name from bill_assign b,service_customer c where b.customer=c.customer_id and b.billing_cycle<="'.$today.'" and b.cycle="license" and b.billed=0 group by b.customer order by b.customer';
...
$ssqqll = 'select * from bill_assign where customer="'.$data->customer.'" and billing_cycle<="'.$today.'" and cycle="license" and billed=0 order by customer';
...
$max_count = find_a_field('bill_assign','max(bill_count)+1','id="'.$details->id.'"');
...
$billing_cycle_update = 'update bill_assign set bill_count="'.$max_count.'",billed=1 where id="'.$details->id.'"';
...
$sql = "select b.bill_no, d.customer_name, b.bill_date, b.manual_bill_no, b.status,b.customer,b.service_type,t.type as type_name,t.ledger_id as credit_ledger from bill_info b, service_customer d,acc_bill_type t where b.customer=d.customer_id and b.status='PENDING' and b.customer='".$_SESSION['new_biller']."' and b.service_type=t.id";
...
if($_POST['check'.$data->bill_no]=='checked'){
		...
		$bill_amount  = find_a_field('bill_details','sum(service_charge)','bill_no="'.$data->bill_no.'"'); 
...
$update = 'update bill_info set status="BILL SUBMITTED",discount_amt="'.$discount.'",net_receivable_amt="'.$net_receivable.'" where bill_no="'.$data->bill_no.'"';
		...
		<form action="bill_submit.php" method="post" name="codz" id="codz">
				...
				<input   name="bill_date" type="text" id="bill_date"  value="<?=($bill_date!='')?$bill_date:date('Y-m-d')?>"   required autocomplete="off" />
							...
							<div class="form-group row m-0 pb-1">
								...
								<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
									...
									<? $dealer_data = find_all_field('service_customer','','customer_id='.$_SESSION['new_biller']);
							...
							<div class="form-group row m-0 pb-1">
								...
								<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
									...
									<select name="service_type" id="service_type">
										...
										<? foreign_relation('acc_bill_type','id','type',$service_type,'1')?>
			...
			<div class="container-fluid pt-3 p-0">
				...
				<table  id="grp"  class="table1  table-striped table-bordered table-hover table-sm">
					...
					$sql = "select b.bill_no,d.customer_name, b.bill_date, b.manual_bill_no, b.amount, b.status,b.service_type from bill_info b, service_customer d where b.customer=d.customer_id and b.status='PENDING' and b.customer='".$_SESSION['new_biller']."'";
					...
					while($data=mysqli_fetch_object($query)){$i++;
						...
						$service_type = find_a_field('acc_bill_type','type','id="'.$data->service_type.'"');
