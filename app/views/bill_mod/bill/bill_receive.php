...
if(isset($_REQUEST['confirmit']))
{
        ...
        add_to_sec_journal($proj_id, $jv_no, $jv_date, $customer_ledger->ledger_id, $narration,'0', $receipt_amt, $tr_from, $tr_no,'',$tr_id=0,$cc_code,$group_for);
		if($discount>0){
 ...
 $receive_journal_amt = find_a_field('secondary_journal','sum(cr_amt)','ledger_id="'.$data->ledger_id.'" and tr_no="'.$data->bill_no.'" and tr_from="'.$tr_from.'"');
...
function tot_check(id){
	var receipt_amt = (document.getElementById('receipt_amt_'+id).value)*1;
	var tot_due_amt = (document.getElementById('tot_due_amt_'+id).value)*1;
	var tax = (document.getElementById('tax_'+id).value)*1;
	var discount = (document.getElementById('discount_'+id).value)*1;
	var adjust = tax+discount;
	
	var net_receipt = receipt_amt - adjust;
	
	if(net_receipt>tot_due_amt){
        alert('Overflow!');
        document.getElementById('receipt_amt_'+id).value = '';
		document.getElementById('tax_'+id).value = '';
...
/*}*/
	...
	<div class="form-container_large">
								...
								<div class="form-group row m-0 pb-1">
									...
									<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
										...
										<span id="cash_bank_filter">
											...
											<select name="dr_ledger_id" id="dr_ledger_id" required>
        ...
        //$sql = 'select sum(cr_amt) from journal where ledger_id="'.$data->ledger_id.'" and tr_no="'.$data->bill_no.'" and tr_from="'.$data->type_name.' Receive"';
		$received_amt = find_a_field('journal','sum(cr_amt)','ledger_id="'.$data->ledger_id.'" and tr_no="'.$data->bill_no.'" and tr_from="'.$data->type_name.' Receive"');
		 //echo $sql = 'select sum(cr_amt) from journal where ledger_id="'.$data->ledger_id.'" and tr_no="'.$data->bill_no.'" and tr_from="'.$data->type_name.' Receive"';
						...
						<tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
							...
							<td>
							    <!--<a href="invoice_print_view.php?manual_bill_no=<?=$data->manual_bill_no?>" target="_blank"><span class="style13"><?=$data->manual_bill_no?></span></a>-->
							    <a href="invoice_print_view.php?manual_bill_no=<?= urlencode($data->manual_bill_no) ?>" target="_blank"><span class="style13"><?=$data->manual_bill_no?></span></a>
							    <a title="WO Preview" target="_blank" href="../../../sales_mod/pages/wo/work_order_print_view.php?v_no=<?=$data->manual_bill_no?>"></a>
							</td>
							...
							<td><input name="discount_<?=$data->bill_no?>" id="discount_<?=$data->bill_no?>" type="text" value="" onkeyup="sum_sum(<?=$data->bill_no?>)"  /></td>
							<td><input name="tax_<?=$data->bill_no?>" id="tax_<?=$data->bill_no?>" type="text" value="" onkeyup="sum_sum(<?=$data->bill_no?>)"  /></td>
							<td>
								...
								<input name="receipt_amt_<?=$data->bill_no?>" id="receipt_amt_<?=$data->bill_no?>" type="text" onkeyup="tot_check(<?=$data->bill_no?>)" value="" />	</td>
