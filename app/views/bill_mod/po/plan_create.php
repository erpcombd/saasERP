...

{
		 ...
		 $sqlll = "update daily_progress_master set status='CHECKED' where d_id=".$_POST['master_id']."";
		db_query($sqlll);
		...
		header('Location:select_pending_po.php');
...
<input  name="date" type="text" id="date" value="<?=$date?>" style="width:98%;"  /></td>
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  $res='select a.id, a.date, a.day, a.customer_name, a.sales_order as "order", a.sales, a.delivery, a.gap_analysis, a.collection, a.outstanding, "x" from daily_plan_details a where  a.d_id='.$d_id; 
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  $res='select a.id, a.date, a.day, a.customer_name, a.plan, a.man_power as Assigned, a.progress, a.gap_analysis, "x" from daily_plan_details a where  a.d_id='.$d_id; 
	echo link_report_add_del_auto($res,'',0,0);
?>
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  $res='select a.id, a.date, a.day as Time, a.customer_name as Adress, a.plan as Work_Details, a.progress as Progress, a.gap_analysis as Problem, "x" from daily_plan_details a where  a.d_id='.$d_id; 
	echo link_report_add_del_auto($res,'',0,0);
?>
...
<? } elseif($progress_for=='4'){ ?>
		<table  width="100%" border="1" align="left"  style="border-collapse:collapse; border:1px solid #caf5a5;" cellpadding="2" cellspacing="2">
					...
					<input  name="progress_for" type="hidden" id="progress_for" value="<?=$progress_for?>"/>
					<input  name="progress_date" type="hidden" id="progress_date" value="<?=$progress_date?>"   />
					<input name="date" type="text" class="input3" id="date" style="width:98%;" /></td>	
...
<? }?>
  ...
  <table width="100%" border="0">
    ...
    <tr>
      ...
      <input name="master_id"  type="hidden"  value="<?=$master_id?>"  />
