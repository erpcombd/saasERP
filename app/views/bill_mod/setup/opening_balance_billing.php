...
$j_sql="select ledger_id,cr_amt,dr_amt,narration from journal where tr_from in ('Opening') and group_for ='".$_SESSION['user']['group']."' and ledger_id like '1%' or ledger_id like '2%'";
...
var tr_from=(document.getElementById('tr_from_'+id).value); 

var flag=(document.getElementById('flag_'+id).value); 



var strURL="opening_balance_billing_ajax.php?item_id="+item_id+"&cr="+cr+"&dr="+dr+"&opdate="+opdate+"&narration="+narration+"&flag="+flag+"&tr_from="+tr_from;
			...
			<div class="row">
				...
				<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9">
					<div class="form-group row m-0">
						...
						foreign_relation('ledger_group','group_id','group_name',$_POST['group_id'],'group_for="'.$_SESSION['user']['group'].'" and group_id=124003');
...
?>
			...
			<table class="table1  table-striped table-bordered table-hover table-sm">
				...
				<tbody class="tbody1">
					...
					$narrations=$narration[$data[0]];
					 ...
					 //$dr_amt = find_a_field('journal','sum(dr_amt)','ledger_id="'.$data[0].'" and tr_from in ("MCOpening","LFOpening")');
					 //$cr_amt = find_a_field('journal','sum(cr_amt)','ledger_id="'.$data[0].'" and tr_from in ("MCOpening","LFOpening")');
	  ...
	  <? echo $data[1];?></td>
					...
					<td><input type="text" name="narration_<?=$data[0]?>" id="narration_<?=$data[0]?>" value="<?
					$mcopening = find_a_field('journal','dr_amt','ledger_id="'.$data[0].'" and tr_from="MCOpening"');
					$lfopening = find_a_field('journal','dr_amt','ledger_id="'.$data[0].'" and tr_from="LFOpening"');
					if($mcopening>0) echo 'MCOpening('.$mcopening.'). ';
					if($lfopening>0) echo 'LFOpening('.$lfopening.') ';
					?>" /></td>
					

					<td><select name="tr_from_<?=$data[0]?>" id="tr_from_<?=$data[0]?>"><option>MCOpening</option><option>LFOpening</option></select></td>
					<td>
