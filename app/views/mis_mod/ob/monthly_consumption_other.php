...
<table width="80%" border="0" align="center">
  ...
  <tr>
	...
	<option></option>
      <?
foreign_relation('item_group','group_id','group_name',$_POST['group'],'1 and  group_type="OTHER" order by status');
?>
        </select></td>
...
<?
$sql = "select sum(item_in)-sum(item_ex) qty, i.item_id from journal_item j, item_info i where i.item_id=j.item_id and  j.warehouse_id='".$_SESSION['user']['depot']."' and j.ji_date<='".$_POST['odate']."' and j.tr_from!='OPENING-1909' and sub_group_id=".$_POST['sub_group']." group by i.item_id ";
$query = db_query($sql);
while($data = mysqli_fetch_object($query)){
$pre_stock[$data->item_id] = $data->qty;
}

$sql = "select item_price,item_in-item_ex as unit_qty,i.item_id from journal_item j, item_info i where i.item_id=j.item_id and  j.warehouse_id='".$_SESSION['user']['depot']."' and j.ji_date='".$_POST['odate']."' and j.tr_from='OPENING-1909' and sub_group_id=".$_POST['sub_group']." group by i.item_id ";
...
while($data=mysqli_fetch_object($query)){$i++;
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
    ...
    <td><input name="orate1_<?=$data->item_id?>2" id="orate1_<?=$data->item_id?>2" value="" style="width:70px;"/></td>
    <td><input name="orate1_<?=$data->item_id?>" id="orate1_<?=$data->item_id?>" value="<?=($_SESSION['user']['depot']!=5&&$adj_rate[$data->item_id]==0)?$data->f_price:number_format($adj_rate[$data->item_id],4);?>" style="width:70px;"/></td>
<td><input name="oqty_<?=$data->item_id?>" id="oqty_<?=$data->item_id?>" type="text" size="10" maxlength="10" value="<?=($pre_stock[$data->item_id]+$adj[$data->item_id])?>" style="width:60px;" /></td>
