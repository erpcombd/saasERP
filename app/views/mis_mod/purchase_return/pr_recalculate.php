...
if($chalan_no>0)
{
     $sec_del = db_query("insert into secondary_journal_del select * from secondary_journal where tr_from in ('".$type."') and tr_no=".$chalan_no."");
	 $sql= "DELETE FROM `secondary_journal` WHERE tr_from in ('".$type."') and  tr_no=".$chalan_no."";
	 $sql3 = db_query($sql);
	
	$j_del = db_query('insert into journal_del select * from journal where tr_from in ("'.$type.'") and tr_no='.$chalan_no.'');
	$sql3 = db_query("DELETE FROM `journal` WHERE tr_from in ('".$type."') and  tr_no=".$chalan_no."");
	auto_insert_purchase_return_secoundary_toriqul($chalan_no);
