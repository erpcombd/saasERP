...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0">
  ...
  <?
  $sql = "select * from item_info where sub_group_id=".$_POST['sub_group']." order by item_name limit 5  ";
  $query = db_query($sql);
  while($data=mysqli_fetch_object($query)){$i++;
  
$stock = find_a_field('journal_item','sum(item_in)-sum(item_ex)','warehouse_id="'.$_SESSION['user']['depot'].'" and item_id = "'.$data->item_id.'" and ji_date<"'.$_POST['odate'].'"');
$ssssql = @db_query('select item_price,sum(item_in)-sum(item_ex) as unit_qty from journal_item where warehouse_id="'.$_SESSION['user']['depot'].'" and item_id = "'.$data->item_id.'" and tr_from="Adjustment" and ji_date="'.$_POST['odate'].'"');
...
$adjustment_rate = $sss->item_price;
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
    ...
    <td><input name="orate1_<?=$data->item_id?>" id="orate1_<?=$data->item_id?>" value="<?=($adjustment_rate)?>" style="width:70px;"/></td>
