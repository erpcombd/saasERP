...
if(isset($_POST['confirm'])){
		...
		while($data=mysqli_fetch_object($query))
		{
			if(($_POST['chalan_'.$data->id]>0))
			{
				...
				$amount = ($qty*$rate);
				$total = $total + $amount;
$q = "INSERT INTO `purchase_receive` (`pr_no`, `po_no`, `order_no`, `rec_date`, `vendor_id`, `item_id`, `warehouse_id`, `rate`, `qty`, `unit_name`, `amount`, `qc_by`, `entry_by`, `entry_at`,ch_no) VALUES('".$pr_no."', '".$po_no."', '".$data->id."', '".$rec_date."',".$vendor_id.", ".$item_id.",".$warehouse_id.", ".$rate.", '".$qty."', '".$unit_name."',  '".$amount."', '".$qc_by."',  '".$_SESSION['user']['id']."', '".$now."', '".$ch_no."')";
				db_query($q);
$xid = db_insert_id();
...
{
$item_purchase_receive_head = find_a_field('warehouse','ledger_id',"warehouse_id=".$warehouse_id);
}
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" align="center">
  <tr>
    ...
    <? $field='po_subject';?>
      <div>
        <label style="width:85px;" for="<?=$field?>">Subject:</label>
      ...
      <div>
	  <label style="width:85px;" for="<?=$field?>">Vendor:</label>
      <input  name="<?=$field?>" type="text" id="<?=$field?>" value="<?=find_a_field($table,$show_field,$get_field.'='.$vendor_id)?>"/>
      </div>
              <? $field='delivery_within';?>
		<div>
		<label style="width:85px;" for="<?=$field?>">Delivery By:</label>
		<input  name="<?=$field?>" type="text" id="<?=$field?>" value="<?=$$field?>" style="width:75px;" required/>
		days</div>
      <div>
...
?>
  ...
  <tr>
    <td colspan="5" valign="top"><table width="80%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
      ...
      <tbody>
          ...
          <tr bgcolor="<?=(($bg%2)==1)?'#FFEAFF':'#DDFFF9'?>">
            ...
            <td><?=$row->item_id?>
			  ...
			  <? if($unrec_qty>0){$cow++;?>
                <input name="chalan_<?=$row->id?>" type="text" id="chalan_<?=$row->id?>" style="width:70px; float:none" value="" required="required" />
                <? } else echo 'Done';?></td>
              </tr>
          <? }?>
      </tbody>
