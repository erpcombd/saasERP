...
if(isset($_POST['add'])&&($_POST[$unique_master]>0))
{
		...
		if($_SESSION['line_id'] == 5)
		$_POST['unit_pricer']= find_a_field('item_info','p_price','item_id='.$_POST['item_id']);
		else
		$_POST['unit_pricer']= find_a_field('item_info','f_price','item_id='.$_POST['item_id']);
		}
		$_POST['total_amt']= ($_POST['total_unit'] * $_POST['unit_price']);

		$xid = $crud->insert();
		journal_item_control($_POST['item_id'] ,$_SESSION['user']['depot'],$_POST['pi_date'],0,$_POST['total_unit'],'Issue',$xid,$_POST['unit_pricei'],$_SESSION['line_id'],$_POST['remarks']);
		journal_item_control($_POST['item_id'] ,$_SESSION['line_id'],$_POST['pi_date'],$_POST['total_unit'],'0','Issue',$xid,$_POST['unit_pricer'],$_SESSION['user']['depot'],$_POST['remarks']);
...
<table  width="100%" border="1" align="left"  style="border-collapse:collapse; border:1px solid #caf5a5;" cellpadding="2" cellspacing="2">
  <tr>
    ...
    <td align="center" bgcolor="#0099FF"><strong>Crt</strong></td>
  ...
  <tr>
    <td align="center" bgcolor="#CCCCCC"><input  name="<?=$unique_master?>" type="hidden" id="<?=$unique_master?>" value="<?=$$unique_master?>"/>
        ...
        <input  name="item_id" type="text" id="item_id" value="<?=$item_id?>" style="width:300px;" required="required" onblur="getData2('production_receive_ajax_fg.php', 'pr', this.value, document.getElementById('warehouse_from').value);"/>
    ...
    <td colspan="4" align="center" bgcolor="#CCCCCC"><span id="pr">
      <input name="total_unit2" type="text" class="input3" id="total_unit2"  maxlength="100" style="width:50px;" required="required"/>
      ...
      <input name="total_unit5" type="text" class="input3" id="total_unit5"  maxlength="100" style="width:80px;" required="required"/>
    </span></td>
    <td align="center" bgcolor="#CCCCCC"><input name="total_unit" type="hidden" class="input3" id="total_unit"  maxlength="100" style="width:40px;" required="required"/>
        <input name="total_pkt" type="text" class="input3" id="total_pkt"  maxlength="100" style="width:40px;" required="required"/></td>
    <td align="center" bgcolor="#CCCCCC"><input name="total_pcs" type="text" class="input3" id="total_pcs"  maxlength="100" style="width:40px;" required="required" value="0"/></td>
  </tr>
</table>
<br /><br /><br /><br />
...
$res='select a.id,b.finish_goods_code as code,b.item_name,FLOOR(a.total_unit/b.pack_size) as pkt_qty,a.total_unit%b.pack_size as pcs_qty,"X" from production_issue_detail a,item_info b where b.item_id=a.item_id and a.pi_no='.$$unique_master.' order by a.id';
?>
...
<form action="select_prodiction_line_i.php" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
