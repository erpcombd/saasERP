...
switch ($_POST['report']) {
	...
	break;
		...
		i.unit_name as unit,
		a.item_in as qty,
		i.d_price as cost_price,
		(a.item_in*i.d_price) as Total_Amt,
		(a.item_in*i.d_price) as DP_Total
		...
		sum(a.item_in*i.d_price)/sum(a.item_in) as rate,
		sum(a.item_in*i.d_price) as sale_Amt
		from journal_item a, item_info i, user_activity_management c,warehouse w  where w.use_type="SD" and a.item_in>0 and a.relevant_warehouse='.$_SESSION['user']['depot'].' and 
		...
		case 7:
		$report="Chalan Wise Issue Report";
...
m.pi_no, m.pi_date,  w.warehouse_name as Depot, m.remarks as sl_no, m.carried_by,
		...
		from journal_item a, item_info i, user_activity_management c,warehouse w,production_issue_master m,production_issue_detail d  where w.use_type="SD" and a.item_in>0 and a.relevant_warehouse='.$_SESSION['user']['depot'].' and 
		d.id=a.tr_no and d.pi_no=m.pi_no and w.warehouse_id=a.warehouse_id and (a.tr_from="Issue" or a.tr_from="Transfered" or a.tr_from="Transit") and c.user_id=a.entry_by and a.item_id=i.item_id'.$con.' group by d.pi_no order by a.id';
		...
		d.id=a.tr_no and d.pi_no=m.pi_no and w.warehouse_id=a.relevant_warehouse and (a.tr_from="Issue" or a.tr_from="Transfered" or a.tr_from="Transit") and c.user_id=a.entry_by and a.item_id=i.item_id'.$date_con.$warehouse_con.$item_con.$item_brand_con.' group by d.id order by a.id';
...
m.pi_no, m.pi_date,  w.warehouse_name as Depot, m.remarks as sl_no, m.carried_by,
		sum(a.item_in*i.d_price) as amt
		from journal_item a, item_info i, user_activity_management c,warehouse w,production_issue_master m,production_issue_detail d  where a.item_in>0 and a.warehouse_id='.$_SESSION['user']['depot'].' and 
		d.id=a.tr_no and d.pi_no=m.pi_no and w.warehouse_id=a.relevant_warehouse and (a.tr_from="Issue" or a.tr_from="Transfered" or a.tr_from="Transit") and c.user_id=a.entry_by and a.item_id=i.item_id'.$con.' group by d.pi_no order by a.id';
...
$s='select sum(a.item_in-a.item_ex) as final_stock,sum((a.item_in-a.item_ex)*(a.item_price)) as Stock_price  
...
$sql='select distinct i.item_id,i.unit_name,i.item_name,s.sub_group_name,i.finish_goods_code,i.pack_size,i.p_price as item_price
		   from item_info i, item_sub_group s where i.product_nature = "Salable" and 
...
<td><?=$data->item_name?></td>
<td><?=$data->unit_name?></td>
<td style="text-align:right; background-color:#CCFFFF;"><?=floor($i->final_stock/$data->pack_size)?></td>
...
<td style="text-align:right"><?=number_format(($data->item_price*$i->final_stock),2)?></td>
</tr>
<?
...
if(isset($t_date)) 
		...
		$sql="select i.item_id,i.item_name,i.sales_item_type,i.pack_size,i.finish_goods_code,i.sales_item_type from item_info i where i.finish_goods_code>0 order by i.item_id";
		$table = 'journal_item';
