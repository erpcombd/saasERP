...
if(isset($_POST['confirm'])){
		...
		while($data=mysqli_fetch_object($query))
		{
			if(($_POST['chalan_'.$data->id]>0)||($_POST['chalan2_'.$data->id]>0))
			{
				...
				VALUES 
				('".$data->id."', '".$chalan_no."', '".$do_no."', '".$data->item_id."', '".$dealer_code."', '".$data->unit_price."', '".$data->pkt_size."', '".$chalan_pkt."', '".$chalan_dist."', '".$unit_qty."', '".$total_amt."', '".$chalan_date."', '".$_SESSION['user']['depot']."', '".$driver_name."','".$driver_name_real."', '".$vehicle_no."', '".$delivery_man."', '".$_SESSION['user']['id']."', '".$now."')";
				db_query($q);
				$xid = db_insert_id();
				journal_item_control($data->item_id ,$_SESSION['user']['depot'],$_POST['chalan_date'],0,$unit_qty,'Sales',$xid);
			}
		}
		if($xid>0)
		{
      ...
      <td colspan="5"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td bgcolor="#9966FF"><strong>
           <select name="chalan_date" id="chalan_date" required="required">
			<? select_dates('7','0','');?>
			</select>
          </strong></td>
