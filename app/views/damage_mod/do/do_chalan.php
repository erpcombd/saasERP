...
if(isset($_POST['confirm'])){
		...
		$dealer_ledger= find_a_field('dealer_info','account_code',"dealer_code=".$_POST['dealer_code']);
...
$q = "INSERT INTO  sale_do_chalan (order_no, chalan_no, do_no, item_id, dealer_code, unit_price, pkt_size, pkt_unit, dist_unit, total_unit, total_amt, chalan_date, depot_id, driver_name, driver_name_real, vehicle_no, delivery_man, entry_by, entry_at)
 VALUES 
('".$data->id."', '".$chalan_no."', '".$do_no."', '".$data->item_id."', '".$dealer_code."', '".$data->unit_price."', '".$data->pkt_size."', '".$chalan_pkt."', '".$chalan_dist."', '".$unit_qty."', '".$total_amt."', '".$chalan_date."', '".$data->depot_id."', '".$driver_name."','".$driver_name_real."', '".$vehicle_no."', '".$delivery_man."', '".$_SESSION['user']['id']."', '".$now."')";
				db_query($q);
				$ch_id = db_insert_id();
				journal_item_control($data->item_id,$data->depot_id,$_POST['chalan_date'],0,$unit_qty,'Sales',$ch_id,'','',$chalan_no);
    ...
    <tr>
      ...
      <td><fieldset style="width:300px;">
        <div>
          ...
          <input name="do_date" type="text" id="do_date" style="width:215px;" value="<? if($$unique_master>0) echo (find_a_field($table_master,'do_date','do_no='.$do_no));?>" tabindex="10" readonly="readonly" />
        </div>
        <div>
      ...
      <td colspan="5"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td bgcolor="#9966FF"><strong>
			<input style="width:105px;"  name="chalan_date" type="text" id="chalan_date" required="required" value="<?=date('Y-m-d')?>"/>
          </strong></td>
...
<? 
$sql='select a.id,a.item_id,b.finish_goods_code,b.item_name,a.pkt_unit,a.dist_unit,a.total_unit,a.pkt_size from sale_do_details a,item_info b where b.item_id=a.item_id and a.do_no='.$$unique_master;
$res=db_query($sql);
?>
          ...
          <tr bgcolor="<?=(($bg%2)==1)?'#FFEAFF':'#DDFFF9'?>">
			...
			<input type="hidden" name="item_fg_<?=$row->id?>" id="item_fg_<?=$row->id?>" value="<?=$row->finish_goods_code?>" />
              ...
              <td align="center" bgcolor="#66FF66" style="text-align:center">
				...
				<input type="text" name="chalan_<?=$row->id?>" id="chalan_<?=$row->id?>" required style="width:40px; float:none" onchange="cal(<?=$row->id?>)" /><? } else echo 'Done';?></td>
              <td align="center" bgcolor="#6699FF" style="text-align:center">
			  <? if($undel_qty>0){$cow++;
			  if($dealer->product_group==''||$row->finish_goods_code>1999){
			  ?>
<input name="chalan2_<?=$row->id?>" type="text" id="chalan2_<?=$row->id?>" style="width:40px; float:none" value="0" required="required" onchange="cal2(<?=$row->id?>)" />
                <? }else echo '-';} else echo 'Done';?></td>
              </tr>
          <? }?>
		    <tr>
            <td colspan="9">&nbsp;</td>
