...
$res='select m.pi_date,m.pi_no,m.pi_date, w.warehouse_name, m.batch_type, m.status from production_floor_issue_master m, warehouse w where m.warehouse_to=w.warehouse_id '.$con.' and m.status="PENDING" group by m.pi_date ';
...
<tbody>
<tr>
<td style="text-align: center;"><a href="chalan_view_edit_manual_pending.php?pi_date=<?=$row->pi_date?>&&warehouse_to=<?=$_POST['warehouse_to']?>&&batch_type=<?=$_POST['batch_type'];?>" target="_blank" style="display:block;">Pi Date : <?=$row->pi_date?></a></td>
</tr>
</tbody>
