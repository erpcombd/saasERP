...
$pi_date = $_POST['pi_date'];
 ...
 $select = 'select * from production_floor_issue_master where pi_date="'.$pi_date.'" and warehouse_to = "'.$_GET['warehouse_to'].'" and status="PENDING"';
...
while($row = mysql_fetch_object($queryy)){
...
//journal_item_control($row->item_id,12,$pi_date,$row->batch_qty,0,'Production Receive',$row->pi_no,$consump_rate,'',$row->pi_no);
...
//journal_item_control($row->item_id,37,$pi_date,$row->batch_qty,0,'Production Receive',$row->pi_no,$consump_rate,'',$row->pi_no);
...
$q = "UPDATE `production_floor_issue_master` SET `status`='UNCHECKED', approved_at='".$now."',unit_price='".$consump_rate."', approved_by='".$_SESSION['user']['id']."'  where pi_no=".$row->pi_no;
...
//		$final_amt =find_a_field('production_floor_issue_detail','sum(total_amt)','pi_no='.$row->pi_no);




//			$pi_date_1 = strtotime($pi_date);



//	  $test="INSERT INTO `journal` (`id`, `proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `user_id`, `tr_id`, `group_for`, `entry_at`) VALUES ( NULL, 'Alinfoods', '".$jv."', '".$pi_date_1."', '".$dr_ledger."', '(PI#".$row->pi_no.") sub group id-".$sub_group_id." item id-".$row->item_id."', '".$final_amt."', '0.00', 'Production Receive', '".$row->pi_no."', '', '0', '".$_SESSION['user']['id']."', '', '2', '".date('Y-m-d H:i:s')."')";
...
//	$qu=mysql_query($cr_select);


//	while($roww=mysql_fetch_object($qu)){



//		mysql_query("INSERT INTO `journal` (`id`, `proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `user_id`, `tr_id`, `group_for`, `entry_at`) VALUES ( NULL, 'Alinfoods', '".$jv."', '".$pi_date_1."', '".$roww->ledger_id_1."', '(PI#".$row->pi_no.") sub group id-".$sub_group_id." item id-".$roww->item_id."', '0.00','".$roww->total_amt."',  'Production Receive', '".$row->pi_no."', '', '0', '".$_SESSION['user']['id']."', '', '2', '".date('Y-m-d H:i:s')."')");
...
$select = 'select i.item_name, i.unit_name,m.pi_no,m.batch_ctn,m.batch_pcs,w.warehouse_name,m.remarks,m.batch_type from production_floor_issue_master m, item_info i, warehouse w where m.warehouse_to=w.warehouse_id and  m.item_id=i.item_id and m.status="PENDING" and m.pi_date="'.$_GET['pi_date'].'" '.$batch.''.$warehouse.'';
