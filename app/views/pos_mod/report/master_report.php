...
if(isset($_SESSION['user']['group']))
	$str 	.= '<img width="50%" src="../../../logo/'.$_SESSION['user']['group'].'.png'.'">';
$str 	.= '</th>';
...
<th>Unit  </th>
<th>Units in CTN </th>
<th bgcolor="#5390F5"><div align="center">MRP Price</div></th>
...
elseif($_POST['report']==2019)

{		if($item_id>0) 				{$item_con=' and i.item_id='.$item_id;}



		$report="Product Barcode Report";
		...
		<tr>
		  ...
		  <td style="border:2px solid #000">SL</td>

		  <td style="border:2px solid #000">Item Name</td>

		  <td style="border:2px solid #000">Barcode</td>

		</tr>

        <?

		  $barsql = 'select i.item_name,i.item_id,i.finish_goods_code from item_info i where 1 '.$item_con.'';

		  $barquery = mysql_query($barsql);

		  while($bardata = mysql_fetch_object($barquery)){



		  $barcode_content = $bardata->finish_goods_code;
		...
		<tr height="100">
		  ...
		  <td style="border:1px solid #333333">SL</td>

		  <td style="border:1px solid #333333"><?=$bardata->finish_goods_code.'-'.$bardata->item_name;?></td>

		  <td style="border:1px solid #333333">&nbsp;&nbsp;&nbsp;<?='<img class="barcode" alt="'.$barcodeText.'" src="barcode.php?text='.$barcodeText.'&codetype='.$barcodeType.'&orientation='.$barcodeDisplay.'&size='.$barcodeSize.'&print='.$printText.'" style="width:50%;"/>' ?>&nbsp;&nbsp;&nbsp;</td>
...
{		if($item_id>0) 				{$item_con=' and i.item_id='.$item_id;}
        ...
        <table width="100%" border="0" cellpadding="2" cellspacing="0" style="border:1px solid #000;">

        <?

		  $barsql = 'select i.item_name,i.item_id,i.finish_goods_code from item_info i where i.item_id="'.$_POST['item_id'].'"';
        ...
        <?
		   for($i=1; $i<=$bpq; $i++){
		  ?>
		<tr height="130" bordercolor="#000000">
          
		  
		  <td><div align="center">&nbsp;&nbsp;&nbsp;<?='<img class="barcode" alt="'.$barcodeText.'" src="barcode.php?text='.$barcodeText.'&codetype='.$barcodeType.'&orientation='.$barcodeDisplay.'&size='.$barcodeSize.'&print='.$printText.'" style="width:40%;"/>' ?>&nbsp;&nbsp;&nbsp;<br /><?=$bardata->finish_goods_code.'-'.$bardata->item_name;?></div></td>
