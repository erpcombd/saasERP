...
mysql_query("UPDATE `master_requisition_master` SET `status` = 'MANUAL' WHERE `req_no` = ".$req_no);
...
<td bgcolor="#FF9966"><input type="text" name="tdate" id="tdate" style="width:80px;" value="<?=$_POST['tdate']?>" /></td>
...
$res='select a.req_no,a.req_no,a.req_for,w.warehouse_name as req_for,a.manual_req_no , b.fname as entry_by ,a.entry_at,a.status from master_requisition_master a,user_activity_management b,warehouse w where a.status  in ("ISSUE RETURN") '.$con.'  and w.warehouse_id=a.req_for and b.user_id = a.entry_by ';
