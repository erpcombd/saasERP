...
<?
		...
		$str 	.= '<h1 style="text-align:center;">'.$_SESSION['company_name'].'</h1>';
		...
		$str 	.= '<h2 style="text-align:center;">'.$report.'</h2>';
		...
		$str 	.= '<h2 style="text-align:center;">Item Name: '.find_a_field('item_info','item_name','item_id='.$item_id).'</h2>';
		...
		$str 	.= '<h2 style="text-align:center;">Date Interval : '.$fr_date=date("d-m-Y").' To '.$to_date=date("d-m-Y").'</h2>';
		...
		$str 	.= '<h2 style="text-align:center;">Product Group : '.$product_group.'</h2>';
...
c.chalan_date,m.pi_no, m.do_no,m.do_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,a.AREA_NAME as area, d.team_name as team,w.warehouse_name as depot,d.product_group as grp,c.driver_name,m.cash_discount commission from 
...
where a.AREA_CODE=d.area_code and m.status in ('CHECKED','COMPLETED') and m.do_no=c.do_no and  m.dealer_code=d.dealer_code and w.warehouse_id=m.depot_id".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con.$product_team_con." order by c.chalan_no";
...
<thead><tr><td style="border:0px;" colspan="13">'.$str.'saeadfads</td></tr>
...
<th>Undelivery value</th>



<th>Actual Delivery value</th>
...
$tctn = 'select sum(pkt_unit) from sale_do_chalan   where chalan_no='.$data->chalan_no;
...
$tcn = mysql_fetch_row(mysql_query($tctn));
...
$com = ($info[0]*($data->commission/100));



$rec = $info[0] - ($info[0]*($data->commission/100));
...
echo '<tr class="footer"><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>'.number_format($total_ctnn,2).'</td><td>'.number_format($dp_t,2).'</td><td>'.number_format($com_t,2).'</td><td>'.number_format($rec_t,2).'</td></tr></tbody></table>';
...
$sql = "select concat(i.finish_goods_code,'- ',item_name) as item_name,c.chalan_type,c.pkt_unit as crt,c.dist_unit as pcs,c.total_amt as DP_Total,(o.t_price*c.total_unit) as TP_Total, FORMAT((c.total_amt-(c.total_amt*d.commission/100)),2) as Receiveable from 
