...
<option></option>
<? 
$company = $_SESSION['user']['group'];
foreign_relation_sql("select vendor_id,vendor_name from vendor where group_for='$company' order by vendor_name");?>
</select>
        </label></td>
...
?>
	<tr>
	  ...
	  <td width="15%"><?=number_format($info->qty,0)?></td>
	  <td width="15%"><? $rq = find_a_field('purchase_receive','sum(qty)','order_no="'.$info->id.'"'); echo number_format($rq,0);?></td>
	  <td width="15%"><? $dq = $info->qty - $rq; if($dq>0) echo number_format($dq,0);?></td>
	</tr>
<? }?>
</table>
	  </td>
