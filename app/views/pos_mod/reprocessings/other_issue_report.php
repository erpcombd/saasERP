...
$sql1="select b.* from warehouse_other_issue_detail b where b.oi_no = '".$oi_no."' and b.issue_type='Reprocess Issue'";
$data1=mysql_query($sql1);
...
<table width="800" border="0" cellspacing="0" cellpadding="0" align="center">
  <tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td bgcolor="#666666" style="text-align:center; color:#FFF; font-size:18px; font-weight:bold;">KIT Pack Report</td>
      </tr>
    </table></td>
              </tr>
            </table></td>
...
<?php $all_kit= find_a_field('warehouse_other_issue_detail','item_id','issue_type="Reprocess Receive" and oi_no='.$oi_no);
$kit_item=find_a_field('item_info','item_name','item_id='.$all_kit);
?>
<h2 style="text-align:center;">Kit Pack Receive </h2>
<table width="100%" class="tabledesign" border="1" bordercolor="#000000" cellspacing="0" cellpadding="5">
       <tr>
		...
		<td align="center" bgcolor="#CCCCCC"><strong>Rate should be</strong></td>
		
        </tr>
       
<?php 
 $kit_recsql='select * from warehouse_other_issue_detail where issue_type="Reprocess Receive" and oi_no='.$oi_no;
$kit_rec_query=mysql_query($kit_recsql);
while($row2=mysql_fetch_object($kit_rec_query)){
$item2 = find_all_field('item_info','','item_id='.$row2->item_id);
$re_issue_amt = find_a_field('warehouse_other_issue_detail','sum(amount)','oi_no="'.$oi_no.'" and issue_type="Reprocess Issue"');
$avg_rate = $re_issue_amt/$row2->qty;

?>
      <tr>
        <td align="center" valign="top"><?=++$i?></td>
        <td align="left" valign="top"><?=$item2->item_id?></td>
        <td align="left" valign="top"><?=$item2->finish_goods_code?></td>
        <td align="left" valign="top"><?=$item2->item_name?></td>
        <td align="right" valign="top"><?=$item2->unit_name?></td>
        <td align="right" valign="top"><?=$row2->qty?></td>
		<td align="right" valign="top"><?=$row2->rate?></td>
		<td align="right" valign="top"><?=$avg_rate?></td>
        </tr>
<? }?>
  </table>
  <br /><br />
  <h2 style="text-align:center;">Kit Pack Issue </h2>
<table width="100%" class="tabledesign" border="1" bordercolor="#000000" cellspacing="0" cellpadding="5">
       <tr>
        <td align="center" bgcolor="#CCCCCC"><strong>SL</strong></td>
        <td align="center" bgcolor="#CCCCCC"><strong>Code</strong></td>
