...
while($p_data=mysql_fetch_object($pquery)){
 	$p_price=number_format($p_data->item_price,2);
}*/
$warehouse_id = $data[1];
}
?>
<input name="unit_name" type="text" class="input3" id="unit_name" style="width:122px;float:left;" value="<?=$item_all->unit_name?>" readonly required onfocus="focuson('rate')"/>
<input name="rate" type="text" class="input3" id="rate"  maxlength="100" style="width:112px;float:left;" onchange="count()" value="<?=number_format($p_price,2,'.','')?>"   required/>