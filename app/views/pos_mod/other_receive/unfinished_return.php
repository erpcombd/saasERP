...
and "'.date('Y-m-d').'"';}
          ...
          $res ='select a.or_no,a.or_date,a.entry_by,a.requisition_from,a.approved_by,a.or_date,u.fname

  from warehouse_other_receive a, user_activity_management u

  where a.entry_by=u.user_id and a.warehouse_id="'.$_SESSION['user']['depot'].'" and a.status="MANUAL" '.$con.' order by a.or_no desc';
          ...
          while($data = mysql_fetch_object($query)){
            ...
            <tr>
              ...
              <td class="text-center"><?php echo date('d-m-Y',strtotime($data->or_date));?></td>
			  <td class="text-center"><?=$data->requisition_from?></td>
			  <td class="text-center"><?=$data->approved_by?></td>
             
              <td class="text-center">
                <a href="local_purchase.php?or_no=<?=$data->or_no?>">
                  <input type="button"  value="Complete OI"  class="btn1 btn1-bg-submit"/>
                </a>
              </td>
            </tr>
          <?php } ?>
...
and "'.date('Y-m-d').'"';}
  ...
  from warehouse_other_receive a,vendor d,user_activity_management u
