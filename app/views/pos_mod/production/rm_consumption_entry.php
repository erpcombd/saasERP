...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
			...
			$issue_no = next_transection_no($group_for,$issue_date,'rm_consumption','issue_no');
		...
		$sql = "SELECT i.item_id, i.item_name, i.unit_name FROM item_info i, item_sub_group s WHERE i.sub_group_id=s.sub_group_id  and s.group_id=100000000 order  by i.sub_group_id, i.item_id ";
...

		{
...

			{
				...
				$issue_qty=$_POST['issue_qty_'.$data->item_id];
				$avg_price=$_POST['avg_price_'.$data->item_id];
				$total_amt=$_POST['total_amt_'.$data->item_id];

   $ins_invoice = 'INSERT INTO rm_consumption (issue_no, issue_date, group_for, warehouse_id, item_id, unit_name, issue_qty, avg_price, total_amt, remarks, entry_at, entry_by)
  
  VALUES("'.$issue_no.'", "'.$issue_date.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$item_id.'", "'.$data->unit_name.'", "'.$issue_qty.'", "'.$avg_price.'", "'.$total_amt.'", "'.$remarks.'", "'.$entry_at.'", "'.$entry_by.'")';

mysql_query($ins_invoice);

$xid = mysql_insert_id();

journal_item_control($item_id, $warehouse_id, $issue_date,  0, $issue_qty, 'RmIssue', $xid, $avg_price, '', $issue_no, '', '', $group_for, $avg_price, '' );
//journal_item_control($item_id ,$warehouse_id,$issue_date,0,$issue_qty,'RmIssue',$xid,$avg_price);
...
if($issue_no>0)
{
//auto_insert_rm_onsumption_secoundary($issue_no);
}

header('location:rm_consumption_entry.php');
}
...
var issue_qty = document.getElementById('issue_qty_'+item_id).value*1;
var avg_price = document.getElementById('avg_price_'+item_id).value*1;
var total_amt = document.getElementById('total_amt_'+item_id).value= (issue_qty*avg_price).toFixed(2);
					...
					<div class="row">
						<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9">
							<div class="form-group row m-0">
								<label class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">ISSUE DATE</label>
								<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0">
									<input name="issue_date" type="text" id="issue_date"  value="<?=($issue_date!='')?$issue_date:date('Y-m-d')?>"   required />
								</div>
							</div>
					...
					<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 pt-1 pb-1">
						<div class="form-group row m-0">
							...
							<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">
								<input name="invoice_date" type="text" id="invoice_date"  readonly="" value="<?=$_POST['issue_date']?>"   required />
							</div>
						</div>
			...
			<? if(isset($_POST['submit'])){ ?>
				...
				<div class="container-fluid pt-5 p-0 ">
					<table class="table1  table-striped table-bordered table-hover table-sm">
						...
						$sql = "select item_id, sum(item_in-item_ex) as stock_qty from journal_item where ji_date<='".$_POST['issue_date']."'  group by item_id";
						$query = mysql_query($sql);
						while($data=mysql_fetch_object($query)){
						...
						while($data=mysql_fetch_object($query)){$i++;
							...
							<tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
								...
								<td>
									...
									<input name="issue_qty_<?=$data->item_id?>" id="issue_qty_<?=$data->item_id?>" type="text"  value=""  onkeyup="EXPcalculation(<?=$data->item_id?>)" />
								</td>
								<td><input name="avg_price_<?=$data->item_id?>" id="avg_price_<?=$data->item_id?>" type="text"  value="<?=number_format($avg_price = $tot_stock_amt[$data->item_id]/$tot_stock_in[$data->item_id],2,'.','');?>" readonly="" onkeyup="EXPcalculation(<?=$data->item_id?>)" /></td>
								<td align="center"><input name="total_amt_<?=$data->item_id?>" id="total_amt_<?=$data->item_id?>" type="text"  readonly=""  value="" />	</td>
							</tr>
...
<div class="box" style="width:100%;">
    ...
    <td ><input style="width:220px; height:32px;"  name="issue_date" type="text" id="issue_date"  value="<?=($issue_date!='')?$issue_date:date('Y-m-d')?>"   required /></td>
...
<? if(isset($_POST['submit'])){ ?>
	...
	<div class="box" style="width:100%;">
			...
			<input style="width:220px; height:32px;"  name="invoice_date" type="text" id="invoice_date"  readonly="" value="<?=$_POST['issue_date']?>"   required />									</td>
...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0" style="font-size:12px; text-transform:uppercase;">
 <thead>
  <tr>
    ...
    <th width="13%">Issue Qty </th>
    <th width="11%">Avg Price </th>
...
<input name="item_id_<?=$data->item_id?>" id="item_id_<?=$data->item_id?>" type="hidden"   value="<?=$data->item_id?>"  onkeyup="EXPcalculation(<?=$data->item_id?>)" style="width:120px; height:25px;"  />
 ...
 <input name="issue_qty_<?=$data->item_id?>" id="issue_qty_<?=$data->item_id?>" type="text"  value=""  onkeyup="EXPcalculation(<?=$data->item_id?>)" style="width:120px; height:25px;"  />	</td>
    <td><input name="avg_price_<?=$data->item_id?>" id="avg_price_<?=$data->item_id?>" type="text"  value="<?=$avg_price = $tot_stock_amt[$data->item_id]/$tot_stock_in[$data->item_id];?>" readonly="" onkeyup="EXPcalculation(<?=$data->item_id?>)"  style="width:100px; height:25px;"  /></td>
