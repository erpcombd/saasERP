...
$con .= 'and a.req_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
 ...
 $res='select  	a.req_no,a.req_no, DATE_FORMAT(a.req_date, "%d-%m-%Y") as Req_Date, b.warehouse_name as warehouse,  DATE_FORMAT(a.need_by, "%d-%m-%Y") as Need_By_Date, c.fname as entry_by, a.entry_at,a.status from requisition_master a,warehouse b,user_activity_management c where a.warehouse_id='.$_SESSION['user']['depot'].' and a.warehouse_id=b.warehouse_id and a.entry_by=c.user_id '.$con.' and a.status in ("UNCHECKED","CHECKED") order by a.req_no desc limit 15';
