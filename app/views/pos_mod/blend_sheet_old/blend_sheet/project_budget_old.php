...
$$unique = $_SESSION[$unique] = $_POST['budg_id'];} else{

$$unique  =  find_a_field('cons_budget_master','max(budg_id)+1','1');}
...

{
		...
		$sql = "delete from cons_budget_material_details where major_task_id =".$_GET['bmid']." and budget_m_id=".$$unique;
...

{
		...
		$crud   = new crud('cons_budget_material_details');
...

{
		...
		$crud   = new crud('cons_budget_task_details');
		$_POST['entry_by']=$_SESSION['user']['id'];
...

{
		...
		$crud   = new crud('cons_budget_labour_details');
		$_POST['entry_by']=$_SESSION['user']['id'];
...
if($$unique>0) $btn_name='Update Blend Sheet Information'; else $btn_name='Initiate Blend Sheet Information';
      ...
      <tr>
        ...
        <td><fieldset>
          ...
          <div>
           ...
           <input name="line_id" type="hidden" id="line_id"  value="<?=$line_id=$_POST['line_id']?>" />
      ...
      <tr>
		...
		<input  name="budj_id" type="hidden" id="budj_id" value="<?=$$unique?>"/>
          <select name="mjr_task_name">
            <option value="1" selected>Select Section Name</option>
			<option value="2"  selected>Section Name 1</option>
			<option value="3"  selected>Section Name 2</option>
			<option value="4"  selected>Section Name 3</option>
			<option value="5"  selected>Section Name 4</option>
			<option value="6"  selected>Section Name 5</option>
...
$budg_master=mysql_query('select m.*  from cons_budget_major_task m where m.budj_id='.$$unique.'  order by m.id DESC');
  ...
  <form action="?<?=$unique?>=<?=$$unique?>" method="post" name="cloud" id="cloud">
    <table  width="100%" border="1" align="left"  style="border-collapse:collapse; border:1px solid #caf5a5;" cellpadding="2" cellspacing="2">
      ...
      <tr>
        ...
        <td align="center" bgcolor="#CCCCCC"><input  name="budget_m_id" type="hidden" id="budget_m_id" value="<?=$$unique?>"/>
          <input  name="major_task_id" type="hidden" id="major_task_id" value="<?=$budg_data->mjr_task_name?>"/>
          <input  name="pkgs" type="text" class="input3" id="pkgs" style="width:35px;"/></td>
        <td align="center" bgcolor="#CCCCCC"><select name="sam_pay" id="sam_pay" style="width:40px;" onfocus="count2()">
          <option></option>
          <option>Yes</option>
		  <option>No</option>
        </select></td>
        <td align="center" bgcolor="#CCCCCC"><input  name="sam_qty" type="text" class="input3" id="sam_qty" style="width:35px;"/></td>
        ...
        <td width="7%" align="center" bgcolor="#CCCCCC"><input  name="total_volume" type="text" class="input3" id="total_volume<?=$budg_data->mjr_task_name?>" style="width:70px;"/></td>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><div class="tabledesign2">
            <? 
$res='SELECT b.id, b.sale_no, b.lot_no, g.garden_name, b.invoice_no, i.item_name, b.pkgs, b.sam_pay, b.sam_qty, b.qty, b.rate, b.amount, "X" from cons_budget_material_details b, item_info i, tea_garden g, cons_budget_major_task l where g.garden_id=b.garden_id and i.item_id=b.item_id  and d.major_task_id='.$budg_data->mjr_task_name;
