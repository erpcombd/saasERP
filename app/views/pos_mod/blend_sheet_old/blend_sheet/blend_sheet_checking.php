...

{
		...
		$sqll='update blend_sheet_master set status="CHECKED" where blend_id='.$_SESSION[$unique];
		mysql_query($sqll);
		
		$blend_sqll='select m.line_id, m.blend_date, b.* from blend_sheet_master m, blend_sheet_details b where m.blend_id=b.blend_id and  b.blend_id='.$_SESSION[$unique];
		
		$blend_query=mysql_query($blend_sqll);
		
		while($blend=mysql_fetch_object($blend_query)){
			
			journal_item_control($blend->item_id ,$blend->line_id,$blend->blend_date,$blend->qty,0,'Blend',$blend->id,$blend->rate,'',$_SESSION[$unique],'', $blend->garden_id );
		
			journal_item_control($blend->item_id ,$_SESSION['user']['depot'],$blend->blend_date,0,$blend->qty,'Blend',$blend->id,$blend->rate,'',$_SESSION[$unique],'', $blend->garden_id );
		...
		echo "<script>
		window.location.assign('select_unapproved_blend_sheet.php');
		</script>";
...
if($_SESSION[$unique]>0) $btn_name='Update Blend Sheet Information'; else $btn_name='Initiate Blend Sheet Information';
      ...
      <tr>
		...
		<input  name="blend_id" type="hidden" id="blend_id" value="<?=$$unique?>"/>
          <select name="blend_section">
			...
			<option value="1" <?php echo ($_POST['blend_section']==1)?'selected':''?> >Blend Section 1</option>
			<option value="2"  <?php echo ($_POST['blend_section']==2)?'selected':''?>>Blend Section 2</option>
			<option value="3"  <?php echo ($_POST['blend_section']==3)?'selected':''?>>Blend Section 3</option>
			<option value="4"  <?php echo ($_POST['blend_section']==4)?'selected':''?>>Blend Section 4</option>
			<option value="5"  <?php echo ($_POST['blend_section']==5)?'selected':''?>>Blend Section 5</option>
        ...
        <td align="center" bgcolor="#FF0000"><div class="button">
		  ...
		  <td align="center" bgcolor="#FF0000"><a href="view_blend_sheet_report.php?v_no=<?=$$unique?>" target="_blank"><img src="../../images/print.png" width="26" height="26" /></a></td>
      </tr>
    </table>
    <? }?>
  </form>
