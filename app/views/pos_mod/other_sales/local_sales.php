...
if(isset($_POST['confirmm']))
{
		...
		$oi_no = $_POST['oi_no'];
        $sql = 'select w.*,s.ledger_id from warehouse_other_issue_detail w, item_info i, item_sub_group s where i.item_id=w.item_id and s.sub_group_id=i.sub_group_id and w.oi_no="'.$oi_no.'" and w.issue_type="Local Sales"';
		$qry = mysql_query($sql);
		$page_for = 'LocalSales';
		...
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $data->ledger_id, $narration, '0',$data->amount,  $tr_from, $tr_no,'',$tr_id,$cc_code,$group_for);
		$total_amt +=$data->amount;
		}
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $localSalesLedger, $narration, $total_amt,'0', $tr_from, $tr_no,'',$tr_id=0,$cc_code,$group_for);
		unset($_POST);
		$_POST[$unique]=$$unique;
		$_POST['entry_by']=$_SESSION['user']['id'];
		$_POST['entry_at']=date('Y-m-d h:s:i');
