...
switch ($_POST['report']) {
    case 1:
		...
		if(isset($dealer)) 				        {$dealer_con=' and m.vendor_id='.$dealer;} 
		...
		{$to_date=$t_date; $fr_date=$f_date; $date_con=' and m.oi_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
		  ...
		  $sql='select m.oi_no as LS_No,m.oi_date as LS_DATE,m.vendor_name as Sales_From,i.item_name,s.qty,s.rate,s.amount from warehouse_other_issue m, warehouse_other_issue_detail s, item_info i where i.item_id=s.item_id and m.oi_no=s.oi_no and m.issue_type in ("Local Sales","LocalSales") '.$warehouse_con.$item_sub_con.$dealer_con.$item_con.$date_con.'';
