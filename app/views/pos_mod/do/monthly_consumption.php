...
$ssssql = @mysql_query('select item_price,sum(item_in)-sum(item_ex) as unit_qty from journal_item where warehouse_id="'.$_SESSION['user']['depot'].'" and item_id = "'.$data->item_id.'" and tr_from="Adjustment" and ji_date="'.$_POST['odate'].'"');
$flag = mysql_num_rows($ssssql);
$sss = @mysql_fetch_object(($ssssql));
