...
if($_SESSION['user']['level']<=4){
	$query = "select a.do_no,b.dealer_code,b.dealer_name_e from sale_do_master a,dealer_info b where status='CHECKED' and b.dealer_code=a.dealer_code and a.entry_by='".$_SESSION['user']['id']."'  order by a.do_no";	
	
}
else {

  $query = "select a.do_no,b.dealer_code,b.dealer_name_e from sale_do_master a,dealer_info b where a.status='CHECKED' and b.dealer_code=a.dealer_code  order by a.do_no";
...
while($data=mysql_fetch_object($que)){
 ...
 echo '<option value="'.$data->do_no.'">'.$data->do_no.'-'.$data->dealer_name_e.'</option>';
    ...
    <td bgcolor="#FF9966"  style="text-align:center"><strong>
      ...
      <input type="submit" name="submitit" id="submitit" value="Chalan DO" style="width:170px; font-weight:bold; font-size:12px; height:30px; color:DodgerBlue"/>
