...
//$sql = "select distinct chalan_no from sale_do_chalan where chalan_date>'2014-12-31'";

//$sql = "select distinct chalan_no from sale_do_chalan where chalan_date>'2015-01-01'";
//$data=mysql_query($sql);
//while($x = mysql_fetch_object($data)){
//auto_insert_sales_chalan_secoundary($x->chalan_no);
//$xx++;
//echo $xx.'<BR>';
//}

$sql = "select distinct tr_no from journal where narration LIKE '%Collection DO#%' and tr_from = 'Collection'";

$data=mysql_query($sql);
while($x = mysql_fetch_object($data)){
auto_do_collection($x->tr_no);
echo $xx++;
echo '-'.$x->tr_no.'<BR>';
}
...
//$sql = "select concat('DO#',m.do_no,'/',l.ledger_name,'/',m.payment_by,'/',m.branch,'/',m.acc_note,'.') as narration,tr_no from sale_do_master m,journal jo,accounts_ledger l where jo.cr_amt>0 and jo.tr_from='Collection' and jo.tr_no=m.do_no and jo.ledger_id=l.ledger_id and jo.tr_no = '38593' limit 1";
//$data=mysql_query($sql);
//while($x = mysql_fetch_object($data)){
//
//echo $sqls = "update journal j set narration = '".addslashes($x->narration)."' where dr_amt>0 and tr_from='Collection' and tr_no='".$x->tr_no."'"; 
//$update1 = mysql_query($sqls);
//$xx++;
//echo $xx.'<BR>';
//}
//
//
//
//$sql = "select concat('DO#',m.do_no,'/',l.ledger_name,'/',m.payment_by,'/',m.branch,'/',m.acc_note,'.') as narration,tr_no from sale_do_master m,journal jo,accounts_ledger l where jo.dr_amt>0 and jo.tr_from='Collection' and jo.tr_no=m.do_no and jo.ledger_id=l.ledger_id and jo.tr_no = '38593' limit 1";
//$data=mysql_query($sql);
//while($x = mysql_fetch_object($data)){
//echo $sqls = "update journal j set narration = '".addslashes($x->narration)."' where cr_amt>0 and tr_from='Collection' and tr_no='".$x->tr_no."'"; 
//$update1 = mysql_query($sqls);
//$xx++;
//echo $xx.'<BR>';
//}
...
//}
//update journal j 
//set narration = (select concat('DO#',m.do_no,'/',l.ledger_name,'/',m.payment_by,'/',m.branch,'/',m.acc_note) from sale_do_master m,journal jo,accounts_ledger l where jo.cr_amt>0 and jo.tr_from='Collection' and j.tr_no=jo.tr_no and jo.tr_no=m.do_no and jo.ledger_id=l.ledger_id) where dr_amt>0 and tr_from='Collection' and tr_no=46182
//
//update
//
//journal as j,sale_do_master m,journal jo,accounts_ledger l 
//
//set 
//j.narration = concat('DO#',m.do_no,'/',l.ledger_name,'/',m.payment_by,'/',m.branch,'/',m.acc_note) 
//
//where jo.cr_amt>0 and jo.tr_from='Collection' and j.tr_no=jo.tr_no and jo.tr_no=m.do_no and jo.ledger_id=l.ledger_id and j.dr_amt>0 and j.tr_from='Collection' and m.do_date between '201'
...
$sql1="select c.*,d.gift_on_order,d.gift_on_item,d.gift_id from sale_do_chalan c,sale_do_details d where d.id=c.order_no and c.chalan_no = '".$chalan_no."'";
...
$gift_on_item[] = $info->gift_on_item;
...
if($unit_price[$i]>0)
{
$xxxxxxxxxx = '('.$items->finish_goods_code.')';
}
else
{
$xxxxxxxxxx = '(Offer No-'.$gift_id[$i].')';
}
?>
      <tr>
        <td align="center" valign="top"><?=$i+1?></td>
        <td align="left" valign="top"><?=find_a_field('item_info','item_name','item_id='.$item_id[$i]);?><?=$xxxxxxxxxx;?></td>
        ...
        <td align="right" valign="top"><? $ttt = (($total_amt[$i])); if($ttt>0){echo number_format($ttt,2); $atot = $atot + $ttt;} ?></td>
        <td align="right" valign="top"><? $ttt = (($unit_price[$i])*$total_unit[$i]); if($ttt<0){echo number_format((-1)*$ttt,2); $tot = $tot + $ttt;} ?></td>
        </tr>
<? }?>
      <tr style="border-bottom:#FFFFFF">
        ...
        <td align="right" valign="top"><strong>
          <?=number_format($atot,2)?>
        </strong></td>
        <td align="right" valign="top"><strong>
          <?=number_format((-1)*$tot,2)?>
        </strong></td>
      </tr>

	        <tr>
        <td colspan="7" align="center" valign="top"><div align="right">Total Accounts Transection Amt: </div></td>
        <td align="right" valign="top"><?=number_format(($atot-$tot),2)?></td>
