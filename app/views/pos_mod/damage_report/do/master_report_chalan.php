...
c.chalan_date, m.do_no,m.do_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,a.AREA_NAME as area, d.team_name as team,w.warehouse_name as depot,d.product_group as grp,c.driver_name,d.commission from 

sale_do_master m,sale_do_chalan c,dealer_info d  , warehouse w, area a

where a.AREA_CODE=d.area_code and m.status in ('CHECKED','COMPLETED') and m.do_no=c.do_no and  m.dealer_code=d.dealer_code and w.warehouse_id=c.depot_id".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con.$product_team_con." order by c.chalan_no";
...
<tr><td><?=$s?></td><td><a href="chalan_view.php?v_no=<?=$data->chalan_no?>" target="_blank"><?=$data->chalan_no?></a></td><td><?=$data->chalan_date?></td><td><?=$data->driver_name?></td><td><?=$data->do_no?></td><td><?=$data->do_date?></td><td><?=$data->dealer_name?></td><td><?=$data->area?></td><td><?=$data->depot?></td><td><?=$data->team?></td><td><?=$info[0]?></td><td><?=number_format($com,2)?></td><td><?=number_format($rec,2)?></td></tr>

<?

}

echo '<tr class="footer"><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>'.number_format($dp_t,2).'</td><td>'.number_format($com_t,2).'</td><td>'.number_format($rec_t,2).'</td></tr></tbody></table>';
