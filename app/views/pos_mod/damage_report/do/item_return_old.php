...
if($order->gift_on_order>0)
	{
		$sss = "select * from sale_gift_offer where item_id='".$_POST['item_id']."' and start_date<='".$order->do_date."' and end_date>='".$order->do_date."' and (d.item_id=1096000100010239 or d.item_id=1096000100010312) limit 1";
		$qqq = mysql_query($sss);
...
//			if($gift->gift_id==1096000100010239 || $gift->gift_id==1096000100010312)
//			{
			$qty = $_POST['qty'];
...
if($or_details>0)
{
$sql_query = 'select concat(i.item_name,"#>",i.item_id),i.finish_goods_code from item_info i,sale_do_chalan c where c.chalan_no="'.$or_details.'" and i.finish_goods_code!="2000" and i.item_id=c.item_id';
auto_complete_from_db_sql($sql_query,'item_id');

//auto_complete_from_db('item_info','finish_goods_code','concat(item_name,"#>",item_id)','1 and item_id in (select item_id from sale_do_chalan where chalan_no="'.$or_details.'" and finish_goods_code != "2000")','item_id');
}
else
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" align="center">
  <tr>
    ...
    <? echo $oi_details;$field='or_details';?>
      <div>
        <label for="oi_details">Chalan No:</label>
		<select id="or_details" name="or_details" required>
		<option value="">Select Chalan</option>
		<? foreign_relation('sale_do_chalan','distinct chalan_no','chalan_no',$$field,'dealer_code='.$dealer.' order by id desc limit 5',$or_details);?>
		<option value="0" <?=($or_details=='0')?'Selected':'';?>>Undefined</option>
		</select>
      </div>
