...
where m.status in ('PROCESSING','COMPLETED') and  m.dealer_code=d.dealer_code and w.warehouse_id=d.depot".$date_con.$pg_con." order by m.entry_at";
		break;
...
where m.status in ('PROCESSING','COMPLETED') and m.do_no=c.do_no and a.AREA_CODE=d.area_code and m.dealer_code=d.dealer_code and w.warehouse_id=d.depot and d.dealer_type='Distributor'".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con." order by c.chalan_no";
...
where m.status in ('PROCESSING','COMPLETED') and m.do_no=c.do_no  and m.dealer_code=d.dealer_code and w.warehouse_id=d.depot and d.dealer_type != 'Distributor'".$depot_con.$date_con.$pg_con.$dealer_con." order by m.do_date,m.do_no";
$query = mysql_query($sql);
echo '<table width="100%" cellspacing="0" cellpadding="2" border="0">
...
where m.do_no=o.do_no and i.item_id=o.item_id and m.dealer_code=d.dealer_code and c.order_no = o.id and m.status in ('PROCESSING','COMPLETED') and w.warehouse_id=d.depot ".$date_con.$item_con.$depot_con.$dtype_con.$pg_con.$dealer_con;
...
sale_do_master m,sale_do_details o,sale_do_chalan c, item_info i,dealer_info d , warehouse w
where c.chalan_no='".$chalan_no."' and c.order_no = o.id and m.do_no=o.do_no and i.item_id=o.item_id and m.dealer_code=d.dealer_code and m.status in ('PROCESSING','COMPLETED') and w.warehouse_id=d.depot ".$date_con.$item_con.$depot_con.$dtype_con.$do_con." order by m.do_date desc";
}
...
from 
sale_do_master m,sale_do_details o, item_info i, warehouse w, dealer_info d, area a
where m.do_no=o.do_no and m.dealer_code=d.dealer_code and w.warehouse_id=d.depot and i.item_id=o.item_id and a.AREA_CODE=d.area_code  and m.status in ('PROCESSING','COMPLETED') and a.ZONE_ID=".$zone->ZONE_CODE." ".$date_con.$item_con.$item_brand_con.$pg_con.$area_con.'';
