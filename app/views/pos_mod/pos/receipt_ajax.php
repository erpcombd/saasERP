<?php
session_start();
require_once "../../../assets/support/inc.all.php";
$pos_id = $_REQUEST['pos_id'];
//master info
$sqlm = "select sale_pos_master.*, dealer_pos.dealer_name, dealer_pos.contact_no from sale_pos_master left join dealer_pos on dealer_pos.dealer_code = sale_pos_master.dealer_id where pos_id = '$pos_id'";
$querym = mysql_query($sqlm);
$datas['master_info'] = mysql_fetch_assoc($querym);

// deatils info
$sqli = "select sale_pos_details.*, item_info.item_name from sale_pos_details left join item_info on item_info.item_id = sale_pos_details.item_id where pos_id = '$pos_id'";
$qi = mysql_query($sqli);
$i = 0;
while($datass = mysql_fetch_assoc($qi)){
    extract($datass);
	if($serial_no!=''){
    $datas['details_info'][$i]['item_name'] = $item_name.'( '.$serial_no.' )';
	}else{
	$datas['details_info'][$i]['item_name'] = $item_name;
	}
    $datas['details_info'][$i]['qty'] = number_format($qty, 0, '.', '');
    $datas['details_info'][$i]['rate'] = number_format($rate, 2, '.', '');
    $datas['details_info'][$i]['discount'] = number_format($discount, 2, '.', '');
    $datas['details_info'][$i]['total_amt'] = number_format($qty*$rate, 2, '.', '');
    $i++;
}

// payment history
$psql = "select * from pos_payment where pos_id = '$pos_id'";
$rr = mysql_query($psql);
$j  = 0;
while($datasss = mysql_fetch_assoc($rr)){
    extract($datasss);   
    $datas['payment_history'][$j]['total_item'] = number_format($total_item, '2', '.', '');
    $datas['payment_history'][$j]['total_amt'] = number_format($total_amt, '2', '.', '');
    $datas['payment_history'][$j]['paid_amt'] = number_format($paid_amt, '2', '.', '');
    $datas['payment_history'][$j]['left_amt'] = number_format($left_amt, '2', '.', '');
    $datas['payment_history'][$j]['payment_method'] = find_a_field('accounts_ledger','ledger_name','ledger_id="'.$payment_method.'"');
    $datas['payment_history'][$j]['status'] = $status;
    $j++;
}

echo json_encode($datas);
?>