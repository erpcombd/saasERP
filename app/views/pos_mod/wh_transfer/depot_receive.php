  ...
  <tr>
    ...
    <? $field='pi_date';?>
      ...
      <div>
        ...
        <label style="width:85px;" for="<?=$field?>">Invoice No:</label>
...
<?php /*?>	<? if($ex<time()){?>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr height="30">
        ...
        <td width="13%" align="right" bgcolor="#9999FF"><strong>Receive No: </strong></td>

        <td width="12%" align="right" bgcolor="#9999FF"><strong>

          <input style="width:120px;"  name="rec_sl_no" type="text" id="rec_sl_no" value=""/>

        </strong></td>

        <td width="16%" align="right" bgcolor="#9999FF"><strong>Receive Date:</strong></td>
...
<? if($$unique>0){
   ...
   $sql='select a.id, a.item_id, b.finish_goods_code,  b.item_name, b.unit_name, a.pkt_size, a.pkt_unit, a.total_unit as total_unit from warehouse_transfer_detail a, item_info b 
    ...
    <tr>
      ...
      <tbody>
          ...
          <tr>
            ...
            <th width="12%" rowspan="2">FG Code</th>

            <th width="47%" rowspan="2">Item Name</th>

            <th rowspan="2" bgcolor="#FFFFFF">Unit</th>

            <th colspan="2" bgcolor="#FFFFFF">Stock</th>
            <th colspan="2" bgcolor="#0099CC">Receive Qty  </th>
          </tr>
          <tr>
            <th bgcolor="#FFFFFF">Ctn</th>
            <th bgcolor="#FFFFFF">Qty</th>
            <th bgcolor="#0099CC">Ctn</th>
          ...
          <tr bgcolor="<?=(($bg%2)==1)?'#FFEAFF':'#DDFFF9'?>">
            ...
            <td><?=$row->finish_goods_code?>
              ...
              <td><?=$row->item_name?>
                ...
                <input type="hidden" name="rate_<?=$row->id?>" id="rate_<?=$row->id?>" value="<?=$row->c_price?>" /></td>

              <td width="11%" align="center"><?=$row->unit_name?>
              ...
              <td width="6%" align="center"><? $final_stock =warehouse_product_stock($row->item_id, $warehouse_to);?> <?=$pack_unit=($final_stock/$row->pkt_size)?></td>
              <td width="5%" align="center"><?=number_format($final_stock,2);?></td>
              <td width="6%" align="center" bgcolor="#6699FF" style="text-align:center">

			  <? if($status=="SEND"){?>

                <input name="pack_chalan_<?=$row->id?>" type="text" id="pack_chalan_<?=$row->id?>" style="width:80px; float:none" readonly="" value="<?=$row->pkt_unit?>" onkeyup="TRcalculation(<?=$row->id?>)"  />

                <? } else echo 'Done';?></td>
              <td width="5%" align="center" bgcolor="#6699FF" style="text-align:center">
			   <? if($status=="SEND"){?>

                <input name="qty_chalan_<?=$row->id?>" type="text" id="qty_chalan_<?=$row->id?>" style="width:80px; float:none" readonly="" value="<?=$row->total_unit?>"  />
...
<input name="delete" type="submit" class="btn1" value="CANCEL ORDER" style="width:270px; font-weight:bold; font-size:12px;color:#F00; height:30px"  />

<input  name="vendor_id" type="hidden" id="vendor_id" value="<?=$vendor_id;?>"/></td>

<td align="center"><input name="confirm" type="submit" class="btn1" value="CONFIRM RECEIVE" style="width:270px; font-weight:bold; font-size:12px; height:30px; color:#090; float:right;" /></td>
