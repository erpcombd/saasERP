...
$sql_by_product = 'select item_id as by_product_item_id,unit_price from batch_by_product where batch_no="'.$batch_no.'"';
$by_product_query = db_query($sql_by_product);
while($data_by_product=mysqli_fetch_object($by_product_query))
{


if($_POST['by_qty_'.$data_by_product->by_product_item_id]>0){


	$by_product_qty += $_POST['by_qty_'.$data_by_product->by_product_item_id];
...
("'.$pr_no.'", "'.$pr_date.'", "'.$batch_no.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$data_by_product->by_product_item_id.'", "'.$_POST['by_qty_'.$data_by_product->by_product_item_id].'", "'.$data_by_product->unit_price.'", "'.$total_by_product_amt.'", "'.$status.'", "'.$entry_at.'", "'.$entry_by.'","By_Product")';
...
if($fg_item_id>0) {
	  ...
	  $ins_sql1 = 'INSERT INTO production_receive_master (pr_no, year, inv_id, inv_type, pr_date, group_for, warehouse_id, batch_no, fg_item_id, unit_name, batch_qty, pr_qty, by_product_qty,status, entry_at, entry_by,section_id) VALUES

("'.$pr_no.'", "'.$YR.'", "'.$cy_id_batch.'", "'.$inv_type.'", "'.$pr_date.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$batch_no.'", "'.$fg_item_id.'", "'.$unit_name.'", "'.$pr_qty.'", "'.$pr_qty.'", "'.$by_product_qty.'", "'.$status.'", "'.$entry_at.'", "'.$entry_by.'", "'.$section_id.'")';
...
if(isset($_POST['confirm'])){
	...
	while($databy =mysqli_fetch_object($queryby)){
		...
		if($_POST['by_qty_'.$databy->item_id]>0){
			if($databy->type=="Ratio"){
				$rate = ((($databy->rate_ratio / 100) * $tot_production_cost)/$_POST['by_qty_'.$databy->item_id]);
			}else{
			...
			$updateby_rec = "update  production_receive_detail  set unit_price = ".$rate.",total_unit=".$_POST['by_qty_'.$databy->item_id].",total_amt=".$rate*$_POST['by_qty_'.$databy->item_id].", status='".$status."'
			  where pr_no=".$pr_no." and item_id=".$databy->item_id." and item_type='By_Product'";

			db_query($updateby_rec);


			$amount_by += $_POST['by_qty_'.$databy->item_id]* $rate;
	journal_item_control($databy->item_id ,$section_id,$pr_date,$_POST['by_qty_'.$databy->item_id],0,'Production Receive',$pr_no,$rate,0,$pr_no,'','',$group_for,'');
...
<div class="box" style="width:100%;">
 ...
 <tr>
     <td style="background-color: yellow;"  align="right" ><strong>Total Batch Qty: </strong></td>

    <td style="background-color: yellow;"  ><?=$batch_data->batch_qty?></td>

	<td style="background-color: yellow;" colspan="4"></td>
	
	     <td style="background-color: yellow;"  ><strong>Batch receive Qty: </strong></td>

    <td style="background-color: yellow;" ><?=find_a_field('production_receive_detail','sum(total_unit)','batch_no="'.$batch_no.'" and status not in ("MANUAL");');?></td>
 </tr>
	...
	<a href="../fg_from_pl/invoice_print_view.php?bom_no=<?=$batch_no;?>" target="_blank"><input name="batch" type="text" id="batch" style="width:90%; height:32px;" value="<?=$batch_data->inv_type;?><?=$batch_data->batch_no;?>" readonly="" required tabindex="1" /></a>	</td>
	 ...
	 <td width="17%" rowspan="8" align="center" ><strong>
      ...
      <input type="submit" name="master_data" id="master_data" value="Initiate Entry" style="width:180px; text-align:center; font-weight:bold; font-size:12px; height:30px; color:#090"/>
						...
						<?  $sql ="select b.* ,i.item_name from  batch_by_product b,item_info i where i.item_id=b.item_id and b.batch_no='".$batch_no."'";
						
							$query = db_query($sql);
							while($data=mysqli_fetch_object($query)){
									
								?>
								<td>By Product (<?=$data->item_name;?>)</td>
								<td><input name="by_qty_<?=$data->item_id;?>" type="text" id="by_qty_<?=$data->item_id;?>"  value="" required tabindex="1" /></td>
...
<div class="box" style="width:100%;">
	...
	value="<?=$bch_data->inv_type;?><?=$bch_data->batch_no;?>" readonly="" required tabindex="1" /></a>	 </td>
						...
						<?   $sql ="select b.* ,i.item_name from  production_receive_detail  b,item_info i where i.item_id=b.item_id and b.pr_no='".$ms_data->pr_no."' and b.item_type='By_Product'";
							...
							while($data=mysqli_fetch_object($query)){
								...
								?>
								<td>By Product (<?=$data->item_name;?>)</td>
								<td><input name="by_qty_<?=$data->item_id;?>" type="text" id="by_qty_<?=$data->item_id;?>"  value="<?=$data->total_unit;?>" required tabindex="1" readonly /></td>
...
from production_receive_detail a, item_info i, item_sub_group s 
where i.item_id=a.item_id  and i.sub_group_id=s.sub_group_id and a.pr_no='".$ms_data->pr_no."' and a.total_unit>0 and a.item_type='By_Product' order by i.sub_group_id, i.item_name";
...
<input name="batch_qty_by_<?=$databy->id?>" type="hidden" id="batch_qty_by_<?=$databy->id?>" value="<?=$databy->total_unit?>" />



<input name="total_unit_by_<?=$databy->id?>" type="text" id="total_unit_by_<?=$databy->id?>" value="<?=$total_qty_bom=$databy->total_unit; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; "  readonly="readonly" /></td>
...
<input name="actual_unit_by_<?=$databy->id?>" type="text" id="actual_unit_by_<?=$databy->id?>" value="<?=$total_qty=$databy->total_unit; $grand_by_unit +=$total_qty;  ?>" onkeyup="TRcalculationpo(<?=$databy->id?>)" style="width:80px; height:30px; "  /></td>
...
<input type="hidden" name="Button" value="EDIT"  onclick="update_value2(<?=$databy->id?>)" style="width:70px; font-size:12px; height:30px; background-color:#66CC66; font-weight:700;"/>
