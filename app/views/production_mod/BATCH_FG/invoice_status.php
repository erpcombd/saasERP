...
$title='Production Receive Status';
do_calander('#fdate');
do_calander('#tdate');
        ...
        <table id="grp" class="table1  table-striped table-bordered table-hover table-sm">
          ...
          <tbody class="tbody1">
			...
			//		}
				...
				$res="select  m.* from  production_receive_master m, production_rm_consumption d where m.pr_no=d.pr_no and  m.status!='MANUAL'  ".$con." group by m.pr_no order by m.pr_date, m.pr_no";
			...
			<?php /*?><td onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->inv_type;?><?=$data->pr_no;?></td><?php */?>
			
			<td onClick="custom('<?=url_encode($data->pr_no);?>','<?=url_encode($c_id);?>');" <?=(++$z%2)?'':'class="alt"';?>><?=$data->inv_type;?><?=$data->pr_no;?></td>
			<?php /*?><td onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?= date("d-m-Y",strtotime($data->pr_date));?></td><?php */?>
			
			<td onClick="custom('<?=url_encode($data->pr_no);?>','<?=url_encode($c_id);?>');" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?= date("d-m-Y",strtotime($data->pr_date));?></td>
			
			<?php /*?><td onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>BATCH-<?=$data->batch_no;?></td><?php */?>
			
			<td onClick="custom('<?=url_encode($data->pr_no);?>','<?=url_encode($c_id);?>');" <?=(++$z%2)?'':'class="alt"';?>>BATCH-<?=$data->batch_no;?></td>
			
			<?php /*?><td onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('item_info','item_name','item_id="'.$data->fg_item_id.'"');?></td><?php */?>
			
			<td onClick="custom('<?=url_encode($data->pr_no);?>','<?=url_encode($c_id);?>');" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('item_info','item_name','item_id="'.$data->fg_item_id.'"');?></td>
			
	<?php /*?>		<td onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('user_activity_management','fname','user_id="'.$data->entry_by.'"');?></td><?php */?>
			
			<td onClick="custom('<?=url_encode($data->pr_no);?>','<?=url_encode($c_id);?>');" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('user_activity_management','fname','user_id="'.$data->entry_by.'"');?></td>
			</tr>
