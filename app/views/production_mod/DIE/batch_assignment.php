...
$title='DIE Selection for Extrusion Plant';
do_calander('#fdate');
do_calander('#tdate');
...
{
	window.open('<?=$target_url?>?bom_no='+theUrl);
}
</script>
...
<?php /*?>          <div class="col-sm-5 col-md-5 col-lg-5 col-xl-5">
          ...
          <div class="col-sm-2 col-md-2 col-lg-2 col-xl-2 d-flex justify-content-center align-items-center">
		   <input type="submit" name="submitit" id="submitit" value="Set DIE" class="btn1 btn1-submit-input"/>
          </div>
      ...
      <?php /*?><div class="container-fluid pt-5 p-0">
        <table id="grp" class="table1  table-striped table-bordered table-hover table-sm">
          ...
          <tbody class="tbody1">
				...
				if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and m.bom_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
				...
				//		}
				   ...
				   $res="select  m.* from  bom_master m, bom_raw_material d where m.bom_no=d.bom_no and  m.status!='MANUAL'  ".$con.$fg_item_con." group by m.bom_no order by m.bom_date, m.bom_no";
				...
				<td onClick="custom(<?=$data->bom_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->inv_type;?><?=$data->bom_no;?></td>
				<td onClick="custom(<?=$data->bom_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?= date("d-m-Y",strtotime($data->bom_date));?></td>
				<td onClick="custom(<?=$data->bom_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('item_info','item_name','item_id="'.$data->fg_item_id.'"');?></td>
				<td onClick="custom(<?=$data->bom_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('user_activity_management','fname','user_id="'.$data->entry_by.'"');?></td>
				</tr>
