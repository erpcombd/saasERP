...
$ddd = 'select sum(p.qty) from purchase_invoice i, purchase_receive p,requisition_master a, requisition_order b where a.statu="PENDING" and a.req_no=b.req_no and b.id=i.req_id
...
$s_con=' and a.req_for='.$_POST['warehouse_id'];
 ...
 $r='select a.req_no,a.req_no,a.req_date,a.status ,b.qty, b.item_id, i.item_name,b.unit_name, g.group_name from  requisition_master a, requisition_order b, item_info i, item_sub_group s, item_group g where  a.status in ("CHECKED","PENDING") and a.req_no=b.req_no '.$con.$group_con.$s_con.$sub_group_con.' and b.item_id=i.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id group by g.group_name order by a.req_no asc';
...
<th width="5%">SL/NO</th>
 ...
 $res='select b.id,b.item_id, a.req_no,a.req_no,a.req_date,a.status,sum(b.qty) as qty,s.sub_group_name, sum(b.purchase_qty) as purchase_qty,a.req_note, b.item_id, i.item_name,b.unit_name, g.group_name from  requisition_master a, requisition_order b, item_info i, item_sub_group s, item_group g where a.status in ("CHECKED","PENDING") and a.req_no=b.req_no and b.item_id=i.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id and g.group_name="'.$s->group_name.'" '.$sub_group_con.$con.$s_con.' group by b.item_id  order by i.item_name asc';
 ...
 $rec_qty=find_a_field('purchase_master a, purchase_invoice b, purchase_receive c, item_info i, item_sub_group s, item_group g','sum(c.qty)',' 1 '.$dcon.'  and a.po_no=b.po_no and b.id=c.order_no and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id and b.item_id=i.item_id and b.item_id="'.$data->item_id.'" group by c.item_id');
  ...
  <tr>
	<td valign="top"><div align="center"><?=++$sl?></div></td>
		...
		<td valign="top">
		<div align="center">
 <a href="pr_master_report.php?report=400002&item_id=<?=$data->item_id?>&t_date=<?=$_POST['t_date']?>&f_date=<?=$_POST['f_date']?>&group_id=<?=$_POST['group_id']?>&warehouse_id=<?=$_POST['warehouse_id']?>" target="_blank"> <?=$data->qty;?> </a> 
...
<th width="14%">SL/NO</th>
 ...
 $res = 'select a.req_no,a.req_date,a.status,i.item_name,w.warehouse_name,g.group_name,b.qty,b.remarks from   requisition_master a, requisition_order b, item_info i, item_sub_group s, item_group g, warehouse w where a.status in ("CHECKED","PENDING") and w.warehouse_id=a.req_for and a.req_no=b.req_no and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id and b.item_id=i.item_id '.$item_con.$date_con.$con.$g_con.$w_con.' ';
