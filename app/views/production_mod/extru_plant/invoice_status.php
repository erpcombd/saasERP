      ...
      <div class="container-fluid pt-5 p-0">
        <table id="grp" class="table1  table-striped table-bordered table-hover table-sm">
          ...
          <tbody class="tbody1">
			...
			<td onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->inv_type;?><?=$data->pr_no;?></td>
			<td onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?= date("d-m-Y",strtotime($data->pr_date));?></td>
			<td onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>BATCH-<?=$data->batch_no;?></td>
			<td onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('item_info','item_name','item_id="'.$data->fg_item_id.'"');?></td>
			<td onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('user_activity_management','fname','user_id="'.$data->entry_by.'"');?></td>
			</tr>
