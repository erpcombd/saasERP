...
$table_master='master_wip_master';

$table_details='master_wip_details';
...
$res='select a.id,a.item_id,concat(b.item_name) as item_name,a.qoh as Floor_qty,a.exp_date,a.remarks,a.qty,a.rate,"x" from master_wip_details a,item_info b where b.item_id=a.item_id and  a.req_no='.$$unique;

$q=db_query($res);

$masterAll=find_all_field('master_wip_master','','req_no='.$$unique);

while($r=mysqli_fetch_object($q)){

if($_POST['qty_'.$r->id]>0){

$update = "update master_wip_details set qty='".$_POST['qty_'.$r->id]."', order_qty = '".$_POST['qty_'.$r->id]."' where id='".$r->id."' ";

db_query($update);

//journal_item_control($r->item_id, $masterAll->from_warehouse_id, $masterAll->req_date, 0, $r->qty, 'WIPIssue', $r->id, $r->rate, '', $$unique, '', '',0, $r->rate,'');
...
//$delete = "delete from master_wip_details where id='".$r->id."' ";
...
echo '<script> window.location.href = "mr_create.php?new=2"; </script>';
...
<label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text req-input">From Warehouse :</label>
...
<label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text req-input">To Warehouse :</label>

<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">

<select name="<?=$field?>" style="margin-left: 34px;" type="text" id="<?=$field?>"onchange="getData2('manual_req_no_ajax.php', 'manual_req_no', this.value, document.getElementById('req_no').value);" required="required">
...
echo $a2="select sub_group_id, sub_group_name from item_sub_group where 1 ".$con1."";
...
<input  name="item_id" type="text" list="item_ids" id="item_id" value="<? echo $item_id; ?>" style="width:170px;" required="required" onblur="getData2('mr_ajax.php', 'mr', this.value, document.getElementById('from_warehouse_id').value);" autocomplete="off"/>
...
$res='select a.id,a.item_id,concat(b.item_name) as item_name,a.qoh as Floor_qty,a.exp_date,a.remarks,b.sub_group_id,a.qty,"x" from master_wip_details a,item_info b where b.item_id=a.item_id and  a.req_no='.$req_no.' order by a.item_id';
...
<?php $detai=find_a_field('master_wip_details','count(id)','req_no='.$req_no); if($detai>0){?>
