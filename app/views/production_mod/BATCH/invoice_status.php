...
//		}
    ...
    $res="select  m.* from  batch_master m, batch_raw_material d where m.batch_no=d.batch_no and  m.status!='MANUAL'  ".$con." group by m.batch_no order by m.batch_date, m.batch_no";
...
<?php /*?><td onClick="custom(<?=$data->batch_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->batch_no;?></td><?php */?>

<td onClick="custom('<?=url_encode($data->batch_no);?>','<?=url_encode($c_id);?>');" <?=(++$z%2)?'':'class="alt"';?>><?=$data->batch_no;?></td>

<?php /*?><td onClick="custom(<?=$data->batch_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?= date("d-m-Y",strtotime($data->batch_date));?></td><?php */?>

<td onClick="custom('<?=url_encode($data->batch_no);?>','<?=url_encode($c_id);?>');" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?= date("d-m-Y",strtotime($data->batch_date));?></td>

<?php /*?><td onClick="custom(<?=$data->batch_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('item_info','item_name','item_id="'.$data->fg_item_id.'"');?></td><?php */?>

<td onClick="custom('<?=url_encode($data->batch_no);?>','<?=url_encode($c_id);?>');" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('item_info','item_name','item_id="'.$data->fg_item_id.'"');?></td>

<?php /*?><td onClick="custom(<?=$data->batch_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('user_activity_management','fname','user_id="'.$data->entry_by.'"');?></td><?php */?>

<td onClick="custom('<?=url_encode($data->batch_no);?>','<?=url_encode($c_id);?>');" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('user_activity_management','fname','user_id="'.$data->entry_by.'"');?></td>
</tr>
