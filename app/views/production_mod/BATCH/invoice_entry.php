...
"'.$batch_qty.'", "'.$warehouse_id.'", "'.$status.'", "'.$entry_at.'", "'.$entry_by.'","'.$section_id.'")';
...
$rm_batch='INSERT INTO batch_raw_material (batch_no, bom_no, group_for, warehouse_id, batch_date, fg_item_id, item_id, unit_name, batch_qty, bom_qty, batch_cal_qty, final_unit, final_qty, status, entry_at, entry_by,section_id)
VALUES("'.$batch_no.'", "'.$bom_no.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$batch_date.'", "'.$fg_item_id.'", "'.$data2->item_id.'", "'.$data2->unit_name.'", "'.$batch_qty.'", "'.$data2->total_unit.'", "'.$batch_cal_qty.'", "'.$data2->total_unit.'", "'.$batch_cal_qty.'", "'.$status.'", "'.$entry_at.'", "'.$entry_by.'","'.$section_id.'")';
...
add_to_sec_journal('CloudMVC', $jv, $brow->batch_date, $brow->item_ledger,'Batch issue', '0', $cost_amt,'Batch issue', $batch_no,$brow->sub_ledger_id,'',$cc_code,$batch_all->group_for,$_SESSION['user']['id'],'');
...
//$item_id, $warehouse_id,$ji_date,$item_in,$item_ex,$tr_from,$tr_no,$rate='',$r_warehouse='',$sr_no='',$bag_size='',$bag_unit='',$group_for='',$final_price='',$barcode='',$serial_no='',$lot_no=''
...
add_to_sec_journal('CloudMVC', $jv, $batch_all->batch_date,$pl_main_ledger[$pl_sub_ledger[$batch_all->warehouse_id]],'Batch issue', $gr_cost_amt, '0','Batch issue', $batch_no,$pl_sub_ledger[$batch_all->warehouse_id],'',$cc_code,$batch_all->group_for,$_SESSION['user']['id'],'');
...
<?php /*?>	<a href="../BOM/invoice_print_view.php?bom_no=<?=$bom_no;?>" target="_blank"><input name="bom" type="text" id="bom" style="width:90%; height:32px;" value="<?=$bom_data->inv_type;?><?=$bom_data->bom_no;?>" readonly="" required tabindex="1" /></a>	</td><?php */?>
	
	
	<a href="../BOM/invoice_print_view.php?c=<?=rawurlencode(url_encode($c_id));?>&v=<?=rawurlencode(url_encode($bom_no));?>" target="_blank"><input name="bom" type="text" id="bom" style="width:90%; height:32px;" value="<?=$bom_data->inv_type;?><?=$bom_data->bom_no;?>" readonly="" required tabindex="1" /></a>	</td>
...
<tr>
			...
			<td><strong>Section</strong></td>
									...
									<option></option>
										...
										<? foreign_relation('warehouse','warehouse_id','warehouse_name',$_POST['section_id'],'use_type="SC"');?>
...
$sstk=find_a_field('journal_item','sum(item_in-item_ex)',' warehouse_id = '.$datafg->section_id.' and item_id='.$datafg->item_id);
...
$queryfg = db_query($sqlfg);
 ...
 while($databy=mysqli_fetch_object($queryfg)){$i++;
...
<input name="total_unit_by_<?=$databy->id?>" type="text" id="total_unit_by_<?=$databy->id?>" value="<?=$databy->final_qty; $tot_total_unit +=$databy->total_unit;?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " /></td>
...
<h1><center><span style="color: red;font-size: 30px !important;">Some Material's Stock Less Then Required Qty! </span></center></h1>
