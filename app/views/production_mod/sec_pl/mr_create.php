...
$table_details='pl_issue_details';
...
if(isset($_POST['confirmm'])){
		// Details insert start
			 $res='select a.id,a.item_id,concat(b.item_name) as item_name,a.qoh as Floor_qty,a.exp_date,a.remarks,a.qty,"x" from pl_issue_details a,item_info b where b.item_id=a.item_id and  a.req_no='.$$unique;
			  ...
			  while($r=mysqli_fetch_object($q)){
			    ...
			    if($_POST['qty_'.$r->id]>0){
					...
					$update = "update pl_issue_details set qty='".$_POST['qty_'.$r->id]."', order_qty = '".$_POST['qty_'.$r->id]."' where id='".$r->id."' ";
					...
					$delete = "delete from pl_issue_details where id='".$r->id."' ";
		...
		// Details insert end	
			  	$tr_from='pl_issue';
				$jv=next_journal_sec_voucher_id('','pl_issue',$_SESSION['user']['depot']);


				$sql='select d.id,d.item_id,d.qty,d.rate,m.req_date,m.req_no,m.req_for as to_warehouse,m.warehouse_id as from_warehouse,s.item_ledger ,i.sub_ledger_id from pl_issue_master m, pl_issue_details d,item_info i,item_sub_group s where i.item_id=d.item_id and  i.sub_group_id=s.sub_group_id and m.req_no=d.req_no and d.req_no='.$$unique;
				...
				$cost_amt=($data->qty*$cost_rate[$data->item_id]);

				$wip_ledger = find_a_field('warehouse','ledger_id','warehouse_id="'.$data->to_warehouse.'"');

				

				add_to_sec_journal('CloudMVC', $jv, $data->req_date, $data->item_ledger,'pl_issue', '0', $cost_amt,'pl_issue',$data->req_no,$data->sub_ledger_id,'',$cc_code,$_SESSION['user']['group'],$_SESSION['user']['id'],'');


				add_to_sec_journal('CloudMVC', $jv, $data->req_date,$pl_main_ledger[$wip_ledger],'pl_issue',$cost_amt,'0','pl_issue', $data->req_no,$wip_ledger,'',$cc_code,$_SESSION['user']['group'],$_SESSION['user']['id'],'');

				

				journal_item_control($data->item_id ,$data->from_warehouse,$data->req_date,0,$data->qty,'pl_issue',$data->req_no,$data->rate,0,$databy->id,'','',$_SESSION['user']['group'],'');

				journal_item_control($data->item_id ,$data->to_warehouse,$data->req_date,$data->qty,0,'pl_issue',$data->req_no,$data->rate,0,$databy->id,'','',$_SESSION['user']['group'],'');
...
<div class="form-container_large">
	...
	<form action="mr_create.php" method="post" name="codz" id="codz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
      ...
      <div class="container-fluid bg-form-titel">
		  ...
		  <div class="container n-form2">
            ...
            <div class="form-group pb-1 row m-0">
			 ...
			 <? $field='req_no';?>
              ...
              <label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Issue No :</label>
			 ...
			 <div class="form-group pb-1 row m-0">
              ...
              <label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Issue Date :</label>
		...
		<? foreign_relation($table,$get_field,$show_field,$_POST['warehouse_id'],'use_type="SC"');?>
		  ...
		  <div class="container n-form2">
            ...
            <div class="form-group pb-1 row m-0">
              ...
              <label  for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Issue For :</label>
...
<table  class="table1  table-striped table-bordered table-hover table-sm">
          ...
          <thead class="thead1">
                 ...
                 <tr class="bgc-info">
                        ...
                        <td width="11%" align="center"><strong>SC Qty</strong></td>
          ...
          <tbody class="tbody1">
					...
					<td align="center">
                          ...
                          $a2="select sub_group_id, sub_group_name from item_sub_group where 1 ".$con1."";
							...
							<input  name="warehouse" type="hidden" id="warehouse" value="<?=$warehouse_id?>"/>



                            <input  name="item_id" type="text" list="item_ids" id="item_id" value="<? echo $item_id; ?>" style="width:170px;" required="required" onblur="getData2('mr_ajax.php', 'mr', this.value, document.getElementById('warehouse').value);" autocomplete="off"/>
 ...
 <? //i.sub_group_id like "%'.$get_data.'%"
  ...
  foreign_relation('item_info i,item_sub_group s','concat(i.item_name,"#>",i.item_id,"#>",i.item_id)','i.item_name',$item_id,' 1  and i.sub_group_id=s.sub_group_id ');?>
...
<td colspan="2" align="center"><span id="mr">
...
<td align="center"><input name="order_qty" onblur="check(this.value)" type="number" step="any" class="input3" id="order_qty"  maxlength="100" style="width:120px;" required/></td>
...
$res='select a.id,a.item_id,concat(b.item_name) as item_name,a.qoh as Floor_qty,a.exp_date,a.remarks,b.sub_group_id,a.qty,"x" from pl_issue_details a,item_info b where b.item_id=a.item_id and  a.req_no='.$req_no.' order by a.item_id';
...
<!--<input name="delete"  type="submit" value="DELETE AND CANCEL REQUISITION" class="btn1 btn1-bg-cancel" />-->
			 ...
			 <?php $detai=find_a_field('pl_issue_details','count(id)','req_no='.$req_no); if($detai>0){?>
...
<? }?>
  ...
  function formatWithLeadingZeros(number) {
  ...
  function check(data) {
    ...
    if (formatWithLeadingZeros(data) > stk) {
