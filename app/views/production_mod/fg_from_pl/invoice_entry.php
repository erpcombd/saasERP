...
$sql_by_product = 'select item_id as by_product_item_id,sum(final_qty) as total_qty,unit_price from batch_by_product where batch_no="'.$batch_no.'" group by item_id';
...
$section_id = $_POST['section_id'];
	...
	$by_product_qty =$_POST['actual_unit_by'];	
	...
	while($databy =mysqli_fetch_object($queryby)){
		...
		if($databy->final_unit>0){
			if($databy->type=="Ratio"){
				$rate = ((($databy->rate_ratio / 100) * $tot_production_cost)/$databy->final_unit);
			}else{
			...
			$updateby_rec = "update  production_receive_detail  set unit_price = ".$rate.",total_unit=".$databy->final_qty.",total_amt=".$rate*$databy->final_qty.", status='".$status."' where pr_no=".$pr_no." and item_id=".$databy->item_id." and item_type='By_Product'";
			
			db_query($updateby_rec);
			
			$amount_by += $databy->final_qty* $rate;
	journal_item_control($databy->item_id ,$section_id,$pr_date,$databy->final_qty,0,'Production Receive',$pr_no,$rate,0,$pr_no,'','',$_SESSION['user']['group'],'','','',$batch_no);
...
journal_item_control($item_id,$warehouse_id,$pr_date,0,$total_unit,'Consumption',$data->id,$unit_price,0,$pr_no,'','',$_SESSION['user']['group'],'','','',$batch_no);
...
//$item_id, $warehouse_id,$ji_date,$item_in,$item_ex,$tr_from,$tr_no,$rate='',$r_warehouse='',$sr_no='',$bag_size='',$bag_unit='',$group_for='',$final_price='',$barcode='',$serial_no='',$lot_no=''
	...
	$con_sql1="UPDATE production_receive_master SET cost_price='".$pr_unit_price."', cost_amt='".$tot_production_cost."', status='".$status."',  checked_by='".$entry_by."', checked_at='".$entry_at."' WHERE pr_no = '".$pr_no."' ";
	 ...
	 journal_item_control($fg_item_id ,$section_id,$pr_date,$pr_qty,0,'Production Receive',$pr_no,$pr_unit_price,0,$pr_no,'','',$_SESSION['user']['group'],'','','',$batch_no);
...
$pl_main_ledger[$crow->sub_ledger_id]=$crow->ledger_id;
	 ...
	 //db_query($fg_insert);
 $arr_sql_fg = "select item_type,sum(total_amt) as tot_amt from production_receive_detail  where pr_no=".$pr_no." group by item_type";

	$arr_queryfg = db_query($arr_sql_fg);
	while($arr_datafg =mysqli_fetch_object($arr_queryfg)){	 
	 $amount_check[$arr_datafg->item_type]=$arr_datafg->tot_amt; 
	 }
	...
	/*$fg_without_by_insert="INSERT INTO `secondary_journal` (`proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `entry_by`, `tr_id`, `group_for`, `entry_at`) VALUES ('".$_SESSION['proj_id']."', '".$jv."', '".$pr_date."', '".$fg_ledger."','',".$fg_total_amt_without_by.",'0.00', '".$tr_from."', '".$pr_no."', '".$fg_sub_ledger."', '0', '".$_SESSION['user']['id']."', '', '".$group_for."', '".date('Y-m-d H:i:s')."')";
	 ...
	 //Item Sub Group Dr
$fg_total_amt_without_by=$rm_sum+$oh_sum;
...
if($fg_total_amt_without_by>0){
$fg_without_by_insert="INSERT INTO `secondary_journal` (`proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `entry_by`, `tr_id`, `group_for`, `entry_at`) VALUES ('".$_SESSION['proj_id']."', '".$jv."', '".$pr_date."', '".$fg_ledger."','',".$fg_total_amt_without_by.",'0.00', '".$tr_from."', '".$pr_no."', '".$fg_sub_ledger."', '0', '".$_SESSION['user']['id']."', '', '".$group_for."', '".date('Y-m-d H:i:s')."')";

	 db_query($fg_without_by_insert);
	 
	$fg_total_amt_without_by=0; 
}
...
<div class="box" style="width:100%;">
 ...
 <tr>
	...
	<?php /*?><a href="../fg_from_pl/invoice_print_view.php?bom_no=<?=$batch_no;?>" target="_blank"><input name="batch" type="text" id="batch" style="width:90%; height:32px;" value="<?=$batch_data->inv_type;?><?=$batch_data->batch_no;?>" readonly="" required tabindex="1" /></a>	</td><?php */?>
	
	<a
  href="../BOM/invoice_print_view.php?c=<?=rawurlencode(url_encode($c_id));?>&v=<?=url_encode($batch_no);?>"
  target="_blank"
  style="display:inline-block; width:90%; height:32px; line-height:32px; border:1px solid #ccc; padding-left:5px; text-decoration:none; color:black;"
>
  <?=$batch_data->inv_type;?><?=$batch_data->batch_no;?>
</a>
...
<? //if($po_no[$data->po_no]==0) { } ?>
   ...
   <tr style="background-color: <?= ($i % 2) ? '#E8F3FF' : '#fff'; ?>;">
	...
	<td style="background-color:#99CCFF;"><input name="stock_<?=$datafg->id?>" type="text" id="stock_<?=$datafg->id?>" value="<?=$sstk; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " readonly="" /></td>

	

    <td style="background-color:#99CCFF;"><input name="unit_price_<?=$datafg->id?>" type="text" id="unit_price_<?=$datafg->id?>" value="<?=$final_price[$datafg->item_id]; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " readonly="" />
	<input name="item_id_<?=$datafg->id?>" type="hidden" id="item_id_<?=$datafg->id?>" value="<?=$datafg->item_id; ?>"  />
	</td>

	
    <td style="background-color:#99CCFF;"><input name="actual_unit_<?=$datafg->id?>" type="text" id="actual_unit_<?=$datafg->id?>" value="<?=$total_qty ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " readonly="" /></td>

    <td style="background-color:#99CCFF;"><input name="total_amt_<?=$datafg->id?>" type="text" id="total_amt_<?=$datafg->id?>" value="<?=$total_amt=$total_qty*$final_price[$datafg->item_id]; $grand_total_amt +=$total_amt; ?>" style="width:80px; height:30px; " readonly="" /></td>
    </tr>
...
<input name="total_unit_by_<?=$databy->id?>" type="text" id="total_unit_by_<?=$databy->id?>" value="<?=$total_qty_bom=$databy->final_qty; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; "  readonly="readonly" /></td>
...
<input name="actual_unit_by_<?=$databy->id?>" type="text" id="actual_unit_by_<?=$databy->id?>" value="<?=$total_qty=$databy->final_qty; $grand_by_unit +=$total_qty;  ?>" onkeyup="TRcalculationpo(<?=$databy->id?>)" style="width:80px; height:30px; "  /></td>
