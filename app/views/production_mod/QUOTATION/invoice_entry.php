...
//   	 $lot_no_generate='LOT'.$tr_no.''.$cy_id;
	 ...
	 if($rfq_no>0) {
	  $ins_sql = 'INSERT INTO purchase_quotation_master (invoice_no, year, inv_id, view_invoice_no, invoice_date, group_for, warehouse_id, req_no, view_req_no, currency_type, vendor_id,  status, entry_at, entry_by, rfq_no, view_rfq_no) VALUES

("'.$invoice_no.'", "'.$YR.'", "'.$cy_id.'", "'.$view_invoice_no.'", "'.$invoice_date.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$req_no.'", "'.$view_req_no.'", "'.$currency_type.'", "'.$vendor_id.'", 
 "'.$status.'", "'.$entry_at.'", "'.$entry_by.'", "'.$rfq_no.'", "'.$view_rfq_no.'")';
...
if(isset($_POST['confirm'])){
	...
	$view_invoice_no = $_POST['view_invoice_no'];
	 ...
	 $sql = 'select a.*, s.sub_group_name, i.finish_goods_code, i.item_name, i.unit_name from purchase_rfq_details a, item_info i, item_sub_group s where i.item_id=a.item_id  and i.sub_group_id=s.sub_group_id and a.invoice_no="'.$rfq_no.'" order by a.id';
  ...
  $foe_invoice = 'INSERT INTO purchase_quotation_details (invoice_no, view_invoice_no, invoice_date, group_for, warehouse_id, req_no, rfq_no, currency_type, vendor_id, order_no, req_order_no, item_id, unit_name, rfq_qty, unit_price, total_unit, status, entry_at, entry_by)
  
  VALUES("'.$invoice_no.'", "'.$view_invoice_no.'", "'.$invoice_date.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$req_no.'", "'.$rfq_no.'", "'.$currency_type.'", "'.$vendor_id.'", "'.$order_no.'", "'.$req_order_no.'", "'.$item_id.'", "'.$data->unit_name.'", "'.$rfq_qty.'", "'.$unit_price.'", "'.$total_unit.'", "'.$status.'", "'.$entry_at.'", "'.$entry_by.'")';
...
<div class="box" style="width:100%;">
    ...
    <td width="8%" align="right" ><strong> RFQ No: </strong></td>



    <td width="13%" >
	<? $rfq_data = find_all_field('purchase_rfq_master','','invoice_no="'.$rfq_no.'"'); ?>
	
	<input name="rfq_no" type="hidden" id="rfq_no" style="width:90%; height:32px;" value="<?=$rfq_data->invoice_no;?>" readonly="" required tabindex="1" />
	<input name="req_no" type="hidden" id="req_no" style="width:90%; height:32px;" value="<?=$rfq_data->req_no;?>" readonly="" required tabindex="1" />
	<input name="warehouse_id" type="hidden" id="warehouse_id" style="width:90%; height:32px;" value="<?=$rfq_data->warehouse_id;?>" readonly="" required tabindex="1" />
	
	<a href="../RFQ/invoice_print_view.php?invoice_no=<?=$rfq_no;?>" target="_blank"><input name="view_rfq_no" type="text" id="view_rfq_no" style="width:90%; height:32px;" value="<?=$rfq_data->view_invoice_no;?>" readonly="" required tabindex="1" /></a>	</td>
	 ...
	 <select name="group_for" id="group_for"  style="width:90%; height:32px;" required>
		...
		<? foreign_relation('user_group','id','group_name',$_POST['group_for'],'id="'.$rfq_data->group_for.'"');?>
	 ...
	 <select name="currency_type" id="currency_type"  style="width:90%; height:32px;" required>
		...
		<? foreign_relation('currency_type','currency_type','currency_type',$_POST['currency_type'],'currency_type="'.$rfq_data->currency_type.'"');?>
     ...
     <input type="submit" name="master_data" id="master_data" value="Initiate Entry" style="width:160px; text-transform:uppercase; background:#87CEFA; color:#000000; text-align:center; font-weight:bold; font-size:12px; height:30px; "/>
								  ...
								  <tr>
								    <td align="right" ><strong> REQ No: </strong></td>
								    <td >
									<a href="../REQUISITION/invoice_print_view.php?invoice_no=<?=$rfq_data->req_no;?>" target="_blank"><input name="view_req_no" type="text" id="view_req_no" style="width:90%; height:32px;" value="<?=$rfq_data->view_req_no;?>" readonly="" required tabindex="1" /></a>
									</td>
									...
									<select name="vendor_id" id="vendor_id"  style="width:90%; height:32px;" required>
										...
										<? foreign_relation('vendor','vendor_id','vendor_name',$_POST['vendor_id'],'vendor_id="'.$rfq_data->vendor_id.'"');?>
...
<div class="box" style="width:100%;">
	...
	<input name="rfq_no" type="hidden" id="rfq_no" style="width:90%; height:32px;" value="<?=$ms_data->rfq_no;?>" readonly="" required tabindex="1" />
	...
	<input name="view_invoice_no" type="text" id="view_invoice_no" style="width:90%; height:32px;" value="<?=$ms_data->view_invoice_no;?>" readonly="" required tabindex="1" /></td>
	 ...
	 <td width="11%" align="right"><strong>RFQ No: </strong></td>
	 <td width="18%" align="right"><a href="../RFQ/invoice_print_view.php?invoice_no=<?=$ms_data->rfq_no;?>" target="_blank">
	<input name="view_rfq_no" type="text" id="view_rfq_no" style="width:90%; height:32px;" value="<?=$ms_data->view_rfq_no;?>" readonly="" required tabindex="1" /></a>	</td>
...
//		}
     ...
     $sqlfg = "select a.*, s.sub_group_name, i.finish_goods_code, i.item_name, i.unit_name from purchase_rfq_details a, item_info i, item_sub_group s where i.item_id=a.item_id  and i.sub_group_id=s.sub_group_id and a.invoice_no='".$ms_data->rfq_no."' order by a.id";
...
<? //if($po_no[$data->po_no]==0) { } ?>
 ...
 <tr style="background-color: <?= ($i % 2) ? '#F5F5F5' : '#fff'; ?>;">
	...
	<input name="req_order_no_<?=$datafg->id?>" type="hidden" id="req_order_no_<?=$datafg->id?>" value="<?=$datafg->order_no?>" />
      <input name="item_id_<?=$datafg->id?>" type="hidden" id="item_id_<?=$datafg->id?>" value="<?=$datafg->item_id?>" />
	 
	<input name="rfq_qty_<?=$datafg->id?>" type="text" id="rfq_qty_<?=$datafg->id?>" value="<?=$datafg->total_unit; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" readonly="" style="width:100px; height:30px; " /></td>
    <td bgcolor="#99CCFF"><input name="unit_price_<?=$datafg->id?>" type="text" id="unit_price_<?=$datafg->id?>" value="" style="width:100px; height:30px; "/></td>
    <td style="background-color:#99CCFF;"><input name="total_unit_<?=$datafg->id?>" type="text" id="total_unit_<?=$datafg->id?>" value="1"  readonly="" style="width:100px; height:30px; "/></td>
  </tr>
