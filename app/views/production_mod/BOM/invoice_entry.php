...
<? //if($po_no[$data->po_no]==0) { } ?>
  ...
  <tr style="background-color: <?= ($i % 2) ? '#E8F3FF' : '#fff'; ?>;">
	...
	<input name="total_unit_<?=$datafg->id?>" type="text" id="total_unit_<?=$datafg->id?>" value="<?=$datafg->total_unit; $tot_by_unit +=$datafg->total_unit;?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " /></td>
 ...
 <?php } ?>
	...
	<tr>
		  ...
		  <td>
		   ...
		   <option></option>
		   <? foreign_relation('accounts_ledger','ledger_id','ledger_name',$_POST['ledger_id'],'ledger_group_id in (421025)')?>
		  </select>
