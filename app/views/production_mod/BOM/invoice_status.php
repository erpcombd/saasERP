...
//		}
 ...
 $res="select  m.* from  bom_master m, bom_raw_material d where m.bom_no=d.bom_no and  m.status!='MANUAL'  ".$con." group by m.bom_no order by m.bom_date, m.bom_no";
...
<td>
<!--<a href="invoice_print_view.php?bom_no=<?=rawurlencode(url_encode($data->bom_no));?>" target="_blank" class="btn btn-info">View</a>-->

<a href="invoice_print_view.php?c='<?=rawurlencode(url_encode($c_id));?>&v=<?=rawurlencode(url_encode($data->bom_no));?>'" target="_blank" class="btn btn-info">View</a>
<a href="invoice_entry.php?request_no=<?=$data->bom_no?>" target="_blank" class="btn btn-danger">Edit</a>
</td>
