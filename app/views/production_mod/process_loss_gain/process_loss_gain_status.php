		...
		<th>Transection Type</th>
                   ...
                   <? 
					...
					if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and m.req_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
					 	...
					 	$res="select m.*,w.warehouse_name,u.username

  								from process_loss_gain_master m,warehouse w, user_activity_management u
						
						where m.warehouse_id=w.warehouse_id and m.entry_by=u.user_id and m.status!='MANUAL' ".$con." order by m.req_no asc ";
					$query = db_query($res);
					...
					?>
                        <tr> 
							...
							<td class="text-left"><?=$data->warehouse_name?></td>
									<td class="text-left"><?=$data->issue_type;?></td>
