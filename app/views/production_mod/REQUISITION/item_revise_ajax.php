...
if($total_unit==0)
{
$del_sql = "DELETE from purchase_requisition_details where id='".$order_no."'";
db_query($del_sql);
}else{
$up_sql='update purchase_requisition_details set  total_unit="'.$total_unit.'",  edit_by="'.$entry_by.'", entry_at="'.$entry_at.'" where id='.$order_no;
db_query($up_sql);
}
