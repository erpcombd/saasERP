...
$title='Upcoming Purchase Receive List';
...
$target_url = '../pof/pr_bill_create.php';
...
$con .= 'and a.rec_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
   ...
   $res='select  a.pr_no, a.pr_no, DATE_FORMAT(a.rec_date, "%d-%m-%Y") as rec_date, a.po_no, 
 v.vendor_name as supplier_name,  IF(a.status="Received", "Bill Pending", "Omar") as Status, c.fname as entry_by, a.entry_at from purchase_receive a,warehouse b,user_activity_management c, vendor v where  a.warehouse_id=b.warehouse_id and a.entry_by=c.user_id and a.vendor_id=v.vendor_id and a.status="Received" '.$con.' group by a.pr_no order by a.pr_no';
