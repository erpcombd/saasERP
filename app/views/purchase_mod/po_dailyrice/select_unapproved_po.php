...
$unique = 'po_no';
$status = 'UNCHECKED';
$target_url = '../po_dailyrice/po_checking.php';
/*if($_POST[$unique]>0)
{
                    ...
                    <tbody class="tbody1">
						<? 
							...
							$con .= 'and p.po_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
							 ...
							 $res='select p.po_no,p.po_no,p.po_date,v.vendor_name,w.warehouse_name,u.fname as created_by,p.entry_at,p.status from purchase_master p, user_activity_management u, vendor v, warehouse w where p.warehouse_id=w.warehouse_id and p.group_for="'.$_SESSION['user']['group'].'" and v.vendor_id=p.vendor_id and u.user_id=p.entry_by and p.status="UNCHECKED" '.$con. ' order by p.po_no desc';
