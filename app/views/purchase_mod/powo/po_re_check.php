...
getData2('po_re_check_ajax.php', 'ppp',id,info);
}
...
<form action="?<?=$unique?>=<?=$$unique?>" method="post" name="cloud" id="cloud" >
  <table  width="100%" border="1" align="left"  style="border-collapse:collapse; border:1px solid #caf5a5;" cellpadding="2" cellspacing="2">
                      <tr>
                        ...
                        <td width="5%" align="center" bgcolor="#0099FF"><strong>Bag/Crt</strong></td>
...
<td>
<input name="price" type="text" class="input3" id="price" style="width:50px;float:left; height:20px;"  readonly="readonly"/></td>
</tr>
</table>
</span></td>
<td align="center" bgcolor="#CCCCCC"><input name="pkt_unit" type="text" class="input3" id="pkt_unit"  maxlength="100" style="width:50px; height:20px;" /></td>
<td align="center" bgcolor="#CCCCCC"><input name="qty" type="text" class="input3" id="qty"  maxlength="100" style="width:70px; height:20px;" onchange="count()" required="required"/></td>
	  ...
	  <br />
        ...
        <td><div class="tabledesign2">
          <table id="grp" width="100%" cellspacing="0" cellpadding="0"><tbody>
			...
			<th width="5%" rowspan="2">Bag/Crt</th>
...
<?
$s=0;
 $res='select a.id,  b.finish_goods_code as item_code, concat(b.item_name) as item_name, a.warehouse_id, w.warehouse_name, a.rate as unit_price,
...
<select name="<?='warehouse_id#'.$po_item->id?>" id="<?='warehouse_id#'.$po_item->id?>" style="width:180px;" required >
	  ...
	  <? foreign_relation('warehouse','warehouse_id','warehouse_name',$po_item->warehouse_id,'1 and use_type = "WH"');?>
	  </select>
</td>
...
<td><input type="text" name="<?='pkt_unit#'.$po_item->id?>" id="<?='pkt_unit#'.$po_item->id?>" value="<?=$po_item->pkt_unit?>" style="width:50px;  height:20px;" onchange="PIcalculation(<?=$po_item->id?>)" /></td>
<td><input type="text" name="<?='qty#'.$po_item->id?>" id="<?='qty#'.$po_item->id?>" value="<?=$po_item->qty?>" style="width:65px;  height:20px;" onchange="PIcalculation(<?=$po_item->id?>)" /></td>
...
<td><input type="text" name="<?='amount#'.$po_item->id?>" id="<?='amount#'.$po_item->id?>" value="<?=$po_item->amount?>" style="width:70px;  height:20px;"  readonly=""/></td>
...
<form action="purchase_order_re_checked.php" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
