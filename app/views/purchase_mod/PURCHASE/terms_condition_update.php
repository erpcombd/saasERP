...
if(isset($_POST['confirm'])){
	...
	$view_invoice_no = $_POST['view_invoice_no'];
	 ...
	 $sql = 'select a.* from purchase_terms_conditions a where a.invoice_no="'.$invoice_no.'" order by a.id';
...

		{
				...
				$tearms=$_POST['tearms_'.$data->id];
				$terms_conditions=$_POST['terms_conditions_'.$data->id];
				

$con_sql1='UPDATE purchase_terms_conditions SET tearms="'.$tearms.'", terms_conditions="'.$terms_conditions.'",  edit_by="'.$entry_by.'", edit_at="'.$entry_at.'"
 WHERE invoice_no ="'.$invoice_no.'" and id="'.$order_no.'" ';
	 db_query($con_sql1);
...
<script language="javascript">
window.location.href = "po_terms_condition_update.php";
</script>
...
<div class="box" style="width:100%;">
	...
	<input name="view_quote_no" type="text" id="view_quote_no" style="width:90%; height:32px;" value="<?=$ms_data->view_quote_no;?>" readonly="" required tabindex="1" /></a>	</td>
	 <td width="15%" style="text-align:right;"><strong>Currency:</strong></td>
	 <td width="21%" >
	<select name="currency_type" id="currency_type"  style="width:90%; height:32px;" required>
		...
		<? foreign_relation('currency_type','currency_type','currency_type',$_POST['currency_type'],'currency_type="'.$ms_data->currency_type.'"');?>
								  ...
								  <tr>
								    <td style="width:11%; text-align:right;"  ><strong> PO Date: </strong></td>
								    <td style="width:16%;" >
								    ...
								    <td style="width:13%; text-align:right;" ><strong> Vendor: </strong></td>
								    ...
								    <td style="width:15%; text-align:right;" ><strong>Freight Charge:</strong></td>
								    <td width="21%">
...
<table  id="grp"  style="width:100%; border:0; border-collapse:collapse; padding:0; text-center:center; font-size:12px; text-transform:uppercase;">

  <tr>
    <th style="width:21%;">Terms </th>
    <th style="width:79%;">Terms &amp; Condition</th>
    </tr>
  ...
  if($_POST['fdate']!=''&&$_POST['tdate']!=''){ $date_con .= ' and c.chalan_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';}
	...
	$po_dt_con=" and m.po_date='".$_POST['po_date']."'";}
     ...
     $sqlfg = "select a.* from purchase_terms_conditions a where a.invoice_no='".$ms_data->invoice_no."' order by a.id";
...
<? //if($po_no[$data->po_no]==0) { } ?>
  ...
  <tr style="background-color: <?=($i%2)?'#F5F5F5':'#fff';?>;">
    <td style="background-color:#99CCFF;">
        <input name="order_no_<?=$datafg->id?>" 
               type="hidden" 
               id="order_no_<?=$datafg->id?>" 
               value="<?=$datafg->id; ?>" 
               readonly 
               style="width:100px; height:30px;" />

        <input name="tearms_<?=$datafg->id?>" 
               type="text" 
               id="tearms_<?=$datafg->id?>" 
               value="<?=$datafg->tearms; ?>" 
               style="width:100px; height:30px;" />
    </td>

    <td style="background-color:#99CCFF;">
        <input name="terms_conditions_<?=$datafg->id?>" 
               type="text" 
               id="terms_conditions_<?=$datafg->id?>" 
               value="<?=$datafg->terms_conditions; ?>" 
               style="width:100px; height:30px;" />
    </td>
</tr>
