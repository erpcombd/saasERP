...
if(isset($_POST['convert'])){
 ...
 $pdata = mysqli_fetch_object($qry);
  $insert = 'insert into purchase_master(`group_for`,`po_date`,`vendor_id`,`req_no`,`quotation_no`,`quotation_date`,`warehouse_id`,`entry_by`,`entry_at`) value("'.$_SESSION['user']['group'].'","'.date('Y-m-d').'","'.$pdata->vendor_id.'","'.$pdata->req_no.'","'.$pdata->invoice_no.'","'.$pdata->invoice_date.'","'.$pdata->warehouse_id.'","'.$_SESSION['user']['id'].'","'.date('Y-m-d H:i:s').'")';
 $master_insert = db_query($insert);
 $po_no = db_insert_id();
 
 echo $sql2 = 'select * from purchase_quotation_details where invoice_no="'.$quotation_no.'" and app_status=1';
 $qry=db_query($sql2);
 while($data=mysqli_fetch_object($qry)){
    $details_insert = 'insert into purchase_invoice(`po_no`,`po_date`,`req_no`,`quotation_no`,`quotation_id`,`vendor_id`,`item_id`,`warehouse_id`,`rate`,`qty`,`amount`,`entry_by`,`entry_at`) value("'.$po_no.'","'.date('Y-m-d').'","'.$data->req_no.'","'.$data->invoice_no.'","'.$data->id.'","'.$data->vendor_id.'","'.$data->item_id.'","'.$data->warehouse_id.'","'.$data->unit_price.'","'.$data->req_qty.'","'.($data->unit_price*$data->req_qty).'","'.$_SESSION['user']['id'].'","'.date('Y-m-d H:i:s').'")';
...
echo '<script>window.location.href="../po/po_create.php"</script>';
}
