...
$status = 'UNCHECKED';
 ...
 $pdata = mysqli_fetch_object($qry);
  $insert = 'insert into purchase_master(`group_for`,`po_date`,`purchase_type`,`vendor_id`,`req_no`,`quotation_no`,`quotation_date`,`vat`,`tax`,vat_include,tax_include,deductible,`ait`,`cash_discount`,`rebateable`,`rebate_percentage`,`ref_no`,`warehouse_id`,`entry_by`,`entry_at`,`qc_with_parameter`) value("'.$_SESSION['user']['group'].'","'.date('Y-m-d').'","'.$pdata->po_type.'","'.$pdata->vendor_id.'","'.$pdata->req_no.'","'.$pdata->invoice_no.'","'.$pdata->invoice_date.'","'.$pdata->vat.'","'.$pdata->tax.'","'.$pdata->vat_include.'","'.$pdata->tax_include.'","'.$pdata->deductible.'","'.$pdata->ait.'","'.$pdata->discount.'","'.$pdata->rebate.'","'.$pdata->rebate_percentage.'","'.$pdata->ref.'","'.$pdata->warehouse_id.'","'.$_SESSION['user']['id'].'","'.date('Y-m-d H:i:s').'","'.$pdata->qc_with_parameter.'")';
