...
//   	 $lot_no_generate='LOT'.$tr_no.''.$cy_id;
	 ...
	 if($req_no>0) {
	  echo $ins_sql = 'INSERT INTO purchase_quotation_master (invoice_no, year, inv_id, view_invoice_no, invoice_date, group_for, warehouse_id, req_no, view_req_no, currency_type, vendor_id,  status, entry_at, entry_by, file_upload) VALUES
...
window.location.href = "invoice_entry_test.php?request_no=<?=$invoice_no?>";
</script>
...
<?php /*?>	  <input name="group_for" type="text" id="group_for" style="width:90%; height:32px;" readonly="" required tabindex="1" list="gf" />
	  <datalist id="gf">
	  <? foreign_relation('user_group','id','company_name',$_POST['group_for'],'id="'.$req_data->group_for.'"');?>
	  </datalist><?php */?>
	...
	<tr>
								    ...
								    <td >
		<select name="department" id="department"  style="width:90%; height:32px;"  >
		...
		<? foreign_relation('wh_section','id','section_name',$req_data->department,'1');?>
								  ...
								  <tr>
								    ...
								    <td ><input name="invoice_date" type="text" id="invoice_date" style="width:90%; height:32px;" value="<?=$req_date;?> " required tabindex="1" /></td>
							      </tr>
								  <tr>
...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0" style="font-size:12px; text-transform:uppercase;">
  ...
  if($_POST['fdate']!=''&&$_POST['tdate']!=''){ $date_con .= ' and c.chalan_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';}
	...
	$po_dt_con=" and m.po_date='".$_POST['po_date']."'";
		 ...
		 echo $vendor_id[$info->item_id]=$info->vendor_id;
		}
		
		 $sql = "select vendor_id, vendor_name from vendor where 1 group by vendor_id";
		 $query = db_query($sql);
		 while($vinfo=mysqli_fetch_row($query)){
  		  $vendor_name[$info->vendor_id]=$vinfo->vendor_name;
		}
