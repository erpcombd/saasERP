...
<?
				...
				$requisition_no[$i] = $data->req_no;
		 		}
				
				
				
				  $sql  = 'select d.id,d.vendor_id,d.item_id,d.req_no, d.origin,  d.qty, d.quotation_price as qut_price,d.order_no
				 from quotation_master m, quotation_detail d
				 where m.quotation_no=d.quotation_no and m.req_no="'.$req_no.'" group by d.vendor_id,d.order_no order by d.id,d.vendor_id asc';
				$query = db_query($sql);	
				$i=1;
				$k = 0;
				while($data=mysqli_fetch_object($query)){
				 ...
				 $totalP[$data->vendor_id][$data->order_no] = $data->qut_price*$data->qty;
				 $origin[$data->vendor_id][$data->order_no] = $data->origin;
...
for($i=1;$i<=$cols;$i++)
{
  ...
  $rate = $vals[$vendor_id[$i]][$info2->id];
 // var_dump($rate);
if($rate>0){


?>
    <td><?=number_format($vals[$vendor_id[$i]][$info2->id],2) ?></td>
	<td><?=number_format($totalP[$vendor_id[$i]][$info2->id],2); $totalSub += $totalP[$vendor_id[$i]][$info2->id];?></td>
	<td><?=$origin[$vendor_id[$i]][$info2->id]?></td>
...
$is_po = 'SELECT vendor_name, total_amount
    FROM ( select v.vendor_name,(d.qty * d.quotation_price) AS total_amount
    FROM quotation_master m
    JOIN quotation_detail d ON m.quotation_no = d.quotation_no
    JOIN vendor v ON d.vendor_id = v.vendor_id
    WHERE m.is_po = 1 and m.req_no="'.$req_no.'" and d.item_id="'.$info2->item_id.'" ) AS sub
...
<? }?>
		...
		<tr>
			...
			<?  $s_sql = 'select vendor_id, sum(qty*quotation_price) amt,id from quotation_detail where req_no="'.$req_no.'"  group by vendor_id order by id,vendor_id asc ';
