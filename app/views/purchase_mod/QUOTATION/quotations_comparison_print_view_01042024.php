...
<table  class="tabledesign"  bordercolor="#000000" cellspacing="0" cellpadding="0">
<?

	  
	   			  $sql  = 'select q.req_no, q.vendor_id, v.vendor_name
				from quotation_detail q,  vendor v
				 where q.vendor_id=v.vendor_id and q.req_no="'.$req_no.'"  group by q.vendor_id order by q.id,q.vendor_id asc';
				$query = db_query($sql);
				...
				$vendor_id[$i] = $data->vendor_id;
				  ...
				  $sql  = 'select d.id,d.vendor_id,d.item_id,d.req_no, d.origin,  d.qty, d.quotation_price as qut_price
				 from quotation_master m, quotation_detail d
				 where m.quotation_no=d.quotation_no and m.req_no="'.$req_no.'" group by d.vendor_id,d.item_id order by d.id,d.vendor_id asc';
				$query = db_query($sql);	
				$i=1;
				$k = 0;
				while($data=mysqli_fetch_object($query)){
				 
				 $vals[$i][$data->vendor_id][$data->item_id] = $data->qut_price;
				 
				 $totalP[$i][$data->vendor_id][$data->item_id] = $data->qut_price*$data->qty;
				 $origin[$i][$data->vendor_id][$data->item_id] = $data->origin;
				 
				 
				 if($k>0 && $data->vendor_id!=$old_vendor){ $i=1; }
				 //echo $i.'-'.$data->vendor_id.'-'.$data->item_id;
				 //echo '<br>';
				 $i++;
...
?>
       ...
       <tr>
        ...
        <td bgcolor="#F5F5F5" rowspan="2" colspan="1" ><strong>SL</strong></td>

        <td  bgcolor="#F5F5F5" rowspan="2" colspan="1" ><strong> Item Description </strong></td>
		<td  bgcolor="#F5F5F5" rowspan="2" colspan="1" ><strong> Brand </strong></td>
		<td  bgcolor="#F5F5F5" rowspan="2" colspan="1" ><strong> Quantity </strong></td>
        <td  bgcolor="#F5F5F5" rowspan="2" colspan="1" ><strong>Unit</strong></td>
		
		 <?

for($i=1;$i<=$cols;$i++)
{
echo '<td bgcolor="#98FB98" colspan="3"  style="border: 1px solid #000000;" ><strong>'.$vendor_name[$i].'</strong></td>';
}
$colsinto3 = $cols*3;
?>

		<td  bgcolor="#F5F5F5" rowspan="2" colspan="1" ><strong>Selected Total in BD</strong></td>
		<td  bgcolor="#F5F5F5" rowspan="2" colspan="1" ><strong>Selected Supplier</strong></td>
		<td  bgcolor="#F5F5F5" rowspan="2" colspan="1" ><strong>Remarks</strong></td>
...
$sql3="select a.*, s.sub_group_name, i.sub_group_id, i.item_name,i.item_id, a.qty,  i.finish_goods_code from requisition_order a,item_info i, item_sub_group s where a.item_id=i.item_id  and i.sub_group_id=s.sub_group_id and a.req_no='".$req_no."' order by i.sub_group_id, i.item_name";
...
<tr>
		...
		<?
$s_sql = 'select vendor_id, sum(qty*quotation_price) amt from quotation_detail where req_no="'.$req_no.'"  group by vendor_id order by sum(qty*quotation_price) asc ';
$s_qry = db_query($s_sql);

while($sdt = mysqli_fetch_object($s_qry )){
$amtAr[]  = $sdt->amt;	
$q_sql = 'select * from quotation_detail where req_no="'.$req_no.'" and item_id="'.$info2->item_id.'" and vendor_id='.$sdt->vendor_id.' order by id,vendor_id asc';
//$q_sql = 'select q.vendor_id,q.item_id from requisition_order r left join quotation_detail q on q.req_no=r.req_no where r.req_no="'.$req_no.'" and r.item_id="'.$info2->item_id.'" group by r.item_id';

$q_qry = db_query($q_sql);
while($qdata=mysqli_fetch_object($q_qry)){

for($i=1;$i<=$cols;$i++)
{
$rate = $vals[$i][$qdata->vendor_id][$info2->item_id];
if($rate>0){


?>
    <td><?=number_format($vals[$i][$qdata->vendor_id][$info2->item_id],2).'-'.$qdata->vendor_id?></td>
	<td><?=number_format($totalP[$i][$qdata->vendor_id][$info2->item_id],2); $totalSub += $totalP[$i][$qdata->vendor_id][$info2->item_id];?></td>
	<td><?=$origin[$i][$qdata->vendor_id][$info2->item_id]?></td>
	...
	$is_po = 'select v.vendor_name,(d.qty*quotation_price) as total_amount from quotation_master m, quotation_detail d, vendor v where d.vendor_id=v.vendor_id and m.quotation_no=d.quotation_no and m.is_po=1 and m.req_no="'.$req_no.'" and d.item_id="'.$info2->item_id.'"';
	$qry = db_query($is_po);
	$po_data = mysqli_fetch_object($qry);
	?>
	
	<td><?=number_format($po_data->total_amount,2);  ?></td>
	<td><?=$po_data->vendor_name?></td>
	<td></td>
...
<td>
	 ...
	 echo ' Only';
        ...
        <tr>
          <td align="center"><strong>Asst. Manager - ICT</strong></td>
          <td align="center"><strong>BizDev(Manager)</strong></td>
