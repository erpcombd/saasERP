...
<?
				...
				from purchase_quotation_master q,  vendor v
				 where q.vendor_id=v.vendor_id and q.req_no="'.$invoice_no.'"  group by q.vendor_id order by q.invoice_no';
				$query = db_query($sql);
				...
				$view_invoice_no[$i] = $data->view_invoice_no;
				 ...
				 where m.invoice_no=d.invoice_no and m.req_no="'.$invoice_no.'"  group by d.vendor_id, d.order_no ';
				$query = db_query($sql);	
				while($data=mysqli_fetch_object($query)){++$i;
...
?>
       ...
       <tr>
        ...
        <td width="13%" rowspan="2" bgcolor="#F5F5F5" ><strong>Type</strong></td>
        <? if($count_sum>0) {?><td width="9%" rowspan="2" bgcolor="#F5F5F5" ><strong>Count</strong></td>
        <? }?>
        <td width="5%" rowspan="2" bgcolor="#F5F5F5" ><strong>Unit</strong></td>
        <td width="8%" rowspan="2" bgcolor="#F5F5F5" ><strong>Country of Origin</strong></td>
