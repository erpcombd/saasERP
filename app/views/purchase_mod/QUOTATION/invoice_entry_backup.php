...
if(isset($_POST['master_data'])){
	...
	$invoice_date = $_POST['invoice_date'];
	 ...
	 if($req_no>0) {
	   $ins_sql = 'INSERT INTO purchase_quotation_master (invoice_no, year, inv_id, view_invoice_no, invoice_date, group_for, warehouse_id, req_no, view_req_no, currency_type, vendor_id, vat, tax,vat_include, tax_include,deductible, ait, discount, rebate, status, entry_at, entry_by, file_upload,ref,terms_condition,po_type) VALUES

("'.$invoice_no.'", "'.$YR.'", "'.$cy_id.'", "'.$view_invoice_no.'", "'.$invoice_date.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$req_no.'", "'.$view_req_no.'", "'.$currency_type.'", "'.$vendor_id.'",  "'.$vat.'",  "'.$tax.'","'.$tax_include.'","'.$vat_include.'","'.$deductible.'",  "'.$ait.'",  "'.$discount.'",  "'.$rebate.'", 
...
if(isset($_POST['confirm'])){
    ...
    $foe_invoice = 'INSERT INTO purchase_quotation_details (invoice_no, view_invoice_no, invoice_date, group_for, warehouse_id, req_no,  currency_type, vendor_id, order_no,  item_id, unit_name, unit_price, total_unit, status, entry_at, entry_by, item_type, count_type, req_qty, country_origin, last_po_no, last_po_id,remarks,vendor_specification)
  ...
  "'.$req_qty.'",  "'.$data->country_origin.'", "'.$last_po_no.'", "'.$last_po_id.'", "'.$remarks.'", "'.$vendor_specification.'")';
...
function calculateTotal() {
    ...
    // Loop through each row and calculate unit * price
...
<input name="requisition_user_name" type="text" id="requisition_user_name" readonly="readonly" style="width:90%; height:32px;" value="<?=find_a_field('user_activity_management','fname','user_id='.$req_data->entry_by);?>"   tabindex="1" /></td>
		...
		<? foreign_relation('warehouse','warehouse_id','warehouse_name',$_POST['section'],'1 and warehouse_id="'.$req_data->warehouse_id.'"');?>
								  ...
								  <tr>
								  	<td align="right" ><strong> AIT: </strong>  </td>
								    <td ><input name="ait" type="text" id="ait" style="width:90%; height:32px;" value=""   tabindex="1" /></td>
								    <td align="right" ><strong> Discount: </strong></td>
								    <td ><input name="discount" type="text" id="discount" style="width:90%; height:32px;" value=""   tabindex="1" /></td>
								    <td align="right"><strong> Vat Rebate: </strong></td>
								    <td >
									<select name="rebate" value="" >
											<option></option>
			...
			<div class="row pt-3">
				<div class="col-md-12">
							<textarea name="terms_condition"  id="terms_condition"></textarea>
				</div>
			</div>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <td width="20%" ><input name="invoice_no" type="hidden" id="invoice_no" style="width:90%; height:32px;" value="<?=$ms_data->invoice_no;?>" readonly="" required tabindex="1" />
          <input name="req_no2" type="hidden" id="req_no2" style="width:90%; height:32px;" value="<?=$ms_data->req_no;?>" readonly="" required tabindex="1" />
          <input name="warehouse_id2" type="hidden" id="warehouse_id2" style="width:90%; height:32px;" value="<?=$ms_data->warehouse_id;?>" readonly="" required tabindex="1" />
      ...
      <td width="18%" align="right"><a href="../../warehouse_mod/mr/mr_print_view.php?req_no=<?=$ms_data->req_no;?>" target="_blank">
        <input name="view_req_no2" type="text" id="view_req_no2" style="width:90%; height:32px;" value="<?=$ms_data->view_req_no;?>" readonly="" required tabindex="1" />
      </a> </td>
      <td width="19%" align="right"><strong>Company: </strong></td>
      <td width="21%" ><select name="select" id="select"  style="width:90%; height:32px;" required>
          <? foreign_relation('user_group','id','company_name',$_POST['group_for'],'id="'.$req_data->group_for.'"');?>
        </select>
      </td>
    </tr>
    <tr>
      ...
      <td width="20%" ><input name="invoice_date2" type="text" id="invoice_date2" style="width:90%; height:32px;" value="<?=$ms_data->invoice_date?>" required readonly="" tabindex="1" />
      </td>
      <td align="right" width="11%"><strong> Vendor: </strong></td>
      <td width="18%" align="right"><select type="text" list="vendor" name="select" id="vendor_id"  required  >
      ...
      <td width="21%"><select name="select" id="select2"  style="width:90%; height:32px;" required>
          <? foreign_relation('currency_type','currency_type','currency_type',$_POST['currency_type'],'currency_type="'.$ms_data->currency_type.'"');?>
      </select></td>
    </tr>
    <tr>
      <td align="right" ><strong>Department: </strong></td>
      <td ><input name="requisition_by2" type="text" id="requisition_by2" readonly="readonly" style="width:90%; height:32px;" value="<?=$req_data->req_for;?>"   tabindex="1" /></td>
      <td align="right"><strong> Requisition From: </strong></td>
      <td ><input name="requisition_user_name2" type="text" id="requisition_user_name2" readonly="readonly" style="width:90%; height:32px;" value="<?=find_a_field('user_activity_management','fname','user_id='.$req_data->entry_by);?>"   tabindex="1" /></td>
      <td align="right"><strong> Section: </strong></td>
      <td ><select name="select" id="select3"  style="width:90%; height:32px;"  readonly="readonly" >
          <? foreign_relation('warehouse','warehouse_id','warehouse_name',$_POST['section'],'1 and warehouse_id="'.$req_data->warehouse_id.'"');?>
      </select></td>
    </tr>
    <tr>
      <td align="right" ><strong> Requisition Remarks: </strong></td>
      <td ><input name="remarks2" type="text" id="remarks2" style="width:90%; height:32px;" value="<?=$req_data->req_note;?>" readonly=""   tabindex="1" />
      </td>
      <td align="right" ><strong> Vat(%): </strong></td>
      <td ><input name="vat2" type="text" id="vat2" style="width:90%; height:32px;" value="<?=$ms_data->vat;?>"   tabindex="1" /></td>
      <td align="right"><strong> Tax(%): </strong></td>
      <td ><input name="tax2" type="text" id="tax2" style="width:90%; height:32px;" value="<?=$ms_data->tax;?>"   tabindex="1" /></td>
    </tr>
    <tr>
      <td align="right" ><strong> Vat: </strong></td>
      <td ><input name="vat_include2" type="text" id="vat_include2" style="width:90%; height:32px;" value="<?=$ms_data->vat_include;?>" readonly=""   tabindex="1" />
      </td>
      <td align="right" ><strong> Tax: </strong></td>
      <td ><input name="tax_include2" type="text" id="tax_include2" style="width:90%; height:32px;" value="<?=$ms_data->tax_include;?>" readonly=""   tabindex="1" /></td>
      <td align="right"><strong> Deductible: </strong></td>
      <td ><input name="deductible2" type="text" id="deductible2" style="width:90%; height:32px;" value="<?=$ms_data->deductible;?>"   tabindex="1" /></td>
    </tr>
    <tr>
      <td align="right" ><strong> AIT: </strong> </td>
      <td ><input name="ait2" type="text" id="ait2" style="width:90%; height:32px;" value="<?=$ms_data->ait;?>"   tabindex="1" /></td>
      <td align="right" ><strong> Discount: </strong></td>
      <td ><input name="discount2" type="text" id="discount2" style="width:90%; height:32px;" value="<?=$ms_data->discount;?>"   tabindex="1" /></td>
      <td align="right"><strong> Vat Rebate: </strong></td>
      <td ><select name="select" value="<?=$ms_data->rebate;?>" >
          <option></option>
    ...
    <tr>
      <td align="right" ><strong> Reference: </strong></td>
      <td ><input name="ref2" type="text" id="ref2" style="width:90%; height:32px;" value="<?=$ms_data->ref;?>"   tabindex="1" /></td>
      <td align="right" ><strong> Attachment: </strong></td>
      <td ><input name="file_upload2" type="file" id="file_upload2" style="width:90%; height:32px;" value="<?=$ms_data->file_upload;?>" readonly=""   tabindex="1" />
...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0" style="font-size:12px; text-transform:uppercase;">
  ...
  if($_POST['fdate']!=''&&$_POST['tdate']!=''){ $date_con .= ' and c.chalan_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';}
	...
	$po_dt_con=" and m.po_date='".$_POST['po_date']."'";
       ...
       $sqlfg = "select a.*, s.sub_group_name, i.finish_goods_code, i.item_name, i.unit_name from requisition_order a, item_info i, item_sub_group s where i.item_id=a.item_id  and i.sub_group_id=s.sub_group_id and a.req_no='".$ms_data->view_req_no."' order by a.id";
...
<input name="req_qty_<?=$datafg->id?>" type="text" id="req_qty_<?=$i?>" value="<?=$datafg->qty;?>" onkeyup="TRcalculationpo(<?=$i?>)" readonly="" style="width:80px; height:30px; " /></td>
    <td bgcolor="#99CCFF"><input name="unit_price_<?=$datafg->id?>" type="text" id="unit_price_<?=$i?>" onkeyup="calculateTotal()" value="" style="width:80px; height:30px; "/></td>
