...
require_once "../../../controllers/routing/print_view.top.php";
 ...
 $po_no = url_decode(str_replace(' ', '+', $_REQUEST['v']));
...
<?php /*?>		<td class="Qrl_code">
	...
	<td class="qr-code">
        ...
        $req_no_qr_data = url_encode($po_no);
		//$print_url = "https://saaserp.ezzy-erp.com/app/views/purchase_mod/po/po_print_view_qr.php?c=".$cid."v=" . $req_no_qr_data;
        $print_url = "https://saaserp.ezzy-erp.com/app/views/purchase_mod/po/po_print_view.php?c=".rawurlencode($company_id)."&v=" . rawurlencode($req_no_qr_data);
	...
	<div class="row">
		...
		<div class="col-md-6 col-sm-6 col-lg-6 left">
			...
			<p class="left-text">Purchase Requisition No: <span> <? if (!empty($master->req_no) && $master->req_no != "0") {echo $master->req_no; }else{echo 'N/A';}?></span></p>
			<p class="left-text">Estimated Delivery Date: <span><? if (!empty($master->estimated_delivery_date) && $master->estimated_delivery_date != "0") {echo $master->estimated_delivery_date;} else {echo 'N/A';}?></span></p>

<p class="left-text"> Remarks: <span> <?php if (!empty($master->remarks)) {echo $master->remarks;} else {echo 'N/A';}?></span></p>
			...
			<div class="new-div">
				...
				<p class="text1">Email: <span><?php  $vendor_email=find_a_field('vendor','email','vendor_id="'.$master->vendor_id.'"'); if (!empty($vendor_email)) {echo $vendor_email;} else {echo 'N/A';}?> </span></p>
			</div>
		...
		<div class="col-md-6 col-sm-6 col-lg-6 right">
			...
			<p class="right-text"> RFQ No:  <span><?if(!empty($master->quotation_no)){ echo $master->quotation_no; }else{echo 'N/A';} ?></span></p>
			<p class="right-text">RFQ Date: <span><?if(!empty($master->quotation_date)){echo $master->quotation_date; }else{echo 'N/A';} ?></span></p>
...
<!--<div class="main-content">-->
    ...
    Unit Price 
    <?php 
        if($master->vat_include == 'Including'){
            echo "<br><span style='font-weight: normal;'>(Including VAT)</span>";
        } else {
            echo "<br><span style='font-weight: normal;'>(Excluding VAT)</span>";
        }
    ?>
</th>
    ...
    <?php 
        if($master->vat_include == 'Including'){
            ...
            echo " <span style='font-weight: normal;'>(Excluding VAT)</span>";
        }
    ?>
</th>
		...
		?>
			...
			<tr>
				...
				<td style="text-align:right;"><?=number_format($info->with_vat_rate,2);?></td>
				
				<td style="text-align:right;"><?=number_format($info->with_vat_amt,2);?></td>
			  </tr>
		<? 
...
<?php 
    $scs = number_format((float)$payable_amount, 2, '.', '');  
    $credit_amt = explode('.', $scs);  


    if (!empty($credit_amt[0]) && $credit_amt[0] > 0) {
        echo convertNumberToWordsForIndia($credit_amt[0]) . ' Taka';
    }


    if (!empty($credit_amt[1]) && intval($credit_amt[1]) > 0) {
        $paisa = intval($credit_amt[1]);  
        echo ' and ' . convertNumberToWordsForIndia($paisa) . ' Paisa';
    }
...
<p class="p-text"> The Vendor has agreed to provide the above goods/services to <?=$group_data->group_name?> agreed to accept the merchandise from the Vendor in the quantities, at the prices, at the time and subject to the terms, provisions and conditions stated below: <p>
	...
	<table class="terms-table">
			<tr>
			  <td colspan="2" class="terms-titel">Terms &amp; Condition : </td>
			</tr>
			...
			$sql = 'select * from purchase_master where po_no="'.$po_no.'"';
				...
				?>		
			<tr style="letter-spacing:.2px; line-height: 16px;">
...
<div class="footer1" id="footer1">
    <table class="footer-table" style="margin-left:0; text-align:left; width:100%;">
        <tr>
            <td colspan="10">&nbsp;</td>
        </tr>

        <tr>
            <!-- Prepared By (Always first column) -->
            <td class="text-left w-25">
                <p style="font-weight:600; margin: 0; padding-left: 17px !important;">
                    <?=find_a_field('user_activity_management','fname','user_id='.$master->entry_by);?>
                </p>
                <p style="font-size:11px; margin: 0;">
                    (<?=find_a_field('user_activity_management','designation','user_id='.$master->entry_by);?>) <br/>
                    <?=$master->entry_at?>
                </p>
            </td>

            <!-- Approval Members -->
            <?php while($row4=mysqli_fetch_object($query_approval_member3)){ ?>
                <td class="text-left w-25">
                    <p style="font-weight:600; margin: 0;"><?=$row4->fname?></p>
                    <p style="font-size:11px; margin: 0;">
                        (<?=$row4->layer_name?>) <br/> <?=$row4->approved_at?>
                    </p>
                </td>
            <?php } ?>
        </tr>
        ...
        <tr>
            ...
            <?php while($row2=mysqli_fetch_object($query_approval_member)){ ?>
                <td class="text-left">-------------------------------</td>
            <?php } ?>
        </tr>
        ...
        <tr>
            ...
            <?php while($row3=mysqli_fetch_object($query_approval_member2)){ ?>
                <td class="text-left"><strong><?=$row3->signatory_tittle?></strong></td>
            <?php } ?>
        </tr>
    </table>
