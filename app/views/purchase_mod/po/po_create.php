...
$tablecontact = 'item_parameter_details';
...
{
$_POST['po_no']=$_SESSION['po_no'];
$_POST['po_master_id']=$_SESSION['po_no'];
...
$_POST['entry_by']=$_SESSION['user']['id'];
		...
		$msg='New Entry Successfully Inserted.';
		 echo "<script>window.top.location='../po/po_create.php'</script>";
}
...
$_POST['edit_by']=$_SESSION['user']['id'];
        ...
        echo "<script>window.top.location='../po/po_create.php?view'</script>";
}
...

{
		...
		$item_id = find_a_field($table_details, 'item_id', 'id='.$_GET['del']);
		$po_item = find_a_field($table_details, 'po_no', 'id='.$_GET['del']);
		$condition="id=".$_GET['del'];		

		$crud->delete_all($condition);
		
		 $crud2 = new crud('item_parameter_details');
    	$crud2->delete_all('item_id="'.$item_id.'" and po_no="'.$po_item.'"');
...
{		
	...
	$tr_type = "Remove";
    $crud    = new crud('item_parameter_details');

    // delete only that row from item_parameter_details by its id
    $condition = "id=".$_GET['dele'];        
    $crud->delete_all($condition);

    $type = 1;
    $msg  = 'Successfully Deleted from item_parameter_details.';
...
{		
		...
		$id_detail = find_all_field('purchase_invoice','','id="'.$_POST['iddd'].'"');
		$id_master = find_all_field('purchase_master','','po_no="'.$id_detail->po_no.'"');

		$rate = $id_detail->rate;
		$amount = $id_detail->rate*$_POST['quantity'];
		
		
		if($id_master->vat_include=='Including'){
		
			$vat_rate=number_format((($rate/(100+$id_detail->vat))*$id_detail->vat),4,'.','');
			
			$vat_amt = $vat_rate*$_POST['quantity'];
			if($id_master->rebate == 'Yes'){
			...
			$with_vat_amt =number_format(($with_vat_rate * $_POST['quantity']),4,'.','');
		}else{
		
			$vat_rate=(($rate* $id_detail->vat)/100);
			$vat_amt = $vat_rate*$_POST['quantity'];
		...
		$tax_rate = (($with_vat_rate*$id_detail->tax)/100);
		$tax_amt=number_format(($tax_rate*$_POST['quantity']),4,'.','');
		
		if($id_master->tax_include=='Including'){
			if($id_master->rebate == 'Yes'){
				if($id_master->vat_include=='Excluding'){$grand_amount  = $amount+$vat_amt-$tax_amt;}
				else{ $grand_amount  = $amount-$tax_amt; }
			}
			else{ 
				$tax_rate = ((($with_vat_rate-$vat_rate)*$id_detail->tax)/100);
				$tax_amt=number_format(($tax_rate*$_POST['quantity']),4,'.','');
				if($id_master->vat_include=='Excluding'){$grand_amount  = $amount+$vat_amt-$tax_amt;}
				else{ $grand_amount  = $amount-$tax_amt; }
			}
		}else{
			if($id_master->vat_include=='Including'){ $tax_rate = (($with_vat_rate*$id_detail->tax)/100); }
			else{ $tax_rate = (($rate*$id_detail->tax)/100); }
			
			$tax_amt=number_format(($tax_rate*$_POST['quantity']),4,'.','');
			$with_vat_rate=$with_vat_rate+$tax_rate;
			$with_vat_amt =$with_vat_rate * $_POST['quantity'];
		...
		$tolerance_qty=$_POST['quantity']+($_POST['quantity']* (10/100));
		
		echo $updatee_sql = 'UPDATE purchase_invoice
								SET
								  qty = "'.$_POST['quantity'].'",
								  amount = "'.$amount.'",
								  vat_amt = "'.$vat_amt.'",
								  ...
								  vat_rate = "'.$vat_rate.'",
								  tax_rate = "'.$tax_rate.'",
								  with_vat_rate = "'.$with_vat_rate.'",
								  with_vat_amt = "'.$with_vat_amt.'"
								WHERE
								  id = "'.$id_detail->id.'"';
		db_query($updatee_sql);
}
...
auto_complete_from_db('item_info','item_name','concat(finish_goods_code,"##",item_name,"##",item_id)','status="Active" and group_for="'.$group_for.'"','item_id');
                    ...
                    <div class="container n-form2">
						...
						<div class="form-group row m-0 pb-1">
                            ...
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
									...
									<option></option>						
<? foreign_relation('vendor v','concat(v.vendor_name,"##",v.vendor_id)','""',$vendor_id_draft,'v.status="ACTIVE" and group_for in ('.$user_group_define.')');?>
 ...
 value="<?=find_a_field('vendor','concat(vendor_name,"##",vendor_id)','vendor_id="'.$vendor_id.'"');?>" readonly=""  required/>
						...
						<? $field='qc_with_parameter';?>

                            <label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Qc Enable</label>

                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">

                                	 

							<select name="<?=$field?>"  id="<?=$field?>" <? if(!empty($$field)){ echo'disabled';} ?> required >
						...
						<div class="form-group row m-0 pb-1">
						 ...
						 <? $field='file_upload';?>
                            ...
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
									  ...
									  <a href="<?=SERVER_CORE?>core/upload_view.php?name=<?=$$field?>&folder=purchase&proj_id=<?=$_SESSION['proj_id']?>" target="_blank">
									<?php /*?>  <a href="../../../controllers/uploader/upload_view.php?proj_id=<?=$_SESSION['proj_id']?>&&folder=purchase&&name=<?=$file_upload;?>" target="_blank"><?php */?>
...
elseif($_SESSION[$unique]>0){ ?>
	...
	<? 
		...
		$master = find_all_field('purchase_master','','po_no='.$$unique.' ');
		?>
...
?>
        <div class="container-fluid pt-5 p-0 ">
			<? if($purchase_type==2 && $master->qc_with_parameter == 'NO'){ ?>
			<table class="table1  table-striped table-bordered table-hover table-sm">
...
<input  name="item_id" type="text" id="item_id" value="<?=$item_id?>" style="width:320px;" required onblur="getData2('po_ajax.php', 'po',this.value,document.getElementById('warehouse_id').value);"/>
				...
				elseif($purchase_type==2 && $master->qc_with_parameter == 'Yes'){ ?>
			<table class="table1  table-striped table-bordered table-hover table-sm">
        ...
        <div class="container-fluid pt-5 p-0 ">
            <table class="table1  table-striped table-bordered table-hover table-sm">
                ...
                <tr>
                    ...
                    <td>
					<form action="?<?= $unique ?>=<?= $$unique ?>" method="post" name="cloud" id="cloud">
					<input name="quantity" type="text" class="input3" id="quantity" value="<?=$data->total_unit?>" />
					<input name="iddd" type="hidden" class="input3" id="iddd" value="<?=$data->id?>" />
					...
					<!--<td><a href="?edit=<?=$data->id?>"><button type="button" class="btn2 btn1-bg-cancel"><i class="fa-solid fa-edit"></i></button></a></td>-->
					
					<td><input name="edit" type="submit" id="edit" class="btn1 btn1-bg-submit" value="EDIT" tabindex="12"  />
...
<td></td>
		...
		echo	$res='select a.id, b.finish_goods_code as item_code,a.item_id, a. with_vat_rate, a.vat_amt,a.tax_amt, a. with_vat_amt, concat(b.item_name) as item_name,b.unit_name,a.grand_amount, a.qty as total_unit,a.rate, a.amount,"x" from purchase_invoice a,item_info b where b.item_id=a.item_id and a.po_no='.$po_no;
?>
    ...
    <table class="table1  table-striped table-bordered table-hover table-sm">
        ...
        while($data=mysqli_fetch_object($query)){ ?>
            <tr>
                ...
                <td>
                    <input name="item_id" type="hidden" class="input3" id="item_id" value="<?=$data->item_id?>" />
                    <input name="po_details_id" type="hidden" class="input3" id="po_details_id" value="<?=$data->id?>" />
                    <button class="btn2 btn1-bg-update toggle" data-toggle="modal" data-target="#contactmodal_<?=$data->id?>"><i class="fa-solid fa-user-plus" style="color: #ffffff;"></i> Add parameter</button>
                </td>
            </tr>
            ...
            <tr>			
                <td colspan="12">
                    <div class="row m-0 p-0">
                        <?php
                         $sqlTasks = "SELECT * FROM item_parameter_details WHERE po_no ='".$po_no."' AND item_id = '".$data->item_id."'";
                            ...
                            <div class="col-sm-2 p-1" style="zoom: 80%">
                                <div class="content1 rounded p-2" style="background-color: #f9f9f9 !important; 
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                      ...
                                      <a href="?dele=<?=$row->id?>"><button type="button" class="btn2 btn1-bg-cancel"><i class="fa-solid fa-trash"></i></button></a>
                                    </div>
                                    <div> 
                                        <p class="m-0 text-left">Maximum: <strong><?=$row->maximum?></strong></p>
                                        <p class="m-0 text-left">Minimum: <strong><?=$row->minimum?></strong></p>
                                    </div>
                                </div>
                            </div>
                        <?php } ?>
            ...
            <!-- Modal for each item - FIXED VERSION -->
            <div class="modal fade" id="contactmodal_<?=$data->id?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Parameter Information</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <i class="fa-solid fa-xmark"></i>
                            </button>
                        </div>

                        <form id="contactformidnew_<?=$data->id?>" method="post">
                            <div class="modal-body">
                                <input type="hidden" name="lead_id" id="lead_id_<?=$data->id?>" value="<?=$id?>" />
                                <input type="hidden" name="activity_type" value="Call" />
                                <input type="hidden" name="main" value="1" />

                                <div class="form-group row m-0 pt-1">
                                    <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-start align-items-center pr-1 bg-form-titel-text">Item Name</label>
                                    <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">
                                        <input class="form-control req" name="item_id" id="item_id_<?=$data->id?>" value="<?=find_a_field('item_info','item_id','item_id="'.$data->item_id.'"');?>" readonly="readonly"> 
                                    </div>
                                </div>

                                <div class="form-group row m-0 pt-1">
                                    <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-start align-items-center pr-1 bg-form-titel-text">Parameter Name:</label>
                                    <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0"> 
                                        <select id="parameter_id_<?=$data->id?>" name="parameter_id" required style="width:220px;" onchange="fillMaxMin_<?=$data->id?>(this)">
                                            <option value="">Select Parameter</option>
                                            <?php
                                            $params = mysqli_query($conn, "SELECT i.parameter_id, i.maximum, i.minimum, p.parameter_name FROM item_parameter_setup i JOIN parameter_info p ON i.parameter_id=p.id
                                                 WHERE i.item_id='".$data->item_id."' AND i.status='Active'" );
                                            while($row = mysqli_fetch_assoc($params)){
                                                echo '<option value="'.$row['parameter_id'].'" 
                                                             data-max="'.$row['maximum'].'" 
                                                             data-min="'.$row['minimum'].'">'.
                                                             $row['parameter_name'].'</option>';
                                            }
                                            ?>
                                        </select>
                                    </div>
                                ...
                                <div class="form-group row m-0 pt-1">
                                    <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-start align-items-center pr-1 bg-form-titel-text">Maximum:</label>
                                    <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">
                                        <input class="form-control req" name="maximum" id="maximum_<?=$data->id?>" >
                                    </div>
                                </div>

                                <div class="form-group row m-0 pt-1">
                                    <label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-start align-items-center pr-1 bg-form-titel-text">Minimum:</label>
                                    <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">
                                        <input type="text" name="minimum" id="minimum_<?=$data->id?>" value="" class="form-control req" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="modal-footer">
                                ...
                                <input name="item_id" type="hidden" class="input3" value="<?=$data->item_id?>" />
                                <input name="po_details_id" type="hidden" class="input3" value="<?=$data->id?>" />
                                <input name="contactSubmit" type="submit" id="contactsave_<?=$data->id?>" value="SAVE" class="btn1 btn1-bg-submit">
                                <input name="updatecontact" type="submit" id="contactedit_<?=$data->id?>" value="Update" class="btn1 btn1-bg-update d-none">
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <!-- JavaScript function for this specific modal -->
            <script>
            function fillMaxMin_<?=$data->id?>(selectElement) {
                var selectedOption = selectElement.options[selectElement.selectedIndex];
                var maxValue = selectedOption.getAttribute("data-max");
                var minValue = selectedOption.getAttribute("data-min");
                
                document.getElementById("maximum_<?=$data->id?>").value = maxValue || '';
                document.getElementById("minimum_<?=$data->id?>").value = minValue || '';
            }
            </script>
