...
if(empty($_REQUEST['cs_no'])){
    ...
    <tr><th>CS No</th><th>CS Name</th><th>Add Row</th><th>Add Vendor</th><th>Edit Data</th><th>PO</th></tr>";
    ...
    foreach($cs_all as $fileN)
    {
        ...
        if(!empty($e[1])){
			...
			if(is_file("cs-extra-$e[0].txt"))
			$extradata = explode('||',file_get_contents("cs-extra-$e[0].txt"));
		
			if(!empty($extradata[5])) $pono = $extradata[5];
				
			$cs_title = explode('||',file_get_contents("cs-name-$e[0].txt"));
			...
			if(!empty($cs_title[3])) $name = "$cs_name ($cs_qty $cs_unit)";
			
            echo "<tr>
			<td><a href='?cs_no=$e[0]&dataupdate=1&preview=1'>$e[0]</a></td>
			<td><a style='text-decoration:none' href='?cs_no=$e[0]&dataupdate=1&preview=1'>$name</a></td>
			<td align=center><a href='?cs_no=$e[0]&rowadd=1'>Add</a></td>
			<td align=center><a href='?cs_no=$e[0]&vendoradd=1'>Add</a></td>
			<td align=center><a href='?cs_no=$e[0]&dataupdate=1'>Edit</a></td>
			<td align=center><a target='_blank' href='/app/views/purchase_mod/po/po_print_view.php?po_no=$pono'>$pono</a></td>
			</tr>";
        }
    }
    ...
    if(!empty($_POST['savecs'])){
        ...
        file_put_contents("cs-extra-$_REQUEST[cs_no].txt","$_POST[preparedby]||$_POST[checkedby]||$_POST[approvedby]||$_POST[remark]||$_POST[note]||$_POST[po]||");
        ...
        $data_history = "||$cs_company||$cs_xname||$cs_qty||$cs_unit||$data_Vwrite||$data_Twrite||$data_cs_write||$_POST[remark]||$_POST[note]||$_POST[preparedby]||$_POST[checkedby]||$_POST[approvedby]||$_POST[po]||";
    ...
    if(!empty($_REQUEST['vendoradd'])){
		...
		?>
			...
			<select name='vendor' id='vendor'>
				 <option></option>
				<?php foreign_relation('vendor','vendor_name','vendor_name',$vendor,'1');?>
			</select>

		<?php
		
		// echo "<input name=vendor required id=vendor>";
    ...
    if(!empty($_REQUEST['dataupdate'])){
		...
		if(!empty($_REQUEST['preview'])) $podata = "";
		else $podata = "PO Number: <input name='po' value='$pono'>";
        ...
        <table width=100%><tr><td>CS Number: $_REQUEST[cs_no]</td><td>$podata</td><td align=right>Date: ".date('d/m/Y')."</td></tr></table>
