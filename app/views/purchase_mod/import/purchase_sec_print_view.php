...
elseif($jv_all->tr_from=='Import'){$voucher_name='Import JOURNAL';$vtypes='Import';}
...
//$po_no = find_a_field('secondary_journal','tr_no','tr_from = "Import" and jv_no='.$jv_no);
//$po = find_all_field('Import_master','po_no','po_no='.$po_no);

	//$ssql='update Import_invoice set bill_no="'.$bill_no.'", bill_date="'.$bill_date.'" where po_no="'.$prold->po_no.'"';
	//mysql_query($ssql);
...
$jvold = find_a_field('secondary_journal','tr_id','tr_from = "Import" and jv_no='.$jv_no);
$prold = find_all_field('Import_invoice','po_no','id='.$jvold);
	...
	$cc_code.'", checked_at="'.$time_now.'", checked_by="'.$_SESSION['user']['id'].'", checked="YES" , 	final_jv_no="'.$jv.'" where tr_from = "Import" and jv_no="'.$jv_no.'"';
	mysql_query($ssql);
	$sssql = 'delete from journal where group_for="'.$_SESSION['user']['group'].'" and tr_from ="Import" and tr_no="'.$prold->po_no.'"';
	mysql_query($sssql);
	$jv=next_journal_voucher_id();
	sec_journal_journal($jv_no,$jv,'Import');
}
...
{
	var URL = 'unchecked_voucher_view_popup_Import.php?'+theUrl;
	popUp(URL);
}
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
	    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="1%">
			<img src="../../../logo/<?=$_SESSION['proj_id']?>.png" style="width:100px;" />		</td>
...
echo $_SESSION['proj_name'];
              ...
              <tr>
                <td align="center"><?=find_a_field('user_group','address',"id=".$jv->group_for);?></td>
              </tr>
              <tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#00CC00">
  <tr>
    <td width="42"><input name="button" type="button" onclick="hide();window.print();" value="Print" /></td>
    <td width="99" align="right"><a href="../../../warehouse_mod/pages/pr_packing_mat/chalan_view2.php?v_no=<?php echo $jv->tr_no; ?>">View Import Order</a></td>
...
<input name="voucher_date" type="text" id="voucher_date" value="<?=$jv->jv_date;?>" />
<input type="button" name="Submit" value="EDIT VOUCHER"  onclick="DoNav('<?php echo '&v_no='.$jv_no.'&view=Show' ?>');" /></td>
    <td width="127"><input name="check" type="submit" id="check" value="RE-CHECK" />
        <input type="hidden" name="req_no" id="req_no" value="<?=$jv->jv_on?>" /></td>
  </tr>
</table>
