...
if(isset($_POST['master_data'])){
	...
	$quotation_deadline = $_POST['quotation_deadline'];
		...
		$YR = date('Y',strtotime($invoice_date));
 		...
 		$inv_cy_id = find_a_field('purchase_rfq_master','max(inv_id)','year="'.$YR.'"')+1;
  		$cy_id = sprintf("%06d", $inv_cy_id);
   		$invoice_no=''.$year.''.$month.''.$cy_id;
		$view_invoice_no='RFQ-'.$year.''.$month.''.$cy_id;
	 ...
	 if($req_no>0) {
	  $ins_sql = 'INSERT INTO purchase_rfq_master (invoice_no, year, inv_id, view_invoice_no, invoice_date, group_for, warehouse_id, req_no, view_req_no, currency_type, vendor_id, quotation_deadline, status, entry_at, entry_by) VALUES

("'.$invoice_no.'", "'.$YR.'", "'.$cy_id.'", "'.$view_invoice_no.'", "'.$invoice_date.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$req_no.'", "'.$view_req_no.'", "'.$currency_type.'", "'.$vendor_id.'", "'.$quotation_deadline.'", "'.$status.'", "'.$entry_at.'", "'.$entry_by.'")';
...
if(isset($_POST['confirm'])){
	...
	$view_invoice_no = $_POST['view_invoice_no'];
	 ...
	 $sql = 'select a.*, s.sub_group_name, i.finish_goods_code, i.item_name, i.unit_name from purchase_requisition_details a, item_info i, item_sub_group s where i.item_id=a.item_id  and i.sub_group_id=s.sub_group_id and a.invoice_no="'.$req_no.'" order by a.id';
  ...
  $foe_invoice = 'INSERT INTO purchase_rfq_details (invoice_no, view_invoice_no, invoice_date, group_for, warehouse_id, req_no, currency_type, vendor_id, order_no, item_id, unit_name, req_qty, total_unit, status, entry_at, entry_by)
  
  VALUES("'.$invoice_no.'", "'.$view_invoice_no.'", "'.$invoice_date.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$req_no.'", "'.$currency_type.'", "'.$vendor_id.'", "'.$order_no.'", "'.$item_id.'", "'.$data->unit_name.'", "'.$req_qty.'", "'.$rfq_qty.'", "'.$status.'", "'.$entry_at.'", "'.$entry_by.'")';
...
db_query($foe_invoice);
	...
	$con_sql1="UPDATE purchase_rfq_master SET status='".$status."', checked_by='".$entry_by."', checked_at='".$entry_at."' WHERE invoice_no = '".$invoice_no."' ";
	 db_query($con_sql1);
...
if(isset($_POST['delete'])){
	...
	$sql_del_ms = 'delete from purchase_rfq_master where  invoice_no="'.$invoice_no.'"';
	db_query($sql_del_ms);
	
	$sql_del_foe = 'delete from purchase_rfq_details where invoice_no="'.$invoice_no.'"';
...
<div class="box" style="width:100%;">
    ...
    <td  style="width:8%; text-align:right" ><strong> REQ. No: </strong></td>



    <td style="width:13%;">
	<? $req_data = find_all_field('purchase_requisition_master','','invoice_no="'.$req_no.'"'); ?>
	
	<input name="req_no" type="hidden" id="req_no" style="width:90%; height:32px;" value="<?=$req_data->invoice_no;?>" readonly="" required tabindex="1" />
	...
	<a href="../REQUISITION/invoice_print_view.php?invoice_no=<?=$req_no;?>" target="_blank"><input name="view_req_no" type="text" id="view_req_no" style="width:90%; height:32px;" value="<?=$req_data->view_invoice_no;?>" readonly="" required tabindex="1" /></a>	</td>
	
	 <td   style="width:12%; text-align:right;"><strong>  Company: </strong></td>
	 <td  style="width:22%;" >
	 
	 <select name="group_for" id="group_for"  style="width:90%; height:32px;" required>

		<? foreign_relation('user_group','id','group_name',$_POST['group_for'],'id="'.$req_data->group_for.'"');?>

      </select></td>
	 <td style="width:12%; text-align:right;" ><strong>Currency: </strong></td>
	 <td style="width:12%;" >
     ...
     <input type="submit" name="master_data" id="master_data" value="Initiate Entry" style="width:160px; text-transform:uppercase; background:#87CEFA; color:#000000; text-align:center; font-weight:bold; font-size:12px; height:30px; "/>
								  ...
								  <tr>
								    <td style=" text-align:right;" ><strong> RFQ Date: </strong></td>
								    <td ><input name="invoice_date" type="text" id="invoice_date" style="width:90%; height:32px;" value="<?=$invoice_date;?>"  required tabindex="1" /></td>
								    ...
								    <td  style="text-align:right;" ><strong>Quotation Deadline:</strong>  </td>
								    <td ><input name="quotation_deadline" type="text" id="quotation_deadline" style="width:90%; height:32px;" value="<?=$quotation_deadline;?> " required tabindex="1" /></td>
							      </tr>
								</table>
...
<div class="box" style="width:100%;">
    ...
    <td style="width:11%; text-align:right;"><strong> RFQ No: </strong></td>



    <td  style="width:20%;">
	<? $ms_data = find_all_field('purchase_rfq_master','','invoice_no="'.$_REQUEST['request_no'].'"'); ?>
	...
	<input name="view_invoice_no" type="text" id="view_invoice_no" style="width:90%; height:32px;" value="<?=$ms_data->view_invoice_no;?>" readonly="" required tabindex="1" /></td>
	 ...
	 <td  style="width:11%; text-align:right;"><strong>  RFQ Date: </strong></td>
	 <td style="width:18%; text-align:right;"><input name="invoice_date" type="text" id="invoice_date" style="width:90%; height:32px;" value="<?=$ms_data->invoice_date?>" required readonly="" tabindex="1" /></td>
	 <td style="width:19%; text-align:right;"><strong>REQ. No: </strong></td>
	 <td width="21%" >
	...
	<input name="view_req_no" type="text" id="view_req_no" style="width:90%; height:32px;" value="<?=$ms_data->view_req_no;?>" readonly="" required tabindex="1" /></a>	 </td>
								  ...
								  <tr>
								    <td style="width:11%; text-align:right;"><strong> Vendor: </strong></td>
								    <td width="20%" >
									<select name="vendor_id" id="vendor_id"  style="width:90%; height:32px;" required>
								    ...
								    <td style="width:11%; text-align:right;"><strong>Currency:</strong></td>
		...
		<? foreign_relation('currency_type','currency_type','currency_type',$_POST['currency_type'],'currency_type="'.$ms_data->currency_type.'"');?>
								    ...
								    <td style="width:19%; text-align:right;"><strong>Quotation Deadline:</strong></td>
								    <td width="21%"><input name="quotation_deadline" type="text" id="quotation_deadline" style="width:90%; height:32px;" value="<?=$ms_data->quotation_deadline;?>" readonly="" required tabindex="1" /></td>
							      </tr>
								</table>
...
<table id="grp" style="width:100%; border:0; border-collapse:collapse; text-align:center; padding:0; font-size:12px; text-transform:uppercase;">

  <tr>
    <th style="width:17%">Category</th>
    <th style="width:41%">Item Name</th>
    <th style="width:13%">Unit Name </th>
    <th style="width:14%">Requisition Qty </th>
    <th style="width:15%">RFQ Qty </th>
    </tr>
  ...
  if($_POST['fdate']!=''&&$_POST['tdate']!=''){ $date_con .= ' and c.chalan_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';}
	...
	$po_dt_con=" and m.po_date='".$_POST['po_date']."'";}
     ...
     $sqlfg = "select a.*, s.sub_group_name, i.finish_goods_code, i.item_name, i.unit_name from purchase_requisition_details a, item_info i, item_sub_group s where i.item_id=a.item_id  and i.sub_group_id=s.sub_group_id and a.invoice_no='".$ms_data->req_no."' order by a.id";
...
<? //if($po_no[$data->po_no]==0) { } ?>
 ...
 <tr style="background-color: <?= ($i % 2) ? '#F5F5F5' : '#fff'; ?>;">
  ...
  <td style="background-color:#99CCFF;">
    <input name="req_qty_<?= $datafg->id ?>" 
           type="text" 
           id="req_qty_<?= $datafg->id ?>" 
           value="<?= htmlspecialchars($datafg->total_unit) ?>" 
  ...
  <td style="background-color:#99CCFF;">
    ...
    <input name="rfq_qty_<?= $datafg->id ?>" 
           type="text" 
           id="rfq_qty_<?= $datafg->id ?>" 
           value="<?= htmlspecialchars($datafg->total_unit) ?>" 
           onkeyup="TRcalculationpo(<?= $datafg->id ?>)" 
           style="width:100px; height:30px;">
  </td>
</tr>
