...
$title='Vendor Statement(Individual)';
...
<option value="%">All</option>
									...
									foreign_relation('general_sub_ledger','sub_ledger_id','sub_ledger_name',$sub_ledger_id,"1 and tr_from='vendor' and group_for='".$_SESSION['user']['group']."'  order by sub_ledger_id");
...
<div class="form-group row  m-0 mb-1 pl-3 pr-3">
							<label for="group_for" class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Cut off Date:</label>
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">

								<input name="cdate" type="text" id="cdate" size="12" class="form-control" value="<?php if($_REQUEST['cdate']>0){echo $_REQUEST['cdate'];} else{ echo date('Y-m-d');};?>" autocomplete="off"/>
						...
						<div class="form-group row  m-0 mb-1 pl-3 pr-3">
							<label for="group_for" class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Mature Days:</label>
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">

								<input name="mday" type="text" id="mday" size="12" class="form-control" value="<?php  echo $_REQUEST['mday'];?>" autocomplete="off"/>
	  ...
	  $sub_ledger_name_get[$srow->sub_ledger_id]=$srow->sub_ledger_name;
	  					  ...
	  					  $ssql='select * from vendor where 1 group by sub_ledger_id';
	  $squery=db_query($ssql);
	  while($srow=mysqli_fetch_object($squery)){
	  $vendor_name_get[$srow->sub_ledger_id]=$srow->vendor_name;
	    $vendor_mature_days[$srow->sub_ledger_id]=$srow->vendor_credit_days;
	  }
...
while ($data = mysqli_fetch_row($sql)) {
    ...
    $today = $_REQUEST["cdate"]; // Current/reference date (YYYY-MM-DD)
    $invoice_date = $data[0];

    $today_date = new DateTime($today);
    $invoice_date_obj = new DateTime($invoice_date);

    $interval = $today_date->diff($invoice_date_obj);
    $interval_day_get = $interval->days;

    // Check if mday is set and is a number
    $mday = isset($_REQUEST["mday"]) && is_numeric($_REQUEST["mday"]) ? $_REQUEST["mday"] : null;

    // Show all rows if mday is not set or is empty
    if ($mday === null || $mday === '') {
        $show = true;
    } else {
        // Only show rows where the day difference is greater than or equal to mday
        $show = ($interval_day_get >= $mday);
    }
