...
$assetcount[$info->PBI_ID] = $asset['count'][$info->PBI_ID] + 1;
	...
	$query=db_query($qqq);
	$out.='<table width="100%" cellspacing="0" cellpadding="0" border="1" id="ExportTable">';
	$out.='<thead><tr>
	<th rowspan="2">SL</th>
	<th rowspan="2">ID</th>
...
break;
	...
	$query=db_query($qqq);
	$out.='<table width="100%" cellspacing="0" cellpadding="2" border="1" id="ExportTable">';
	$out.='<thead><tr>
	<th>SL</th>
	<th>ID</th>
	<th>Name</th>
	...
	$vendor_con=' and a.vendor_id="'.$_POST['vendor_id'].'"';
	if($_POST['vendor_cat']>0)
	$vendor_cons=' and a.vendor_category="'.$_POST['vendor_cat'].'"';
	$report="Vendor Information";

         $sql.="select a.vendor_id as vendor_id";
		
		if($_POST['vendor_name']==1) $sql.= ",a.vendor_name as Name";
		if($_POST['vendor_company']==1) $sql.= ",a.vendor_company as vendor_company";
		if($_POST['vendor_category']==1) $sql.= ",c.category_name";
		
		
		if($_POST['contact_no']==1) $sql.= ",a.contact_no as main_phone";
		if($_POST['sms_mobile_no']==1) $sql.= ",a.sms_mobile_no as sms_phone";
		if($_POST['fax_no']==1) $sql.= ",a.fax_no as fax_no";
		
		if($_POST['email']==1) $sql.= ",a.email as main_email";
		if($_POST['cc_email']==1) $sql.= ",a.cc_email as cc_email";
		if($_POST['beneficiary_name']==1) $sql.= ",a.beneficiary_name as beneficiary_name";
		if($_POST['beneficiary_bank']==1) $sql.= ",a.beneficiary_bank as beneficiary_bank";
		if($_POST['beneficiary_bank_ac']==1) $sql.= ",a.beneficiary_bank_ac as beneficiary_bank_account";
		if($_POST['swift_code']==1) $sql.= ",a.swift_code as swift_code";
		if($_POST['address']==1) $sql.= ",a.address as address";
		...
		if($_POST['vendor_credit_days']==1) $sql.= ",a.vendor_credit_days as vendor_credit_days";
		
		
		
		$sql.= " from vendor a left join vendor_category c on c.id=a.vendor_category where	1 and a.group_for='".$_SESSION['user']['group']."' ".$vendor_con.$vendor_cons."";
//echo $sql;
break;
		//ROW_NUMBER() OVER(ORDER BY Id) AS Row
}
...
<?
 $sql;
		...
		$str 	.= '<h1  style="font-weight: bolder; text-align: center;">'.$_SESSION['company_name'].'</h1>';
		if(isset($report))
		$str 	.= '<h2>'.$report.'</h2>';
		if(isset($to_date))
