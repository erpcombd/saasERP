...
$title = "Purchase management Dashboard";
 ...
 function getMonthlySales($month, $year) {
    ...
    return  $new_purchase_order_amount=find_a_field('purchase_invoice pi, purchase_master pm','SUM(pi.amount)','1 and pi.po_no = pm.po_no and pm.po_date BETWEEN  "'.$start_date.'" AND "'.$end_date.'" and pm.status NOT IN ("MANUAL","CANCELED") and pm.entry_by="'.$_SESSION['user']['id'].'"');
...
function getYearlySales($year) {
 ...
 function getMonthlySalesReturn($month, $year) {
	...
	'SUM(pi.total_amt)','1 and pi.pr_no = pm.pr_no and pm.pr_date BETWEEN "'.$start_date.'" AND "'.$end_date.'"   and pm.entry_by="'.$_SESSION['user']['id'].'"');
...
//   AND pm.entry_by = 'your_user_id';
 ...
 $new_purchase_order=find_a_field('purchase_master ','count(*)','1 and YEAR(po_date) ="'.$current_year.'" and status="UNCHECKED" and entry_by="'.$_SESSION['user']['id'].'"');
 $new_purchase_order_amount=find_a_field('purchase_invoice pi, purchase_master pm','SUM(pi.amount)','1 and pi.po_no = pm.po_no and YEAR(pm.po_date) ="'.$current_year.'" and pm.status="UNCHECKED" and pm.entry_by="'.$_SESSION['user']['id'].'"');
 ...
 $new_local_purchase_order_amount=find_a_field('warehouse_other_receive_detail pi, warehouse_other_receive pm','SUM(pi.amount)','1 and pi.or_no = pm.or_no and YEAR(pm.or_date) ="'.$current_year.'" and pm.status="UNCHECKED" and pm.entry_by="'.$_SESSION['user']['id'].'"');
 ...
 $receive_order_amount=find_a_field('purchase_receive pr, purchase_master pm','sum(pr.amount)','1 and pm.po_no=pr.po_no and YEAR(rec_date) ="'.$current_year.'" and pm.entry_by="'.$_SESSION['user']['id'].'" group by pm.po_no');


 $purchase_return=find_a_field('purchase_return_master','count(*)','1 and YEAR(pr_date) ="'.$current_year.'"  and entry_by="'.$_SESSION['user']['id'].'"');
 $purchase_return_amount=find_a_field('purchase_return_details pi, purchase_return_master pm','SUM(pi.total_amt)','1 and pi.pr_no = pm.pr_no and YEAR(pm.pr_date) ="'.$current_year.'"  and pm.entry_by="'.$_SESSION['user']['id'].'"');
 ...
 $approved_po_amount=find_a_field('purchase_invoice pi, purchase_master pm','SUM(pi.amount)','1 and pi.po_no = pm.po_no and YEAR(pm.po_date) ="'.$current_year.'" and pm.status="CHECKED" and pm.entry_by="'.$_SESSION['user']['id'].'"');
...
LEFT JOIN purchase_master pm ON dates.ji_date = DATE(pm.po_date)
    ...
    AND pm.entry_by = "'.$_SESSION['user']['id'].'"
...
$thisYearSales = find_a_field('purchase_invoice pi, purchase_master pm','SUM(pi.amount)','1 and pi.po_no = pm.po_no and pm.po_date BETWEEN  "'.$thisYearSdate.'" AND "'.$thisYearEdate.'" and pm.status NOT IN ("MANUAL","CANCELED") and pm.entry_by="'.$_SESSION['user']['id'].'"');

$thisYearLocalPurchase = find_a_field('warehouse_other_receive_detail pi, warehouse_other_receive pm','SUM(pi.amount)','1 and pi.or_no = pm.or_no and pm.or_date BETWEEN  "'.$thisYearSdate.'" AND "'.$thisYearEdate.'" and pm.status="UNCHECKED" and pm.entry_by="'.$_SESSION['user']['id'].'"');
...
$lastYearSales = find_a_field('purchase_invoice pi, purchase_master pm','SUM(pi.amount)','1 and pi.po_no = pm.po_no and pm.po_date BETWEEN  "'.$lastYearSdate.'" AND "'.$lastYearEdate.'" and pm.status NOT IN ("MANUAL","CANCELED") and pm.entry_by="'.$_SESSION['user']['id'].'"');

$lastYearLocalPurchase = find_a_field('warehouse_other_receive_detail pi, warehouse_other_receive pm','SUM(pi.amount)','1 and pi.or_no = pm.or_no and pm.or_date BETWEEN  "'.$lastYearSdate.'" AND "'.$lastYearEdate.'" and pm.status="UNCHECKED" and pm.entry_by="'.$_SESSION['user']['id'].'"');

$preYearSdate = $previousYear.'-01-01';
$preYearEdate = $previousYear.'-12-31';
$preYearSales = find_a_field('purchase_invoice pi, purchase_master pm','SUM(pi.amount)','1 and pi.po_no = pm.po_no and pm.po_date BETWEEN  "'.$preYearSdate.'" AND "'.$preYearEdate.'" and pm.status NOT IN ("MANUAL","CANCELED") and pm.entry_by="'.$_SESSION['user']['id'].'"');

$preYearLocalPurchase = find_a_field('warehouse_other_receive_detail pi, warehouse_other_receive pm','SUM(pi.amount)','1 and pi.or_no = pm.or_no and pm.or_date BETWEEN  "'.$preYearSdate.'" AND "'.$preYearEdate.'" and pm.status="UNCHECKED" and pm.entry_by="'.$_SESSION['user']['id'].'"');



$preLastYearSdate = $previousLastYear.'-01-01';
$preLastYearEdate = $previousLastYear.'-12-31';
$preLastYearSales = find_a_field('purchase_invoice pi, purchase_master pm','SUM(pi.amount)','1 and pi.po_no = pm.po_no and pm.po_date BETWEEN  "'.$preLastYearSdate.'" AND "'.$preLastYearEdate.'" and pm.status NOT IN ("MANUAL","CANCELED") and pm.entry_by="'.$_SESSION['user']['id'].'"');

$preLastSalesReturn = find_a_field('warehouse_other_receive_detail pi, warehouse_other_receive pm','SUM(pi.amount)','1 and pi.or_no = pm.or_no and pm.or_date BETWEEN  "'.$preLastYearSdate.'" AND "'.$preLastYearEdate.'" and pm.status="UNCHECKED" and pm.entry_by="'.$_SESSION['user']['id'].'"');
...
<div class="container-fluid">
			<div class="row m-0 p-0">
				...
				<div class="col-3 new">
					<div class="card">
					  <div class="card-body">
						<h5 class="card-title bold">Purchase <span> || Year 2024</span></h5>
						...
						<div class="new-icon-text">
						 	<p class="p-sub bold"><?=$new_purchase_order?></p>
							<p class="p-sub1"> <?=$new_purchase_order_amount?> <span>BDT</span></p>
						 </div>
				...
				<div class="col-3 new">
					<div class="card">
					  <div class="card-body">
						<h5 class="card-title bold">Local Purchase<span> || Year 2024</span></h5>
						
						<div class="d-flex ustify-content-between p-3">
						<div class="new-icon success"><i class="fa-solid fa-bangladeshi-taka-sign"></i></div>
						<div class="new-icon-text">
						 	<p class="p-sub bold"><?=$new_local_purchase_order?></p>
							<p class="p-sub1"> <?=$new_local_purchase_order_amount?> <span>BDT</span></p>
						 </div>
				...
				<div class="col-3 new">
					<div class="card">
					  <div class="card-body">
						<h5 class="card-title bold">Purchase Receive <span> || Year 2024</span></h5>
						
						<div class="d-flex ustify-content-between p-3">
						<div class="new-icon danger"><i class="fa-solid fa-bangladeshi-taka-sign"></i></i></div>
						<div class="new-icon-text">
						 	<p class="p-sub bold"><?=$receive_order?></p>
							<p class="p-sub1"><?=$receive_order_amount?> <span>BDT</span></p>
						 </div>
				...
				<div class="col-3 new">
					<div class="card">
					  <div class="card-body">
						<h5 class="card-title bold">Purchase Return<span> || Year 2024</span></h5>
						
						<div class="d-flex ustify-content-between p-3">
						<div class="new-icon info"><i class="fa-solid fa-bangladeshi-taka-sign"></i></div>
						<div class="new-icon-text">
						 	<p class="p-sub bold"><?=$purchase_return?></p>
							<p class="p-sub1"> <?=$purchase_return_amount?> <span>BDT</span></p>
						 </div>
				...
				<div class="col-3 new">
					<div class="card">
					  <div class="card-body">
						<h5 class="card-title bold">Approve PO <span> || Year 2024</span></h5>
						
						<div class="d-flex ustify-content-between p-3">
						<div class="new-icon warning"><i class="fa-solid fa-bangladeshi-taka-sign"></i></div>
						<div class="new-icon-text">
						 	<p class="p-sub bold"><?= $approved_po?></p>
							<p class="p-sub1"><?=$approved_po_amount?> <span>BDT</span></p>
						 </div>
						
						</div>
						
						<a href="#" class="d-flex justify-content-center a">
							<button type="button" class="btn bg-warning button-cs"> <i class="fas fa-check-circle"></i> New Order</button>
						</a>
					  </div>
					</div>
				</div>
