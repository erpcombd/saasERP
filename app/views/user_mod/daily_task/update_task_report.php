...
$target_url = 'add_details_task.php';
$crud = new crud($table);
?>
...
$res='select d.id, m.task_id,d.task_name, m.task_date, p.PROJECT_NAME,b.PBI_NAME,d.status from daily_give_task_details d,daily_give_task_master m,  project p,personnel_basic_info b where m.given_to=b.PBI_ID and m.status !="COMPLETED" and  m.project_id=p.PROJECT_ID and m.given_to='.$_SESSION['user']['id'].' '.$con.' and d.task_id=m.task_id order by m.task_id desc';
