  ...
  <tr  class="alt">
    ...
    <td colspan="3"><span class="oe_form_group_cell">
      <select name="dept" style="width:160px;" id="dept">
		<option value="MTSS">Modern Trade-Super Shop</option>
      </select>
    </span></td>
  </tr>
  <tr >
    ...
    <td colspan="3"><span class="oe_form_group_cell">
	  ...
	  <select name="JOB_LOCATION" id="JOB_LOCATION" style="width:160px;" required="required">
...
$month_find = find_a_field('hrm_portal_setup','mon','1 and level=58');
...
<?		
		...
		//echo $jday=date('d').' <br>';
		$j_date=$start_date;
		$sql = "select p.* from personnel_basic_info p, salary_info s where p.PBI_ID=s.PBI_ID and p.PBI_JOB_STATUS='In Service' and p.PBI_DEPARTMENT like '".$_POST['dept']."' and p.PBI_ORG='".$_POST['PBI_ORG']."' ".$con."  order by (s.basic_salary+s.consolidated_salary) desc,p.PBI_ID";
...
<td align="center"><span id="divi_<?=$info->PBI_ID?>">
            <? 
			  ...
			  if($status=='Edit')
			  {
			  if($_SESSION['user']['level']==5||$_SESSION['user']['level']==58)
