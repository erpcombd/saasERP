...
$holy_query = db_query($holysql);
	...
	{
	$holy_final_sql = "update hrm_att_summary set holyday = 1 where att_date='".$holy_data->holy_day."'";
...
//and h.att_date not in ('2020-02-01')
...
echo $late_min = round(abs($in_time - $from_time) / 60,2);
...
if($grace_no<6) 
{
	$grace = $grace_no;
	if($late_min<11)  
	    {$final_late_min = 0;$final_late_status = 0;}
	else{
