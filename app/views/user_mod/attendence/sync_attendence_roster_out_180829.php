...
echo $sql = "SELECT xenrollid , xdate , min(`xtime`) in_time,max(xtime) out_time FROM `hrm_attdump` h, hrm_machice_type m,personnel_basic_info p
...
for($x=1;$x<=$sl;$x++){
			...
			$found = find_all_field('hrm_att_summary','1','emp_id="'.$value[$x]['emp_id'].'" and att_date="'.$value[$x]['att_date'].'"');
		
			if($found->id>0){
			
				
				if($found->sch_out_time>$found->sch_in_time)
				...
				echo $sql2 = "SELECT `xenrollid` , `xdate` , min(`xtime`) in_time 
					...
					and h.xmechineid=m.mac_id and m.mac_type!='In' and p.PBI_ID='".$value[$x]['emp_id']."' 
					and xdate ='".$value[$x]['roster_att_date']."' ";
					
					$query2 = db_query($sql2);
					$data2 = mysqli_fetch_object($query2);
					
		$sql='update hrm_att_summary set out_time="'.$data2->in_time.'"
					where  emp_id="'.$value[$x]['emp_id'].'" and att_date="'.$value[$x]['roster_att_date'].'" ';
				}
