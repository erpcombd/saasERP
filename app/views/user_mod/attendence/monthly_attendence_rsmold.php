...
<tr class="oe_list_header_columns">
  ...
  <tr >
    ...
    <td colspan="3"><span class="oe_form_group_cell">
      <select required name="PBI_GROUP" id="PBI_GROUP" style="">
        <? 
...
<?		
		...
		//echo $jday=date('d').' <br>';
		$j_date=$start_date;
		$sql = "select p.* from personnel_basic_info p, salary_info s where p.PBI_ID=s.PBI_ID and p.PBI_JOB_STATUS='In Service' and p.PBI_DEPARTMENT like '".$_POST['dept']."' and p.PBI_ORG='".$_POST['PBI_ORG']."' ".$con."  order by (s.basic_salary+s.consolidated_salary) desc";
...
value="<? if($att->td>0) echo $att->td; else {if($new_emp_days>0) echo $new_emp_days; else echo $days_mon;}?>" size="2" maxlength="2" /></td>
...
<td align="center"><input name="ab_<?=$info->PBI_ID?>" type="text" id="ab_<?=$info->PBI_ID?>" style="font-size:10px; width:20px; min-width:20px;" value="<?=($data->ab=='')?$att->ab:$data->ab;?>" size="2" maxlength="2" onchange="cal_all(<?=$info->PBI_ID?>)"/>
...
<td align="center"><input <? if($start_date>$info->PBI_DUE_DOJ) echo 'readonly';?> name="lv_<?=$info->PBI_ID?>" type="text" id="lv_<?=$info->PBI_ID?>" style="font-size:10px; width:35px; min-width:28px;" value="<?=($att->lv=='')?$leave_days_lv:$att->lv;?>" size="4" maxlength="4"  onchange="cal_all(<?=$info->PBI_ID?>)"/></td>
<td align="center"><input name="pre_<?=$info->PBI_ID?>" type="text" id="pre_<?=$info->PBI_ID?>" style="font-size:10px; width:45px; min-width:40px;" onchange="cal_all(<?=$info->PBI_ID?>)" value="<?=($data->pre=='')?$att->pre:$data->pre;?>" size="2" maxlength="2" /></td>
<td align="center"><input name="pay_<?=$info->PBI_ID?>" type="text" id="pay_<?=$info->PBI_ID?>" style="font-size:10px; width:45px; min-width:40px;" value="<?=($data->pay=='')?$att->pay:$data->pay;?>" size="2" maxlength="2" /></td>
...
<td align="center"><input name="remarks_<?=$info->PBI_ID?>" type="text" id="remarks_<?=$info->PBI_ID?>" style="font-size:10px; width:45px; min-width:20px;" 
 value="<?=$att->remarks;?>" size="6" maxlength="6" /></td>
<td align="center"><span id="divi_<?=$info->PBI_ID?>">
            <? 
			if($att->status>0){ echo 'Held-UP';
			}
			else
			{
			  if($status=='Edit')
        ...
        <tfoot>
        <tr><td colspan="2"></td><td></td><td></td><td></td><td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
