...
elseif( ($_POST['type']==2) && $PBI->EMPLOYMENT_TYPE !='Permanent'){
...
/*$to = "bimoldas2@gmail.com";

$subject = "Leave Granted";

$txt = "Granted!";

$headers = "From: bimol@erp.com.bd" . "\r\n" .

"CC: info@newjfeed.com";
...
$ss = "select p.PBI_ID,p.PBI_NAME, desg.DESG_DESC,(select PBI_NAME from personnel_basic_info where PBI_ID=l.leave_responsibility_name) as duties_carried_by, dept.DEPT_DESC, (select PROJECT_DESC from project where PROJECT_ID=p.JOB_LOCATION) as project, l.s_date,l.e_date, l.total_days, t.leave_type_name 

from personnel_basic_info p, designation desg, department dept, hrm_leave_info l,hrm_leave_type t 

where 1 and l.s_date='".$_REQUEST['s_date']."' and l.e_date='".$_REQUEST['e_date']."'  and l.PBI_ID=p.PBI_ID and p.DESG_ID=desg.DESG_ID and p.DEPT_ID=dept.DEPT_ID and l.type=t.id and 

l.PBI_ID='".$leave_id->PBI_ID."' order by l.PBI_ID desc";
...
$headers .= "From: QUF Human Resources<kawsar@erp.com.bd>" . "\r\n" .

"CC: kawsar@erp.com.bd";
...
<? //=$reporting_auth;include('../../common/title_bar.php');?>
...
<div class="oe_form_sheet oe_form_sheet_width">
    ...
    <span style=" text-align:center; font-size:12px;" ><?php echo $msggg; ?></span>
...
<td colspan="1" class="oe_form_group_cell oe_form_group_cell_label">&nbsp;&nbsp;Duration : </td>
...
<input name="s_date" type="text" id="s_date"  value="<?php if($s_date=='') echo ''; else echo $s_date ; ?>" autocomplete="off"  required /></td>
...
<input name="e_date" type="text" id="e_date"  value="<?php if($e_date=='') echo ''; else echo $e_date ; ?>" autocomplete="off"  onchange="getData2('leave_ajax.php', 'leave',document.getElementById('s_date').value,document.getElementById('e_date').value)" required/>
...
<span id="leave"><input type="text" value="<?=find_a_field('hrm_leave_info','total_days','id='.$$unique);?>" name="total_days" id="total_days" readonly="" style=" border:0px solid $ccc;"/></span>
...
<textarea name="reason" style="width:100%;" required><?=$reason?></textarea>
...
<td colspan="4" bgcolor="#E8E8E8" class="oe_form_group_cell"><input type="file" name="att_file" id="att_file" /></td>
...
<td colspan="2"  class="oe_form_group_cell"><input type="text" name="leave_apply_date" id="leave_apply_date" value="<?=$leave_apply_date?>"  required /></td>

<td colspan="1" class="oe_form_group_cell oe_form_group_cell_label">&nbsp;&nbsp;Joining Date After Leave: </td>

<td class="oe_form_group_cell" colspan="2"><script language="javascript">
...
<input style="width:220px" name="leave_join_date" type="text" id="leave_join_date" value="<?php if($leave_join_date) echo $leave_join_date; ?>" onclick="altmsg()"  required /></td>
...
<td colspan="2" class="oe_form_group_cell"><input type="file" name="att_file" id="att_file" /></td>
...
<button name="insert" id="insert" onclick="notempty()" accesskey="S" class="btn btn-danger" type="submit">Apply</button>
...
<td width="109" height="25" align="center" valign="middle"  bgcolor="#FFFFFF"><div align="center" ><?=24-$leave_days_half?></div></td>

<td width="125" align="center" valign="middle"  bgcolor="#FFFFFF"><div align="center" ><?=5-$leave_days_marrige?></div></td>
...
<td width="127" align="center" valign="middle"  bgcolor="#FFFFFF"><div align="center"><?=180-$leave_days_maternity?></div></td>

<? }else{ ?>



      <td width="127" align="center" valign="middle"  bgcolor="#FFFFFF"><div align="center"><?=3-$leave_days_paternity?></div></td>

	  <? } ?>



      <td width="125" align="center" valign="middle"  bgcolor="#FFFFFF"><div align="center"><?=10-$leave_days_Hajj?></div></td>

      <td width="103" height="25" align="center" valign="middle"  bgcolor="#FFFFFF"><div align="center"><?=$leave_days_lwp?></div></td>

      <td width="130" align="center" valign="middle"  bgcolor="#FFFFFF"><div align="center"><?=$leave_days_EOL?></div></td>
