...
$headers = "From: kawsar@erp.com.bd" . "\r\n" .
...
$_POST['e_date'] = $_POST['e_date'];

$_POST['leave_join_date'] = $_POST['leave_join_date'];

$_POST['total_days'] = $_POST['total_days'];
...
//Leave Automation FOR HRM ATT SUMMARY START COMMENT BY KAWSAR MAHMUD
...
$data=mysqli_fetch_object($query);
	...
	$up_query="update hrm_att_summary set leave_id='".$full_leave->id."', leave_type='".$full_leave->type."', leave_reason='".$full_leave->reason."',leave_duration='".$leave_duration."', 
	leave_approved_by='".$_PBI_ID."', leave_entry_at='".$full_leave->entry_at."', leave_entry_by='".$full_leave->PBI_ID."' where id=".$data->id;

	db_query($up_query);

	}else{

  $ins_query="INSERT INTO hrm_att_summary( att_date, emp_id, leave_id, leave_type, leave_reason, leave_duration,leave_approved_by, leave_entry_at, leave_entry_by) VALUES ('".$att_date."','".$full_leave->PBI_ID."', '".$full_leave->id."', '".$full_leave->type."', '".$full_leave->reason."','".$leave_duration."', '".$_PBI_ID."', '".$full_leave->entry_at."', '".$full_leave->PBI_ID."')";
...
//Leave Automation FOR HRM ATT SUMMARY END COMMENT BY KAWSAR MAHMUD
...
$_POST['leave_status'] = 'Not Granted';
              ...
              <div class="">
                ...
                <div class="oe_form_sheetbg">
                  ...
                  <div class="oe_form_sheet oe_form_sheet_width"> <span style=" text-align:center; font-size:12px;" ><?php echo $msggg; ?></span>
                    ...
                    <table  border="1" class="table table-bordered table-sm" cellpadding="0" cellspacing="0">
                  ...
                  <td colspan="3" class="oe_form_group_cell"><input type="file" name="att_file" /></td>
				                ...
				                <? if($leave_id->dayoff_working_date>0){?>
				  
				               <tr class="oe_form_group_row">

                                  <td colspan="1" bgcolor="#E8E8E8" class="oe_form_group_cell oe_form_group_cell_label">&nbsp;&nbsp;Work Date: </td>

                    <td colspan="4" bgcolor="#E8E8E8" class="oe_form_group_cell"><input type="date" name="dayoff_working_date" id="dayoff_working_date" value="<?=$leave_id->dayoff_working_date?>"  /></td>
           ...
           $( "#leave_join_date" ).datepicker({
            ...
            error: function () {}
                                ...
                                <tr class="oe_form_group_row">
								  ...
								  foreign_relation('personnel_basic_info p, designation d','p.PBI_ID','concat(p.PBI_NAME," :: ",d.DESG_DESC)',$leave_responsibility_name,'p.DESG_ID=d.DESG_ID and p.PBI_JOB_STATUS="In Service" and p.JOB_LOCATION="'.$projec_t.'" and p.PBI_ID not in (8,9,25)  and p.PBI_ID != '.$emp_id.' order by p.PBI_NAME');
									 ...
									 foreign_relation('personnel_basic_info p, designation d','p.PBI_ID','concat(p.PBI_NAME," :: ",d.DESG_DESC)',$leave_responsibility_name,'p.DESG_ID=d.DESG_ID and p.PBI_JOB_STATUS="In Service"  and p.DEPT_ID="'.$dept.'" and p.PBI_ID not in (8,9,25) and p.PBI_ID != '.$emp_id.' order by p.PBI_NAME');
								  ...
								  ?>
                                    ...
                                    <input type="hidden" name="reporting_auth" value="<?=find_a_field('personnel_basic_info','incharge_id','PBI_ID='.$leave_id->PBI_ID)?>" />                                
                        ...
                        <tr>
                          ...
                          <td><div align="center">
							  ...
							  <? //if(isset($_GET[$unique]) && $PBI_ID==10001 || $PBI_ID==1002){?>
...
$dayoff=find_a_field('hrm_leave_info','sum(total_days)','type=10 and leave_status="Granted" and s_date>="'.$g_s_date.'" and e_date<="'.$g_e_date.'"   and PBI_ID='.$emp_id);
