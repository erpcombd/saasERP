...
$sql1="select b.* from sale_do_details b where b.do_no = '".$datas->do_no."'";
$data1=db_query($sql1);
$pi=0;
$total=0;
while($infos=mysqli_fetch_object($data1)){ 
	...
	$sps = find_a_field('item_info','sub_pack_size','item_id='.$infos->item_id);
	$order_total_qty[] = $infos->total_unit;
	$sub_pkt_size[] = (($sps>1)?$sps:1);
	
	//$pkt_unit[] = $info->pkt_unit;
	//$dist_unit[] = $info->dist_unit;
$pkt_unit[] = (int)($info->total_unit/$infos->pkt_size);
...
<? for($i=0;$i<$pi;$i++){$fgc = find_a_field('item_info','finish_goods_code','item_id='.$item_id[$i]);if($fgc!=2000 and $fgc!=2001 ){?>
      ...
      <tr style="font-size:16px;">
        ...
        <td align="right" valign="top"><? echo $pkt_unit[$i]*1; $t_pkt = $t_pkt + $pkt_unit[$i];?></td>
...
<? }}?>
  ...
  <tr>
    <td colspan="2"><span class="style9">Chalan Print At: <?=date('Y-m-d h:s:i A');?> </span></td>
    </tr>
  <tr>
    <td width="50%">&nbsp;</td>
    <td>&nbsp;</td>
