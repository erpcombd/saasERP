...
<tr><th>Do No</th><th>Do Date</th><th>Dealer Name</th><th>Area</th><th>Depot</th><th>Payment Details</th>
...
$res="select m.do_no,m.do_date,concat(d.dealer_code,'- ',d.dealer_name_e,'(',product_group,')') as dealer_name,(select AREA_NAME from area where AREA_CODE=d.area_code) as area,concat(m.payment_by,':',m.bank,':',m.remarks) as Payment_Details,m.rcv_amt as SEND_AMT,m.received_amt as RCV_AMT from 

sale_do_master m, dealer_info d 

where m.status in ('PROCESSING')  and m.dealer_code=d.dealer_code ".$con." and d.dealer_type='SuperShop' order by m.do_date,d.dealer_name_e";
//echo $res;
$query = db_query($res);
...
<tr onClick="custom(<?=$data->do_no;?>);"<?=(++$z%2)?'':'class="alt"';?> <?=($data->RCV_AMT>0)?'style="background-color:#FFCCFF"':'';?>>
	...
	<td>&nbsp;<? echo $do_amt = find_a_field('sale_do_details','sum(total_amt)','do_no='.$data->do_no)?></td>
