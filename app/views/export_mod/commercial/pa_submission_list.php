...
$target_url = '../commercial/pa_submission.php';
...
$con .= ' and m.id in ('.$_POST['pi_no'].') ';
 		...
 		$sql = "select pi_id, count(pi_no) as pi_no  from commercial_pa_submission where1 group by pi_id ";
		 $query = db_query($sql);
   ...
   $res="select m.id as pi_no, m.id as pi_no, m.pi_no as grne_pi_no, m.view_pi_no, m.dealer_group, m.dealer_code, m.pi_date, m.pi_type, sum(d.total_unit) as pi_qty, sum(d.total_amt) as pi_amt, m.entry_by from pi_master m, pi_details d, commercial_lc_receive l where m.id=d.pi_id and m.id=l.pi_id and m.pi_type=1 ".$con." group by m.id order by m.pi_date, m.id ";
