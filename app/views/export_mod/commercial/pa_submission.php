...
if(isset($_POST['confirm'])){
		...
		$invoice_no = next_transection_no('0',$invoice_date,'commercial_pa_submission','invoice_no');	
 
   $so_invoice = 'INSERT INTO commercial_pa_submission (invoice_no, invoice_date, export_lc_no, export_lc_date, expiry_date, bank_buyers, group_for, pi_id, pi_no, view_pi_no, pi_date, pi_type, dealer_group, dealer_code, remarks, tolerance, tenor_days, status, entry_at, entry_by, do_no, job_no, marketing_team, marketing_person,contact_no)
  
  VALUES("'.$invoice_no.'", "'.$invoice_date.'", "'.$_POST['export_lc_no'].'", "'.$_POST['export_lc_date'].'", "'.$_POST['expiry_date'].'", "'.$_POST['bank_buyers'].'", "'.$_POST['group_for'].'", "'.$_POST['pi_id'].'", "'.$_POST['pi_no'].'", "'.$_POST['view_pi_no'].'", "'.$_POST['pi_date'].'", "'.$_POST['pi_type'].'", "'.$_POST['dealer_group'].'", "'.$_POST['dealer_code'].'", "'.$_POST['remarks'].'", "'.$_POST['tolerance'].'", "'.$_POST['tenor_days'].'", "CHECKED", "'.$entry_at.'", "'.$entry_by.'",
   "'.$_POST['do_no'].'",  "'.$_POST['job_no'].'",  "'.$_POST['marketing_team'].'",  "'.$_POST['marketing_person'].'",  "'.$_POST['contact_no'].'")';
...
<table width="100%" border="1" align="center" style="font-size:12px;">
  ...
  <tr>
	...
	<tr>
		  <td>&nbsp;&nbsp;DATE: </td>
		  <td><input name="invoice_date" type="text" id="invoice_date" required  style="width:220px; height:30px;" autocomplete="off" value="<?=date('Y-m-d')?>" /></td>
		  </tr>
	
	<tr>
		  <td>&nbsp;&nbsp;APPLICANT NAME:
		  <? $pi_data = find_all_field('pi_master','','id='.$pi_no);?>
		  <? $pid_data = find_all_field('pi_details','','pi_id='.$pi_no);?>
		  <? $lc_data = find_all_field('commercial_lc_receive','','pi_id='.$pi_no);?>
		   </td>
		  <td><input name="dealer_group" type="hidden" id="dealer_group"  required style="width:220px; height:30px;" autocomplete="off" 
        ...
        <? foreign_relation('bank_buyers','bank_id','bank_name',$_POST['bank_buyers'],'dealer_code="'.$pi_data->dealer_code.'" and bank_id="'.$lc_data->bank_buyers.'" order by bank_id');?>
    </select></td>
		  </tr>
		  
		  <tr>
		  <td>&nbsp;&nbsp;LC NO: </td>
		  <td><input name="export_lc_no" type="text" id="export_lc_no" required  readonly="" style="width:220px; height:30px;" autocomplete="off"  
		  value="<?=$lc_data->export_lc_no;?>" /></td>
		  </tr>
		  
		  <tr>
		  <td>&nbsp;&nbsp;LC DATE: </td>
		  <td><input name="export_lc_date" type="text" id="export_lc_date"  readonly="" required style="width:220px; height:30px;" autocomplete="off"  
		  value="<?=$lc_data->export_lc_date;?>" /></td>
		  </tr>
		  
		<tr>
			<td width="38%">&nbsp;&nbsp;EXPIRY DATE:</td>
			<td width="62%">
			<input name="expiry_date" type="text" id="expiry_date" readonly=""  required style="width:220px; height:30px;" autocomplete="off"  
		  value="<?=$lc_data->expiry_date;?>" /></td>
		</tr>
		<tr>
		  <td>&nbsp;&nbsp;REMARKS:</td>
		  <td><input name="remarks" type="text" id="remarks"   style="width:220px; height:30px;" autocomplete="off"  value="<?=$remarks;?>" /></td>
		  </tr>
	</table>	</td>
	...
	<table width="100%"  border="1"  align="center">
		<tr>
			...
			<input name="view_pi_no" type="text" id="view_pi_no" readonly="" required style="width:200px; height:30px;" autocomplete="off"  value="<?=$pi_data->view_pi_no;?>" />
			
<input name="job_no" type="hidden" id="job_no" readonly="" required style="width:200px; height:30px;" autocomplete="off"  value="<?=$pid_data->job_no;?>" />
<input name="do_no" type="hidden" id="do_no" readonly="" required style="width:200px; height:30px;" autocomplete="off"  value="<?=$pid_data->do_no;?>" />
<input name="marketing_team" type="hidden" id="marketing_team" readonly="" required style="width:200px; height:30px;" autocomplete="off"  value="<?=$pid_data->marketing_team;?>" />
<input name="marketing_person" type="hidden" id="marketing_person" readonly="" required style="width:200px; height:30px;" autocomplete="off"  value="<?=$pid_data->marketing_person;?>" />
		...
		<tr>
		  <td>&nbsp;&nbsp;ORDER/LC VALUE:</td>
		  <td><input name="order_value" type="text" id="order_value" readonly="" required style="width:200px; height:30px;" autocomplete="off"  
		  value="<?= $order_value = find_a_field('pi_details','sum(total_amt)','pi_id='.$pi_no);?>" /></td>
		  </tr>
		<tr>
		  <td>&nbsp;&nbsp;TOLERANCE:</td>
		  <td><table>
		  	<tr>
				<td>
				<input name="tolerance" type="text" id="tolerance" required style="width:100px; height:30px;" autocomplete="off" readonly=""  value="<?=$lc_data->tolerance;?>" /> </td>
				<td>&nbsp;&nbsp; %(+/-)</td>
			</tr>
		  </table></td>
		  </tr>
		<tr>
		  ...
		  <td>
		  <?
		  $tolerance_value = ($order_value*$lc_data->tolerance)/100;
		  
		  $value_range_up = $order_value+$tolerance_value;
		  $value_range_down = $order_value-$tolerance_value;
		  ...
		  <table>
		  	<tr>
				<td><input name="value_range_up" type="text" id="value_range_up" readonly="" required style="width:100px; height:30px;" autocomplete="off"  value="<?=$value_range_up;?>" /> </td>
				<td>		  <input name="value_range_down" type="text" id="value_range_down" readonly="" required style="width:100px; height:30px;" autocomplete="off"  value="<?=$value_range_down;?>" />	</td>
			</tr>
		  </table>
		    </td>
		  </tr>
		<tr>
		  <td>&nbsp;&nbsp;TENOR:</td>
		  <td><select name="tenor_days" id="tenor_days"  required style="width:200px; height:30px;">
	
	
        <? foreign_relation('tenor_days','tenor_days','tenor_type',$_POST['tenor_days'],'tenor_days="'.$lc_data->tenor_days.'" order by id');?>
    </select></td>
		  </tr>
		  
		  <tr>
		  <td>&nbsp;&nbsp;CONTACT NO:</td>
		  <td><input name="contact_no" type="text" id="contact_no"   style="width:220px; height:30px;" autocomplete="off"  readonly="" value="<?=$lc_data->contact_no;?>" /></td>
		  </tr>
	</table>	</td>
	</tr>
</table>
