...
$title='Commercial Invoice Status';
do_calander('#fdate');
do_calander('#tdate');
...
$target_url = 'commercial_invoice_view.php';
...
<div class="form-container_large">
  <form action="" method="post" name="codz" id="codz">
	...
	<select name="pi_no" id="pi_no" style="width:280px;">
		...
		foreign_relation('delivery_note','pi_id','pi_no',$_POST['pi_no'],'1');
...
<tr>
  <th width="23%">C/I No </th>
  <th width="17%"> C/I Date</th>
  <th width="17%">PI No </th>
...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and a.ci_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
    ...
    $res="select a.ci_no, a.ci_no, a.pi_no,  a.ci_date, a.dealer_code,  d.dealer_name_e, a.entry_by from 
commercial_invoice a, dealer_info d
where  a.dealer_code=d.dealer_code ".$group_for_con.$con.$dealer_con."  group by a.ci_no order by  a.ci_date, a.ci_no ";
$query = db_query($res);
...
<td onClick="custom(<?=$data->ci_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->ci_no;?></td>
<td onClick="custom(<?=$data->ci_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?php echo date('d-m-Y',strtotime($data->ci_date));?></td>
<td onClick="custom(<?=$data->ci_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->pi_no;?></td>
<td onClick="custom(<?=$data->ci_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->dealer_name_e;?></td>
<td onClick="custom(<?=$data->ci_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;
