...
$target_url = 'delivery_note_view.php';
...
<tr>
  <th width="23%">Delivery Note  No </th>
  <th width="17%"> Date</th>
...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and a.dn_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
    ...
    $res="select a.dn_no, a.dn_no, a.pi_no,  a.dn_date, a.dealer_code,  d.dealer_name_e, a.entry_by from 
delivery_note a, dealer_info d
where  a.dealer_code=d.dealer_code ".$group_for_con.$con.$dealer_con."  group by a.dn_no order by  a.dn_date, a.dn_no ";
$query = db_query($res);
...
<td onClick="custom(<?=$data->dn_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->dn_no;?></td>
<td onClick="custom(<?=$data->dn_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?php echo date('d-m-Y',strtotime($data->dn_date));?></td>
<td onClick="custom(<?=$data->dn_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->pi_no;?></td>
<td onClick="custom(<?=$data->dn_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->dealer_name_e;?></td>
<td onClick="custom(<?=$data->dn_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;
