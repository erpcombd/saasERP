...
if(isset($_POST['confirm'])){
		 ...
		 and r.lc_no='.$lc_no.' group by s.id order by s.item_id ';
			...
			$packet=$_POST['pack_'.$datac->id];
			
			db_query('update sale_do_details set pack="'.$packet.'" where id="'.$datac->id.'"');
...
<table width="900" border="0" cellspacing="0" cellpadding="0" align="center">
 ...
 <tbody>
  ...
  <tr>
    ...
    <td colspan="2">
   <form method="post">   
      <table width="100%" class="tabledesign"   border="1" bordercolor="#CCCCCC" cellspacing="0" cellpadding="5"  style="font-size:12px;font-weight: bold;">
       <tr >
          <th   align="center" bgcolor="#CCCCCC">SL No</th>
          <th   align="center" bgcolor="#CCCCCC" width="27%">Description of Goods</th>
		  <th   align="center" bgcolor="#CCCCCC" width="24%">Measurement</th>
		   <th   align="center" bgcolor="#CCCCCC">Bayer Name</th>
		   <th   align="center" bgcolor="#CCCCCC">Style/PO</th>
		   <th   align="center" bgcolor="#CCCCCC">Order Quantity</th>
			<th   align="center" bgcolor="#CCCCCC">Unit</th>
          <th  align="center" bgcolor="#CCCCCC">Delivery Pack</th>
...
//			while($datac = mysqli_fetch_object($queryc)){
        ...
        <tr style="font-size:12px;">
		  ...
		  <td><?=$datac->bayer_name;?></td>
		  <td><?=$datac->style_no;?></td>
		  <td align="right" valign="top"><?=number_format($datac->total_unit,0); $grand_tot_unit1 +=$datac->total_unit; ?></td>	
          <td align="left" valign="top">/<?=$datac->unit_name;?></td>
          <td align="center" ><?php if($datac->pack==''){?><input type="text" name="pack_<?=$datac->id?>"/><? }else{ echo $datac->pack." Pack";}?></td>
        ...
        <tr style="font-size:12px;">
		<td></td>
        <td align="right" colspan="4" valign="middle"><strong> Total Delivery Quantity & Pack =</strong></td>
       
        <td align="right" valign="middle"><strong>
          <?=number_format($grand_tot_unit1,0) ;?>
        </strong></td>
		<td align="left"><!--Dzn/Roll/Cone/G.Yds/Pcs--></td>
		<td><?=$totpack;?> Pack</td>
  ...
  <?
$cf= find_a_field('pi_details p,sale_do_details d','d.pack','p.do_no=d.do_no and p.pi_id="'.$lc_no.'"');
