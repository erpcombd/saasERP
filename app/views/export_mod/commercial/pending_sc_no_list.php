...
$target_url = '../commercial/sc_update.php';
...
{
	window.open('<?=$target_url?>?sc_no='+theUrl);
}
...
<tr>
  <th width="11%">SC S/L</th>
  <th width="15%"> Date </th>
  <th width="33%">Customer Group </th>
...
$con .= ' and m.do_no in ('.$_POST['do_no'].') ';
   ...
   $res="select * from sales_contract_master where status='MANUAL'  ".$con."  order by sc_no";
...
<td onClick="custom(<?=$data->sc_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=find_a_field('dealer_group','dealer_group',
'id='.$data->dealer_group);?></td>
<td onClick="custom(<?=$data->sc_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('dealer_info','dealer_name_e','dealer_code="'.$data->dealer_code.'"');?></td>
<td onClick="custom(<?=$data->sc_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->status;?></td>
</tr>
