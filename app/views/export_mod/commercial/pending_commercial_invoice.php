...
$title='Pending PI For Commercial Invoice Entry';
do_calander('#fdate');
do_calander('#tdate');
...
$target_url = '../commercial/commercial_invoice.php';
...
$con .= ' and m.id in ('.$_POST['pi_no'].') ';
 		...
 		$sql = "select pi_id, count(pi_no) as pi_no  from commercial_invoice where1 group by pi_id ";
		 $query = db_query($sql);
   ...
   $res="select m.id as pi_no, m.id as pi_no, m.pi_no as grne_pi_no, m.view_pi_no, m.dealer_group, m.dealer_code, m.pi_date, m.pi_type, sum(d.total_unit) as pi_qty, sum(d.total_amt) as pi_amt, m.entry_by from pi_master m, pi_details d where m.id=d.pi_id  ".$con." group by m.id order by m.pi_date, m.id ";
