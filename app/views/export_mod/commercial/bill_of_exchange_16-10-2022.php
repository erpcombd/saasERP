...
<!--	<tr style="font-weight:bold;">
	  ...
	  <td valign="right" align="right">Date : <?php echo date('d-m-Y',strtotime($lc_data->export_lc_date));?></td>
	  </tr>
	   
		  	<tr style="font-weight:bold;">
  	  <td valign="top">Export Sales Contract No : <?=$lc_data->exp_sales_contact_no;?></td>
  	  <td valign="middle" align="left"></td>
	  <td valign="right" align="right">Date : <?php 
	  ...
	  <?php } ?>
	  	  <tr style="font-weight:bold;">
  	  <td valign="top">H.S. CODE NO : 6217.10.00</td>
  	  <td valign="middle" align="center"></td>
  	  <td valign="right" align="right">&nbsp;</td>
	  </tr>
	<?php /*?><tr>
	...
	<tr>
	  ...
	  At <?=$lc_data->tenor_days;?> days sight of this First Bill of Exchange (Second of the same tenor and  date  being unpaid) pay to the order of  <?=$seller_bank->bank_name;?>, 
	 ...
	 echo ' Only';
		...
		?>
		 ...
		 </strong>. Value received and charge the same to account of <b><?=$dealer->buyer_name?>. <?=$dealer->address?>.</b> Drawn Under <?=$buyer_bank->bank_name;?>,  <?=$buyer_bank->branch_name;?>.<br />  
...
$a=0;
		 ...
		 WHERE a.lc_no=b.lc_no  and b.pi_id=c.pi_id and a.lc_no="'.$lc_no.'" GROUP by c.pi_id ';
			...
			$a++;
			if ($a>1) echo ', ';
echo $pi_data->pi_no.' DT. '.date('d-m-Y',strtotime($pi_data->pi_date));}?></b>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0"  style="font-size:14px">
	...
	<tr>
	 ...
	 echo ' Only';
		...
		?>
		 </strong>. Value received and charge the same to account of <b>
		 <?=$dealer->buyer_name?>
		 . 
		 <?=$dealer->address?>
		 .</b> Drawn Under <?=$buyer_bank->bank_name;?>,  <?=$buyer_bank->branch_name;?>. 
		 <br />
		 <b>PROFORMA INVOICE NO. <?  
$a=0;
		 $pi_sql = 'SELECT  c.pi_no, c.pi_date FROM lc_master a, lc_receive b, pi_details c 
