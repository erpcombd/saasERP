...

{
		...
		$YR = date('Y',strtotime($job_date));
  		...
  		$job_cy_id = find_a_field('sale_do_master','max(job_id)','year="'.$YR.'"')+1;
   		...
   		$job_no_generate='NPL'.$yer.''.$month.''.$cy_id;
...
<thead>
		...
		$sql = "select do_no, sum(total_amt) as lc_amt, sum(total_unit) as lc_qty  from lc_receive_details where 1 group by do_no";
		 ...
		 $lc_amt[$info->do_no]=$info->lc_amt;
			...
			$dealer_con .= ' and sm.dealer_code = "'.$dealer_code.'" ';
 ...
 sc.style_no, sc.po_no, sc.destination, sc.referance, sc.sku_no, sc.pack_type, sc.color, sc.size, sc.ply, sc.L_cm, sc.W_cm, sc.H_cm, sc.measurement_unit, sc.paper_combination, sc.sqm_rate, sc.unit_name, sc.final_price, sc.unit_price, sc.printing_info, sc.additional_info, sc.additional_charge, sc.delivery_date, sc.delivery_place,  sum(sc.total_unit) as total_unit, sum(sc.total_amt) as total_amt
		...
		if($delay_days>60 && $delay_days<100 ){
		$delay_party = 1;
		?>
...
<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
		...
		<td bgcolor="#AFEEEE"><?=$row->total_unit;  $total_sales_qty +=$row->total_unit;?></td>
		
		<td bgcolor="#AFEEEE">
		    $ <?=number_format($row->total_amt,2);  $total_total_amt +=$row->total_amt;?>		  </td>

		  
		<td bgcolor="#90EE90"><?=$challan_qty[$row->do_no]; $tot_challan_qty +=$challan_qty[$row->do_no];?></td>
		
		
		<td bgcolor="#90EE90">
		    $ <?=number_format($challan_amt[$row->do_no],2); $tot_challan_amt +=$challan_amt[$row->do_no];?>		</td>
		
		  <td bgcolor="#90EE90"><?=$delivery_date[$row->do_no];?></td>
		  <td bgcolor="#FF7F50"><?=$lc_qty[$row->do_no]; $tot_lc_qty +=$lc_qty[$row->do_no];?></td>
		  <td bgcolor="#FF7F50"> $
...
<input name="delete"  type="submit" class="btn1" value="DELETE WO" style="width:150px; font-weight:bold; background:#FF6347; font-size:12px;color: #000000; height:30px" />
...
<? if($delay_party!=1) {?>
<input name="confirm" type="submit" class="btn1" value="CONFIRM THIS WO" style="width:270px; font-weight:bold; font-size:12px; background:#32CD32; height:30px; color: #000000; float:right" />
<? }?>
</td>
