...
//
//
// $dealer_ledger =  find_a_field('dealer_info_foreign','account_code','dealer_code="'.$dealer_code.'"');
...
$dealer = find_all_field('dealer_info_foreign','','dealer_code='.$dealer_code);

auto_complete_from_db('dealer_info_foreign','item_name','concat(item_name,"#>",finish_goods_code)','','vai_cutomer');
...
document.getElementById('customer_name').value);"  autocomplete="off" required>
  <datalist id="dealer_name_e">
     ...
     <? foreign_relation('dealer_info_foreign','CONCAT(dealer_code, "->", dealer_name_e)','dealer_name_e',$dealer_code,'1');?>
  </datalist>
...
<label style="width:220px;">Customer: </label>
		 ...
		 <input name="group_for2" type="text" id="group_for2" required readonly="" style="width:250px;" 
		 value="<?=find_a_field('dealer_info_foreign','dealer_name_e','dealer_code='.$dealer_code);?>" tabindex="105" />
...
<?
	 $YR=date('Y');
 	 $year=date('y');
	 $month=date('m');
	 $job_id = find_a_field('sale_do_master_foreign','max(job_id)','year="'.$YR.'"')+1;
...
<label style="width:220px;">Customer's PO: </label>
...
<label style="width:220px;">PO Date: </label>
