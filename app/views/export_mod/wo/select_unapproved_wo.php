...
$table_details='sale_do_details_foreign';
//$unique_chalan='id';
...
if(isset($_POST['approved'])){
		...
		$max_delivery_date = find_a_field_sql('select max(delivery_date) as max_delivery_date from sale_do_details_foreign where do_no="'.$_POST['do_no'].'" GROUP by do_no');


	$sql = "update sale_do_master_foreign set status='CHECKED', max_delivery_date='".$max_delivery_date."', checked_by='".$_POST['checked_by']."', checked_at='".$_POST['checked_at']."' where do_no=".$_POST['do_no']."";
	db_query($sql);
  ...
  <form action="" method="post" name="codz" id="codz">
    <table width="80%" border="0" align="center">
      ...
      <tr>
        ...
        <? foreign_relation('sale_do_master_foreign','do_no','job_no',$_POST['old_do_no'],'status="UNCHECKED"');?>
    </select>		</td>
...
$con .= ' and m.do_no in ('.$_POST['old_do_no'].') ';
  ...
  $res="select m.do_no as old_do_no, m.do_no, m.job_no, m.dealer_code, m.do_date,   m.buyer_code, m.merchandizer_code, m.status from sale_do_master_foreign m, sale_do_details_foreign d where m.do_no=d.do_no and m.status='UNCHECKED'  ".$con." group by m.do_no order by m.do_date, m.do_no ";
...
<td onClick="custom(<?=$data->old_do_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('dealer_info_foreign','dealer_name_e','dealer_code="'.$data->dealer_code.'"');?></td>
