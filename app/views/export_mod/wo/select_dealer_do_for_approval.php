...
if ($prevent_multi<1) {
		...
		$max_delivery_date = find_a_field_sql('select max(delivery_date) as max_delivery_date from sale_do_details_foreign where do_no="'.$or_no.'" GROUP by do_no');
		

$sql3 = 'update sale_do_master_foreign set status="CHECKED", max_delivery_date="'.$max_delivery_date.'", checked_by="'.$_POST['checked_by'].'", checked_at="'.$_POST['checked_at'].'"
 where do_no = '.$or_no;
db_query($sql3);
