  ...
  <form action="" method="post" name="codz" id="codz">
    <table width="80%" border="0" align="center">
      ...
      <tr>
        ...
        <? foreign_relation('dealer_info_foreign','dealer_code','dealer_name_e',$_POST['dealer_code'],'1 order by dealer_code');?>
    </select>		</td>
	...
	<select name="do_no" id="do_no" style="width:280px;">
		...
		foreign_relation('sale_do_master_foreign','do_no','job_no',$_POST['do_no'],'1');
...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and m.do_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
    ...
    $res="select m.do_no, m.do_no, m.job_no, m.wo_no, m.do_date, m.dealer_code, m.buyer_code, m.merchandizer_code,  d.dealer_name_e, m.entry_by, m.status from 
sale_do_master_foreign m, sale_do_details_foreign c, dealer_info_foreign d, user_group u 
where 
...
<td onClick="custom(<?=$data->do_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->wo_no;?></td>
