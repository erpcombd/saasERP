...

{
		...
		$customer_name = $_POST['customer_name'];
 		...
 		$customer_con=" and p.dealer_code=".$customer[0];
		
		$buyer_info = $_POST['buyer'];
 		$buyer=explode("->",$buyer_info);
 		$buyer[0];
 		if($_POST['buyer']!='')
 		$buyer_con=" and p.buyer_code=".$buyer[0];
		
		
		if($_POST['ply']!='')
 		$ply_con=" and p.ply=".$_POST['ply'];
		
		if($_POST['approval']!='')
 		$approval_con=" and p.approval='".$_POST['approval']."'";



          $sql="select  p.id, p.paper_combination, p.ply,  p.sqm_rate, p.approval, d.dealer_name_e, b.buyer_name, p.entry_by, p.edit_by, p.approved_by, p.approved_at from paper_combination p, dealer_info d, buyer_info b where p.dealer_code=d.dealer_code and p.buyer_code=b.buyer_code ".$customer_con.$buyer_con.$ply_con.$approval_con." order by p.approval desc ";
...
<tr height="30"><th bgcolor="#00A59A">SL</th>

  <th bgcolor="#00A59A">Customer Name </th>
  <th bgcolor="#00A59A">Buyer Name </th>
  <th bgcolor="#00A59A">Paper Combination </th>
  <th bgcolor="#00A59A">Ply</th>
  <th bgcolor="#00A59A">Sqm Rate </th>
  <th bgcolor="#FF0000">Approval</th>
  <th bgcolor="#FF0000">Entry By </th>
  <th bgcolor="#FF0000"><strong>Approved</strong> By </th>
  <th bgcolor="#FF0000"><strong>Approved</strong> At </th>
...
$ajax_page = "paper_combination_approve_ajax.php";
...
<tr><td><?=$s?></td>
 ...
 <?=$datas[3];?>
  <? } else {?>
   <input name="sqm_rate#<?=$datas[0]?>" type="text" id="sqm_rate#<?=$datas[0]?>" value="<?=$datas[3]?>" readonly=""  style="width:80px;" placeholder="Sqm Rate"/>
   
   <? }?>
  </td>
  <td align="center"><strong><?=$datas[4]?></strong>
    <input type="hidden" name="id#<?=$datas[0]?>" id="id#<?=$datas[0]?>" value="<?=$datas[0]?>" /></td>
  <td align="center"><?=find_a_field('user_activity_management','fname','user_id='.$datas[7])?></td>
  <td align="center"><? if ($datas[4]=="Yes") {?><?=find_a_field('user_activity_management','fname','user_id='.$datas[9])?><? }?></td>
  <td align="center"><? if ($datas[4]=="Yes") {?><?=$datas[10]?><? }?></td>
  <td><div id="po<?=$datas[0]?>">
  <? if ($datas[4]=="No") {?>
  <input type="button" name="Change" value="Approve" onclick="getData2('<?=$ajax_page?>', 'po<?=$datas[0]?>',<?=$datas[0]?>,
document.getElementById('sqm_rate#<?=$datas[0]?>').value);
" />
<? }?>
...

{
		...
		$customer_name = $_POST['customer_name'];
 		...
 		$customer[0];
 		if($_POST['customer_name']!='')
 		$customer_con=" and a.dealer_code=".$customer[0];
		
		$buyer_info = $_POST['buyer'];
 		$buyer=explode("->",$buyer_info);
 		$buyer[0];
 		if($_POST['buyer']!='')
 		$buyer_con=" and a.buyer_code=".$buyer[0];
		
		
		if($_POST['approval']!='')
 		$approval_con=" and a.approval='".$_POST['approval']."'";
		
		


          $sql="select  a.id, a.additional_information,   a.additional_charge, a.approval, d.dealer_name_e, b.buyer_name from additional_information a, dealer_info d, buyer_info b where a.dealer_code=d.dealer_code and a.buyer_code=b.buyer_code ".$customer_con.$buyer_con.$ply_con.$approval_con." order by a.approval desc ";
...
<tr height="30"><th bgcolor="#00A59A">SL</th>
  ...
  <th bgcolor="#00A59A">Customer Name </th>
  <th bgcolor="#00A59A">Buyer Name </th>
  <th bgcolor="#00A59A">Additional Information</th>
  <th bgcolor="#00A59A">Additional Charge </th>
  <th bgcolor="#FF0000">Approval</th>
  <th bgcolor="#FF0000">Action</th>
</tr>
</thead>
...
$ajax_page = "additional_charge_approve_ajax.php";
...
<tr><td><?=$s?></td>
 ...
 <?=$datas[2];?>
  <? }else { ?>
  <input name="additional_charge#<?=$datas[0]?>" type="text" id="additional_charge#<?=$datas[0]?>" value="<?=$datas[2]?>" readonly=""  style="width:80px;" placeholder="Sqm Rate"/>
  <? }?>
  
  </td>
  <td align="center"><strong><?=$datas[3]?></strong>
    <input type="hidden" name="id#<?=$datas[0]?>" id="id#<?=$datas[0]?>" value="<?=$datas[0]?>" /></td>
  <td><div id="po<?=$datas[0]?>">
  <? if ($datas[3]=="No") {?>
  <input type="button" name="Change" value="Approve" onclick="getData2('<?=$ajax_page?>', 'po<?=$datas[0]?>',<?=$datas[0]?>,
document.getElementById('additional_charge#<?=$datas[0]?>').value);
" />
<? }?>
...

{
		...
		if($_POST['approval']!='')
 		$approval_con=" and a.approval='".$_POST['approval']."'";
          ...
          $sql="select  a.id, a.number_format,   a.number_format, a.approval, d.dealer_name_e, b.buyer_name from decimal_numbers a, dealer_info d, buyer_info b where a.dealer_code=d.dealer_code and a.buyer_code=b.buyer_code ".$customer_con.$buyer_con.$ply_con.$approval_con." order by a.approval desc ";
...
<tr height="30"><th bgcolor="#00A59A">SL</th>
  ...
  <th bgcolor="#00A59A">Customer Name </th>
  <th bgcolor="#00A59A">Buyer Name </th>
  <th bgcolor="#00A59A">Decimal Numbers</th>
  <th bgcolor="#FF0000">Approval</th>
  <th bgcolor="#FF0000">Action</th>
</tr>
</thead>
...
$ajax_page = "decimal_numbers_approve_ajax.php";
...
<tr><td><?=$s?></td>
 ...
 <?=$datas[2];?>
  <? } else {?>
  <input name="number_format#<?=$datas[0]?>" type="text" id="number_format#<?=$datas[0]?>" value="<?=$datas[2]?>" readonly=""  style="width:80px;" placeholder="Sqm Rate"/>
  <? }?>
  ...
  <? if ($datas[3]=="No") {?>
  <input type="button" name="Change" value="Approve" onclick="getData2('<?=$ajax_page?>', 'po<?=$datas[0]?>',<?=$datas[0]?>,
document.getElementById('number_format#<?=$datas[0]?>').value);
" />
<? }?>
...

{
          ...
          $sql="select g.group_name, s.sub_group_name, i.item_id, i.item_name, i.L_cm, i.W_cm,i.H_cm,i.WL,i.WW, i.pack_size, 
 i.sale_price, i.status  from item_group g, item_sub_group s, item_info i where g.group_id=s.group_id and s.sub_group_id=i.sub_group_id ".$item_sub_group_con.$item_name_con.$product_nature.$group_for_con.$group_id_con;
...
<thead>
<tr><td style="border:0px; text-align:center;" colspan="12"><img src="<?=SERVER_ROOT?>public/uploads/logo/<?= $_SESSION['user']['group']?>.png" width="200" /></td></tr>
<tr><td style="border:0px;" colspan="12"><?=$str?></td></tr>

<tr height="30"><th bgcolor="#00A59A">SL</th>

  <th bgcolor="#00A59A">Product Category </th>
  <th bgcolor="#00A59A">Product Code </th>
<th bgcolor="#00A59A">Product Name </th>

<th bgcolor="#00A59A"> L (cm) </th>
<th bgcolor="#00A59A">W (cm) </th>
<th bgcolor="#00A59A">H (cm) </th>
<th bgcolor="#00A59A">WL</th>
<th bgcolor="#00A59A">WW</th>
<th bgcolor="#00A59A">Sqm Price </th>
<th bgcolor="#FF0000">Status</th>
<th bgcolor="#FF0000">Submit</th>
</tr>
</thead>
...
<tr><td><?=$s?></td>
  ...
  <td><?=$datas[8]?></td>
  <td>
  <input name="pack_size#<?=$datas[2]?>" type="hidden" id="pack_size#<?=$datas[2]?>" value="<?=$datas[9]?>"  style="width:80px;" placeholder="Sale Price"/>
  <input name="sale_price#<?=$datas[2]?>" type="text" id="sale_price#<?=$datas[2]?>" value="<?=$datas[10]?>"  style="width:80px;" placeholder="Sale Price"/></td>
  <td>
...
document.getElementById('sale_price#<?=$datas[2]?>').value+'#'+
