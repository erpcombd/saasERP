...
elseif($_POST['report']==210001)
{
		...
		<th width="9%">WO Date </th>
		<th width="19%">Customer Name </th>
		<th width="12%"><span id="buyer_filter">Buyer</span> Name </th>
		<th width="12%">Merchandiser Name </th>
		<th width="12%"><span id="buyer_filter">Marketing Team </span></th>
		<th width="12%"><span id="buyer_filter">Marketing Person</span></th>
		<th width="9%" bgcolor="#FF0000" style="text-align:center">WO Amount </th>
		<th width="5%" bgcolor="#FF0000" style="text-align:center">Status</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		echo $sql="select  d.dealer_name_e, b.buyer_name, m.merchandizer_name, sm.do_no, sm.do_date, sm.job_no, sm.marketing_team, sm.marketing_person, sm.status, 
		 sum(sd.total_amt) as total_amt
		  from dealer_info d, buyer_info b, merchandizer_info m, sale_do_master sm, sale_do_details sd
		 where sm.do_no=sd.do_no and  d.dealer_code=sm.dealer_code and b.buyer_code=sm.buyer_code and m.merchandizer_code=sm.merchandizer_code ".$con." group by  sm.do_no order by sm.do_no";
		...
		<td>
		  <?=number_format($row->total_amt,2); $total_sales_amt +=$row->total_amt;?>
		 </td>
		<td><?=$row->status?></td>
		</tr>
<?  }?>
