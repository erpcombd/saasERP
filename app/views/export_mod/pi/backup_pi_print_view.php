...
$pi_d = find_all_field('pi_details_backup','','pi_id='.$pi_no); 
	...
	<tr>
	  <td colspan="3" valign="top" style="font-size:13px; padding: 5px 0px 0px 0px; letter-spacing: .3px; line-height:20px" ><div align="justify">This Proforma Invoice (PI) has been issued in relation to the Purchase Order (PO) number 
	    <b>  
	      <?  
$o=0;
		$po_sql = 'SELECT m.customer_po_no, m.customer_po_date, m.job_no FROM pi_details_backup p, sale_do_master m WHERE p.do_no=m.do_no and p.pi_id="'.$pi_no.'" GROUP by p.do_no ';
    ...
    <td colspan="2">
      ...
      <table width="100%" class="tabledesign"   border="1" bordercolor="#CCCCCC" cellspacing="0" cellpadding="5"  style="font-size:12px">
        ...
        <tr >
          ...
          <td width="5%" align="center"  bgcolor="#CCCCCC">Ply</td>
          <td width="4%" align="center"  bgcolor="#CCCCCC">UOM</td>
          <td width="7%" align="center" bgcolor="#CCCCCC">Quantity</td>
          <td width="7%" align="center" bgcolor="#CCCCCC">Quantity <br /> 
            in (KG)</td>
		...
		$sqlc = 'select c.delivery_date, c.delivery_place,c.printing_info,c.additional_info, c.measurement_unit, c.total_unit, c.unit_price, i.unit_name, c.total_amt, i.item_id, i.item_name, c.ply, c.style_no, c.referance, c.po_no,  c.paper_combination, c.L_cm, c.W_cm, c.H_cm, c.weight_per_sqm, c.sqm from pi_details_backup c, item_info i,  item_sub_group s, item_group g where i.item_id=c.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and c.pi_id='.$pi_no.' group by c.id order by c.item_id ';
