 ...
 $app_val = $_REQUEST['valh'];
   ...
   $digital_sign=$year.''.$month.''.$cy_id;
	...
	$flag = $_REQUEST['flag'];
			 ...
			 hold_app_by = '".$checked_by."', hold_app_at = '".$checked_at."' WHERE do_no = '".$do_no."'";
			 db_query($new_sql);
  ...
  VALUES('".$app_date."', '".$do_no."', '".$tr_data->job_no."', '".$tr_data->do_date."', 'WO Hold', '".$YR."', '".$digital_sign_id."', '".$digital_sign."', 
...
db_query($am_ins);
 ...
 $up_h="UPDATE sale_do_master SET h_mis_hold='No' WHERE  do_no = '".$do_no."'";
db_query($up_h);

 $hr_ins = "INSERT INTO mis_hold_reject_history (app_date, tr_no, tr_no_view, tr_date, tr_type, tr_from, checked_by, checked_at)
  
  VALUES('".$app_date."', '".$do_no."', '".$tr_data->job_no."', '".$tr_data->do_date."',  'Unhold',  'WO Hold', 
  '".$checked_by."',  '".$checked_at."')";
db_query($hr_ins);

}  elseif ($app_val==3) {

 $up_h="UPDATE sale_do_master SET h_mis_hold='Yes' WHERE do_no = '".$do_no."'";
db_query($up_h);

 $hr_ins = "INSERT INTO mis_hold_reject_history (app_date, tr_no, tr_no_view, tr_date, tr_type, tr_from, checked_by, checked_at)
  
  VALUES('".$app_date."', '".$do_no."', '".$tr_data->job_no."', '".$tr_data->do_date."',  'Hold',  'WO Hold', 
  '".$checked_by."',  '".$checked_at."')";
...
$up_c ="UPDATE sale_do_master SET h_mis_reject='Yes', status='CHECKED' WHERE do_no = '".$do_no."'";
db_query($up_c);

$hr_ins = "INSERT INTO mis_hold_reject_history (app_date, tr_no, tr_no_view, tr_date, tr_type, tr_from, checked_by, checked_at)
  
  VALUES('".$app_date."', '".$do_no."', '".$tr_data->job_no."', '".$tr_data->do_date."',  'Reject', 'WO Hold', 
  '".$checked_by."',  '".$checked_at."')";
db_query($hr_ins);
