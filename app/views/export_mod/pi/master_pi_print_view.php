...
$sale_master_po = find_a_field('sale_do_master','customer_po_no','do_no='.$pi_d->do_no); 
...
$vl_sql = 'SELECT  sum(s.total_amt) as pi_value FROM pi_master a, pi_details b, sale_do_details s WHERE  a.id=b.pi_id and b.do_no=s.do_no and b.pi_id="'.$pi_no.'" GROUP by b.pi_id ';
		$pi_value = find_all_field_sql($vl_sql);
...
<table width="900" border="0" cellspacing="0" cellpadding="0" align="center">
                      ...
                      <tr>
                        <td width="68%">
                       		<table  width="97%" border="0" cellspacing="0" cellpadding="0"  style="font-size:15px">
								<tr>
					    <td width="50%" align="left" style="padding-bottom:0px;"><img src="<?=SERVER_ROOT?>public/uploads/logo/rahimgroup.png"  width="27%" /></td>
 ...
 <tbody>
  ...
  <tr>
   <td colspan="2" align="center"><h4 style="font-size:18px; padding:10px 0; margin:0; font-family:  'MYRIADPRO-REGULAR'; letter-spacing:1px;text-decoration:underline;">PROFORMA INVOICE</h4></td>
  </tr>
 ...
 <tr> <td colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
		  ...
		  <tr>
			...
			<tr>
                ...
                <td width="53%" align="right" valign="middle"><div align="left">Proforma Invoice No</div></td>


			    <td width="4%">: </td> 
			    <td width="43%"><span><?php echo $master->view_pi_no;?></span></td>
			</tr>
			  ...
			  <tr>
                ...
                <td width="53%" align="right" valign="middle"><div align="left">Proforma Invoice Date</div></td>
			  ...
			  <tr>
			   ...
			   <? if ($master->pi_type==1) {?>
			  <tr>
			    <td align="right" valign="middle"><div align="left">Job No </div></td>
			    <td>: </td>
			    <td><?php echo $pi_d->job_no;?></td>
			  </tr>
			     <? }?>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0"  style="font-size:12px">
	...
	<? if ($master->pi_type!=1) {?>
	<tr>
	  <td colspan="3" valign="top" style="font-size:13px; padding: 5px 0px 0px 0px; letter-spacing: .3px; line-height:20px" ><div align="justify">This Proforma Invoice (PI) has been issued in relation to the Purchase Order (PO) number 
	    <b>  
	      <?  
$o=0;
		  $po_sql = 'SELECT m.po_no, m.po_date, m.job_no FROM pi_details p, sale_do_master m WHERE p.do_no=m.do_no and p.pi_id="'.$pi_no.'" GROUP by p.do_no ';
			$po_query=db_query($po_sql);
			while($po_data= mysqli_fetch_object($po_query)){
			$o++;
			if ($o>1) echo ', ';
echo $po_data->customer_po_no.'. DT.'.$po_data->customer_po_date.'. Job No. '.$po_data->job_no;}?>
	      </b> presented by the buyer/customer.</div></td>
	  </tr>
	  <? }?>
  </table>
  ...
  <tr>
    ...
    <td colspan="2">
      ...
      <table width="100%" class="tabledesign"   border="1" bordercolor="#CCCCCC" cellspacing="0" cellpadding="5"  style="font-size:12px">
        ...
        <tr >
          <td width="5%" align="center" bgcolor="#CCCCCC">SL</td>
          <td width="22%" align="center" bgcolor="#CCCCCC">Item Name </td>
          <td width="19%" align="center"  bgcolor="#CCCCCC">Measurement</td>
          <td width="8%" align="center"  bgcolor="#CCCCCC">Style No </td>
          <td width="8%" align="center"  bgcolor="#CCCCCC">PO No </td>
          <td width="6%" align="center"  bgcolor="#CCCCCC">UOM</td>
          <td width="6%" align="center"  bgcolor="#CCCCCC">Weight</td>
          <td width="8%" align="center" bgcolor="#CCCCCC">Quantity</td>
          <td width="7%" align="center"  bgcolor="#CCCCCC"> Price  </td>
          <td width="11%" align="center" bgcolor="#CCCCCC">Total Value </td>
        </tr>
        
        <?  
		
		$sqlc = 'select s.*, i.item_name, i.unit_name from pi_details p, sale_do_details s, item_info i  where p.do_no=s.do_no and i.item_id=s.item_id and p.pi_id='.$pi_no.' group by s.id order by s.item_id ';
        ...
        <tr style="font-size:12px;">
		  ...
		  <? 
		  if ($datac->measurement!="") {
		  echo $datac->measurement;
		  } else {
		  echo 'N/A';
		  }
		  ?>
  ...
  <tr>
	 ...
	 elseif($credit_amt[1]==1)
	 echo  ' & '.convertNumberToWordsForIndia($credit_amt[1]).' Cent ';
	 ...
	 echo ' Only';
		...
		<tr>
          ...
          <td align="left"  width="90%" style="font-size:14px;  border-left:0; line-height:18px; padding: 5px 0 5px 0; "> We reserve the right to reject any L/C for unusal terms &amp; conditions and if the above mentioned terms &amp;<br />
            ...
            Netrakona Accessories Ltd will not be responsible for any claims regard to the quality of the goods after 7 days of<br />
            the delivery.<br />
	...
	<table width="100%" cellspacing="0" cellpadding="0"  >
		...
		<tr style="font-size:12px" >
		  ...
		  <? if ($master->digital_sign>0) {?>
			...
			<p  style="font-size:11px; float:left;   border-left:0; margin:0;  padding: 5px 0 0 50px; letter-spacing:1px; color:#999999; "><?=$master->digital_sign?></p>
