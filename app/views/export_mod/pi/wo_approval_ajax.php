...
$tr_data = find_all_field('sale_do_master','','do_no="'.$do_no.'"');
 ...
 if ($app_val==1) {
   ...
   $digital_sign=$year.''.$month.''.$cy_id;
	...
	$flag = $_REQUEST['flag'];
			...
			$max_delivery_date = find_a_field_sql('select max(delivery_date) as max_delivery_date from sale_do_details where do_no="'.$do_no.'" GROUP by do_no');
			 ...
			 $new_sql="UPDATE sale_do_master SET max_delivery_date='".$max_delivery_date."', status='".$status."',  digital_sign = '".$digital_sign."', 
			 checked_by = '".$checked_by."', checked_at = '".$checked_at."' WHERE do_no = '".$do_no."'";
			 db_query($new_sql);
  ...
  VALUES('".$app_date."', '".$do_no."', '".$tr_data->job_no."', '".$tr_data->do_date."', 'Work Order', '".$YR."', '".$digital_sign_id."', '".$digital_sign."', 
...
db_query($am_ins);
 ...
 $up_h="UPDATE sale_do_master SET wo_mis_hold='No' WHERE  do_no = '".$do_no."'";
db_query($up_h);

 $hr_ins = "INSERT INTO mis_hold_reject_history (app_date, tr_no, tr_no_view, tr_date, tr_type, tr_from, checked_by, checked_at)
  
  VALUES('".$app_date."', '".$do_no."', '".$tr_data->job_no."', '".$tr_data->do_date."',  'Unhold', 'Work Order',
  '".$checked_by."',  '".$checked_at."')";
db_query($hr_ins);

}  elseif ($app_val==3) {

 $up_h="UPDATE sale_do_master SET wo_mis_hold='Yes' WHERE do_no = '".$do_no."'";
db_query($up_h);

 $hr_ins = "INSERT INTO mis_hold_reject_history (app_date, tr_no, tr_no_view, tr_date, tr_type, tr_from, checked_by, checked_at)
  
  VALUES('".$app_date."', '".$do_no."', '".$tr_data->job_no."', '".$tr_data->do_date."',  'Hold', 'Work Order',
  '".$checked_by."',  '".$checked_at."')";
db_query($hr_ins);
