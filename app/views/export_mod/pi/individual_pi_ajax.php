...
$pi_date = $_REQUEST['pi_date'];
$do_no = $_REQUEST['do_no'];
$dealer_group = $_REQUEST['dealer_group'];
...
$pi_dtls = "INSERT INTO pi_details (pi_id, pi_no, pi_date, pi_type, do_no, do_date, job_no, group_for, dealer_code, buyer_code, merchandizer_code, marketing_team, marketing_person, customer_po_no, status, entry_by, entry_at)
  ...
  VALUES('".$pi_no."', '".$pi_no_generate."', '".$pi_date."', '".$pi_type."', '".$do_no."', '".$ms_data->do_date."', '".$ms_data->job_no."', '".$ms_data->group_for."', '".$dealer_code."', '".$ms_data->buyer_code."', '".$ms_data->merchandizer_code."', '".$ms_data->marketing_team."', '".$ms_data->marketing_person."', '".$ms_data->customer_po_no."', '".$status."', '".$entry_by."', '".$entry_at."')";
...
db_query($pi_dtls);
		...
		// $sql = 'select s.*, g.weight_per_sqm from sale_do_details s, paper_combination c, paper_grade_type g where s.paper_combination_id=c.id and c.paper_grade_type=g.id and s.do_no ="'.$do_no.'" and s.lc_id=0';
//
//		$query = db_query($sql);
//
...
db_query($pi_terms);
			 ...
			 //$new_sql="UPDATE sale_do_details SET lc_id = '".$pi_no."'  WHERE do_no = '".$do_no."'";
			// db_query($new_sql);
