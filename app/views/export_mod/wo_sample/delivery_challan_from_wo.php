...
if(isset($_POST['confirm'])){
		...
		$YR = date('Y',strtotime($ch_date));
  		...
  		$ch_cy_id = find_a_field('sale_sample_chalan','max(ch_id)','year="'.$YR.'"')+1;
		...
		//$chalan_no = next_transection_no($group_for,$ch_date,'sale_sample_chalan','chalan_no');
		//$chalan_no = next_transection_no('0',$ch_date,'sale_sample_chalan','chalan_no');
		//$gate_pass = next_transection_no('0',$ch_date,'sale_sample_chalan','gate_pass');

		
		$ms_data = find_all_field('sale_sample_master','','do_no='.$do_no);

		 $sql = 'select * from sale_sample_details where  do_no = '.$do_no;
  ...
  $so_invoice = "INSERT INTO `sale_sample_chalan` (year, ch_id,  `chalan_no`,  gate_pass,  `chalan_date`, `order_no`, item_name, `bundle_issue`, `do_no`, `do_date`, `job_no`,
	...
	<tr  bgcolor="#9999FF">
	  ...
	  <div>
	    ...
	    <?php /*?><select name="delivery_place" required id="delivery_place"  tabindex="7" style="width:220px;">
          <? foreign_relation('delivery_place_info a, sale_sample_details b','b.delivery_place','a.delivery_place',$delivery_place,'a.id=b.delivery_place and a.dealer_code="'.$dealer_code.'" and b.do_no="'.$do_no.'" group by b.delivery_place');?>
        </select><?php */?>
	  </div>		</td>
        </tr>
    </table></td>
...
<? if($$unique>0){
  ...
  a.delivery_place, a.delivery_date, a.style_no, a.po_no, a.destination, a.referance, a.sku_no, a.pack_type, a.color, a.size, b.pack_size from sale_sample_details a,item_info b, item_sub_group s where b.item_id=a.item_id 
      ...
      <tbody>
		...
		<? 
		  ...
		  ?>			  </td>
              ...
              <td width="4%" align="center"><? echo number_format($so_qty = (find_a_field('sale_sample_chalan','sum(total_unit)','order_no="'.$row->id.'" and item_id="'.$row->item_id.'"')*(1)),2);?></td>
