...
$target_url = '../wo_sample/do_unapproved.php';
...
if(isset($_POST['approved'])){
		...
		$max_delivery_date = find_a_field_sql('select max(delivery_date) as max_delivery_date from sale_sample_details where do_no="'.$_POST['do_no'].'" GROUP by do_no');


	$sql = "update sale_sample_master set status='CHECKED', max_delivery_date='".$max_delivery_date."', checked_by='".$_POST['checked_by']."', checked_at='".$_POST['checked_at']."' where do_no=".$_POST['do_no']."";
	db_query($sql);
  ...
  <form action="" method="post" name="codz" id="codz">
    <table width="80%" border="0" align="center">
      ...
      <tr>
		...
		foreign_relation('sale_sample_master','do_no','job_no',$_POST['old_do_no'],'status="MANUAL"');
...
$con .= ' and m.do_no in ('.$_POST['old_do_no'].') ';
  ...
  $res="select m.do_no as old_do_no, m.do_no, m.job_no, m.dealer_code, m.do_date,   m.buyer_code, m.merchandizer_code, m.status from sale_sample_master m, sale_sample_details d where m.do_no=d.do_no and m.status='UNCHECKED'  ".$con." group by m.do_no order by m.do_date, m.do_no ";
