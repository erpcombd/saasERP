...
$prevent_multi=find_a_field('sale_sample_chalan','chalan_no','do_no='.$or_no);

if ($prevent_multi<1) {

		
		//$master = find_all_field_sql("select * from sale_sample_master m, dealer_info d where d.dealer_code = m.dealer_code and m.do_no = '".$or_no."'");
		
		$max_delivery_date = find_a_field_sql('select max(delivery_date) as max_delivery_date from sale_sample_details where do_no="'.$or_no.'" GROUP by do_no');
		

$sql3 = 'update sale_sample_master set status="CHECKED", max_delivery_date="'.$max_delivery_date.'", checked_by="'.$_POST['checked_by'].'", checked_at="'.$_POST['checked_at'].'"
 where do_no = '.$or_no;
db_query($sql3);
