...
$destination_count= find_a_field('sale_sample_chalan','COUNT(destination)','chalan_no="'.$chalan_no.'" and destination!=""');
$referance_count= find_a_field('sale_sample_chalan','COUNT(referance)','chalan_no="'.$chalan_no.'" and referance!=""');
$sku_no_count= find_a_field('sale_sample_chalan','COUNT(sku_no)','chalan_no="'.$chalan_no.'" and sku_no!=""');
$pack_type_count= find_a_field('sale_sample_chalan','COUNT(pack_type)','chalan_no="'.$chalan_no.'" and pack_type!=""');
$color_count= find_a_field('sale_sample_chalan','COUNT(color)','chalan_no="'.$chalan_no.'" and color!=""');
$size_count= find_a_field('sale_sample_chalan','COUNT(size)','chalan_no="'.$chalan_no.'" and size!=""');

$do_no= find_a_field('sale_sample_chalan','do_no','chalan_no='.$chalan_no);

$master= find_all_field('sale_sample_master','','do_no='.$do_no);


$ch_data= find_all_field('sale_sample_chalan','','chalan_no='.$chalan_no);
            ...
            <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td width="15%">
                      <?php /*?>  <img src="<?=SERVER_ROOT?>public/uploads/logo/<?=$master->group_for?>.png" width="100%" /><?php */?>
    ...
    <td><div id="pr">
      ...
      <table width="100%" class="tabledesign" border="1" bordercolor="#000000" cellspacing="0" cellpadding="5"  style="font-size:10px">
        ...
        <?    $sqlc = 'select c.order_no, c.item_name as ch_item_name, c.delivery_date, c.delivery_place,c.printing_info,c.additional_info, c.measurement_unit, sum(c.total_unit) as total_unit,
		 ...
		 from sale_sample_chalan c, item_info i,  item_sub_group s, item_group g where i.item_id=c.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and c.chalan_no='.$chalan_no.' and c.total_unit>0 group by c.id, c.total_unit order by c.style_no ';
        ...
        <tr style="font-size:10px;">
          ...
          <td align="center" valign="top"><?= find_a_field('sale_sample_details','total_unit','id="'.$datac->order_no.'"');?></td>
  ...
  <tr>
  	<td colspan="2">
		...
		sum(c.bundle_1+c.bundle_2+c.bundle_3) as tot_bundle
		 from sale_sample_chalan c, item_info i,  item_sub_group s, item_group g where i.item_id=c.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and c.chalan_no='.$chalan_no.' group by c.item_id order by s.sub_group_id ';
			$queryc=db_query($sqlc);
			while($datac = mysqli_fetch_object($queryc)){
		  ...
		  <tr>
		    ...
		    <td width="100%" valign="top">
		      ...
		      <table width="100%" border="0" cellspacing="0" cellpadding="3"  style="font-size:12px">
		        ...
		        <tr>
		          <td width="10%" align="left" valign="middle">Product</td>
		          <td width="28%">:	&nbsp;Carton</td>
	              <td width="7%">PO No </td>
	              <td width="13%">: &nbsp;<?=$master->customer_po_no;?></td>
	              <td width="9%">Challan No </td>
	              <td width="11%">: &nbsp;<?=$ch_data->chalan_no;?></td>
	              <td width="8%">Vehicle No </td>
	              <td width="14%">: &nbsp;<?=$ch_data->vehicle_no;?></td>
