...
$title='Sample Sales Order Entry';
...
$table_detail='sale_sample_details';

$unique_detail='id';


$table_chalan='sale_sample_chalan';
...
{$$unique_master=find_a_field('sale_sample_details','do_no','id='.$_GET['del']); $del = $_GET['del'];}
...

{
		...
		$YR = date('Y',strtotime($job_date));
  		...
  		$job_cy_id = find_a_field('sale_sample_master','max(job_id)','year="'.$YR.'"')+1;
		
   		$cy_id = sprintf("%06d", $job_cy_id);
		
   		$job_no_generate='SAMPLE'.$yer.''.$month.''.$cy_id;
...
$gunny_sql = "INSERT INTO `sale_sample_details` 
...
//$cid = $crud->insert();
  ...
  //$challan_sql = "INSERT INTO `sale_sample_chalan` 
...
$do_details="SELECT a.*, i.item_name FROM sale_sample_details a, item_info i WHERE  a.item_id=i.item_id and do_no=".$do_no." order by id desc limit 1";
...
//, (a.init_pkt_unit*a.unit_price) as Total,(a.init_pkt_unit-a.inStock_ctn) as Shortage
  ...
  $res='select a.id, a.item_id, s.sub_group_name,  b.item_name, a.additional_info, a.printing_info,  a.ply, a.paper_combination, a.L_cm, a.W_cm, a.H_cm, a.measurement_unit, a.unit_price, a.final_price, a.total_unit, a.total_amt from sale_sample_details a, item_info b, item_sub_group s where b.item_id=a.item_id  and b.sub_group_id=s.sub_group_id and a.do_no='.$$unique_master.' order by a.id desc';
