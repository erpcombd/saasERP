...
$title='FOC Work Order Status';
do_calander('#fdate');
do_calander('#tdate');
$table_master='sale_foc_master';
$unique_master='foc_no';
...
<div class="form-container_large">
  <form action="" method="post" name="codz" id="codz">
	...
	<select name="foc_no" id="foc_no" style="width:280px;">
...
<tr>
  <th width="7%">FOC_No</th>
  <th width="7%">FOC Date</th>
  <th width="10%">Job No </th>
  <th width="23%">Customer Name</th>
  <th width="13%">Buyer  Name</th>
  <th width="15%">Merchandiser Name</th>
  <!--<th>Zone</th>-->
...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and m.foc_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
    ...
    $res="select m.foc_no, m.foc_no, m.job_no,  m.foc_date, m.dealer_code, m.buyer_code, m.merchandizer_code,  d.dealer_name_e, m.entry_by, m.status from 
sale_foc_master m, sale_foc_details c, dealer_info d, user_group u 
where 

  m.group_for=u.id  and m.dealer_code=d.dealer_code and m.foc_no=c.foc_no ".$group_for_con.$con.$dealer_con."  group by c.foc_no order by  m.foc_date, m.foc_no ";
$query = db_query($res);
...
<td onClick="custom(<?=$data->foc_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->dealer_name_e;?></td>
...
<td onClick="custom(<?=$data->foc_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;
  <?=find_a_field('user_activity_management','fname','user_id='.$data->entry_by);?></td>
<td onClick="custom(<?=$data->foc_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->status;?></td>
</tr>
<?
