...
$foc_chalan_no 		= $_REQUEST['v_no'];


$destination_count= find_a_field('sale_foc_chalan','COUNT(destination)','foc_chalan_no="'.$foc_chalan_no.'" and destination!=""');
$referance_count= find_a_field('sale_foc_chalan','COUNT(referance)','foc_chalan_no="'.$foc_chalan_no.'" and referance!=""');
$sku_no_count= find_a_field('sale_foc_chalan','COUNT(sku_no)','foc_chalan_no="'.$foc_chalan_no.'" and sku_no!=""');
$pack_type_count= find_a_field('sale_foc_chalan','COUNT(pack_type)','foc_chalan_no="'.$foc_chalan_no.'" and pack_type!=""');
$color_count= find_a_field('sale_foc_chalan','COUNT(color)','foc_chalan_no="'.$foc_chalan_no.'" and color!=""');
$size_count= find_a_field('sale_foc_chalan','COUNT(size)','foc_chalan_no="'.$foc_chalan_no.'" and size!=""');

$foc_no= find_a_field('sale_foc_chalan','foc_no','foc_chalan_no='.$foc_chalan_no);

$master= find_all_field('sale_foc_master','','foc_no='.$foc_no);


$ch_data= find_all_field('sale_foc_chalan','','foc_chalan_no='.$foc_chalan_no);



  		  $barcode_content = $foc_chalan_no;
...
$to_ctn = find_a_field('sale_foc_chalan','sum(pkt_unit)','foc_chalan_no='.$foc_chalan_no);

$to_pcs = find_a_field('sale_foc_chalan','sum(dist_unit)','foc_chalan_no='.$foc_chalan_no); 
...
<title><?=$master->job_no;?> - CH<?=$foc_chalan_no;?></title>
...
<table width="1200" border="0" cellspacing="0" cellpadding="0" align="center">
    ...
    <td><div class="header">
        ...
        <!--<td bgcolor="#666666" style="text-align:center; color:#FFF; font-size:18px; font-weight:bold;">WORK ORDER </td>-->
					...
					<tr>
					  ...
					  <td><span style="font-size:14px; padding: 3px 0 0 10px; letter-spacing:7px;"><?=$foc_chalan_no?></span></td>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0"  style="font-size:12px">
  	...
  	<tr height="30">
  	  <td valign="top"></td>
  	  <td  style="text-align:center; color:#FFF; font-size:18px; padding:0px 0px 10px 0px; color:#000000; font-weight:bold;"><span style="text-decoration:underline">GATE PASS</span> </td>
		    ...
		    <td width="100%" valign="top">
		      ...
		      <table width="100%" border="0" cellspacing="0" cellpadding="3"  style="font-size:12px">
		        ...
		        <tr>
	              ...
	              <td width="11%">: &nbsp;<?=$master->customer_po_no;?></td>
	              <td width="8%">Challan No </td>
	              <td width="8%">: &nbsp;<?=$ch_data->foc_chalan_no;?></td>
	              <td width="8%">Vehicle No </td>
	              <td width="8%">: &nbsp;<?=$ch_data->vehicle_no;?></td>
		        </tr>
		        <tr>
    ...
    <td><div id="pr">
      ...
      <table width="100%" class="tabledesign" border="1" bordercolor="#000000" cellspacing="0" cellpadding="5"  style="font-size:10px">
        ...
        <?    $sqlc = 'select c.order_no, c.item_name as ch_item_name, c.delivery_date, c.delivery_place,c.printing_info,c.additional_info, c.measurement_unit, sum(c.total_unit) as total_unit,
		 ...
		 from sale_foc_chalan c, item_info i,  item_sub_group s, item_group g where i.item_id=c.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and c.foc_chalan_no='.$foc_chalan_no.' group by c.id, c.total_unit order by c.id asc';
        ...
        <tr style="font-size:10px;">
          ...
          <td align="center" valign="top"><?= find_a_field('sale_foc_details','total_unit','id="'.$datac->order_no.'"');?></td>
  ...
  <tr>
  	<td colspan="2">
		...
		sum(c.bundle_1+c.bundle_2+c.bundle_3) as tot_bundle
		 from sale_foc_chalan c, item_info i,  item_sub_group s, item_group g where i.item_id=c.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and c.foc_chalan_no='.$foc_chalan_no.' group by c.item_id order by s.sub_group_id ';
			$queryc=db_query($sqlc);
			while($datac = mysqli_fetch_object($queryc)){
	...
	<table width="100%" cellspacing="0" cellpadding="0"  >
		...
		<tr>
		  ...
		  <? if ($ch_data->authorized_by==1) {?>
		  	<p style="font-size:16px; color:#000000; margin:0; padding: 0; font-family: 'Andina Demo'; font-weight:400; "> Md. Salahuddin Shakil</p>
