...
if(isset($_POST['confirm'])){
		...
		$foc_chalan_no = next_transection_no('0',$ch_date,'sale_foc_chalan','foc_chalan_no');
		
		$foc_gate_pass = next_transection_no('0',$ch_date,'sale_foc_chalan','foc_gate_pass');

		
		$ms_data = find_all_field('sale_foc_master','','foc_no='.$foc_no);

		 $sql = 'select * from sale_foc_details where  foc_no = '.$foc_no;
  ...
  $so_invoice = "INSERT INTO `sale_foc_chalan` (  foc_chalan_no, foc_gate_pass, chalan_date, foc_no, foc_date, chalan_no, order_no, ch_order_no, item_name, bundle_issue, do_no, do_date, job_no, delivery_date, cbm_no, group_for, dealer_code, buyer_code, merchandizer_code, destination, delivery_place, customer_po_no, unit_name, measurement_unit, ply, paper_combination_id, paper_combination, L_cm, W_cm, H_cm, WL, WW, item_id, formula_id, formula_cal, sqm_rate, sqm, additional_info, additional_charge, final_price, unit_price, pcs_1, bundle_1, pcs_2, bundle_2, pcs_3, bundle_3, total_unit, total_amt, style_no, po_no, referance, sku_no, printing_info, color, pack_type, size, depot_id, status, entry_time, edit_request, edit_accept, request_by, request_at, accept_by, accept_at, entry_by, entry_at, edit_by, edit_at, checked_by, checked_at, remarks, vehicle_type, vehicle_id, vehicle_no, driver_name, driver_mobile, delivery_man, delivery_man_mobile, prepared_by, authorized_by, reason)
  
  VALUES( '".$foc_chalan_no."', '".$foc_gate_pass."', '".$ch_date."', '".$data->foc_no."', '".$data->foc_date."', '".$data->chalan_no."', '".$data->id."', '".$data->order_no."', '".$item_name."', '".$data->bundle_issue."', '".$data->do_no."', '".$data->do_date."', '".$data->job_no."', '".$data->delivery_date."', '".$data->cbm_no."', '".$data->group_for."', '".$data->dealer_code."', '".$data->buyer_code."', '".$data->merchandizer_code."', '".$data->destination."', '".$data->delivery_place."', '".$data->customer_po_no."', '".$data->unit_name."', '".$data->measurement_unit."', '".$data->ply."', '".$data->paper_combination_id."', '".$data->paper_combination."', '".$data->L_cm."', '".$data->W_cm."', '".$data->H_cm."', '".$data->WL."', '".$data->WW."', '".$item_id."', '".$data->formula_id."', '".$data->formula_cal."', '".$data->sqm_rate."', '".$data->sqm."', '".$data->additional_info."', '".$data->additional_charge."', '".$data->final_price."', '".$rate."', '".$pcs_1."', '".$bundle_1."', '".$pcs_2."', '".$bundle_2."', '".$pcs_3."', '".$bundle_3."', '".$qty."', '".$amount."', '".$data->style_no."', '".$data->po_no."', '".$data->referance."', '".$data->sku_no."', '".$data->printing_info."', '".$data->color."', '".$data->pack_type."', '".$data->size."', '".$data->depot_id."', 'UNCHECKED', '0', '0', '0', '0', '0', '0', '0', '".$entry_by."', '".$entry_at."', '0', '0', '0', '0', '".$remarks."', '".$vehicle_type."', '".$vehicle_id."', '".$vehicle_no."', '".$driver_name."', '".$driver_mobile."', '".$delivery_man."', '".$delivery_man_mobile."', '".$prepared_by."', '".$authorized_by."', '".$data->reason."')";

db_query($so_invoice);



		//$ch_chalan_no = next_transection_no($group_for,$ch_date,'sale_do_chalan','chalan_no');
		//$ch_gate_pass = next_transection_no($group_for,$ch_date,'sale_do_chalan','gate_pass');
		//$ch_order_id  = find_a_field('sale_do_chalan','order_no',"id=".$data->order_no);


//$ch_invoice = "INSERT INTO `sale_do_chalan` (  `chalan_no`, gate_pass,  `chalan_date`, `order_no`, item_name, `bundle_issue`, `do_no`, `do_date`, `job_no`,
...
//  
//  VALUES( '".$ch_chalan_no."', '".$ch_gate_pass."',  '".$ch_date."', '".$ch_order_id."', '".$item_name."', '".$bundle_issue."', '".$data->do_no."', '".$data->do_date."', '".$data->job_no."', 
  ...
  <tr>
    ...
    <td colspan="5" valign="top"><table width="40%" border="1" align="center" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
      ...
      <tr>
        ...
        <td rowspan="2" align="left" bgcolor="#CCFF99"><a title="WO Preview" target="_blank" href="foc_order_print_view.php?v_no=<?=$$unique?>" ><img src="../../../images/print.png" alt="" width="30" height="30" /></a></td>
      </tr>
...
<? if($$unique>0){
  ...
  a.delivery_place, a.delivery_date, a.style_no, a.po_no, a.referance, a.sku_no, a.color, a.size, b.pack_size from sale_foc_details a,item_info b, item_sub_group s where b.item_id=a.item_id 
  and b.sub_group_id=s.sub_group_id and  a.foc_no='.$$unique;
      ...
      <tbody>
		...
		<? 
		  ...
		  ?>			  </td>
              ...
              <td width="4%" align="center"><? echo number_format($so_qty = (find_a_field('sale_foc_chalan','sum(total_unit)','order_no="'.$row->id.'" and item_id="'.$row->item_id.'"')*(1)),2);?></td>
