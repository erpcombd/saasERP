...
$title='FOC Delivery Challan Status';
...
$target_url = 'foc_challan_print_view.php';
...
<tr>
  ...
  <th width="16%">Buyer  Name</th>
  <th width="14%">Merchandiser Name</th>
...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and c.chalan_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
		...
		if($_POST['foc_no']!='')
 		$job_no_con=" and c.foc_no='".$_POST['foc_no']."'";


    $res="select c.foc_chalan_no, c.foc_gate_pass, c.chalan_date,  m.do_no, m.job_no,  m.do_date, m.dealer_code, m.buyer_code, m.merchandizer_code,  d.dealer_name_e, c.entry_by, c.status from 
sale_foc_master m, sale_foc_chalan c, dealer_info d, user_group u 
where 

 m.group_for=u.id  and m.dealer_code=d.dealer_code and m.foc_no=c.foc_no ".$group_for_con.$con.$dealer_con.$job_no_con."  group by c.foc_chalan_no order by  c.chalan_date, m.foc_no,c.foc_chalan_no ";
$query = db_query($res);
...
<tr <?=($data->RCV_AMT>0)?'style="background-color:#FFCCFF"':'';?>>
  <td><a title="Challan Preview" target="_blank" href="foc_challan_print_view.php?v_no=<?=$data->foc_chalan_no?>" style=" font-weight:700; color:#000000" ><?=$data->foc_chalan_no;?></a></td>
  <td><a title="Gate Pass Preview" target="_blank" href="foc_gate_pass_print_view.php?v_no=<?=$data->foc_gate_pass?>"  style=" font-weight:700; color:#000000" ><?=$data->foc_gate_pass;?></a></td>
