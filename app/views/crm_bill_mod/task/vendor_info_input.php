...
$payable_ledger	= $_REQUEST['payable_ledger'];
...
if(isset($_POST['nvendor']))
{
	...
	$account_code=next_ledger_ids('1004');
...
VALUES ("'.$account_code.'","'.trim($_POST['dealer_name_e']).'",1004,0.00,"Both", 0.00, 0.00,'.strtotime(date("Y-m-d H:i:s")).',"philips","NO", 2, 0, 0, 0)';
...
db_query($acc_query);
		...
		//$sub_ledger_id=number_format(next_sub_ledger_id($payable_ledger), 0, '.', '');
//		$ledger_group=find_a_field('accounts_ledger','ledger_group_id',"ledger_id=".$payable_ledger);
//		$ledger_layer=2;
//		ledger_create_dealer($sub_ledger_id,$vendor_name,$ledger_group,$group_for,$payable_ledger,'Both','','', time(),$ledger_layer,'YES','');
...
entry_at
		...
		VALUES ('$vendor_name', '$vendor_company', '$vendor_bank', '$vendor_bank_ac', '$branch_name','$iban_no' ,'$swift_code' ,'$group_for' ,'$vendor_category' ,'$country' ,'$contact_no' ,'$fax_no' ,'$email' ,'$address' ,'$status','$payable_ledger','$sub_ledger_id' ,'$entry_by','$entry_at')";
		//echo $sql;
		$query=db_query($sql);
		$type=1;
...
//$query=db_query($sql2);
	...
	$msg='Given Name('.$vendor_name.') is already exists.';
	 ...
	 /*?>	 $vendor_id= $vendor_id;
		 $ledger_id=find_a_field('vendor','ledger_id',"vendor_id=".$vendor_id);
		...
		ledger_create_dealer($sub_ledger_id,$vendor_name,$ledger_group,$group_for,$payable_ledger,'Both','','', time(),$ledger_layer,'YES','');
		 ...
		 $sql_acc="UPDATE `vendor` SET
							
							`ledger_id`='$sub_ledger_id'
							
							 WHERE 
							 `vendor_id` 		= $vendor_id LIMIT 1";
		$qry_acc=db_query($sql_acc);
...
-->
 ...
 <form id="form2" name="form2" method="post" action="vendor_info_input.php?vendor_id=<?php echo $vendor_id;?>" onsubmit="return check()">
                                  ...
                                  <td><div class="box">
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
									  ...
									  <tr>
                                        <td>Vendor Name: </td>
                                        <td>
										<input name="vendor_id" type="hidden" id="vendor_id" value="<?php echo $data[0];?>"  />
										<input name="vendor_name" type="text" id="vendor_name" class="form-control" value="<?php echo $data[3];?>" required />
										</td>
									  </tr>
									
                                      <tr>
                                        <td>Beneficiary Name: </td>
                                        <td><input name="vendor_company" type="text" id="vendor_company" value="<?php echo $data[5];?>" size="30" maxlength="100" /></td>
									  </tr>
	...
	<option value="<?=$ledg[0]?>" <?php if($data[2]==$ledg[0]) echo " Selected "?>><?=$ledg[0].':'.$ledg[1]?>
			...
			<input name="payable_ledger" type="hidden"  id="payable_ledger" tabindex="10"
						 value="<?=find_a_field('config_group_class','payable',"group_for=".$_SESSION['user']['group']);?>" />
