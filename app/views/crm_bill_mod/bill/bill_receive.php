...
if(isset($_REQUEST['confirmit']))
{
		...
		$cheque_date = $_POST['cheque_date'];
		 ...
		 $sql = "select b.bill_no,d.customer_name, b.bill_date, b.manual_bill_no, b.amount, b.status,d.ledger_id,b.customer,t.type as type_name from crm_bill_info b, crm_service_customer d,crm_acc_bill_type t where b.customer=d.customer_id and b.status='BILL SUBMITTED' and b.service_type=t.id ".$con."";
...

		{
			...
			if($_FILES['receive_att'.$data->bill_no]['tmp_name']!=''){
						...
						db_query('delete from crm_document_upload where master_id="'.$rand.'" and tr_from="BillReceive"');

						move_uploaded_file($file_tmp, $uploaded_file);

						

						$file_insert = 'insert into crm_document_upload (`master_id`,`tr_from`,`file_name`,`entry_at`,`entry_by`) value("'.$data->bill_no.'","BillReceive","'.$uploaded_file.'","'.date('Y-m-d H:i:s').'","'.$_SESSION['user']['id'].'")';
        ...
        add_to_sec_journal($proj_id, $jv_no, $jv_date, $customer_ledger->ledger_id, $narration,'0', $total_receivable, $tr_from, $tr_no,'',$tr_id=0,$cc_code,$group_for,'','','','','','','','NO');
		if($discount>0){
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $config_ledger->sales_discount, $narration, $discount,'0', $tr_from, $tr_no,'',$tr_id=0,$cc_code,$group_for,'','','','','','','','NO');
		}
		if($tax>0){
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $config_ledger->tds, $narration, $tax,'0', $tr_from, $tr_no,'',$tr_id=0,$cc_code,$group_for,'','','','','','','','NO');
		}
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $bank_ledger, $narration, $receipt_amt, '0', $tr_from, $tr_no,'',$tr_id=0,$cc_code,$group_for,'','','','','','','','NO');
 	...
 	$bill_amount  = find_a_field('crm_bill_info','sum(net_receivable_amt)','bill_no="'.$data->bill_no.'"'); 

 $receive_journal_amt = find_a_field('secondary_journal','sum(cr_amt)','ledger_id="'.$data->ledger_id.'" and tr_no="'.$data->bill_no.'" and  tr_from in(select concat(type," Receive") from crm_acc_bill_type)');

if($receive_journal_amt>=$bill_amount){

 $update = 'update crm_bill_info set status="BILL RECEIVED",bank="'.$bank_ledger.'",cheque_no="'.$cheque_no.'",checkq_date="'.$cheque_date.'",receive_date="'.$jv_date.'",bill_received_by="'.$_SESSION['user']['id'].'",bill_received_at="'.date('Y-m-d H:i:s').'" where bill_no="'.$data->bill_no.'"';
...
window.location.href = "bill_receive.php";
...
/*}*/
	...
	<div class="form-container_large">
		...
		<form action="" method="post" name="codz" id="codz" enctype="multipart/form-data">
								...
								<div class="form-group row m-0 pb-1">
									...
									<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
										...
										<select name="bill_no" id="bill_no" >
											...
											<? foreign_relation('crm_bill_info','bill_no','manual_bill_no',$_POST['bill_no'],'status="BILL SUBMITTED"'); ?>
									...
									<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
										...
										<span id="cash_bank_filter">
											...
											<select name="dr_ledger_id" id="dr_ledger_id" required="required">
												...
												<option></option>
             <? foreign_relation('accounts_ledger','ledger_id','ledger_name',$_POST['dr_ledger_id'],'ledger_group_id in (127001,127002)');?>
			...
			<div class="container-fluid pt-3 p-0">
				...
				<table  id="grp"  class="table1  table-striped table-bordered table-hover table-sm">
					...
					$sql = "select b.bill_no,d.customer_name, b.bill_date, b.manual_bill_no, b.status,d.ledger_id,b.discount_amt,t.type as type_name from crm_bill_info b, crm_service_customer d,crm_acc_bill_type t where b.customer=d.customer_id and b.status='BILL SUBMITTED' and t.id=b.service_type ".$con."";
		...
		$invoice_amt = find_a_field('crm_bill_details','sum(service_charge)','bill_no="'.$data->bill_no.'"');
...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0" style="font-size:12px; text-transform:uppercase;">
 ...
 if($_POST['account_code']!='')
  ...
  $con .=" and b.customer=".$_POST['account_code'];
    ...
    $sql = "select b.bill_no,d.customer_name, b.bill_date, b.manual_bill_no, b.status,d.ledger_id,b.discount_amt from crm_bill_info b, crm_service_customer d where b.customer=d.customer_id and b.status='BILL SUBMITTED' ".$con."";
