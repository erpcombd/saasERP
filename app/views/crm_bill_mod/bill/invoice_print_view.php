...
$sql="select * from crm_bill_info where  bill_no='$bill_no'";
$data=db_query($sql);
...
$customer=find_all_field('crm_service_customer','','customer_id='.$all->customer);
...
$pi=0;
$total=0;
 $sql2="select b.*, a.type from crm_bill_details b, crm_acc_bill_type a where a.id=b.service_type and b.bill_no='$bill_no'";
$data2=db_query($sql2);
//echo $sql2;
...
?>
      <tr>
        ...
        <td valign="top" align="left"><?=$info->type?> for the period of <?=date('M-Y',strtotime($all->bill_date))?></td>
        <td valign="top" align="right"><?=number_format($info->service_charge,2); $total_amount+=$info->service_charge; $vat=($total_amount*10)/100;$g_total=$total_amount+$vat;?></td>
