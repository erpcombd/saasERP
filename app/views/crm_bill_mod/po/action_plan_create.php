...
$table_master='daily_action_plan_master';

$table_details='daily_action_plan_details';
...
//			}
		...
		header('Location:action_plan_create.php');
...
$sql3='update requisition_order r, daily_action_plan_details p set r.req_status=0 where p.req_id=r.id and p.id='.$_GET['del'];
db_query($sql3);
...

{
        ...
        //$sql = 'select * from daily_progress_details where d_id='.$$unique;
//	   $qu = db_query($sql);
//	   while($re = mysqli_fetch_object($qu)){
//	   		 $update_sql = 'update daily_progress_details set qty = "'.$_POST['qty'.$re->id].'", rate = "'.$_POST['unit_price'.$re->id].'", amount = "'.$_POST['amount'.$re->id].'" 
//			where id='.$re->id;
//			db_query($update_sql);
//	   }
		...
		header('Location:action_plan_create.php?new=2');
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
 ...
 $res='select a.id, a.date, a.day, a.customer_name as name, a.address as section, a.pet_type as action_plan, a.delivery as target_amount, a.gap_analysis as challenges, a.collection as progress, a.man_power as follow_up, "x" from daily_action_plan_details a where  a.d_id='.$d_id; 
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
 ...
 $res='select a.id, d.particulars,a.customer_name,a.amount, a.plan, a.progress, a.problem,a.requisition,"x" 
  from daily_action_plan_details a,daily_progress_setup d where a.particular=d.id and  a.d_id='.$d_id; 
	echo link_report_add_del_auto($res,'',0,0);
?>
...
<input  name="progress_for" type="hidden" id="progress_for" value="<?=$progress_for?>"/>
<input  name="progress_date" type="hidden" id="progress_date" value="<?=$progress_date?>"/>
<input name="address" type="text" class="input3" id="address" style="width:98%;" /></td>
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  $res='select a.id, a.address as project, a.customer_name as section, a.plan as action_plan, a.man_power as challenges, a.service_type as departmental_step, a.progress, a.gap_analysis as recommendation, a.findings as suggestion, a.next_visit as time_line, "x" from daily_action_plan_details a where  a.d_id='.$d_id; 
	echo link_report_add_del_auto($res,'',0,0);
?>
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  $res='select a.id, a.date, a.customer_name, a.man_power as lot_no, a.plan as product_name, a.amount as unit_price, a.collection as production_target, a.outstanding as production_achieved, a.category as delivery_target, a.pet_type as delivery_achieved, "x" from daily_action_plan_details a where  a.d_id='.$d_id; 
	echo link_report_add_del_auto($res,'',0,0);
?>
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  $res='select a.id, a.date, a.day as Time, a.customer_name as Adress, a.plan as Work_Details, a.progress as Progress, a.gap_analysis as Problem, "x" from daily_action_plan_details a where  a.d_id='.$d_id; 
	echo link_report_add_del_auto($res,'',0,0);
?>
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  $res='select a.id, a.customer_name as Owner,a.address,a.man_power as Pet_name,a.pet_type,a.pet_category,a.service_type,a.mobile,a.email,a.next_visit,a.amount,a.findings as remarks, "x" from daily_action_plan_details a where  a.d_id='.$d_id; 
	echo link_report_add_del_auto($res,'',0,0);
?>
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  from daily_action_plan_details a where   a.d_id='.$d_id;  
	echo link_report_add_del_auto($res,'',0,0);
?>
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  $res='select a.id, a.date, a.day, a.customer_name as section, a.plan as action_plan, a.man_power as challenges,a.collection as qty, a.progress, a.gap_analysis as follow_up,  "x" from daily_action_plan_details a where  a.d_id='.$d_id; 
	echo link_report_add_del_auto($res,'',0,0);
?>
...
<input type="hidden" name="req_no"  value="<?=$req_no?>"/>
  <? $d_d=find_a_field('daily_action_plan_details','count(d_id)','d_id='.$d_id);
   if($d_d>0){
  ?>
