...
$sql3='update requisition_order r, daily_plan_details p set r.req_status=0 where p.req_id=r.id and p.id='.$_GET['del'];
db_query($sql3);
...
<? } elseif($progress_for=='53'){ ?>
			<table  width="100%" border="1" align="left"  style="border-collapse:collapse; border:1px solid #caf5a5;" cellpadding="2" cellspacing="2">
                      ...
                      <tr>
                        ...
                        <td width="11%" align="center" bgcolor="#0099FF"><strong>Consumption Plan</strong></td>
                        <td width="11%" align="center" bgcolor="#0099FF"><strong>Progress</strong></td>
...
<input name="day" type="text" class="input3" id="day" style="width:98%;"  />
    ...
    <td ><textarea name="customer_name" class="input3" rows="2" id="customer_name"></textarea></td>
    <td ><input name="plan" type="text" class="input3" id="plan" style="width:98%;" /></td>
	<td width="25%"><textarea name="man_power" class="input3" rows="2" id="man_power"></textarea></td>
	<td ><textarea name="progress" class="input3" rows="2" id="progress"></textarea></td>
	<td width="25%" ><textarea name="requisition" class="input3" rows="2" id="requisition"></textarea></td>
	<td ><textarea name="amount" class="input3" rows="2" id="amount"></textarea></td>
      </tr>
    </table>
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  $res='select a.id, a.day as project_name, a.customer_name as action_plan, a.plan as man_power, a.man_power as consumption_plan, a.progress,a.requisition as actual_consumption, a.amount as production, "x" from daily_plan_details a where  a.d_id='.$d_id; 
	echo link_report_add_del_auto($res,'',0,0);
?>
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  $res='select a.id, a.customer_name, a.man_power as lot_no, a.plan as product_name, a.delivery as consumption_cost,  a.amount as operating_cost, a.collection as overhead_cost, a.outstanding as production_target, a.category as production_achieved, a.pet_type as production_cost, "x" from daily_plan_details a where  a.d_id='.$d_id; 
	echo link_report_add_del_auto($res,'',0,0);
?>
...
<? }elseif($progress_for=='54'){?>
	<table  width="100%" border="1" align="left"  style="border-collapse:collapse; border:1px solid #caf5a5;" cellpadding="2" cellspacing="2">
					...
					<input  name="progress_for" type="hidden" id="progress_for" value="<?=$progress_for?>"/>
					<input  name="progress_date" type="hidden" id="progress_date" value="<?=$progress_date?>"   />
					<input name="particular" type="text" class="input3" id="particular" style="width:98%;" />
					<!--<select name="particular" type="text" class="input3" id="particular" style="width:98%;" >
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
	...
	$res='select a.id,  a.particular,  a.head_office as action_plan, a.showroom as amount, a.ld_hospital as follow_up, "x" 
  from daily_plan_details a where  a.d_id='.$d_id;
...
<input type="hidden" name="req_no"  value="<?=$req_no?>"/>
  <? $d_d=find_a_field('daily_plan_details','count(d_id)','d_id='.$d_id);
   if($d_d>0){
  ?>
