...
$found = find_a_field('item_info','count(item_id)','finish_goods_code like "'.$_POST['finish_goods_code'].'"');
if($found<1){
	$group_name = trim($_POST['finish_goods_code']);
	if (!empty($group_name)) {
		$found2 = find_a_field('item_info','count(item_id)','item_name like "'.$_POST['item_name'].'"');
		if($found2<1){
			$group_name2 = trim($_POST['item_name']);
...
$msg='Product Name is already exist';
	...
	$msg='Empty value can not be inserted fff';
}
}else{
...

{
	...
	$found = find_a_field('item_info','count(item_id)','finish_goods_code like "'.$_POST['finish_goods_code'].'" and item_id != "'.$_POST['item_id'].'"');
	if($found<1){
		$group_name = trim($_POST['finish_goods_code']);
		if (!empty($group_name)) {
			$found2 = find_a_field('item_info','count(item_id)','item_name like "'.$_POST['item_name'].'" and item_id != "'.$_POST['item_id'].'"');
			if($found2<1){
				$group_name2 = trim($_POST['item_name']);
				if (!empty($group_name2)) {
...
$_POST['item_description'] = str_replace("'","`",$_POST['item_description']);
		...
		$msg='Empty value can not be inserted xx';
	}
	}else{
	$msg='Product Name is already exist';
	}
		...
		<div class="col-sm-6 col-md-6 col-lg-6">
			...
			<table  class="w-100">
				<tbody>
					...
					<tr class="tr">
						<td class="td1 fs-14 bold">Product Category :</td>
						<td class="td2">
										...
										$a2="select a.sub_group_id, a.sub_group_name,b.group_name from item_sub_group a ,item_group b where 1 and a.group_id=b.group_id and a.group_for='".$_SESSION['user']['group']."' and a.status='1'";

										

										$a1=db_query($a2);

										echo "<select name=\"sub_group_id\" id=\"sub_group_id\"\" required  tabindex=2>";
										echo "<option></option>";
...

										{
											...
											if($a[0]==$sub_group_id)
                                               {
												echo "<option value=\"".$a[0]."\" selected>".$a[2].'--'.$a[1].'hhhhhh'.$sub_group_id."</option>";
											   }
											else
											   {
												echo "<option value=\"".$a[0]."\">".$a[2].'--'.$a[1].'hhhhhh'.$sub_group_id."</option>";
											   }
										}
...
<table id="example" class="table1  table-striped table-bordered table-hover table-sm">
								...
								<?php
								 ...
								 $td="select a.item_id,a.item_name,a.sub_group_id, a.item_description,a.finish_goods_code, a.status, a.entry_by ,b.sub_group_name
								 FROM item_info a,item_sub_group b where 1 and a.sub_group_id=b.sub_group_id ".$con."";

								$report=db_query($td);
                                //print_r($report);
								while($rp=mysqli_fetch_array($report)){$i++; if($i%2==0){$cls=' class="alt"';} else {$cls='';}
								...
								?>
									...
									<tr<?=$cls?> onclick=";">
										...
										<td class="text-left"><?=$rp[7];?></td>
										<td><?=$rp[5];?></td>
										<td><? if($rp[6]>0){ echo find_a_field('user_activity_management','fname','user_id="'.$rp[6].'"');}?></td>
										<td><button type="button" onclick="nav('<?=$rp[0];?>');" class="btn2 btn1-bg-update"><em class="fa-solid fa-pen-to-square"></em></button></td>
...
<?
datatable_esourcing("#example");
require_once SERVER_ROOT."public/assets/datatable_esourcing/datatable.php";
