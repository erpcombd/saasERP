...
{	
	...
	$status=$_POST['status'];
		...
		$_POST['entry_by'] = $_SESSION['user']['id'];
		 ...
		 $_POST['group_for'] = 1;	
		 $found = find_a_field('vendor_category','count(id)','category_name like "'.$_POST['category_name'].'"');
		 if($found<1){
			$group_name = trim($_POST['category_name']);
			if (!empty($group_name)) {
...
{
	$found = find_a_field('vendor_category','count(id)','category_name like "'.$_POST['category_name'].'" and id != "'.$_POST['id'].'"');
	if($found<1){
	   $group_name = trim($_POST['category_name']);
	   if (!empty($group_name)) {
		...
		// echo '<script>window.location.href = "vendor_category.php?id=''"</script>';
		}else{
			$msg='Empty value can not be inserted';
		}
		}
			...
			<table  class="w-100">
				<caption></caption>
					<tr class="tr">
						...
						<td class="td2">
							<input name="<?=$unique?>" id="<?=$unique?>" value="<?=$_SESSION[$unique]?>" type="hidden" required/>
