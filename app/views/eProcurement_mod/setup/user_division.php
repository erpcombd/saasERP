...
$found = find_a_field('user_division','count(id)','division_name like "'.$_POST['division_name'].'"');
if($found<1){
	$group_name = trim($_POST['division_name']);
	if (!empty($group_name)) {
		$crud->insert();
		$type=1;
	...
	$msg='Event Divison is already exist';
	}
}
...
{
	$found = find_a_field('division_name','count(id)','division_name like "'.$_POST['division_name'].'" and id != "'.$_POST['id'].'"');
if($found<1){
	$group_name = trim($_POST['division_name']);
	if (!empty($group_name)) {
	...
	<!-- <div class="col-sm-4 col-md-4 col-lg-4"></div> -->
		<div class="col-sm-4 col-md-4 col-lg-4">
			...
			<table  class="w-100">
				<caption></caption>
					<tr class="tr">
						<td class="td1 fs-14 bold req-input">New User Division</td>
						<td class="td2">
							...
							<input class="m-0" name="division_name" required type="text" id="division_name" tabindex="1"  value="<?=$division_name?>" />
...
<tr class="bgc-info" >
	...
	<th scope="col" scope="col">User Division</th>
...
<tbody>
				<?  $data = "SELECT * FROM  user_division WHERE 1"; 
					$query = db_query($data);
					while($row =mysqli_fetch_object($query)){
				?>
					<tr>
						<td><?=$row->id;?></td>
						<td style="text-align:left;"><?=$row->division_name;?></td>
