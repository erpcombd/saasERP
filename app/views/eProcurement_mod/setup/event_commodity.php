...
$unique='id';  		
$title='Event Commodity Information' ; 	
$page="event_commodity.php";		
$table='event_commodity';		
...
$found = find_a_field('event_commodity','count(id)','event_commodity like "'.$_POST['event_commodity'].'"');
if($found<1){
	$group_name = trim($_POST['event_commodity']);
	if (!empty($group_name)) {
		$crud->insert();
		$type=1;
...
{
	$found = find_a_field('event_commodity','count(id)','event_commodity like "'.$_POST['event_commodity'].'" and id != "'.$_POST['id'].'"');
if($found<1){
	$group_name = trim($_POST['event_commodity']);
	if (!empty($group_name)) {
...
<h1 class="container" style=" font-size: 30px !important; ">Event Commodity Information</h1>
	...
	<!-- <div class="col-sm-4 col-md-4 col-lg-4"></div> -->
		<div class="col-sm-4 col-md-4 col-lg-4">
			...
			<table  class="w-100">
				<caption></caption>
					<tr class="tr">
						<td class="td1 fs-14 bold req-input">New Event Commodity</td>
						<td class="td2">
							...
							<input class="m-0" name="event_commodity" required type="text" id="event_commodity" tabindex="1"  value="<?=$event_commodity?>" />
...
<tbody>
				<?  $data = "SELECT * FROM  event_commodity WHERE 1"; 
					$query = db_query($data);
					while($row =mysqli_fetch_object($query)){
				?>
					<tr>
						<td><?=$row->id;?></td>
						<td style="text-align:left;"><?=$row->event_commodity;?></td>
