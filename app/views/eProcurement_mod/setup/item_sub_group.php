...

{
	...
	$found = find_a_field('item_sub_group','count(sub_group_id)','sub_group_name like "'.$_POST['sub_group_name'].'"');
	if($found<1){
		$group_name = trim($_POST['sub_group_name']);
		if (!empty($group_name)) {
...
$msg='Product Group is already exist';
}
...
{
	$found = find_a_field('item_sub_group','count(sub_group_id)','sub_group_name like "'.$_POST['sub_group_name'].'" and sub_group_id !="'.$_POST['sub_group_id'].'"');
	if($found<1){
		$group_name = trim($_POST['sub_group_name']);
		if (!empty($group_name)) {
	...
	$msg='Product Sub Group is already exist';
	}
		...
		<div class="col-sm-6 col-md-6 col-lg-6">
			...
			<table  class="w-100">
				<tbody>
					...
					<tr class="tr">
						...
						<td class="td2">
						    <select name="group_id" id="group_id" required="required" tabindex=1>
                                <option></option>
                                <?	$sql="select * from item_group where  group_for='".$_SESSION['user']['group']."' and status='1' order by group_id";
...

                                {
                                    ...
                                    ?>
                                    <option <? if($datas->group_id==$group_id) {echo 'Selected ';}else if($datas->group_id==$_POST['group_id']){echo 'Selected ';}?> value="<?=$datas->group_id?>">
                                        <?=$datas->group_name?>
                                    </option>
                                <? } ?>
                            </select>
					...
					<td class="td2"><input name="description" type="text" id="description" value="<?php echo $description==''?$_POST['description']:$description;?>" class="required" tabindex=2/></td>
				...
				<tbody>
					...
					<tr class="tr">
						...
						<td class="td2">
                            ...
                            <input name="sub_group_name" type="text" required id="sub_group_name" value="<?php echo  $sub_group_name==''?$_POST['sub_group_name']:$sub_group_name;?>" class="required" tabindex=3/>
					...
					<select name="status" required id="status"  tabindex=5 required>
								  <option <?php if($status=='1'||$_POST['status']=='1') echo 'selected'; ?> value="1">Active</option>
								  <option <?php if($status=='0' || $_POST['status']=='0') echo 'selected'; ?> value="0">Inactive</option>
						</select>
                    </td>
					</tr>
...
<table id="example" class="table1  table-striped table-bordered table-hover table-sm" >
                    ...
                    <thead class="thead1">
                        ...
                        $rrr = "select b.sub_group_id,b.sub_group_id,a.group_name, b.sub_group_name, b.description, b.entry_by,b.status from item_sub_group b,item_group a where a.group_id=b.group_id and b.group_for='".$_SESSION['user']['group']."'   order by a.status";
