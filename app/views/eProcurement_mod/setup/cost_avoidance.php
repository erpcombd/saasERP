...
{	
	$status=$_POST['status'];		
	$cost_avoidance=$_POST['cost_avoidance'];		
$_POST['entry_at']=date('Y-m-d H:i:s');
$_POST['entry_by']=$_SESSION['user']['id'];
$found = find_a_field('cost_avoidance','count(id)','cost_avoidance like "'.$_POST['cost_avoidance'].'"');
if($found<1){
	$group_name = trim($_POST['cost_avoidance']);
	if (!empty($group_name)) {
		$crud->insert();
		$type=1;
	...
	$msg='Savings Type is already exist';
	}
}
...
{
	$found = find_a_field('cost_avoidance','count(id)','cost_avoidance like "'.$_POST['cost_avoidance'].'" and id != "'.$_POST['id'].'"');
	if($found<1){
		$group_name = trim($_POST['cost_avoidance']);
		if (!empty($group_name)) {
