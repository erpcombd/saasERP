...
$found = find_a_field('item_group','count(group_id)','group_name like "'.$_POST['group_name'].'"');
if($found<1){
...
{ 
	...
	$found = find_a_field('item_group','count(group_id)','group_name like "'.$_POST['group_name'].'" and group_id !="'.$_POST['group_id'].'"');
if($found<1){
	$group_name = trim($_POST['group_name']);
	if (!empty($group_name)) {
...
<table id="example" class="table1  table-striped table-bordered table-hover table-sm">
    <caption> </caption>
					...
					$rrr = "select b.group_id,  b.group_name, b.description, b.entry_by,b.status from item_group b, user_group a where a.id=b.group_for and group_for='".$_SESSION['user']['group']."' ";
