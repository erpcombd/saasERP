...
$unique='id';  		
$title='Unit Management Information' ; 	
...
{		
$_POST['entry_at']=strtotime(date('Y-m-d H:i:s'));
$_POST['entry_by']=$_SESSION['user']['id'];
$found = find_a_field('unit_management','count(id)','unit_name like "'.$_POST['unit_name'].'"');
if($found<1){
	$group_name = trim($_POST['unit_name']);
	if (!empty($group_name)) {
		$crud->insert();
		$type=1;
		$msg='New Entry Successfully Inserted.'.$_POST['unit_name'];
...
if(isset($_POST['update']))
{
	...
	$found = find_a_field('unit_management','count(id)','unit_name like "'.$_POST['unit_name'].'" and id !="'.$_POST['id'].'"');
if($found<1){
	$group_name = trim($_POST['unit_name']);
	if (!empty($group_name)) {
...
<h1 class="container" style=" font-size: 30px !important; ">Unit Management Information</h1>
	...
	<div class="row m-0 p-0 pt-1 pb-3">
				...
				<caption></caption>
					<tr class="tr">
						...
						<td class="td2">
							...
							<input class="m-0" name="unit_name" required type="text" id="unit_name" tabindex="1"  value="<?=$unit_name?>" />
						</td>
						<td class="td1 fs-14 bold">Unit Detail</td>
						<td class="td2">
							<input class="m-0" name="unit_detail"  type="text" id="unit_detail" tabindex="2"  value="<?=$unit_detail?>" />
						</td>
		...
		<div class="w-100 pt-3">
					 ...
					 <? if(isset($_GET[$unique])){?>
					  ...
					  <input class="btn1 btn1-bg-hrm" name="Delete" type="submit" id="Delete" value="Delete"  tabindex="4"/>
...
<tr class="bgc-info" >
	...
	<th scope="col" scope="col">Unit Name</th>
	<th scope="col" scope="col">unit_detail</th>
...
<tbody>
				<?  $data = "SELECT * FROM  unit_management WHERE 1"; 
					$query = db_query($data);
