...
$unique='id';  		
$title='Sourcing Type Information' ; 	
$page="sourcing_type.php";		
$table='sourcing_type';		
...
{		
	$status=$_POST['status'];	
	$sourcing_type=$_POST['sourcing_type'];	
$_POST['entry_at']=date('Y-m-d H:i:s');
$_POST['entry_by']=$_SESSION['user']['id'];
$found = find_a_field('sourcing_type','count(id)','sourcing_type like "'.$_POST['sourcing_type'].'"');
if($found<1){
	$group_name = trim($_POST['sourcing_type']);
	if (!empty($group_name)) {
		$crud->insert();
		$type=1;
		$msg='New Entry Successfully Inserted.';
		// echo '<script>window.location.href = "sourcing_type.php"</script>';
	}else{
		$msg='Empty value can not be inserted';
	}
	}else{
	$msg='Sourcing Type is already exist';
	}
}
...
{
	$found = find_a_field('sourcing_type','count(id)','sourcing_type like "'.$_POST['sourcing_type'].'" and id != "'.$_POST['id'].'"');
	if($found<1){
		$group_name = trim($_POST['sourcing_type']);
		if (!empty($group_name)) {
...
<h1 class="container" style=" font-size: 30px !important; ">Sourcing Type Information</h1>
	...
	<!-- <div class="col-sm-4 col-md-4 col-lg-4"></div> -->
		<div class="col-sm-6 col-md-6 col-lg-6">
			...
			<table  class="w-100">
				<caption></caption>
					<tr class="tr">
						<td class="td1 fs-14 bold req-input">New Sourcing Type</td>
						<td class="td2">
							...
							<input class="m-0" name="sourcing_type" required type="text" id="sourcing_type" tabindex="1"  value="<?=$sourcing_type?>" />
...
<tr class="bgc-info" >
	...
	<th scope="col" scope="col">Sourcing Type</th>
...
<tbody>
				<?  $data = "SELECT * FROM  sourcing_type WHERE 1"; 
					$query = db_query($data);
					while($row =mysqli_fetch_object($query)){
				?>
					<tr>
						<td><?=$row->id;?></td>
						<td style="text-align:left;"><?=$row->sourcing_type;?></td>
