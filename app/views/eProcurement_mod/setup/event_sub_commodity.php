...
$unique='id';  		
$title='Event Sub Commodity Information' ; 	
$page="event_sub_commodity.php";		
$table='event_sub_commodity';		
...
{	
	$event_sub_commodity=$_POST['event_sub_commodity'];	
	$status=$_POST['status'];	
	$event_commodity=$_POST['event_commodity'];	

$_POST['entry_at']=date('Y-m-d H:i:s');
$_POST['entry_by']=$_SESSION['user']['id'];
$found = find_a_field('event_sub_commodity','count(id)','event_sub_commodity like "'.$_POST['event_sub_commodity'].'"');
if($found<1){
	$group_name = trim($_POST['event_sub_commodity']);
	if (!empty($group_name)) {
		$crud->insert();
		$type=1;
	...
	$msg='Event Commodity is already exist';
	}
}
...
{
	$found = find_a_field('event_sub_commodity','count(id)','event_sub_commodity like "'.$_POST['event_sub_commodity'].'" and id !="'.$_POST['id'].'"');
	if($found<1){
		$group_name = trim($_POST['event_sub_commodity']);
		if (!empty($group_name)) {
...
<h1 class="container" style=" font-size: 30px !important; ">Event Sub Commodity Information</h1>
<? if($msg !=''){ ?>
...
<div class="container pt-0 mt-5 p-0 ">
	...
	<div class="col-sm-4 col-md-4 col-lg-4">
		<table  class="w-100">
				<caption></caption>
					<tr class="tr">
						<td colspan="2" class="td1 fs-14 bold req-input">New Event Sub Commodity</td>
						<td class="td2">
							...
							<input class="m-0"  name="event_sub_commodity" required type="text" id="event_sub_commodity" tabindex="1"  value="<?=$event_sub_commodity?>" />
		...
		<div class="col-sm-4 col-md-4 col-lg-4">
			...
			<table  class="w-100">
				<caption></caption>
					<tr class="tr">
						<td class="td1 fs-14 bold req-input" colspan="2">Event Commodity</td>
						<td class="td2">
							<select class="m-0" name="event_commodity" required type="text" id="event_commodity" tabindex="1" list="commodities"  value="<?=$event_commodity?>" />
							<!--<datalist id="commodities">-->
								<option></option>
								<? foreign_relation('event_commodity','id','event_commodity',$event_commodity,'1 and status ="1"');?>
							<!--</datalist>-->
							</select>
						</td>
					</tr>
...
<tr class="bgc-info" >
	...
	<th scope="col" scope="col">Event Sub Commodity</th>
	<th scope="col" scope="col">Event Commodity</th>
...
<tbody>
				<?  $data = "SELECT * FROM  event_sub_commodity WHERE 1"; 
					$query = db_query($data);
					while($row =mysqli_fetch_object($query)){
				?>
					<tr>
						<td><?=$row->id;?></td>
						<td  style="text-align:left;"><?=$row->event_sub_commodity;?></td>
						<td  style="text-align:left;"><? if($row->event_commodity) echo find_a_field('event_commodity','event_commodity','id='.$row->event_commodity);?></td>
