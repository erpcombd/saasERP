...
require_once('../../../controllers/mailer/phpmailwithCC.php');
$current_page = "Events";
...
if($_GET['rfx_stage']=='Auction' || $_GET['rfx_stage']=='Forward Auction'){
	$_POST['when_unseal'] = 'after_event_ends';
}else{
...
$_POST['rfq_version'] = $max_rfq_no.'-Round.0'.'-REQ_NO-'.$_GET['req'];
$_POST['req_no']=$_GET['req'];
...
if(isset($_POST['unseal'])){
 ...
 $sql = 'select a.id,u.email,u.fname,a.action,a.is_master,m.event_name,m.master_rfq_no,m.rfq_version from rfq_evaluation_team a, user_activity_management u,rfq_master m where a.user_id=u.user_id and a.rfq_no="'.$_SESSION[$unique].'" and a.rfq_no=m.rfq_no';
 $qry = db_query($sql);
 $teamsinfo = [];
...
$subject = 'Event Unsealed'.$teamsinfo[0]->event_name.'#'.$teamsinfo[0]->rfq_version;
$body .= '<p><strong>Dear Concerned:</strong></p><p><strong>The event is now unsealed; please proceed with your respective responsibilities</strong></p>';
$body .= '<br>';

$body .= 'Event      : '.$teamsinfo[0]->event_name.'-'.$_SESSION[$unique];
$body .= '<br>';
$body .= '<br>';
$body .= '<br>';
$body .= '<br>';         
...
$_POST['rfq_no'] = $_SESSION[$unique];
$_POST['field_name'] = 'Status';
$_POST['field_value'] = 'Event Holded. Remarks : '.$_POST['event_hold_remarks'];
$_POST['entry_at'] = $now;
$_POST['entry_by'] = $_SESSION['user']['id'];
...
$_POST['rfq_no'] = $_SESSION[$unique];
$_POST['field_name'] = 'Status';
$_POST['field_value'] = 'Event UnHolded';
$_POST['entry_at'] = $now;
$_POST['entry_by'] = $_SESSION['user']['id'];
...
$_SESSION['roundingMessage'] = 'Edit mode is on now';
...
?>
<input type="hidden" id="hidden_rfq_number" name="hidden_rfq_number" value="<?=$_SESSION[$unique]?>">
...
<div class="row p-0 pb-2">
	...
	<? if($_SESSION['user_role']=='Owner'){?>
		...
		<? if($rfq_data->status == 'MANUAL'){?>
					...
					<? } elseif($rfq_data->status == 'HOLD'){ ?>
							<button type="submit" name="unhold" id="unhold" class="btn1 btn1-bg-update" style="width:20% !important;">UnHold</button>
					...
					<button type="button" name="event_completed" id="event_completed" class="btn1 btn1-bg-update" data-toggle="modal" data-target="#myModal" style="width:20% !important;">Event Completed</button>
						...
						<div class="modal-dialog modal-sm">
						  <div class="modal-content">
							<div class="modal-header">
							  ...
							  <h4 class="modal-title">Complete Modal</h4>
							</div>
							<div class="modal-body">
							  <form action="">
								<div class="form-group">
								  <label for="email" class="req-input">Approve No:</label>
								  <input type="text" class="form-control" required id="approve_no"  name="approve_no">
								</div>
								<div class="form-group">
								  <label for="pwd" class="req-input">Approve Date of PA:</label>
								  <input type="date" class="form-control" required id="approve_date_of_pa" name="approve_date_of_pa">
								</div>
								
								<div class="form-group">
								  <label for="email" class="req-input">PA Amount:</label>
								  <input type="text" class="form-control" required id="pa_amt"  name="pa_amt">
								</div>
								<div class="form-group">
								  <label for="pwd" class="req-input">Base Amount:</label>
								  <input type="text" class="form-control" required id="base_amt" name="base_amt">
								</div>
								
								<button type="submit" name="eventComplete" class="btn btn-danger">Submit</button>
							  </form>
							</div>
						  </div>
						</div>
					...
					<button type="button" name="event_hold" id="event_hold" onclick="hold_event_confirmation()" class="btn1 btn1-bg-warning" style="width:20% !important;">Hold</button>
	...
	<script>
	 ...
	 <script>
	  ...
	  function hold_event_confirmation(){
        var userInput = prompt("Are you sure you want to hold this event? If yes, please provide a reason:");
			if (userInput !== null && userInput.trim() !== "") {
			  console.log(userInput.trim());
				document.getElementById('event_hold_remarks').value = userInput.trim();
				document.getElementById('action_form').submit();
	  ...
	  function complete_event_confirmation(){
        var userInput = prompt("Are you sure you want to hold this event? If yes, please provide a reason:");
			if (userInput !== null && userInput.trim() !== "") {
        ...
        // Create labels
        var labels = {
            'approve_no': 'Approval Number:',
            'approve_date_of_pa': 'Approval Date of PA:',
            'pa_amt': 'PA Amount:',
            'base_amt': 'Base Amount:'
        };
        
        // Add labels to each input field if they don't already have labels
	...
	<input type="hidden" name="event_hold_remarks" id="event_hold_remarks" value="" />
...
<ul class="nav nav-tabs" id="myTab" role="tablist">
 ...
 <?php if($rfx_stage=='Forward Auction' || $rfx_stage=='Auction'){ ?> 
  ...
  <li class="nav-item">
    <a class="nav-link" id="reverse-tab" data-toggle="tab" href="#tab7" role="tab" aria-controls="reverse" aria-selected="false" onclick="settingF('tab7');"> Auction Response</a>
  </li>
  
  <li class="nav-item">
    <a class="nav-link" id="graph-tab" data-toggle="tab" href="#tab8" role="tab" aria-controls="reverse" aria-selected="false" onclick="settingF('tab8');">Auction Graph View</a>
  </li>
...
<div class="tab-content" id="myTabContent">
 ...
 <? include_once('timeline_tab.php');?>
  ...
  <?
  include_once('reverse_tab.php');
  ?> 

       
<?
  include_once('graph_view_tab.php');
  ?> 
...
function required_field_check_old(){
  ...
  ?>
	...
	var final_request_recieved_date = document.getElementById("final_request_recieved_date").value;
...
if(sendNotification=='Yes'){
  if (immediate_event_shoot) {
	...
	}else if(final_request_recieved_date==''){
    alert('Please add final request recieved date')
  }else{
  ...
  var hidden_rfq_data = document.getElementById('hidden_rfq_number').value;
   ...
   xhr.onreadystatechange = function() {
       if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        xhrs.open('POST', 'sub_comodity_ajax.php', true);
        xhrs.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        xhrs.send('commodity=' + rfq_no + '&type=commodity');
  ...
  var hidden_rfq_data = document.getElementById('hidden_rfq_number').value;
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          var xhrs = new XMLHttpRequest();
          xhrs.open('POST', 'event_team_ajax.php', true);
          xhrs.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
          xhrs.send('user_id=' + user_id + '&level=' + level);
          xhrs.onload = function() {
                        if (xhrs.status == 200) {
                  var res = JSON.parse(xhrs.responseText);
  ...
  var hidden_rfq_data = document.getElementById('hidden_rfq_number').value;
   ...
   xhr.onreadystatechange = function() {
       if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        var xhrs = new XMLHttpRequest();
        xhrs.open('POST', 'event_team_cancel_ajax.php', true);
        xhrs.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        xhrs.send('team_id=' + id);
        xhrs.onload = function() {
                      if (xhrs.status == 200) {
                var res = JSON.parse(xhrs.responseText);
                        document.getElementById('team').innerHTML = res['teamList'];
  ...
  var hidden_rfq_data = document.getElementById('hidden_rfq_number').value;
   ...
   xhr.onreadystatechange = function() {
       if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            var historic_value = document.getElementById('historic_value').value;
            var ceiling_value = document.getElementById('ceiling_value').value;

            var second_part = item_info +"|"+ unit_name +"|"+ qty +"|"+ price +"|"+ currency +"|"+historic_value+"|"+ceiling_value;
              ...
              if(item_info != '' && qty >0 ){
                ...
                document.getElementById('historic_value').value='0';
                document.getElementById('ceiling_value').value='0';
              }else{
...
<script>
     ...
     var myLink = document.getElementById('reverse-tab');
     
     }else if(tab=='tab8'){
      var myLink = document.getElementById('graph-tab');
     }
     ...
     document.getElementById('tab7').className="tab-pane fade";
     document.getElementById('tab8').className="tab-pane fade";
  ...
  var hidden_rfq_data = document.getElementById('hidden_rfq_number').value;
   ...
   xhr.send("hidden_rfq_data=" + encodeURIComponent(hidden_rfq_data));
    ...
    xhr.onreadystatechange = function() {
      ...
      xhrs.open('POST', 'visibility_save_ajax.php', true);
      xhrs.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');

      xhrs.send('content_group=' + content_group);
