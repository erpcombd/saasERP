	 			...
	 			<div class="allrowshow">
					<div class="pl-3 row">
							<div class="col-sm-16 col-md-6 col-lg-6">
								...
								<a href="<?=SERVER_CORE?>./utilities/api_upload_attachment_show.php?name=<?php echo $res->new_name;?>&folder=sourcing_terms_condition&original_name=<?=$res->original_name?>" target="_blank" rel="noopener" style="font-family: Helvetica,Arial,sans-serif !important; color:#0094e4 !important"><?=$res->original_name?></a>
							</div>
		   ...
		   ?>
				...
				<div class="col-sm-10 col-md-10 col-lg-10 pb-1">
					<div class="rounded p-2" style="background-color: #f7f7f7 !important; border: 1px solid #e6e6e6!important; ">
						<a href="<?=SERVER_CORE?>./utilities/api_upload_attachment_show.php?name=<?php echo $res->new_name;?>&folder=sourcing_terms_buery_attahment&original_name=<?=$res->original_name?>" 
...
<h1 class="h1 m-0 p-0 pt-4 pl-3"><em class="fa-duotone fa-calendar-days"></em> Timeline </h1>
  ...
  <div class="tab-pane fade" id="tab2" role="tabpanel" aria-labelledby="timeline-tab" style="display: none;">
	...
	<div class="col-12 pt-4 pb-4">		
  	...
  	<table class="table1  table-striped table-bordered table-hover table-sm">
					...
					$sql = '
					SELECT t.*, m.*
					FROM rfq_vendor_terms_condition t
					INNER JOIN rfq_master m ON m.rfq_no = t.rfq_no
  ...
  <div class="row m-0 p-0 pt-4">
	...
	<?
	 ...
	 $sql_rfq = 'select * from rfq_vendor_response where vendor_id='.$vendor.' and rfq_no='.$rfq->rfq_no.' and id='.$_GET['response_id'];
	...
	<div class="col-2"><a href="download_all_files.php?rfq_no=<?=$_SESSION['rfq_no']?>&vendor_id=<?=$vendor?>&response_id=<?=$_GET['response_id']?>" target="_blank">Download all files</a></div>
  	...
  	<div class="col-12 pt-4 pb-4">
	   <h1 class="h1 m-0 p-0 pt-4 pl-3" style="font-size: 30px !important;"><em class="fa-solid fa-paperclip" style="color: #eb7100; font-size: 30px !important;"></em> Attachments </h1>
		...
		<div class="row m-3 ">
				...
				<div class="col-sm-6 col-md-6 col-lg-6 p-3" style="background-color:#268ecd;" ><p class="m-0 text-white  bold">Response</p>
				</div>
		</div>
...
<?php
	  ...
	  $general_check = find_a_field('rfq_section_evaluation_team','action','user_id="'.$_SESSION['user']['id'].'" and rfq_no="'.$_SESSION['rfq_no'].'" and action like "'.$res->section_type.'" ');
	  if($general_check!='' || $_SESSION['user_role']=='Owner'){
...
<form >
	...
	?>	
				...
				<div class="col-sm-10 col-md-10 col-lg-10 pb-1"><div class="rounded p-2" style="background-color: #f7f7f7 !important; border: 1px solid #e6e6e6!important; "><a href="<?=SERVER_CORE?>./utilities/api_upload_attachment_show.php?name=<?php echo $res2->new_name;?>&folder=doc_section&original_name=<?=$res2->original_name?>" target="_blank" rel="noopener">
	...
	<? } ?>			
			...
			<div class="attachment-toggle"> 
						...
						<div class="fileuploadcontainer" >
							...
							<div class="attachmentshowcontainer container row m-0">
		  <?php
	 $sql1 = 'select * from rfq_documents_information where tr_from like "sourcing_attachment_response'.$res->id.'" and rfq_no="'.$_SESSION['rfq_no'].'" and entry_by = "'.$vendor.'" and section_id='.$_GET['response_id'];
		  $qry1 = db_query($sql1);
		  ...
		  ?>
								<div class="col-sm-10 col-md-10 col-lg-10 pb-1">
									...
									<a href="<?=SERVER_CORE?>./utilities/api_upload_attachment_show.php?name=<?=$res1->new_name?>&folder=<?=$res1->tr_from?>&original_name=<?=$res1->original_name?>" target="_blank" rel="noopener">
		...
		<? } ?>						
							...
							<div class="attachmentUrlshowcontainer container row m-0">
							<?php
						  $sql1 = 'select * from rfq_documents_url_information where tr_from like "sourcing_attachment_response_url'.$res->id.'" and rfq_no="'.$_SESSION['rfq_no'].'" and entry_by = "'.$entry_by.'" and section_id='.$_GET['response_id'];
						  $qry1 = db_query($sql1);
						  while($res1 = mysqli_fetch_object($qry1)){
					   ?> 
					   <div class="col-sm-8 col-md-8 col-lg-8 pb-1">
					   ...
					   <? } ?>
							...
							<div class="attachmentTextshowcontainer container row m-0">
							 <?php
			  $sql1 = 'select * from rfq_documents_url_information where tr_from like "sourcing_attachment_response_text'.$res->id.'" and rfq_no="'.$_SESSION['rfq_no'].'" and entry_by = "'.$entry_by.'" and section_id='.$_GET['response_id'];
			  $qry1 = db_query($sql1);
			  while($res1 = mysqli_fetch_object($qry1)){
	...
	<?php
	$form_check = find_a_field('rfq_section_evaluation_team','action','user_id="'.$_SESSION['user']['id'].'" and rfq_no="'.$_SESSION['rfq_no'].'" and action="Form"');
	if($form_check!='' || $_SESSION['user_role']=='Owner'){

		 $sql = 'select * from rfq_form_response where   rfq_no="'.$rfq->rfq_no.'" and vendor_id = '.$vendor.' and section_id='.$_GET['response_id'];
		  $qry = db_query($sql);
		  while($res = mysqli_fetch_object($qry)){
...
<form >
	...
	<?php
		...
		<? } ?>		
			...
			<div class="attachment-toggle">  
						...
						<div class="fileuploadcontainer" >
							...
							<div class="attachmentshowcontainer container row m-0">
		  <?php
		  $sql1 = 'select * from rfq_documents_information where tr_from like "details_item_response" and entry_by="'.$entry_by.'" and rfq_no="'.$_SESSION['rfq_no'].'" and section_id='.$_GET['response_id'];
		  $qry1 = db_query($sql1);
		  while($res1 = mysqli_fetch_object($qry1)){
		...
		<? } ?>						
							...
							<div class="attachmentUrlshowcontainer container row m-0">
							<?php
						  $sql1 = 'select * from rfq_documents_url_information where tr_from like "details_item_response_url" and rfq_no="'.$_SESSION['rfq_no'].'" and entry_by = "'.$entry_by.'" and section_id='.$_GET['response_id'];
						  $qry1 = db_query($sql1);
						  while($res1 = mysqli_fetch_object($qry1)){
					   ?> 
					   <div class="col-sm-8 col-md-8 col-lg-8 pb-1">
					   ...
					   <? } ?>
							...
							<div class="attachmentTextshowcontainer container row m-0">
							 <?php
			  $sql1 = 'select * from rfq_documents_url_information where tr_from like "details_item_response_text" and rfq_no="'.$_SESSION['rfq_no'].'" and entry_by = "'.$entry_by.'" and section_id='.$_GET['response_id'];
			  $qry1 = db_query($sql1);
			  while($res1 = mysqli_fetch_object($qry1)){
		...
		<div class="pt-1">
  	    ...
  	    <table class="table1  table-striped table-bordered table-hover table-sm" id="ExportTable">
					...
					<?php
						  ...
						  $itemResponse = find_all_field('rfq_vendor_item_response','*','rfq_no='.$rfq->rfq_no.' and item_id='.$res->item_id.' and vendor_id='.$vendor.' and section_id='.$_GET['response_id']);
						 
					?> 
					    <tr>
							<td><?=$sl++;?></td>
							<td><?=find_a_field('vendor','vendor_name','vendor_id='.$vendor);?></td>
							<td><?=$res->item_name?></td>
							<td><?=$res->expected_qty?></td>
							<td><?=$res->unit_name?></td>                            
							<td><?=$itemResponse->unit_price?></td>
							<td><?=$res->currency?></td>
							<td><?=($res->expected_qty*$itemResponse->unit_price);?></td>
                        </tr>	
					<? } ?>							
					</tbody>
                </table>
