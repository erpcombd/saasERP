...
<div class="row p-0 pb-5">
	...
	<h1 class="container" style=" font-size: 30px !important; ">Vendor Evaluation - Event #<?=$_SESSION[$unique]?><span style="font-size:11px;">(Evaluation)</span></h1>
	...
	<div class="col-sm-3 col-lg-3 col-md-3 col-3">
			...
			<?
		 $sql = 'select * from rfq_evaluation_section_vendor where rfq_no="'.$rfq_no.'" and vendor_id="'.$_GET['vendor_id'].'" and rfq_no="'.$_SESSION['rfq_no'].'"';
		 $qry = db_query($sql);
    ...
    <legend class="scheduler-border" style="font-size: 16px !important;">&nbsp;<?=$doc->section_name.'-'.$doc->section_percent?>%</legend>
			...
			<?
		 $sql2 = 'select * from rfq_evaluation_section_child_vendor where rfq_no="'.$rfq_no.'" and section_id="'.$doc->id.'"';
		 $qry2 = db_query($sql2);
		...
		?>
           ...
           <tr>
			...
			<? } ?>
			<div  ><strong>Total Marking : <?=$total_mark?>%</strong></div>
			</div>
			</form>
			</div>
