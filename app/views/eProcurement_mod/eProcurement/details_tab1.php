...
<?
$general_check = find_a_field('rfq_section_evaluation_team','action','user_id="'.$_SESSION['user']['id'].'" and rfq_no="'.$_SESSION['rfq_no'].'" and action="General"');
  ...
  <div class="modal-dialog" role="document" style="max-width: 650px;">
    <div class="modal-content">
		...
		<div class="row m-0 p-0 pt-4">
			...
			<div class="col-6">
				...
				<textarea id="gterms" name="gterms" rows="2" cols="5" placeholder="Instruction.."></textarea>
				  ...
				  <input type="checkbox" id="att_response_general" name="att_response_general"  <?php if($att_response_general=='1') {echo 'checked';}else{echo 'checked';}?> value="1">
					<label for="sa2">Allow Supplier to response with attachment</label><br>
					
					<input type="checkbox" id="is_required_general" name="is_required_general" <?php if($is_required_general=='1') {echo 'checked';}else{echo 'checked';}?> value="1">
      ...
      <div class="modal-footer justify-content-center">
	  			<? if($_SESSION['master_status']=='MANUAL'){?>
				 <button type="button" name="att_details" class="btn1 btn1-bg-update" id="uploadButton" onclick="event_att_insert(document.getElementById('gsection_name').value,document.getElementById('gterms').value,document.getElementById('gsection_type').value,document.getElementById('att_response_general').value,document.getElementById('is_required_general').value,'details_att_general')" data-dismiss="modal" style="display: none;">Add Attachment</button>
				 <? } ?>
...
<!--general section Modal Edit Start-->
<div class="modal fade" id="addattachmentsEdit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document" style="max-width: 650px;">
    <div class="modal-content">
      <div class="modal-header">
	  ...
	  <form method="post" id="att_edit_section_individual" enctype="multipart/form-data">
      <div class="modal-body">
		<!--Attachments Modal body Start-->
		...
		<div class="row m-0 p-0 pt-4">
			...
			<div class="col-6">			
			<div class="pl-3">
			<input type="hidden" class="section_type" name="doc_id_individual" id="doc_id_individual" value="general">
			<input type="hidden" class="section_type" name="details_att_show_div" id="details_att_show_div" value="general">
				<p class="p-0 m-0" style="font-weight:bold"> Attachment Name </p>
				<p class="p-0 m-0" ><input type="text" class="section_name" name="gsection_name" id="gsection_name_edit" value=""><input type="hidden" class="section_type" name="gsection_type" id="gsection_type" value="general"> </p>
			</div>
			
			<div class="pl-3 pt-3">
				<p class="p-0 m-0" style="font-weight:bold"> Attachment</p>
				<p class="p-0 m-0" ><input type="file" name="details_doc_edit[]" class="att_file" id="att_edit_file_general" accept="*/*" multiple> </p>
			...
			<div class="col-6">
				...
				<textarea id="gterms_edit" name="gterms" rows="2" cols="5" placeholder="Instruction.."></textarea>
      ...
      <div class="modal-footer justify-content-center">
	  			<? if($_SESSION['master_status']=='MANUAL'){?>
				 <button type="button" name="att_details" class="btn1 btn1-bg-update" id="uploadButtonedit" onclick="event_att_insert_edit(document.getElementById('gsection_name_edit').value,document.getElementById('gterms_edit').value,document.getElementById('gsection_type').value,document.getElementById('att_response_general').value,document.getElementById('is_required_general').value,document.getElementById('details_att_show_div').value,document.getElementById('doc_id_individual').value)" data-dismiss="modal" style="display: none;">Add Attachment</button>
				 <? } ?>
...
<script>
	        var uploadButton = document.getElementById("uploadButtonedit");
		uploadButton.style.display = "block"
    document.getElementById("att_file_general").addEventListener("change", function() {
        var att_file_general = document.getElementById("att_file_general");
        var uploadButton = document.getElementById("uploadButton");
        if (att_file_general.files.length > 0) {
            uploadButton.style.display = "block";
        } else {
...
<!--Attachments Edit Modal End-->	

<!--general section data view collapsed-->
<div class="accordion" id="accordionExample">
  <div class="card">
  <button class="btn btn-link  p-0" type="button" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
	  ...
	  <span id="details_att_general">
		...
		<div class="row m-0 p-0 pt-4">
			...
			?>
				...
				<p class="p-0 m-0" >
				<a href="../../../controllers/utilities/api_upload_attachment_show.php?name=<?=$att_data->new_name?>&&folder=doc_section&original_name=<?=$att_data->original_name?>" target="_blank" rel="noopener">
												<em class="fa-solid fa-file fa-2xl fs-22" style="color: #d6960a;"></em> 
												<span><?=$att_data->original_name?></span>
											</a>
											<? if($_SESSION['master_status']=='MANUAL'){?>
											<button type="button" style=" display: inline !important;" class="border-0" onclick="event_details_att_cancel_individual('general',<?=$att_data->documents_id?>,'details_att_general',<?=$doc->id?>)">
		...
		<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addattachmentsEdit" onclick="event_details_att_edit_individual('general','details_att_general',<?=$doc->id?>,'<?=$doc->terms?>','<?=$doc->section_name?>')">Edit</button>
		 <? } ?>
		</div>
			</div>
...
<!--general section data end-->		

<? } 

$technical_check = find_a_field('rfq_section_evaluation_team','action','user_id="'.$_SESSION['user']['id'].'" and rfq_no="'.$_SESSION['rfq_no'].'" and action="Technical"');
if($technical_check!='' || $_SESSION['user_role']=='Owner'){
...
<div class="modal fade" id="technicalSection" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document" style="max-width: 650px;">
    <div class="modal-content">
		...
		<div class="row m-0 p-0 pt-4">
			...
			<div class="col-6">
				...
				<textarea id="tterms" name="tterms" rows="2" cols="5" placeholder="Instruction.."></textarea>
				  ...
				  <input type="checkbox" id="att_response_technical" name="att_response_technical" <?php if($att_response_technical=='1') {echo 'checked';}else{echo 'checked';}?> value="1">
					<label for="sa2">Allow Supplier to response with attachment</label><br>
					
					<input type="checkbox" id="is_required_technical" name="is_required_technical" <?php if($is_required_technical=='1') {echo 'checked';}else{echo 'checked';}?> value="1">
					<label for="sa3">Make response required</label><br>
      ...
      <div class="modal-footer justify-content-center">
	  			<? if($_SESSION['master_status']=='MANUAL'){?>
				 <button type="button" name="att_details" class="btn1 btn1-bg-update" id="uploadButton1" onclick="event_att_insert(document.getElementById('tsection_name').value,document.getElementById('tterms').value,document.getElementById('tsection_type').value,document.getElementById('att_response_technical').value,document.getElementById('is_required_technical').value,'details_att_technical')" data-dismiss="modal" style="display: none;">Add Attachment</button>
				 <? } ?>
...
<script>
    document.getElementById("att_file_technical").addEventListener("change", function() {
        var att_file_technical = document.getElementById("att_file_technical");
        var uploadButton1 = document.getElementById("uploadButton1");
        if (att_file_technical.files.length > 0) {
...
<!--Attachments Section View Start collapsed-->
<div class="accordion" id="showTechSection">
  <div class="card">
  <button class="btn btn-link  p-0" type="button" data-toggle="collapse" data-target="#techView" aria-expanded="false" aria-controls="collapseTwo">
    <div class="card-header" id="headingTwo">
       <h1 class="h1 m-0 p-0 pl-3"><em class="fa-solid fa-list"></em> Technical Section</h1>
    </div>
	</button>
    <div id="techView" class="collapse show" aria-labelledby="headingTwo" data-parent="#showTechSection">
      <div class="card-body">
	  <!--Attachments Section View Body strat-->
	  ...
	  <span id="details_att_technical">
		...
		<div class="row m-0 p-0 pt-4">
			...
			?>
				...
				<a href="../../../controllers/utilities/api_upload_attachment_show.php?name=<?=$att_data->new_name?>&&folder=doc_section&original_name=<?=$att_data->original_name?>" target="_blank" rel="noopener">
											...
											<? if($_SESSION['master_status']=='MANUAL'){?>
											<button type="button" style=" display: inline !important;" class="border-0" onclick="event_details_att_cancel_individual('technical',<?=$att_data->documents_id?>,'details_att_technical',<?=$doc->id?>)">
												<em class="fa-solid fa-xmark"></em>
											</button>
											<? } ?>
										</p>
		...
		<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addattachmentsEdit" onclick="event_details_att_edit_individual('technical','details_att_technical',<?=$doc->id?>,'<?=$doc->terms?>','<?=$doc->section_name?>')">Edit</button>
		 <? } ?>
		</div>
			</div>
...
<?
$commercial_check = find_a_field('rfq_section_evaluation_team','action','user_id="'.$_SESSION['user']['id'].'" and rfq_no="'.$_SESSION['rfq_no'].'" and action="Commercial"');
if($commercial_check!='' || $_SESSION['user_role']=='Owner'){
?>
			<? if($_SESSION['master_status']=='MANUAL'){?>
<h1 class="h1 m-0 p-0 pl-3">
			<em class="fa-solid fa-paperclip"></em> 
...
<div class="modal fade" id="addCommercialSection" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document" style="max-width: 650px;">
    <div class="modal-content">
		...
		<div class="row m-0 p-0 pt-4">
			...
			<div class="col-6">
				...
				<textarea id="cterms" name="cterms" rows="2" cols="5" placeholder="Instruction.."></textarea>
				  ...
				  <input type="checkbox" id="att_response_commercial" name="att_response_commercial" <?php if($att_response_commercial=='1') {echo 'checked';}else{echo 'checked';}?> value="1">
					<label for="sa2">Allow Supplier to response with attachment</label><br>
					
					<input type="checkbox" id="is_required_commercial" name="is_required_commercial" <?php if($is_required_commercial=='1') {echo 'checked';}else{echo 'checked';}?> value="1">
					<label for="sa3">Make response required</label><br>
      ...
      <div class="modal-footer justify-content-center">
	  			<? if($_SESSION['master_status']=='MANUAL'){?>
				 <button type="button" name="att_details" class="btn1 btn1-bg-update" id="uploadButton2" onclick="event_att_insert(document.getElementById('csection_name').value,document.getElementById('cterms').value,document.getElementById('csection_type').value,document.getElementById('att_response_commercial').value,document.getElementById('is_required_commercial').value,'details_att_commercial')" data-dismiss="modal" style="display: none;">Add Attachment</button>
				 <? } ?>
...
<script>
    document.getElementById("att_file_commercial").addEventListener("change", function() {
        var att_file_technical = document.getElementById("att_file_commercial");
...
<div class="accordion" id="showItemInfo">
  <div class="card">
  <button class="btn btn-link  p-0" type="button" data-toggle="collapse" data-target="#itemView" aria-expanded="false" aria-controls="collapseTwo">
    <div class="card-header" id="headingTwo">
      <h1 class="h1 m-0 p-0 pl-3"><em class="fa-solid fa-list"></em> Commercial Section</h1>
    </div>
	</button>
    <div id="itemView" class="collapse show" aria-labelledby="headingTwo" data-parent="#showItemInfo">
      <div class="card-body">
	  <!--Attachments Section View Body strat-->
	  ...
	  <span id="details_att_commercial">
		...
		<div class="row m-0 p-0 pt-4">
			...
			?>
				...
				<a href="../../../controllers/utilities/api_upload_attachment_show.php?name=<?=$att_data->new_name?>&&folder=doc_section&original_name=<?=$att_data->original_name?>" target="_blank" rel="noopener">
											...
											<? if($_SESSION['master_status']=='MANUAL'){?>
											<button type="button" style=" display: inline !important;" class="border-0" onclick="event_details_att_cancel_individual('commercial',<?=$att_data->documents_id?>,'details_att_commercial',<?=$doc->id?>)">
												<em class="fa-solid fa-xmark"></em>
											</button>
											<? } ?>
										</p>
		...
		<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addattachmentsEdit" onclick="event_details_att_edit_individual('commercial','details_att_commercial',<?=$doc->id?>,'<?=$doc->terms?>','<?=$doc->section_name?>')">Edit</button>
			...
			<span class=" pl-4 fs-18 bold">Attachments</span> <div class="col-6 attachment-toggle">
						...
						?>
                                      <div class="col-sm-10 col-md-10 col-lg-10 pb-1">
										...
										<a href="../../../controllers/utilities/api_upload_attachment_show.php?name=<?=$item->new_name?>&folder=<?=$item->tr_from?>&original_name=<?=$item->original_name?>" target="_blank" rel="noopener">												<em class="fa-light fa-file fa-2xl fs-22" style="color: #d6960a;"></em> 
  	...
  	<table class="table1  table-striped table-bordered table-hover table-sm">
                    ...
                    <tr class="bgc-info">
						...
						<th scope="col" width="35%" class="req-input">Item Description</th>
                        <th scope="col" width="10%">Quantity</th>
						<th scope="col" width="10%">UOM</th>
						<th scope="col" width="8%">Ceiling amount (per unit)</th>
						<th scope="col" width="10%">Historical amount (per unit)</th>
                    ...
                    <tbody class="tbody1">
					    ...
					    <tr>
							<td>#1</td>
							<td><input type="text" name="item_desc" id="item_desc" list="item" onclick="base_currency_list(<?=$_SESSION[$unique]?>);"/>
							<datalist id="item">
							 <? foreign_relation('item_info','item_name','""',$item_desc,'1')?>
							</datalist></td>
							...
							<datalist id="unit_list">
							 <? foreign_relation('unit_management','unit_name','""',$expected_qty,'1')?>
							</datalist>
							</td>
							<td><input type="text" name="ceiling_value" id="ceiling_value" style="text-align:right" value="0" /></td>
							<td><input type="text" name="historic_value" id="historic_value" style="text-align:right" value="0" /></td>
							...
							<?php 
								$sql = "select c.id, c.currency from currency_info c, rfq_multiple_currency m where c.id=m.currency_id and rfq_no=".$_SESSION['rfq_no'];
								$qry = db_query($sql);
								$cnt = 0;
								while($res = mysqli_fetch_object($qry)){
								$cnt++;
							...
							<td><input type="text" name="need_by" id="need_by" readonly /></td>
		...
		?>
						<tr>
							...
							<td style="text-align:right;"><?=$doc->need_by?>
							&nbsp;
										<? if($_SESSION['master_status']=='MANUAL'){?>
							<button type="button" name="add_event_team" 
...
<div class="accordion" id="showFormInfo">
  <div class="card">
  <button class="btn btn-link  p-0" type="button" data-toggle="collapse" data-target="#formView" aria-expanded="false" aria-controls="collapseTwo">
    <div class="card-header" id="headingTwo">
       <h1 class="h1 m-0 p-0 pl-3"><em class="fa-solid fa-list"></em> Form Section</h1>
    </div>
	</button>
    <div id="formView" class="collapse show" aria-labelledby="headingTwo" data-parent="#showFormInfo">
      <div class="card-body">
	  <!--Attachments Section View Body strat-->
