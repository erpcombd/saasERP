...
if($rfq_no>0 && $id>0){
$del = 'delete from rfq_evaluation_team where id="'.$id.'"';
db_query($del);
...
<?php
		 $sql = 'select a.id,u.fname,a.action from rfq_evaluation_team a, user_activity_management u where a.user_id=u.user_id and a.rfq_no="'.$rfq_no.'"';
		 $qry = db_query($sql);
