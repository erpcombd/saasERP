...
$title='Import Receive Serial Wise';
...
$sql_all = 'select pr.*,s.ledger_id_2 as item_ledger,i.item_name,sum(pr.amount) as invoice_amt,pr.item_id from warehouse_other_receive_detail pr, item_info i, item_sub_group s where i.sub_group_id=s.sub_group_id and pr.item_id=i.item_id and pr.or_no="'.$or_no.'" and pr.journal_status="Pending" group by pr.item_id';
$qrr_all = mysql_query($sql_all);
while($pr_data = mysql_fetch_object($qrr_all)){
$jv_date = $pr_data->or_date;
...
header("Location:unfinished_import.php");
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
 ...
 <span id="s_check_<?=$row->id?>">
      ...
      <tbody>
          ...
          <? 
		     $res='select p.import_no,i.item_name,i.unit_name,p.serial_no as item_serial_no,i.unit_name as action from lc_import p,item_info i where i.item_id=p.item_id and p.import_no="'.$_REQUEST['or_no'].'"';
			 $qr = mysql_query($res);
		  while($row=mysql_fetch_object($qr)){$bg++?>
