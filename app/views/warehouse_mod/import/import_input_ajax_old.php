...
$config_ledger = find_all_field('config_group_class','sales_ledger',"group_for=".$_SESSION['user']['group']);
...

		{
...

			{
				...
				$serial_no = $_POST['serial_no_'.$data->id];
				...
				if(($serial_check=='' or $serial_check==0) && $po_qty>$rcv_qty){
				$qty=1;
				...
				$lc_insert = 'insert into lc_import(`import_no`,`item_id`,`serial_no`,`rate`,`amount`,`import_date`,`entry_by`,`import_rcv_date`) value("'.$or_no.'","'.$item_id.'","'.$serial_no.'","'.$rate.'","'.$amount.'","'.$rec_date.'","'.$_SESSION['user']['id'].'","'.$rec_date.'")';
				db_query($lc_insert);
...
$res='select p.import_no,p.import_no,i.item_name,i.unit_name,p.serial_no as item_serial_no,i.unit_name as action from lc_import p,item_info i where i.item_id=p.item_id and p.import_no="'.$or_no.'"';
