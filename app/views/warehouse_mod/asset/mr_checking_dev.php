...

{
		...
		$query = db_query($sql);
		$_POST['requisition_'.$data->id];
		while($data=mysqli_fetch_object($query)){

			if(($_POST['requisition_'.$data->id]>0)){
			
				$qty=$_POST['requisition_'.$data->id];
				
				echo $inv ='update requisition_order SET qty = "'.$qty.'"  where id='.$data->id;	
					db_query($inv);	
			}
		}
...
if(document.getElementById("warehouse_id").value>0)
 ...
 <form action="" method="post" name="codz" id="codz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}" enctype="multipart/form-data">
		<!--<form action="" method="post" name="codz" id="codz">-->
			...
			$res='select a.id,b.item_name as item_name,a.specification,a.qoh as stock_qty,a.last_p_qty as last_pur_qty,a.last_p_date as last_pur_date,a.qty,"x" from requisition_order a,item_info b where b.item_id=a.item_id and a.req_no='.$req_no;
			...
			?>
					...
					<tr>
						...
						<td><input name="requisition_<?=$row->id?>" type="text" id="requisition_<?=$row->id?>"  class="text-center" value="<?=$row->qty?>"/></td>
