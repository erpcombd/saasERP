    ...
    <form action="" method="post" name="codz" id="codz">
		...
		<? 
					...
					if($_SESSION['user']['level']==5){
					 $res='select  	a.req_no,a.req_no, a.req_date, a.req_for, b.warehouse_name as warehouse, a.req_note as note, a.need_by, c.fname as entry_by, a.entry_at,a.status from requisition_master a,warehouse b,user_activity_management c where a.status !="MANUAL" and a.warehouse_id=b.warehouse_id and a.entry_by=c.user_id '.$con.' order by a.req_no DESC';
					 }
					 else{
					 
					 	$res='select  	a.req_no,a.req_no, a.req_date, a.req_for, b.warehouse_name as warehouse, a.req_note as note, a.need_by, c.fname as entry_by, a.entry_at,a.status from requisition_master a,warehouse b,user_activity_management c where a.status !="MANUAL" and  a.warehouse_id=b.warehouse_id and a.entry_by=c.user_id 
and a.entry_by="'.$_SESSION['user']['id'].'"'.$con.' order by a.req_no DESC';
		...
		?>
					...
					?>
                        ...
                        <tr>
							...
							<td><?= $row->status?></td>
                            <td>
							<?php /*?><button type="button" name="show" id="show" value="VIEW" onclick="custom(<?= $row->req_no?>)" class="btn2 btn1-bg-submit">
							<i class="fa-solid fa-eye"></i>
							</button><?php */?>
							
							<button type="button" name="show" id="show" value="VIEW"
  onclick="custom('<?=url_encode($row->req_no)?>','<?=url_encode($c_id)?>')"
  class="btn2 btn1-bg-submit">
  <i class="fa-solid fa-eye"></i>
</button>
