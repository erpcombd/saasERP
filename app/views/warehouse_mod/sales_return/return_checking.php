...
if($_REQUEST['old_sr_no']>0)

$$unique_master=$_REQUEST['old_sr_no'];

elseif(isset($_GET['del']))

{$$unique_master=find_a_field('sale_return_details','sr_no','id='.$_GET['del']); $del = $_GET['del'];}
...

{
		...
		$_POST['sr_no'] = find_a_field($table_master,'max(sr_no)','0')+0;
...
(`sr_no`,  `sr_date`, dealer_code,  via_customer, `item_id`, depot_id,`unit_price`, `bag_unit`, dist_unit, total_unit, `total_amt`,   entry_by, entry_at, remarks) VALUES
('".$sr_no."',  '".$_POST['sr_date']."', '".$_POST['dealer_code']."',  '".$_POST['via_customer']."',  '".$item_gunny."', 
...
$_REQUEST['total_amt'] = $poly_price*$_REQUEST['bag_unit'];
 ...
 $gunny_sql = "INSERT INTO `sale_return_details` 
(`sr_no`,  `sr_date`, dealer_code,  via_customer,  `item_id`, depot_id, `unit_price`, `bag_unit`, dist_unit, total_unit, `total_amt`,   entry_by, entry_at, remarks) VALUES
('".$sr_no."',  '".$_POST['sr_date']."', '".$_POST['dealer_code']."',  '".$_POST['via_customer']."',  '".$item_poly."',  
...
// auto_complete_from_db('item_info','item_name','concat(item_name,"#>",finish_goods_code)','1','vai_cutomer');
                    ...
                    <div class="container n-form2">
                        <div class="form-group row m-0 pb-1">
                            ...
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
                                <input   name="sr_no" type="text" id="sr_no" value="<? if($$unique_master>0) echo $$unique_master; else echo (find_a_field($table_master,'max('.$unique_master.')','1')+1);?>" readonly/>

<input name="group_for" type="hidden" id="group_for" required readonly=""  value="<?=$_SESSION['user']['group']?>" tabindex="105" />
 <input name="vat" type="hidden"  readonly="" id="vat"  value="15" tabindex="101" />
                            </div>
                        </div>
                        ...
                        <div class="form-group row m-0 pb-1">
                            ...
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
                                <input   name="sr_date" type="hidden" id="sr_date" value="<?=$sr_date;?>"  required/>

								<input   name="do_date2" type="text" id="do_date2" value="<?=$sr_date;?>" readonly="" required/>
                            </div>
                        </div>
                    ...
                    <div class="container n-form2">
                        <div class="form-group row m-0 pb-1">
                            ...
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
								...
								<? if ($dealer_code>0) {?>
								<select  id="dealer_code" name="dealer_code"   required>
                        ...
                        <div class="form-group row m-0 pb-1">
                            ...
                            <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
                                <select name="sales_type" required id="sales_type"   >
	...
	<form action="<?=$page?>" method="post" name="codz2" id="codz2">
		...
		<? 
		  ...
		  $res='select s.*,i.item_name,i.unit_name from sale_return_details s left join item_info i on i.item_id=s.item_id where s.sr_no="'.$$unique_master.'"';
    ...
    <form action="unapproved_return.php" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
        <div class="container-fluid p-0 ">
            ...
            <div class="n-form-btn-class">
			<input name="return"  type="submit" class="btn1 btn1-bg-help" value="RETURN TO USER" onclick="return_function()"  />
	   <input  name="sr_no" type="hidden" id="sr_no" value="<?=$sr_no?>"/><input  name="dealer_code" type="hidden" id="dealer_code" value="<?=$dealer_code?>"/><input  name="sr_date" type="hidden" id="sr_date" value="<?=$sr_date?>"/><input type="hidden" name="return_remarks" id="return_remarks">

			<input name="cancel"  type="submit" class="btn1 btn1-bg-cancel" value="CANCEL" />
			
			<input type="button" class="btn1 btn1-bg-update" value="CLOSE" onclick="window.location.href='unapproved_return.php'" />
			
			<input name="confirm" type="submit" class="btn1 btn1-bg-submit" value="CONFIRM SALES RETURN"  />
