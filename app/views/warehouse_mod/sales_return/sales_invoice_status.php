...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and m.sr_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
    ...
    $res="select m.sr_no, m.sr_no, m.sr_date, m.dealer_code, m.do_type, m.depot_id,  d.dealer_name_e, m.entry_by, m.status from 
sale_return_master m, sale_return_details c,  dealer_info d, user_group u 
where 
  m.group_for=u.id  and m.dealer_code=d.dealer_code and m.status!='MANUAL' and m.sr_no=c.sr_no ".$group_for_con.$con.$dealer_con."  group by c.sr_no order by  m.sr_date DESC ";
$query = db_query($res);
		...
		?>
					...
					?>
                        ...
                        <tr>
                            ...
                            <td <?= $row->sr_no?> ><?= find_a_field('sales_return_type','sales_return_type','id='.$row->do_type);?></td>
							...
							<td><?= $row->status?></td>
                            <td>
			<button type="button" name="submitit" id="submitit" onclick="custom('<?=url_encode($row->sr_no);?>','<?=url_encode($c_id);?>')" class="btn2 btn1-bg-submit">
...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and m.do_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
    ...
    $res="select m.sr_no, m.sr_no, m.do_date, m.dealer_code, m.sales_type, m.depot_id,  d.dealer_name_e, m.entry_by, m.status from 
sale_return_master m, sale_return_details c,  dealer_info d, user_group u 
where 
  m.group_for=u.id  and m.dealer_code=d.dealer_code and m.sr_no=c.sr_no ".$group_for_con.$con.$dealer_con."  group by c.sr_no order by  m.do_date, m.sr_no ";
$query = db_query($res);
...
<td onClick="custom(<?=$data->sr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->sr_no;?></td>
<td onClick="custom(<?=$data->sr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?php echo date('d-m-Y',strtotime($data->do_date));?></td>
<td onClick="custom(<?=$data->sr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->dealer_name_e;?></td>
<td onClick="custom(<?=$data->sr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('sales_return_type','sales_return_type','id='.$data->sales_type);?></td>
<td onClick="custom(<?=$data->sr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('warehouse','warehouse_name','warehouse_id='.$data->depot_id);?></td>
<td onClick="custom(<?=$data->sr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;
  <?=find_a_field('user_activity_management','fname','user_id='.$data->entry_by);?></td>
  
  <td onclick="custom(<?=$data->sr_no;?>);" <?=(++$z%2)?'':'class="alt"';?>><?=$data->status?></td>
</tr>
<?
