...
$do_master = find_all_field('sale_return_master','sr_no','sr_no='.$or_no);

 $do_sql = "select a.*, m.sr_no,m.depot_id, m.sr_date from sale_return_master m, sale_return_details a where m.sr_no=a.sr_no and a.sr_no='".$or_no."' ";

$doSql=db_query($do_sql);
while($data=mysqli_fetch_object($doSql)){

journal_item_control($data->item_id,$_SESSION['user']['depot'],$data->sr_date,$data->total_unit,0,'BulkSales Return',$data->id, $data->unit_price,'',$or_no);

}
auto_insert_bulksales_return_secoundary_toriqul($or_no);

db_query('update sale_return_master set status="CHECKED" where sr_no="'.$or_no.'"');
$tr_type="Complete";
}
...

{
	...
	window.open('<?=$target_url?>?old_sr_no='+theUrl+'','_self');
...
if($_POST['fdate']!=''&&$_POST['tdate']!='') $con .= ' and m.sr_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
     ...
     $res="select m.sr_no, m.do_no,m.sr_date, m.do_no, m.dealer_code, m.do_type,  m.depot_id,  d.dealer_name_e, m.entry_by, m.status from 
...
where 
  ...
  m.group_for=u.id  and m.dealer_code=d.dealer_code and m.sr_no=c.sr_no and m.status='UNCHECKED' ".$group_for_con.$con.$dealer_con."  group by c.sr_no order by  m.sr_no ";
		...
		?>
					...
					?>
                        ...
                        <tr>
							...
							<input type="button" name="submitit" id="submitit" value="VIEW" class="btn1 btn1-bg-submit" onclick="custom(<?= $row->sr_no?>)"/ >
