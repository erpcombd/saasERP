    ...
    <label for="req_note" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Carried by :</label>
        ...
        <label for="req_note" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Req. No:</label>
                ...
                <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
						...
						$group_for=find_a_field('master_requisition_master','group_for','req_no='.$req_no);	
						?>

						<input name="req_no" type="text" id="req_no"  value="<?=$req_no?>" readonly required/>
                        <input name="group_for" type="hidden" id="group_for"  value="<?=$group_for?>" readonly required/>
                </div>
						...
						<div class="container n-form2">
						  ...
						  <div class="form-group row m-0 pb-1">
							...
							<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
							  ...
							  <input name="pi_date" type="text" id="pi_date" value="<?=($pi_date>0) ? $pi_date : date('Y-m-d')?>" autocomplete="off" required/>			
...
<input name="manual_req_no" type="text" id="manual_req_no" value="<?=find_a_field('master_requisition_master','manual_req_no','req_no='.$_SESSION['req_no'])?>" tabindex="105" />
         ...
         <label for="req_note" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">From :</label>
                ...
                <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
  <?  $from_warehouse=find_a_field('master_requisition_master','warehouse_id','req_no='.$_SESSION['req_no']) ?>   
  
        <input name="warehouse_from" type="hidden" id="warehouse_from"  value="<?=$from_warehouse?>" />

    <input name="warehouse_from3" type="text" id="warehouse_from3"  value="<?=find_a_field('warehouse','warehouse_name','warehouse_id='.$from_warehouse)?>" readonly required/>
			  ...
			  <div class="form-group row m-0 pb-1">
                ...
                <label for="req_note" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Transit warehouse:</label>

                <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
                  <select name="transit_warehouse"  id="transit_warehouse"  required>
				  <option></option>
                    <? foreign_relation('warehouse','warehouse_id','warehouse_name',$transit_warehouse,'use_type="WH" and warehouse_id!="'.$from_warehouse.'"');?>
                  </select>
                </div>
...
<? 
	 ...
	 $res='select w.id,i.item_name,i.item_id,i.item_description,i.unit_name,w.order_qty,w.qty,w.warehouse_id 
	 from master_requisition_details w,item_info i 
	 where i.item_id=w.item_id and w.req_no='.$_SESSION['req_no'];                      
	...
	while($row=mysqli_fetch_object($rs)){
		...
		?>
		  ...
		  <tr>
                ...
                <td>
				<input  type="text" name="stk<?=$row->id?>" id="stk<?=$row->id?>"
				 value="<?=$stock=find_a_field('journal_item','sum(item_in-item_ex)','item_id="'.$row->item_id.'" and warehouse_id='.$row->warehouse_id); ?>" readonly />
				</td>
   ...
   <input type="number" step="any" onchange="limitCross(<?=$row->id?>)" id="id<?=$row->id?>" name="id<?=$row->id?>" value="<?=($row->order_qty);?>" 
   required readonly=""/>
					
					<? }else{?>
					
	<input type="number" step="any" onchange="limitCross(<?=$row->id?>)" id="id<?=$row->id?>" name="id<?=$row->id?>" value="<?=($stock);?>" required readonly=""/>
