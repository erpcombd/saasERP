...
if(isset($_POST['confirm'])){
		...
		$req_master= find_a_field('master_requisition_master','bom_no','req_no='.$req_no);
		...
		while($row=mysqli_fetch_object($rs)){
			...
			$rate = find_a_field('journal_item','final_price','item_id='.$row->item_id.' and final_price>0 order by id desc');
...
('".$pi_no."', '".$req_no."', '".$row->id."', '".$master->pi_date."', '".$row->item_id."', '".$master->warehouse_from."', '".$master->warehouse_to."', '".$row->order_qty."','".$issue_qty."', '".$rate."', '0', '', 'PENDING')";
...
journal_item_control($row->item_id ,$master->warehouse_from,$master->pi_date,0,$issue_qty,'Issue',$xid,$rate,$master->transit_warehouse,$master->pi_no,'','',$master->group_for,'','','',$req_master);

journal_item_control($row->item_id ,$master->transit_warehouse,$master->pi_date,$issue_qty,'0','Issue',$xid,$rate,$master->warehouse_from,$master->pi_no,'','',$master->group_for,'','','',$req_master);
							...
							?>
										...
										<tr>
											...
											<td>
												...
												<a target="_blank" href="../production_issue_with_transit/production_issue_report.php?v_no=<?=$rs->req_no?>&req_status=<?=$rs->status?>"><span><strong>Complete</strong></span></a>
												...
												<a href="../production_issue_with_transit/production_issue.php?req=<?=$rs->req_no?>"><span class="btn1 btn1-submit-input">Show</span></a>
