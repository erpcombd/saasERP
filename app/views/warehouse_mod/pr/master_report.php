	...
	case 5:
		...
		{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.ji_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
		 ...
		 $sql='select ji_date as GR_Date,a.sr_no as GR_no,pm.po_date,pm.po_no,i.item_name,s.sub_group_name as Category,i.unit_name as unit,a.item_in as `RQ`,a.item_price as rate,((a.item_in+a.item_ex)*a.item_price) as amount,v.vendor_name,a.entry_at,c.fname as User 
		   ...
		   where pm.vendor_id=v.vendor_id and c.user_id=a.entry_by and s.sub_group_id=i.sub_group_id and a.item_id=i.item_id and a.warehouse_id="1" and a.tr_no=pr.id and pr.po_no=pm.po_no '.$date_con.$item_con.$vendor_con.$warehouse_con.$status_con.$item_sub_con.' order by a.id';
	break;
}
}
...
echo $sql="select m.do_no,i.item_name,m.do_date,d.dealer_name_e as dealer_name,
