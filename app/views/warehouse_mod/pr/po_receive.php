...
$q = "INSERT INTO `purchase_receive` (`pr_no`, `po_no`, `order_no`, `rec_no`,`rec_date`, `vendor_id`, `item_id`, `warehouse_id`, `rate`, `qty`, `unit_name`, `amount`, `qc_by`, `truck_no`,`entry_by`, `entry_at`,ch_no, `lot_no`, `garden_id`,  `shed_id`, `quality`, `invoice_no` ) VALUES('".$pr_no."', '".$po_no."', '".$data->id."', '".$rec_no."','".$rec_date."',".$vendor_id.", ".$item_id.",".$warehouse_id.", ".$rate.", '".$qty."', '".$unit_name."',  '".$amount."', '".$qc_by."', '".$truck_no."', '".$_SESSION['user']['id']."', '".$now."', '".$ch_no."', '".$lot_no."', '".$garden_id."', '".$shed_id."', '".$quality."',  '".$invoice_no."' )";
...
journal_item_control($data->item_id ,$warehouse_id,$rec_date,$qty,0,'Purchase',$xid,$rate,'',$pr_no, $quality );
...
?>
        ...
        <tr>
          ...
          <td align="center" bgcolor="#FFFF99"><a target="_blank" href="../pr/chalan_view.php?v_no=<?=$aaa->pr_no?>"><img src="../../images/print.png" width="15" height="15" /></a></td>
          ...
          <tr bgcolor="<?=(($bg%2)==1)?'#FFEAFF':'#DDFFF9'?>">
            ...
            <td><?=$row->lot_no?></td>
              ...
              <td><?=$row->item_name?>
				...
				<input type="hidden" name="lot_no_<?=$row->id?>" id="lot_no_<?=$row->id?>" value="<?=$row->lot_no?>" />
				<input type="hidden" name="garden_id_<?=$row->id?>" id="garden_id_<?=$row->id?>" value="<?=$row->garden_id?>" />
				<input type="hidden" name="shed_id_<?=$row->id?>" id="shed_id_<?=$row->id?>" value="<?=$row->shed_id?>" />
				<input type="hidden" name="quality_<?=$row->id?>" id="quality_<?=$row->id?>" value="<?=$row->quality?>" />
				<input type="hidden" name="invoice_no_<?=$row->id?>" id="invoice_no_<?=$row->id?>" value="<?=$row->invoice_no?>" />				</td>
