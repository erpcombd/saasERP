...
$emp=find_all_field('personnel_basic_info','PBI_ORG','PBI_ID='.$entry_by);


$title='MR checking department head';
...
if(isset($_POST['confirmm']))
{
		...
		// towsif sir call me april-22 for change it PRE-CHECKED to CHECKED
		...
		$_POST['checked_by']=$_SESSION['employee_selected'];
  ...
  <tr>
    ...
    <? $field='warehouse_id'; $table='warehouse';$get_field='warehouse_id';$show_field='warehouse_name';?>
      <div>
		...
		<input name="warehouse_id2" type="hidden" id="warehouse_id2" value="<?=find_a_field('user_group','group_name','id='.$warehouse_id)?>" readonly="" />
      </div>
    </fieldset></td>
    <td>
			<fieldset>
...
<table width="40%" border="1" align="center" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
  ...
  <tr>
    ...
    <td align="left" bgcolor="#CCFF99">&nbsp;&nbsp;<?=find_a_field('hrm_user_access','user_name','emp_id='.$entry_by);?></td>
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                document.getElementById('unit_name').value = data[1];
                        ...
                        <input name="item_for" id="item_for" type="text"  style="width:80px;" />						</td>
                        <td  align="center" bgcolor="#CCCCCC">
						<span id="mr">
...
<br /><br /><br /><br />
<? 
$res='select a.id,concat(b.item_name," :: ",b.item_description) as item_name,a.item_for as remarks,a.qoh as stock_qty,a.last_p_qty as last_pur_qty,a.last_p_date as last_pur_date,a.qty,"x" from requisition_order_stationary a,item_info b where b.item_id=a.item_id and a.req_no='.$req_no;
?>
