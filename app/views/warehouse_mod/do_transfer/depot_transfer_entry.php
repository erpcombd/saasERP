	...
	<? if($$unique_master>0){?>
        ...
        <div class="container-fluid pt-5 p-0 ">
            ...
            <table class="table1  table-striped table-bordered table-hover table-sm">
                ...
                <tr>
					<td><?=$data->do_no;?>
						<input type="hidden" name="req_qty_<?=$data->id?>" id="req_qty_<?=$data->id?>" value="<?=$data->qty;?>"  />
						<input type="hidden" name="req_no_<?=$data->id?>" id="req_no_<?=$data->id?>" value="<?=$data->or_no;?>"  />
						<input type="hidden" name="req_id_<?=$data->id?>" id="req_id_<?=$data->id?>" value="<?=$data->id;?>"  />
						<input type="hidden" name="do_no_<?=$data->id?>" id="do_no_<?=$data->id?>" value="<?=$data->do_no;?>"  />
						<input  name="item_id_<?=$data->id?>" type="hidden" id="item_id_<?=$data->id?>" value="<?=$data->item;?>"/>
					</td>
                    <td><?=$data->item_name;?></td>
					<td><?=$data->unit_name;?></td>
					<td><?=$stock = $data->qty;?></td>
					<td><?=$issued = find_a_field('do_issue_detail','sum(total_unit)','do_no="'.$data->do_no.'" and item_id='.$data->item_id.'');?></td>
					<td><?=$balance = ($stock-$issued);?>
						<input type="hidden" id="balance_<?=$data->id?>" value="<?=$balance; ?>"  />
					</td>
                    <td>
						<input type="hidden" name="unit_price_<?=$data->id?>" id="rate_<?=$data->id?>" value="<?=$data->rate;?>"  />
						<?=$data->rate;?>
					</td>
                    <td><input type="text" name="total_unit_<?=$data->id?>" id="qty_<?=$data->id?>" class="form-control" value="" onkeyup="calculate(<?=$data->id?>); check(<?=$data->id?>)"  /></td>
                    <td><input type="text" name="total_amt_<?=$data->id?>" id="amount_<?=$data->id?>" class="form-control" value="" readonly=""  /></td>
					<td><input type="text" name="note_<?=$data->id?>" id="remarks_<?=$data->id?>" class="form-control" value=""  /></td>
                </tr>
				<? } ?>
                </tbody>
            </table>
