...
if(isset($_POST['new']) && $_SESSION['csrf_token']===$_POST['csrf_token'])
{
...
if(isset($_POST['confirmm']))
{
		...
		$or_no = $_POST['or_no'];
        $sql = 'select w.*,s.item_ledger,i.sub_ledger_id from warehouse_other_receive_detail w, item_info i, item_sub_group s, vendor v where i.item_id=w.item_id and s.sub_group_id=i.sub_group_id and v.vendor_id=w.vendor_id and w.or_no="'.$or_no.'"';
		$qry = db_query($sql);
		...
		$vendor_ledger=find_all_field('vendor','','vendor_id="'.$vendor_id.'"');
       // $localPurchaseLedger = find_a_field('config_group_class','localPurchase','group_for="'.$group_for.'"');
		...
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $data->item_ledger, $narration, $data->amount,'0',  $tr_from, $tr_no,$data->sub_ledger_id,$tr_id,$cc_code,$group_for);
		$total_amt +=$data->amount;
		}
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $vendor_ledger->ledger_id, $narration, '0',$total_amt, $tr_from, $tr_no,$vendor_ledger->sub_ledger_id,$tr_id=0,$cc_code,$group_for);
		unset($_POST);
		$_POST[$unique]=$$unique;
		$_POST['entry_by']=$_SESSION['user']['id'];
		$_POST['entry_at']=date('Y-m-d h:s:i');
