...
//debit	
$cogs_rate = find_a_field('journal_item', '(sum(item_in*final_price)-sum(item_ex*final_price))/(sum(item_in)-sum(item_ex))', 'item_id = "'.$ch_data->item_id.'" and warehouse_id = "'.$_SESSION['user']['depot'].'"');
$new_cogs_rate=number_format($cogs_rate,2,'.','');
$cogs_amt = $new_cogs_rate*$ch_data->qty;
...
?>
<input name="dealer" type="text" id="dealer" class="form-control" />
    </strong></td>
