...
if(isset($_POST['confirm'])){
 ...
 $sql_all = 'select pr.*,s.item_ledger as item_ledger,i.item_name,sum(pr.amount) as invoice_amt,pr.item_id from lc_import pr, item_info i, item_sub_group s where i.sub_group_id=s.sub_group_id and pr.item_id=i.item_id and pr.import_no="'.$or_no.'" and pr.journal_status="Pending" group by pr.item_id';
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
 ...
 <span id="s_check_<?=$row->id?>">
			  ...
			  <? if($unrec_qty>0){$cow++;?>
                ...
                <input name="serial_no_<?=$row->id?>" type="text" id="serial_no_<?=$row->id?>" style="width:200px; float:none" onchange="check_serial(<?=$row->id?>)" value=""  />
