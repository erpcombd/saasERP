...
$target_url = '../wo/delivery_challan_primitek.php';
...
$con .= ' and m.do_no in ('.$_POST['do_no'].') ';
  ...
  $res="select  m.do_no, m.job_no, m.dealer_code, m.do_date,m.depot_id,  m.status, sum(d.total_unit) as wo_qty,m.status, m.entry_by, w.warehouse_id, w.warehouse_name, wd.warehouse_id, wd.user_id
 
 
  from sale_do_master m, sale_do_details d, warehouse w, warehouse_define wd
  
 
 where m.do_no=d.do_no and m.depot_id=w.warehouse_id and w.warehouse_id=wd.warehouse_id and m.entry_by=wd.user_id and wd.status='Active' and  m.status  in ('CHECKED') ".$con." group by m.do_no order by m.do_date desc, m.do_no  ";
