...
$group_data = find_all_field('user_group','group_name','id='.$master_all->group_for);
    ...
    <td><div class="header">
        ...
        <!--<td bgcolor="#666666" style="text-align:center; color:#FFF; font-size:18px; font-weight:bold;">WORK ORDER </td>-->
					...
					<tr>
					 ...
					 <?php /*?> <td><?='<img class="barcode" alt="'.$barcodeText.'" src="barcode.php?text='.$barcodeText.'&codetype='.$barcodeType.'&orientation='.$barcodeDisplay.'&size='.$barcodeSize.'&print='.$printText.'"/>' ?></td><?php */?>
              ...
              $print_url = "https://saaserp.ezzy-erp.com/app/views/warehouse_mod/wo/sales_invoice_print_view.php?c=" . rawurlencode($company_id) . "&v=" . rawurlencode($req_no_qr_data);
...
$total_amount = $total_amount + $data->total_amt;
		...
		<?php if($master->ait>0){ ?>
        <tr>
          <td colspan="6"><div align="right"><strong>AIT(
                  <?=$master->ait?>
            %):</strong></div></td>
          <td><strong>
