...
if($_GET['do_no']>0){
$$unique=$_SESSION[$unique]=$_GET['do_no'];
}
...

{
		...
		$delete_approval_sql = 'DELETE FROM approval_matrix_history WHERE tr_no="'.$$unique.'" and tr_from="Sales" and remarks="CHECKED"';
		db_query($delete_approval_sql);
...
$_SESSION['csrf_token'] = bin2hex(random_bytes(32));
		...
		$ms_data = find_all_field('sale_do_master','','do_no='.$do_no);
		  ...
		  $sql = 'select d.*,i.pack_size as item_pack_size,i.ctn_size from sale_do_details d, item_info i where  d.item_id=i.item_id and d.do_no = '.$do_no;
...

		{
			...
			echo	$qty=$_POST['chalan_'.$data->id];

      if ($qty > $data->item_pack_size) {
    // First, get packets & loose pieces
			$chalan_pkt  = (int)($qty / $data->item_pack_size);
			$chalan_dist = (int)($qty % $data->item_pack_size);
		
			// Now, get cartons from packets
			$chalan_ctn  = (int)($chalan_pkt / $data->ctn_size);
		
			// Remaining packets after full cartons
			$chalan_pkt  = (int)($chalan_pkt % $data->ctn_size);
	}
	      ...
	      $cost_price=find_a_field('journal_item','final_price','item_id="'.$item_id.'"   order by id desc ');
		  
		   $stock_qty=find_a_field('journal_item','sum(item_in-item_ex)','item_id="'.$item_id.'" and warehouse_id="'.$ms_data->depot_id.'"');
		   $chalan_qty=find_a_field('sale_do_chalan','sum(total_unit)','item_id="'.$item_id.'" and do_no="'.$data->do_no.'" and order_no="'.$data->id.'"');
			$tot_chalan_qty=$chalan_qty+$qty;

				$cost_amt = ($qty*$cost_price); 

 if($data->unit_price_gift_item>0){
 $gift_item_unit_price=$data->unit_price_gift_item;
 }
 if($tot_chalan_qty<=$data->total_unit && $qty<=$stock_qty){
 
 
  echo $so_invoice = 'INSERT INTO sale_do_chalan (year, ch_id, chalan_no, chalan_date, order_no, do_no, job_no, do_date, item_id, dealer_code, unit_price,pkt_size,ctn_size,ctn_unit,pkt_unit, dist_unit, total_unit, total_amt, discount, depot_id, group_for, rec_name, rec_mob, vehicle_no, driver_name, driver_mobile, delivery_point, delivery_man, delivery_man_mobile, entry_by, entry_at, status, cost_price, cost_amt,unit_price_gift_item,gift_on_order,gift_on_item,gift_id)


  

  VALUES("'.$YR.'","'.$ch_cy_id.'","'.$chalan_no.'","'.$ch_date.'","'.$data->id.'","'.$data->do_no.'","'.$data->job_no.'","'.$data->do_date.'","'.$item_id.'","'.$data->dealer_code.'","'.$rate.'","'.$data->item_pack_size.'","'.$data->ctn_size.'","'.$chalan_ctn.'","'.$chalan_pkt.'","'.$chalan_dist.'","'.$qty.'","'.$amount.'","'.$discount.'","'.$ms_data->depot_id.'","'.$ms_data->group_for.'","'.$rec_name.'","'.$rec_mob.'","'.$vehicle_no.'","'.$driver_name.'","'.$driver_mobile.'","'.$delivery_point.'","'.$delivery_man.'","'.$delivery_man_mobile.'","'.$entry_by.'","'.$entry_at.'","CHECKED", "'.$cost_price.'", "'.$cost_amt.'","'.$gift_item_unit_price.'", "'.$data->gift_on_order.'", "'.$data->gift_on_item.'", "'.$data->gift_id.'")';
...
journal_item_control($item_id, $ms_data->depot_id, $ch_date,  0, $qty, 'Sales', $xid, $rate, '', $chalan_no, '', '',$ms_data->group_for, $rate->unit_price, '' );
...
$tr_type="Add";
		...
		echo "<h1 style='text-align:center;color:red;font-weight:bold;'>Sales Order Qty Or Stock Qty Exceeded</h1>";
	}
...
header("Location:delivery_challan.php?do_no=".$do_no);
...
<!--        top form start hear-->
		...
		<!--<h4 style="text-align:center; color:#FF0000">"<strong>Please Don't Reload This Page</strong>"</h4>-->
									...
									<thead>
										...
										?>
												...
												<tr>
												  ...
												  <td ><a target="_blank" href="delivery_challan_print_view.php?v_no=<?=rawurlencode(url_encode($aaa->chalan_no));?>"><?=$aaa->chalan_no?></a></td>
        ...
        <div class="n-form1 fo-white pt-0 p-0">
			...
			<table class="table1  table-striped table-bordered table-hover table-sm">
					  ...
					  <tr>
						...
						<td rowspan="2" align="center" ><a title="WO Preview" target="_blank" href="../../../views/sales_mod/wo/sales_order_print_view_store.php?v_no=<?=rawurlencode(url_encode($$unique));?>" ><img src="../../../images/print.png" alt="" width="30" height="30" /></a></td>
					  </tr>
...
<div class="container-fluid bg-form-titel">
            ...
            <div class="row">
                ...
                <input name="chalan_date" type="text" id="chalan_date" required="required" readonly value="<?=($ch_data->chalan_date!='')?$ch_data->chalan_date:date(format: 'Y-m-d')?>"/>
     ...
     $sql='select a.id,  a.item_id,  a.unit_price, a.depot_id, b.item_name,  b.unit_name,   a.total_unit as qty ,a.ctn_unit,a.pkt_unit,a.dist_unit,b.ctn_size ,b.pack_size,a.gift_id,a.do_no
...
?>
          ...
          <tr>
		   ...
		   $gsql = 'select g.offer_name,d.* from sale_gift_offer g, sale_do_details d where  g.id=d.gift_id and  g.id='.$row->gift_id.' and d.item_id="'.$row->item_id.'" and d.do_no="'.$row->do_no.'"';
        ...
        $stock=find_a_field('journal_item','sum(item_in)-sum(item_ex)','item_id="'.$row->item_id.'" and warehouse_id="'.$row->depot_id.'"');
        $stock_ctn=(int)($stock/$row->pack_size);
        
        ?>
			  
		<td align="center"><? echo number_format($stock_ctn,decimals: 0); ?>
        <input type="hidden" name="stk_ctn_<?=$row->id?>" id="stk_ctn_<?=$row->id?>" value="<?=$stock_ctn?>"  onKeyUp="calculation(<?=$row->id?>)" /></td>
        <td>
        <? echo number_format($stock,2);?> 

        <input type="hidden" name="stk_qty_<?=$row->id?>" id="stk_qty_<?=$row->id?>" value="<?=$stock?>"  onKeyUp="calculation(<?=$row->id?>)" /></td>
        </td>

        <? $total_qty = find_a_field('sale_do_chalan','sum(total_unit)','order_no="'.$row->id.'" and item_id="'.$row->item_id.'"'); 

            $unso_qty = $row->qty-$total_qty;
          
          ?>



		<td><?=number_format($row->ctn_unit,0)?></td>
        <td><?=number_format($row->pkt_unit,0)?></td>
        <td align="center"><?=number_format($row->dist_unit,2);?></td>
        <td align="center"><?=number_format($row->qty,2);?></td>

        <? 
        
            $ctn_qty = (int)($total_qty/$row->pack_size);
            $pcs_qty = (int)($total_qty % $row->pack_size);
			
			
			// Now, get cartons from packets
			$chalan_ctn  = (int)($ctn_qty / $row->ctn_size);
		
			// Remaining packets after full cartons
			$ctn_qty  = (int)($ctn_qty % $row->ctn_size);

        
        ?>

        <!-- delivered qty -->
		<td align="center"><? echo number_format($chalan_ctn,0);?></td>
        <td align="center"><? echo number_format($ctn_qty,0);?></td>

        <td align="center"><? echo number_format($pcs_qty,2);?></td>
        ...
        <? 
            ...
            $unso_ctn  = (int)($unso_qty / $row->pack_size);
            $unso_pcs  = (int)($unso_qty % $row->pack_size);
			
			$unso_m_ctn  = (int)($unso_ctn / $row->ctn_size);
		
			// Remaining packets after full cartons
			$unso_ctn  = (int)($unso_ctn % $row->ctn_size);
		...
		<td><? echo number_format($unso_m_ctn,0);?></td>

        <td align="center"><? echo number_format($unso_ctn,0);?><input type="hidden" name="unso_ctn_<?=$row->id?>" id="unso_ctn_<?=$row->id?>" value="<?=$unso_ctn?>"  onKeyUp="calculation(<?=$row->id?>)" /></td>

        <td><? echo number_format($unso_pcs,2);?></td>

        <input type="hidden" name="unso_pcs_<?=$row->id?>" id="unso_pcs_<?=$row->id?>" value="<?=$unso_pcs?>"  onKeyUp="calculation(<?=$row->id?>)" /></td>

        <td><? echo number_format($unso_qty,2);?><input type="hidden" name="unso_qty_<?=$row->id?>" id="unso_qty_<?=$row->id?>" value="<?=$unso_qty?>"  
		onKeyUp="calculation(<?=$row->id?>)" />
      ...
      <!-- chalan qty -->
        ...
        <td align="center">
		<? if($unso_qty>0){$cow++;?> <input name="chalan_<?=$row->id?>" type="text" id="chalan_<?=$row->id?>" 
		value="<? if ($stock>$unso_qty) {echo $unso_qty;} else {echo $stock;}?>"   
		onKeyUp="calculation(<?=$row->id?>)" /><? }
		 else echo 'Done';?>
        </td>
    ...
    <!--button design start-->
        ...
        <div class="container-fluid p-0 ">
          ...
          <div class="n-form-btn-class">
			...
			<input name="confirm" type="submit" class="btn1 btn1-submit-input" value="CONFIRM CHALLAN" onclick="this.style.display='none'" /></td>

			<? } else{?>
			<input name="return" type="submit" class="btn1 btn1-bg-cancel" value="RETURN" onclick="return_function()" />

			<input type="button" class="btn1 btn1-bg-help" value="CLOSE"  onclick="window.location.href='select_wo_for_challan.php'" />

			<input  name="do_no" type="hidden" id="do_no" value="<?=$do_no;?>"/><input type="hidden" name="return_remarks" id="return_remarks">
			<input  name="csrf_token" type="hidden" id="csrf_token" value="<?=$_SESSION['csrf_token']?>"/>
			</td>
			<input name="confirm" type="submit" class="btn1 btn1-bg-submit" value="CONFIRM CHALLAN" onclick="this.style.display='none'" />
			<? } }?>
            </div>
