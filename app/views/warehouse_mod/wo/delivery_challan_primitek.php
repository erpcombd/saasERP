...
$_SESSION['csrf_token'] = bin2hex(random_bytes(32));
    ...
    // First, get packets & loose pieces
	      ...
	      $cost_price=find_a_field('journal_item','final_price','item_id="'.$item_id.'"   order by id desc ');
		   ...
		   $chalan_qty=find_a_field('sale_do_chalan','sum(total_unit)','item_id="'.$item_id.'" and do_no="'.$data->do_no.'"');
...
header("Location:delivery_challan_primitek.php?do_no=".$do_no);
...
?>
      ...
      <!-- chalan qty -->
        ...
        <td align="center">
		<? if($unso_qty>0){$cow++;?> <input name="chalan_<?=$row->id?>" type="number" id="chalan_<?=$row->id?>" 
