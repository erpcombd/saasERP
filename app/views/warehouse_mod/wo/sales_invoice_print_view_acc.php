...
<table width="1000" border="0" cellspacing="0" cellpadding="0" align="center">
    ...
    <td><div id="pr">
      ...
      <table width="100%" border="1" cellpadding="5" cellspacing="0" bordercolor="#000000" class="tabledesign"  style="font-size:12px">
        <tr>
		  ...
		  <th width="9%" bgcolor="#CCCCCC">Cost Price</th>
 ...
 $res='select  s.item_id,i.item_name,s.unit_price,s.cost_amt, sum(s.total_unit) as total_unit,sum(s.total_amt) as total_amt
...
$total_cogs = $total_cogs+$data->cost_amt;
...
$total_amount = $total_amount + $data->total_amt;
        ...
        <tr>
          <td colspan="7"><div align="right"><strong>Discount(
            <?=$master->discount?>
            %):</strong></div></td>
          <td><strong>
		...
		<?php if($master->vat>0){ ?>
        <tr>
          <td colspan="7"><div align="right"><strong>VAT(
                  <?=$master->vat?>
            %):</strong></div></td>
          <td><strong>
		...
		<?php if($master->ait>0){ ?>
        <tr>
          <td colspan="7"><div align="right"><strong>AIT(
                  <?=$master->ait?>
            %):</strong></div></td>
          <td><strong>
            <?=number_format($nit_ait=$master->ait/100*$total_amount,2);?>
          </strong></td>
        </tr>
        <?php } ?>
        ...
        <tr>
          <td colspan="7"><div align="right"><strong>Transport Cost:</strong></div></td>
          <td><strong>
...
$grand_total=($total_amount-$nit_discount)+$nit_vat+$ch_data->transport_cost+$nit_ait;
?>
        <tr>
          <td colspan="7"><div align="right"><strong> Grand Total:</strong></div></td>
          <td><strong>
