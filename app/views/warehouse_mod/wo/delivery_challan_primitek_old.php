...

{
		...
		echo '<script>window.location.replace("select_wo_for_challan_primitek.php")</script>';
...
<div class="container-fluid bg-form-titel">
     ...
     $sql='select a.id,  a.item_id,  a.unit_price, a.depot_id, b.item_name,  b.unit_name,   a.total_unit as qty ,a.pkt_unit,a.dist_unit ,b.pack_size,a.gift_id,a.do_no
...
?>
      ...
      <!-- chalan qty -->
        ...
        <td align="center"><? if($unso_qty>0){$cow++;?> <input name="chalan_<?=$row->id?>" type="number" id="chalan_<?=$row->id?>" 
		value="<? if ($stock>$unso_qty) {echo $unso_qty;} else {echo $stock;}?>"   
		onKeyUp="calculation(<?=$row->id?>)" /><? } else echo 'Done';?>
        </td>
