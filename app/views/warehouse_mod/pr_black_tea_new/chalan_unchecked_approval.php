...
$st = find_a_field('purchase_receive','status','pr_no="'.$pr_no.'"');
 
 if($st=="UNCHECKED"){
 
 
 $ss = "UPDATE `purchase_receive` SET `status`='CHECKED' where pr_no='".$pr_no."'";
 db_query($ss);
 
$s = "select * from purchase_receive where pr_no='".$pr_no."'";
$q = db_query($s);

while($r = mysqli_fetch_object($q)){
journal_item_control($r->item_id ,$r->warehouse_id,$r->rec_date,$r->qty,0,'Purchase',$r->id,$r->rate,'',$r->pr_no, $r->quality, $r->garden_id, $r->sale_no, $r->lot_no, $r->invoice_no);
 ...
 $bt =  "<button style='background: red; color: white; padding: 5px; margin-left: 5px; border: 0px;'>UPDATED</button>";
}
...
<input type="hidden" name="pr_no" value="<?=$_REQUEST['v_no']?>" />
<input type="submit" name="approved" value="APPROVED" style="background: green; color: white; border:0px; padding: 5px; margin: 5px;" />
<?=$bt?>
</form>
  </div>
