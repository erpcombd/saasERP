...
$data=explode('##',$str);
 ...
 $master_all= find_all_field('warehouse_other_issue','','oi_no="'.$do_no.'"');

 $do_sql='SELECT  * FROM sale_do_master WHERE do_no="'.$do_no.'" ';
 $do_data = find_all_field_sql($do_sql);
 
 $stock_in_pcs = find_a_field('journal_item','sum(item_in)-sum(item_ex)','item_id="'.$item_id.'" and warehouse_id="'.$master_all->warehouse_id.'" ');
...
<tr bgcolor="#CCCCCC">
	 ...
	 <td width="15%"><input name="unit_name" type="text" class="input3"  value="<?=$item_all->unit_name;?>" id="unit_name" style="width:90%; height:30px;" readonly="" /></td>
