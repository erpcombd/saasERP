...
("'.$pr_no.'", "'.$pr_date.'", "'.$batch_no.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$fg_item_id.'", "'.$pr_qty.'", "'.$unit_price.'", "'.$total_amt.'","Finished Goods","'.$status.'", "'.$entry_at.'", "'.$entry_by.'")';
...
$ins_sql_by = 'INSERT INTO production_receive_detail (pr_no, pr_date,batch_no,group_for, warehouse_id, item_id, total_unit, unit_price, total_amt,item_type,status, entry_at, entry_by) VALUES
...
("'.$pr_no.'", "'.$pr_date.'", "'.$batch_no.'", "'.$group_for.'", "'.$warehouse_id.'", "'.$data3->item_id.'", "'.$batch_cal_qty.'", "", "","By Product","'.$status.'", "'.$entry_at.'", "'.$entry_by.'")';
	...
	while($databy =mysqli_fetch_object($queryby)){
		...
		if($databy->final_unit>0){
			...
			if($databy->type=='Rate'){
				...
				$final_price = find_a_field('journal_item','final_price','1 and item_id= '.$raw_id[$databy->item_id].'');



				if($databy->rate_ratio>0){
					...
					$rate = (($databy->rate_ratio / 100) * $final_price);
			...
			db_query($updateby);
			 ...
			 $update_by_product = "update  production_receive_detail  set unit_price ='".$rate."' ,total_amt='".($databy->final_qty*$rate)."',status='RECEIVED' where item_id='".$databy->item_id."' and pr_no='".$pr_no."'";



			db_query($update_by_product);
			...
			journal_item_control2($databy->item_id ,$databy->warehouse_id,$pr_date,$databy->final_qty,0,'Production Receive',$pr_no,$rate,0,$pr_no,'','',$_SESSION['user']['group'],'');
...
journal_item_control($item_id,$warehouse_id,$pr_date,0,$total_unit,'Consumption',$data->id,$unit_price,0,$pr_no,'','',$_SESSION['user']['group'],'');
	...
	$con_sql1="UPDATE production_receive_master SET cost_price='".$pr_unit_price."', cost_amt='".$tot_production_cost."', status='".$status."',  checked_by='".$entry_by."', checked_at='".$entry_at."' WHERE pr_no = '".$pr_no."' ";
	 ...
	 $con_sql2="UPDATE production_receive_detail SET unit_price='".$pr_unit_price."', total_amt='".$tot_production_cost."', status='".$status."', checked_by='".$entry_by."', checked_at='".$entry_at."' WHERE batch_no = '".$batch_no."' and item_id=".$fg_item_id." ";
	 ...
	 journal_item_control2($fg_item_id ,$warehouse_id,$pr_date,$pr_qty,0,'Production Receive',$pr_no,$pr_unit_price,0,$pr_no,'','',$_SESSION['user']['group'],'');
	...
	//factory over head journal
	 ...
	 $rm_sql="SELECT g.ledger_group_id,sum(p.total_amt) as total_amt,p.id,p.pr_no,p.pr_date FROM production_rm_consumption p LEFT JOIN item_info i on p.item_id=i.item_id LEFT JOIN item_sub_group s on i.sub_group_id=s.sub_group_id LEFT JOIN item_group g on s.group_id=g.group_id  WHERE p.pr_no='".$pr_no."' group by p.item_id";	
	 ...
	 $rm_insert="INSERT INTO `secondary_journal` (`proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `entry_by`, `tr_id`, `group_for`, `entry_at`) VALUES ('".$_SESSION['proj_id']."', '".$jv."', '".$pr_date."', '".$rm_data->ledger_group_id."','".$narration."', '0.00',".$rm_data->total_amt.", '".$tr_from."', '".$pr_no."', '', '0', '".$_SESSION['user']['id']."', '', '".$_SESSION['user']['group']."', '".date('Y-m-d H:i:s')."')";
	 ...
	 $fg_sql="SELECT g.ledger_group_id,p.total_amt as total_amt,p.id,p.pr_no,p.pr_date FROM production_receive_detail p LEFT JOIN item_info i on p.item_id=i.item_id LEFT JOIN item_sub_group s on i.sub_group_id=s.sub_group_id LEFT JOIN item_group g on s.group_id=g.group_id  WHERE p.pr_no='".$pr_no."' group by p.item_id ";	



	 $fg_query = db_query($fg_sql);
	 ...
	 while($fg_data=mysqli_fetch_object($fg_query)){

	 	$journal_amt = $fg_data->total_amt;

	 $fg_insert="INSERT INTO `secondary_journal` (`proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `entry_by`, `tr_id`, `group_for`, `entry_at`) VALUES ('".$_SESSION['proj_id']."', '".$jv."', '".$pr_date."', '".$fg_data->ledger_group_id."','".$narration."',".($journal_amt).",'0.00', '".$tr_from."', '".$pr_no."', '', '0', '".$_SESSION['user']['id']."', '', '".$_SESSION['user']['group']."', '".date('Y-m-d H:i:s')."')";
	...
	sec_journal_journal2($jv,$jv,$tr_from);
...
<div class="box" style="width:100%;">
    ...
    <td width="9%" align="right" ><strong> BOM No: </strong></td>
	...
	<a href="../BOM/invoice_print_view.php?bom_no=<?=$batch_no;?>" target="_blank"><input name="batch" type="text" id="batch" style="width:90%; height:32px;" value="<?=$batch_data->inv_type;?><?=$batch_data->bom_no;?>" readonly="" required tabindex="1" /></a>	</td>
...
//		}
	...
	from ledger_group l, accounts_ledger a where l.group_id=a.ledger_group_id and a.ledger_group_id=50123 and a.flag=1
...
<input name="foe_amt_<?=$data->ledger_id?>" type="text" id="foe_amt_<?=$data->ledger_id?>" value="<?=$foe_exp[$data->ledger_id]; $tot_foe_exp +=$foe_exp[$data->ledger_id];?>" onkeyup="SUMcalculation(<?=$data->ledger_id?>)"  style="width:120px; height:30px;" readonly="" readonly="readonly"  /></td>
...
//		$do_con .= ' and m.do_no in ('.$_POST['do_no'].') ';
    ...
    (SELECT final_price 
     ...
     WHERE item_id = j.item_id AND item_in > 0 

     ORDER BY id DESC 

     LIMIT 1) AS latest_final_price
...
ORDER BY j.id DESC; ";
		 ...
		 while($info=mysqli_fetch_object($query)){
  		 ...
  		 $final_price[$info->item_id]=$info->latest_final_price;
...
<? //if($po_no[$data->po_no]==0) { } ?>
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
	...
	<td bgcolor="#99CCFF"><input name="stock_<?=$datafg->id?>" type="text" id="stock_<?=$datafg->id?>" value="<?=$final_stock[$datafg->item_id]; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; "  readonly="readonly" />
    ...
    <td bgcolor="#99CCFF"><input name="unit_price_<?=$datafg->id?>" type="text" id="unit_price_<?=$datafg->id?>" value="<?=$final_price[$datafg->item_id]; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; "  readonly="readonly" /></td>
	...
	<input name="total_unit_<?=$datafg->id?>" type="text" id="total_unit_<?=$datafg->id?>" value="<?=$total_qty_bom=$datafg->bom_qty*$ms_data->pr_qty; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px;" readonly /></td>
	...
	<input name="actual_unit_<?=$datafg->id?>" type="text" id="actual_unit_<?=$datafg->id?>" value="<?=$total_qty=$datafg->final_unit*$ms_data->pr_qty; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " readonly="readonly" />
	...
	<? 
		...
		if(($total_qty>$final_stock[$datafg->item_id]) || ($final_stock[$datafg->item_id]=='') ){
    ...
    <td bgcolor="#99CCFF"><input name="total_amt_<?=$datafg->id?>" type="text" id="total_amt_<?=$datafg->id?>" value="<?=$total_amt=$total_qty*$final_price[$datafg->item_id]; $grand_total_amt +=$total_amt; ?>" style="width:80px; height:30px; "  readonly="readonly" /></td>
...
//		$do_con .= ' and m.do_no in ('.$_POST['do_no'].') ';
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
	...
	<input name="total_unit_by_<?=$databy->id?>" type="text" id="total_unit_by_<?=$databy->id?>" value="<?=$total_qty_bom=$databy->bom_qty*$ms_data->pr_qty; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:80px; height:30px; " readonly="" readonly="readonly" /></td>
	...
	<input name="actual_unit_by_<?=$databy->id?>" type="text" id="actual_unit_by_<?=$databy->id?>" value="<?=$total_qty=$databy->final_unit*$ms_data->pr_qty; $grand_by_unit +=$total_qty;  ?>" onkeyup="TRcalculationpo(<?=$databy->id?>)" style="width:80px; height:30px; "  readonly="readonly" /></td>
