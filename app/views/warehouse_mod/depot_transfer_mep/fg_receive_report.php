...
$unique = 'req_no';
$status = 'UNCHECKED';
$target_url = '../depot_transfer_mep/print_view_receive.php';
		...
		<option <? if($_POST['status']=='ALL SEND') echo 'Selected';?>>ALL SEND</option>
            ...
            <div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
                <div class="form-group row m-0">
                    ...
                    <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">
                        <select name="depot" id="depot" required style="width:200px;">

<? 
//$wids = user_wid_list($_SESSION['user']['id']);
//if($wids!=''){
  if($_SESSION['user']['level'==5]){
optionlist("select d.warehouse_id,w.warehouse_name from warehouse_define d, warehouse w 
where w.warehouse_id=d.warehouse_id and d.user_id='".$_SESSION['user']['id']."' and w.warehouse_id>0 and d.status='enable'",$_POST['depot']);

}else{
foreign_relation('warehouse','warehouse_id','warehouse_name',$_POST['depot'],'1 and warehouse_id in("'.$_SESSION['user']['depot'].'") order by warehouse_name');
}
?>
...
if($_POST['status']=='' || $_POST['status']=='IN TRANSIT') $con .=  'and a.status="SEND"';
elseif($_POST['status']=='' || $_POST['status']=='TRANSFERED')$con .=  'and a.status!="SEND"';
else {$do = 'nothing';}

if($_POST['depot']!='' && $_POST['depot']!='ALL') $con .= 'and a.warehouse_to="'.$_POST['depot'].'"';

 $res='select a.pi_no as transfer_id,a.req_no,a.pi_no as transfer_id, a.pi_date as date, a.invoice_no, b.warehouse_name as Depot_from, a.status, u.fname as entry_by, a.entry_at ,a.group_for, a.remarks as note
from user_activity_management u, fg_issue_master a, warehouse b ,fg_issue_detail d
where a.entry_by=u.user_id and a.warehouse_from=b.warehouse_id and a.pi_no=d.pi_no 
'.$con.' 
...
?>
				...
				<table class="table1  table-striped table-bordered table-hover table-sm">
                    <thead class="thead1">
                    <tr class="bgc-info">
                        <th>Transfer Id</th><th>Req No</th>
                        <th>Date</th>
                        <th>Company</th>
					...
					?>
                        ...
                        <tr>
                            ...
                            <td><?=$row->req_no?><br>
                            <?=find_a_field('requisition_fg_master','req_date','req_no="'.$row->req_no.'"');?>
                            </td>
                            <td><?=$row->date?></td>
                            <td></td>
...
$res='select  	a.pi_no as transfer_id,a.pi_no as transfer_id, a.pi_date as date, a.invoice_no, b.warehouse_name as Depot_from, a.status, u.fname as entry_by, a.entry_at from user_activity_management u, 
fg_issue_master a, warehouse b where   a.entry_by=u.user_id and a.warehouse_from=b.warehouse_id and a.warehouse_to="'.$_SESSION['user']['depot'].'" and b.use_type!="PL" '.$con.' order by a.pi_no desc';
