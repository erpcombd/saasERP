...
// bin card hit
$page_for = 'Transit';
$sql = 'select a.*,i.group_for from fg_issue_detail a, item_info i where a.pi_no="'.$$unique_master.'" and i.item_id=a.item_id';
		$qry = db_query($sql);		
		while($data=mysqli_fetch_object($qry)){
		$tr_id = $data->id;
		journal_item_control($data->item_id ,$data->warehouse_from,$data->pi_date,0,$data->total_unit,$page_for,$tr_id,$data->unit_price,$data->warehouse_to,$data->pi_no,'','',$data->group_for, '', '');
		
		//journal_item_control2($item_id, $data->warehouse_id, $rec_date, $qty, 0, 'Purchase', $xid, $urate,'',$pr_no,'','',$data->group_for, $final_avg_price, '' );
		// journal_item_control($data->item_id ,$data->warehouse_to,$pi_date,0,$qty,$page_for,$tr_id,$unit_price,$data->warehouse_to,$$unique_master);
		}
