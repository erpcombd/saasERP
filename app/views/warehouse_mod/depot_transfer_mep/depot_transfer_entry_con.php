...
$page = 'depot_transfer_entry_con.php';
...
if(isset($_POST['add'])&&($_POST[$unique_master]>0)){
        ...
        if($_POST['item_id']>0){
		    ...
		    $check_old_item=find1("select item_id from fg_issue_detail where pi_no='".$$unique_master."' and item_id='".$_POST['item_id']."' ");
            if($check_old_item==''){     
		            $xid = $crud->insert();
            }
...
<div class="row">
    ...
    <div class="col-md-3">
<input name="warehouse_from" type="hidden" id="warehouse_from"  value="<?=$_SESSION['user']['depot']?>" required/>
<input name="warehouse_from3" type="text" id="warehouse_from3" class="form-control" value="<?=find_a_field('warehouse','warehouse_name','warehouse_id='.$_SESSION['user']['depot'])?>" required/>
    </div>
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" align="center">
  ...
  <form action="<?=$page?>" method="post" name="codz2" id="codz2">
    ...
    <table  width="100%" border="1" align="left"  style="border-collapse:collapse; border:1px solid #caf5a5;" cellpadding="2" cellspacing="2">
	  ...
	  <tr><td align="center" colspan="8"><strong>--Main Items--</strong></td> </tr>
	  <tr>
        ...
        <td  rowspan="5" width="11%" align="center" bgcolor="#FF0000"><div class="button">
		<input name="add" type="submit" id="add" value="ADD" tabindex="12" class="update" onclick="recal();"/></div></td>
      </tr>
...
if($returnable=='YES' && $_SESSION['user']['depot']==69){ 
    ...
    $sqlr='select i.item_id,concat(i.finish_goods_code," # ",i.item_name) from item_info i 
    where item_id in ('.$item_list.') order by i.item_name';
...
<tr>
  <td align="center" colspan="8"><strong>--Consumed Items--</strong></td> 
</tr>
<tr>
        <td align="center" bgcolor="red"><div align="center">
            <input  name="<?=$unique_master?>" type="hidden" id="<?=$unique_master?>" value="<?=$$unique_master?>"/>
            <input  name="warehouse_from" type="hidden" id="warehouse_from" value="<?=$warehouse_from?>"/>
            <input  name="warehouse_to" type="hidden" id="warehouse_to" value="<?=$warehouse_to?>"/>
            <input  name="pi_date" type="hidden" id="pr_date" value="<?=$pi_date?>"/>
...
<td bgcolor="red"><input name="item_name" type="text" class="form-control" id="item_name" value="" required/></td>        
<td bgcolor="red"><input name="note" type="text" class="form-control" id="note" value=""/></td>
<td bgcolor="red"><input name="unit_name" type="text" class="form-control" id="unit_name" value=""/></td>
<td bgcolor="red"><input name="stock" type="text" class="form-control" id="stock" value=""/></td>
<td bgcolor="red"><input name="unit_price" type="text" class="form-control" id="unit_price" value="" onkeyup="stock_check(); total_amtt()" required/></td>
<td align="center" bgcolor="red"><input name="total_unit" type="text" class="form-control" id="total_unit" required onkeyup="stock_check(); total_amtt()"/>
<td align="center" bgcolor="red"><input name="total_amt" type="text" class="input3" id="total_amt"  style="width:98%;" required="required"/></td>
</tr>
