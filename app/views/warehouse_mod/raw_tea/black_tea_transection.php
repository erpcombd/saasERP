...
var strURL="black_tea_transection_ajax.php?item_id="+item_id+"&opening="+opening+"&issue="+issue+"&sale="+sale+"&closing="+closing+"&se_id="+se_id+"&p_date="+p_date+"&flag="+flag;
...
<table width="80%" border="0" align="center">
  ...
  <tr>
    ...
    <td colspan="4" bgcolor="#FF0000"><div align="center" class="style1">Black Tea Transection</div></td>
    </tr>
  ...
  <tr>
    ...
    <td rowspan="2" bgcolor="#FF9966"><a href="black_tea_transection_sheet.php?v_no=<?=$se_sale->sale_no?>" target="_blank"><img src="../../images/print.png" width="26" height="26" /></a></td>
  </tr>
  ...
  <tr>
    ...
    <td align="right" bgcolor="#FF9966"><strong>Select : </strong></td>
...
$led2=db_query("select ledger_id, ledger_name from accounts_ledger where ledger_group_id='1086' and parent=0 order by ledger_id");
...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <th bgcolor="#66CC99">Receive</th>

    <th bgcolor="#FF9999">Issue</th>
 ...
 $sql = "select * from item_info where black_tea_grade=1 and sub_group_id=1096000300010000".$con;
...
<td> <input readonly name="opening_<?=$data->item_id?>" id="opening_<?=$data->item_id?>" type="text" size="12" style="width: 80px;"  value="<?=($opening_old==0)?'0.00':$opening_old;?>" style="width:50px;" /> </td>

<td bgcolor="#66CC99"> <input name="issue_<?=$data->item_id?>" id="issue_<?=$data->item_id?>" type="text" size="12" style="width: 80px;"  value="<?=$info->today_receive;?>" style="width:50px;" onKeyUp="calculation(<?=$data->item_id?>)" /> </td>

<td bgcolor="#FF9999"> <input name="sale_<?=$data->item_id?>" id="sale_<?=$data->item_id?>" type="text" size="12" style="width: 80px;"  value="<?=$info->today_issue;?>" style="width:50px;" onKeyUp="calculation(<?=$data->item_id?>)" /> </td>

<td> <input readonly name="closing_<?=$data->item_id?>" id="closing_<?=$data->item_id?>" type="text" size="12" style="width: 80px;" value="<?=$info->today_close;?>" style="width:50px;" /> </td>
...
?>
    ...
    <input type="button" name="Button" value="Save"  onclick="calculation(<?=$data->item_id?>);update_value(<?=$data->item_id?>)" style="width:50px; height:20px;background-color:#66CC66"/>
...
<? }
			...
			{?>
				  ...
				  <input type="button" name="Button" value="Edit"  onclick="calculation(<?=$data->item_id?>);update_value(<?=$data->item_id?>)" style="width:50px; height:20px; background-color:#FF3366"/>
...
<?php /*?><div class="tabledesign2" style="width:100%">
