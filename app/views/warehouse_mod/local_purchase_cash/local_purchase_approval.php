...
if(isset($_GET['or_no'])){
    $$unique=$_SESSION['or_no2'] = $_GET['or_no'];
...
if(isset($_POST['confirmm']))
{
		...
		$tr_id = $data->id;
		journal_item_control($data->item_id ,$master_all->warehouse_id,$data->or_date,$data->qty,0,$page_for,$tr_id,$data->rate,'',$_SESSION['or_no2']);
...
<!--        top form start hear-->
        <div class="container-fluid bg-form-titel">
            ...
            <div class="form-group row m-0 pb-1">
							...
							<option></option>
							<option value="30000173" <?=($cash_ledger==30000173)?'selected':''?>>Kushira Intenatiomal </option>
								<? foreign_relation('general_sub_ledger','sub_ledger_id','sub_ledger_name',$$field,'type in ("Cash at Bank","Cash In Hand")')?>
							</select>
    ...
    <? if($_SESSION['or_no2']>0 && $cash_ledger>0){ ?>
 ...
 from requisition_order a,item_info b
  ...
  where b.item_id=a.item_id and a.req_no='.$req_no;
						...
						while ($data = mysqli_fetch_object($query)) {
							...
							$already_po_qty = find_a_field('warehouse_other_receive_detail', 'sum(qty)', 'req_no="' . $req_no . '" and item_id="' . $data->item_code . '" and req_id="' . $data->id . '"');
        ...
        <tr>
          ...
          <td><input type="text" style="text-align:center" onkeyup="count_1(<?= $i ?>);total_cal()" onblur="update_item(<?=$data->id ?>)" name="qty<?= $data->id ?>" id="qty<?= $i ?>"
										value="<?=$this_po->qty ?>" readonly="readonly" required /></td>
          <td><?= $data->Unit ?>
          ...
          <td><input type="hidden" required name="details_id<?= $data->id ?>"
										value="<? echo $data->id; ?>" />
            <input type="text"  style="text-align:right"  onkeyup="count_1(<?= $i ?>);total_cal()"
										onblur="update_item(<?= $data->id ?>)" required name="unit_price<?= $data->id ?>"
										id="unit_price<?= $i ?>" value="<?= $this_po->rate ?>" />          </td>
          <td><input type="text"  style="text-align:right"  readonly required name="amount<?= $data->id ?>" id="amount<?= $i ?>"
    ...
    <form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">-->
        <div class="container-fluid p-0 ">
          ...
          <div class="n-form-btn-class">
                ...
                <input name="confirmm" type="submit" class="btn1 btn1-bg-submit" value="CONFIRM AND FORWARD LP" onclick="" />
          </div>
