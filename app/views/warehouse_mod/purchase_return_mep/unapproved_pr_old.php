...
$group_for=$do_master->group_for;
  ...
  $do_sql = "select a.*, m.pr_no,m.depot_id,m.return_type, m.pr_date,s.return_type,g.ledger_group_id as ledger_id ,  v.sub_ledger_id as vendor_ledger
 
 from purchase_return_master m, purchase_return_details a, item_info b, purchase_return_type s, item_group g , vendor v
 
 where b.item_group=g.group_id and m.return_type=s.id and m.pr_no=a.pr_no and b.item_id=a.item_id and v.vendor_id=m.vendor_id and a.pr_no='".$or_no."' order by a.id";
...
$do_query = mysql_query($do_sql);	
		...
		{ $tr_id = $do_data->id;
		$avg_rate = find_a_field('journal_item', '(sum(item_in*final_price)-sum(item_ex*final_price))/(sum(item_in)-sum(item_ex))', 'item_id = "'.$do_data->item_id.'"');
...
add_to_sec_journal2($proj_id, $jv_no, $jv_date, $do_data->ledger_id, $narration, '0', ($do_data->total_amt), $do_data->return_type, $tr_no,'',$tr_id,$cc_code,$group_for);
  $total_amt +=$do_data->total_amt;
  $return_type = $do_data->return_type;
  
  $vendor_sub_ledger=$do_data->vendor_ledger;
		}
add_to_sec_journal2($proj_id, $jv_no, $jv_date, $vendor_ledger, $narration, $total_amt, '0', $return_type, $tr_no,$vendor_sub_ledger,$tr_id=0,$cc_code,$group_for);

		$sql2 = 'update purchase_return_details set status="CHECKED" where pr_no = '.$or_no;
		mysql_query($sql2);
		
		
		$checked_by=$_SESSION['user']['depot'];
		  $sql3 = 'update purchase_return_master set status="CHECKED", checked_by='.$checked_by.',checked_at="'.date('Y-m-d H:i:s').'" where pr_no = '.$or_no;
		mysql_query($sql3);
                ...
                <table class="table1  table-striped table-bordered table-hover table-sm">
                    ...
                    <tbody class="tbody1">
                       ...
                       <? 
							...
							<td <?=$data->pr_no;?>;" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;<?=$data->vendor_name;?></td>
							<td <?=$data->pr_no;?>;" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('purchase_return_type','return_type','id='.$data->return_type);?></td>
							<td <?=$data->pr_no;?>;" <?=(++$z%2)?'':'class="alt"';?>><?= find_a_field('warehouse','warehouse_name','warehouse_id='.$data->depot_id);?></td>
							<td <?=$data->pr_no;?>;" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;
							  <?=find_a_field('user_activity_management','fname','user_id='.$data->entry_by);?></td>
							  
							  <td>
							  <input type="button" name="submitit"  value="View" class="btn1 btn1-bg-submit" onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?> / >
