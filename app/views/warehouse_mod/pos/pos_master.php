...
switch ($_POST['report']) {
    case 1:
		...
		{$to_date=$t_date; $fr_date=$f_date; $date_con=' and m.pos_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
		 ...
		 $sql='select m.pos_id,m.pos_date,d.dealer_name_e,i.item_name, s.qty,s.rate,s.total_amt, m.comments from sale_pos_master m, sale_pos_details s left join dealer_info d on d.dealer_code=s.dealer_id left join item_info i on i.item_id=s.item_id where m.pos_id=s.pos_id '.$warehouse_con.$item_sub_con.$item_con.$date_con.$dealer_con.' ';
