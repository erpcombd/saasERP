...
$st_date = $_POST['st_date'];
  ...
  $select = 'select * from fg_transfer_master where st_date="'.$st_date.'" and warehouse_to = "'.$_GET['warehouse_to'].'" and status="CHECKED"';
...
//$consump_amt = find_a_field('production_floor_issue_detail','sum(total_amt)','st_no='.$row->st_no);
...
//journal_item_control($row->item_id,12,$st_date,$row->batch_qty,0,'Production Receive',$row->st_no,$consump_rate,'',$row->st_no);
...
//journal_item_control($row->item_id,37,$st_date,$row->batch_qty,0,'Production Receive',$row->st_no,$consump_rate,'',$row->st_no);
...
$q = "UPDATE `fg_transfer_master` SET `status`='CHECKED', approved_at='".$now."',unit_price='".$consump_rate."', approved_by='".$_SESSION['user']['id']."'  where st_no=".$row->st_no;
...
//     $dr_ledger=find_a_field('item_sub_group','ledger_id','sub_group_id='.$sub_group_id);
...
//		$final_amt =find_a_field('production_floor_issue_detail','sum(total_amt)','st_no='.$row->st_no);




//			$st_date_1 = strtotime($st_date);



//	  $test="INSERT INTO `journal` (`id`, `proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `user_id`, `tr_id`, `group_for`, `entry_at`) VALUES ( NULL, 'Alinfoods', '".$jv."', '".$st_date_1."', '".$dr_ledger."', '(PI#".$row->st_no.") sub group id-".$sub_group_id." item id-".$row->item_id."', '".$final_amt."', '0.00', 'Production Receive', '".$row->st_no."', '', '0', '".$_SESSION['user']['id']."', '', '2', '".date('Y-m-d H:i:s')."')";
...
//	$cr_select="SELECT i.item_id,p.total_amt,s.ledger_id_1 FROM production_floor_issue_detail p,item_info i, item_sub_group s WHERE i.item_id=p.item_id and i.sub_group_id=s.sub_group_id AND p.st_no=".$row->st_no;
...
//		db_query("INSERT INTO `journal` (`id`, `proj_id`, `jv_no`, `jv_date`, `ledger_id`, `narration`, `dr_amt`, `cr_amt`, `tr_from`, `tr_no`, `sub_ledger`, `cc_code`, `user_id`, `tr_id`, `group_for`, `entry_at`) VALUES ( NULL, 'Alinfoods', '".$jv."', '".$st_date_1."', '".$roww->ledger_id_1."', '(PI#".$row->st_no.") sub group id-".$sub_group_id." item id-".$roww->item_id."', '0.00','".$roww->total_amt."',  'Production Receive', '".$row->st_no."', '', '0', '".$_SESSION['user']['id']."', '', '2', '".date('Y-m-d H:i:s')."')");
...
//journal_item_control($item,12,$st_date,$batch_qty,0,'Production Receive',$st_no,'','',$st_no);
...
<? //echo $avg='SELECT sum(batch_qty*`unit_price`)/sum(batch_qty) as cost_price FROM `fg_transfer_master` WHERE `item_id` = 500100317 and unit_price!=0 and status="COMPLETE"'
...
$avg=find_a_field('fg_transfer_master','sum(batch_qty*unit_price)/sum(batch_qty)','item_id = 500100317 and unit_price!=0 and status="COMPLETE"');
...
$select = 'select i.item_name, i.unit_name,m.st_no,m.batch_ctn,m.batch_pcs,w.warehouse_name,m.remarks,m.batch_type from fg_transfer_master m, item_info i, warehouse w where m.warehouse_to=w.warehouse_id and  m.item_id=i.item_id and m.status="PENDING" and m.st_date="'.$_GET['st_date'].'" '.$batch.''.$warehouse.'';
...
<td align="right" valign="top"><?=$row->st_no?>&nbsp;</td>
...
<input type="hidden" name="st_date" value="<?=$_GET['st_date']?>" />
