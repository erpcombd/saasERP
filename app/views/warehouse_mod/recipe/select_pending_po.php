...
$target_url = '../recipe/chalan_view_edit_manual_pending.php';
...
$con .= 'and m.st_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';

if($_POST['batch_type']!=''&&$_POST['batch_type']!='ALL')


$con .= 'and m.Issue_type="'.$_POST['batch_type'].'"';

if($_POST['warehouse_to']>0)
$con .= 'and m.warehouse_from = "'.$_POST['warehouse_to'].'"  ';



echo $res='select m.st_date,m.st_no, w.warehouse_name, m.status 

from fg_transfer_master m, warehouse w 

where m.warehouse_to=w.warehouse_id '.$con.' and m.status="CHECKED" group by m.st_date ';
...
<tbody>
<tr>
<td style="text-align: center;"><a href="chalan_view_edit_manual_pending.php?pi_date=<?=$row->st_date?>&&warehouse_to=<?=$_POST['warehouse_to']?>&&batch_type=<?=$_POST['Issue_type'];?>" target="_blank" style="display:block;">ST Date : <?=$row->st_date?></a></td>
</tr>
</tbody>
