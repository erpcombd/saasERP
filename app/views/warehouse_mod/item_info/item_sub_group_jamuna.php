...
$title=' Create Product Sub Sub Group';
...
if ($ledger_gl_found==0) {
     $acc_ins_led = 'INSERT INTO accounts_ledger (ledger_name, ledger_group_id, acc_class, acc_sub_class, acc_sub_sub_class, opening_balance, balance_type, depreciation_rate, credit_limit, proj_id, budget_enable, group_for, parent, cost_center, entry_by, entry_at)
  
  VALUES("'.$_POST['ledger_name'].'", "'.$_POST['ledger_group'].'", "'.$gl_group->acc_class.'", "'.$gl_group->acc_sub_class.'",
   "'.$gl_group->acc_sub_sub_class.'", "0", "Both", "0", "0", "'.$proj_id.'", "YES", "'.$gl_group->group_for.'", "0", "0", "'.$_POST['entry_by'].'", "'.$_POST['entry_at'].'")';

$tt=db_query($acc_ins_led);
$ledger_id = find_a_field('accounts_ledger','ledger_id','ledger_name="'.$_POST['ledger_name'].'"');
}
$_POST['item_ledger'] =$ledger_id;
$crud->insert();
...
<div class="container-fluid p-0">
    <div class="row">
        <div class="col-sm-7 p-0 pr-2">
			...
			<h4 align="center" class="n-form-titel1">Search Product Category</h4>
...
<?php /*?>                    <?php
                    ...
                    <table id="table_head" class="table table-bordered table-bordered table-striped table-hover table-sm" cellspacing="0">
						...
						<th><span>Product  Group </span></th>
                            <th><span>Product Sub Group </span></th>
                            <th><span>Product  Sub Sub Group </span></th>
                        ...
                        //$rrr = "select b.sub_group_id,b.sub_group_id,a.group_name, b.sub_group_name, b.description from item_sub_group b,item_group a where a.group_id=b.group_id and b.group_for='".$_SESSION['user']['group']."' ".$con." order by a.status";

 $rrr='select cg.group_id,cg.group_name as custom_group_name,g.group_id,g.group_name,s.group_id,s.sub_group_id,s.sub_group_name,s.description from item_group_cus cg,item_group g,item_sub_group s where cg.group_id=g.group_id_cus and g.group_id=s.group_id '.$con.'';
                        ...
                        while($rp=mysqli_fetch_row($report)){$i++; if($i%2==0)$cls=' class="alt"'; else $cls='';?>
                            ...
                            <tr<?=$cls?> onclick="DoNav('<?php echo $rp[5];?>');">
                                <td><?=$rp[1];?></td>
								 <td><?=$rp[3];?></td>
                                <td><?=$rp[6];?></td>
                                <td><?=$rp[4];?></td>
            ...
            <div class="col-sm-5 p-0  pl-2">
                <form id="form1" name="form1" class="n-form setup-fixed" method="post" action="" onsubmit="return check()">
                        ...
                        <div class="col-sm-8 p-0">
                            	...
                            	<? if ($item_ledger==0) {?>
									<select name="ledger_group" required="required" id="ledger_group" tabindex="9">
										...
										<option></option>
										<? foreign_relation('ledger_group','group_id','group_name',$ledger_group,'acc_sub_sub_class in(122,121,123) and group_for="'.$_SESSION['user']['group'].'"');?>
									</select>
								<? }?>
					...
					<div class="form-group row m-0 pl-3 pr-3 p-1">
                        ...
                        <div class="col-sm-8 p-0">
									...
									<select name="cogs_ledger" id="cogs_ledger" tabindex="9">
										...
										foreign_relation('accounts_ledger','ledger_id','ledger_name',$cogs_ledger,'ledger_group_id 																in(411001,121022,426069,426066,426067,426068,426069,426070,511001,411002) and group_for="'.$_SESSION['user']['group'].'"');?>
									</select>
