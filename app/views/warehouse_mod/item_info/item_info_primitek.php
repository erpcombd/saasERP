...
$_POST['finish_goods_code'] = str_replace('"',"``",$_POST['finish_goods_code']);
...
createSubLedger($custome_code,$_POST['item_name'],'item',$_POST[$unique],$_POST['account_code'] ,$_POST['sub_group_id'],$_POST['group_for']);
...
unset($$unique);
header("Location: item_info_primitek.php");
}
...
$_POST['item_description'] = str_replace("'","`",$_POST['item_description']);
 ...
 $update_sub_ledger="update general_sub_ledger set sub_ledger_name='".$_POST['item_name']."',group_for='".$_POST['group_for']."',edit_at='".$_POST['edit_at']."',edit_by='".$_POST['edit_by']."' where tr_id='".$_POST[$unique]."' ";
$up_query=db_query($update_sub_ledger);
		...
		<div class="row">
			<div class="col-sm-7 p-0 pr-2">
				...
				<div class="container n-form1">
						...
						<?
/*$_SESSION['src_item_id'] = str_replace('"',"``",$_POST['src_item_id']);

$_SESSION['src_item_id'] = str_replace("'","`",$_POST['src_item_id']);

$_SESSION['fg_code'] = str_replace('"',"``",$_POST['fg_code']);
						...
						if($_SESSION['item_group']>0||$_SESSION['src_sub_group_id']>0||$_SESSION['src_item_id']!=''||$_SESSION['fg_code']>0){
							...
							<table id="table_head" class="table table-striped table-bordered table-hover table-sm" cellspacing="0">
								...
								<?php
								 ...
								 $td="select a.item_id,a.item_name,b.sub_group_name,a.item_description,a.finish_goods_code, a.status, a.group_for 
								 FROM item_info a, item_sub_group b 
								 where b.sub_group_id=a.sub_group_id ".$con." order by item_id";
								...
								while($rp=mysqli_fetch_row($report)){$i++; if($i%2==0)$cls=' class="alt"'; else $cls='';?>
									...
									<tr<?=$cls?> onclick="DoNav('<?php echo $rp[0];?>');">
										...
										<td><?=$rp[2];?></td>
										<td><?=$rp[5];?></td>
										<td><?=find_a_field('user_group','group_name','id='.$rp[6]);?></td>
									</tr>
...
<!--					</div>-->
					...
					<div class="form-group row m-0 pl-3 pr-3 p-1">
						<label for="group_name" class="col-sm-3 pl-0 pr-0 col-form-label req-input">Item Category :</label>
						<div class="col-sm-9 p-0">
...
<!--<fieldset class="scheduler-border" style=" margin: 0px 15px 10px !important; ">
					...
					<div class="form-group row m-0 pl-3 pr-3 p-1">
						<label for="group_name" class="col-sm-3 pl-0 pr-0 col-form-label">Wight</label>
						<div class="col-sm-9 p-0">

							<input type="text" class="m-0" name="weight" id="weight" value="<?=$weight?>" />
					...
					<div class="form-group row m-0 pl-3 pr-3 p-1">
						<label for="group_name" class="col-sm-3 pl-0 pr-0 col-form-label">GSM :</label>
							<div class="col-sm-9 p-0">
									...
									<option></option>
										<? foreign_relation('gsm','id','gsm',$_POST['gsm'],'1 order by id');?>
									</select>
								</div>
					</div>
					<div class="form-group row m-0 pl-3 pr-3 p-1">
						<label for="group_name" class="col-sm-3 pl-0 pr-0 col-form-label">Colour :</label>
							<div class="col-sm-9 p-0">
							
									<select  id="colour" name="colour" class="form-control" >
									<option value="<?=$colour?>"><?=find_a_field('colour','colour','id='.$colour);?></option>
									<option></option>
										<? foreign_relation('colour','id','colour',$_POST['colour'],'1 order by id');?>
									</select>
								</div>
					</div>
