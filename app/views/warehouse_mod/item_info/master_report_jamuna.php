...
window.addEventListener('beforeprint', function () {
    ...
    document.querySelectorAll('.dt-buttons, .dt-search, .dt-length, .dt-info, .dt-paging').forEach(el => {
        el.style.display = 'none';
    });

    // Optional: hide the #pr element before print if needed
    const prElement = document.getElementById('pr');
    if (prElement) {
        prElement.style.display = 'none';
    }
});

window.addEventListener('afterprint', function () {
    // Show DataTables controls back
    document.querySelectorAll('.dt-buttons, .dt-search, .dt-length, .dt-info, .dt-paging').forEach(el => {
        el.style.display = '';
    });

    // Show #pr element again
    const prElement = document.getElementById('pr');
    if (prElement) {
        prElement.style.display = '';
    }
});
</script>
...
<table width="100%" cellspacing="0" cellpadding="2" border="0" class="example display" id="ExportTable">
...
<thead>
	<tr>
		...
		<th>Product Sub Sub Group</th>
		<th>Product Information</th>
	</tr>
</thead>
<tbody>
<?php 
	  $sql='select cg.group_id,cg.group_name as custom_group_name,g.group_id,g.group_name,s.group_id,s.sub_group_id,s.sub_group_name from item_group_cus cg,item_group g,item_sub_group s where cg.group_id=g.group_id_cus and g.group_id=s.group_id';
	$query=db_query($sql);
	while($srow=mysqli_fetch_object($query)){
?>
	<tr>
		<td><?php echo ++$i;?></td>
		<td><?php echo $srow->custom_group_name;?></td>
		<td><?php echo $srow->group_name;?></td>
		<td><?php echo $srow->sub_group_name;?></td>
		<td> </td>
	</tr>
	<?php } ?>
</tbody>
...

{
            ...
            $sql="select g.group_name, s.sub_group_name, i.item_id, i.item_name,i.group_for, i.unit_name,  i.d_price,  i.status, i.finish_goods_code, i.cost_price, i.sale_price,g.group_id_cus,c.group_name as custom_group_name  from item_group_cus c,item_group g, item_sub_group s, item_info i where c.group_id=g.group_id_cus and g.group_id=s.group_id and i.group_for='".$_SESSION['user']['group']."' and s.sub_group_id=i.sub_group_id   ".$item_sub_group_con.$item_id_con.$product_nature.$group_for_con.$group_id_con;
...
<th rowspan="2">Item Sub Sub Group </th>
...
<tr><th>S/L</th>
      ...
      <th >Barcode</th>
       <th >UPC/Body Barcode</th>
       <th >Style/Attribute Code</th>
       <th >Master Barcode </th>
       <th >Brand </th>
       <th >Product Name</div></th>
       <th >UOM</th>
       <th>CTN Size</th>
       <th>Bin/Position Number</th>
       <th>Bin/Position Description</th>
	   <th>Shelf/Level Name</th>
	   <th>Shelf/Level Description</th>
	   <th>Rack/Bay Name</th>
	   <th>Rack/Bay Description</th>
	   <th>Lane/Aisle  Name</th>
	   <th>Lane/Aisle Description</th>
	   <th>Zone/Location Name</th>
	   <th>Zone/Location Description</th>
	   <th>Category</th>
	   <th> Sub Category</th>
