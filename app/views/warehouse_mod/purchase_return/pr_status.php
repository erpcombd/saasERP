                ...
                <table class="table1  table-striped table-bordered table-hover table-sm">
                    ...
                    <tbody class="tbody1">
                       ...
                       <? 
							...
							where 
							  m.group_for=u.id  and m.vendor_id=d.vendor_id and m.pr_no=c.pr_no and m.invoice_no=''  ".$group_for_con.$con.$dealer_con."  group by c.pr_no order by  m.pr_date, m.pr_no  ";
							$query = db_query($res);
							...
							<td <?=$data->pr_no;?>;" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;
							  ...
							  <td>
							  <button type="button" name="submitit" value="View" class="btn2 btn1-bg-submit" onClick="custom('<?=url_encode($data->pr_no);?>','<?=url_encode($c_id);?>');" <?=(++$z%2)?'':'class="alt"';?> >
