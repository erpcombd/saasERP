...
$vendor_ledger = find_all_field('vendor','*','vendor_id="'.$_POST['vendor_id'].'"');
...
$do_master = find_all_field('purchase_return_master','pr_no','pr_no='.$or_no);
   ...
   $do_sql = "select a.*, m.pr_no,m.depot_id,m.return_type, m.pr_date,s.return_type,sub.item_ledger,b.sub_ledger_id from purchase_return_master m, purchase_return_details a, item_info b, purchase_return_type s, item_sub_group sub where b.sub_group_id=sub.sub_group_id and  m.return_type=s.id and  m.pr_no=a.pr_no and  b.item_id=a.item_id  and a.pr_no='".$or_no."' order by a.id ";
...
add_to_sec_journal($proj_id, $jv_no, $jv_date, $do_data->item_ledger, $narration, '0', ($do_data->total_amt), $do_data->return_type, $tr_no,$do_data->sub_ledger_id,$tr_id,$cc_code,$group_for,'','','','','','','',"NO");
...
add_to_sec_journal($proj_id, $jv_no, $jv_date, $vendor_ledger->ledger_id, $narration, $total_amt, '0', $return_type, $tr_no,$vendor_ledger->sub_ledger_id,$tr_id=0,$cc_code,$group_for,'','','','','','','',"NO");
                    ...
                    <tbody class="tbody1">
                       ...
                       <? 
							...
							<td <?=$data->pr_no;?>;" <?=(++$z%2)?'':'class="alt"';?>>&nbsp;
							  ...
							  <td>
<button type="button" name="submitit"  value="View"  onClick="custom(<?=$data->pr_no;?>);" <?=(++$z%2)?'':'class="alt"';?> class="btn2 btn1-bg-submit">
