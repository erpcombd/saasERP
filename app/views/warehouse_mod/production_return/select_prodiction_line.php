...
$s = "select * from production_receive_detail where pr_no='".$_GET['pr_no']."'";
$q = db_query($s);

while($data=mysqli_fetch_object($q)){

journal_item_control($data->item_id,$_SESSION['user']['depot'],$data->pr_date,$data->total_unit,'0','Production',$data->id,'0','0',$data->pr_no);
        ...
        db_query('update production_receive_master set status="RECEIVED" where pr_no='.$_GET['pr_no']);
...
<form action="production_receive.php?req=<? echo $_POST['line_id']?>" method="post" name="codz" id="codz">
