...
if(isset($depot_id)) {$depot_con=' and d.depot="'.$depot_id.'"';} 
	...
	case 4:
	if($_REQUEST['do_no']>0)
header("Location:../do/do_view.php?v_no=".$_REQUEST['do_no']);
	break;
	case 5:
if(isset($t_date)) {$to_date=$t_date; $fr_date=$f_date; $date_con=' and m.do_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
if(isset($product_group)) 	{$pg_con=' and d.product_group="'.$product_group.'"';} 
...
where  o.depot_id='".$_POST['depot_id']."' and m.do_no=o.do_no and m.dealer_code=d.dealer_code and i.item_id=o.item_id  and m.status in ('CHECKED','COMPLETED') ".$date_con.$item_con.$item_brand_con.$pg_con.' group by i.finish_goods_code';
	break;
		case 8:
if(isset($t_date)) {$to_date=$t_date; $fr_date=$f_date; $date_con=' and m.do_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
if(isset($product_group)) 	{$pg_con=' and d.product_group="'.$product_group.'"';} 
...
where m.status in ('CHECKED','COMPLETED') and  m.dealer_code=d.dealer_code and w.warehouse_id=d.depot".$date_con.$pg_con." order by m.entry_at";
		break;
						case 50:
		$report="Undelivered DO List";
$to_date=date('Y-m-d'); $fr_date=date('Y-m-d',time()-24*60*60*31); $date_con=' and m.do_date between \''.$fr_date.'\' and \''.$to_date.'\'';
...
<?
		...
		if($_POST['depot_id']>0) 
		$str 	.= '<h2>Depot: '.find_a_field('warehouse','warehouse_name','warehouse_id='.$_POST['depot_id']).'</h2>';
		if(isset($report)) 
		$str 	.= '<h2>'.$report.'</h2>';
		if(isset($to_date)) 
...
if($_POST['report']==1) 
{
$sql="select m.do_no,m.do_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,(select AREA_NAME from area where AREA_CODE=d.area_code) as area,w.warehouse_name as depot,d.product_group as grp, m.rcv_amt,concat(m.payment_by,':',m.bank,':',m.remarks) as Payment_Details from 
sale_do_master m,dealer_info d  , warehouse w
where m.status in ('CHECKED','COMPLETED')  and m.dealer_code=d.dealer_code and w.warehouse_id=d.depot".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con." order by m.do_date,d.dealer_name_e";
$query = db_query($sql);
echo '<table width="100%" cellspacing="0" cellpadding="2" border="0">
<thead><tr><td style="border:0px;" colspan="9">'.$str.'</td></tr><tr><th>S/L</th><th>Do No</th><th>Do Date</th><th>Dealer Name</th><th>Area</th><th>Depot</th><th>Grp</th></tr></thead>
<tbody>';
...
$tp_t = $tp_t+$info[1];
?>
<tr><td><?=$s?></td><td><a href="../do/do_view.php?v_no=<?=$data->do_no?>" target="_blank"><?=$data->do_no?></a></td></td><td><?=$data->do_date?></td><td><?=$data->dealer_name?></td><td><?=$data->area?></td><td><?=$data->depot?></td><td><?=$data->grp?></td></tr>
<?
}
echo '<tr class="footer"><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr></tbody></table>';
}
elseif($_POST['report']==50) 
{


$sql="select m.do_no,m.do_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,(select AREA_NAME from area where AREA_CODE=d.area_code) as area,w.warehouse_name as depot,d.product_group as grp from 
sale_do_master m,dealer_info d  , warehouse w
where m.status in ('CHECKED','COMPLETED')  and m.dealer_code=d.dealer_code and w.warehouse_id=d.depot".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con." order by m.do_date,m.do_no";
...
<tr><th>S/L</th><th>Do No</th><th>Do Date</th><th>Dealer Name</th><th>Area</th><th>Depot</th><th>Grp</th><th>UnDel</th></tr></thead>
<tbody>
<?
while($data=mysqli_fetch_object($query)){$s++;
$item_order = find_a_field('sale_do_details','count(1)','do_no='.$data->do_no);
$item_chalan = find_a_field('sale_do_chalan','count(DISTINCT order_no)','do_no='.$data->do_no);

if($item_order>$item_chalan){
?>
<tr><td><?=$s?></td><td><a href="../do/do_chalan.php?do=<?=$data->do_no?>" target="_blank"><?=$data->do_no?></a></td></td><td><?=$data->do_date?></td><td><?=$data->dealer_name?></td><td><?=$data->area?></td><td><?=$data->depot?></td><td><?=$data->grp?></td><td><?=($item_order - $item_chalan);?></td></tr>
<?
}}
?><tr class="footer"><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr></tbody></table>
<?
}
...
{
$sql2 	= "select distinct o.do_no, d.dealer_code,d.dealer_name_e,w.warehouse_name,m.do_date,d.address_e,d.mobile_no,d.product_group,m.remarks from 
sale_do_master m,sale_do_details o, item_info i,dealer_info d , warehouse w
where m.do_no=o.do_no and i.item_id=o.item_id and m.dealer_code=d.dealer_code and m.status in ('CHECKED','COMPLETED') and w.warehouse_id=d.depot ".$date_con.$item_con.$depot_con.$dtype_con.$pg_con.$dealer_con;
$query2 = db_query($sql2);
...
{
$str 	.= '<p style="width:100%">Dealer Name: '.$dealer_name.' - '.$dealer_code.'('.$data->product_group.')</p>';
$str 	.= '<p style="width:100%"><b>DO NO: '.$do_no.' </b>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Depot:'.$warehouse_name.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Date:'.$do_date.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Ref.:'.$remarks.'</p>
<p style="width:100%">Destination:'.$data->address_e.'('.$data->mobile_no.')</p>';

$dealer_con = ' and m.dealer_code='.$dealer_code;
$do_con = ' and m.do_no='.$do_no;
$sql = "select case when gift_on_order > 0 then concat(i.finish_goods_code,'- ',item_name,' <b>(Gift Item)</b>') else concat(i.finish_goods_code,'- ',item_name) end as item_name,o.pkt_unit as crt,o.dist_unit as pcs from 
sale_do_master m,sale_do_details o, item_info i,dealer_info d , warehouse w
where i.finish_goods_code!='2000' and  i.finish_goods_code!='2001' and  m.do_no=o.do_no and i.item_id=o.item_id and m.dealer_code=d.dealer_code and m.status in ('CHECKED','COMPLETED') and w.warehouse_id=d.depot ".$date_con.$item_con.$depot_con.$dtype_con.$do_con."";
}
