...
if($_POST['report']==1) 
{
$sql="select m.do_no,m.do_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,a.AREA_NAME as area,w.warehouse_name as depot,d.product_group as grp, m.rcv_amt,concat(m.payment_by,':',m.bank,':',m.remarks) as Payment_Details from 
sale_do_master m,dealer_info d  , warehouse w, area a
where m.status in ('PROCESSING','COMPLETED') and a.AREA_CODE=d.area_code and m.dealer_code=d.dealer_code and w.warehouse_id=d.depot".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con." order by m.do_date,m.do_no";
$query = db_query($sql);
