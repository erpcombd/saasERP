...
echo 'Mhafuz';
	...
	//	header("Location: mr_create.php?req_no='".$$unique."'");
...
if(isset($_POST['confirmm'])){
    ...
    // check wid
    $master_wid=findall("select warehouse_id,warehouse_to from requisition_fg_master where req_no='".$$unique."'");
    $details_wid=findall("select warehouse_id,warehouse_to from requisition_fg_order where req_no='".$$unique."' order by id desc limit 1");
...
// check do status
$req_status=findall("select entry_by,status from requisition_fg_master where req_no='".$$unique."'");
//if($do_status->status!='MANUAL'){redirect('mr_create.php?new=2'); exit();}

// locker user
if($req_status->entry_by != $_SESSION['user']['id']){ redirect('mr_create.php?new=2'); exit();}
...
$uinfo =find("select entry_by from requisition_fg_master where req_no='".$$unique."'");
...
<? if($$field!=''){ ?>
<option value="<?=$$field?>"><?=find("select warehouse_name from warehouse where warehouse_id='".$$field."'");?></option> 
<? }else{?>
<option></option>
...
<? }?>
        ...
        <? $field='warehouse_to'; $table='warehouse';$get_field='warehouse_id';$show_field='warehouse_name';?>
             ...
             <select name="warehouse_to" id="warehouse_to"  required <? if($_SESSION[$unique]>0){?> readonly <? }?> >
        	  ...
        	  <? 
        	  if($_SESSION['user']['depot']==51 || $_SESSION['user']['depot']==70){
        	  foreign_relation('warehouse','warehouse_id','warehouse_name','','1 and warehouse_id!="'.$_SESSION['user']['depot'].'" and use_type in("WH","PL","DM") order by use_type,warehouse_name');
        	  }else{
        ...
        <? $field='group_for';?>
            ...
            <option value="<?=$$field?>"><?=find("select group_name from user_group where id='".$$field."'");?></option> 
...
// check real user
$real_user = find("select entry_by from requisition_fg_master where req_no='".$_SESSION[$unique]."'");
if($real_user != $_SESSION['user']['id']){
    unset($$unique); unset($_SESSION[$unique]); $type=1;
    ?><script>window.location.href = "mr_create.php"</script><?
...
<input list="items" name="item_id" type="text" class="input3"  value="" id="item_id" style="width:100px;" onChange="getData()" autocomplete="off" required autofocus/>
 ...
 <datalist id="items">
  ...
  <?php optionlist('select item_id,concat(finish_goods_code," # ",item_name) from item_info where 1 and group_for="'.$group_for.'" order by item_name');?>

  <?php optionlist('select item_id,item_name from item_info where 1 and sub_group_id in (300200000) order by item_name');?>
...
<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
  ...
  <table width="100%" border="0">
    ...
    <tr>
      ...
      <input name="delete"  type="submit" class="btn btn-danger" value="DELETE" style="width:270px; font-weight:bold; font-size:12px;color:#ffff; height:30px" />
      ...
      <input name="confirmm" type="submit" class="btn btn-success" value="Submit For Approval" style="width:300px; font-weight:bold; font-size:12px; height:30px; color:#ffff" />
