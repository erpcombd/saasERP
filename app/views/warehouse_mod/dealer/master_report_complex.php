...
break;
	...
	$dealer_con2.=' and a.dealer_type="'.$_POST['dealer_type'].'"';}
	$report="Customer Information";
		...
		if($_POST['dealer_type']==1) {$sql.= ",t.dealer_type";}
		...
		if($_POST['cc_email']==1) {$sql.= ",a.cc_email as cc_email";}
		 ...
		 $sql.= " from dealer_info a left join dealer_type t on t.id=a.dealer_type left join branch r on r.BRANCH_ID=a.region_code left join zon z on z.ZONE_CODE=a.zone_code left join warehouse w on w.warehouse_id=a.depot left join area ar on ar.AREA_CODE=a.area_code where	1 ".$dealer_con2."";
//echo $sql;
break;
		//ROW_NUMBER() OVER(ORDER BY Id) AS Row
}
