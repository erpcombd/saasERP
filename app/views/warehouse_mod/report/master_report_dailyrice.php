...
{	
  ...
  if(isset($warehouse_id)) {$warehouse_con=' and a.warehouse_id='.$warehouse_id;
  	...
  	$warehouse_con=' and a.warehouse_id='.$_SESSION['user']['depot'];
		...
		$stockSql='select distinct i.item_id,i.unit_name,i.item_name,s.sub_group_name,i.finish_goods_code,i.d_price
...
<?
		 ...
		 echo "<h2> Raw Material Receive & Comsumption</h2>";
?>
<?php echo $str3;?>
<h4 >Date: <?php echo "$date to $date1" ;?></h4>
...
<table width="100%;"  id="ExportTable" cellspacing="0" cellpadding="2" border="0" >
	...
	if ($date){
		 ...
		 $sql_open='SELECT j.item_id, i.item_name,i.unit_name, SUM(j.item_in-j.item_ex) as open,  j.tr_from   
FROM journal_item j, item_info i 
WHERE ji_date  < "'.$date.'" and i.item_id=j.item_id AND j.tr_from in ("Purchase","Opening","RmIssue") and i.sub_group_id = 100010000
group by j.item_id';

$query1 = db_query($sql_open);

while($row1=mysqli_fetch_object($query1)){
 
 
 $open_new[$row1->item_id]= $row1->open;

}



		$sql_close ='SELECT j.item_id, i.item_name,i.unit_name, SUM(j.item_in) as item_in, SUM(j.item_ex) as item_out,  j.tr_from   
FROM journal_item j, item_info i 
WHERE ji_date  BETWEEN "'.$date.'" AND "'.$date1.'" and i.item_id=j.item_id AND j.tr_from in ("Purchase","Opening","RmIssue") and i.sub_group_id = 100010000
group by j.item_id';

$query2 = db_query($sql_close);

while($row2=mysqli_fetch_object($query2)){
 
 
 $item_in_new[$row2->item_id]= $row2->item_in;
 $item_out_new[$row2->item_id]= $row2->item_out;
}



		
		$sql='SELECT  i.item_id ,i.item_name, i.unit_name FROM item_info i WHERE i.sub_group_id = 100010000';
...
$query=db_query($sql);
	...
	?>
		...
		<tr>
			...
			<td><? echo $open_new[$row->item_id];?> </td>

			<td><? echo $item_in_new[$row->item_id] ?></td>			
			<td><? echo $item_out_new[$row->item_id] ?></td>
...
<?
		 ...
		 $str3 	.= '<h4 style="text-align:center;">'.find_a_field('warehouse','warehouse_name','warehouse_id='.$_SESSION['user']['depot']).'</h4>';
		 echo "<h2> Finish Goods Receive</h2>";
?>
<?php echo $str3;?>
<h4 >Date: <?php echo "$date to $date1" ;?></h4>
...
<table width="100%;"  id="ExportTable" cellspacing="0" cellpadding="2" border="0" >
	...
	if ($date){
		 ...
		 $sql_open='SELECT j.item_id, i.item_name,i.unit_name, SUM(j.item_in-j.item_ex) as open,  j.tr_from   
FROM journal_item j, item_info i 
WHERE ji_date  < "'.$date.'" and i.item_id=j.item_id AND j.tr_from in ("Receive","Sales","CASH Sales") and i.sub_group_id in ("200010000","200020000","200030000")
group by j.item_id';
...
while($row1=mysqli_fetch_object($query1)){
 ...
 $open_new[$row1->item_id]= $row1->open;
		...
		$sql_close ='SELECT j.item_id, i.item_name,i.unit_name, SUM(j.item_in) as item_in, SUM(j.item_ex) as item_out,  j.tr_from   
FROM journal_item j, item_info i 
WHERE ji_date  BETWEEN "'.$date.'" AND "'.$date1.'" and i.item_id=j.item_id AND j.tr_from in ("Receive","Sales","CASH Sales") and i.sub_group_id in ("200010000", "200020000", "200030000")
group by j.item_id';
...
while($row2=mysqli_fetch_object($query2)){
 ...
 $item_out_new[$row2->item_id]= $row2->item_out;
		...
		$sql='SELECT  i.item_id ,i.item_name, i.unit_name FROM item_info i WHERE i.sub_group_id in ("200010000","200020000","200030000")';
...
<?
		...
		// echo $str3;
?>
<h1 style="text-align:center; padding: 5px;"> Raw Material Receive & Comsumption</h1>
<h4 >Date: <?php echo "$date to $date1" ;?></h4>
...
<table width="100%;"  id="ExportTable" cellspacing="0" cellpadding="2" border="0" >
	<thead>
		<tr>
				...
				<th>RM Received Qty </th>
				<th>RM Consumption Qty</th>
...
$query=db_query($sql);
	...
	?>
		...
		<tr>
			...
			<td align="right"><? echo $open_new[$row->item_id];?> </td>

			<td align="right"><? echo $item_in_new[$row->item_id]; $item_in_total +=$item_in_new[$row->item_id]; ?></td>			
			<td align="right"><? echo $item_out_new[$row->item_id]; $item_out_total +=$item_out_new[$row->item_id];?></td>
		...
		<tr align="right" style="font-weight:bold;">
			...
			<td><?=number_format( $item_in_total,2)?></td>
			<td><?=number_format( $item_out_total,2)?></td>
			<td></td>
		</tr>
	</tbody>
</table>
...
<br />
<h1 style="text-align:center;"> Finish Goods Receive & Deliver </h1>
...
$query=db_query($sql);
	...
	?>
		...
		<tr>
			...
			<td align="right"><? echo $item_in_new[$row->item_id]; $item_in_fg += $item_in_new[$row->item_id]; ?></td>			
			<td align="right"><? echo $item_out_new[$row->item_id]; $item_out_fg += $item_out_new[$row->item_id]; ?></td>
			<td align="right"><?php echo $Balance;?> </td>
		...
		<tr align="right" style="font-weight:bold;">
			<td colspan="4">TOTAL</td>
			<td><?=number_format( $item_in_fg,2)?></td>
			<td><?=number_format( $item_out_fg,2)?></td>
			<td></td>
		</tr>
	</tbody>
</table>
...
<?
		...
		// echo $str3;
?>
<h1 style="text-align:center; padding: 5px;"> Finish Goods Receive</h1>
<h4 >Date: <?php echo "$date to $date1" ;?></h4>
...
<table width="100%;"  id="ExportTable" cellspacing="0" cellpadding="2" border="0" >
	...
	<body>
	<?php 
		 $sql = 'SELECT f.receive_no,f.receive_date,f.item_id,i.item_name,f.unit_name,f.receive_qty,f.cost_price,f.total_amt,f.entry_by,u.fname,f.entry_at 

FROM fg_production f, item_info i, user_activity_management u 

WHERE f.receive_date BETWEEN "'.$date.'" AND "'.$date1.'" AND f.item_id=i.item_id AND f.entry_by=u.user_id ';
	...
	<tr>
		...
		<td><?=$row->item_name?> </td>
		<td><?=$row->unit_name ?> </td>
		<td align="right"><?=$row->receive_qty; $receive_total += $row->receive_qty?>  </td>
		<td align="right"><?=$row->cost_price ?>  </td>
		<td align="right"><?=$row->total_amt; $total_amount += $row->total_amt?>  </td>
		<td><?=$row->fname ?>  </td>
		<td><?=$row->entry_at ?>  </td>
	</tr>
	...
	<td align="right"><?=number_format( $receive_total,2)?> </td>
	<td></td>
...
<table width="100%;"  id="ExportTable" cellspacing="0" cellpadding="2" border="0" >
	...
	<body>
	<?php 
		 $sql = 'SELECT  r.issue_date,r.item_id, f.receive_date, f.batch_no, r.batch_no, f.item_id as fg_item, i.item_name,f.unit_name, f.receive_qty, f.cost_price, f.total_amt, f.entry_by, u.fname, f.entry_at 

FROM rm_consumption r left join fg_production f on r.batch_no=f.batch_no, item_info i,  user_activity_management u

WHERE r.issue_date BETWEEN "'.$date.'" AND "'.$date1.'" and r.item_id=i.item_id and r.entry_by=u.user_id ';
	...
	?>
	<tr>
		<td><?=find_a_field('rm_consumption','issue_date','batch_no="'.$row->batch_no.'"');?> </td>
		<td><?=$row->receive_date?> </td>
		<td><?=$row->item_id?> </td>
		<td><?=$row->item_name?> </td>
		<td><?=$row->unit_name ?> </td>
		<td><?=$row->batch_no ?> </td>
		<td><?=find_a_field('rm_consumption','issue_qty','batch_no="'.$row->batch_no.'"');?> </td>
		<td><?=find_a_field('rm_consumption','avg_price','batch_no="'.$row->batch_no.'"');?></td>
		<td><?=$rm_total=find_a_field('rm_consumption','total_amt','batch_no="'.$row->batch_no.'"');?></td>
		<td><?=find_a_field('item_info','item_name','item_id="'.$row->fg_item.'"');?> </td>
		<td align="right"><?=$row->receive_qty; $receive_total += $row->receive_qty?>  </td>
		<td align="right"><?=$row->cost_price ?>  </td>
		<td align="right"><?=$row->total_amt; $total_amount += $row->total_amt?>  </td>
		<td><? echo $rm_total/$row->total_amt?></td>
		<td><?=$row->fname ?>  </td>
		<td><?=$row->entry_at ?>  </td>
	</tr>
...
<?
		...
		// echo $str3;
?>
<h1 style="text-align:center; padding: 5px;"> Raw Material Receive</h1>
<h4 >Date: <?php echo "$date to $date1" ;?></h4>
...
<table width="100%;"  id="ExportTable" cellspacing="0" cellpadding="2" border="0" >
	...
	<?php 
		$sql = 'SELECT p.pr_no,p.rec_date,p.item_id,i.item_name, p.unit_name,p.qty,p.rate,p.amount,p.entry_by,u.fname,p.entry_at
FROM purchase_receive p, item_info i, user_activity_management u 

WHERE p.rec_date BETWEEN "'.$date.'" AND "'.$date1.'" AND p.item_id=i.item_id AND p.entry_by=u.user_id';
	...
	<tr>
		...
		<td><?=$row->item_name ?>  </td>
		<td><?=$row->unit_name ?>  </td>
		<td align="right"><?=$row->qty; $total_qty += $row->qty?>  </td>
		<td  align="right"><?=$row->rate ?>  </td>
		<td  align="right"><?=$row->amount; $total_amount += $row->amount ?>  </td>
		<td><?=$row->fname ?>  </td>
		<td><?=$row->entry_at ?>  </td>
	</tr>
			<? } ?>
...
<table width="100%;"  id="ExportTable" cellspacing="0" cellpadding="2" border="0" >
	...
	<body>
	<?php 
	$sql = 'SELECT r.issue_no,r.issue_date,r.item_id,i.item_name,r.unit_name,r.issue_qty,r.avg_price,r.total_amt,r.entry_by,u.fname,r.entry_at 
	FROM rm_consumption r, item_info i, user_activity_management u 
	WHERE r.issue_date BETWEEN "'.$date.'" AND "'.$date1.'"  AND r.item_id=i.item_id AND r.entry_by=u.user_id';
	...
	<tr>
		...
		<td><?=$row->item_name?> </td>
		<td><?=$row->unit_name ?> </td>
		<td align="right"><?=$row->issue_qty; $issue_total += $row->issue_qty?>  </td>
		<td align="right"><?=$row->avg_price ?> </td>
		<td align="right"><?=$row->total_amt; $total_rm_amt += $row->total_amt;?> </td>
		<td><?=$row->fname ?>  </td>
		<td><?=$row->entry_at ?>  </td>
	</tr>
	...
	<td colspan="5" align="right">Total</td>
	<td align="right"><?=number_format( $issue_total,2)?> </td>
	<td></td>
	<td align="right"><?=number_format( $total_rm_amt,2)?> </td>
	<td></td>
	<td></td>
	</tr>
