...
switch ($_POST['report']) {
    case 1:
		...
		if($_SESSION['user']['depot']==1){
		  $sql='select ji_date,i.item_id,i.finish_goods_code as fg_code,i.item_name,s.sub_group_name as Category,i.unit_name as unit,a.item_in as `IN`,a.item_ex as `OUT`,a.item_price as rate,((a.item_in+a.item_ex)*a.item_price) as amount,a.tr_from as tr_type,a.tr_no,sr_no,(select lc_number from lc_purchase_closing where closing_no=a.tr_no ) as LC_number,(select warehouse_name from warehouse where warehouse_id=a.relevant_warehouse) as warehouse,a.entry_at,c.fname as User 
		   
		   from journal_item a, item_info i, user_activity_management c , item_sub_group s, item_group g where c.user_id=a.entry_by and  g.group_id=s.group_id and  s.sub_group_id=i.sub_group_id and
		    a.item_id=i.item_id '.$date_con.$warehouse_con.$item_con.$status_con.$item_sub_con.$group_id_con.' order by a.id';
		}else{
		 $sql='select ji_date,i.item_id,i.finish_goods_code as fg_code,i.item_name,s.sub_group_name as Category,i.unit_name as unit,a.item_in as `IN`,a.item_ex as `OUT`,a.item_price as rate,((a.item_in+a.item_ex)*a.item_price) as amount,a.tr_from as tr_type,a.tr_no,sr_no,(select lc_number from lc_purchase_closing where closing_no=a.tr_no) as LC_number,(select warehouse_name from warehouse where warehouse_id=a.relevant_warehouse) as warehouse,a.tr_no,a.entry_at,c.fname as User 
		   
		   from journal_item a, item_info i, user_activity_management c , item_sub_group s, item_group g where c.user_id=a.entry_by and  g.group_id=s.group_id and  s.sub_group_id=i.sub_group_id and
		    a.item_id=i.item_id and a.warehouse_id="'.$_SESSION['user']['depot'].'" '.$date_con.$item_con.$status_con.$item_sub_con.$group_id_con.' order by a.id';}
...
{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.po_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
		   ...
		   $sql=' select a.po_no as po_no, a.po_date,(select po_type from purchase_type where id=a.purchase_type) as purchase_type, c.vendor_name as vendor_name,a.vat as vat_percent,a.vat_include as vat_type,a.tax,a.tax_include  as tax_type,a.rebateable,a.rebate_percentage,a.deductible as vds_payable ,a.status, b.id as order_id,d.sub_group_name, e.item_name, b.qty,b.rate,b.with_vat_rate as calculated_rate,b.with_vat_amt as calculated_amount, f.fname as entry_by, a.entry_at
	...
	case 113:
		...
		{$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.ji_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
		 ...
		 $sql='select ji_date as Date,i.item_id,i.finish_goods_code as fg_code,i.item_name,s.sub_group_name as Category,i.unit_name as unit,a.item_in as `Opening Qty`,a.item_price as Opening_rate,((a.item_in+a.item_ex)*a.item_price) as amount,(select warehouse_name from warehouse where warehouse_id=a.warehouse_id) as warehouse,a.entry_at,c.fname as User 
		   
		   from journal_item a, item_info i, user_activity_management c , item_sub_group s, item_group g 
		   where a.tr_from="Opening" and c.user_id=a.entry_by and s.sub_group_id=i.sub_group_id and  g.group_id=s.group_id and
		    a.item_id=i.item_id '.$warehouse_con.$item_con.$status_con.$item_sub_con.$group_id_con.' order by a.id';
	...
	case 20022:
		$report="Stock Position Alert Report";
		if(isset($warehouse_id)) 			{$warehouse_con=' and a.warehouse_id='.$warehouse_id;} 
		if(isset($sub_group_id)) 			{$item_sub_con=' and i.sub_group_id='.$sub_group_id;} 
	    if(isset($group_id)) 			    {$group_id_con=' and s.group_id ='.$group_id;} 
...
$str 	.= '<th style="border: 0px" colspan="8" width="50%" align="center" >';
...
$str 	.= '</th>';
$str 	.= '</tr>';
$str 	.= '<tr><th style="border: 0px" colspan="24">';
$str 	.= '<div class="date" style="margin: 1px; font-size:12px; text-align:right"> <strong>Reporting Time:</strong> '.date("h:i A d-m-Y").'</div>';
$str 	.= '</th></tr>';
...
<h2>Closing Stock As On-<?=$to_date;?></h2>
...
$avg_rate = find_a_field('journal_item', '(sum(item_in*item_price)-sum(item_ex*item_price))/sum(item_in-item_ex)', 'item_id = "'.$data->item_id.'" and warehouse_id="'.$warehouse_id.'" and ji_date <= "'.$to_date.'"');
...
<td style="text-align:right"><?=number_format(($i->final_stock/$data->pack_size),2);?></td>
<td style="text-align:right"><?=$stockk=$i->final_stock?></td>
<td style="text-align:right"><? if($warehouse_id !=''){ echo number_format($ratte=$avg_rate,2);} else{ echo number_format($ratte_all = $avg_rate_all,2); }?></td>
<td style="text-align:right"><? if($warehouse_id !=''){ echo number_format(($tot_value=$stockk*$ratte),2); $total_tot_value += $tot_value;} 
                              else{ echo number_format(($tot_value=$stockk*$ratte_all),2); $total_tot_value += $tot_value; }?>
</td>
</tr>
<?
}
...
<td></td>
<td></td>
<td style="text-align:right"><?=number_format(($total_tot_value),2)?></td>
</tr>
<?
...
{	 
   ...
   if(isset($t_date)) {$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.ji_date <="'.$to_date.'"';}
   		 ...
   		 $stock_warehouse ="select sum(item_in-item_ex) as stock,item_id,a.warehouse_id from journal_item a,warehouse w where a.ji_date<='".$to_date."' and w.warehouse_id=a.warehouse_id and  w.use_type in ('WH','SC')  group by item_id,warehouse_id";
		$query = db_query($stock_warehouse);

		while($data=mysqli_fetch_object($query)){

			$stock[$data->item_id][$data->warehouse_id] = $data->stock;
			$total_stock[$data->item_id]  += $data->stock;
		}
		
		


		$warehouse = "select warehouse_id,warehouse_name from warehouse where use_type in ('WH','SC') and group_for=".$_SESSION['user']['group']." ";
		$query = db_query($warehouse);
		$i=1;
		while($wh_data = mysqli_fetch_object($query)){
			$w++;
			$warehouse_id_l[$w]=$wh_data->warehouse_id;
			$warehouse_name_l[$w] = $wh_data->warehouse_name;
		}
	...
	JOIN 
		(
			...
			FROM journal_item where ji_date<='".$to_date."' and final_price>0
			GROUP BY item_id
		) AS latest_ids
	ON 
	...
	while($row6=mysqli_fetch_object($res6)){
		$war[$row6->item_id]= number_format($row6->latest_final_price,2,'.','');
	}
		  ...
		  $stockSql='select distinct i.item_id,i.unit_name,i.item_name,s.sub_group_name,i.finish_goods_code,i.d_price,i.pack_size,sum(j.item_in-item_ex) as stock
		   from item_info i, item_sub_group s, item_group g,journal_item j
		   where i.group_for="'.$_SESSION['user']['group'].'" and  g.group_id=s.group_id and i.item_id=j.item_id
		   and i.sub_group_id=s.sub_group_id'.$item_sub_con.$item_con.$group_id_con.' group by j.item_id order by i.finish_goods_code,s.sub_group_name,i.item_name';
...
<? for($i=1;$i<=$w;$i++){ ?>
	<th><?=$warehouse_name_l[$i]."-".$warehouse_id_l[$i]?></th>
<? } ?>
...
<? for($j=1;$j<=$w;$j++){ ?>
	<td style="text-align:right"><?=$stock[$data->item_id][$warehouse_id_l[$j]]?></td>
<? } ?>

<td style="text-align:right"><?=number_format($total_stock[$data->item_id],2); $tot_stock +=$total_stock[$data->item_id]; ?></td>
<td style="text-align:right"><?=$war[$data->item_id];?></td>
<td style="text-align:right"><?=$total_amt=($total_stock[$data->item_id]*$war[$data->item_id]); $tot_amt += $total_amt; ?></td>
</tr>
<?
...
{	 
   ...
   if(isset($t_date)) {$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.ji_date <="'.$to_date.'"';}
   		 ...
   		 $stock_warehouse ="select sum(item_in-item_ex) as stock,item_id,a.warehouse_id from journal_item a,warehouse w where a.ji_date<='".$to_date."' and w.warehouse_id=a.warehouse_id and  w.use_type in ('PL')  group by item_id,warehouse_id";
		$query = db_query($stock_warehouse);
		...
		$warehouse = "select warehouse_id,warehouse_name from warehouse where use_type in ('PL') and group_for=".$_SESSION['user']['group']." ";
		$query = db_query($warehouse);
		$i=1;
		while($wh_data = mysqli_fetch_object($query)){
			$w++;
...
<? for($j=1;$j<=$w;$j++){ ?>
	<td><?=$stock[$data->item_id][$warehouse_id_l[$j]]?></td>
<? } ?>

<td><?=$total_stock[$data->item_id]; $tot_stock +=$total_stock[$data->item_id]; ?></td>
<td><?=$war[$data->item_id];?></td>
<td><?=$total_amt=($total_stock[$data->item_id]*$war[$data->item_id]); $tot_amt += $total_amt; ?></td>
</tr>
<?
}
...
$t_date = $_REQUEST['t_date'];
    if($_POST['opening_date']!=''){
        $opening_date_con=' and j.ji_date>="'.$_POST['opening_date'].'"';    
    }
 
if($_POST['item_sub_group']>0) 	$sub_group_id=$_POST['item_sub_group'];
if($_POST['group_id']>0) 	{$item_gcon=' and i.item_group='.$_POST['group_id'];}
if($_POST['item_id']>0) $item_id=$_POST['item_id'];
$date_con = ' and j.ji_date between "'.$f_date.'" and "'.$t_date.'" ';
if(isset($sub_group_id)) 	{$item_sub_con=' and i.sub_group_id='.$sub_group_id;} 
if(isset($item_id)) 		{$item_con=' and i.item_id='.$item_id;} 
if($_POST['receive_status']!=''){
        $tr_con=' and j.tr_from="'.$_POST['receive_status'].'"';   }
if($_POST['group_for']!=''){
        $com_con=' and i.group_for="'.$_POST['group_for'].'"';    
    }else{ 
	$com_con=' and i.group_for="'.$_SESSION['user']['group'].'"';
	
	}
if($_POST['warehouse_id']!=''){
        $wh_con=' and j.warehouse_id="'.$_POST['warehouse_id'].'"';    
    }elseif($_SESSION['user']['level']==5){
	
	 $wh_connnnnnnnnnnnnn=' and i.group_for="'.$_POST['group_for'].'"'; 
	 }
	// else{ 
//	
//	 $wh_sql="SELECT GROUP_CONCAT(DISTINCT w.warehouse_id) as defined_wh FROM `warehouse_define` w where w.user_id='".$_SESSION['user']['id']."' and w.status='enable'";
//	 
...
//	 
//	 if($data->defined_wh !=''){
//	 	$wh_con = ' and j.warehouse_id in ('.$data->defined_wh.','.$_SESSION['user']['depot'].') ';
//	 }else{
//	 
//	 $wh_con = ' and j.warehouse_id in ('.$_SESSION['user']['depot'].') ';
//	 }
//	 
//	}
// opening
   $sql1="select j.item_id as code,i.unit_name,sum(j.item_in - j.item_ex) balance
from journal_item j, item_info i 
where j.item_id=i.item_id
and ji_date < '".$f_date."' ".$item_con.$item_sub_con.$opening_date_con.$wh_con.$com_con.$tr_con." group by i.item_id";
...
// Closing
 $sql2="select j.item_id as code,i.unit_name,sum(j.item_in - j.item_ex) balance
from journal_item j, item_info i 
where j.item_id=i.item_id
and ji_date <= '".$t_date."'  ".$item_con.$item_sub_con.$opening_date_con.$wh_con.$com_con.$tr_con." group by i.item_id";
	
$res = db_query($sql2);
	while($row=mysqli_fetch_object($res))
	{
		$bin_closing[$row->code] = $row->balance;
	}	
// ----------------- IN -------------------------
// ALL purchase	
   $sql3="select j.item_id as code,i.unit_name,sum(j.item_in) balance
from journal_item j, item_info i 
where j.item_id=i.item_id 
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND  j.tr_from IN ('Purchase','Import','Local Purchase','Other Receive','Production Return','Opening') group by i.item_id";
...
// production	
 $sql4="select j.item_id as code,i.unit_name,sum(j.item_in) balance
from journal_item j, item_info i 
where j.item_id=i.item_id 
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND  j.tr_from IN ('Production Receive') group by i.item_id";
...
// section Receive	
 $sql12="select j.item_id as code,i.unit_name,sum(j.item_in) balance
from journal_item j, item_info i 
where j.item_id=i.item_id 
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND  j.tr_from IN ('WIPReceive','Issue') group by i.item_id";
	
$res = db_query($sql12);
	while($row=mysqli_fetch_object($res))
	{
	 $sec_receive[$row->code] = $row->balance;
	}
	
	
// --------- sales return	
$sql5="select j.item_id as code,i.unit_name,sum(j.item_in) balance
from journal_item j, item_info i 
where j.item_id=i.item_id 
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND  j.tr_from IN ('Sales Return') group by i.item_id";
	
$res = db_query($sql5);
	while($row=mysqli_fetch_object($res))
	{
		$sales_return[$row->code] = $row->balance;
	}
	
	
// --------- Transfer IN ------------	
$sql6="select j.item_id as code,i.unit_name,sum(j.item_in) balance
from journal_item j, item_info i 
where j.item_id=i.item_id 
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND  j.tr_from IN ('Transfered','Transit') group by i.item_id";
	
$res = db_query($sql6);
	while($row=mysqli_fetch_object($res))
	{
		$transfer_in[$row->code] = $row->balance;
	}
// ------------------ Other receive	
$sql13="select j.item_id as code,i.unit_name,sum(j.item_in) balance
from journal_item j, item_info i 
where j.item_id=i.item_id 
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND  j.tr_from NOT IN ('Purchase','Import','Local Purchase','receive','Sales Return','Transfered','Transit') group by i.item_id";
...
// -----------------------out --------------------------
// -------------------------------- Consumption	
$sql7="select j.item_id as code,i.unit_name,sum(j.item_ex) balance
from journal_item j, item_info i 
where j.item_id=i.item_id 
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND  j.tr_from IN ('Consumption','Reprocess Issue') group by i.item_id";
...
$res = db_query($sql7);
	...
	// -------------------------------- Sales	
$sql14="select j.item_id as code,i.unit_name,sum(j.item_ex) balance
from journal_item j, item_info i 
where j.item_id=i.item_id 
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND  j.tr_from IN ('Sales') group by i.item_id";
...
// wip_issue	
 $sql8="select j.item_id as code,i.unit_name,sum(j.item_ex) balance
from journal_item j, item_info i 
where j.item_id=i.item_id 
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND j.tr_from IN ('WIPIssue','Issue') group by i.item_id";
	
$res = db_query($sql8);
	while($row=mysqli_fetch_object($res))
	{
		$wip_issue[$row->code] = $row->balance;
	}
// --------- Production Issue ------------	
$sql9="select j.item_id as code,i.unit_name,sum(j.item_ex) balance
from journal_item j, item_info i 
where j.item_id=i.item_id
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND  j.tr_from IN ('Production Issue') group by i.item_id";
	
$res = db_query($sql9);
	while($row=mysqli_fetch_object($res))
	{
		$production_issue[$row->code] = $row->balance;
	}
// --------------------------------- Replace issue	
$sql10="select j.item_id as code,i.unit_name,sum(j.item_ex) balance
from journal_item j, item_info i 
where j.item_id=i.item_id 
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND  j.tr_from  IN ('Replace Issue') group by i.item_id";
	
$res = db_query($sql10);
	while($row=mysqli_fetch_object($res))
	{
		$replace_issue[$row->code] = $row->balance;
	}	
	
	// --------------------------------- damage_issue 	
$sql11="select j.item_id as code,i.unit_name,sum(j.item_ex) balance
from journal_item j, item_info i 
where j.item_id=i.item_id 
and j.ji_date between '".$f_date."' and '".$t_date."'  ".$item_con.$item_sub_con.$wh_con.$com_con.$tr_con." 
AND  j.tr_from  IN ('Damage Issue') group by i.item_id";
	
$res = db_query($sql11);
	while($row=mysqli_fetch_object($res))
	{
		$damage_issue[$row->code] = $row->balance;
	}		
...
<table id="ExportTable" width="100%" border="0" cellspacing="0" cellpadding="2">
<thead>
<tr>
<td style="border:0px;" colspan="24">
<div class="header">
...
<tr>
  ...
  <th bgcolor="#009999">Store Receive</th>
  <th bgcolor="#009999">Section Receive</th>
<th bgcolor="#009999">Production Receive</th>
<th bgcolor="#009999">Total Receive</th>
  
<th bgcolor="#FF6699">Sales</th>
<th bgcolor="#FF6699">Consumption</th>
<th bgcolor="#FF6699">Store Return </th>
<th bgcolor="#FF6699">WIP Issue</th>
<th bgcolor="#FF6699">Issue To Replace</th>
<th bgcolor="#FF6699">Issue To Damage</th>
<th bgcolor="#FF6699">Total Issue</th>
  <th>Closing</th>
  <th>Bin Closing </th>
  <th>Diff</th>
...
<tbody>
<?
  $sql_item="SELECT item_id as code,i.finish_goods_code as fg_code,i.sub_group_id,s.sub_group_name,i.item_name,i.unit_name,i.group_for, u.group_name
FROM  item_info i, item_sub_group s, user_group u
where i.sub_group_id=s.sub_group_id and u.id=i.group_for
".$item_con.$item_sub_con.$item_gcon."
order by i.sub_group_id,i.item_name";
$query = db_query($sql_item);
while($data= mysqli_fetch_object($query)){ 
$in_total = $opening[$data->code] + $purchase[$data->code] + $production[$data->code]+ $sec_receive[$data->code];
$out_total = $sales[$data->code] +$consumption[$data->code] + $production_issue[$data->code] + $wip_issue[$data->code] + $replace_issue[$data->code]+ $damage_issue[$data->code];
...
<tr>
  ...
  <td><?=$data->item_name?></td>
  <td style="text-align:center"><?=$data->unit_name?></td>
  <td style="text-align:right"><?=$opening[$data->code]?></td>
  <td style="text-align:right"><?=$purchase[$data->code]?></td>
  
  <td style="text-align:right"><?=$sec_receive[$data->code]?></td>
  <td style="text-align:right"><?=$production[$data->code]?></td>
  
  <?php /*?><td><?=$sales_return[$data->code]?></td>
  <td><?=$transfer_in[$data->code]?></td>
  
  <td><?=$other_receive[$data->code]?></td><?php */?>
  <td style="text-align:right"><?=number_format($in_total,4);?></td>
  <td style="text-align:right"><?=$sales[$data->code]?></td>
  <td style="text-align:right"><?=$consumption[$data->code]?></td>
  <td style="text-align:right"><?=$production_issue[$data->code]?></td>
  <td style="text-align:right"><?=$wip_issue[$data->code]?></td>
  <td style="text-align:right"><?=$replace_issue[$data->code]?></td>
  <td style="text-align:right"><?=$damage_issue[$data->code]?></td>
  <td style="text-align:right"><?=number_format($out_total,4);?></td><?php $closing = $in_total - $out_total; ?>  
  <td style="text-align:right"><?=number_format($closing,4);?></td>
  <td style="text-align:right"><? if($_SESSION['user']['level']==5) echo $bin_closing[$data->code]; else echo $bin_closing[$data->code]=0;?></td>
  <td style="text-align:right"><?=number_format(($closing-$bin_closing[$data->code]),4);?></td>
  
</tr>
<? 
$total_opening += $opening[$data->code];
$total_purchase += $purchase[$data->code];
$total_sec_receive += $sec_receive[$data->code];
$total_production += $production[$data->code];
//$total_sales_return+=$sales_return[$data->code];
//$total_transfer_in+=$transfer_in[$data->code];
//$total_other_receive += $other_receive[$data->code];
$total_in_total += $in_total;
$total_sales += $sales[$data->code];
$total_consumption += $consumption[$data->code];

$total_production_issue += $production_issue[$data->code];
$total_wip_issue += $wip_issue[$data->code];
$total_replace_issue += $replace_issue[$data->code];
$total_damage_issue += $damage_issue[$data->code];
$total_out_total += $out_total;
$total_closing += $closing;
$total_bin_closing += $bin_closing[$data->code];
}} ?>
<tr>
  ...
  <td bgcolor="#99CC99">&nbsp;</td>
  <td bgcolor="#99CC99">&nbsp;</td>
  <td bgcolor="#99CC99" style="text-align:right"><strong>Total :</strong></td>
  <td bgcolor="#99CC99">&nbsp;</td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_opening,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_purchase,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_sec_receive,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_production,4);?></strong></td>
  <?php /*?><td bgcolor="#99CC99"><strong><?=$total_sales_return;?></strong></td>
  <td bgcolor="#99CC99"><strong><?=$total_transfer_in;?></strong></td>
  <td bgcolor="#99CC99"><strong><?=$total_other_receive;?></strong></td><?php */?>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_in_total,4);?></strong></td>
  
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_sales,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_consumption,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_production_issue,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_wip_issue,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_replace_issue,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_damage_issue,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_out_total,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_closing,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><strong><?=number_format($total_bin_closing,4);?></strong></td>
  <td bgcolor="#99CC99" style="text-align:right"><? number_format(($total_closing-$total_bin_closing),4); 
  							$closingone = number_format($total_closing, 4, '.', '');
							$closingtwo = number_format($total_bin_closing, 4, '.', '');
							
							echo $closingone-$closingtwo;
  ?></td>
...
{	 if($item_id>0){
   ...
   if(isset($t_date)) {$to_date=$t_date; $fr_date=$f_date; $date_con=' and a.ji_date <="'.$to_date.'"';}
		...
		$stockSql='select distinct i.item_id,i.unit_name,i.item_name,s.sub_group_name,i.finish_goods_code,i.d_price,i.min_stock
		   ...
		   i.sub_group_id=s.sub_group_id and g.group_id=s.group_id '.$item_sub_con.$item_con.$group_id_con.' order by i.finish_goods_code,s.sub_group_name,i.item_name';
...
<h2><?=$report?></h2>
<h2><?=$warehouse_name?></h2>
<h2>Closing Stock of Date-<?=$to_date?></h2>
...
$avg_rate = find_a_field('journal_item', '(sum(item_in*item_price)-sum(item_ex*item_price))/(sum(item_in)-sum(item_ex))', 'item_id = "'.$data->item_id.'" and warehouse_id="'.$warehouse_id.'" and ji_date BETWEEN "'.$fr_date.'" and "'.$to_date.'"');

$avg_rate_all = find_a_field('journal_item', '(sum(item_in*item_price)/sum(item_in) )', 'item_id = "'.$data->item_id.'" and ji_date BETWEEN "'.$fr_date.'" and "'.$to_date.'"');
...
<td><?=$data->item_name?></td>
<td style="text-align:center"><?=$data->unit_name?></td>
<td style="text-align:right"><?=$data->min_stock?></td>
<?
...
$sql = "select m.do_no,m.do_date,m.dealer_code,m.depot_id, i.item_id,i.unit_name,i.pack_size, o.unit_price, o.total_unit,d.product_group as grp,concat(i.finish_goods_code,'- ',item_name) as item_name, o.id from 
...
where m.do_no=o.do_no and i.item_id=o.item_id and m.group_for='".$_SESSION['user']['group']."' and m.dealer_code=d.dealer_code and m.status in ('CHECKED','COMPLETED') ".$date_con.$item_con.$depot_con.$dtype_con.$dealer_con.$item_brand_con;
...
?>
    ...
    <tr>
      ...
      <td style="text-align:right"><?=number_format($info->total_unit); $tot_total_unit+=$info->total_unit;?></td>
      <td style="text-align:right"><?=number_format($ch_qty); $total_ch_qty+=$ch_qty;?></td>
      <td style="text-align:right"><?=number_format($due_qty); $total_due_qty+=$due_qty;?></td>
...
<? } ?>
    ...
    <tr>
      ...
      <td style="text-align:right"><?=number_format($tot_total_unit);?></td>
      <td style="text-align:right"><?=number_format($total_ch_qty);?></td>
      <td style="text-align:right"><?=number_format($total_due_qty);?></td>
