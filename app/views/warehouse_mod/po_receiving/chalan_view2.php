...
<div class="container">
	<div class="row">
		...
		<div class="col-2 text-center">
			<img src="<?=SERVER_ROOT?>public/uploads/logo/<?=$c_id?>.png"  width="100%" />
		</div>
		
		<div class="row align-items-center text-center" style="margin-bottom: 10px;">
    <!-- Left Spacer or Logo (optional) -->
    <div class="col-2"></div>

    <!-- Center Section: Company Info -->
    <div class="col-8">
        <h1 style="font-family:Tahoma; margin-bottom:5px;">
            <?=$group_data->group_name?>
        </h1>
        <!--<h5 style="letter-spacing:1px;">Quality product at affordable cost</h5>-->
        <p style="margin:0;"><?=$group_data->address?></p>
        <p style="margin:0;">
            Cell: <?=$group_data->mobile?> | Email: <?=$group_data->email?>
        </p>
        <p style="margin:0;"><?=$group_data->vat_reg?></p>
    </div>
    ...
    <div class="col-2 text-end">
        ...
        $req_no_qr_data = url_encode($pr_no); // Change variable as needed
        $print_url = "https://saaserp.ezzy-erp.com/app/views/warehouse_mod/po_receiving/chalan_view2.php?c=" . rawurlencode($company_id) . "&v=" . rawurlencode($req_no_qr_data);
...
<?php /*?><td><a href="../../../../app/views/purchase_mod/po/po_print_view.php?po_no=<?=rawurlencode(url_encode($po_no));?>" target="_blank"><?=$po_no; ?></a></td><?php */?>

<td><a href="../../../../app/views/purchase_mod/po/po_print_view.php?c=<?=rawurlencode(url_encode($c_id));?>&v=<?=rawurlencode(url_encode($po_no));?>" target="_blank"><?=$po_no; ?></a></td>
</tr>
...
<? for($i=0;$i<$pi;$i++){?>
      ...
      <tr style="text-align:center;">
		...
		<!--<td><?=number_format($rate[$i],2)?></td>-->
          <td><?php 
		  $po_qty=find_a_field('purchase_invoice','sum(qty)','po_no="'.$po_no.'" and item_id="'.$item_id[$i].'"');
		echo $unit=number_format($po_qty,2)."<br>"; 
		 ?></td>
        <td><?php 
...
<? }?>
  ...
  <tr><td colspan="5"><div align="right"><strong>Total Qty: </strong></div></td>
