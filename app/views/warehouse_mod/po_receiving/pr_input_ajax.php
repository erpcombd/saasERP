...
$config_ledger = find_all_field('config_group_class','sales_ledger',"group_for=".$_SESSION['user']['group']);
...

		{
...

			{
				...
				$pr_qty = find_a_field('purchase_receive','sum(qty)','pr_no="'.$pr_no.'" and item_id="'.$data->item_id.'"');
				$serial_check = find_a_field('secondary_journal_item','sum(item_in)','serial_no="'.$serial_no.'" and warehouse_id="'.$_SESSION['user']['depot'].'"');
...
$q = "INSERT INTO `purchase_receive` (`pr_no`, `po_no`, `order_no`, `rec_no`,`rec_date`, `vendor_id`, `item_id`, `warehouse_id`, `rate`, `qty`, `unit_name`, `amount`, `qc_by`, `entry_by`, `entry_at`,ch_no,serial_no,receive_note) VALUES('".$pr_no."', '".$po_no."', '".$data->id."', '".$rec_no."','".$rec_date."',".$vendor_id.", ".$item_id.",".$warehouse_id.", ".$rate.", '".$qty."', '".$unit_name."',  '".$amount."', '".$qc_by."',  '".$_SESSION['user']['id']."', '".$now."', '".$ch_no."','".$serial_no."','".$receive_note."')";

mysql_query($q);

$xid = mysql_insert_id();

//secondary_journal_item_control($data->item_id ,$warehouse_id,$rec_date,$qty,0,'Purchase',$xid,$rate,'',$pr_no);
$secondary_journal_item_sql = 'insert into secondary_journal_item (`ji_date`,`item_id`,`warehouse_id`,`lot_no`,`serial_no`,`item_in`,`item_price`,`final_price`,`tr_from`,`tr_no`,`sr_no`,`entry_by`,`entry_at`,`primary_id`) value("'.$rec_date.'","'.$item_id.'","'.$warehouse_id.'","'.$pr_no.'","'.$serial_no.'","'.$qty.'","'.$rate.'","'.$rate.'","Purchase","'.$po_no.'","'.$pr_no.'","'.$_SESSION['user']['id'].'","'.date('Y-m-d h:i:s').'","'.$xid.'")';
mysql_query($secondary_journal_item_sql);
...
$res='select p.id,p.pr_no,p.item_id as item_code,i.item_name,i.unit_name,p.qty,p.serial_no as item_serial_no,p.id as action from purchase_receive p,item_info i where i.item_id=p.item_id and p.journal="Pending" and p.warehouse_id="'.$_SESSION['user']['depot'].'" and po_no="'.$po_no.'"';
