...
{		
		...
		$sql = 'select * from requisition_order where req_no = '.$$unique;
		$query = db_query($sql);
		$_POST['requisition_qty_'.$data->id];
		while($data=mysqli_fetch_object($query)){

			if(($_POST['requisition_qty_'.$data->id]>0)){
			
				$qty=$_POST['requisition_qty_'.$data->id];
				
				$remarks=$_POST['requisition_remarks_'.$data->id];
				
				 $inv ='update requisition_order SET qty = "'.$qty.'", remarks = "'.$remarks.'"  where id='.$data->id;	
					db_query($inv);	
			}
		}
		
		
			  $note_sql = 'insert into approver_notes_new(`master_id`,`type`,`note`,`entry_at`,`entry_by`) value("'.$$unique.'","MR","'.$remarks.'","'.date('Y-m-d H:i:s').'","'.$_SESSION['user']['id'].'")';
		db_query($note_sql);
...
// page where you want the current tab to redirect after opening new tab
...
echo "<script>
        ...
        // Redirect current tab to another page
			...
			<div class="container-fluid bg-form-titel">
				<div class="row">
							...
							<div class="form-group row m-0 pb-1">
								...
								<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
        							...
        							<select name="warehouse_id" id="warehouse_id"  class="form-control">
										<? foreign_relation('warehouse','warehouse_id','warehouse_name',$warehouse_id,'1 and warehouse_id>1 and group_for='.$_SESSION['user']['group'].'') ;?>
           							  </select>
					...
					<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
						...
						<div class="form-group row m-0 pb-1">
								...
								<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2">
									...
									<datalist id="dept">
									<? foreign_relation('department','DEPT_DESC','DEPT_DESC',$req_for,'DEPT_id>0 order by DEPT_DESC asc');?>
									</datalist>
							...
							<div class="form-group row m-0 pb-1">
								...
								<label for="<?=$field?>" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Company Name :</label>
					...
					<option></option>
							  ...
							  <? foreign_relation('user_group','id','company_name',$group_for,'1 and id>1');?>
    ...
    <div class="d-flex justify-content-center pt-5">
        <div class="n-form1 fo-white pt-0 p-0">
			...
			<table class="table1  table-striped table-bordered table-hover table-sm">
					...
					<?php /*?>	<td rowspan="2" align="center" ><a href="mr_print_view.php?req_no=<?=$req_no?>" target="_blank"><img src="../../../../public/assets/images/print.png" width="36" height="36" /></a></td><?php */?>
						
						
							<td rowspan="2" align="center">
  <a href="mr_print_view.php?c=<?=rawurlencode(url_encode($c_id))?>&v=<?=rawurlencode(url_encode($req_no))?>" target="_blank">
    <img src="../../../../public/assets/images/print.png" width="36" height="36" />
  </a>
</td>
		...
		<form action="" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
			...
			$res='select a.id,a.remarks,b.item_name as item_name,a.specification,a.qoh as stock_qty,a.last_p_qty as last_pur_qty,a.last_p_date as last_pur_date,a.qty,"x" from requisition_order a,item_info b where b.item_id=a.item_id and a.req_no='.$req_no;
			...
			?>
					...
					<tr>
						...
						<td><input name="requisition_qty_<?=$row->id?>" type="text" id="requisition_qty_<?=$row->id?>"  class="text-center" value="<?=$row->qty?>"/></td>
						<td><input name="requisition_remarks_<?=$row->id?>" type="text" id="requisition_remarks_<?=$row->id?>"  class="text-center" value="<?=$row->remarks?>"/></td>
			...
			<div class="container-fluid p-0 ">
				...
				<div class="n-form-btn-class">
					   ...
					   <input name="cancel"  type="submit" class="btn1 btn1-bg-cancel" value="Cancel"  />
						<!--<input type="button" class="btn1 btn1-bg-help" value="Cancel"  onclick="window.location.href='select_unapproved_mr.php'" />--></td>
				<input type="hidden" name="confirm_remarks" id="confirm_remarks">
					  <input name="confirmm" type="submit" class="btn1 btn1-bg-submit" onclick="confirm_function()" value="CONFIRM AND FORWARD REQUSITION" />
...
function confirm_function() {
  var notes = prompt("Why Confirm This Requisition?","");
  if (notes!=null) {
