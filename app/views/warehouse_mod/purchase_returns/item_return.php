...
header("Location: item_return.php?or_no=$_POST[$unique]");
}
...
<div class="row ">
        ...
        <input  name="receive_type" type="hidden" id="receive_type" value="<?=$page_for?>"  required="required"/>
		  ...
		  <div class="col-md-3 form-group">
			...
			<option value="<?=$vendor?>"><? if($vendor_name=='') echo find_a_field('vendor','concat(vendor_name,"(",vendor_id,")")','vendor_id='.$vendor); else echo $vendor_name;?></option>
			</select>
			...
			<? //$field='vendor_name';   ?>
            <label for="<?=$field?>">Vendor Name  : </label>
            <input  name="<?=$field?>" class="form-control"  type="text" id="<?=$field?>" value="<? if($vendor_name=='') echo find_a_field('vendor','concat(vendor_name,"(",vendor_id,")")','vendor_id='.$vendor); else echo $vendor_name;?>" readonly />
          </div>-->
...
<form action="select_vendor_return.php" method="post" name="cz" id="cz" onSubmit="if(!confirm('Are You Sure Execute this?')){return false;}">
