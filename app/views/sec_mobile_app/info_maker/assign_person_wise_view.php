...
<!--        <div class="calendar bg-theme shadow-xl rounded-m">
        ...
        <div class="calendar bg-theme shadow-xl rounded-m">
            <div class="cal-footer">
                <h6 class="cal-sub-title uppercase bold bg-highlight color-white">Assign Person  Schedule Box</h6>
...
// Array of assign_person IDs to search for
$searchIds = $_POST['assign_person'];

// Dynamically create the FIND_IN_SET conditions
$findInSetConditions = [];
foreach ($searchIds as $id) {
    $findInSetConditions[] = "FIND_IN_SET('$id', assign_person) > 0";
}

// Join conditions with OR
$conditionsStr = implode(' OR ', $findInSetConditions);

// Construct the final SQL query
$sqlRemainder = "SELECT * FROM crm_task_add_information WHERE ($conditionsStr) AND status != 'cancelled' ORDER BY `task_id` DESC;";

// Execute the query
$resultRemainder = $conn->query($sqlRemainder);

if ($resultRemainder->num_rows > 0) {
    // Output data of each row
    while($row = $resultRemainder->fetch_assoc()) {
        echo "Task ID: " . $row["task_id"]. " - Assign Person: " . $row["assign_person"]. "<br>";
    }
} else {
    echo "0 results";
}
...
?>
            ...
            echo $sqlRemainder= "SELECT * FROM crm_task_add_information a WHERE assign_person IN ($ffffff) AND status != 'cancelled' ORDER BY `task_id` DESC;";
