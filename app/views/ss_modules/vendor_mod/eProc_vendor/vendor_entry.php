...
<?php if($_SESSION['event_end_msg']!=''){ ?>
 <div class="alert alert-danger" role="alert">
  <?php echo $_SESSION['event_end_msg'];?>
</div>
<?php unset($_SESSION['event_end_msg']); } ?>
...
<form action="update_pass.php" method="post">
					<button class="btn1 btn1-bg-hrm" type="submit">Change password</button>
	</form>
...
<div class="d-flex justify-content-end d-flex-bg-color">
	...
	<div style=" padding: 5px; ">
		...
		<div class="col-sm-10">
		        <select id="search_with" name="search_with" class="form-control" style="height: 28px !important;padding-top: 4px; ">
					...
					<option value="rfq_no">Event ID </option>
      			</select>
		</div>
	  </div>
  </div>
	<div style=" padding: 5px; ">
		...
		<input type="text" name="search_box" id="search_box" class="form-control" placeholder="Search" aria-label="Search" aria-describedby="basic-addon1" style="height: 28px !important;padding-top: 2px; border-right: none; ">
		  <button type="submit" class="input-group-prepend" style=" width: 25%; height: 27px !important; border: none; border-radius: 0px 5px 5px 0px; background-color: white; " onclick="get_event(document.getElementById('search_with').value,document.getElementById('search_box').value)"> Search
		  </button>
		</div>
	  <!-- </form> -->
	</div>
  	...
  	<table class="table1  table-striped table-bordered table-hover table-sm">
	            <caption></caption>
                    ...
                    <tr class="bgc-info">
                        ...
                        <th scope="col" scope="col">Start Date and Time</th>
						<th scope="col" scope="col">End Date and Time</th>
					...
					$eventEndAtvendor = $data->eventEndDate." ".$data->eventEndTime;
						if($data->status=='COMPLETE'){
						$status = 'Completed';
						}elseif($data->status=='CANCELED'){
						$status = 'Cancelled';
						}elseif($eventEndAtvendor<=date('Y-m-d H:i:s')){
						$status = 'Evaluation';
						}elseif($data->status=='MANUAL'){
						$status = 'Live';
						}elseif($data->status=='CHECKED'){
						$status = 'Live';
					...
					?>
					    <tr>
							...
							<td><?=$data->eventStartDate?> <?=$data->eventStartTime?></td>
							<td><?=$eventEndAtvendor;?></td> 
							<td><?=$status?></td> 
							<td><?=$data->rfx_stage?> </td>
							<td><?=$response_data = find_a_field('rfq_vendor_response','count(id)','vendor_id="'.$vendor.'" and rfq_no="'.$data->rfq_no.'" and status = "SUBMITED" ')?></td>
							<td>
								<?php if($eventStart<=$startString && $eventEnd>=$nowString && $data->status=="CHECKED" && $data->reject_status=="No"){
									  if($data->multiple_response=='no' && $response_data>0){
									  		echo 'Multiple response not allowed for this event';
									  }else{
								 ?>
...
function get_event(search_with,search_text){
	// if(search_with!="ALL"){
	 getData2('get_event_ajax.php','table_show_div',search_with,search_text);
