    ...
    LIMIT 1;';
	 ...
	 // if($general_check!='' || $_SESSION['user_role']=='Owner'){
		  ...
		  if($_SESSION['user_role']=='Owner'){
				...
				$gSql = 'select * from rfq_section_evaluation_team where user_id='.$_SESSION['user']['id'].' and rfq_no="'.$_SESSION['rfq_no'].'"  ';
				$gSql_query = db_query($gSql);
				while ($gSql_res = mysqli_fetch_object($gSql_query)) {
					$subfolders[] = strtolower($gSql_res->action);
				}
		  }
              ...
              $file_doc_section_id = $res2->tr_from;
                ...
                if (file_exists($file_path)) {
					...
					if($_SESSION['user_role']=='Owner'){
						...
						$general_check = find_a_field('rfq_section_evaluation_team','action','user_id="'.$_SESSION['user']['id'].'" and rfq_no="'.$_SESSION['rfq_no'].'" and action like "'.$section_type.'" ');
						
						if($general_check !='' ){
							$subfolder = $section_type;
						}
					}
