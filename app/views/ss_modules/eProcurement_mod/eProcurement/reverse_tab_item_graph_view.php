...
$(document).ready(function() {
    var itemId = <?= isset($_GET['item_id']) ? $_GET['item_id'] : 'null'; ?>; // Get item_id from $_GET

    if (itemId !== null) {
        bidconsoletab(itemId);  // Call the function with itemId when the page is loaded
    }
});
...
function bidconsoletab(itemId) {
    ...
    $.ajax({
        url: '../api/bid_seected_item_graph_api.php',
        type: 'POST',
        dataType: 'json',
        ...
        success: function(response) {
            ...
            response.vendors.forEach(function(vendor) {
                ...
                datasets.push({
                    ...
                    borderColor: getUniqueRandomDarkColor(), // Use a unique random dark color for each line
...
function getUniqueRandomDarkColor() {
    ...
    } while (usedColors.includes(color)); // Regenerate if the color has been used
    usedColors.push(color); // Add the new color to the array
    return color;
}
