...
$extractedid = explode("#",$data[1]);
$id=$extractedid[0];
 $doc_id=$extractedid[1];


if($rfq_no>0 && $id>0){
$del = 'delete from rfq_documents_information where documents_id="'.$id.'"';
db_query($del);

$count_sql = 'SELECT COUNT(*) AS count FROM rfq_documents_information WHERE section_id="'.$doc_id.'"';
$count_qry = db_query($count_sql);
$count_data = mysqli_fetch_assoc($count_qry);
$count = $count_data['count'];

if ($count == 0) {
  $del_doc_details = 'delete from rfq_doc_details WHERE id="'.$doc_id.'"';
    db_query($del_doc_details);
}
...
$sql = 'select * from rfq_doc_details where 1 and rfq_no="'.$rfq_no.'" and section_type="'.$type.'"';
		 $qry = db_query($sql);
		...
		<div class="row m-0 p-0 pt-4">
			...
			?>
				...
				<a href="../../../../../assets/support/api_upload_attachment_show.php?name=<?=$att_data->new_name?>&&folder=doc_section" target="_blank" rel="noopener">
											...
											<button type="button" style=" display: inline !important;" class="border-0" onclick="event_details_att_cancel_individual('<?=$type?>',<?=$att_data->documents_id?>,'<?=$show_id?>',<?=$doc->id?>)">
		...
		class="btn2 btn1-bg-cancel" onclick="event_details_att_cancel('<?=$type?>',<?=$doc->id?>,'<?=$show_id?>')">Delete</button>
