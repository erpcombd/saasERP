...
<h1 class="container" style=" font-size: 30px !important; ">Alternative Mail Setup</h1>
	...
	<div class="col-sm-4 col-md-4 col-lg-4"></div>
		<div class="col-sm-4 col-md-4 col-lg-4">
			...
			<table  class="w-100">
				<caption></caption>
					<tr class="tr">
						<td class="td1 fs-14 bold">Vendor Name</td>
						<td class="td2">
							...
							<select class="m-0" name="vendor_id" required type="text" id="vendor_id" tabindex="1">
							<option></option>
					...
					<tr class="tr">
						<td class="td1 fs-14 bold">Emai;</td>
						<td class="td2">
							
							<input class="m-0" name="email" required type="text" id="email" tabindex="1"  value=""  autocomplete="off" />
...
<table class="table1  table-striped table-bordered table-hover table-sm">
    <caption></caption>
				...
				<tr class="bgc-info">
						...
						<th scope="col" scope="col">Alternative Emails</th>
				...
				$td='select a.vendor_id,  a.vendor_name as vendor_name,   a.email, a.contact_person_name, a.language from vendor a, user_group u
				where   a.group_for=u.id  and a.group_for="'.$_SESSION['user']['group'].'"    '.$con.' order by a.vendor_id ';

				$report=db_query($td);

				while($rp=mysqli_fetch_row($report)){$i++; if($i%2==0){$cls=' class="alt"';} else {$cls='';}
				 $count_Sql=find_a_field('rfq_alternative_mail','count(vendor_id)','vendor_id="'.$rp[0].'"');
				 if($count_Sql){
				?>
					...
					<tr<?=$cls?>  bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
						...
						<td>
						<?$sql_alternative = 'SELECT * FROM `rfq_alternative_mail` WHERE vendor_id="'.$rp[0].'"';
