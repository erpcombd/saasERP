...
$target_url = 'sales_order_print_view_mamun.php';
        ...
        <div class="container-fluid pt-5 p-0 ">
                <table class="table1  table-striped table-bordered table-hover table-sm">
                   ...
                   <? 
					...
					where 
					  m.group_for=u.id  and m.dealer_code=d.dealer_code  and m.status not in ('MANUAL') and m.do_no=c.do_no ".$group_for_con.$con.$dealer_con."  group by c.do_no order by m.do_no desc";
					$query = db_query($res);
					...
					?>
                        <tr> 
							...
							<td><?php
							if($data->status=='PROCESSING' && $data->status !='MANUAL'){
							echo "<span style='color:red;font-weight:bold;'>Accounts Approval Pending</span>";
							}
							elseif($data->status=='UNCHECKED' ){
							echo "<span style='color:red;font-weight:bold;'>Audit Approval Pending</span>";
							}
							elseif($data->status=='INVOICE' ){
							echo "<span style='color:red;font-weight:bold;'>Final Approval Pending</span>";
							}
