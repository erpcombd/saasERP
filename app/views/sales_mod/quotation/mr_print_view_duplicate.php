...
<table width="95%" class="tabledesign" border="1" bordercolor="#000000" cellspacing="0" cellpadding="3">
       <tr>
        <td align="center" ><strong>SL</strong></td>
        <td align="center" colspan="<? if($all->origin==0) echo 2;  ?>" ><strong>Item Name </strong></td>
...
$pi=0;
$total=0;
$sql2="select * from sale_requisition_details where  do_no='$req_no'";
$data2=db_query($sql2);
//echo $sql2;
...
?>
      <tr>
        <td valign="top"><?=$sl?></td>
        <td align="left" valign="top" colspan="<? if($all->origin==0) echo 2;  ?>"><?=$info->item_description?></td>
        <td align="center" valign="top"><?=$brand?></td>
		<? if($all->origin==1) {?>
        <td align="center" valign="top"><?=$info->origin_detail?></td>
		<? } ?>
	  ...
	  <? if($all->discount >0){ ?>
	  <tr>
	  	<td colspan="6" style="text-align:right">Discount (<?=$all->discount?> %) &nbsp;</td>
	  ...
	  <tr>
	  	<td colspan="6" style="text-align:right">Deduct &nbsp;</td>
	  ...
	  <tr>
	   ...
	   <? if($all->vat >0){ ?>
	  <tr>
	  	<td colspan="6" style="text-align:right">VAT (<?=$all->vat?> %) &nbsp;</td>
	  ...
	  <? } ?>
	   ...
	   <? if($all->ait >0){ ?>
	  <tr>
	  	<td colspan="6" style="text-align:right">AIT( <?=$all->ait?>%) &nbsp;</td>
	  ...
	  <? } ?>
	   ...
	   <? if($all->vat_ait >0){ ?>
	  <tr>
	  	<td colspan="6" style="text-align:right">VAT & AIT(<?=$all->vat_ait?>%) &nbsp;</td>
  ...
  <tr>
          ...
          <ul>
            ...
            <li>Payement Term: 30 days from the date of receipt of materials at the end.</li>
            <li>The Above price <? if($all->is_included==1){ echo "is included"; if($all->vat > 0){ echo " Vat";} if($all->ait > 0){ echo ",Ait";} if($all->vat_ait > 0){ echo ".Vat & Ait";} }else { echo "is excluding of any Vat,Tax";}?></li>
...
<p>Yours sincerely</p>
<? $entry_info =find_all_field('user_activity_management','','user_id='.$all->entry_by);?>
<p><?=$entry_info->fname .', '.$entry_info->mobile;?></p>	
</div>
