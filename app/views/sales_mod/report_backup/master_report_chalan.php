...
$company_name = find_a_field('user_group','group_name','id="'.$_SESSION['user']['group'].'"');
...
if(isset($area_id)) 		{$area_con=' and d.area_code="'.$area_id.'"';}
 ...
 $sqls="select c.chalan_no,m.do_no,c.driver_name as serial_no,c.chalan_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,w.warehouse_name as depot,sum(total_amt) as total_amt from 
...
where  m.do_no=c.do_no and m.dealer_code=d.dealer_code and w.warehouse_id=c.depot_id  ".$depot_con.$date_con.$pg_con.$dealer_con.$dealer_type_con.$area_con.$zone_con.$region_con." group by chalan_no order by c.chalan_no";
