                  ...
                  <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
						...
						<table width="100%" border="0" cellspacing="0" align="center" cellpadding="0" style="font-size:15px; margin:0; padding:0;">
									...
									<tr>
									<td>
						  	          <div align="left"><img src="<?=SERVER_ROOT?>public/uploads/logo/<?=$_SESSION['user']['group']?>.png"   style=" width:75%;"  />
...
<th width="14%" bgcolor="#CCCCCC">S Product</th>
<th width="14%" bgcolor="#CCCCCC">S Company</th>
<th width="14%" bgcolor="#CCCCCC">Code</th>
<th width="33%" bgcolor="#CCCCCC">Item Description</th>
<th width="12%" bgcolor="#CCCCCC">Unit</th>
<th width="12%" bgcolor="#CCCCCC">Unit-Price</th>
<th width="13%" bgcolor="#CCCCCC">UP After Discount</th>
...
//, (a.init_pkt_unit*a.unit_price) as Total,(a.init_pkt_unit-a.inStock_ctn) as Shortage
  ...
  $res='select  s.sub_group_name, b.finish_goods_code,  b.item_name, a.*,o.item_name as s_item_name,o.company
   from sale_do_details a left join item_info_other o on o.item_id=a.c_item_id, item_info b, item_sub_group s 
...
<td><?=number_format($grandTot=$data->do_price*$data->dist_unit,2); $tot_total_amt +=$grandTot;?></td>
</tr>
        
        <? 
		$totDo+=($data->do_price-$data->unit_price)*$data->dist_unit;
		 } ?>
        <tr>
...
<tr>
          <td colspan="7"><div align="right"><strong> Discount: </strong></div></td>
          <td>
		  
		  <strong>
 <? // number_format($discount_amt= ($tot_total_amt*$master->discount)/100,2); ?>
 
 <?=number_format($totDo,2) ?>
</strong>		 </td>
        </tr>

        <tr>
          <td colspan="7"><div align="right"><strong> Invoice Amount :</strong></div></td>
          <td>
		  <strong>
 <?=number_format($invoice_amt= ($tot_total_amt-$totDo),2); ?>
</strong>		  </td>
        </tr>
        <tr>
          <td colspan="8"><span class="style8" style="font-size:14px; font-weight:500; letter-spacing:.3px;">In Word:
             <? $scs =  $invoice_amt;
