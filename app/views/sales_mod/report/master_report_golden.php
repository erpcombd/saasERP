...
switch ($_REQUEST['report']) {
	...
	break;
	case 7111:
	$report="Sales Order Wise Delivery Report";
	break;
	case 1100:
...
$sql2='select 
	...
	case 20231:
	$report="Customer Wise Work Order & Chalan Report";
	break;
...
$sql="select m.do_no, m.po_no,i.item_name,s.unit_name,s.unit_price,s.total_unit,m.do_date,m.entry_at ,m.entry_time,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,
...
<?php /*?><td><?=$data->depot?></td><?php */?>
<?php /*?><td style="text-align:right"><?=$data->rcv_amt?></td>
<td style="text-align:right"><?=$data->acc_amt?></td>
<td><?=$data->Payment_Details?></td><?php */?>

<td><?=$data->unit_name?></td>
<td align="right"><?=$data->unit_price?></td>
<td align="right"><?=$data->total_unit?></td>
<td align="right"><?=number_format($info[0],2);?></td>
</tr>
<? } ?>
<tr>
<td colspan="9" align="right" style="font-weight:bolder">Total Amount</td>
<?php /*?><td style="text-align:right"><?=number_format($rcv_t,2)?></td>
<td style="text-align:right"><?=number_format($acc_t,2)?></td><?php */?>
<td align="right" style="font-weight:bolder"><?=number_format($dp_t,2)?></td>
</tr>
</tbody>
</table>
<? }
...
$sql = "select m.do_no,m.do_date,m.dealer_code,m.po_no, i.item_id,i.unit_name,i.pack_size, o.unit_price, o.total_unit,d.product_group as grp,concat(i.finish_goods_code,'- ',item_name) as item_name, o.id, o.entry_by, o.entry_at from 
...
where m.do_no=o.do_no and i.item_id=o.item_id and m.dealer_code=d.dealer_code and m.status in ('CHECKED','COMPLETED') ".$date_con.$item_con.$depot_con.$dtype_con.$dealer_con.$item_brand_con." order by m.do_no asc";
...
?>
    ...
    <tr>
      ...
      <td style="text-align:right"><?=number_format($due_qty); $total_due_qty+=$due_qty;?></td>
		...
		<td style="text-align:center;"> <a href="so_wise_challan_view_golden.php?v_no=<?=$info->do_no?>" target="_blank">
		<button style="background-color: #51e7ff;border-radius: 3px;border: 1px solid #333; cursor: pointer;">View</button></a>
		</td>
...
<? } ?>
    ...
    <tr>
      <td colspan="9" style="text-align:right">Totel</td>
...
{
echo $sql="select m.do_no,m.do_date,m.entry_time,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,(select AREA_NAME from area where AREA_CODE=d.area_code) as area,w.warehouse_name as depot, m.rcv_amt,concat(m.payment_by,':',m.bank,':',m.remarks) as Payment_Details 
...
<tr><th>S/L</th><th>Do No</th><th>Do Date</th><th>Confirm At</th><th>Entry At</th><th>Dealer Name</th><th>Area</th><th>Depot</th><th>Grp</th><th>Rcv Amt</th><th>Payment Details</th><th>DP Total</th><th>TP Total</th></tr></thead>
<tbody>';
while($data=mysqli_fetch_object($query)){$s++;
...
$sql="select m.do_no,m.po_no,m.do_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,w.warehouse_name as depot,d.product_group as grp, sum(ds.total_amt) as do_amt,m.rcv_amt,concat(m.payment_by,' ',m.bank,' ',m.remarks) as Payment_Details from 
...
<td><?=$data->po_no?></td>
<td><?=$data->dealer_name?></td>
<?php /*?><td><?=$data->area?></td><?php */?>
<td><?=$data->depot?></td>
<?php /*?><td><?=$data->grp?></td><?php */?>
<?php /*?><td align="right"><?=$data->rcv_amt?></td>
<td><?=$data->Payment_Details?></td><?php */?>
<td align="right"><?=number_format($data->do_amt,2);?></td>
<td align="right"><?=number_format($do_ch_amt[$data->do_no],2)?></td>
<td align="right"><?=number_format($due_amt,2);?></td></tr>
<?
}
echo '<tr class="footer"><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td align="right">'.number_format($dp_t,2).'</td><td align="right">'.number_format($tp_t,2).'</td><td align="right">'.number_format($due_t,2).'</td></tr></tbody></table>';
}
elseif($_REQUEST['report']==20231) 
{
...
$sql="select m.do_no,m.po_no,m.do_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,w.warehouse_name as depot,d.product_group as grp,sum(ds.total_unit) as do_qty, sum(ds.total_amt) as do_amt,m.rcv_amt,concat(m.payment_by,' ',m.bank,' ',m.remarks) as Payment_Details, m.status from 
sale_do_master m,dealer_info d  , warehouse w,sale_do_details ds
where m.do_no=ds.do_no and m.status in ('CHECKED','COMPLETED')  and m.dealer_code=d.dealer_code and w.warehouse_id=d.depot and ds.total_amt>0 ".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con." group by m.do_no order by m.do_date,m.do_no";
$query = db_query($sql);
?>
...
$tp_t = $tp_t+$do_ch_amt[$data->do_no];
$due_t = $due_t+$due_amt;
$ch_qty=find_a_field('sale_do_chalan','sum(total_unit)','do_no = '.$data->do_no);
$due_qty = $data->do_qty - $ch_qty;
?>
<tr>
<td><?=$s?></td>
<td><a href="../wo/sales_order_print_view.php?v_no=<?=$data->do_no?>" target="_blank"><?=$data->do_no?></a></td>
...
<td><?=$data->depot?></td>
<td align="right"><?=number_format($data->do_qty,2);?></td>
<td align="right"><?=number_format($data->do_amt,2);?></td>
<td align="right"><?=number_format($ch_qty,2);?></td>
<td align="right"><?=number_format($do_ch_amt[$data->do_no],2)?></td>
<td align="right"><?=number_format($due_qty,2);?></td>
<td align="right"><?=number_format($due_amt,2);?></td>
<td><?=$data->status?></td>
</tr>
<?
}
echo '<tr class="footer"><td colspan="6" align="right" >Total</td><td align="right" >&nbsp;</td><td align="right">'.number_format($dp_t,2).'</td><td align="right">&nbsp;</td><td align="right">'.number_format($tp_t,2).'</td><td align="right" >&nbsp;</td><td align="right">'.number_format($due_t,2).'</td><td>&nbsp;</td></tr></tbody></table>';
}
elseif($_REQUEST['report']==2023) 
{

 	
	if(isset($t_date)) {$to_date=$t_date; $fr_date=$f_date; 
		$date_con2 .= ' and c.chalan_date between \''.$fr_date.'\' and \''.$to_date.'\'';}
		
if((strlen($_REQUEST['cut_date'])==10)) $cut_date=$_REQUEST['cut_date'];
if(isset($cut_date)) 					{$cut_date_con=' and c.chalan_date <="'.$cut_date.'"';}

//$sqld = 'SELECT c.chalan_no,c.do_no, m.do_no FROM sale_do_chalan c, sale_do_master m WHERE chalan_no AND m.do_no=c.do_no '.$date_con.$cut_date_con.' GROUP BY c.chalan_no';
//$queryd = db_query($sqld);
//while($info = mysqli_fetch_object($queryd)){
//$do_ch_amt[$info->do_no] = $info->ch_amt;
//}

 $sql= 'SELECT c.chalan_no,c.chalan_date,c.do_no,c.do_date,c.item_id,c.total_unit,c.unit_price,c.qty_bag,c.bag_size,m.po_no,d.dealer_code,d.dealer_name_e, i.item_name, i.unit_name, i.secondary_unit_name
FROM sale_do_chalan c, sale_do_master m, dealer_info d, item_info i
WHERE chalan_no AND m.do_no=c.do_no and m.dealer_code=d.dealer_code and c.item_id=i.item_id '.$date_con2.$cut_date_con.'';
...
<tr>
<td><?=$s?></td>
<td><a href="../../../warehouse_mod/pages/wo_golden/delivery_challan_print_view.php?v_no=<?=$data->chalan_no?>" target="_blank"><?=$data->chalan_no?></a></td>
<td><?=$data->chalan_date?></td>
<td><?=$data->do_no?></td>
<td><?=$data->do_date?></td>
<td><?=$data->po_no?></td>
<td><?=$data->dealer_code?> - <?=$data->dealer_name_e?></td>
<td><?=$data->item_name?></td>
<td><?=$data->unit_name?></td>
<?php /*?><td><?=$data->unit_price?></td><?php */?>
<td align="right"><?=$data->qty_bag; $total_qty_bag+=$data->qty_bag;?></td>
<td align="right"><?=$data->bag_size?></td>
<td align="right"><?=$data->total_unit; $total_unit_size+=$data->total_unit;?></td>
</tr>
<?
}

echo '<tr class="footer"><td colspan="9" align="right"> Total</td><td align="right">'.number_format($total_qty_bag).'</td><td> </td><td align="right">'.number_format($total_unit_size).'</td></tr></tbody></table>';
}
elseif($_REQUEST['report']==231119) 
{
	if(isset($t_date)) {
		$to_date=$t_date; $fr_date=$f_date; 
		$date_con2 .= ' and c.chalan_date between \''.$fr_date.'\' and \''.$to_date.'\'';
		$date_con3 = ' and s.oi_date between \''.$fr_date.'\' and \''.$to_date.'\'';
	}
		
if((strlen($_REQUEST['cut_date'])==10)) $cut_date=$_REQUEST['cut_date'];
if(isset($cut_date)) {
$cut_date_con=' and c.chalan_date <="'.$cut_date.'"';
$cut_date_con1=' and s.oi_date <="'.$cut_date.'"';
...
<tr>
	<td style="border:0px;" colspan="12"><h2 style=" text-align: center; font-weight: 600;"> Cash Sales Delivery Challan</h2></td>
</tr>

<tr>
	<td style="border:0px; padding: 0px;" colspan="12">
	<table width="100%" cellspacing="0" cellpadding="2" border="1">
		<thead>
			<tr style=" font-weight: 600; ">
				<td>Sl NO</td>
				<td>Cash Sales NO</td>
				<td>Cash Sales Date</td>
		...
		<tbody>
<?
 $sql1='select m.oi_no as LS_No,m.oi_date as LS_DATE,m.vendor_name as Sales_From,i.item_name,s.qty,s.rate,s.amount from warehouse_other_issue m, warehouse_other_issue_detail s, item_info i where i.item_id=s.item_id and m.oi_no=s.oi_no and m.issue_type in ("CASH Sales","CASHSales") '.$date_con3.$cut_date_con1.'';
...
<td><?=$data->LS_No?></td>
<td><?=$data->LS_DATE?></td>
<td><?=$data->Sales_From?></td>
<td><?=$data->item_name?></td>

<td><?=$data->rate?></td>
<td align="right"><?=$data->qty; $total_qty+=$data->qty;?></td>
<td align="right"><?=$data->amount; $total_amount+=$data->amount;?></td>
</tr>
<?
}
echo '<tr class="footer"><td colspan="6" align="right"> Total</td><td align="right">'.number_format($total_qty,2).'</td><td align="right">'.number_format($total_amount,2).'</td></tr>';
 ?>
 </tbody>
 </table>
...
<tr>
	<td style="border:0px;" colspan="12"><h2 style=" text-align: center; font-weight: 600;">Sales Order Delivery Challan</h2></td>
</tr>
<tr>
<th>S/L</th>
<th>Cash Sales No</th>
...
echo '<tr class="footer"><td colspan="9" align="right"> Total</td><td align="right">'.number_format($total_qty_bag).'</td><td> </td><td align="right">'.number_format($total_unit_size).'</td></tr>';
?>
<tr>
	<td style="border:0px;" colspan="12"><h2 style=" text-align: center; font-size:24px; font-weight: 600;"> Total Delivery Challan</h2></td>
</tr>
<tr>
	<td style="border:0px;" colspan="12">
	
	<table width="100%" cellspacing="0" cellpadding="2" border="1" style=" font-size: 23px; ">
	<thead>
		<tr>
			<td align="center">Total Cash Sales </td>
			<td align="center">Total Sales Order</td>
			<td align="center"> Total Delivery Challan</td>
		</tr>	
	</thead>
	<tbody>
			<tr>
				<td align="right"><?=number_format($total_qty,2);?></td>
				<td align="right"><?=number_format($total_unit_size,2);?></td>
				<td align="right"><? $TDC_total=$total_qty+$total_unit_size;
				 echo number_format($TDC_total,2);?></td>
			</tr>	
	</tbody>
	</table>
...
<thead>
<tr>
<td style="border:0px;" colspan="14"><?=$str?></td>
</tr>
<tr>
<th>S/L</th>
<th>Challan No</th>
...
$sub_amt = $data->unit_price * $data->total_unit;
?>
<tr>
<td><?=$s?></td>
<td><a href="../../../warehouse_mod/pages/wo_golden/delivery_challan_print_view.php?v_no=<?=$data->chalan_no?>" target="_blank"><?=$data->chalan_no?></a></td>
...
<td align="right"><?=$data->total_unit; $total_unit_size+=$data->total_unit;?></td>
<td align="right"><?=$data->unit_price?></td>
<td align="right"><?=number_format($sub_amt,2); $total_amt+=$sub_amt;?> </td>
</tr>
<?
}

echo '<tr class="footer"><td colspan="9" align="right"> Total</td><td align="right">'.number_format($total_qty_bag).'</td><td> </td><td align="right">'.number_format($total_unit_size).'</td><td></td><td>'.number_format($total_amt,2).'</td></tr></tbody></table>';
}
