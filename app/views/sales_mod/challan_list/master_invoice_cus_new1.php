...
<table class="table table-bordered">
  <thead>
    <tr style="text-align:center;">
      ...
      <th scope="col" width="">Order Quantity</th>
	  <th  scope="col" width="">Weight Lost</th>
	  
	  <th scope="col" width="">Total Quantity</th>
  ...
  <tbody>
  <?php
  // $sql1='SELECT s.do_no,s.chalan_no,s.item_id,s.dealer_code,s.unit_price,sum(s.total_unit) as total_unit,sum(s.total_amt) as total_amt,s.chalan_date,s.dealer_code,i.item_name,i.unit_name,i.pack_size,i.secondary_unit_quantity,i.secondary_unit_name
...
// where s.chalan_no in(select chalan_no from master_invoice_master where master_invoice_no="'.$get_master_inv_no.'") and s.item_id=i.item_id group by s.chalan_no';
   ...
   $sql='SELECT s.do_no,s.chalan_no,s.weight_lost,s.item_id,s.dealer_code,s.unit_price,s.total_unit,s.total_amt,s.chalan_date, s.dealer_code,i.item_name,i.unit_name,i.pack_size,i.secondary_unit_quantity,i.secondary_unit_name
from sale_do_chalan s,item_info i
where s.chalan_no in(select chalan_no from master_invoice_master where master_invoice_no="'.$get_master_inv_no.'") and s.item_id=i.item_id';


$query=db_query($sql);

while($data = mysqli_fetch_object($query)){
$totel_weight = $data->total_unit - $data->weight_lost;
$total = $data->unit_price * $totel_weight;
?>
    <tr class="text-center">
      ...
      <td><?=$data->item_name;?></td>
        <td><?=$data->unit_name;?></td>
     	   <td><?=round($data->total_unit,0);?></td>
		   <td><?=round($data->weight_lost,0);?></td>
		   <td><?=$totel_weight;?></td>
	 
<input type="hidden" name="chalan_no" id="chalan_no" value="<?=$data->chalan_no; ?>">
      <td><?=$unit_price=number_format($data->unit_price,2);?> </td>

      <td><?=number_format($total,2);?></td>

    </tr>
   <? $total1 =$total1+$total;
   	$total2 =  $total2+$totel_weight;
...
<?php } ?>
<tr class="text-center">
<td colspan="7" align="right" style="font-weight:bold;">Gross Total</td>
	<td></td>
	<td></td>
