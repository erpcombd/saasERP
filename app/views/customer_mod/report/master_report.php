...
<?
 // do summery report modify jan 24 2018

 $dealer_code=find_a_field('user_activity_management','dealer_code','user_id="'.$_SESSION['user']['id'].'"');

 $sql="select c.chalan_no, c.chalan_date,sum(c.total_unit) as unit,sum(c.total_amt) as amt, m.do_no, m.job_no,  m.do_date, m.dealer_code,  d.dealer_name_e, c.entry_by, c.status 

from  sale_do_master m, sale_do_chalan c, dealer_info d, user_group u 

where m.group_for=u.id  and m.dealer_code=d.dealer_code and m.do_no=c.do_no and m.dealer_code=".$dealer_code."   group by c.chalan_no order by  c.chalan_date, m.do_no,c.chalan_no ";
...
<td><a href="delivery_challan_print_view.php?v_no=<?=$data->chalan_no?>" target="_blank"><?=$data->chalan_no?></a></td>

<td><?=$data->chalan_date?></td>

<td align="right"><?=$data->unit?></td>

<td align="right"><?=$data->amt?></td>
...
<td style="text-align:right"><?=number_format($totu,2)?></td>

<td style="text-align:right"><?=number_format($tota,2)?></td>
