...
$con .='and a.warehouse_name like "%'.$_POST['warehouse_name'].'%" ';
 ...
 $td='select a.'.$unique.',  a.'.$shown.', b.sub_sub_class_name,l.group_name from '.$table.' a,ledger_group l, acc_sub_sub_class b, acc_sub_class c where a.ledger_group_id=l.group_id and l.acc_sub_sub_class=b.id and b.acc_sub_class=c.id and c.sub_class_name like "%Non-Current Asset%" '.$con.' order by a.ledger_id  ';
		...
		<form class="n-form" action="" method="post" enctype="multipart/form-data" name="form1" id="form1" onsubmit="return check()">
                ...
                <div class="form-group row m-0 pl-3 pr-3">
                    ...
                    <div class="col-sm-8 p-0">
										...
										<option></option>
                      						<? foreign_relation('ledger_group l,acc_sub_class a,acc_sub_sub_class b','l.group_id','CONCAT(l.group_id, ": ", l.group_name)',$ledger_group_id,'l.acc_sub_sub_class=b.id and b.acc_sub_class=a.id and a.sub_class_name like "%Non-Current Asset%"');?>
                    					</select></span>
                    </div>
                </div>
