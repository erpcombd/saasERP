...
function get_op_dpc($asset_tag,$fdate){

$sql = 'select i.depreciation_rate,a.depreciation_start_date,a.price from item_info i, asset_register a where i.item_id=a.item_id and a.asset_id="'.$asset_tag.'" and a.depreciation_start_date<"'.$fdate.'"';
$query=db_query($sql);
...
function get_op_price($asset_tag,$fdate){

$sql = 'select a.price from asset_register a where a.asset_id="'.$asset_tag.'" and a.depreciation_start_date<"'.$fdate.'"';
$query=db_query($sql);
$a = mysqli_fetch_object($query);
return $a->price;
...
function depreciation_opening($sub_group_id,$fdate)
{

$sql = 'select a.asset_id from asset_register a where a.sub_group_id="'.$sub_group_id.'" and a.depreciation_start_date<"'.$fdate.'"';
$query=db_query($sql);
while($a = mysqli_fetch_object($query)){

$total_dpc_value   += get_op_dpc($a->asset_id,$fdate);
$total_asset_value += get_op_price($a->asset_id,$fdate);

}
$rest = $total_asset_value-$total_dpc_value;
return $rest;
}


function addition($sub_group_id,$fdate,$tdate)
{

$sql = 'select sum(a.price) as asset_addition from asset_register a where a.sub_group_id="'.$sub_group_id.'" and a.depreciation_start_date between "'.$fdate.'" and "'.$tdate.'" group by a.sub_group_id';
$query=db_query($sql);
$a = mysqli_fetch_object($query);
$total_dpc_value = $a->asset_addition;
return $total_dpc_value;
}

function disposal($sub_group_id,$fdate,$tdate)
{

$sql = 'select sum(a.po_value) as asset_disposal from asset_disposal_info a, item_info i, item_sub_group s where a.item_id=i.item_id and i.sub_group_id=s.sub_group_id and s.sub_group_id="'.$sub_group_id.'" and a.disposal_date between "'.$fdate.'" and "'.$tdate.'" group by s.sub_group_id';
$query=db_query($sql);
$a = mysqli_fetch_object($query);
$total_dpc_value = $a->asset_disposal;
return $total_dpc_value;
}

function dpc_op($sub_group_id,$fdate)
{
...
?>
		...
		<td  width="50%" style="border:0px; border-color:white;">
			<table width="100%"  >
				<tr align="center" >
					<td style="font-size:18px; border:0px; border-color:white;"><strong><?=find_a_field('user_group','group_name','id='.$_REQUEST['group_for'])?></strong></td>	
				</tr>
				<tr align="center" >
					<td style="font-size:14px; border:0px; border-color:white;"><strong><?=$report;?></strong></td>	
				</tr>
...
$wdv_days=365-$count_days;
		...
		<th width="3%" rowspan="2" bgcolor="#FFFACD">SL_No </th>
		<th width="26%" rowspan="2" bgcolor="#FFFACD">FIXED ASSET LEDGER</th>
		...
		$sql = "select fixed_asset_id,  sum(cr_amt) as cr_amt from fixed_asset_journal where jv_date between '".$f_date."' and '".$t_date."' and tr_from='Adjustment' group by  fixed_asset_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
...
$con .=' and j.asset_tag="'.$_POST['asset_id'].'"';
		...
		$sql_sub="select l.ledger_name,l.ledger_id from accounts_ledger l, ledger_group g, acc_sub_class s where l.ledger_group_id=g.group_id and g.acc_sub_class=s.id and s.sub_class_name like '%Non-Current Asset%'";
		$data_sub=db_query($sql_sub);
		...
		<tr>
		  ...
		  <td bgcolor="#eaeded">&nbsp;</td>
		  <td bgcolor="#eaeded"><div align="left" style="text-transform:uppercase; font-size:14px; font-weight:700;"><strong><?=$info_sub->ledger_name?></strong></div></td>
		  <td bgcolor="#eaeded">&nbsp;</td>
		  <td bgcolor="#eaeded">&nbsp;</td>
		  <td bgcolor="#eaeded">&nbsp;</td>
		  <td bgcolor="#eaeded">&nbsp;</td>
		...
		<?
 		...
 		$sql="select s.sub_group_id, s.sub_group_name, s.depreciation_rate from item_sub_group s where s.asset_ledger='".$info_sub->ledger_id."'";
		...
		<td><? $sl++;?><?=$row->id?></td>
		<td><?=$row->sub_group_id?></td>
		<td><?=$row->sub_group_name?></td>
		<td><div align="right">
		  <?= number_format($opening_balance= depreciation_opening($row->sub_group_id,$f_dt),2); $sub_tot_opening_balance +=$opening_balance;?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($dr_balance= addition($row->sub_group_id,$f_dt,$ttdate),2);  $sub_tot_dr_balance +=$dr_balance;?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($cr_balance= disposal($row->sub_group_id,$f_dt,$ttdate),2); $sub_tot_cr_balance +=$cr_balance;?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($cost_balance= ($opening_balance+$dr_balance)-$cr_balance,2); $sub_cost_balance +=$cost_balance;?>
		  </div></td>
		<td><div align="center">
		    <?=$row->depreciation_rate;?>%</div></td>
		<td><div align="right">
		  <?= number_format($dep_opening_balance= dpc_op($row->sub_group_id,$f_dt),2); $sub_dep_opening_balance +=$dep_opening_balance;?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($dep_during_year= (($cost_balance)*$row->depreciation_rate)/100,2);  $sub_dep_during_year +=$dep_during_year;?>
		  </div></td>
		<td><div align="right">
