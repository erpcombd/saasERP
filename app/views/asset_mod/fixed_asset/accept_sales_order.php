...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0" style="font-size:11px; text-transform:uppercase; font-family: Arial, Helvetica, sans-serif;">
  ...
  <tr>
    ...
    <th width="19%">narration</th>
    <th width="9%">Type</th>
	<th width="7%">Maintenance Type</th>
  ...
  if($_POST['fdate']!=''&&$_POST['tdate']!='') $so_date_con .= ' and m.do_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
	...
	if($_POST['asset_id']!='')
	$con =" and f.fixed_asset_id='".$_POST['asset_id']."'";
...
$sql="SELECT distinct p.id,j.jv_no,j.jv_date,p.expense_head ledger_id,a.ledger_name, p.amount dr_amt, j.tr_no,p.asset_id,n.maintenance_type,j.tr_from

FROM purchase_invoice p, vendor_invoice_details v,journal j, fixed_asset_journal f, accounts_ledger a, maintenance_type n 

WHERE p.po_no=v.po_no and v.system_invoice_no and v.system_invoice_no=j.tr_no and j.tr_from='service_bill' and f.fixed_asset_id=p.asset_id and p.id=v.pr_id and j.dr_amt>0 and j.ledger_id=a.ledger_id and p.item_id=v.item_id and p.maintenance_type=n.id and n.capitalized='Yes' and p.expense_head=j.ledger_id and j.jv_no!=f.tr_no and p.expense_head=a.ledger_id ".$src_ledger_id_con.$con."";
//		}
...
//where  j.ledger_id=a.ledger_id and f.asset_id=m.asset_id and j.tr_from='service_bill' and m.id=j.tr_id  and n.id=m.maintenance_type and n.capitalized='Yes' and i.item_id=f.item_id and i.sub_group_id=s.sub_group_id   ".$src_ledger_id_con.$con." group by j.id order by j.jv_date, j.id desc";
  ...
  while($data=mysqli_fetch_object($query)){$i++;
   ...
   $exist_add=find_a_field('fixed_asset_journal','tr_no','tr_no="'.$data->jv_no.'" and fixed_asset_id="'.$data->asset_id.'" and tr_from="Addition"');
...
<? if($exist_add==0) { ?>
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
    ...
    <td>
	<input type="text" name="fixed_asset_id_<?=$data->id?>" id="fixed_asset_id_<?=$data->id?>"  style="width:90%; height:32px;" value="<?=$data->asset_id?>"  readonly=""/>
...
<script>
  ...
  var group_for = document.getElementById('group_for').value;
    ...
    $.ajax({
      url: 'accept_sales_order_ajax.php',
      type: 'POST',
      data: {
        asset_id: asset_id,
		order_no: order_no,
