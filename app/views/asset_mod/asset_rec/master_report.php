...
$con .= ' and a.date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'"';}

if($_POST['type']!=''){

$con .= 'and a.type="'.$_POST['type'].'"';}

if($_POST['item_id']!=''){

$con .= 'and a.item_id="'.$_POST['item_id'].'"';}

 $res='select a.*,p.fname,i.item_name from fixed_asset_receive_master a,user_activity_management p,item_info i where a.user_id=p.user_id and a.item_id=i.item_id '.$con.'';
...
?>
	...
	<td><?=++$i;?></td>
	  ...
	  <td valign="top"><?=$data->fname;?></td>

	  <td valign="top"><?=$data->date;?></td>
      <td valign="top"><?=$data->asset_id;?></td>
	  <td valign="top"><?=$data->item_name;?></td>
	  <td valign="top"><?=$data->serial_no;?></td>
