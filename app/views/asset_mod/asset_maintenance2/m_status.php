...
$unique = 'm_id';
$status = 'UNCHECKED';
$target_url = '../asset_maintenance/asset_maintenance.php';
$target_url_view = '../asset_maintenance/m_print_view.php';
$tr_from="Warehouse";
...
function custom_view(theUrl)
{
	window.open('<?=$target_url_view?>?<?=$unique?>='+theUrl);
}
</script>
...
<div class="form-container_large">
    ...
    <form action="" method="post" name="codz" id="codz">
        ...
        <div class="container-fluid bg-form-titel">
            <div class="row">
                    ...
                    <div class="form-group row m-0">
                        ...
                        <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">
						  <select name="status" id="status" >
							<option></option>
							<option value="MANUAL" <?=($_POST['status']=='MANUAL')?'selected':''?>>Draft</option>
							<option value="UNCHECKED" <?=($_POST['status']=='UNCHECKED')?'selected':''?>>UNCHECKED</option>
							<option value="CHECKED" <?=($_POST['status']=='CHECKED')?'selected':''?>>CHECKED</option>
						 </select>
                ...
                <div class="col-sm-2 col-md-2 col-lg-2 col-xl-2">
                    ...
                    <input type="submit" name="submitit" id="submitit" value="Search" class="btn1 btn1-submit-input"/ >
                </div>
		...
		<? 
					...
					$con .= 'and a.m_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
					
					$res='select a.*,w.warehouse_name,u.fname from asset_maintenance_info a, warehouse w, user_activity_management u where u.user_id=a.entry_by and a.warehouse_id=w.warehouse_id '.$con.'';
		            $query = db_query($res);
		?>
					...
					?>
                        ...
                        <tr>
                            ...
                            <td><? if($row->status=='MANUAL'){ ?>
							<button type="button" name="submitit" id="submitit" value="VIEW" onclick="custom_view(<?=$row->m_id?>)" class="btn2 btn1-bg-submit">
							<i class="fa-solid fa-eye"></i>
							</button>
							
							<button type="button" name="submitit" id="submitit" value="VIEW" onclick="custom(<?=$row->m_id?>)" class="btn2 btn1-bg-submit">
