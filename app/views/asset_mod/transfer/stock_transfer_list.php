...
$target_url = '../depot_transfer/stock_transfer_status.php';
...
if($_POST['warehouse_id']>0){
 $con .= ' and c.warehouse_from = '.$_POST['warehouse_id'].'';
 }else{
 $con .= ' and c.warehouse_from = '.$_SESSION['user']['depot'].'';
 }
...
//where m.status in ('PROCESSING') and d.area_code=a.AREA_CODE  and m.dealer_code=d.dealer_code ".$con." and d.dealer_type='Distributor' order by m.do_date,d.dealer_name_e";
 ...
 $res="select c.* from production_issue_master c where 1 ".$con." group by c.pi_no order by c.pi_no desc";
