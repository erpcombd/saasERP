...
switch ($_REQUEST['report']) {
	...
	$report="Asset Item Info";

	break;
	
	case 506:

	$report="Registered Asset Info";
...
<!--		<img src="../../../logo/fg-logo.png" style="width:100px;">-->
			<img src="../../../logo/<?=$_SESSION['user']['group']?>.png" style=" height:70px; width:auto;" />
		</td>
...
<!--		<img src="../../../logo/fg-logo.png" style="width:100px;">-->
		...
		//if($sales_amt[$row->item_id]<>0 || $foc_amt[$row->item_id]<>0 ){}
		$op = depreciation_opening();
		?>
		
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
		<td><?=$sl++;?></td>
		<td><?=$row->dealer_name_e?></td>
		<td><?=number_format(depreciation_opening(),2);  $sub_total_opening_amt +=$opening_amt[$customer_ledger]; ?></td>
...
elseif($_REQUEST['report']==230303001)
{
		...
		<td width="25%" align="center" style="border:0px; border-color:white;">
		<!--<img src="../../../logo/<?=$_SESSION['user']['group']?>.png" style=" height:70px; width:auto;" />-->
<!--		<img src="../../../logo/tecno.png" style="width:220px;">-->
		</td>
		<td  width="50%" style="border:0px; border-color:white;">
			<table width="100%"  >
				<tr align="center" >
					<td style="font-size:18px; border:0px; border-color:white;"><strong><?=find_a_field('user_group','group_name','id='.$_POST['group_for'])?></strong></td>	
				</tr>
				<tr align="center" >
					<td style="font-size:14px; border:0px; border-color:white;"><strong><?=$report;?></strong></td>	
				</tr>
        ...
        <tr>
		 ...
		 <tr>
          <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;">For the Period of: <?php echo date('d-m-Y',strtotime($_REQUEST['f_date']));?>
		   <strong>To</strong> <?php echo date('d-m-Y',strtotime($_REQUEST['t_date']));?>
		   
(Duration <?
		
//echo $tomorrow =  date('Y-m-d', strtotime('+1 day', strtotime($_POST['t_date'])));
	
$f_dt = $_REQUEST['f_date'];		   
$t_dt =  date('Y-m-d', strtotime('+1 day', strtotime($_REQUEST['t_date'])));
...
$wdv_days=365-$count_days;
		...
		<tr height="30">
		<th width="3%" rowspan="2" bgcolor="#FFFACD">ID</th>
		<th width="3%" rowspan="2" bgcolor="#FFFACD">ASSET ID </th>
		<th width="26%" rowspan="2" bgcolor="#FFFACD">FIXED ASSET INFO</th>
		<th colspan="4" bgcolor="#d6eaf8" style="text-align:center">COST</th>
		<th colspan="4" bgcolor="#99FFFF" style="text-align:center">DEPRECIATION</th>
		<th width="9%" rowspan="2" bgcolor="#ebdef0" style="text-align:center">Written Down Value as at <?php echo date('d-M-Y',strtotime($t_date));?></th>
		<th width="4%" rowspan="3" bgcolor="#ebdef0" style="text-align:center">Year of Validity</th>
		</tr>
		<tr height="30">
		  ...
		  <th width="7%" bgcolor="#d6eaf8" style="text-align:center">Balance as at <br /> <?php echo date('d-M-Y',strtotime($t_date));?></th>
		  <th width="5%" bgcolor="#99FFFF" style="text-align:center">Depreciation Rate</th>
		  <th width="8%" bgcolor="#99FFFF" style="text-align:center">Balance as at <br /> <?php echo date('d-M-Y',strtotime($f_date));?></th>
		  <th width="7%" bgcolor="#99FFFF" style="text-align:center">Charged during the year</th>
		  <th width="10%" bgcolor="#99FFFF" style="text-align:center">Balance as at <br /> <?php echo date('d-M-Y',strtotime($t_date));?></th>
		</tr>
		<tr height="30">
		  ...
		  <th bgcolor="#99FFFF" style="text-align:center">9 = (7+8)</th>
		  <th bgcolor="#ebdef0" style="text-align:center">10 = 5-9</th>
		  </tr>
		</thead><tbody>
		<? $sl=1;
		

		if($_REQUEST['fixed_asset_type']) 		{$fixed_asset_type_con=' and a.fixed_asset_type="'.$_REQUEST['fixed_asset_type'].'"';}
		
		if($_REQUEST['group_for']) 		{$group_for_con=' and a.group_for="'.$_REQUEST['group_for'].'"';}
		
		
		
 		 $sql = "select fixed_asset_id,  sum(dr_amt-cr_amt) as opening_amt from fixed_asset_journal where jv_date <'".$f_date."' and tr_from!='Depreciation'  group by  fixed_asset_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $opening_amt[$info->fixed_asset_id]=$info->opening_amt;
		}
		
		
		 $sql = "select fixed_asset_id, sum(dr_amt) as dr_amt from fixed_asset_journal where jv_date between '".$f_date."' and '".$t_date."' group by fixed_asset_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $dr_amt[$info->fixed_asset_id]=$info->dr_amt;
		}
		
		$sql = "select fixed_asset_id,  sum(cr_amt) as cr_amt from fixed_asset_journal where jv_date between '".$f_date."' and '".$t_date."' and tr_from in ('Adjustment','Disposal') group by  fixed_asset_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $cr_amt[$info->fixed_asset_id]=$info->cr_amt;
		}
		

		 $sql = "select fixed_asset_id,  sum(cr_amt) as dep_opening_amt from fixed_asset_journal where jv_date <'".$f_date."' and tr_from='Depreciation' group by  fixed_asset_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $dep_opening_amt[$info->fixed_asset_id]=$info->dep_opening_amt;
		}
		
		$sql = "select fixed_asset_id,  sum(cr_amt) as dep_addition from fixed_asset_journal where jv_date between '".$f_date."' and '".$t_date."' and tr_from='Depreciation' group by  fixed_asset_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $dep_addition[$info->fixed_asset_id]=$info->dep_addition;
		}
		

		/* $sql = "select r.fixed_asset_category,  sum(a.dr_amt) as grp_dr_amt from fixed_asset_journal a, fixed_asset_register r where a.fixed_asset_id=r.id and a.jv_date <'".$t_date."' 
		".$group_for_con." group by r.fixed_asset_category ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $grp_dr_amt[$info->fixed_asset_category]=$info->grp_dr_amt;
		}*/
		
		
		$sql_grp="select a.group_id, a.group_name from item_group a where 1 and ptype='asset' group by a.group_id order by a.group_id";
		$data_grp=db_query($sql_grp);

		while($info_grp=mysqli_fetch_object($data_grp)){
		
		$sql_sub="select a.sub_group_id, a.sub_group_name from item_sub_group a where a.group_id='".$info_grp->group_id."' group by a.sub_group_id order by a.sub_group_name";
		$data_sub=db_query($sql_sub);
	...
	<? //if($grp_dr_amt[$info_sub->id]>0 ){ ?>	
		
		<tr>
		  <td bgcolor="#eaeded">&nbsp;</td>
		  <td bgcolor="#eaeded">&nbsp;</td>
		  <td bgcolor="#eaeded"><div align="left" style="text-transform:uppercase; font-size:14px; font-weight:700;"><strong><?=$info_sub->sub_group_name?></strong></div></td>
		  <td bgcolor="#eaeded">&nbsp;</td>
		  <td bgcolor="#eaeded">&nbsp;</td>
		  <td bgcolor="#eaeded">&nbsp;</td>
		  <td bgcolor="#eaeded">&nbsp;</td>
		...
		<?
 		...
 		$sql="select a.*,i.item_name as asset_name from asset_register a, item_info i where i.item_id=a.item_id and a.sub_group_id='".$info_sub->sub_group_id."'  ".$group_for_con." order by a.asset_id";
		...
		<td><div align="right">
		  <?= number_format($opening_balance= $opening_amt[$row->asset_id],2); $sub_tot_opening_balance +=$opening_balance;?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($dr_balance= $dr_amt[$row->asset_id],2);  $sub_tot_dr_balance +=$dr_balance;?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($cr_balance= $cr_amt[$row->asset_id],2); $sub_tot_cr_balance +=$cr_balance;?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($cost_balance= ($opening_balance+$dr_balance)-$cr_balance,2); $sub_cost_balance +=$cost_balance;?>
		  </div></td>
		<td><div align="center">
		    <?=$row->dpc_rate;?>%</div></td>
		<td><div align="right">
		  <?= number_format($dep_opening_balance= $dep_opening_amt[$row->asset_id],2); $sub_dep_opening_balance +=$dep_opening_balance;?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($dep_during_year= $dep_addition[$row->asset_id],2);  $sub_dep_during_year +=$dep_during_year;?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($depreciation_balance = ($dep_opening_balance+$dep_during_year),2); $sub_depreciation_balance +=$depreciation_balance; ?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($written_down_value=($cost_balance-$depreciation_balance),2);  $sub_written_down_value +=$written_down_value; ?>
		  </div></td>
		<td>
		  <div align="center">
		    <?=$row->year_validity;?>
...
<?  }?>
		...
		<tr>
		  <td bgcolor="#82e0aa">&nbsp;</td>
		  <td bgcolor="#82e0aa">&nbsp;</td>
		  <td bgcolor="#82e0aa"><div align="right" style=" text-transform:uppercase;"><strong><?=$info_grp->fixed_asset_type;?> TOTAL:</strong></div></td>
		  <td bgcolor="#82e0aa"><div align="right"><span class="style7">
		    <?= number_format($grand_sub_tot_opening_balance,2); $final_grand_sub_tot_opening_balance +=$grand_sub_tot_opening_balance;?>
	      </span></div></td>
		  <td bgcolor="#82e0aa"><div align="right"><span class="style7">
		    <?= number_format($grand_sub_tot_dr_balance,2); $final_grand_sub_tot_dr_balance +=$grand_sub_tot_dr_balance;?>
	      </span></div></td>
		  <td bgcolor="#82e0aa"><div align="right"><span class="style7">
		    <?= number_format($grand_sub_tot_cr_balance,2); $final_grand_sub_tot_cr_balance +=$grand_sub_tot_cr_balance;?>
	      </span></div></td>
		  <td bgcolor="#82e0aa"><div align="right"><span class="style7">
		    <?= number_format($grand_sub_cost_balance,2); $final_grand_sub_cost_balance +=$grand_sub_cost_balance;?>
	      </span></div></td>
		  <td bgcolor="#82e0aa"><div align="right"></div></td>
		  <td bgcolor="#82e0aa"><div align="right"><span class="style7">
		    <?= number_format($grand_sub_dep_opening_balance,2); $final_grand_sub_dep_opening_balance +=$grand_sub_dep_opening_balance;?>
	      </span></div></td>
		  <td bgcolor="#82e0aa"><div align="right"><span class="style7">
		    <?= number_format($grand_sub_dep_during_year,2); $final_grand_sub_dep_during_year +=$grand_sub_dep_during_year;?>
	      </span></div></td>
		  <td bgcolor="#82e0aa"><div align="right"><span class="style7">
		    <?= number_format($grand_sub_depreciation_balance,2); $final_grand_sub_depreciation_balance +=$grand_sub_depreciation_balance;?>
	      </span></div></td>
		  <td bgcolor="#82e0aa"><div align="right"><span class="style7">
		    <?= number_format($grand_sub_written_down_value,2); $final_grand_sub_written_down_value +=$grand_sub_written_down_value;?>
	      </span></div></td>
		  <td bgcolor="#82e0aa"><div align="center"></div></td>
		</tr>
		...
		<? 
		  ...
		  $grand_sub_tot_opening_balance = 0;
		  $grand_sub_tot_dr_balance = 0;
		  $grand_sub_tot_cr_balance = 0;
		  $grand_sub_cost_balance = 0;
		  $grand_sub_dep_opening_balance = 0;
		  $grand_sub_dep_during_year = 0;
		  $grand_sub_depreciation_balance = 0;
		  $grand_sub_written_down_value = 0;
		...
		<tr>
		  ...
		  <td bgcolor="#85c1e9">&nbsp;</td>
		  <td bgcolor="#85c1e9"><div align="right" style=" text-transform:uppercase;"><strong>GRAND TOTAL:</strong></div></td>
		  <td bgcolor="#85c1e9"><div align="right"><span class="style7">
		    <?= number_format($final_grand_sub_tot_opening_balance,2); ?>
	      </span></div></td>
		  <td bgcolor="#85c1e9"><div align="right"><span class="style7">
		    <?= number_format($final_grand_sub_tot_dr_balance,2);?>
	      </span></div></td>
		  <td bgcolor="#85c1e9"><div align="right"><span class="style7">
		    <?= number_format($final_grand_sub_tot_cr_balance,2); ?>
	      </span></div></td>
		  <td bgcolor="#85c1e9"><div align="right"><span class="style7">
		    <?= number_format($final_grand_sub_cost_balance,2); ?>
	      </span></div></td>
		  <td bgcolor="#85c1e9"><div align="right"></div></td>
		  <td bgcolor="#85c1e9"><div align="right"><span class="style7">
		    <?= number_format($final_grand_sub_dep_opening_balance,2); ?>
	      </span></div></td>
		  <td bgcolor="#85c1e9"><div align="right"><span class="style7">
		    <?= number_format($final_grand_sub_dep_during_year,2);?>
	      </span></div></td>
		  <td bgcolor="#85c1e9"><div align="right"><span class="style7">
		    <?= number_format($final_grand_sub_depreciation_balance,2);?>
	      </span></div></td>
		  <td bgcolor="#85c1e9"><div align="right"><span class="style7">
		    <?= number_format($final_grand_sub_written_down_value,2); ?>
	      </span></div></td>
		  <td bgcolor="#85c1e9"><div align="center"></div></td>
		</tr>
...
elseif($_POST['report']==30525)

{

$report="Asset Item Asset Information";

if($_POST['group_for']!=''){
$con =' and i.group_for="'.$_POST['group_for'].'"';
...
<tbody>
	...
	<tr>
		...
		<th>Capitilization  Date</th>
		<th>Acquisition Cost</th>
		<th>Addition</th>
		<th>Total Value</th>
		<th>Current Year Depreciation</th>
		<th>Accmulated Depreciation</th>
		<th>Book Value After Depreciation</th>
...
if($_POST['evaluation_group']!=''){

$con .=' and i.evaluation_group="'.$_POST['evaluation_group'].'"';
}
if($_POST['section']!=''){

$con .=' and i.section="'.$_POST['section'].'"';
}

if($_POST['asset_category']!=''){

$con .=' and i.asset_category="'.$_POST['asset_category'].'"';


}
//Asset Registered Info

$asset_regi='select r.*,i.depreciation_rate, c.company_short_code as company 

from asset_register r, item_info i, user_group c 

where i.item_id=r.item_id and c.id=i.group_for '.$con.' group by r.item_id';

$query = db_query($asset_regi);

while($ass=mysqli_fetch_object($query))

{
$asset_item[$ass->item_id]=$ass->item_id;

$capitalized_on[$ass->item_id]=$ass->reg_date;
$life_time[$ass->item_id]=$ass->life_duration;

$acquisition_value[$ass->item_id]=$ass->price;

$dpe_rate[$ass->item_id]=$ass->depreciation_rate;

$company[$ass->item_id]=$ass->company;

}

//Registered Journal
$ass_reg='select sub_ledger,sum(dr_amt-cr_amt) regi_value from journal  where tr_from="Registered"  group by sub_ledger';

$query = db_query($ass_reg);

while($ass_journal=mysqli_fetch_object($query))

{
$registered_value[$ass_journal->sub_ledger]=$regi_value->regi_value;


}

//Addition Journal
$add_journal='select sub_ledger,sum(dr_amt-cr_amt) addition_value from journal  where tr_from="Addition"  group by sub_ledger';

$query = db_query($add_journal);

while($ass_add=mysqli_fetch_object($query))

{
$addition_value[$ass_add->sub_ledger]=$ass_add->addition_value;
...
$dep_journal='select sub_ledger,sum(dr_amt-cr_amt) dep_value from journal  where tr_from="Depreciation"  group by sub_ledger';

$query = db_query($dep_journal);

while($ass_dep=mysqli_fetch_object($query))

{
$depreciation_value[$ass_dep->sub_ledger]=$ass_dep->dep_value;


}

//Current Year Depreciation Journal
$cydep_journal='select sub_ledger,sum(dr_amt-cr_amt) cydep_value from journal  where tr_from="Depreciation"  and year(jv_date)="'.date("Y").'" group by sub_ledger';

$query = db_query($cydep_journal);

while($ass_cpdep=mysqli_fetch_object($query))

{
$cpdep_value[$ass_cpdep->sub_ledger]=$ass_cpdep->cydep_value;


}


 $res='select i.*,s.sub_group_name,g.group_name, c.category_name,e.group_name as eval_group,l.section_name 
from item_sub_group s, item_group g,item_info i

left join evaluation_group e on    i.evaluation_group=e.id  
left join asset_category c on c.id=i.asset_category
left join asset_section l on l.id=i.section

where i.sub_group_id=s.sub_group_id and s.group_id=g.group_id and  g.ptype="asset"  '.$con.'';
...
{
if($acquisition_value[$data->item_id]>0){
?>
	<tr>
		 <td><?=++$i;?></td>
		 <td><?=$company[$data->item_id];?></td>
		 ...
		 <td><?=$data->sub_group_name?></td>
		  <td><?=$data->category_name?></td>
		   <td><?=$data->eval_group?></td>
		    <td><?=$data->section_name?></td>
			
			<td><?=$life_time[$data->item_id];?></td>
			<td><?=$dpe_rate[$data->item_id];?></td>
			<td><?=$capitalized_on[$data->item_id];?></td>
			<td><?=$acquisition_value[$data->item_id]; $tacq_value+=$acquisition_value[$data->item_id];?></td>
			<td><?=$addition_value[$data->item_sub_ledger]; $taddition_value+=$addition_value[$data->item_sub_ledger];?></td>
			<td><?=$total=$acquisition_value[$data->item_id]+$addition_value[$data->item_sub_ledger]; $ttacq_value+=$total;?></td>
			<td><?=$cpdep_value[$data->sub_ledger]; $tcpdep_value+=$cpdep_value[$data->sub_ledger];;?></td>
			<td><?=$depreciation_value[$data->sub_ledger]; $tdep_value+=$depreciation_value[$data->sub_ledger];?></td>
	<td><? $book_value=$depreciation_value[$data->sub_ledger]+$cpdep_value[$data->sub_ledger]; echo $total_book=$total-$book_value; $tbook_value+=$total_book;?></td>
...
<td><?=number_format($tacq_value,2);?></td>
<td><?=number_format($taddition_value,2);?></td>
<td><?=number_format($ttacq_value,2);?></td>
<td><?=number_format($tcpdep_value,2);?></td>
<td><?=number_format($tdep_value,2);?></td>
<td><?=number_format($tbook_value,2);?></td>
</tr>
...
?>
	<tr>
		 ...
		 <td><?=$data->item_name?></td>
		 <td><?=$data->asset_id?></td>
		 <td><?=$data->price; $tac_cost+=$data->price;?></td>
		 <td><?=$data->salvage_value;$tsal_cost+=$data->salvage_value;?></td>
...
<?
	...
	<td colspan=""><strong><?=$tac_cost;?></strong></td>
	<td colspan=""><strong><?=$tsal_cost;?></strong></td>
	<td colspan="4">&nbsp;</td>
	</tr>
