...
$report="Serial Number Movement Report";
...
<? 
 $pr='select tr_from,tr_no from journal_asset_item where serial_no="'.$_POST['sl_no'].'" group by id';
$pQuery=db_query($pr);
while($pData=mysqli_fetch_object($pQuery)){

	if($pData->tr_from=="Issue"){
		 $crb='SELECT j.serial_no,j.tr_no,p.PBI_NAME,d.quality,d.note FROM journal_asset_item j,fixed_asset_issue_details d,personnel_basic_info p 
		WHERE j.tr_no=d.id and d.PBI_ID=p.PBI_ID and j.tr_from="Issue" and tr_no="'.$pData->tr_no.'" and j.serial_no="'.$_POST['sl_no'].'"';
		$crQuerys=db_query($crb);
		while($cData=mysqli_fetch_object($crQuerys)){
				$currentUser[$cData->serial_no][$cData->tr_no]=$cData->PBI_NAME;
				$status[$cData->serial_no][$cData->tr_no]=$cData->quality;
				$note[$cData->serial_no][$cData->tr_no]=$cData->note;
			}
	}
	if($pData->tr_from=="Receive"){
	 $crb='SELECT j.serial_no,j.tr_no,p.PBI_NAME,d.quality,d.note FROM journal_asset_item j,fixed_asset_receive_details d,personnel_basic_info p 
	WHERE j.tr_no=d.id and d.PBI_ID=p.PBI_ID and j.tr_from="Receive" and tr_no="'.$pData->tr_no.'" and j.serial_no="'.$_POST['sl_no'].'"';
	$crQuerys=db_query($crb);
	while($cData=mysqli_fetch_object($crQuerys)){
			$currentUser[$cData->serial_no][$cData->tr_no]=$cData->PBI_NAME;
				$status[$cData->serial_no][$cData->tr_no]=$cData->quality;
 ...
 $res='select * from journal_asset_item where serial_no="'.$_POST['sl_no'].'"';
...
?>
	<tr>
		 ...
		 <td valign="top"><?=$data->tr_from;?></td>
		 <td><?=$currentUser[$data->serial_no][$data->tr_no] ?></td>
	 	 <td valign="top"><?=$status[$data->serial_no][$data->tr_no] ?></td>
		  <td valign="top"><?=$note[$data->serial_no][$data->tr_no] ?></td>
