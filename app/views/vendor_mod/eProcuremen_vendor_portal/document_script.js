...
$('.fileuploadcontainer form').submit(function (e) {
  ...
  formData.getAll('eprocfiles[]').forEach(function (file, index) {
    ...
    if (file.size <= 100 * 1024 * 1024) { // Check if file size is less than or equal to 10 MB
...
function deleteAttachmentseventinfo(button,attachmentid, masterId, trFrom, entryBy,motherContainerValue,datashowContainerValue,section_id) {
  ...
  var $attachmentcontainershow = $(button).closest('.attachmentshowcontainer');
    ...
    $.ajax({
        url: '../../../views/eProcurement_mod/api/api_attachment_vendor_delete.php',
        type: 'POST',
        data: JSON.stringify({
