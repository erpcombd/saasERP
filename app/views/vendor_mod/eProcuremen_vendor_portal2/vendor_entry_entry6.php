			...
			<form action="" method="post" class="col-sm-8" >
				<table width="100%" cellpadding="5" cellspacing="2">
					...
					?> 
					   <tr>
						 <td><?=$res1->lebel?></td>
						 <td colspan="2"><input type="text" name="lebel_<?=$unique_no?>" id="dropdown_name"/></td>
					   </tr>
					 <? if($res1->form_element=='Checkbox'){?>  
						 ...
						 <? if($res1->option_1 != ''){?>
						  <label class="form-check-label">
							<input type="checkbox" name="is_required_<?=$unique_no?>" id="is_required_<?=$unique_no?>" class="form-check-input" 
							value="<?=$res1->option_1?>" >
							<?=$res1->option_1?>
						  </label>
						  <? } ?>
