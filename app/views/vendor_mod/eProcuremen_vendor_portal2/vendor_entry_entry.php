...
<div class="row p-0 pb-5">
	<div class="col-sm-9 col-lg-9 col-md-9 col-9">
		<div class="pt-5" style=" font-size: 18px; padding-left: 2%;">Erp revenue Shar.... -Event #<?=$rfq_no?><span>(Active)</span></div>
	</div>
...
<div class="container">
		...
		<hr style="height:1px;border:none;color:#333;background-color:#333;">
		 <?php
		  $sql = 'select * from rfq_documents_information where tr_from like "sourcing_terms_condition" and rfq_no="'.$_GET['rfq_no'].'"';
		  $qry = db_query($sql);
		  while($res = mysqli_fetch_object($qry)){
		   ...
		   ?>
					...
					<div class="pl-3 row">
						...
						<div class="col-sm-16 col-md-6 col-lg-6">
							...
							<div class="form_element">
								<label class="inlineSelection" style="font-family: Helvetica,Arial,sans-serif !important; color: #60768a !important;font-size: 16px !important; font-weight: 400 !important;">
								<input type="radio" name="term_<?=$res->documents_id;?>"  <? if($res->terms_accept=='Yes') echo 'checked';?> id="term_<?=$res->documents_id;?>_true" value="Yes" class="s-termAccept" <?=$disabled?>    onclick="remove_form(this.value,<?=$res->documents_id;?>,'file',<?=$rfq_no;?>)">
								Yes
								</label>
								<br>
								<label class="inlineSelection" style="font-family: Helvetica,Arial,sans-serif !important; color: #60768a !important;font-size: 16px !important; font-weight: 400 !important;">
								<input type="radio"  <? if($res->terms_accept=='No') echo 'checked';?> name="term_<?=$res->documents_id;?>" id="term_<?=$res->documents_id;?>_false" value="No" class="s-termReject"  onclick="remove_form(this.value,<?=$res->documents_id;?>,'file',<?=$rfq_no;?>)">
								No
								</label>
							</div>
			...
			<?php
		  $sql = 'select * from rfq_documents_url_information where tr_from like "sourcing_terms_condition" and rfq_no="'.$_GET['rfq_no'].'"';
		  $qry = db_query($sql);
		  while($res = mysqli_fetch_object($qry)){
	 			...
	 			<div class="allrowshow row">
					<div class=" col-sm-6 col-md-6 col-lg-6 pl-3">
						...
						<div class="col-sm-16 col-md-6 col-lg-6">
							...
							<div class="form_element">
								<label class="inlineSelection" style="font-family: Helvetica,Arial,sans-serif !important; color: #60768a !important;font-size: 16px !important; font-weight: 400 !important;">
								<input type="radio" name="term_<?=$res->documents_url_id ;?>" <? if($res->terms_accept=='Yes') echo 'checked';?> id="term_<?=$res->documents_url_id ;?>_true" value="Yes" class="s-termAccept" <?=$disabled?>   onclick="remove_form(this.value,<?=$res->documents_url_id ;?>,'text',<?=$rfq_no;?>)" >
								Yes
								</label>
								<br>
								<label class="inlineSelection" style="font-family: Helvetica,Arial,sans-serif !important; color: #60768a !important;font-size: 16px !important; font-weight: 400 !important;">
								<input type="radio" <? if($res->terms_accept=='No') echo 'checked';?> name="term_<?=$res->documents_url_id ;?>" id="term_<?=$res->documents_url_id ;?>_false" value="No" class="s-termReject"  onclick="remove_form(this.value,<?=$res->documents_url_id ;?>,'text',<?=$rfq_no;?>)">
								No
								</label>
							</div>
...
<span class="attachment-toggle-add-file icon-search fs-14" style="cursor: pointer; color: blue;" onmouseover="this.style.color='orange'" onmouseout="this.style.color='blue'">File</span>
          ...
          <div class="fileuploadcontainer" >
			...
			<div class="attachmentTextshowcontainer container row m-0">
          <?php
			  $sql = 'select * from rfq_documents_url_information where tr_from like "5" and rfq_no="'.$_GET['rfq_no'].'" and entry_by = "'.$_SESSION['user']['id'].'" ';
			  $qry = db_query($sql);
			  while($res = mysqli_fetch_object($qry)){
		   ...
		   <? } ?>
						...
						<div class="attachmentUrlshowcontainer container row m-0">
                        <?php
						  $sql = 'select * from rfq_documents_url_information where tr_from like "sample_value" and rfq_no="'.$_GET['rfq_no'].'" and entry_by = "'.$_SESSION['user']['id'].'" ';
						  $qry = db_query($sql);
						  while($res = mysqli_fetch_object($qry)){
					   ?> 
					   <div class="col-sm-8 col-md-8 col-lg-8 pb-1">
							...
							<a href="<?=$res->attachment_url?>" target="_blank"><i class="fa-duotone fa-link fa-2xl" style="--fa-primary-color: #d6960a; --fa-secondary-color: #d6960a;"></i>
								<span><?=$res->attachment_url?></span></a>
									<button type="button" style=" display: inline !important;" class="border-0" onclick="deleteAttachmentURLseventinfo(this,'<?=$res->documents_url_id?>' , '<?=$res->rfq_no?>', 'sample_value', '<?=$res->entry_by?>', 'attachmenturluploadcontainereventinfo','attachmentshowcontainereventinfo-url')"><i class="fa-solid fa-xmark"></i>
									</button>
							</div>
						</div>
...
<h1 class="h1 m-0 p-0 pt-4 pl-3"><i class="fa-duotone fa-calendar-days"></i> Timeline </h1>
		...
		<div class="mt-5 w-100 pr-2" align="right" id="response_button">
		<?
  ...
  <div class="row m-0 p-0 pt-4">
	...
	<?
	 ...
	 ?>	
		...
		<input id="response_name" required id="response_name" value="<?=$response_name->response_name;?>" type="text" style="outline-color: orange !important;" onblur="response_nameupdate(this.value,<?=$rfq->rfq_no;?>)"  />
	</div>
...
<form >
	...
	?>	
				...
				<div class="col-sm-10 col-md-10 col-lg-10 pb-1"><div class="rounded p-2" style="background-color: #f7f7f7 !important; border: 1px solid #e6e6e6!important; "><a href="../../../../assets/support/upload_view.php?name=<?php echo $res2->new_name;?>&mod=eProcurement_mod&folder=rfq&proj_id=robi" target="_blank"><i class="fa-light fa-file fa-2xl fs-22" style="color: #d6960a;"></i> <span style="color:#268ecd;"><?php echo $res2->original_name;?></span></a></div>
				</div>
	<? } ?>			
			...
			<div class="attachment-toggle">
						...
						<div class="fileuploadcontainer" >
							...
							<input type="hidden" name="tr_from" value="sourcing_attachment_response">
							<input type="hidden" name="entry_by" value="<?=$_SESSION['user']['id']?>">
							<input type="hidden" name="vendor_id" value="<?=$vendor?>">
							<input type="hidden" name="motherContainer" value="fileuploadcontainer">
							<input type="hidden" name="datashowContainer" value="attachmentshowcontainer">
							...
							<div class="attachmentshowcontainer container row m-0">
		  <?php
		  $sql1 = 'select * from rfq_documents_information where tr_from like "sourcing_attachment_response" and rfq_no="'.$_GET['rfq_no'].'"';
		  $qry1 = db_query($sql1);
		  ...
		  ?>
								<div class="col-sm-10 col-md-10 col-lg-10 pb-1">
									...
									<a href="../../../../../assets/support/api_upload_attachment_show.php?name=<?=$res1->new_name?>&folder=<?=$res1->tr_from;?>" target="_blank">
									<i class="fa-light fa-file fa-2xl fs-22" style="color: #d6960a;"></i> <span><?=$res1->original_name?></span></a>
									<button type="button" style=" display: inline !important;" class="border-0" onclick="deleteAttachmentseventinfo(this,'<?=$res1->documents_id?>','<?=$rfq->rfq_no?>','sourcing_attachment_response','<?=$_SESSION['user']['id']?>','fileuploadcontainer','attachmentshowcontainer')">
...
?> 
		<div class="row">
					...
					<? if($res1->form_element=='Dropdown'){?> 
					   <tr>
						 ...
						 <td colspan="2"><select  name="<?=$res1->id?>" id="<?=$res1->id?>" >
						 						<option <? if($value[$res1->form_no][$res1->id]==$res1->option_1) echo 'selected'; ?> ><?=$res1->option_1?></option>
												<option <? if($value[$res1->form_no][$res1->id]==$res1->option_2) echo 'selected'; ?> ><?=$res1->option_2?></option>
					<?php
					  $sql2 = 'select d.* from rfq_form_element_options d where   rfq_no="'.$_GET['rfq_no'].'" and form_no="'.$res->form_no.'" ';
					  $qry2 = db_query($sql2);
					  while($res2 = mysqli_fetch_object($qry2)){
					?> 
					  <option <? if($value[$res1->form_no][$res1->id]==$res2->options) echo 'selected'; ?> ><?=$res2->options?></option>
					<? } ?>
						 				</select>
						 </td>
					   </tr>
					 ...
					 <? if($res1->form_element=='Checkbox'){?>   
						 ...
						 <? if($res1->option_1 != ''){?>
						  <label class="form-check-label">
							<input type="checkbox" <? if($value[$res1->form_no][$res1->id]==$res1->option_1) echo 'checked'; ?> name="<?=$res1->id?>" id="<?=$res1->id?>" class="form-check-input" 
							value="<?=$res1->option_1?>" >
							<?=$res1->option_1?>
						  </label>
						  <? } ?>
						...
						<tr>
						  ...
						  <? if($res1->option_2 != ''){?>
						  <label class="form-check-label">
							<input type="checkbox" <? if($value[$res1->form_no][$res1->id]==$res1->option_2) echo 'checked'; ?> name="<?=$res1->id?>" id="<?=$res1->id?>" class="form-check-input" 
							value="<?=$res1->option_2?>" >
							<?=$res1->option_2?>
						  </label>
						  <? } ?>
			...
			<? if($res1->form_element=='Text Field'){?>
					<tr>
						 <td><?=$res1->lebel?></td>
						 <td><input type="text" name="<?=$res1->id?>" value="" id="<?=$res1->id?>"/></td>
					</tr>
			<? } ?>		
			<!------------ TextBox------------------------------------------------------------------------------> 
			...
			<? if($res1->form_element=='Text Area'){?>
					<tr>
						 <td><?=$res1->lebel?></td>
						 <td><textarea name="<?=$res1->id?>" id="<?=$res1->id;?>"><? $value[$res1->form_no][$res1->id]; ?></textarea></td>
					</tr>
			<? } ?>	
			...
			<? if($res1->form_element=='File'){?>
					<tr>
						 ...
						 <div class="attachment-toggle" >
								...
								<form>
										  ...
										  <div class="fileuploadcontainer" >
											...
											<div class="attachmentshowcontainer container row m-0">
                        <?
						  $sql100 = 'SELECT * FROM rfq_documents_information WHERE rfq_no = "'.$_SESSION[$unique].'" AND tr_from ="sourcing_basic_settings"';
						  $qry100100 = db_query($sql100);
						  while($item=mysqli_fetch_object($qry100100)){
						?>
                                      <div class="col-sm-10 col-md-10 col-lg-10 pb-1">
										<div class="rounded p-2" style="background-color: #f7f7f7 !important; border: 1px solid #e6e6e6!important; ">
											<a href="../../../../../assets/support/api_upload_attachment_show.php?name='+item.new_name+'" target="_blank">
												<i class="fa-light fa-file fa-2xl fs-22" style="color: #d6960a;"></i> 
                       ...
                       <? } ?>
											  ...
											  <div class="attachmentUrlshowcontainer container row m-0">
											<?php
												$sql100 = 'select * from rfq_documents_url_information where tr_from like "sourcing_basic_settings" and rfq_no="'.$_SESSION[$unique].'" and entry_by = "'.$_SESSION['user']['id'].'"  AND attachment_url IS NOT NULL';
												$qry100100 = db_query($sql100);
												...
												<div class="col-sm-12 col-md-12 col-lg-12 pb-1">
													<div class="rounded p-2" style=" word-break: break-all; background-color: #f7f7f7 !important; border: 1px solid #e6e6e6!important; ">
														<span><i class="fa-solid fa-text-size fa-xl" style="color: #df5c16;"></i>
															...
															<button type="button" style=" display: inline !important;" class="border-0" onclick="deleteAttachmentTextseventinfo(this,'<?=$res100->documents_url_id?>', '<?=$res100->rfq_no?>', '5', '<?=$res100->entry_by?>', 'attachmenttextuploadcontainereventinfo','attachmentshowcontainereventinfo-text')">
																<i class="fa-solid fa-xmark"></i>
															</button>
													</div>
												</div>
											<? } ?>
											  ...
											  <div class="attachmentTextshowcontainer container row m-0">
											  <?php
												$sql100 = 'select * from rfq_documents_url_information where tr_from like "sourcing_basic_settings" and rfq_no="'.$_SESSION[$unique].'" and entry_by = "'.$_SESSION['user']['id'].'"  AND attachment_text IS NOT NULL';
												$qry100100 = db_query($sql100);
												while($res100 = mysqli_fetch_object($qry100100)){
											?> 
											<div class="col-sm-8 col-md-8 col-lg-8 pb-1">
													<div class="rounded p-2" style="word-break: break-all; background-color: #f7f7f7 !important; border: 1px solid #e6e6e6!important; ">
													<a href="<?=$res100->attachment_url?>" target="_blank"><i class="fa-solid fa-text-size fa-xl" style="color: #df5c16;"></i>
														<span><?=$res100->attachment_text?></span></a>
															<button type="button" style=" display: inline !important;" class="border-0" onclick="deleteAttachmentURLseventinfo(this,'<?=$res100->documents_url_id?>' , '<?=$res100->rfq_no?>', 'sample_value', '<?=$res100->entry_by?>', 'attachmenturluploadcontainereventinfo','attachmentshowcontainereventinfo-url')"><i class="fa-solid fa-xmark"></i>
															</button>
													</div>
												</div>
		...
		<div class="pt-1">
  	    ...
  	    <table class="table1  table-striped table-bordered table-hover table-sm">
					...
					<?php
						  ...
						  echo 'rfq_no='.$rfq->rfq_no.' and item_id='.$res->item_id.' and vendor_id='.$vendor.' ';
					...
					?> 
					    <tr>
							<td><?=$res->item_name?></td>
							<td><?=$res->item_desc?></td>
							<td><input type="number" id="capacity<?=$res->id?>" required name="capacity<?=$res->id?>" value="<?=$itemResponse->capacity?>" ></td>
                            <td><?=$res->expected_qty?></td>
							<td><input type="number" id="vendor_price<?=$res->id?>" required name="vendor_price<?=$res->id?>" onkeyup="cal(<?=$res->id?>)" value="<?=$itemResponse->unit_price?>" >
...
$.ajax({
    url: 'https://robi.clouderp.com.bd/eProcurement_mod/api/new_api_file_upload_vendor.php',
    type: 'POST',
    data: formData,
    xhr: function () {
...
$.each(responseObject, function(index, item) {
	...
	var card = $('<div class="col-sm-10 col-md-10 col-lg-10 pb-1"><div class="rounded p-2" style="background-color: #f7f7f7 !important; border: 1px solid #e6e6e6!important; "><a href="../../../../../assets/support/api_upload_attachment_show.php?name='+item.new_name+'&folder='+item.tr_from+'" target="_blank"><i class="fa-light fa-file fa-2xl fs-22" style="color: #d6960a;"></i> <span>'+item.original_name+'</span></a><button type="button" style=" display: inline !important;" class="border-0" onclick="deleteAttachmentseventinfo(this,\''+item.attachment_id+'\',\''+item.rfq_no+'\',\''+item.tr_from+'\',\''+item.entry_by+'\',\''+motherContainerValue+'\',\''+datashowContainerValue+'\')"><i class="fa-solid fa-xmark"></i></button></div></div>');
...
<script>
    ...
    const countdownTimer = setInterval(function() {
      ...
      // Ensure both days and hours are formatted correctly
    ...
    function formatTime(time) {
      return time < 10 ? `0${time}` : time; // Ensure leading zero for single-digit values
    }
</script>
