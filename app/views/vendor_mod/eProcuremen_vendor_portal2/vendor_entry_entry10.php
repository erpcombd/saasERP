...
<h1 class="h1 m-0 p-0 pt-4 pl-3"><i class="fa-duotone fa-calendar-days"></i> Timeline </h1>
		...
		<?
			...
			$sql = ' select count(tr_from) from documents_information where tr_from like "sourcing_terms_condition" and master_id='.$rfq->rfq_no;
			$query = db_query($sql);
			$result = mysqli_fetch_row($query);
			$required = $result[0];
			
			$sql = ' select count(tr_from) from documents_url_information where tr_from like "sourcing_terms_condition" and master_id='.$rfq->rfq_no;
			$query = db_query($sql);
			$result = mysqli_fetch_row($query);
			$required += $result[0];
...
?>     <form >
		...
		<div class="row m-5 p-0 pt-1">
			...
			<h1 class="m-0 p-0  fs-22">Response t
			  <?=$res->section_name;?>
			  o </h1>
			...
			<div class="attachment-toggle">
						...
						<div class="fileuploadcontainer" >
							...
							<input type="hidden" name="master_id" value="<?=$rfq->rfq_no;?>">
							...
							<div class="attachmentshowcontainer container row m-0">
		  <?php
		  $sql = 'select * from documents_information where tr_from like "sourcing_attachment_response" and master_id="'.$_GET['rfq_no'].'"';
		  $qry = db_query($sql);
		  while($res = mysqli_fetch_object($qry)){
		  ...
		  ?>
								<div class="col-sm-10 col-md-10 col-lg-10 pb-1">
									...
									<a href="../../../../assets/support/upload_view.php?name=<?php echo $res->new_name;?>&mod=eProcurement_mod&folder=rfq&proj_id=robi" target="_blank">
									<i class="fa-light fa-file fa-2xl fs-22" style="color: #d6960a;"></i> <span><?=$res->original_name?></span></a>
									<button type="button" style=" display: inline !important;" class="border-0" onclick="deleteAttachmentseventinfo(this,'<?=$res->documents_id?>','<?=$rfq->rfq_no?>','sourcing_attachment_response','<?=$_SESSION['user']['id']?>','fileuploadcontainer','attachmentshowcontainer')">
...
<!-------------------------------------------------------------------------------------------------------------------->		
	...
	<?php
		  $sql = 'select * from rfq_form_response where   rfq_no="'.$_GET['rfq_no'].'" and vendor_id = '.$vendor.' ';
		  $qry = db_query($sql);
		  while($res = mysqli_fetch_object($qry)){
		  
		     echo $value[$res->form_no][$res->form_id] = $res->value;
...
<?php
  $sql = 'select d.* from rfq_form_details d where   rfq_no="'.$_GET['rfq_no'].'" group by form_no ';
  $qry = db_query($sql);
  while($res = mysqli_fetch_object($qry)){
?> 
		<div class="row">
					...
					<div class="attachment-toggle">
								...
								<span class="attachment-toggle-add-file icon-search fs-14" style="cursor: pointer; color: blue;" onmouseover="this.style.color='orange'" onmouseout="this.style.color='blue'">File</span>
										  ...
										  <div class="fileuploadcontainer" >
											...
											<div class="attachmentshowcontainer container row m-0">
                        <?
						  $sql = 'SELECT * FROM documents_information WHERE master_id = "'.$_SESSION[$unique].'" AND tr_from ="sourcing_basic_settings"';
						  $qry = db_query($sql);
                       ...
                       <?}?>
											  ...
											  <div class="attachmentUrlshowcontainer container row m-0">
											<?php
												$sql = 'select * from documents_url_information where tr_from like "sourcing_basic_settings" and master_id="'.$_SESSION[$unique].'" and entry_by = "'.$_SESSION['user']['id'].'"  AND attachment_url IS NOT NULL';
												$qry = db_query($sql);
												while($res = mysqli_fetch_object($qry)){
											...
											<? } ?>
											  ...
											  <div class="attachmentTextshowcontainer container row m-0">
											  <?php
												$sql = 'select * from documents_url_information where tr_from like "sourcing_basic_settings" and master_id="'.$_SESSION[$unique].'" and entry_by = "'.$_SESSION['user']['id'].'"  AND attachment_text IS NOT NULL';
												$qry = db_query($sql);
												while($res = mysqli_fetch_object($qry)){
											?> 
											<div class="col-sm-8 col-md-8 col-lg-8 pb-1">
													<div class="rounded p-2" style="word-break: break-all; background-color: #f7f7f7 !important; border: 1px solid #e6e6e6!important; ">
													<a href="<?=$res->attachment_url?>" target="_blank"><i class="fa-solid fa-text-size fa-xl" style="color: #df5c16;"></i>
