...
//do_calander('#tdate');
//echo $_SESSION['rfq_req_no'];
$module_name = find_a_field('user_module_manage','module_file','id='.$_SESSION["mod"]);

$req_data = find_all_field('requisition_master','','req_no="'.$_SESSION['rfq_req_no'].'"');

$req_no=$_SESSION['rfq_req_no'];
...
window.location.href = "mr_create.php?request_no=<?=$invoice_no?>";
</script>
...
db_query($foe_invoice);
	...
	$con_sql1="UPDATE purchase_quotation_master SET status='".$status."', checked_by='".$entry_by."', checked_at='".$entry_at."' WHERE invoice_no = '".$invoice_no."' ";
	 db_query($con_sql1);
	 //$update = 'update rfq_vendor_info set status=""';
...
<script language="javascript">
window.location.href = "../req_list/req_list.php";
</script>
...
<div class="box" style="width:100%;">
	...
	<input name="view_req_no" type="text" id="view_req_no" style="width:90%; height:32px;" value="<?=$ms_data->view_req_no;?>" readonly="" required tabindex="1" />	</td>
	...
	<select name="group_for" id="group_for" style="width:90%; height:32px;" required>
		...
		<? foreign_relation('user_group','id','group_name',$_POST['group_for'],'id="'.$ms_data->group_for.'"');?>
								  ...
								  <tr>
								    ...
								    <td width="18%" align="right">
									<input type="text" list="vendor" name="vendor_id" id="vendor_id"  required  value="<?=find_a_field('vendor','vendor_name','vendor_id="'.$_SESSION['vendor_id'].'"');?>"/>
...
//		$sql = "select id, item_type from item_type where 1 group by id";
//		 $query = db_query($sql);
//		
//		 while($info=mysqli_fetch_row($query)){
//  		 $item_type[$info->id]=$info->item_type;
//		}
		
	//	$sql = "select id, count from count_type where 1 group by id";
//		 $query = db_query($sql);
//	
//		 while($info=mysqli_fetch_row($query)){
//  		 $count_type[$info->id]=$info->count;
//		}
