...
<div class="container-fluid pt-5 p-0 ">
			...
			<table id="vendor_table" class="table1  table-striped table-bordered table-hover table-sm">
				...
				where   a.group_for=u.id and a.'.$unique.'="'.$user_to_vendor.'" and a.group_for="'.$_SESSION['user']['group'].'"    '.$con.' order by a.vendor_id ';
				...
				while($rp=mysqli_fetch_row($report)){$i++; if($i%2==0)$cls=' class="alt"'; else $cls='';?>
					...
					<tr<?=$cls?>  bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
						<td><? foreign_relation('vendor','vendor_id','','vendor_id="'.$user_to_vendor.'"');?></td>

						<td style="text-align:left"><?=$rp[1];?></td>

						<td><?=$rp[3];?></td>
						<td style="text-align:left"><?=$rp[2];?></td>
						<td><input type="button" onclick="DoNav('<?php echo $rp[0];?>');" class="btn1 btn1-bg-update" value="Edit"/></td>
					</tr>
				<?php }?>
				</tbody>
			</table>
