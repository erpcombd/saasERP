...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
		...
		$receipt_no = next_transection_no($group_for,$receipt_date,'lc_bill_payment','payment_no');
 ...
 if($_POST['company_id']!='')
  $company_con=" and company_id=".$_POST['company_id'];
		 ...
		 $sql = "SELECT * FROM insurance_payment_voucher WHERE 1 ".$company_con." group by id";
...

		{
			...
			if($_POST['insurance_exp_'.$data->id]>0)

			{
				...
				$refund_amt=$_POST['refund_amt_'.$data->id];
				$insurance_exp=$_POST['insurance_exp_'.$data->id];
				$order_no=$_POST['order_no_'.$data->id];



   $so_invoice = 'INSERT INTO insurance_refund_voucher (receipt_no, receipt_date, order_no, payment_no, payment_date, insurance_no, group_for, lc_no, lc_number, ledger_id, company_id, company_ledger, receipt_dr_ledger, payment_amt, refund_amt, insurance_exp, cheque_no, cheque_date, of_bank, entry_at, entry_by)
  
  VALUES("'.$receipt_no.'", "'.$receipt_date.'", "'.$order_no.'", "'.$data->payment_no.'", "'.$data->payment_date.'", "'.$data->insurance_no.'", "'.$data->group_for.'", "'.$data->lc_no.'", "'.$data->lc_number.'", "'.$data->ledger_id.'", "'.$data->company_id.'", "'.$data->company_ledger.'", "'.$receipt_dr_ledger.'", "'.$payment_amt.'", "'.$refund_amt.'", "'.$insurance_exp.'", "'.$cheque_no.'", "'.$cheque_date.'", "'.$of_bank.'", "'.$entry_at.'", "'.$entry_by.'")';
...
$ins3 = 'INSERT INTO lc_bill_payment (payment_no, payment_date, lc_no, lc_number, lc_ledger, bank_pay_id, lc_part_no, group_for, bill_type, bill_category, category_view, cr_ledger_id, pay_amt_in, pay_amt_out, remarks, note, r_no, r_date, status, entry_by, entry_at)
  
  VALUES("'.$receipt_no.'", "'.$receipt_date.'", "'.$data->lc_no.'", "'.$data->lc_number.'", "'.$data->ledger_id.'", "0", "", "'.$data->group_for.'", "1", "1", "Insurance Expense", 
  "'.$data->company_ledger.'", "'.$insurance_exp.'", "0", "'.$remarks.'", "'.$note.'", "'.$r_no.'", "'.$r_date.'", "CHECKED", "'.$entry_by.'", "'.$entry_at.'")';
...
if($receipt_no>0)
{
auto_insert_insurance_refund_secoundary($receipt_no);
...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0" style="font-size:12px; text-transform:uppercase;">
 ...
 $sql = "select order_no, order_no from insurance_refund_voucher where 1 group by order_no ";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
//}
     ...
     $sql = "SELECT id, payment_no, payment_date, lc_no, lc_number, ledger_id, payment_amt FROM insurance_payment_voucher WHERE 1 ".$company_con." group by id";
