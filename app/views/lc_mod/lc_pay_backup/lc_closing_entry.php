...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
		...
		$sql = "SELECT p.*, i.item_name FROM lc_purchase_receive p, item_info i WHERE p.item_id=i.item_id and  p.po_no=".$po_data->po_no." order  by  p.id ";
...
//$xid = db_insert_id();
//journal_item_control($data->item_id, $_SESSION['user']['depot'], $closing_date,  $rec_qty, 0,  'Purchase', $xid, $cost_price, $_SESSION['user']['depot'], $closing_no, '', '', $data->po_no, $lc_no, '', $group_for, $cost_price);
		  	...
		  	<tr>
				<td><input name="company" type="text" id="company"  readonly="" style="width:220px; height:32px;" value="<?=find_a_field('user_group','group_name','id='.$lc_data->group_for); ?>"   tabindex="105" />	</td>
								...
								<select name="vendor_id" id="vendor_id" required style="width:220px;" >
										...
										<? foreign_relation('vendor','vendor_id','vendor_name',$_POST['vendor_id'],'vendor_id="'.$po_data->vendor_id.'"');?>
									</select>									</td>
									<td>VENDOR AGENT:</td>
									<td>
...
$tot_rec_amt_usd[$data->lc_no]=$data->tot_rec_amt_usd;
	...
	WHERE a.id=b.bill_type and b.id=c.bill_category and c.lc_no=".$lc_data->id." and a.id!=3 group by b.bill_type order by c.bill_type ";
