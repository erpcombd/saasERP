...
$con .= ' and a.id in ('.$_POST['lc_no'].') ';
      ...
      $res="select a.id, p.pi_no, p.po_date, a.lc_number, a.group_for,a.status as master_status, c.status from lc_number_setup a, lc_purchase_master p,lc_commercial_invoice c  where a.id=p.lc_no and p.lc_no=c.lc_no ".$con."  group by p.lc_no order by p.po_date";
...
<?php 
	if($data->master_status=='CHECKED'){
							echo "<span style='color:red;font-weight:bold;'>Receive Pending</span>";
							}
							elseif($data->master_status=='COMPLETED'){
							echo "<span style='color:blue;font-weight:bold;'>Receive Done</span>";
							}
