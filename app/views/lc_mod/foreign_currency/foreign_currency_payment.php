...
$title='Foreign Currency Payment Entry';
...

{
		...
		$currency_ledger = $_POST['currency_ledger'];
...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
		...
		$invoice_no = next_transection_no($tr_unique,$invoice_date,'foreign_currency_payment','invoice_no');
		...
		$sql = "SELECT p.id, p.invoice_no, p.invoice_date, p.ledger_id, p.purchase_amt, p.exchange_rate, p.total_amt_bdt, a.ledger_name FROM foreign_currency_purchase p, accounts_ledger a
	WHERE p.ledger_id=a.ledger_id ".$foreign_currency_con." order  by p.invoice_date, p.id";
		...
		{
			if($_POST['pay_amt_bdt_'.$data->id]>0)

			{

				$order_no=$_POST['order_no_'.$data->id];
				$ledger_id=$_POST['ledger_id_'.$data->id];
				$pay_amt=$_POST['pay_amt_'.$data->id];
				$exchange_rate=$_POST['exchange_rate_'.$data->id];
				$pay_amt_bdt=$_POST['pay_amt_bdt_'.$data->id];

   $ins_invoice = 'INSERT INTO foreign_currency_payment (invoice_no, invoice_date, group_for, purchase_no, order_no, reference_by, remarks, dr_ledger_id, ledger_id, pay_amt, exchange_rate, pay_amt_bdt, entry_by, entry_at)
  
  VALUES("'.$invoice_no.'", "'.$invoice_date.'", "'.$group_for.'", "'.$data->invoice_no.'", "'.$order_no.'", "'.$reference_by.'", "'.$remarks.'", "'.$dr_ledger_id.'", "'.$ledger_id.'", "'.$pay_amt.'", "'.$exchange_rate.'", "'.$pay_amt_bdt.'", "'.$entry_by.'", "'.$entry_at.'")';
...
$ledger_group = find_a_field('accounts_ledger','ledger_group_id','ledger_id="'.$dr_ledger_id.'"');
...
if($ledger_group==1013) {
		...
		$pay_data = find_all_field('foreign_currency_payment','','invoice_no="'.$invoice_no.'"');


 $ins_hnd = 'INSERT INTO handcash_payment (pay_id, year, payment_no, payment_date, group_for, transaction_type, reference_by, remarks, cr_ledger_id, handcash_id, ledger_id, payment_amt, note, currency_pay_id, currency_pay_invoice, currency_purchase_no, currency_order_no, currency_pay_amt, exchange_rate, entry_by, entry_at)
  
  VALUES("'.$pay_id.'", "'.$YR.'", "'.$payment_no.'", "'.$invoice_date.'", "'.$group_for.'", "FOREIGN CURRENCY", "'.$reference_by.'", "'.$remarks.'", "'.$currency_ledger.'", "'.$handcash_id.'", "'.$dr_ledger_id.'", "'.$pay_data->pay_amt_bdt.'", "FOREIGN CURRENCY", "'.$pay_data->id.'", "'.$pay_data->invoice_no.'", "'.$pay_data->purchase_no.'", "'.$pay_data->order_no.'", "'.$pay_data->pay_amt.'", "'.$pay_data->exchange_rate.'", "'.$entry_by.'", "'.$entry_at.'")';
...
window.location.href = "foreign_currency_payment.php";
</script>
...
var pay_amt = document.getElementById('pay_amt_'+id).value*1;
var exchange_rate = document.getElementById('exchange_rate_'+id).value*1;
var pay_amt_bdt = document.getElementById('pay_amt_bdt_'+id).value= (pay_amt*exchange_rate);



 if(pay_amt>balance_amt)
  {
alert('Can not entry more than balance amount.');

document.getElementById('pay_amt_'+id).value='';
document.getElementById('pay_amt_bdt_'+id).value='';
...
<div class="box" style="width:100%;">
    ...
    <td align="right" ><strong>Foreign Currency: </strong></td>
...
<div class="box" style="width:100%;">
			...
			<input style="width:220px; height:32px;"  name="invoice_date" type="text" id="invoice_date"  value="<?=($invoice_date!='')?$invoice_date:date('Y-m-d')?>"   required />									</td>
									...
									<input style="width:220px; height:32px;" name="currency_ledger" type="hidden" id="currency_ledger"  value="<?=$_POST['foreign_currency']?>"    />
								 ...
								 <td>PAYMENTLEDGER (DR):</td>
									...
									<input style="width:220px; height:32px;" name="reference_by" type="text" id="reference_by"  value="<?=$_POST['reference_by']?>"  required="required"   />
									</td>
								  </tr>
...
//}
 ...
 $sql = "select order_no, sum(pay_amt) as pay_amt  from foreign_currency_payment where 1 group by order_no";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
$pay_amt[$data->order_no]=$data->pay_amt;
}
...
<? if ($due_amt=$data->purchase_amt-$pay_amt[$data->id]>0) {?>
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
    ...
    <td><?=number_format($data->purchase_amt,2);?></td>
    <td><?=number_format($pay_amt[$data->id],2);?></td>
    <td><?=number_format($balance_amt =$data->purchase_amt-$pay_amt[$data->id],2); ?>
...
<input name="ledger_id_<?=$data->id?>" id="ledger_id_<?=$data->id?>" type="hidden"   value="<?=$data->ledger_id?>"  onkeyup="EXPcalculation(<?=$data->id?>)" style="width:80px; height:25px;"  />
	
	
 <input name="pay_amt_<?=$data->id?>" id="pay_amt_<?=$data->id?>" type="text"  value=""  onkeyup="EXPcalculation(<?=$data->id?>)" style="width:80px; height:25px;"   />	</td>
    ...
    <td align="center"><input name="pay_amt_bdt_<?=$data->id?>" id="pay_amt_bdt_<?=$data->id?>" type="text"  readonly=""  value="" style="width:100px; height:25px;"  />	</td>
  </tr>
