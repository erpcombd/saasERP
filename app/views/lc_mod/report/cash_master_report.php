...
//if($_REQUEST['do_type']>0) 			$do_type=$_REQUEST['do_type'];
...
elseif($_POST['report']=='3333'){
       ...
       <table width="100%">
		...
		<?php /*?><img src="<?=SERVER_ROOT?>public/uploads/logo/fg-logo.png" style="width:100px;"><?php */?>
		</td>
        ...
        <tr>
		 ...
		 <? if ($_POST['sale_type']!='') {?>
		 <tr>
          <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;"><strong>SALES TYPE: <?=$_POST['sale_type'];?></strong></td>
         </tr>
		 <? }?>
        ...
        <table width="100%" cellspacing="0" cellpadding="0" border="1" id="ExportTable" bordercolor="#000000" >
      <thead>
	  <?
		...
		if($_POST['sale_type']!='')	
		$sale_type_con .= ' and b.do_type = "'.$_POST['sale_type'].'" ';
...
'.$group_for_con.$zone_con.$item_sub_con.$item_group_con.$item_mother_group_con.$item_type_con.$dealer_con.$sale_type_con.'  group by c.dealer_code, month(a.chalan_date), year(a.chalan_date) order by a.chalan_date';
...
if($_POST['sale_type']=='REGULAR' || $_POST['sale_type']=='') {	
		
		 $year_sql = 'select year(b.do_date) yr, month(b.do_date) mon, sum(a.return_amt) as return_amt, b.dealer_code,c.zone_code, z.ZONE_NAME 
		 from sale_return_details a, sale_return_master b, dealer_info c, zon z,  item_info i, item_sub_group s, item_group g where 
 i.item_id=a.item_id and s.sub_group_id=i.sub_group_id  and s.group_id=g.group_id and a.do_no = b.do_no and b.dealer_code = c.dealer_code and c.zone_code=z.ZONE_CODE and
a.do_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods"
'.$group_for_con.$zone_con.$item_sub_con.$item_group_con.$item_mother_group_con.$item_type_con.$dealer_con.'  group by c.dealer_code, month(b.do_date), year(b.do_date) order by b.do_date';
		...
		while($value = mysqli_fetch_object($year_query)){
		    ...
		    $val_return[$value->dealer_code][$pr] = $value->return_amt;
			
		    $dealer_total_return[$value->dealer_code] = $dealer_total_return[$value->dealer_code] + $value->return_amt;
			
		    $mon_total_return[$pr] = $mon_total_return[$pr] + $value->return_amt;
			
		    $zone_total_return[$value->zone_code][$pr] = $zone_total_return[$value->zone_code][$pr] +  $value->return_amt;
			
			$zone_gtotal_return[$value->zone_code] = $zone_gtotal_return[$value->zone_code] +  $value->return_amt;
			$all_total_return = $all_total_return +  $value->return_amt;
...
//		echo $sql_101 = 'select c.dealer_code, c.dealer_name_e, z.ZONE_CODE zone_code, z.ZONE_NAME zone_name 
...
//		and c.zone_code=z.ZONE_CODE and a.chalan_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods" 
//		'.$group_for_con.$zone_con.$item_sub_con.$item_group_con.$item_mother_group_con.$item_type_con.$dealer_con.' group by  c.dealer_code  order by c.zone_code';
		...
		$sql_101 = 'select c.dealer_code, c.dealer_name_e, z.ZONE_CODE zone_code, z.ZONE_NAME zone_name  from dealer_info c, zon z where c.zone_code=z.ZONE_CODE 
		'.$zone_con.$dealer_con.' group by  c.dealer_code  order by c.zone_code';
		...
		if($zone_gtotal[$old_zone_code]-$zone_gtotal_return[$old_zone_code]<>0)
		{
		?>
		<tr bgcolor="#E1F4FA" style=" font-weight:700; ">
        <td colspan="2" bgcolor="#E1F4FA" style="text-transform:uppercase">Total <?=$old_zone_name?></td>
        <? for($i=1;$i<=$t;$i++){ ?><td bgcolor="#E1F4FA"><?  echo  number_format($zone_total[$old_zone_code][$period[$i]]-$zone_total_return[$old_zone_code][$period[$i]],2);?></td> 
        <? }?> 
		<td bgcolor="#E1F4FA"><strong><?  echo  number_format($zone_gtotal[$old_zone_code]-$zone_gtotal_return[$old_zone_code],2);?></strong></td>
        </tr>
		<?
		}
		}
		...
		if($dealer_total[$r->dealer_code]-$dealer_total_return[$r->dealer_code]<>0) {
        ?>
        <tr>
          <td><?=$sl++?></td>
        <td><?=$r->dealer_name_e;?></td>
        <? for($i=1;$i<=$t;$i++){ 
		?><td><?=number_format($val[$r->dealer_code][$period[$i]]-$val_return[$r->dealer_code][$period[$i]],2);?></td> <? }?> 
        

        <td><?=number_format($dealer_total[$r->dealer_code]-$dealer_total_return[$r->dealer_code],2);?></td>
        ...
        <? for($i=1;$i<=$t;$i++){ ?><td><?  echo  number_format($zone_total[$old_zone_code][$period[$i]]-$zone_total_return[$old_zone_code][$period[$i]],2);?></td> <? }?> 
		<td><strong><?  echo  number_format($zone_gtotal[$old_zone_code]-$zone_gtotal_return[$old_zone_code],2);?></strong></td>
        </tr>
		<? }?>
		<tr>
        <td colspan="2" style="font-weight:700;">GRAND TOTAL</td>
        <? for($i=1;$i<=$t;$i++){ ?><td><strong><?  echo number_format($mon_total[$period[$i]]-$mon_total_return[$period[$i]],2);?></strong></td> <? }?> 
		<td><strong><?=number_format($all_total-$all_total_return,2);?></strong></td>
        </tr>
...
elseif($_POST['report']=='333311'){
	  ...
	  if($nm_date>$ets_date) break;
	           ...
	           $year_sql = 'select year(a.chalan_date) yr, month(a.chalan_date) mon, sum(a.total_unit) as total_amt, b.dealer_code,c.zone_code, z.ZONE_NAME from sale_do_chalan a, sale_do_master b, dealer_info c, zon z,  item_info i, item_sub_group s, item_group g where 
 i.item_id=a.item_id and s.sub_group_id=i.sub_group_id  and s.group_id=g.group_id and a.do_no = b.do_no and b.dealer_code = c.dealer_code and c.zone_code=z.ZONE_CODE and
a.chalan_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods" and g.group_id!=900000000
'.$group_for_con.$zone_con.$item_sub_con.$item_group_con.$item_mother_group_con.$item_type_con.$dealer_con.$sale_type_con.'  group by c.dealer_code, month(a.chalan_date), year(a.chalan_date) order by a.chalan_date';
...
if($_POST['sale_type']=='REGULAR' || $_POST['sale_type']=='') {	
		 ...
		 $year_sql = 'select year(b.do_date) yr, month(b.do_date) mon, sum(a.total_unit) as return_amt, b.dealer_code,c.zone_code, z.ZONE_NAME 
		 from sale_return_details a, sale_return_master b, dealer_info c, zon z,  item_info i, item_sub_group s, item_group g where 
 i.item_id=a.item_id and s.sub_group_id=i.sub_group_id  and s.group_id=g.group_id and a.do_no = b.do_no and b.dealer_code = c.dealer_code and c.zone_code=z.ZONE_CODE and
a.do_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods" and g.group_id!=900000000
'.$group_for_con.$zone_con.$item_sub_con.$item_group_con.$item_mother_group_con.$item_type_con.$dealer_con.'  group by c.dealer_code, month(b.do_date), year(b.do_date) order by b.do_date';
...
//		'.$group_for_con.$zone_con.$item_sub_con.$item_group_con.$item_mother_group_con.$item_type_con.$dealer_con.' group by  c.dealer_code  order by c.zone_code';
		...
		{
		?>
		<tr bgcolor="#E1F4FA" style=" font-weight:700; font-size:14px;">
        <td colspan="2" bgcolor="#E1F4FA" style="text-transform:uppercase">Total <?=$old_zone_name?></td>
        <? for($i=1;$i<=$t;$i++){ ?><td bgcolor="#E1F4FA"><?  echo  number_format($zone_total[$old_zone_code][$period[$i]]-$zone_total_return[$old_zone_code][$period[$i]],2);?></td> 
        <? }?> 
		<td bgcolor="#E1F4FA"><strong><?  echo  number_format($zone_gtotal[$old_zone_code]-$zone_gtotal_return[$old_zone_code],2);?></strong></td>
		...
		<? }?>
		<tr>
        <td colspan="2" style="font-weight:700; font-size:14px;">GRAND TOTAL</td>
        <? for($i=1;$i<=$t;$i++){ ?><td><strong><?  echo number_format($mon_total[$period[$i]]-$mon_total_return[$period[$i]],2);?></strong></td> <? }?> 
		<td><strong><?=number_format($all_total-$all_total_return,2);?></strong></td>
        </tr>
...
a.chalan_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods" and g.group_id!=900000000
'.$group_for_con.$zone_con.$item_sub_con.$dealer_con.$item_group_con.$item_mother_group_con.$item_type_con.$sale_type_con.'  group by a.item_id, month(a.chalan_date), year(a.chalan_date) ';
...
if($_POST['sale_type']=='REGULAR' || $_POST['sale_type']=='') {
		 ...
		 $year_sql = 'select year(b.do_date) yr, month(b.do_date) mon, sum(a.return_amt) as return_amt, i.item_id, s.sub_group_id from 
			   sale_return_details a, sale_return_master b, dealer_info c, zon z,  item_info i, item_sub_group s, item_group g where 
 i.item_id=a.item_id and a.do_no = b.do_no and b.dealer_code = c.dealer_code and c.zone_code=z.ZONE_CODE and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id and
b.do_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods" and g.group_id!=900000000
'.$group_for_con.$zone_con.$item_sub_con.$dealer_con.$item_group_con.$item_mother_group_con.$item_type_con.'  group by a.item_id, month(b.do_date), year(b.do_date) ';
		...
		while($value = mysqli_fetch_object($year_query)){
		    ...
		    $val_return[$value->item_id][$pr] = $value->return_amt;
			
		    $item_total_return[$value->item_id] = $item_total_return[$value->item_id] + $value->return_amt;
			
		    $mon_total_return[$pr] = $mon_total_return[$pr] + $value->return_amt;
			
		    $sub_group_total_return[$value->sub_group_id][$pr] = $sub_group_total_return[$value->sub_group_id][$pr] +  $value->return_amt;
			
			$sub_group_gtotal_return[$value->sub_group_id] = $sub_group_gtotal_return[$value->sub_group_id] +  $value->return_amt;
			$all_total_return = $all_total_return +  $value->return_amt;
...
//		and a.chalan_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods" and g.group_id!=900000000 group by i.item_id
//		order by  s.sub_group_sl, i.item_name ';
		
		
		 $sql_101 = 'select i.item_id, i.item_name, s.sub_group_id, s.sub_group_name 
		 from item_info i, item_sub_group s, item_group g
		 where  i.sub_group_id=s.sub_group_id and s.group_id=g.group_id '.$item_sub_con.$group_for_item_con.$item_mother_group_con.$item_type_con.'
		 and g.product_type="Finish Goods" and g.group_id!=900000000 group by i.item_id
		order by  s.sub_group_sl, i.item_name ';
		...
		if($sub_group_gtotal[$old_sub_group_id]-$sub_group_gtotal_return[$old_sub_group_id]<>0)
		{
		?>
		<tr bgcolor="#E1F4FA" style=" font-weight:700; ">
        <td colspan="2" style="text-transform:uppercase">Total <?=$old_sub_group_name?></td>
        <? for($i=1;$i<=$t;$i++){ ?><td><?  echo  number_format($sub_group_total[$old_sub_group_id][$period[$i]]-$sub_group_total_return[$old_sub_group_id][$period[$i]],2);?></td> <? }?> 
		<td><strong><?  echo  number_format($sub_group_gtotal[$old_sub_group_id]-$sub_group_gtotal_return[$old_sub_group_id],2);?></strong></td>
        </tr>
		<?
		}
		}
		
		if($item_total[$r->item_id]-$item_total_return[$r->item_id]<>0) {
        ?>
        <tr>
          <td><?=$sl++?></td>
        <td><?=$r->item_name;?></td>
        <? for($i=1;$i<=$t;$i++){ 
		?><td><?=number_format($val[$r->item_id][$period[$i]]-$val_return[$r->item_id][$period[$i]],2);?></td> <? }?> 
        

        <td><?=number_format($item_total[$r->item_id]-$item_total_return[$r->item_id],2);?></td>
...
if($_POST['sale_type']=='REGULAR' || $_POST['sale_type']=='') {
		 ...
		 $year_sql = 'select year(b.do_date) yr, month(b.do_date) mon, sum(a.total_unit) as return_amt, i.item_id, s.sub_group_id from 
			   sale_return_details a, sale_return_master b, dealer_info c, zon z,  item_info i, item_sub_group s, item_group g where 
 i.item_id=a.item_id and a.do_no = b.do_no and b.dealer_code = c.dealer_code and c.zone_code=z.ZONE_CODE and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id and
b.do_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods" and g.group_id!=900000000
'.$group_for_con.$zone_con.$item_sub_con.$dealer_con.$item_group_con.$item_mother_group_con.$item_type_con.'  group by a.item_id, month(b.do_date), year(b.do_date) ';
...
elseif($_POST['report']=='210501004'){
       ...
       <table width="100%">
	   <tr>
		<td width="25%" align="center" style="border:0px; border-color:white;"><?php /*?><img src="<?=SERVER_ROOT?>public/uploads/logo/fg-logo.png" style="width:100px;" /><?php */?></td>
		<td  width="50%" style="border:0px; border-color:white;">
			<table width="100%"  >
				<tr align="center" >
					<td style="font-size:20px; border:0px; border-color:white;"><strong><?=$_SESSION['company_name'];?></strong></td>	
		...
		<td  width="25%" align="center" style="border:0px; border-color:white;">
		<?php /*?><img src="<?=SERVER_ROOT?>public/uploads/logo/fg-logo.png" style="width:100px;">	<?php */?>	</td>
		</tr>
...
b.pr_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods"
'.$group_for_con.$zone_con.$item_sub_con.$dealer_con.$item_group_con.$item_mother_group_con.$item_type_con.'  group by a.item_id, month(b.pr_date), year(b.pr_date) order by b.pr_date';
	  ...
	  ?>
		...
		if ($group_for==4) {
		 ...
		 '.$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con.' and g.product_type="Finish Goods" and g.group_id!=900000000 order by  s.group_for, s.sub_group_sl, i.item_type, i.item_name ';
	   	$query_101 = db_query($sql_101 );
		$sl = 1 ;
		while($r = mysqli_fetch_object($query_101)){
...
elseif($_POST['report']=='33333322'){
 ...
 i.item_id=a.item_id and a.do_no = b.do_no and b.dealer_code = c.dealer_code and c.zone_code=z.ZONE_CODE and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id and
a.chalan_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods"  and g.group_id!=900000000
'.$group_for_con.$zone_con.$item_sub_con.$dealer_con.$item_group_con.$item_mother_group_con.$item_type_con.$sale_type_con.'  group by s.sub_group_id, month(a.chalan_date), year(a.chalan_date) order by s.sub_group_id';
...
if($_POST['sale_type']=='REGULAR' || $_POST['sale_type']=='') {
		 ...
		 $year_sql = 'select year(b.do_date) yr, month(b.do_date) mon, sum(a.return_amt) as return_amt, i.item_id, s.sub_group_id, g.group_id from 
			   sale_return_details a, sale_return_master b, dealer_info c, zon z,  item_info i, item_sub_group s, item_group g where 
 i.item_id=a.item_id and a.do_no = b.do_no and b.dealer_code = c.dealer_code and c.zone_code=z.ZONE_CODE and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id and
b.do_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods"  and g.group_id!=900000000
'.$group_for_con.$zone_con.$item_sub_con.$dealer_con.$item_group_con.$item_mother_group_con.$item_type_con.'  group by s.sub_group_id, month(b.do_date), year(b.do_date) order by s.sub_group_id';
		...
		while($value = mysqli_fetch_object($year_query)){
		    ...
		    $val_return[$value->sub_group_id][$pr] = $value->return_amt;
			
		    $item_total_return[$value->sub_group_id] = $item_total_return[$value->sub_group_id] + $value->return_amt;
			
		    $mon_total_return[$pr] = $mon_total_return[$pr] + $value->return_amt;
			
		    $sub_group_total_return[$value->group_id][$pr] = $sub_group_total_return[$value->group_id][$pr] +  $value->return_amt;
			
			$sub_group_gtotal_return[$value->group_id] = $sub_group_gtotal_return[$value->group_id] +  $value->return_amt;
			$all_total_return = $all_total_return +  $value->return_amt;
...
//		 group by s.sub_group_id,g.group_id order by  g.group_for, g.item_group_sl, s.group_for, s.sub_group_sl ';
		  ...
		  $sql_101 = 'select i.item_id, i.item_name, s.sub_group_id, s.sub_group_name, s.sub_group_sl, g.group_id, g.group_name 
		  from item_info i, item_sub_group s, item_group g
		 where i.sub_group_id=s.sub_group_id and s.group_id=g.group_id '.$item_sub_con.$group_for_item_con.$item_mother_group_con.$item_type_con.' and g.product_type="Finish Goods"  
		 and g.group_id!=900000000  group by s.sub_group_id,g.group_id order by  s.sub_group_sl ';
		...
		if($item_total[$r->sub_group_id]-$item_total_return[$r->sub_group_id]<>0) {
        ?>
        <tr>
          <td><?=$sl++?></td>
        <td><?=$r->sub_group_name;?></td>
        <? for($i=1;$i<=$t;$i++){ 
		?><td><?=number_format($val[$r->sub_group_id][$period[$i]]-$val_return[$r->sub_group_id][$period[$i]],2);?></td> <? }?> 
        

        <td><?=number_format($item_total[$r->sub_group_id]-$item_total_return[$r->sub_group_id],2);?></td>
...
if($_POST['sale_type']=='REGULAR' || $_POST['sale_type']=='') {
		 ...
		 $year_sql = 'select year(b.do_date) yr, month(b.do_date) mon, sum(a.total_unit) as return_amt, i.item_id, s.sub_group_id, g.group_id from 
			   sale_return_details a, sale_return_master b, dealer_info c, zon z,  item_info i, item_sub_group s, item_group g where 
 i.item_id=a.item_id and a.do_no = b.do_no and b.dealer_code = c.dealer_code and c.zone_code=z.ZONE_CODE and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id and
b.do_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and g.product_type="Finish Goods"  and g.group_id!=900000000
'.$group_for_con.$zone_con.$item_sub_con.$dealer_con.$item_group_con.$item_mother_group_con.$item_type_con.'  group by s.sub_group_id, month(b.do_date), year(b.do_date) order by s.sub_group_id';
...
'.$group_for_con.$zone_con.$item_sub_con.$dealer_con.$item_group_con.$item_mother_group_con.$item_type_con.'  group by s.sub_group_id, month(b.pr_date), year(b.pr_date) order by s.sub_group_id';
	  ...
	  ?>
		...
		if(isset($item_group)) 						{$item_group_con=' and s.group_id="'.$item_group.'"';}
		 ...
		 group by s.sub_group_id,g.group_id order by   g.item_group_sl, s.sub_group_sl';
	   	$query_101 = db_query($sql_101 );
		$sl = 1 ;
		while($r = mysqli_fetch_object($query_101)){
...
elseif($_POST['report']=='3333334411'){
...
elseif($_POST['report']==20060201)
{
  ...
  <? if ($item_mother_group>0) {?>
		 ...
		 <? if ($_POST['do_type']!='') {?>
		 <tr>
          <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;"><strong>Sales Type: <?=$_POST['do_type'];?></strong></td>
         </tr>
		 <? }?>
		...
		<table width="100%" border="0" cellpadding="2" id="ExportTable" cellspacing="0">
		
		<thead>
	
		<tr>
		<th width="2%" rowspan="2" bgcolor="#99CCFF">SL</th>
		<th width="20%" rowspan="2" bgcolor="#99CCFF">Item Name </th>
		...
		if(isset($group_for)) 		{$group_for_con=' and m.group_for="'.$group_for.'"';}
		 ...
		 where c.dealer_code=d.dealer_code  and z.ZONE_CODE=d.zone_code and m.do_no=c.do_no and
		  c.chalan_date between '".$f_date."' and '".$t_date."' ".$con.$group_for_con.$dealer_con.$do_type_con." group by c.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		...
		if($_POST['do_type']=='REGULAR' || $_POST['do_type']=='') {
		 ...
		 where c.dealer_code=d.dealer_code  and z.ZONE_CODE=d.zone_code and m.do_no=c.do_no and i.item_id=c.item_id and i.sub_group_id=s.sub_group_id and g.product_type='Finish Goods' and s.group_id=g.group_id and c.chalan_date between '".$f_date."' and '".$t_date."' ".$con.$group_for_con.$dealer_con.$do_type_con." group by i.sub_group_id";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		...
		i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and g.product_type='Finish Goods' and g.group_id!=900000000
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_id_con." group by i.sub_group_id order by s.sub_group_sl ";
		$data_sub=db_query($sql_sub);
		
		}elseif ($_POST['report_type']==1) {
		
		$sql_sub="select i.sub_group_id, s.sub_group_name from item_info i, item_sub_group s, item_group g where  
		i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and g.product_type='Finish Goods' and g.group_id!=900000000 and g.group_for!=5
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_id_con." group by i.sub_group_id order by s.sub_group_sl ";
		$data_sub=db_query($sql_sub);
		...
		if($group_sales_amt[$info_sub->sub_group_id]>0 || $group_return_amt[$info_sub->sub_group_id]>0){
		
		?>
	
		  <tr>
		  <td bgcolor="#CCCCCC">&nbsp;</td>
		  <td bgcolor="#CCCCCC"><div align="left" style="text-transform:uppercase; font-size:14px; font-weight:700;"><strong><?=$info_sub->sub_group_name?>  <?php /*?><?=$group_sales_qty[$info_sub->sub_group_id]?><?php */?>
		...
		<?
		 ...
		 $sql="select distinct i.item_id, i.item_name, s.sub_group_name from item_info i, item_sub_group s, item_group g where i.sub_group_id=s.sub_group_id and g.product_type='Finish Goods' and s.group_id=g.group_id and i.sub_group_id='".$info_sub->sub_group_id."' and g.group_id!=900000000
		...
		if($sales_amt[$row->item_id]>0 || $return_amt[$row->item_id]>0 ){
		?>
		...
		<td><?=number_format($return_qty[$row->item_id],2); $total_return_qty +=$return_qty[$row->item_id];?> </td>
		<td>
...
elseif($_POST['report']==211100702)
{
  ...
  <? if ($item_mother_group>0) {?>
		...
		if($_POST['item_id_in']!='')  
		$item_con_in .= " and i.item_id in (".$_POST['item_id_in'].")";
		...
		i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and g.product_type='Finish Goods' 
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_id_con.$item_con_in." group by i.sub_group_id order by s.sub_group_sl ";
		$data_sub=db_query($sql_sub);
		
		}elseif ($_POST['report_type']==1) {
		
		$sql_sub="select i.sub_group_id, s.sub_group_name from item_info i, item_sub_group s, item_group g where  
		i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and g.product_type='Finish Goods' and i.group_for!=5
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_id_con.$item_con_in." group by i.sub_group_id order by s.sub_group_sl ";
		$data_sub=db_query($sql_sub);
		...
		<?
		 ...
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_id_con.$item_con_in." order by i.item_type, i.item_name";
...
elseif($_POST['report']==211100706)
{
		$report="PRODUCT MOVEMENT ANALYSIS";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
  ...
  <? if ($item_mother_group>0) {?>
		...
		<tr>
		<th width="2%" rowspan="2" bgcolor="#9999FF">SL</th>
		<th width="36%" rowspan="2" bgcolor="#9999FF">Item Name </th>
		...
		<tr>
		  <th width="8%" bgcolor="#99FFFF" style="text-align:center">BAG</th>
		  <th width="7%" bgcolor="#99FFFF" style="text-align:center">QUANTITY</th>
		  <th width="8%" bgcolor="#99FFFF" style="text-align:center">AMOUNT</th>
		  <th width="7%" bgcolor="#33CCCC" style="text-align:center">BAG</th>
		  <th width="7%" bgcolor="#33CCCC" style="text-align:center">QUANTITY</th>
		  <th width="7%" bgcolor="#33CCCC" style="text-align:center">AMOUNT</th>
		  <th width="5%" bgcolor="#FF0000" style="text-align:center">BAG</th>
		  <th width="6%" bgcolor="#FF0000" style="text-align:center">QUANTITY</th>
		  <th width="7%" bgcolor="#FF0000" style="text-align:center">AMOUNT</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		if(isset($group_for)) 		{$group_for_con=' and m.group_for="'.$group_for.'"';}
		  ...
		  $sql = "select c.item_id, sum(c.bag_unit) as bag_unit
		 from sale_do_master m, sale_do_chalan c, dealer_info d,  zon z
		 where c.dealer_code=d.dealer_code  and z.ZONE_CODE=d.zone_code and m.do_no=c.do_no and c.item_id NOT LIKE '900%' and 
		  c.chalan_date between '".$f_date."' and '".$t_date."' ".$con.$group_for_con.$dealer_con.$do_type_con." group by c.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $bag_unit[$info->item_id]=$info->bag_unit;
		}
		...
		if($_POST['do_type']=='REGULAR' || $_POST['do_type']=='') {
		 ...
		 $sql = "select c.item_id, sum(c.bag_unit) as return_bag_qty
		  from sale_return_master m, sale_return_details c, dealer_info d, zon z
		  where c.dealer_code=d.dealer_code and z.ZONE_CODE=d.zone_code  and m.do_no=c.do_no and c.item_id NOT LIKE '900%' and 
		  c.do_date between '".$f_date."' and '".$t_date."'  ".$con.$group_for_con.$dealer_con." group by c.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $return_bag_qty[$info->item_id]=$info->return_bag_qty;
		}
		...
		<td><?=$sl++;?></td>
		<td><?=$row->item_name?></td>
		<td><?=$bag_unit[$row->item_id]; $total_bag_unit +=$bag_unit[$row->item_id]; ?></td>
		<td> <?php /*?><a href="master_report.php?f_date=<?=$f_date;?>&t_date=<?=$t_date;?>&item_id=<?=$row->item_id;?>&dealer_code=<?=$dealer[0];?>&report=211100707&submit=1" target="_blank" > </a><?php */?>	<?=number_format($sales_qty[$row->item_id],2); $total_sales_qty +=$sales_qty[$row->item_id]; ?> </td>
		<td>
		  <?=number_format($sales_amt[$row->item_id],2); $total_sales_amt +=$sales_amt[$row->item_id]; ?>          </td>
		<td><?=$return_bag_qty[$row->item_id]; $total_return_bag_qty +=$return_bag_qty[$row->item_id]; ?></td>
		<td><?=number_format($return_qty[$row->item_id],2); $total_return_qty +=$return_qty[$row->item_id];?> </td>
		<td>
		  <?=number_format($return_amt[$row->item_id],2); $total_return_amt +=$return_amt[$row->item_id];?>        </td>
		<td><?=$net_bag_unit=($bag_unit[$row->item_id]-$return_bag_qty[$row->item_id]); $total_net_bag_unit +=$net_bag_unit;?></td>
		<td>
		<?=number_format($net_sales_qty=($sales_qty[$row->item_id]-$return_qty[$row->item_id]),2); $total_net_sales_qty +=$net_sales_qty;?></td>
		<td>
		  <?=number_format($net_sales_amt=($sales_amt[$row->item_id]-$return_amt[$row->item_id]),2); $total_net_sales_amt +=$net_sales_amt;?>		  </td>
		</tr>
<? } } ?>
		<tr>
		  <td>&nbsp;</td>
		  <td><div align="right" style="text-transform:uppercase"><strong>Sub Total:</strong></div></td>
		  <td><?=$total_bag_unit; $grand_total_bag_unit +=$total_bag_unit;?></td>
		  <td><span class="style7">
		    <?=number_format($total_sales_qty,2); $total_total_sales_qty +=$total_sales_qty;?>
		  </span></td>
		  <td><span class="style7">
		  ...
		  <td><?=$total_return_bag_qty; $grand_total_return_bag_qty +=$total_return_bag_qty;?></td>
		  <td><span class="style7">
		    <?=number_format($total_return_qty,2); $total_total_return_qty +=$total_return_qty;?>
		  </span></td>
		  <td><span class="style7">
		  ...
		  <td><span class="style7">
            <?=$total_net_bag_unit; $total_total_net_bag_unit +=$total_net_bag_unit;?>
          </span></td>
		  <td><span class="style7">
		    <?=number_format($total_net_sales_qty,2); $total_total_net_sales_qty +=$total_net_sales_qty;?>
		  </span></td>
		  ...
		  $total_sales_qty = 0;
		     $total_sales_amt = 0;
			 $total_return_bag_qty = 0;
			 $total_return_qty = 0; 
			 $total_return_amt = 0; 
			 $total_net_bag_unit = 0;
			 $total_net_sales_qty = 0;
			 $total_net_sales_amt = 0; } } ?>
 ...
 <tr>
		  ...
		  <td><?=$grand_total_return_bag_qty;?></td>
		  <td><span class="style7">
		    <?=number_format($total_total_return_qty,2);?>
		  </span></td>
		  <td><span class="style7">
...
elseif($_POST['report']==220307005)
{
		$report="FOREIGN CURRENCY BALANCE";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
        ...
        <tr>
		 ...
		 <tr>
          <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;"><strong>For the date of: <?php echo date('d-m-Y',strtotime($_POST['t_date']));?></strong></td>
         </tr>
			</table>
		...
		<table width="100%" border="0" cellpadding="2" id="ExportTable" cellspacing="0" style="font-size:14px; text-transform:uppercase;">
		
		<thead>
	
		<tr>
		<th width="6%" bgcolor="#33CCCC">SL</th>
		<th width="12%" bgcolor="#33CCCC">Ledger Code </th>
		<th width="43%" bgcolor="#33CCCC">Ledger Name </th>
		<th width="16%" bgcolor="#33CCCC" style="text-align:center">Balance Amt </th>
		<th width="23%" bgcolor="#33CCCC" style="text-align:center">Balance Amt in BDT </th>
		 </tr>
		</thead><tbody>
		<? $sl=1;
		...
		if($_POST['ledger_id']!='')
		$ledger_id_con .= ' and ledger_id = "'.$_POST['ledger_id'].'" ';
		...
		if(isset($group_for)) 		{$group_for_con=' and m.group_for="'.$group_for.'"';}
		 ...
		 $sql = "select ledger_id, sum(purchase_amt) as purchase_amt, sum(total_amt_bdt) as total_amt_bdt from foreign_currency_purchase
		 where invoice_date <='".$t_date."' ".$con.$group_for_con.$dealer_con.$do_type_con." group by ledger_id";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $purchase_amt[$info->ledger_id]=$info->purchase_amt;
		 $total_amt_bdt[$info->ledger_id]=$info->total_amt_bdt;
		}
		
		
		
		$sql = "select ledger_id, sum(pay_amt) as pay_amt, sum(pay_amt_bdt) as pay_amt_bdt from foreign_currency_payment
		 where invoice_date <='".$t_date."' ".$con.$group_for_con.$dealer_con.$do_type_con." group by ledger_id";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $pay_amt[$info->ledger_id]=$info->pay_amt;
		 $pay_amt_bdt[$info->ledger_id]=$info->pay_amt_bdt;
		}
		...
		<?
		 ...
		 $sql="select ledger_id, ledger_name from accounts_ledger where ledger_group_id=1010 ".$ledger_id_con." order by ledger_name";
		...
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
		<td><?=$sl++;?></td>
		<td><a href="transaction_statement.php?show=show&fdate=<?=$_REQUEST['f_date']?>&tdate=<?=$_REQUEST['t_date']?>&ledger_id=<?=$row->ledger_id?>" target="_blank" style=" color:#000000; ">
		  <?=$row->ledger_id?>
		</a></td>
		<td><a href="cash_master_report.php?&t_date=<?=$t_date;?>&ledger_id=<?=$row->ledger_id;?>&report=211100707&submit=1" target="_blank" 
		style="color:#000000;" > <?=$row->ledger_name?></a> 
		<? $purchase_amt[$row->ledger_id]; ?> <? $total_amt_bdt[$row->ledger_id]; $total_total_amt_bdt +=$total_amt_bdt[$row->ledger_id];?> 
		<? $pay_amt[$row->ledger_id]; ?> <? $pay_amt_bdt[$row->ledger_id]; $total_pay_amt_bdt +=$pay_amt_bdt[$row->ledger_id];?> </td>
		<td><div align="right">
		  <?=number_format($balance_amt = $purchase_amt[$row->ledger_id]-$pay_amt[$row->ledger_id],2);?>
		  </div></td>
		<td><div align="right">
		  <?=number_format($balance_amt_bdt = $total_amt_bdt[$row->ledger_id]-$pay_amt_bdt[$row->ledger_id],2); $total_balance_amt_bdt +=$balance_amt_bdt;?>
		  </div></td>
		</tr>
<? } //} ?>
		<tr>
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
		  <td><div align="right" style="text-transform:uppercase"><strong> Total:</strong></div></td>
		  <td><div align="right"></div></td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($total_balance_amt_bdt,2);?>
	      </span></div></td>
		</tr>
...
elseif($_REQUEST['report']==211100707)
{
        ...
        <tr>
		 ...
		 <tr>
          <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;"><strong>For the date of: <?php echo date('d-m-Y',strtotime($_REQUEST['t_date']));?></strong></td>
         </tr>
			</table>
		...
		<tr height="30" style="text-transform:uppercase;">
		<th width="1%" bgcolor="#33CCCC">SL</th>
		<th width="8%" bgcolor="#33CCCC">Ledger Name </th>
		<th width="10%" bgcolor="#33CCCC">INVOICE No</th>
		<th width="10%" bgcolor="#33CCCC"><span style="text-align:center">INVOICE Date </span></th>
		<th width="8%" bgcolor="#33CCCC" style="text-align:center">Balance Amt </th>
		<th width="10%" bgcolor="#33CCCC" style="text-align:center">Exchange Rate</th>
		<th width="15%" bgcolor="#33CCCC" style="text-align:center">Balance Amt BDT </th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		$sql = "select order_no, sum(pay_amt) as pay_amt, sum(pay_amt_bdt) as pay_amt_bdt from foreign_currency_payment where invoice_date <='".$t_date."' group by order_no ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $pay_amt[$info->order_no]=$info->pay_amt;
		 $pay_amt_bdt[$info->order_no]=$info->pay_amt_bdt;
		}
		...
		<?
 		 ...
 		 $sql="select p.id, p.invoice_no, p.invoice_date, p.ledger_id,  a.ledger_name, p.purchase_amt, p.exchange_rate, p.total_amt_bdt from foreign_currency_purchase p, accounts_ledger a
		 where p.ledger_id=a.ledger_id and p.ledger_id='".$_REQUEST['ledger_id']."' and 
		 p.invoice_date<='".$_REQUEST['t_date']."' order by p.invoice_date, p.id";
		...
		if($balance_amt = $row->purchase_amt-$pay_amt[$row->id]>0){
		?>
		...
		<td><div align="right">
		  <?=number_format($balance_amt = $row->purchase_amt-$pay_amt[$row->id],2); $total_balance_amt +=$balance_amt; ?>
		  </div></td>
		<td><div align="right">
		  <?=number_format($row->exchange_rate,2);?>
		  </div></td>
		<td><div align="right">
		  <?=number_format($balance_amt_bdt = $row->total_amt_bdt-$pay_amt_bdt[$row->id],2); $total_balance_amt_bdt +=$balance_amt_bdt; ?>
		  </div></td>
		</tr>
<?  } }?>
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
		  ...
		  <td>&nbsp;</td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($total_balance_amt,2);?>
	      </span></div></td>
		  <td><div align="right"></div></td>
		  <td>
...
elseif($_POST['report']==220307006)
{
		$report="HAND CASH BALANCE";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
		...
		<?php /*?> <? if ($_POST['ledger_group']>0) {?>
        <tr>
          <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;"><strong>Ledger Group: <?=find_a_field('ledger_group','group_name','group_id='.$_POST['ledger_group'])?></strong></td>
         </tr>
		 <? }?><?php */?>
...
echo $count_days = round($datediff / (60 * 60 * 24));
		...
		<th width="4%" bgcolor="#99CCFF">LEDGER ID </th>
		<th width="20%" bgcolor="#99CCFF">LEDGER NAME </th>
		<th width="14%" bgcolor="#99CCFF" style="text-align:center">OPENING</th>
		<th width="14%" bgcolor="#99CCFF" style="text-align:center">DEBIT</th>
		<th width="14%" bgcolor="#99CCFF" style="text-align:center">CREDIT</th>
		...
		$ledger_id_con .= ' and ledger_id = "'.$_POST['ledger_id'].'" ';
 		...
 		$sql="select  ledger_id, ledger_name from accounts_ledger where  ledger_group_id=1013 ".$ledger_id_con." order by ledger_id";
		...
		//$percentage_if = ($closing_amt[$row->ledger_id]*100)/$dr_amt[$row->ledger_id];
		
		
		if($opening_balance=$opening_amt[$row->ledger_id]<>0 || $dr_amt[$row->ledger_id]<>0 || $cr_amt[$row->ledger_id]<>0 || $closing_amt[$row->ledger_id]<>0){
		
		
		?>
		
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?> <?php /*?><? if ($percentage_if<5.1){ echo 'bgcolor="#64E986"';}elseif($percentage_if>15) {echo 'bgcolor="#E0B0FF"';}?> <?php */?>style="font-size:14px;">
		<td><?=$sl++;?></td>
		<td>
		
		<a href="transaction_statement.php?show=show&fdate=<?=$_REQUEST['f_date']?>&tdate=<?=$_REQUEST['t_date']?>&ledger_id=<?=$row->ledger_id?>" target="_blank" style=" color:#000000; ">
		  <?=$row->ledger_id?>
		</a></td>
		<td><a href="cash_master_report.php?&t_date=<?=$t_date;?>&ledger_id=<?=$row->ledger_id;?>&report=211100708&submit=1" target="_blank" 
		style="color:#000000;" > <?=$row->ledger_name?></a> </td>
		<td><div align="right">
		  <? number_format($opening_balance=$opening_amt[$row->ledger_id],2);  $sub_total_opening_amt +=$opening_amt[$row->ledger_id]; 
		if ($opening_amt[$row->ledger_id]>0) { echo  number_format($opening_balance,2). " (DR)"; } else { echo number_format($opening_balance*(-1),2). " (CR)"; }  ?>
		  </div></td>
		<td><div align="right">
		...
		<td><div align="right">
		  <? number_format($closing_balance=$closing_amt[$row->ledger_id],2); $sub_total_closing_amt +=$closing_amt[$row->ledger_id]; 
		if ($closing_amt[$row->ledger_id]>0) { echo  number_format($closing_balance,2). " (DR)"; } else { echo number_format($closing_balance*(-1),2). " (CR)"; } 
		 ?>
		  </div></td>
		</tr>
<? } } ?>
...
elseif($_REQUEST['report']==211100708)
{
		...
		<tr height="30" style="text-transform:uppercase;">
		<th width="1%" bgcolor="#33CCCC">SL</th>
		<th width="23%" bgcolor="#33CCCC">Ledger Name </th>
		<th width="14%" bgcolor="#33CCCC">Reference</th>
		<th width="14%" bgcolor="#33CCCC">Narration</th>
		<th width="9%" bgcolor="#33CCCC">Payment No</th>
		<th width="10%" bgcolor="#33CCCC"><span style="text-align:center">Payment Date </span></th>
		<th width="9%" bgcolor="#33CCCC" style="text-align:center">Payment Amt </th>
		<th width="9%" bgcolor="#33CCCC" style="text-align:center">Adjusted Amt </th>
		<th width="11%" bgcolor="#33CCCC" style="text-align:center">Balance Amt  </th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		if(isset($item_type)) 					{$item_type_con=' and i.item_type='.$item_type;}
		 ...
		 $sql = "select order_no, sum(receive_amt) as receive_amt from handcash_adjustment where receive_date <='".$t_date."' group by order_no ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $receive_amt[$info->order_no]=$info->receive_amt;
		...
		<?
 		  ...
 		  $sql="select p.id, p.payment_no, p.payment_date, p.ledger_id,  a.ledger_name, p.reference_by, p.note, p.payment_amt from handcash_payment p, accounts_ledger a
		 where p.ledger_id=a.ledger_id and p.ledger_id='".$_REQUEST['ledger_id']."' and 
		 p.payment_date<='".$_REQUEST['t_date']."' order by p.payment_date, p.id";
		...
		if($balance_amt = $row->payment_amt-$receive_amt[$row->id]>0){
		?>
		...
		<td><div align="right">
		  <?=number_format($row->payment_amt,2); $total_payment_amt +=$row->payment_amt; ?>
		  </div></td>
		<td><div align="right">
		  <?=number_format($receive_amt[$row->id],2); $tot_receive_amt+=$receive_amt[$row->id];?>
		  </div></td>
		<td><div align="right">
		 <?=number_format($balance_amt = $row->payment_amt-$receive_amt[$row->id],2); $total_balance_amt +=$balance_amt; ?>
		  </div></td>
		</tr>
<?  } }?>
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
		  ...
		  <td><div align="right">
		  <span class="style7">
		    <?=number_format($tot_receive_amt,2);?>
	      </span>
		  </div></td>
		  <td>
		    <div align="right"><span class="style7">
...
elseif($_POST['report']==223101001)
{
 ...
 <?php /*?> <? if ($item_mother_group>0) {?>
		 ...
		 <? }?><?php */?>
		  ...
		  <? if ($item_sub_group>0) {?>
		 <tr>
          <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;"><strong>Product Group: <?=find_a_field('item_sub_group','sub_group_name','sub_group_id='.$item_sub_group)?></strong></td>
         </tr>
		 <? }?>
		...
		<table width="100%" border="0" cellpadding="2"  id="ExportTable" cellspacing="0" style="font-size:12px;">
		
		<thead>
	
		<tr>
		<th width="3%" rowspan="2" bgcolor="#82D8CF">SL</th>
		<th width="24%" rowspan="2" bgcolor="#82D8CF">PRODUCT GROUP </th>
		<th width="38%" rowspan="2" bgcolor="#82D8CF">PRODUCT NAME </th>
		<th colspan="2" bgcolor="#82D8CF" style="text-align:center">PRODUCT PRICE </th>
		</tr>
		<tr>
		  <th width="17%" bgcolor="#82D8CF" style="text-align:center">REGULER</th>
		  <th width="18%" bgcolor="#82D8CF" style="text-align:center">ADVANCE</th>
		  </tr>
		</thead><tbody>
		<? $sl=1;
		...
		if(isset($item_id)) 					{$item_id_con=' and i.item_id='.$item_id;}
	 	 ...
	 	 $sql_sub="select i.sub_group_id, s.sub_group_name from item_info i, item_sub_group s, item_group g where  
		i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and g.product_type='Finish Goods' and g.group_id!=900000000 and s.item_type=1
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_id_con." group by i.sub_group_id order by s.sub_group_sl ";
		$data_sub=db_query($sql_sub);
		...
		?>
		  ...
		  <tr>
		  <td bgcolor="#E1F4FA">&nbsp;</td>
		  <td bgcolor="#E1F4FA"><div align="left" style="text-transform:uppercase; font-size:14px; font-weight:700;"><strong>
		    <?=$info_sub->sub_group_name?>
          </strong></div></td>
		  <td bgcolor="#E1F4FA">&nbsp;</td>
		  <td bgcolor="#E1F4FA">&nbsp;</td>
		  <td bgcolor="#E1F4FA">&nbsp;</td>
		...
		<?
		  ...
		  $sql="select distinct i.item_id, i.item_name, i.d_price ,i.a_price , s.sub_group_name from item_info i, item_sub_group s, item_group g where i.sub_group_id=s.sub_group_id and g.product_type='Finish Goods' and s.group_id=g.group_id and i.sub_group_id='".$info_sub->sub_group_id."' and g.group_id!=900000000 and i.item_type=1
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_id_con." order by i.item_type, i.item_name";
		...
		<tr <?=($xx%2==0)?' bgcolor="#f0f0f0"':'';$xx++;?>>
		<td><?=$sl++;?></td>
		<td><?=$row->sub_group_name?></td>
		<td><?=$row->item_name?></td>
		<td>
		<?=number_format($row->d_price,2); ?> </td>
		<td>
		  <?=number_format($row->a_price,2); ?>      </td>
		</tr>
<? }  ?>
...
elseif($_POST['report']==210616002)
{
  ...
  <? if ($item_mother_group>0) {?>
	...
	<thead>
		...
		<th width="20%" rowspan="2" bgcolor="#33CCCC">Item Name </th>
		<th colspan="2" bgcolor="#33CCCC" style="text-align:center">REGULAR SALES  </th>
		<th colspan="2" bgcolor="#33CCCC" style="text-align:center">ADVANCE SALES</th>
		<th colspan="2" bgcolor="#33CCCC" style="text-align:center"> SALES RETURN </th>
		<th colspan="2" bgcolor="#33CCCC" style="text-align:center">NET SALES</th>
		</tr>
		<tr>
		  <th width="9%" bgcolor="#33CCCC" style="text-align:center">QUANTITY</th>
		  <th width="10%" bgcolor="#33CCCC" style="text-align:center">AMOUNT</th>
		  <th width="10%" bgcolor="#33CCCC" style="text-align:center">QUANTITY</th>
		...
		if(isset($group_for)) 		{$group_for_con=' and m.group_for="'.$group_for.'"';}
		 ...
		 $sql = "select c.item_id, sum(c.total_unit) as return_qty, sum(c.return_amt) as return_amt
		 from sale_return_master m, sale_return_details c, dealer_info d,  zon z
		 where m.dealer_code=d.dealer_code  and z.ZONE_CODE=d.zone_code and m.do_no=c.do_no and
		  m.do_date between '".$f_date."' and '".$t_date."' 
		  ".$con.$group_for_con.$dealer_con." group by c.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $return_qty[$info->item_id]=$info->return_qty;
		...
		if($regular_sales_amt[$row->item_id]+$advance_sales_amt[$row->item_id]>0  || $return_amt[$row->item_id]>0 ){
		?>
		...
		<td>
		  <?=number_format($regular_sales_amt[$row->item_id],2); $total_regular_sales_amt +=$regular_sales_amt[$row->item_id]; ?>         </td>
		<td>
		<?=number_format($advance_sales_qty[$row->item_id],2); $total_advance_sales_qty +=$advance_sales_qty[$row->item_id];?> </td>
		<td>
		  <?=number_format($advance_sales_amt[$row->item_id],2); $total_advance_sales_amt +=$advance_sales_amt[$row->item_id];?>          </td>
		<td><?=$return_qty[$row->item_id]; $tot_return_qty +=$return_qty[$row->item_id];?></td>
		<td><?=$return_amt[$row->item_id]; $tot_return_amt +=$return_amt[$row->item_id];?></td>
		<td><?=number_format($total_sales_qty = ($regular_sales_qty[$row->item_id]+$advance_sales_qty[$row->item_id])-$return_qty[$row->item_id],2); $sub_total_sales_qty +=$total_sales_qty; ?>		</td>
		<td><?=number_format($total_sales_amt = ($regular_sales_amt[$row->item_id]+$advance_sales_amt[$row->item_id])-$return_amt[$row->item_id],2); $sub_total_sales_amt +=$total_sales_amt; ?>	</td>
		</tr>
<? } } ?>
		<tr>
		  <td>&nbsp;</td>
		  ...
		  <td><span class="style7">
            <?=number_format($tot_return_qty,2); $total_tot_return_qty +=$tot_return_qty;?>
          </span></td>
		  <td><span class="style7">
            <?=number_format($tot_return_amt,2); $total_tot_return_amt +=$tot_return_amt;?>
          </span></td>
		  <td><span class="style7">
 ...
 <tr>
		  ...
		  <td><span class="style7">
		    <?=number_format($total_tot_return_qty,2);?>
		  </span></td>
		  <td><span class="style7">
		    <?=number_format($total_tot_return_amt,2);?>
		  </span></td>
		  <td><span class="style7">
...
elseif($_POST['report']==220411001)
{
		$report="SPARE PARTS STOCK MOVEMENT REPORT";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
  ...
  <? if ($item_mother_group>0) {?>
	...
	<thead>
		<tr>
		<th width="1%" rowspan="2" bgcolor="#33CCCC">SL</th>
		<th width="4%" rowspan="2" bgcolor="#33CCCC">Item Code </th>
		<th width="21%" rowspan="2" bgcolor="#33CCCC">Item Name </th>
		<th width="5%" rowspan="2" bgcolor="#33CCCC">Unit </th>
		<th colspan="3" bgcolor="#33CCCC" style="text-align:center">OPENING STOCK </th>
		<th colspan="3" bgcolor="#33CCCC" style="text-align:center">PURCHASED</th>
		<th colspan="3" bgcolor="#33CCCC" style="text-align:center"> SALES </th>
		<th colspan="3" bgcolor="#33CCCC" style="text-align:center">CLOSING STOCK</th>
		</tr>
		<tr>
		  <th width="5%" bgcolor="#33CCCC" style="text-align:center">QUANTITY</th>
		  <th width="5%" bgcolor="#33CCCC" style="text-align:center">RATE</th>
		  <th width="6%" bgcolor="#33CCCC" style="text-align:center">AMOUNT</th>
		  <th width="7%" bgcolor="#33CCCC" style="text-align:center">QUANTITY</th>
		  <th width="4%" bgcolor="#33CCCC" style="text-align:center">RATE</th>
		  <th width="6%" bgcolor="#33CCCC" style="text-align:center">AMOUNT</th>
		  <th width="6%" bgcolor="#33CCCC" style="text-align:center">QUANTITY</th>
		  <th width="5%" bgcolor="#33CCCC" style="text-align:center">RATE</th>
		  <th width="7%" bgcolor="#33CCCC" style="text-align:center">AMOUNT</th>
		  <th width="6%" bgcolor="#33CCCC" style="text-align:center">QUANTITY</th>
		  <th width="4%" bgcolor="#33CCCC" style="text-align:center">RATE</th>
		...
		if(isset($group_for)) 		{$group_for_con=' and m.group_for="'.$group_for.'"';}
		 ...
		 $sql = "select b.item_id, sum(b.qty) as po_qty, sum(b.amount) as po_value 
		 from purchase_sp_master a, purchase_sp_invoice b
		 where a.po_no=b.po_no and a.po_date<'".$f_date."' and a.status!='MANUAL' group by b.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $po_qty[$info->item_id]=$info->po_qty;
		 $po_value[$info->item_id]=$info->po_value;	 
		}
		
		
		$sql = "select b.item_id, sum(b.qty) as so_qty, sum(b.amount) as so_value  
		 from  spare_parts_requisition_master a, spare_parts_requisition_order b 
		 where a.req_no=b.req_no and a.req_date<'".$f_date."' group by b.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $so_qty[$info->item_id]=$info->so_qty;
		 $so_value[$info->item_id]=$info->so_value;	 
		}
		
			
		 $sql = "select b.item_id, sum(b.qty) as pur_qty, sum(b.amount) as pur_value 
		 from purchase_sp_master a, purchase_sp_invoice b
		 where a.po_no=b.po_no and a.po_date between '".$f_date."' and '".$t_date."' and a.status!='MANUAL' group by b.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $pur_qty[$info->item_id]=$info->pur_qty;
		 $pur_value[$info->item_id]=$info->pur_value;	 
		}
		
		
		$sql = "select b.item_id, sum(b.qty) as sales_qty, sum(b.amount) as sales_value  
		 from  spare_parts_requisition_master a, spare_parts_requisition_order b 
		 where a.req_no=b.req_no and  a.req_date between '".$f_date."' and '".$t_date."'  group by b.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $sales_qty[$info->item_id]=$info->sales_qty;
		 $sales_value[$info->item_id]=$info->sales_value;	 
		}


		 $sql = "select b.item_id, sum(b.qty) as cl_po_qty, sum(b.amount) as cl_po_value 
		 from purchase_sp_master a, purchase_sp_invoice b
		 where a.po_no=b.po_no and a.po_date<'".$t_date."' and a.status!='MANUAL' group by b.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $cl_po_qty[$info->item_id]=$info->cl_po_qty;
		 $cl_po_value[$info->item_id]=$info->cl_po_value;	 
		}
		
		
		$sql = "select b.item_id, sum(b.qty) as cl_so_qty, sum(b.amount) as cl_so_value  
		 from  spare_parts_requisition_master a, spare_parts_requisition_order b 
		 where a.req_no=b.req_no and a.req_date<'".$t_date."' group by b.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $cl_so_qty[$info->item_id]=$info->cl_so_qty;
		 $cl_so_value[$info->item_id]=$info->cl_so_value;	 
		}
...
//		}
		...
		if(isset($item_id)) 					{$item_id_con=' and i.item_id='.$item_id;}
	 	 ...
	 	 $sql_sub="select i.sub_group_id, s.sub_group_name from item_info i, item_sub_group s, item_group g where  
		i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and  g.group_id=1000000000
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_id_con." group by i.sub_group_id order by s.sub_group_name ";
		$data_sub=db_query($sql_sub);
		...
		<?
		  ...
		  $sql="select distinct i.item_id, i.unit_name, i.item_name, s.sub_group_name from item_info i, item_sub_group s, item_group g where i.sub_group_id=s.sub_group_id  and s.group_id=g.group_id and i.sub_group_id='".$info_sub->sub_group_id."' 
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_id_con." order by i.item_name";
		
		$res	 = db_query($sql);
		while($row=mysqli_fetch_object($res))
		{

		if($opening_stock = ($po_qty[$row->item_id]-$so_qty[$row->item_id])<>0  || $pur_qty[$row->item_id]<>0 || $sales_qty[$row->item_id]<>0 ||
		 $closing_stock = ($cl_po_qty[$row->item_id]-$cl_so_qty[$row->item_id])<>0 ){
		
		
		//if($closing_stock = ($cl_po_qty[$row->item_id]-$cl_so_qty[$row->item_id])<0) {}
		?>
		...
		<td>
		  <div align="right">
		    <?=number_format($opening_stock = ($po_qty[$row->item_id]-$so_qty[$row->item_id]),2);?>
	      </div></td>
		<td><div align="right">
		  <?= number_format($avg_price = ($po_value[$row->item_id]/$po_qty[$row->item_id]),3);?>
		  </div></td>
		<td>
		  <div align="right">
		    <?= number_format($opening_value = ($opening_stock*$avg_price),2); $sub_total_op_value +=$opening_value; ?>          
	      </div></td>
		<td>
		  <div align="right">
		    <?= number_format($pur_qty[$row->item_id],2);?> 
	      </div></td>
		<td><div align="right">
		  <?=number_format($pur_price = ($pur_value[$row->item_id]/$pur_qty[$row->item_id]),3); ?>
		  </div></td>
		<td>
		  <div align="right">
		    <?=number_format($pur_value[$row->item_id],2); $sub_total_pur_value +=$pur_value[$row->item_id];?>          
	      </div></td>
		<td><div align="right">
		  <?=number_format($sales_qty[$row->item_id],2);?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($sales_price = ($sales_value[$row->item_id]/$sales_qty[$row->item_id]),3); ?>
		  </div></td>
		<td><div align="right">
		  <?=number_format($sales_value[$row->item_id],2); $sub_total_sales_value +=$sales_value[$row->item_id];?>
		  </div></td>
		<td><div align="right">
		  <?=number_format($closing_stock = ($cl_po_qty[$row->item_id]-$cl_so_qty[$row->item_id]),2);?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($cl_avg_price = ($cl_po_value[$row->item_id]/$cl_po_qty[$row->item_id]),3);?>
		  </div></td>
		<td><div align="right">
		  <?= number_format($closing_value = ($closing_stock*$cl_avg_price),2);  $sub_total_closing_value +=$closing_value; ?>        
		  </div></td>
		</tr>
<? }  } ?>
		<tr>
		  ...
		  <td><div align="right"></div></td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_op_value,2); $grand_total_op_value +=$sub_total_op_value;?>
	      </span></div></td>
		  <td><div align="right"></div></td>
		  <td><div align="right"></div></td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_pur_value,2); $grand_total_pur_value +=$sub_total_pur_value;?>
	      </span></div></td>
		  <td><div align="right"></div></td>
		  <td><div align="right"></div></td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_sales_value,2); $grand_total_sales_value +=$sub_total_sales_value;?>
	      </span></div></td>
		  <td><div align="right"></div></td>
		  <td><div align="right"></div></td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_closing_value,2); $grand_total_closing_value +=$sub_total_closing_value;?>
	      </span></div></td>
		</tr>
		  
		  <? 
		  
		  	 $sub_total_op_value = 0;
		     $sub_total_pur_value = 0;
		     $sub_total_sales_value = 0;
		     $sub_total_closing_value = 0;
			 } // } ?>
 ...
 <tr>
		  ...
		  <td><div align="right"></div></td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($grand_total_op_value,2);?>
	      </span></div></td>
		  <td><div align="right"></div></td>
		  <td><div align="right"></div></td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($grand_total_pur_value,2);?>
	      </span></div></td>
		  <td><div align="right"></div></td>
		  <td><div align="right"></div></td>
		  <td><div align="right"><span class="style7">
		  ...
		  <td><div align="right"></div></td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($grand_total_closing_value,2);?>
	      </span></div></td>
 </tr>
		</tbody>
</table>
...
elseif($_POST['report']==220411002)
{
		$report="SPARE PARTS PURCHASE SUMMARY";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
		
		
 		if($_POST['purchase_manager']!='')
 		$purchase_manager_con=" and a.purchase_manager=".$_POST['purchase_manager'];
  ...
  <? if ($item_mother_group>0) {?>
		 ...
		 <? if ($_POST['purchase_manager']>0) {?>
		 <tr>
          <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;"><strong>Purchase Manager: <?=find_a_field('purchase_manager','purchase_manager','id='.$_POST['purchase_manager'])?></strong></td>
         </tr>
		 <? }?>
	...
	<thead>
		...
		if(isset($group_for)) 		{$group_for_con=' and m.group_for="'.$group_for.'"';}
		 ...
		 $sql = "select b.item_id, sum(b.qty) as pur_qty, sum(b.amount) as pur_value 
		 from purchase_sp_master a, purchase_sp_invoice b
		 where a.po_no=b.po_no and a.po_date between '".$f_date."' and '".$t_date."' and a.status!='MANUAL' ".$purchase_manager_con." group by b.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $pur_qty[$info->item_id]=$info->pur_qty;
		 $pur_value[$info->item_id]=$info->pur_value;	 
		...
		$sql = "select b.item_id, sum(b.qty) as sales_qty, sum(b.amount) as sales_value  
		 ...
		 $sql = "select i.sub_group_id, b.item_id, sum(b.qty) as grp_pur_qty, sum(b.amount) as grp_pur_value 
		 from purchase_sp_master a, purchase_sp_invoice b, item_info i
		 where a.po_no=b.po_no and b.item_id=i.item_id and a.po_date between '".$f_date."' and '".$t_date."' and a.status!='MANUAL'  ".$purchase_manager_con." group by i.sub_group_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $grp_pur_qty[$info->sub_group_id]=$info->grp_pur_qty;
		 $grp_pur_value[$info->sub_group_id]=$info->grp_pur_value;	 
		}
...
//		}
		...
		if($grp_pur_qty[$info_sub->sub_group_id]<>0 ){
...
elseif($_POST['report']==220411003)
{
		$report="SPARE PARTS PURCHASE DETAILS";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
  ...
  <? if ($item_mother_group>0) {?>
	...
	<thead>
		...
		<th width="7%" rowspan="2" bgcolor="#33CCCC">PO No </th>
		<th width="7%" rowspan="2" bgcolor="#33CCCC">Date</th>
		<th width="7%" rowspan="2" bgcolor="#33CCCC">Memo</th>
		<th width="7%" rowspan="2" bgcolor="#33CCCC">Vendor</th>
		<th width="7%" rowspan="2" bgcolor="#33CCCC">Category</th>
		<th width="7%" rowspan="2" bgcolor="#33CCCC">Item Code </th>
		<th width="20%" rowspan="2" bgcolor="#33CCCC">Item Name </th>
		<th width="8%" rowspan="2" bgcolor="#33CCCC">Unit </th>
		<th colspan="3" bgcolor="#33CCCC" style="text-align:center">PURCHASED</th>
		</tr>
		<tr>
		  <th width="10%" bgcolor="#33CCCC" style="text-align:center">QUANTITY</th>
		  <th width="7%" bgcolor="#33CCCC" style="text-align:center">RATE</th>
...
//		}
		...
		<?
		  ...
		  $sql="select  i.item_id, i.unit_name, i.item_name, s.sub_group_name, a.po_no, a.po_date, a.invoice_no, v.vendor_name,  b.unit_name, b.qty, b.rate, b.amount
		  from item_info i, item_sub_group s, item_group g , purchase_sp_master a, purchase_sp_invoice b, vendor v
		  where i.sub_group_id=s.sub_group_id  and s.group_id=g.group_id  and i.item_id=b.item_id and a.po_no=b.po_no and  a.po_date between '".$f_date."' and '".$t_date."' 
		  and a.status!='MANUAL' and v.vendor_id=a.vendor_id ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$purchase_manager_con.$item_id_con."
		   order by a.po_date, i.sub_group_id, i.item_name";
		...
		<td>
		  <div align="right">
		    <?=number_format($row->amount,2); $sub_total_pur_value +=$row->amount;?>          
	      </div></td>
		</tr>
<? }  ?>
		<tr>
...
elseif($_POST['report']==220411004)
{
		$report="SPARE PARTS SALES SUMMARY";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
  ...
  <? if ($item_mother_group>0) {?>
		 ...
		 <tr>
          <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;"><strong>Location: <?=find_a_field('spare_parts_sales_location','warehouse_name','warehouse_id='.$_POST['warehouse_id'])?></strong></td>
         </tr>
		 <? }?>
	...
	<thead>
		...
		<th width="21%" rowspan="2" bgcolor="#33CCCC">Item Name </th>
		<th width="5%" rowspan="2" bgcolor="#33CCCC">Unit </th>
		<th colspan="5" bgcolor="#33CCCC" style="text-align:center">SALES</th>
		</tr>
		<tr>
		  <th width="7%" bgcolor="#33CCCC" style="text-align:center">QUANTITY</th>
		  <th width="4%" bgcolor="#33CCCC" style="text-align:center">RATE</th>
		  <th width="6%" bgcolor="#33CCCC" style="text-align:center">NET AMT </th>
	      <th width="6%" bgcolor="#33CCCC" style="text-align:center">SERVICE AMT </th>
	      <th width="6%" bgcolor="#33CCCC" style="text-align:center">TOTAL AMY </th>
		</tr>
		</thead>
		<tbody>
		<? $sl=1;
		...
		if(isset($group_for)) 		{$group_for_con=' and a.group_for="'.$group_for.'"';}

		
		
		if($_POST['warehouse_id']!='')
		$location_con .= ' and a.warehouse_id = "'.$_POST['warehouse_id'].'" ';
		
		
		
		 $sql = "select b.item_id, sum(b.qty) as sales_qty, sum(b.amount) as sales_value , sum(b.service_amt) as service_amt, sum(b.total_amt) as total_amt
		 from  spare_parts_requisition_master a, spare_parts_requisition_order b
		 where a.req_no=b.req_no and  a.req_date between '".$f_date."' and '".$t_date."' ".$group_for_con.$location_con." group by b.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $sales_qty[$info->item_id]=$info->sales_qty;
		 $sales_value[$info->item_id]=$info->sales_value;	
		  $service_amt[$info->item_id]=$info->service_amt;	
		   $total_amt[$info->item_id]=$info->total_amt;	 
		}
		
		
		
		 $sql = "select i.sub_group_id, b.item_id, sum(b.qty) as grp_sales_qty, sum(b.amount) as grp_sales_value  
		 from  spare_parts_requisition_master a, spare_parts_requisition_order b, item_info i 
		 where a.req_no=b.req_no and  b.item_id=i.item_id and a.req_date between '".$f_date."' and '".$t_date."'   ".$group_for_con.$location_con." group by i.sub_group_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $grp_sales_qty[$info->sub_group_id]=$info->grp_sales_qty;
		 $grp_sales_value[$info->sub_group_id]=$info->grp_sales_value;	 
		}
...
//		}
		...
		if($grp_sales_qty[$info_sub->sub_group_id]<>0 ){
		...
		<td><div align="right">
		  <?=number_format($service_amt[$row->item_id],2); $sub_total_service_amt +=$service_amt[$row->item_id];?>        
		  </div></td>
		<td><div align="right">
		  <?=number_format($total_amt[$row->item_id],2); $sub_total_total_amt +=$total_amt[$row->item_id];?>
		  </div></td>
		</tr>
<? }  } ?>
		<tr>
		  ...
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_service_amt,2); $grand_total_service_amt +=$sub_total_service_amt;?>
	      </span></div></td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_total_amt,2); $grand_total_total_amt +=$sub_total_total_amt;?>
	      </span></div></td>
		</tr>
		  ...
		  <? 
		  	 ...
		  	 $sub_total_op_value = 0;
		     $sub_total_service_amt = 0;
		     $sub_total_sales_value = 0;
		     $sub_total_total_amt = 0;
			 } } ?>
 ...
 <tr>
		  ...
		  <td><div align="right"><span class="style7">
		    <?=number_format($grand_total_service_amt,2);?>
	      </span></div></td>
		  <td><div align="right"><span class="style7">
...
elseif($_POST['report']==220411005)
{
		$report="SPARE PARTS SALES DETAILS";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
		
		
 		if($_POST['machine_id']!='')
 		$machine_id_con=" and b.machine_id=".$_POST['machine_id'];
  ...
  <? if ($item_mother_group>0) {?>
	...
	<thead>
		<tr>
		<th width="1%" rowspan="2" bgcolor="#33CCCC">SL</th>
		<th width="1%" rowspan="2" bgcolor="#33CCCC">SO No </th>
		<th width="1%" rowspan="2" bgcolor="#33CCCC">Date</th>
		<th width="1%" rowspan="2" bgcolor="#33CCCC">Machine No </th>
		<th width="1%" rowspan="2" bgcolor="#33CCCC">Remarks</th>
		<th width="1%" rowspan="2" bgcolor="#33CCCC">Category</th>
		<th width="4%" rowspan="2" bgcolor="#33CCCC">Item Code </th>
		<th width="21%" rowspan="2" bgcolor="#33CCCC">Item Name </th>
		<th width="5%" rowspan="2" bgcolor="#33CCCC">Unit </th>
		<th colspan="5" bgcolor="#33CCCC" style="text-align:center">SALES</th>
...
//		}
		...
		<?
		   ...
		   $sql="select  i.item_id, i.unit_name, i.item_name, s.sub_group_name , a.req_no, a.req_date, b.qty, b.unit_price, b.amount, b.service_amt, b.total_amt, b.remarks, b.machine_id
		  from item_info i, item_sub_group s, item_group g, spare_parts_requisition_master a, spare_parts_requisition_order b
		  where i.sub_group_id=s.sub_group_id and s.group_id=g.group_id and  a.req_no=b.req_no and i.item_id=b.item_id and a.req_date between '".$f_date."' and '".$t_date."' 
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$group_for_con.$machine_id_con.$item_id_con.$location_con." order by a.req_date, s.sub_group_id, i.item_name";
		...
		<td><?=find_a_field('machine_info','machine_short_name','machine_id='.$row->machine_id);?></td>
		<td><?=$row->remarks?></td>
		<td><?=$row->sub_group_name?></td>
		<td><?=$row->item_id?></td>
		<td><?=$row->item_name?></td>
		...
		<td>
		  <div align="right">
		    <?=number_format($row->amount,2); $sub_total_sales_value +=$row->amount;?>          
	      </div></td>
		<td><div align="right">
		  <?=number_format($row->service_amt,2); $sub_total_service_amt +=$row->service_amt;?>        
		  </div></td>
		<td><div align="right">
		  <?=number_format($row->total_amt,2); $sub_total_total_amt +=$row->total_amt;?>        
		  </div></td>
		</tr>
<? }   ?>
		<tr>
		  ...
		  <td><div align="right"></div></td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_sales_value,2); ?>
	      </span></div></td>
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_service_amt,2);?>
	      </span></div></td>
		  <td><div align="right"><span class="style7">
...
elseif($_POST['report']==211100701)
{
  ...
  <? if ($item_mother_group>0) {?>
		...
		<td><?=$row->item_name?></td>
		<td>
		<?=number_format($regular_sales_qty[$row->item_id]-$return_qty[$row->item_id],2); 
		$total_regular_sales_qty +=$regular_sales_qty[$row->item_id]-$return_qty[$row->item_id]; ?> </td>
		<td>
		  <?=number_format($regular_sales_amt[$row->item_id]-$return_amt[$row->item_id],2); $total_regular_sales_amt +=$regular_sales_amt[$row->item_id]-$return_amt[$row->item_id]; ?>         </td>
		<td>
		<?=number_format($advance_sales_qty[$row->item_id],2); $total_advance_sales_qty +=$advance_sales_qty[$row->item_id];?> </td>
		<td>
		  <?=number_format($advance_sales_amt[$row->item_id],2); $total_advance_sales_amt +=$advance_sales_amt[$row->item_id];?>          </td>
...
elseif($_POST['report']==20060202)
{
  ...
  <? if ($item_mother_group>0) {?>
		...
		<tr>
		<th width="2%" rowspan="2" bgcolor="#99CCFF">SL</th>
		<th width="20%" rowspan="2" bgcolor="#99CCFF">Item Group Name </th>
		<th colspan="2" bgcolor="#99FFFF" style="text-align:center">SALES  </th>
		<th colspan="2" bgcolor="#33CCCC" style="text-align:center">SALES RETURN</th>
		<th colspan="2" bgcolor="#FF0000" style="text-align:center">NET SALES </th>
		</tr>
		...
		if(isset($item_type)) 					{$item_type_con=' and i.item_type='.$item_type;}
		 ...
		 where c.dealer_code=d.dealer_code  and z.ZONE_CODE=d.zone_code and m.do_no=c.do_no and i.sub_group_id=s.sub_group_id  and c.item_id=i.item_id and
		  c.chalan_date between '".$f_date."' and '".$t_date."' ".$con.$group_for_con.$dealer_con.$item_sub_con.$item_group_con.$item_type_con.$do_type_con." group by s.sub_group_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		...
		if($_POST['do_type']=='REGULAR' || $_POST['do_type']=='') {
		 ...
		 and c.item_id=i.item_id and
		  c.chalan_date between '".$f_date."' and '".$t_date."' ".$con.$group_for_con.$dealer_con.$item_sub_con.$item_group_con.$item_type_con.$do_type_con." group by s.group_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
	  ...
	  $sql_sub="select i.sub_group_id, s.sub_group_name, g.group_id, g.group_name from item_info i, item_sub_group s, item_group g  where  
		 s.group_id=g.group_id and i.sub_group_id=s.sub_group_id  and g.product_type='Finish Goods'  and g.group_id!=900000000
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con." 
		 group by g.group_id order by g.item_group_sl";
		$data_sub=db_query($sql_sub);
		 ...
		 $sql_sub="select i.sub_group_id, s.sub_group_name, g.group_id, g.group_name from item_info i, item_sub_group s, item_group g  where  
		 s.group_id=g.group_id and i.sub_group_id=s.sub_group_id  and g.product_type='Finish Goods'  and g.group_id!=900000000 and g.group_for!=5
		 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con." 
		 group by g.group_id order by g.item_group_sl";
		$data_sub=db_query($sql_sub);
		...
		if($group_sales_amt[$info_sub->group_id]>0 || $group_return_amt[$info_sub->group_id]>0){
		...
		<?
		 ...
		 $sql="select distinct i.item_id, i.item_name, s.sub_group_id, s.sub_group_name from item_info i, item_sub_group s, item_group g  where 
		  s.group_id=g.group_id and i.sub_group_id=s.sub_group_id  and s.group_id='".$info_sub->group_id."' and g.group_id!=900000000
		   ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con."
		...
		if($sales_amt[$row->sub_group_id]>0 || $return_amt[$row->sub_group_id]>0 ){
		?>
...
elseif($_POST['report']==211100705)
{
  ...
  <? if ($item_mother_group>0) {?>
		...
		<tr>
		<th width="3%" rowspan="2" bgcolor="#B0C4DE">SL</th>
		<th width="17%" rowspan="2" bgcolor="#B0C4DE">Item Group Name </th>
		<th colspan="3" bgcolor="#90EE90" style="text-align:center">REGULAR SALES </th>
		<th colspan="3" bgcolor="#3CB371" style="text-align:center">ADVANCE SALES  </th>
		<th colspan="3" bgcolor="#6495ED" style="text-align:center">TOTAL SALES </th>
		<th colspan="3" bgcolor="#98FB98" style="text-align:center">SALES RETURN</th>
		<th colspan="3" bgcolor="#8FBC8F" style="text-align:center">NET SALES </th>
		</tr>
		<tr>
		  <th width="4%" bgcolor="#90EE90" style="text-align:center">BAG</th>
		  <th width="5%" bgcolor="#90EE90" style="text-align:center">QUANTITY</th>
		  <th width="5%" bgcolor="#90EE90" style="text-align:center">AMOUNT</th>
		  <th width="4%" bgcolor="#3CB371" style="text-align:center">BAG</th>
		  <th width="5%" bgcolor="#3CB371" style="text-align:center">QUANTITY</th>
		  <th width="5%" bgcolor="#3CB371" style="text-align:center">AMOUNT</th>
		  <th width="5%" bgcolor="#6495ED" style="text-align:center">BAG</th>
		  <th width="6%" bgcolor="#6495ED" style="text-align:center">QUANTITY</th>
		  <th width="5%" bgcolor="#6495ED" style="text-align:center">AMOUNT</th>
		  <th width="3%" bgcolor="#98FB98" style="text-align:center">BAG</th>
		  <th width="6%" bgcolor="#98FB98" style="text-align:center">QUANTITY</th>
		  <th width="7%" bgcolor="#98FB98" style="text-align:center">AMOUNT</th>
		  <th width="4%" bgcolor="#8FBC8F" style="text-align:center">BAG</th>
		  <th width="6%" bgcolor="#8FBC8F" style="text-align:center">QUANTITY</th>
		  <th width="10%" bgcolor="#8FBC8F" style="text-align:center">AMOUNT</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		if(isset($group_for)) 		{$group_for_con=' and c.group_for="'.$group_for.'"';}
		...
		$sql = "select c.item_id, s.sub_group_id, sum(c.bag_unit) as bag_unit 
		 from sale_do_master m, sale_do_chalan c, dealer_info d,  zon z, item_info i, item_sub_group s
		 where c.dealer_code=d.dealer_code  and z.ZONE_CODE=d.zone_code and m.do_no=c.do_no and i.sub_group_id=s.sub_group_id  and c.item_id=i.item_id 
		 and s.group_id!=900000000 and m.do_type!='ADVANCE' and 
		  c.chalan_date between '".$f_date."' and '".$t_date."' ".$con.$group_for_con.$dealer_con.$item_sub_con.$item_group_con.$item_type_con.$do_type_con." group by s.sub_group_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $bag_unit[$info->sub_group_id]=$info->bag_unit;
		
		}
		
		
		$sql = "select c.item_id, s.sub_group_id, sum(c.bag_unit) as adv_bag_unit 
		 from sale_do_master m, sale_do_chalan c, dealer_info d,  zon z, item_info i, item_sub_group s
		 where c.dealer_code=d.dealer_code  and z.ZONE_CODE=d.zone_code and m.do_no=c.do_no and i.sub_group_id=s.sub_group_id  and c.item_id=i.item_id and 
		 s.group_id!=900000000 and m.do_type='ADVANCE' and 
		  c.chalan_date between '".$f_date."' and '".$t_date."' ".$con.$group_for_con.$dealer_con.$item_sub_con.$item_group_con.$item_type_con.$do_type_con." group by s.sub_group_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $adv_bag_unit[$info->sub_group_id]=$info->adv_bag_unit;
		 ...
		 $sql = "select c.item_id, s.sub_group_id, sum(c.total_unit) as sales_qty , sum(c.total_amt) as sales_amt
		 from sale_do_master m, sale_do_chalan c, dealer_info d,  zon z, item_info i, item_sub_group s
		 where c.dealer_code=d.dealer_code  and z.ZONE_CODE=d.zone_code and m.do_no=c.do_no and i.sub_group_id=s.sub_group_id  and c.item_id=i.item_id and m.do_type!='ADVANCE' and 
		  c.chalan_date between '".$f_date."' and '".$t_date."' ".$con.$group_for_con.$dealer_con.$item_sub_con.$item_group_con.$item_type_con.$do_type_con." group by s.sub_group_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $sales_qty[$info->sub_group_id]=$info->sales_qty;
		...
		$sql = "select c.item_id, s.sub_group_id, sum(c.total_unit) as adv_sales_qty , sum(c.total_amt) as adv_sales_amt
		 from sale_do_master m, sale_do_chalan c, dealer_info d,  zon z, item_info i, item_sub_group s
		 where c.dealer_code=d.dealer_code  and z.ZONE_CODE=d.zone_code and m.do_no=c.do_no and i.sub_group_id=s.sub_group_id  and c.item_id=i.item_id and m.do_type='ADVANCE' and 
		  c.chalan_date between '".$f_date."' and '".$t_date."' ".$con.$group_for_con.$dealer_con.$item_sub_con.$item_group_con.$item_type_con.$do_type_con." group by s.sub_group_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $adv_sales_qty[$info->sub_group_id]=$info->adv_sales_qty;
		 $adv_sales_amt[$info->sub_group_id]=$info->adv_sales_amt;	 
		}
		...
		//if($_POST['do_type']=='REGULAR' || $_POST['do_type']=='') {}
		  ...
		  $sql = "select c.item_id, s.sub_group_id, sum(c.total_unit) as return_qty, sum(c.return_amt) as return_amt
		  from sale_return_master m, sale_return_details c, dealer_info d, zon z, item_info i, item_sub_group s
		  where m.dealer_code=d.dealer_code and z.ZONE_CODE=d.zone_code  and m.do_no=c.do_no and i.sub_group_id=s.sub_group_id  and c.item_id=i.item_id and
		  m.do_date between '".$f_date."' and '".$t_date."'  ".$con.$group_for_con.$dealer_con.$item_sub_con.$item_group_con.$item_type_con." group by s.sub_group_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $return_qty[$info->sub_group_id]=$info->return_qty;
		 $return_amt[$info->sub_group_id]=$info->return_amt;
		 ...
		 $sql = "select c.item_id, s.sub_group_id, sum(c.bag_unit) as return_bag_unit
		  from sale_return_master m, sale_return_details c, dealer_info d, zon z, item_info i, item_sub_group s
		  where m.dealer_code=d.dealer_code and z.ZONE_CODE=d.zone_code  and m.do_no=c.do_no and i.sub_group_id=s.sub_group_id  and c.item_id=i.item_id and s.group_id!=900000000 and
		  m.do_date between '".$f_date."' and '".$t_date."'  ".$con.$group_for_con.$dealer_con.$item_sub_con.$item_group_con.$item_type_con." group by s.sub_group_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $return_bag_unit[$info->sub_group_id]=$info->return_bag_unit;
...
//		}
	  ...
	  $sql_sub="select i.sub_group_id, s.sub_group_name, g.group_id, g.group_name from sale_do_chalan c, item_info i, item_sub_group s, item_group g  where  
		 s.group_id=g.group_id and i.sub_group_id=s.sub_group_id  and g.product_type='Finish Goods'  and i.item_id=c.item_id
		 ".$item_sub_con.$group_for_con.$item_group_con.$item_mother_group_con.$item_type_con." 
		 group by g.group_id order by g.item_group_sl";
		$data_sub=db_query($sql_sub);
		 ...
		 $sql_sub="select i.sub_group_id, s.sub_group_name, g.group_id, g.group_name from sale_do_chalan c, item_info i, item_sub_group s, item_group g  where  
		 s.group_id=g.group_id and i.sub_group_id=s.sub_group_id  and g.product_type='Finish Goods'   and g.group_for!=5 and i.item_id=c.item_id
		 ".$item_sub_con.$group_for_con.$item_group_con.$item_mother_group_con.$item_type_con." 
		 group by g.group_id order by g.item_group_sl";
		$data_sub=db_query($sql_sub);
		...
		<tr>
		  <td bgcolor="#CCCCCC">&nbsp;</td>
		  <td bgcolor="#CCCCCC"><div align="left" style="text-transform:uppercase; font-size:14px; font-weight:700;"><strong><?=$info_sub->group_name;?> </strong></div></td>
		  <td bgcolor="#CCCCCC">&nbsp;</td>
		  <td bgcolor="#CCCCCC">&nbsp;</td>
		  <td bgcolor="#CCCCCC">&nbsp;</td>
		  <td bgcolor="#CCCCCC">&nbsp;</td>
		...
		if($sales_amt[$row->sub_group_id]>0 || $adv_sales_amt[$row->sub_group_id]>0 || $return_amt[$row->sub_group_id]>0 ){
		?>
		...
		<td><?=$sl++;?></td>
		<td><?=$row->sub_group_name?></td>
		<td><?=$bag_unit[$row->sub_group_id]; $tot_bag_unit +=$bag_unit[$row->sub_group_id];?></td>
		<td>
		    <?=number_format($sales_qty[$row->sub_group_id],2); $total_sales_qty +=$sales_qty[$row->sub_group_id]; ?>
          </td>
		<td>
            <?=number_format($sales_amt[$row->sub_group_id],2); $total_sales_amt +=$sales_amt[$row->sub_group_id]; ?>
          </td>
		<td><?=$adv_bag_unit[$row->sub_group_id]; $tot_adv_bag_unit +=$adv_bag_unit[$row->sub_group_id];?></td>
		<td><?=number_format($adv_sales_qty[$row->sub_group_id],2); $total_adv_sales_qty +=$adv_sales_qty[$row->sub_group_id]; ?></td>
		<td>
		  <?=number_format($adv_sales_amt[$row->sub_group_id],2); $total_adv_sales_amt +=$adv_sales_amt[$row->sub_group_id]; ?>
          </td>
		<td><?= $final_sales_bag = $bag_unit[$row->sub_group_id]+$adv_bag_unit[$row->sub_group_id]; $sub_tot_final_sales_bag +=$final_sales_bag; ?></td>
		<td><?= $final_sales_qty = $sales_qty[$row->sub_group_id]+$adv_sales_qty[$row->sub_group_id]; $sub_tot_final_sales_qty +=$final_sales_qty;?></td>
		<td><?= $final_sales_amt = $sales_amt[$row->sub_group_id]+$adv_sales_amt[$row->sub_group_id]; $sub_tot_final_sales_amt +=$final_sales_amt;?></td>
		<td><?=$return_bag_unit[$row->sub_group_id]; $tot_return_bag_unit +=$return_bag_unit[$row->sub_group_id];?></td>
		<td><?=number_format($return_qty[$row->sub_group_id],2); $total_return_qty +=$return_qty[$row->sub_group_id];?></td>
		<td>
		  <?=number_format($return_amt[$row->sub_group_id],2); $total_return_amt +=$return_amt[$row->sub_group_id];?>
         </td>
		<td><?=$net_sales_bag_unit=($final_sales_bag-$return_bag_unit[$row->sub_group_id]); $total_net_sales_bag_unit +=$net_sales_bag_unit;?></td>
		<td>
		<?=number_format($net_sales_qty=($final_sales_qty-$return_qty[$row->sub_group_id]),2); $total_net_sales_qty +=$net_sales_qty;?></td>
		<td>
		  <?=number_format($net_sales_amt=($final_sales_amt-$return_amt[$row->sub_group_id]),2); $total_net_sales_amt +=$net_sales_amt;?>		  </td>
		</tr>
<? } } ?>
		<tr>
		  <td>&nbsp;</td>
		  <td><div align="right" style="text-transform:uppercase"><strong>Sub Total:</strong></div></td>
		  <td><?=$tot_bag_unit; $grand_tot_bag_unit +=$tot_bag_unit;?></td>
		  <td><span class="style7">
            <?=number_format($total_sales_qty,2); $total_total_sales_qty +=$total_sales_qty;?>
          </span></td>
		  <td><span class="style7">
		  ...
		  <td><?=$tot_adv_bag_unit; $grand_tot_adv_bag_unit +=$tot_adv_bag_unit;?></td>
		  <td><span class="style7">
		    <?=number_format($total_adv_sales_qty,2); $total_total_adv_sales_qty +=$total_adv_sales_qty;?>
		  </span></td>
		  <td><span class="style7">
		    <?=number_format($total_adv_sales_amt,2); $total_total_adv_sales_amt +=$total_adv_sales_amt;?>
          </span></td>
		  <td><span class="style7">
		    <?=number_format($sub_tot_final_sales_bag,2); $grand_tot_sub_tot_final_sales_bag +=$sub_tot_final_sales_bag;?>
          </span></td>
		  <td><span class="style7">
		    <?=number_format($sub_tot_final_sales_qty,2); $grand_tot_sub_tot_final_sales_qty +=$sub_tot_final_sales_qty;?>
          </span></td>
		  <td><span class="style7">
		    <?=number_format($sub_tot_final_sales_amt,2); $grand_tot_sub_tot_final_sales_amt +=$sub_tot_final_sales_amt;?>
          </span></td>
		  <td><?=$tot_return_bag_unit; $grand_tot_return_bag_unit +=$tot_return_bag_unit;?></td>
		  <td><span class="style7">
		    <?=number_format($total_return_qty,2); $total_total_return_qty +=$total_return_qty;?>
		  </span></td>
		  <td><span class="style7">
		  ...
		  <td><span class="style7">
		    <?=$total_net_sales_bag_unit; $total_total_net_sales_bag_unit +=$total_net_sales_bag_unit;?>
		  </span></td>
		  <td><span class="style7">
		    <?=number_format($total_net_sales_qty,2); $total_total_net_sales_qty +=$total_net_sales_qty;?>
		  </span></td>
		  ...
		  <? 
		     ...
		     $total_sales_amt = 0;
			 $tot_adv_bag_unit = 0;
			 $total_adv_sales_qty = 0;
		     $total_adv_sales_amt = 0;
			 $sub_tot_final_sales_bag = 0;
			 $sub_tot_final_sales_qty = 0;
		     $sub_tot_final_sales_amt = 0;
			 $tot_return_bag_unit = 0;
			 $total_return_qty = 0; 
			 $total_return_amt = 0; 
			 $total_net_sales_bag_unit =0;
			 $total_net_sales_qty = 0;
			 $total_net_sales_amt = 0; } } ?>
 ...
 <tr>
		  ...
		  <td><?=$grand_tot_adv_bag_unit;?></td>
		  <td><span class="style7">
		    <?=number_format($total_total_adv_sales_qty,2);?>
		  </span></td>
		  <td><span class="style7">
		    <?=number_format($total_total_adv_sales_amt,2);?>
          </span></td>
		  <td><span class="style7">
		    <?=number_format($grand_tot_sub_tot_final_sales_bag,2);?>
          </span></td>
		  <td><span class="style7">
		    <?=number_format($grand_tot_sub_tot_final_sales_qty,2);?>
          </span></td>
		  <td><span class="style7">
		    <?=number_format($grand_tot_sub_tot_final_sales_amt,2);?>
          </span></td>
		  <td><?=$grand_tot_return_bag_unit;?></td>
		  <td><span class="style7">
		    <?=number_format($total_total_return_qty,2);?>
		  </span></td>
		  <td><span class="style7">
		  ...
		  <td><span class="style7">
		    <?=$total_total_net_sales_bag_unit;?>
		  </span></td>
		  <td><span class="style7">
		    <?=number_format($total_total_net_sales_qty,2);?>
		  </span></td>
...
elseif($_POST['report']==20060203)
{
		...
		<tr>
		<th width="2%" rowspan="2" bgcolor="#9999FF">SL</th>
		<th width="30%" rowspan="2" bgcolor="#9999FF">Dealer Name </th>
		<th colspan="2" bgcolor="#99FFFF" style="text-align:center">SALES</th>
		<th colspan="2" bgcolor="#33CCCC" style="text-align:center">SALES RETURN</th>
		<th colspan="2" bgcolor="#FF0000" style="text-align:center">NET SALES </th>
		</tr>
...
$report_type_con .= " and c.group_for!=5";
$report_type_com_con .= " and m.group_for!=5";
		...
		$sql = "select c.dealer_code, sum(c.total_amt) as sales_amt, sum(c.total_unit) as sales_qty  
		 ...
		 c.chalan_date between '".$f_date."' and '".$t_date."' and g.product_type='Finish Goods' and g.group_id!=900000000
		 ".$con.$item_sub_con.$group_for_con.$item_group_con.$item_mother_group_con.$item_type_con.$do_type_con.$item_con_in.$report_type_con."  group by c.dealer_code ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		...
		// $sales_qty[$info->dealer_code]=$info->sales_qty;
		 ...
		 $sql = "select c.dealer_code, d.zone_code,  sum(c.total_amt) as zone_sales_amt, sum(c.total_unit) as zone_sales_qty  
		 from sale_do_master m, sale_do_chalan c, dealer_info d, item_info i, item_sub_group s, item_group g, zon z
		 where m.do_no=c.do_no and c.dealer_code=d.dealer_code and c.item_id=i.item_id and s.sub_group_id=i.sub_group_id and s.group_id=g.group_id  and d.zone_code=z.ZONE_CODE and
		 c.chalan_date between '".$f_date."' and '".$t_date."' and g.product_type='Finish Goods' 
		 ".$con.$item_sub_con.$group_for_con.$item_group_con.$item_mother_group_con.$item_type_con.$do_type_con.$item_con_in.$report_type_con."  group by d.zone_code";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $zone_sales_amt[$info->zone_code]=$info->zone_sales_amt;
		 $zone_sales_qty[$info->zone_code]=$info->zone_sales_qty;
		...
		$sql = "select c.dealer_code, sum(c.return_amt) as return_amt, sum(c.total_unit) as return_qty
		  ...
		  where  m.do_no=c.do_no and c.dealer_code=d.dealer_code and c.item_id=i.item_id and s.sub_group_id=i.sub_group_id  and s.group_id=g.group_id and d.zone_code=z.ZONE_CODE and
		  c.do_date between '".$f_date."' and '".$t_date."' and g.product_type='Finish Goods'  and g.group_id!=900000000
		  ".$con.$item_sub_con.$group_for_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_con_in.$report_type_com_con." group by c.dealer_code ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		...
		// $return_qty[$info->dealer_code]=$info->return_qty;
		 ...
		 $sql = "select c.dealer_code, d.zone_code, sum(c.return_amt) as zone_return_amt, sum(c.total_unit) as zone_return_qty
		  from sale_return_master m, sale_return_details c, dealer_info d, item_info i, item_sub_group s, item_group g, zon z
		  where  m.do_no=c.do_no and c.dealer_code=d.dealer_code and c.item_id=i.item_id and s.sub_group_id=i.sub_group_id  and s.group_id=g.group_id and d.zone_code=z.ZONE_CODE and
		  c.do_date between '".$f_date."' and '".$t_date."' and g.product_type='Finish Goods' 
		  ".$con.$item_sub_con.$group_for_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_con_in.$report_type_com_con." group by d.zone_code ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $zone_return_amt[$info->zone_code]=$info->zone_return_amt;
		 $zone_return_qty[$info->zone_code]=$info->zone_return_qty;
  		 }
		...
		if(isset($zone_id)) 		{$zon_con=' and z.ZONE_CODE="'.$zone_id.'"';}
		 ...
		 z.ZONE_CODE=d.zone_code ".$zon_con.$dealer_con." group by z.ZONE_CODE";
		$data_sub=db_query($sql_sub);

		while($info_sub=mysqli_fetch_object($data_sub)){ 
		
		
		if($zone_sales_amt[$info_sub->ZONE_CODE]>0 || $zone_return_amt[$info_sub->ZONE_CODE]>0){
		?>
		...
		<?
		 ...
		 where  z.ZONE_CODE=d.zone_code and d.zone_code='".$info_sub->ZONE_CODE."'  ".$con.$dealer_con." order by z.ZONE_CODE,d.dealer_code";
		
		$res	 = db_query($sql);
		while($row=mysqli_fetch_object($res))
		{

		if($sales_amt[$row->dealer_code]>0 || $return_amt[$row->dealer_code]>0 ){
		?>
...
$report_type_com_con .= " and m.group_for!=5";
		...
		$sql = "select c.dealer_code, sum(c.total_unit) as regular_qty , sum(c.total_amt) as regular_amt
		 from sale_do_master m, sale_do_chalan c, dealer_info d, item_info i, item_sub_group s, item_group g, zon z
		 where m.do_no=c.do_no and  c.dealer_code=d.dealer_code and c.item_id=i.item_id and  s.sub_group_id=i.sub_group_id  and s.group_id=g.group_id and  d.zone_code=z.ZONE_CODE and
		 c.chalan_date between '".$f_date."' and '".$t_date."' and g.product_type='Finish Goods' and m.do_type not in ('ADVANCE') and g.group_id!=900000000
		 ".$con.$item_sub_con.$group_for_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_con_in.$report_type_con."  group by c.dealer_code ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		...
		// $regular_amt[$info->dealer_code]=$info->regular_amt;
		 ...
		 from sale_do_master m, sale_do_chalan c, dealer_info d, item_info i, item_sub_group s, item_group g, zon z
		 where m.do_no=c.do_no and  c.dealer_code=d.dealer_code and c.item_id=i.item_id and  s.sub_group_id=i.sub_group_id  and s.group_id=g.group_id and  d.zone_code=z.ZONE_CODE and
		 c.chalan_date between '".$f_date."' and '".$t_date."' and g.product_type='Finish Goods' and m.do_type not in ('ADVANCE') 
		 ".$con.$item_sub_con.$group_for_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_con_in.$report_type_con."  group by c.dealer_code ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		// $regular_qty[$info->dealer_code]=$info->regular_qty;
		 ...
		 from sale_do_master m, sale_do_chalan c, dealer_info d, item_info i, item_sub_group s, item_group g, zon z
		 where m.do_no=c.do_no and  c.dealer_code=d.dealer_code and c.item_id=i.item_id and  s.sub_group_id=i.sub_group_id  and s.group_id=g.group_id and  d.zone_code=z.ZONE_CODE and
		 c.chalan_date between '".$f_date."' and '".$t_date."' and g.product_type='Finish Goods' and m.do_type='ADVANCE'  and g.group_id!=900000000
		 ".$con.$item_sub_con.$group_for_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_con_in.$report_type_con."  group by c.dealer_code ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		...
		if($regular_amt[$row->dealer_code]>0 || $advance_amt[$row->dealer_code]>0 || $return_amt[$row->dealer_code]>0 ){
		?>
...
elseif($_POST['report']==211100703)
{
		$report="ITEM TO ITEM SALES COMPARISON ";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
		...
		$date_con .= ' and m.do_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'"';
		
		if($_POST['dealer_code_in']!='')  
		$dealer_con_in .= " and m.dealer_code in (".$_POST['dealer_code_in'].")";
		
		if($_POST['item_id_in']!='')  
		$item_con_in .= " and c.item_id in (".$_POST['item_id_in'].")";
		...
		$do_type_con .= ' and m.do_type = "'.$_POST['do_type'].'" ';
				...
				$sql  = 'select c.item_id, c.item_name
				from item_info c 
				 where 1 '.$item_con_in.' 
				 ';
				$query = db_query($sql);
				...
				$item_code[$i] = $data->item_id;
		  		$item_name[$i] = $data->item_name;
		 		}
				
				
		
		 		 $sql  = 'select m.dealer_code, c.item_id, i.item_name, sum(c.total_unit) as total_unit
				from sale_do_master m, sale_do_chalan c, dealer_info d, item_info i 
				 where m.do_no=c.do_no and m.dealer_code=d.dealer_code and c.item_id=i.item_id '.$date_con.$dealer_con_in.$item_con_in.$do_type_con.' 
				 group by m.dealer_code, c.item_id';
				$query = db_query($sql);	
				while($data=mysqli_fetch_object($query)){++$i;
				$vals[$data->dealer_code][$data->item_id] = $data->total_unit;
				$total_item[$data->item_id] = $total_item[$data->item_id] + $data->total_unit;
		 		}
				
				if($_POST['do_type']=='REGULAR' || $_POST['do_type']=='') {
				$sql  = 'select m.dealer_code, c.item_id, i.item_name, sum(c.total_unit) as return_unit
				from sale_return_master m, sale_return_details c, dealer_info d, item_info i 
				 where m.do_no=c.do_no and m.dealer_code=d.dealer_code and c.item_id=i.item_id '.$date_con.$dealer_con_in.$item_con_in.' 
				 group by m.dealer_code, c.item_id';
				$query = db_query($sql);	
				while($data=mysqli_fetch_object($query)){++$i;
				$vals_return[$data->dealer_code][$data->item_id] = $data->return_unit;
				$total_return[$data->item_id] = $total_return[$data->item_id] + $data->return_unit;
		 		}
				}
		 
		
		 $sqlcl="select  d.dealer_code, d.dealer_name_e, z.ZONE_NAME, c.item_id from  sale_do_master m, sale_do_chalan c, dealer_info d, zon z
		 where m.do_no=c.do_no and m.dealer_code=d.dealer_code and  z.ZONE_CODE=d.zone_code  ".$date_con.$dealer_con_in.$item_con_in.$do_type_con."
		  group by m.dealer_code order by z.ZONE_CODE, d.dealer_code";
...
for($i=1;$i<=$cols;$i++)
{
echo '<th bgcolor="#66CCCC" ><strong>'.$item_name[$i].'</strong></th>';
}
...
for($i=1;$i<=$cols;$i++)
{
echo '<td>'.number_format($vals[$row->dealer_code][$item_code[$i]]-$vals_return[$row->dealer_code][$item_code[$i]],2).'</td>';
}
...
for($i=1;$i<=$cols;$i++)
{
echo '<td><strong>'.number_format($total_item[$item_code[$i]]-$total_return[$item_code[$i]],2).'</strong></td>';
...
elseif($_POST['report']==220305001)
{
		$report="CUSTOMER TO CUSTOMER COMPARISON";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
...
//		 		}
				...
				$sql  = 'select d.dealer_code, d.dealer_name_e
				from dealer_info d
				 where 1 '.$dealer_con_in.'';
				$query = db_query($sql);
				$cols  = mysqli_num_rows($query);
				while($data=mysqli_fetch_object($query)){++$i;
				$dealer_code[$i] = $data->dealer_code;
		  		$dealer_name_e[$i] = $data->dealer_name_e;
		 		}
				
				
		
		 		 $sql  = 'select m.dealer_code, c.item_id, i.item_name, sum(c.total_unit) as total_unit, i.sub_group_id
				 from sale_do_master m, sale_do_chalan c, dealer_info d, item_info i, item_sub_group s, item_group g 
				 where m.do_no=c.do_no and m.dealer_code=d.dealer_code and c.item_id=i.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id
				 and g.product_type="Finish Goods" and g.group_id!=900000000
				 '.$do_type_con.$date_con.$dealer_con_in.$do_type_con.$item_mother_group_con.$item_group_con.$item_sub_con.$item_id_con.$item_type_con.' 
				 group by m.dealer_code, c.item_id';
				$query = db_query($sql);	
				while($data=mysqli_fetch_object($query)){++$i;
				$vals[$data->item_id][$data->dealer_code] = $data->total_unit;
				$item_total[$data->item_id] = $item_total[$data->item_id] + $data->total_unit;
				$sub_group_total[$data->sub_group_id][$data->dealer_code] = $sub_group_total[$data->sub_group_id][$data->dealer_code] +  $data->total_unit;
				$sub_group_gtotal[$data->sub_group_id] = $sub_group_gtotal[$data->sub_group_id] +  $data->total_unit;
				$total_item[$data->dealer_code] = $total_item[$data->dealer_code] + $data->total_unit;
				$all_total = $all_total +  $data->total_unit;
		 		}
				
				if($_POST['do_type']=='REGULAR' || $_POST['do_type']=='') {
				$sql  = 'select m.dealer_code, c.item_id, i.item_name, sum(c.total_unit) as return_unit, i.sub_group_id
				from sale_return_master m, sale_return_details c, dealer_info d, item_info i, item_sub_group s, item_group g
				where m.do_no=c.do_no and m.dealer_code=d.dealer_code and c.item_id=i.item_id and i.sub_group_id=s.sub_group_id and s.group_id=g.group_id
				and g.product_type="Finish Goods" and g.group_id!=900000000  '.$date_con.$dealer_con_in.$item_mother_group_con.$item_group_con.$item_sub_con.$item_id_con.$item_type_con.' 
				group by m.dealer_code, c.item_id';
				$query = db_query($sql);	
				while($data=mysqli_fetch_object($query)){++$i;
				$vals_return[$data->item_id][$data->dealer_code] = $data->return_unit;
				$item_total_return[$data->item_id] = $item_total[$data->item_id] + $data->return_unit;
				$sub_group_total_return[$data->sub_group_id][$data->dealer_code] = $sub_group_total_return[$data->sub_group_id][$data->dealer_code] +  $data->return_unit;
				$sub_group_gtotal_return[$data->sub_group_id] = $sub_group_gtotal_return[$data->sub_group_id] +  $data->return_unit;
				$total_return[$data->dealer_code] = $total_return[$data->dealer_code] + $data->return_unit;
				$all_total_return = $all_total_return +  $data->return_unit;
		 		}
				}
		 
		
		 $sqlcl="select i.item_id, i.item_name, s.sub_group_id, s.sub_group_name 
		 from item_info i, item_sub_group s, item_group g
		 where  i.sub_group_id=s.sub_group_id and s.group_id=g.group_id ".$item_mother_group_con.$item_group_con.$item_sub_con.$item_id_con.$item_type_con."
		 and g.product_type='Finish Goods' and g.group_id!=900000000 group by i.item_id
		order by  s.sub_group_sl, i.item_name ";
		...
		<tr>
		<th width="4%" bgcolor="#FFFACD">SL</th>
		<th width="17%" bgcolor="#FFFACD">ITEM NAME </th>
...
for($i=1;$i<=$cols;$i++)
{
echo '<th bgcolor="#66CCCC" ><strong>'.$dealer_name_e[$i].'</strong></th>';
}

?>

<th bgcolor="#66CCCC">TOTAL</th>
		...
		if(($old_sub_group_id != $row->sub_group_id)&&$old_sub_group_id >0)
		{
		...
		<tr bgcolor="#E1F4FA" style=" font-weight:700; ">
        <td colspan="2" style="text-transform:uppercase">TOTAL <?=$old_sub_group_name?></td>
        <? for($i=1;$i<=$cols;$i++){ ?><td><?  echo  number_format($sub_group_total[$old_sub_group_id][$dealer_code[$i]]-$sub_group_total_return[$old_sub_group_id][$dealer_code[$i]],2);?></td> <? }?> 
		...
		if($item_total[$row->item_id]-$item_total_return[$row->item_id]<>0) {
		?>
...
for($i=1;$i<=$cols;$i++)
{
echo '<td>'.number_format($vals[$row->item_id][$dealer_code[$i]]-$vals_return[$row->item_id][$dealer_code[$i]],2).'</td>';
} ?>

<td><?=number_format($item_total[$row->item_id]-$item_total_return[$row->item_id],2);?></td>
		</tr>
<? 

}

		$old_sub_group_id = $row->sub_group_id;
		$old_sub_group_name = $row->sub_group_name;
...
<td><?  echo  number_format($sub_group_gtotal[$old_sub_group_id]-$sub_group_gtotal_return[$old_sub_group_id],2);?></td>
...
for($i=1;$i<=$cols;$i++)
{
echo '<td><strong>'.number_format($total_item[$dealer_code[$i]]-$total_return[$dealer_code[$i]],2).'</strong></td>';
...
elseif($_POST['report']==220202001)
{
		...
		$dealer_name_d2 = $_POST['dealer_code_to'];
 		...
 		$dealer_con_d2=" or d.dealer_code=".$dealer_d2[0];
		
		
		$dealer_name_d3 = $_POST['dealer_code_3'];
 		$dealer_d3=explode("-",$dealer_name_d3);
 		$dealer_d3[0];
 		if($_POST['dealer_code_3']!='')
 		$dealer_con_d3=" or d.dealer_code=".$dealer_d3[0];
		
		
		$dealer_name_d4 = $_POST['dealer_code_4'];
 		$dealer_d4=explode("-",$dealer_name_d4);
 		$dealer_d4[0];
 		if($_POST['dealer_code_4']!='')
 		$dealer_con_d4=" or d.dealer_code=".$dealer_d4[0];
		
		
		$dealer_name_d5 = $_POST['dealer_code_5'];
 		$dealer_d5=explode("-",$dealer_name_d5);
 		$dealer_d5[0];
 		if($_POST['dealer_code_5']!='')
 		$dealer_con_d5=" or d.dealer_code=".$dealer_d5[0];
		
		
		$dealer_name_d6 = $_POST['dealer_code_6'];
 		$dealer_d6=explode("-",$dealer_name_d6);
 		$dealer_d6[0];
 		if($_POST['dealer_code_6']!='')
 		$dealer_con_d6=" or d.dealer_code=".$dealer_d6[0];
		
		
		$dealer_name_d7 = $_POST['dealer_code_7'];
 		$dealer_d7=explode("-",$dealer_name_d7);
 		$dealer_d7[0];
 		if($_POST['dealer_code_7']!='')
 		$dealer_con_d7=" or d.dealer_code=".$dealer_d7[0];
		
		
		$dealer_name_d8 = $_POST['dealer_code_8'];
 		$dealer_d8=explode("-",$dealer_name_d8);
 		$dealer_d8[0];
 		if($_POST['dealer_code_8']!='')
 		$dealer_con_d8=" or d.dealer_code=".$dealer_d8[0];
		
		
		$dealer_name_d9 = $_POST['dealer_code_9'];
 		$dealer_d9=explode("-",$dealer_name_d9);
 		$dealer_d9[0];
 		if($_POST['dealer_code_9']!='')
 		$dealer_con_d9=" or d.dealer_code=".$dealer_d9[0];
		
		
		$dealer_name_d10 = $_POST['dealer_code_10'];
 		$dealer_d10=explode("-",$dealer_name_d10);
 		$dealer_d10[0];
 		if($_POST['dealer_code_10']!='')
 		$dealer_con_d10=" or d.dealer_code=".$dealer_d10[0];
		
		
		$dealer_name_d11 = $_POST['dealer_code_11'];
 		$dealer_d11=explode("-",$dealer_name_d11);
 		$dealer_d11[0];
 		if($_POST['dealer_code_11']!='')
 		$dealer_con_d11=" or d.dealer_code=".$dealer_d11[0];
		
		
		$dealer_name_d12 = $_POST['dealer_code_12'];
 		$dealer_d12=explode("-",$dealer_name_d12);
 		$dealer_d12[0];
 		if($_POST['dealer_code_12']!='')
 		$dealer_con_d12=" or d.dealer_code=".$dealer_d12[0];
		
		
		$dealer_name_d13 = $_POST['dealer_code_13'];
 		$dealer_d13=explode("-",$dealer_name_d13);
 		$dealer_d13[0];
 		if($_POST['dealer_code_13']!='')
 		$dealer_con_d13=" or d.dealer_code=".$dealer_d13[0];
		
		
		$dealer_name_d14 = $_POST['dealer_code_14'];
 		$dealer_d14=explode("-",$dealer_name_d14);
 		$dealer_d14[0];
 		if($_POST['dealer_code_14']!='')
 		$dealer_con_d14=" or d.dealer_code=".$dealer_d14[0];
		
		
		$dealer_name_d15 = $_POST['dealer_code_15'];
 		$dealer_d15=explode("-",$dealer_name_d15);
 		$dealer_d15[0];
 		if($_POST['dealer_code_15']!='')
 		$dealer_con_d15=" or d.dealer_code=".$dealer_d15[0];
		...
		$do_type_con .= ' and m.do_type = "'.$_POST['do_type'].'" ';
				 ...
				 $sql  = 'select d.dealer_code, d.dealer_name_e
				from dealer_info d 
				 where 1 '.$dealer_con_d1.$dealer_con_d2.$dealer_con_d3.$dealer_con_d4.$dealer_con_d5.$dealer_con_d6.$dealer_con_d7.$dealer_con_d8.$dealer_con_d9.$dealer_con_d10.$dealer_con_d11.$dealer_con_d12.$dealer_con_d13.$dealer_con_d14.$dealer_con_d15.' 
				 ';
				$query = db_query($sql);
				$cols  = mysqli_num_rows($query);
				while($data=mysqli_fetch_object($query)){++$i;
				$dealer_code[$i] = $data->dealer_code;
		 		...
		 		echo $sql  = 'select m.dealer_code, c.item_id, i.item_name, sum(c.total_unit) as total_unit, i.sub_group_id
				from sale_do_master m, sale_do_chalan c, dealer_info d, item_info i 
				 where m.do_no=c.do_no and m.dealer_code=d.dealer_code and c.item_id=i.item_id '.$date_con.$dealer_con_in.$item_con_in.$do_type_con.' 
				 group by  c.item_id,  m.dealer_code';
				$query = db_query($sql);	
				while($data=mysqli_fetch_object($query)){++$i;
				$vals[$data->item_id][$data->dealer_code] = $data->total_unit;
				$total_item[$data->dealer_code] = $total_item[$data->dealer_code] + $data->total_unit;
				
				$sub_group_total[$data->sub_group_id][$data->dealer_code] = $sub_group_total[$data->sub_group_id][$data->dealer_code] +  $value->total_unit;
		...
		<tr>
		<th width="4%" bgcolor="#66CCCC">SL</th>
		<th width="17%" bgcolor="#66CCCC">ITEM NAME </th>
...
?>
	...
	$sqlcl="select distinct i.item_id, i.item_name, s.sub_group_id, s.sub_group_name from item_info i, item_sub_group s, item_group g where i.sub_group_id=s.sub_group_id and 
		s.group_id=g.group_id  and g.product_type='Finish Goods' and g.group_id!=900000000 and  i.sub_group_id='".$info_sub->sub_group_id."'
		".$item_sub_con.$group_for_item_con.$item_group_con.$item_type_con.$item_id_con." order by i.item_type, i.item_name";
...
<tr>
		<th width="4%" bgcolor="#A2C6E6">&nbsp;</th>
		<th width="17%" bgcolor="#A2C6E6"><?=$info_sub->sub_group_name?> </th>
...
for($i=1;$i<=$cols;$i++)
{
echo '<th bgcolor="#A2C6E6"><strong></strong></th>';
}
...
for($i=1;$i<=$cols;$i++)
{
echo '<td>'.number_format($vals[$row->item_id][$dealer_code[$i]],2).'</td>';
}
...
for($i=1;$i<=$cols;$i++)
{
echo '<td><strong>'.number_format($sub_group_total[$row->sub_group_id][$dealer_code[$i]],2).'</strong></td>';
...
for($i=1;$i<=$cols;$i++)
{
echo '<td><strong>'.number_format($total_item[$dealer_code[$i]],2).'</strong></td>';
...
elseif($_POST['report']==210617001)
{
		...
		if ($_POST['sale_type']=="REGULAR") {
		
		 $customer_ledger = $row->regular_ledger;
		
		 }elseif ($_POST['sale_type']=="ADVANCE") {
		
		 $customer_ledger = $row->advance_ledger;
		
		 }elseif ($_POST['sale_type']=="MAT") {
...
elseif($_POST['report']==210617002)
{
        ...
        <tr>
          <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;"><strong>Customer Group: <?=find_a_field('ledger_group','group_name','group_id='.$_POST['ledger_group'])?></strong></td>
         </tr>
		 <? }?>
...
echo $count_days = round($datediff / (60 * 60 * 24));
		...
		<tr height="30">
		<th width="4%" bgcolor="#99CCFF">SL</th>
		<th width="20%" bgcolor="#99CCFF">DEALER NAME </th>
		<th width="14%" bgcolor="#99CCFF" style="text-align:center">OPENING</th>
		<th width="14%" bgcolor="#99CCFF" style="text-align:center">DEBIT</th>
		<th width="14%" bgcolor="#99CCFF" style="text-align:center">CREDIT</th>
		<th width="14%" bgcolor="#99CCFF" style="text-align:center">BALANCE</th>
		<th width="10%" bgcolor="#99CCFF" style="text-align:center">PERCENTAGE (%)</th>
		<th width="10%" bgcolor="#99CCFF" style="text-align:center">CREDIT DURATION</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		if($_POST['ledger_group']) 		{$ledger_group_con=' and ledger_group_id="'.$_POST['ledger_group'].'"';}
		
		 $sql_sub="select ledger_id, ledger_name from accounts_ledger where ledger_layer=1 ".$ledger_group_con." group by ledger_id order by acc_code";
			$data_sub=db_query($sql_sub);
		...
		<tr style=" font-size:14px;">
		  ...
		  <td bgcolor="#d6eaf8"><div align="left" style="text-transform:uppercase; font-size:14px; font-weight:700;"><strong><?=$info_sub->ledger_name?></strong></div></td>
		  <td bgcolor="#d6eaf8">&nbsp;</td>
		  <td bgcolor="#d6eaf8">&nbsp;</td>
		  <td bgcolor="#d6eaf8">&nbsp;</td>
		  <td bgcolor="#d6eaf8">&nbsp;</td>
		...
		<?
 		...
 		$sql="select distinct ledger_id, ledger_name from accounts_ledger where under_ledger='".$info_sub->ledger_id."'  ".$con." order by under_ledger,ledger_id";
		...
		<tr <?php /*?><?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?><?php */?> <? if ($percentage_if<5.1){ echo 'bgcolor="#64E986"';}elseif($percentage_if>15) {echo 'bgcolor="#E0B0FF"';}?> style="font-size:14px;">
		<td><?=$sl++;?></td>
		<td><a href="customer_ledger_statment.php?show=show&fdate=<?=$_REQUEST['f_date']?>&tdate=<?=$_REQUEST['t_date']?>&ledger_id=<?=$row->ledger_id?>" target="_blank" style=" color:#000000; text-decoration:none; font-size:14px;">
		...
		if ($opening_amt[$row->ledger_id]>0) { echo  number_format($opening_balance,2). " (DR)"; } else { echo number_format($opening_balance*(-1),2). " (CR)"; }  ?></td>
		...
		if ($closing_amt[$row->ledger_id]>0) { echo  number_format($closing_balance,2). " (DR)"; } else { echo number_format($closing_balance*(-1),2). " (CR)"; } 
		 ?></td>
		<td><?= number_format($percentage = ($closing_amt[$row->ledger_id]*100)/$dr_amt[$row->ledger_id],2); ?></td>
		<td><?= number_format($credit_duration = ($closing_amt[$row->ledger_id]/$dr_amt[$row->ledger_id])*$count_days,2); ?> Days</td>
		</tr>
<? } }?>
		<tr  style="font-size:14px;">
		  <td>&nbsp;</td>
...
elseif($_POST['report']==220315001)
{
		$report="CURRENT LIABILITIES REPORT";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
...
echo $count_days = round($datediff / (60 * 60 * 24));
		...
		<?
 		...
 		$sql="select distinct ledger_id, ledger_name from accounts_ledger where under_ledger='".$info_sub->ledger_id."' and ledger_id not in (2018000300150000, 2018000100400000)
		 ".$con." order by under_ledger,ledger_id";
		...
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?> <?php /*?><? if ($percentage_if<5.1){ echo 'bgcolor="#64E986"';}elseif($percentage_if>15) {echo 'bgcolor="#E0B0FF"';}?> <?php */?>style="font-size:14px;">
		<td><?=$sl++;?></td>
		<td><a href="transaction_listledger_for_bank.php?show=show&fdate=<?=$_REQUEST['f_date']?>&tdate=<?=$_REQUEST['t_date']?>&ledger_id=<?=$row->ledger_id?>" target="_blank" style=" color:#000000; text-decoration:none; font-size:14px;">
		<?=$row->ledger_name?></a></td>
		<td><div align="right">
		  <? number_format($opening_balance=$opening_amt[$row->ledger_id],2);  $sub_total_opening_amt +=$opening_amt[$row->ledger_id]; 
		if ($opening_amt[$row->ledger_id]>0) { echo  number_format($opening_balance,2). " (DR)"; } else { echo number_format($opening_balance*(-1),2). " (CR)"; }  ?>
...
elseif($_POST['report']==220315004)
{
		$report="BANK LOANS & LIABILITIES REPORT";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
...
echo $count_days = round($datediff / (60 * 60 * 24));
		...
		<tr height="30">
		<th width="3%" bgcolor="#99CCFF">SL</th>
		<th width="12%" bgcolor="#99CCFF">BANK NAME </th>
		<th width="21%" bgcolor="#99CCFF">LEDGER NAME </th>
		<th width="16%" bgcolor="#99CCFF" style="text-align:center">OPENING</th>
		<th width="16%" bgcolor="#99CCFF" style="text-align:center">DEBIT</th>
		<th width="16%" bgcolor="#99CCFF" style="text-align:center">CREDIT</th>
		<th width="16%" bgcolor="#99CCFF" style="text-align:center">BALANCE</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		$sql = "select ledger_id, bank_group from lc_ltr_loan where 1 ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $bank_group[$info->ledger_id]=$info->bank_group;
		}
		
		
		$sql = "select id, bank_group as bank_group_name from bank_group where 1 ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $bank_group_name[$info->id]=$info->bank_group_name;
		}
		...
		$sql = "select ledger_id, sum(dr_amt-cr_amt) as closing_amt from journal where jv_date <='".$t_date."' group by ledger_id ";
		 ...
		 $sql = "select a.under_ledger, a.ledger_name, sum(j.dr_amt-j.cr_amt) as group_opening_amt from journal j, accounts_ledger a where a.ledger_id=j.ledger_id 
		and j.jv_date <'".$f_date."' and a.ledger_group_id=2022 group by a.under_ledger  ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $group_opening_amt[$info->under_ledger]=$info->group_opening_amt;
		}
		
		
		 $sql = "select a.under_ledger, a.ledger_name, sum(j.dr_amt) as group_dr_amt from journal j, accounts_ledger a where a.ledger_id=j.ledger_id 
		and j.jv_date between '".$f_date."' and '".$t_date."' and a.ledger_group_id=2022 group by a.under_ledger  ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $group_dr_amt[$info->under_ledger]=$info->group_dr_amt;
		}
		
		
		 $sql = "select a.under_ledger, a.ledger_name, sum(j.cr_amt) as group_cr_amt from journal j, accounts_ledger a where a.ledger_id=j.ledger_id 
		and j.jv_date between '".$f_date."' and '".$t_date."' and a.ledger_group_id=2022 group by a.under_ledger  ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $group_cr_amt[$info->under_ledger]=$info->group_cr_amt;
		}
		
		
		
		 $sql = "select a.under_ledger, a.ledger_name, sum(j.dr_amt-j.cr_amt) as group_closing_amtt from journal j, accounts_ledger a where a.ledger_id=j.ledger_id 
		and j.jv_date <='".$t_date."' and a.ledger_group_id=2022 group by a.under_ledger  ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $group_closing_amtt[$info->under_ledger]=$info->group_closing_amtt;
		}

		
		
		
		if($_POST['ledger_group']) 		{$ledger_group_con=' and ledger_group_id="2022"';}
		...
		if($group_opening_amt[$info_sub->ledger_id]<>0 || $group_dr_amt[$info_sub->ledger_id]<>0 || $group_cr_amt[$info_sub->ledger_id]<>0 || $group_closing_amtt[$info_sub->ledger_id]<>0){
		...
		<?
 		...
 		$sql="select distinct ledger_id, ledger_name from accounts_ledger where under_ledger='".$info_sub->ledger_id."' 
		 ".$con." order by under_ledger,ledger_name";
		...
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?> <?php /*?><? if ($percentage_if<5.1){ echo 'bgcolor="#64E986"';}elseif($percentage_if>15) {echo 'bgcolor="#E0B0FF"';}?> <?php */?>style="font-size:14px;">
		<td><?=$sl++;?></td>
		<td><? $bank_grp_id = $bank_group[$row->ledger_id]?><?=$bank_group_name[$bank_grp_id]?></td>
		<td><a href="transaction_listledger_for_bank.php?show=show&fdate=<?=$_REQUEST['f_date']?>&tdate=<?=$_REQUEST['t_date']?>&ledger_id=<?=$row->ledger_id?>" target="_blank" style=" color:#000000; text-decoration:none; font-size:14px;">
		<?=$row->ledger_name?></a></td>
		<td><div align="right">
		  <? number_format($opening_balance=$opening_amt[$row->ledger_id],2);  $sub_total_opening_amt +=$opening_amt[$row->ledger_id]; 
...
elseif($_POST['report']==220315003)
{
		$report="CURRENT LIABILITIES (HAND LOAN)";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
...
echo $count_days = round($datediff / (60 * 60 * 24));
		...
		<?
 		...
 		$sql="select distinct ledger_id, ledger_name from accounts_ledger where ledger_group_id=2023
		 ".$con." order by ledger_name";
...
elseif($_POST['report']==220315002)
{
		$report="FIXED DEPOSIT REPORT";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
...
{
		$report="CUSTOMER MONTHLY SUMMARY REPORT";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
        ...
        <tr style="text-transform:uppercase;">
          <td colspan="15" align="center" style="font-size:14px; border:0px; border-color:white;">CUSTOMER  LEDGER: <?=find_a_field('accounts_ledger','ledger_name','ledger_id='.$_POST['ledger_id'])?></td>
         </tr>
		 <? }?>
		...
		<tr height="30">
		<th width="2%" bgcolor="#99CCFF">SL</th>
		<th width="42%" bgcolor="#99CCFF">MONTH</th>
		<th width="12%" bgcolor="#99FFFF" style="text-align:center">DEBIT</th>
		<th width="10%" bgcolor="#ebdef0" style="text-align:center">CREDIT</th>
		<th width="12%" bgcolor="#CCCCFF" style="text-align:center">BALANCE</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
...
//		}
	  ...
	  $dealer_ledger = $_POST['ledger_id'];
	  
	  
	   $year_sql = 'select year(a.jv_date) yr, month(a.jv_date) mon, day(a.jv_date) day, sum(a.dr_amt) as dr_amt, sum(a.cr_amt) as cr_amt,  a.ledger_id from journal a where 
 
a.jv_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and a.ledger_id="'.$dealer_ledger.'" group by a.ledger_id, month(a.jv_date), year(a.jv_date) order by a.jv_date';
		...
		while($value = mysqli_fetch_object($year_query)){
		    ...
		    $val_dr[$value->ledger_id][$pr] = $value->dr_amt;
			
			$val_cr[$value->ledger_id][$pr] = $value->cr_amt;
			
			$val_yr[$value->ledger_id][$pr] = $value->yr;
			
			$val_mon[$value->ledger_id][$pr] = $value->mon;
			
			$val_day[$value->ledger_id][$pr] = $value->day;
				
		}
		
		
		
		 $year_sql = 'select year(a.jv_date) yr, month(a.jv_date) mon, sum(a.dr_amt-a.cr_amt) as closing_amt, a.ledger_id from journal a where 
 
a.jv_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and a.ledger_id="'.$dealer_ledger.'" group by a.ledger_id, month(a.jv_date), year(a.jv_date) order by a.jv_date';

	   	$year_query = db_query($year_sql );
		
		$count = mysqli_num_rows($year_query);
		...
		while($value = mysqli_fetch_object($year_query)){
		    ...
		    $val_closing[$value->ledger_id][$pr] = $value->closing_amt;
		...
		<tr style="font-size:14px;">
		  <td>&nbsp;</td>
		  <td>Opening Balance of <?php echo date('d-m-Y',strtotime($_POST['f_date']));?></td>
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
		  <td>
		   <? $opening_date = $_POST['f_date'];
		 
		  $opening_sql="SELECT sum(dr_amt-cr_amt) as opening_amt FROM journal  WHERE jv_date<'".$opening_date."' and ledger_id='".$dealer_ledger."'";
$opening_amt = find_a_field_sql($opening_sql); 
	 //$closing_amt;
	
	if ($opening_amt>0) { echo  number_format($opening_amt,2). " (DR)"; } else { echo number_format($opening_amt*(-1),2). " (CR)"; }  ?>		  </td>
		  </tr>
		
		<? for($i=1;$i<=$t;$i++){ ?>
		
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?> style="font-size:14px;">
		<td><?=$sl++;?></td>
		<td><?=$period_name[$i].'-'.substr($period[$i],2,2)?> </td>
		<td><?=number_format($val_dr[$dealer_ledger][$period[$i]],2); $total_dr_amt +=$val_dr[$dealer_ledger][$period[$i]];?></td>
		<td><?=number_format($val_cr[$dealer_ledger][$period[$i]],2); $total_cr_amt +=$val_cr[$dealer_ledger][$period[$i]];?></td>
		<td>
		
		<? number_format($closing_balance=$closing_amt[$row->ledger_id],2); $sub_total_closing_amt +=$closing_amt[$row->ledger_id]; ?>
		 
		 <? $closing_date = $val_yr[$dealer_ledger][$period[$i]].'-'.$val_mon[$dealer_ledger][$period[$i]].'-31';
		 
		  $closing_sql="SELECT sum(dr_amt-cr_amt) as closing_amt FROM journal  WHERE jv_date<='".$closing_date."' and ledger_id='".$dealer_ledger."'";
$closing_amt = find_a_field_sql($closing_sql); 
	 //$closing_amt;
	 
	 $total_closing_amt +=$closing_amt;
	
	if ($closing_amt>0) { echo  number_format($closing_amt,2). " (DR)"; } else { echo number_format($closing_amt*(-1),2). " (CR)"; }  ?> </td>
		</tr>
<?   }?>
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?> style="font-size:14px;">
		  <td>&nbsp;</td>
		  ...
		  <td>
		  <span class="style7">
		    <? number_format($total_closing_amt,2);?>
			
			<? if ($closing_amt>0) { echo  number_format($closing_amt,2). " (DR)"; } else { echo number_format($closing_amt*(-1),2). " (CR)"; }  ?>
		  </span>		  </td>
		  </tr>
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?> style="font-size:14px;">
		  <td>&nbsp;</td>
		  <td><div align="right"><strong>AVERAGE:</strong></div></td>
		  <td><span class="style7">
		    <?=number_format($avg_dr=($total_dr_amt/$count),2); ?>
		  </span></td>
		  <td><span class="style7">
		   <?=number_format($avg_cr=($total_cr_amt/$count),2); ?>
		  </span></td>
		  <td><span class="style7">
		    <? number_format($avg_closing=($total_closing_amt/$count),2); 
			
			if ($avg_closing>0) { echo  number_format($avg_closing,2). " (DR)"; } else { echo number_format($avg_closing*(-1),2). " (CR)"; }
			?>
		  </span></td>
		  </tr>
		  ...
		  <td>&nbsp;</td>
		  <td><span class="style7">
		    <?=number_format($avg_percentage=(($avg_closing*100)/$total_dr_amt),2); ?>
		  </span></td>
		  </tr>
...
elseif($_POST['report']==220209002)
{
		$report="CUSTOMER DATE WISE BALANCE REPORT";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
		...
		<tr height="30">
		<th width="2%" bgcolor="#99CCFF">SL</th>
		<th width="42%" bgcolor="#99CCFF">DEALER NAME </th>
		<th width="14%" bgcolor="#d6eaf8" style="text-align:center">OPENING</th>
		<th width="12%" bgcolor="#99FFFF" style="text-align:center">DEBIT</th>
		<th width="10%" bgcolor="#ebdef0" style="text-align:center">CREDIT</th>
		<th width="12%" bgcolor="#ebdef0" style="text-align:center">BALANCE</th>
		<th width="8%" bgcolor="#ebdef0" style="text-align:center">PERCENTAGE</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
...
a.jv_date BETWEEN  "'.$_POST['f_date'].'" AND "'.$_POST['t_date'].'" and a.ledger_id="'.$dealer_ledger.'" group by a.ledger_id, month(a.jv_date), year(a.jv_date) order by a.jv_date';
		...
		<td><?=$period_name[$i].'-'.substr($period[$i],2,2)?> </td>
		<td>
		<?=$opening_date = $val_yr[$dealer_ledger][$period[$i]].'-'.$val_mon[$dealer_ledger][$period[$i]].'-01';?>
		<?php /*?><? number_format($opening_balance=$opening_amt[$row->ledger_id],2);  $sub_total_opening_amt +=$opening_amt[$row->ledger_id]; 
		if ($opening_amt[$row->ledger_id]>0) { echo  number_format($opening_balance,2). " (DR)"; } else { echo number_format($opening_balance*(-1),2). " (CR)"; }  ?><?php */?></td>
		<td><?=number_format($val_dr[$dealer_ledger][$period[$i]],2); $total_dr_amt +=$val_dr[$dealer_ledger][$period[$i]];?></td>
		<td><?=number_format($val_cr[$dealer_ledger][$period[$i]],2); $total_cr_amt +=$val_cr[$dealer_ledger][$period[$i]];?></td>
		<td><?php /*?><? number_format($closing_balance=$closing_amt[$row->ledger_id],2); $sub_total_closing_amt +=$closing_amt[$row->ledger_id]; 
		if ($closing_amt[$row->ledger_id]>0) { echo  number_format($closing_balance,2). " (DR)"; } else { echo number_format($closing_balance*(-1),2). " (CR)"; } 
		 ?><?php */?></td>
		<td><?= number_format($percentage = ($closing_amt[$row->ledger_id]*100)/$dr_amt[$row->ledger_id],2); ?></td>
		</tr>
<?   }?>
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?> style="font-size:14px;">
		  ...
		  <td><span class="style7">
		   ...
		   <span class="style7">
		    <?=number_format($total_open_amt,2); ?>
          </span>
		   <?php /*?> <? number_format($sub_total_opening_balance=$sub_total_opening_amt,2); $total_sub_total_opening_amt +=$sub_total_opening_amt; 
			if ($sub_total_opening_amt>0) { echo  number_format($sub_total_opening_balance,2). " (DR)";  } else { echo number_format($sub_total_opening_balance*(-1),2). " (CR)";  }  ?><?php */?>
		  </span></td>
		  <td><span class="style7">
		    <?=number_format($total_dr_amt,2); ?>
          </span></td>
		  ...
		  <td>
		  <span class="style7">
		    <?php /*?><? number_format($sub_total_closing_balance=$sub_total_closing_amt,2); $total_sub_total_closing_amt +=$sub_total_closing_amt;
			if ($sub_total_closing_amt>0) { echo  number_format($sub_total_closing_balance,2). " (DR)"; } else { echo number_format($sub_total_closing_balance*(-1),2). " (CR)";  }    
			?><?php */?>
		  </span>		  </td>
		  <td>&nbsp;</td>
...
elseif($_REQUEST['report']==210617005)
{
		...
		<tr height="30">
		<th width="2%" bgcolor="#99CCFF">SL</th>
		<th width="11%" bgcolor="#99CCFF">BANK NAME</th>
		<th width="15%" bgcolor="#99CCFF">ACCOUNT NO. </th>
		<th width="19%" bgcolor="#99CCFF">CONCERN NAME </th>
		<th width="5%" bgcolor="#99CCFF" style="text-align:center">SIDE</th>
		<th width="10%" bgcolor="#99CCFF" style="text-align:center">OPENING</th>
		<th width="9%" bgcolor="#99CCFF" style="text-align:center">DEBIT</th>
		<th width="9%" bgcolor="#99CCFF" style="text-align:center">CREDIT</th>
		<th width="11%" bgcolor="#99CCFF" style="text-align:center">BALANCE</th>
		<th width="9%" bgcolor="#99CCFF" style="text-align:center">SIDE</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		if($_POST['bank_group']!='')
		$bank_group_con .= ' and a.id = "'.$_POST['bank_group'].'" ';
		...
		$sql = "select ledger_id,  sum(dr_amt-cr_amt) as closing_amt from journal where jv_date <='".$t_date."' group by ledger_id ";
 		...
 		$grp_sql="select a.id as group_id, a.bank_group from bank_group a, bank_ledger b  
		where a.id=b.bank_group  ".$bank_group_con." group by a.id order by a.id, b.bank_id";
		
		$grp_query	 = db_query($grp_sql);
		while($grp_data=mysqli_fetch_object($grp_query))
		{
		...
		<tr>
		<td bgcolor="#d6eaf8"></td>
		<td bgcolor="#d6eaf8"><strong><?=$grp_data->bank_group; ?></strong></td>
		<td bgcolor="#d6eaf8">&nbsp;</td>
		<td bgcolor="#d6eaf8">&nbsp;</td>
		<td bgcolor="#d6eaf8">&nbsp;</td>
		<td bgcolor="#d6eaf8">&nbsp;</td>
		...
		<?
 		 ...
 		 $sql="select a.bank_group,  b.ledger_id, b.bank_name as ledger_name, b.account_no, b.concern_group from bank_group a, bank_ledger b  
		where a.id=b.bank_group and b.bank_group='".$grp_data->group_id."' ".$bank_group_con." order by a.id, b.bank_id";
		...
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
		<td><?=$sl++;?></td>
		<td ><?=$row->bank_group?></td>
		<td><a style="color:#000000;" href="transaction_listledger_for_bank.php?show=show&fdate=<?=$_REQUEST['f_date']?>&tdate=<?=$_REQUEST['t_date']?>&ledger_id=<?=$row->ledger_id?>" target="_blank"><?=$row->account_no?></a></td>
		<td><?=$row->concern_group?></td>
		<td><div align="right">
		  <? 	if ($opening_amt[$row->ledger_id]>0) { echo  "DR"; } else { echo  "CR"; } ?>
		  </div></td>
		<td><div align="right">
		  <? number_format($opening_balance= $opening_amt[$row->ledger_id],2);  $sub_total_opening_amt +=$opening_amt[$row->ledger_id]; 
		if ($opening_amt[$row->ledger_id]>0) { echo  number_format($opening_balance,2). ""; } else { echo number_format($opening_balance*(-1),2). ""; }  ?>
		  </div></td>
		<td><div align="right">
		  <?=number_format($dr_amt[$row->ledger_id],2);  $sub_total_dr_amt +=$dr_amt[$row->ledger_id];?>
		  </div></td>
		...
		<td <? if ($closing_amt[$row->ledger_id]<0){ echo 'style="color:#FF0000; font-weight:700;"';}?> ><div align="right">
		  <? number_format($closing_balance=$closing_amt[$row->ledger_id],2); $sub_total_closing_amt +=$closing_amt[$row->ledger_id]; 
...
<?  
		...
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
		  ...
		  <td><div align="right"><strong>GRAND TOTAL</strong></div></td>
		  <td>&nbsp;</td>
		  <td><div align="right"><span class="style9">
		...
		<tr>
		<td colspan="10" align="center"><strong>BANK BALANCE SUMMARY</strong></td>
		</tr>
			...
			<td colspan="3" >
				<table  width="100%" border="0" cellpadding="2" cellspacing="0" style=" font-size:14px;" >
					<tr style="font-weight:700;">
						<td width="63%" bgcolor="#99CCFF">CASH AT BANK: (CD A/C)</td>
						<td width="37%" align="center" bgcolor="#99CCFF">BALANCE</td>
					</tr>
	...
	<?
	 ...
	 $sql = "select b.ledger_id, b.bank_group, sum(j.dr_amt-j.cr_amt) as sm_closing_amt from  bank_group a, bank_ledger b, journal j where a.id=b.bank_group and b.ledger_id=j.ledger_id
		and b.account_type!=2 and j.jv_date <='".$t_date."' group by b.bank_group ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $sm_closing_amt[$info->bank_group]=$info->sm_closing_amt;
		}
		

		
 		 $sql_sm="select a.id as bank_group_id, a.bank_group, b.ledger_id, b.bank_name as ledger_name, b.account_no, b.concern_group from bank_group a, bank_ledger b  
		where a.id=b.bank_group group by b.bank_group order by a.id";
		
		$query_sm	 = db_query($sql_sm);
		while($data_sm=mysqli_fetch_object($query_sm))
		{
		

		?>	
					<tr>
						<td><?=$data_sm->bank_group; ?></td>
						<td><div align="right">
						  <?=number_format($sm_closing_amt[$data_sm->bank_group_id],2);  $total_sm_closing_amt +=$sm_closing_amt[$data_sm->bank_group_id];?>
						</div>
					    </td>
					</tr>
		...
		<? }?>
					...
					<tr style="font-weight:700;">
						...
						<td><div align="right">
						  <?=number_format($total_sm_closing_amt,2);?>
						</div>
					    </td>
					</tr>
		  </table>			</td>
				...
				<table  width="100%" border="0" cellpadding="2" cellspacing="0" style=" font-size:14px;" >
					<tr style="font-weight:700;">
						<td colspan="3" bgcolor="#99CCFF">CASH AT BANK: (CC A/C)</td>
						<td width="13%" align="center" bgcolor="#99CCFF">SANCTION AMT</td>
						<td width="14%" align="center" bgcolor="#99CCFF">USED AMT</td>
					    <td width="12%" align="center" bgcolor="#99CCFF">USABLE AMT</td>
					</tr>
	...
	<?
	 ...
	 $sql = "select b.ledger_id, b.bank_group, sum(j.dr_amt-j.cr_amt) as cc_closing_amt from  bank_group a, bank_ledger b, journal j where a.id=b.bank_group and b.ledger_id=j.ledger_id
		and b.account_type=2 and j.jv_date <='".$t_date."' group by b.ledger_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $cc_closing_amt[$info->ledger_id]=$info->cc_closing_amt;
		}
		

		
 		 $sql_cc="select a.id as bank_group_id, a.bank_group, b.ledger_id, b.bank_name as ledger_name, b.account_no, b.concern_group, sum(b.cr_sanction_amt) as cr_sanction_amt  from bank_group a, bank_ledger b  
		where a.id=b.bank_group and  b.account_type=2 group by b.ledger_id order by a.id, b.ledger_id ";
		
		$query_cc	 = db_query($sql_cc);
		while($data_cc=mysqli_fetch_object($query_cc))
		{
		

		?>	
					<tr>
						<td width="13%"><?=$data_cc->bank_group; ?></td>
						<td width="19%"><a style="color:#000000;" href="transaction_listledger_for_bank.php?show=show&amp;fdate=<?=$_REQUEST['f_date']?>&amp;tdate=<?=$_REQUEST['t_date']?>&amp;ledger_id=<?=$data_cc->ledger_id?>" target="_blank">
						  <?=$data_cc->account_no?>
						</a></td>
						<td width="29%"><?=$data_cc->concern_group?></td>
						<td><div align="right"><?=number_format($data_cc->cr_sanction_amt,2); $total_cr_sanction_amt +=$data_cc->cr_sanction_amt;?></div></td>
						<td><div align="right">
			  
		 <?php /*?><? number_format($cc_closing_balance=$cc_closing_amt[$data_cc->ledger_id],2); $total_cc_closing_amt +=$cc_closing_amt[$data_cc->ledger_id]; 
		if ($cc_closing_amt[$data_cc->ledger_id]>0) { echo  number_format($cc_closing_balance,2). ""; } else { echo number_format($cc_closing_balance*(-1),2). ""; } 
		 ?><?php */?>
		 
		 
		  <?= number_format($cc_closing_balance=$cc_closing_amt[$data_cc->ledger_id]*(-1),2); $total_cc_closing_amt +=$cc_closing_balance; ?>
		 
		
						</div>					    </td>
					    <td><div align="right"> <?= number_format($cc_usable_amt = ($data_cc->cr_sanction_amt-$cc_closing_balance),2);  $total_cc_usable_amt += $cc_usable_amt;?> </div></td>
					</tr>
		...
		<? }?>
					...
					<tr style="font-weight:700;">
						...
						<td><div align="right">
                          <?= number_format($total_cr_sanction_amt,2); ?>
			
                        </div></td>
						<td><div align="right">
	  
			 	<?= number_format($total_cc_closing_amt,2); ?>
						</div>					    </td>
					    <td><div align="right"> <?= number_format($total_cc_usable_amt,2); ?></div></td>
					</tr>
				</table>			</td>
...
elseif($_POST['report']==200602031111)
{
		$report="DEALER WISE SALES SUMMARY REPORT (VALUE)";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
...
$report_type_com_con .= " and m.group_for!=5";
		 ...
		 where m.do_no=c.do_no and  c.dealer_code=d.dealer_code and c.item_id=i.item_id and  s.sub_group_id=i.sub_group_id  and s.group_id=g.group_id
 and  d.zone_code=z.ZONE_CODE and
		  c.chalan_date between '".$f_date."' and '".$t_date."' and g.product_type='Finish Goods' and m.do_type in ('REGULAR','MAT','CASH SALE','WASTAGE') ".$con.$item_sub_con.$group_for_con.$item_group_con.$item_mother_group_con.$item_type_con.$item_con_in.$report_type_con."  group by c.dealer_code ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $regular_qty[$info->dealer_code]=$info->regular_qty;
		 $regular_amt[$info->dealer_code]=$info->regular_amt;
...
elseif($_POST['report']==20112501)
{
		$report="ZONE TO ZONE COMPARISON (PRODUCT QUANTITY)";		
		if(isset($warehouse_id)){$con.=' and j.warehouse_id="'.$warehouse_id.'"';}
		
		$zone_name_1 = $_POST['zone_id_1'];
...
elseif($_POST['report']==20112503)
{
  ...
  <? if ($item_mother_group>0) {?>
	   ...
	   $sql_sub="select i.sub_group_id, s.sub_group_name, g.group_id, g.group_name from item_info i, item_sub_group s, item_group g where  s.group_id=g.group_id and
		i.sub_group_id=s.sub_group_id and g.product_type='Finish Goods'  and g.group_id!=900000000 ".$item_sub_con.$group_for_item_con.$item_group_con.$item_mother_group_con.$item_type_con." 
		group by g.group_id order by g.group_for, g.item_group_sl";
		$data_sub=db_query($sql_sub);
...
elseif($_POST['report']==200602032211)
{
...
elseif($_POST['report']==200602033311)
{
...
elseif($_POST['report']==2006020344)
{
  ...
  <? if ($item_mother_group>0) {?>
	 ...
	 $sql_sub="select i.sub_group_id, s.sub_group_name from sale_return_master m, sale_return_details d, item_info i, item_sub_group s, item_group g where  
		i.sub_group_id=s.sub_group_id and s.group_id=g.group_id  and g.product_type='Finish Goods' and  m.do_no=d.do_no and d.item_id=i.item_id  and g.group_id!=900000000
		...
		<?
		 ...
		 $sql="select distinct i.item_id, i.item_name, s.sub_group_name from sale_return_master m, sale_return_details d, item_info i, item_sub_group s, item_group g where i.sub_group_id=s.sub_group_id and g.product_type='Finish Goods' and m.do_no=d.do_no and d.item_id=i.item_id and s.group_id=g.group_id and i.sub_group_id='".$info_sub->sub_group_id."' and g.group_id!=900000000
