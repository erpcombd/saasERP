...
if(isset($_POST['submit'])&&isset($_POST['report'])&&$_POST['report']>0)
{
	...
	if($_POST['loan_type']>0) 				    $loan_group_id=$_POST['loan_type'];
...
<table width="100%" cellspacing="0" cellpadding="2" border="0" id="ExportTable" ><thead><tr><td style="border:0px;" colspan="9"><div class="header"><h1>M. Ahmed Tea &amp; Lands Co. Ltd</h1><h2><?=$report?></h2>
...
<table width="100%" cellspacing="0" cellpadding="2" border="0" id="ExportTable" ><thead><tr><td style="border:0px;" colspan="10"><div class="header">
<h1>
 <?
...
elseif($_POST['report']==220525001)
{
		$report="Entitlement Report";		
		if(isset($warehouse_id)){$warehouse_con.=' and j.warehouse_id="'.$warehouse_id.'"';}
...
if(isset($t_date)) 
		...
		<tr style="font-size:14px; height:22px;">
		<th width="5%" bgcolor="#82D8CF">S/L </th>
		<th width="11%" bgcolor="#82D8CF">ITEM GROUP </th>
		<th width="20%" bgcolor="#82D8CF">ITEM NAME </th>
		<th width="6%" bgcolor="#82D8CF">UNIT  </th>
		<th width="8%" bgcolor="#FF3300" style="text-align:center">Entitlement Opening</th>
		 <th width="8%" bgcolor="#FF3300" style="text-align:center">Entitlement Qty</th>
		<th width="6%" bgcolor="#FF3300" style="text-align:center">PI Qty</th>
		<th width="6%" bgcolor="#FF3300" style="text-align:center">Balance Qty </th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		 ...
		 $sql = "select item_id, sum(entitlement_value) as entitlement_op from lc_entitlement where  entitlement_date<'".$f_date."' group by item_id";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $entitlement_op[$info->item_id]=$info->entitlement_op;
		}
		
		$sql = "select item_id, sum(qty) as pi_qty_op from lc_purchase_invoice where  po_date<'".$f_date."' group by item_id";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $pi_qty_op[$info->item_id]=$info->pi_qty_op;
		}
		
		
		
		$sql = "select item_id, sum(entitlement_value) as entitlement_qty from lc_entitlement where  entitlement_date between '".$f_date."' and '".$t_date."'
		group by item_id";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $entitlement_qty[$info->item_id]=$info->entitlement_qty;
 
		}
		
	

		 $sql = "select item_id, sum(qty) as pi_qty from lc_purchase_invoice where  po_date between '".$f_date."' and '".$t_date."'
		group by item_id";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $pi_qty[$info->item_id]=$info->pi_qty;
		   ...
		   $sql="select  i.item_id, i.item_name, i.unit_name, i.pack_size,  s.sub_group_name
		  from item_info i, item_sub_group s, lc_entitlement e where i.sub_group_id=s.sub_group_id  and i.item_id=e.item_id  
		 ".$sub_group_id_con.$group_id_con.$group_for_con." group by i.item_id order by i.sub_group_id,  i.item_name";
		...
		<td><?=$row->item_name?></td>
		<td><?=$row->unit_name?></td>
		<td><?=number_format($entitlement_op_qty =($entitlement_op[$row->item_id]-$pi_qty_op[$info->item_id]),2); $total_entitlement_op_qty += $entitlement_op_qty;?></td>
		<td><?=number_format($entitlement_qty[$row->item_id],2); $total_entitlement_qty += $entitlement_qty[$row->item_id];?></td>
		<td><?=number_format($pi_qty[$row->item_id],2); $total_pi_qty+=$pi_qty[$row->item_id];?></td>
		<td><?=number_format($entitlement_balance = (($entitlement_op_qty+$entitlement_qty[$row->item_id])-$pi_qty[$row->item_id]),2); $total_entitlement_balance +=$entitlement_balance;?></td>
		</tr>
		...
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
		  ...
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
		  <td><strong><?=number_format($total_entitlement_op_qty,2);?></strong></td>
		  <td><strong><?=number_format($total_entitlement_qty,2);?></strong></td>
		  <td><strong><?=number_format($total_pi_qty,2);?></strong></td>
		  <td><strong><?=number_format($total_entitlement_balance,2);?></strong></td>
		  </tr>
...
elseif($_POST['report']==220818001)
{
		$report="L/C Entry Status Report";		
		if(isset($warehouse_id)){$warehouse_con.=' and j.warehouse_id="'.$warehouse_id.'"';}
...
if(isset($t_date)) 
		...
		<tr style="font-size:16px; height:22px;">
		<th width="5%" bgcolor="#82D8CF">S/L </th>
		<th width="12%" bgcolor="#82D8CF">TR No </th>
		<th width="15%" bgcolor="#82D8CF">TR Date </th>
		<th width="32%" bgcolor="#82D8CF">L/C No   </th>
		<th width="20%" bgcolor="#82D8CF">TR Type </th>
		<th width="16%" bgcolor="#FF3300" style="text-align:center">Payment Amt </th>
		 </tr>
		</thead><tbody>
		<? $sl=1;
		...
		$sql = "select tr_no, jv_no from secondary_journal where tr_from='LC Journal' group by tr_no";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		...
		if($_POST['lc_no']!='')
		$lc_con .= ' and b.lc_no = "'.$_POST['lc_no'].'" ';
		

		   $sql="select  a.bill_type, b.payment_no, b.payment_date, b.lc_no, b.lc_number,  sum(b.pay_amt_in) as pay_amt
		  from lc_bill_type a, lc_bill_payment b where a.id=b.bill_type  
		 ".$sub_group_id_con.$group_id_con.$lc_con." group by b.payment_no order by b.lc_no, b.payment_date";
		...
		<td><?=$sl++;?></td>
		<td><a href="../../../../AccERP/acc_mod/pages/customer_provision_jv_print_view.php?jv_no=<?=$jv_no[$row->payment_no]?>" target="_blank"><?=$row->payment_no;?> </a> <? $jv_no[$row->payment_no]?></td>
		<td><?=$row->payment_date;?></td>
		<td><?=$row->lc_number;?></td>
		<td><?=$row->bill_type; ?></td>
		<td><?=number_format($row->pay_amt,2); $tot_pay_amt+=$row->pay_amt;?></td>
		</tr>
		...
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>  style="font-size:14px; ">
		  ...
		  <td>&nbsp;</td>
		  <td><strong>Total</strong></td>
		  <td><strong><?=number_format($tot_pay_amt,2);?></strong></td>
		  </tr>
...
elseif($_POST['report']==217217)
{
		$report="Import L/C Report";
		
	
		if(isset($loan_group_id)){$loan_group_id_con.=' and l.ledger_group="'.$loan_group_id.'"';}	
		if(isset($warehouse_id)){$warehouse_con.=' and j.warehouse_id="'.$warehouse_id.'"';}
...
if(isset($t_date)) 
		...
		<tr style="font-size:14px; height:22px;">
		<th width="2%" bgcolor="#82D8CF">S/L </th>
		<th width="9%" bgcolor="#82D8CF">PI No </th>
		<th width="13%" bgcolor="#82D8CF">Insurance</th>
		<th width="13%" bgcolor="#82D8CF">Bill Of Entry No</th>
		<th width="7%" bgcolor="#82D8CF">LC No </th>
		<th width="7%" bgcolor="#82D8CF">LC Date </th>
		<th width="6%" bgcolor="#82D8CF">LC Value (USD) </th>
		<th width="7%" bgcolor="#82D8CF">LC Type </th>
		<th width="7%" bgcolor="#82D8CF">LC Expiry Date </th>
		<th width="4%" bgcolor="#82D8CF">Loan Type </th>
		<th width="5%" bgcolor="#82D8CF">Loan Number</th>
		<th width="5%" bgcolor="#82D8CF">Loan Value(USD)</th>
		<th width="5%" bgcolor="#82D8CF">Loan Paid(USD)</th>
		<th width="5%" bgcolor="#82D8CF">Balance(USD)</th>
		<th width="5%" bgcolor="#82D8CF">Loan Paid(BDT)</th>
		<th width="5%" bgcolor="#82D8CF">Loan Start Date</th>
		<th width="8%" bgcolor="#82D8CF">Acceptance Date</th>
		<th width="6%" bgcolor="#82D8CF">Maturity Date</th>
		<th width="19%" bgcolor="#FF3300" style="text-align:center"> Bank Name </th>
		<th width="19%" bgcolor="#FF3300" style="text-align:center"> Complete Status </th>
		 </tr>
		</thead><tbody>
		<? $sl=1;
		 ...
		 $sql = "select lc_no, acceptance_date, maturity_date, ledger_group from lc_ltr_loan where 1  group by lc_no";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $acceptance_date[$info->lc_no]=$info->acceptance_date;
		 $maturity_date[$info->lc_no]=$info->maturity_date;
		 $ledger_group[$info->lc_no]=$info->ledger_group;
		...
		$sql = "select id, lc_type from lc_type where 1";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $lc_type[$info->id]=$info->lc_type;
 
		}
		
		$lsql='select ledger_id,sum(dr_amt) as amt from journal where 1 group by ledger_id';
		$lquery=db_query($lsql);
		while($ldata=mysqli_fetch_object($lquery)){
		
		$lamt[$ldata->ledger_id]=$ldata->amt;
		}
		
	
		
		if($_POST['bank_type']!=''){
		$lc_bank_con .= ' and a.bank_name = "'.$_POST['bank_type'].'" ';
		}
		
		if($_POST['lc_no']!='')
		$lc_con .= ' and a.lc_no = "'.$_POST['lc_no'].'" ';
		
		if($_POST['lc_type']!='')
		$lc_type_con .= ' and a.lc_type = "'.$_POST['lc_type'].'" ';
		

		   //$sql="select  a.* from lc_bank_entry a
		 //where a.lc_issue_date between '".$f_date."' and '".$t_date."'  ".$sub_group_id_con.$lc_type_con.$lc_con.$lc_bank_con." group by a.lc_no order by a.lc_issue_date, a.lc_no";
		      $sql="select l.*,a.lc_no,a.lc_issue_date,a.pi_no,a.insurance_cover_note_no,a.bank_lc_no,a.lc_value,a.lc_type,a.lc_expiry_date,a.bank_name from lc_ltr_loan l,lc_bank_entry a where l.lc_no=a.lc_no  and a.lc_issue_date between '".$f_date."' and '".$t_date."'  ".$loan_group_id_con.$sub_group_id_con.$lc_type_con.$lc_con.$lc_bank_con." ";
		...
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>  style="font-size:12px; ">
		<td><?=$sl++;?></td>
		<td><?=$row->pi_no; ?></td>
		<td><?=$row->insurance_cover_note_no; ?></td>
			<td><?=find_a_field('lc_bill_payment','boe_no','lc_no='.$row->lc_no); ?></td>
		<td><?php /*?><a href="../../../../AccERP/acc_mod/pages/customer_provision_jv_print_view.php?jv_no=<?=$jv_no[$row->payment_no]?>" target="_blank"><?=$row->payment_no;?> </a><?php */?> <?=$row->bank_lc_no;?> </td>
		<td><?php echo date('d-M-Y',strtotime($row->lc_issue_date));?></td>
		<td><?=number_format($row->lc_value,2); $tot_lc_value+=$row->lc_value;?></td>
		<td><?=$lc_type[$row->lc_type];?></td>
		<td><?php echo date('d-M-Y',strtotime($row->lc_expiry_date));?></td>
		<td><?=$group_name[$ledger_group[$row->lc_no]];?></td>
		<td>
		<?php echo $row->ltr_number;?>
		
		</td>
		<td><?php echo $row->loan_usd_value;?></td>
		<td><?=$row->loan_paid_usd?></td>
		<td><?=$tbalance=$row->loan_usd_value-$row->loan_paid_usd?></td>
		<td><?=$lamt[$row->ledger_id]?></td>
		<td><?php echo $row->loan_start_date;?></td>
		<td><?=$acceptance_date[$row->lc_no] ?></td>
		<td><?=$maturity_date[$row->lc_no] ?></td>
		<td><?=$row->bank_name; ?></td>
		<td><?php if($row->complete_status=="Yes"){echo "Complete";}else{echo "Incompete";} ?></td>
		</tr>
		
		<? 
		$gr_tot_usd+=$row->loan_usd_value;
		$totLoanPaid+=$row->loan_paid_usd;
		$totbalance+=$tbalance;
		$totbdt+=$lamt[$row->ledger_id];
		...
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>  style="font-size:14px; ">
		  ...
		  <td><strong>Total</strong></td>
		  <td><strong>
		    <?=number_format($tot_lc_value,2);?>
		  </strong></td>
		  <td></td>
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
		  <td><strong><?php echo number_format($gr_tot_usd,2);?></strong></td>
		  <td><strong><?=number_format($totLoanPaid,2)?></strong></td>
		  <td><strong><?=number_format($totbalance,2)?></strong></td>
		   <td><strong><?=number_format($totbdt,2)?></strong></td>
		   	   <td>&nbsp;</td>
			   <td>&nbsp;</td>
		  <td>&nbsp;</td>
		   <td>&nbsp;</td>
...
if(isset($t_date)) 
		...
		<th width="7%" bgcolor="#82D8CF">LC Expiry Date </th>
		<th width="4%" bgcolor="#82D8CF">Loan Type </th>
		<th width="5%" bgcolor="#82D8CF">Loan</th>
		<th width="8%" bgcolor="#82D8CF">Acceptance Date</th>
		<th width="6%" bgcolor="#82D8CF">Maturity Date</th>
		<th width="19%" bgcolor="#FF3300" style="text-align:center"> Bank Name </th>
		 </tr>
		...
		<td><?=$row->pi_no; ?></td>
		<td><?=$row->insurance_cover_note_no; ?></td>
		<td><?=find_a_field('lc_bill_payment','boe_no','boe_no!="" and lc_no="'.$row->lc_no.'"'); ?></td>
		<td><?php /*?><a href="../../../../AccERP/acc_mod/pages/customer_provision_jv_print_view.php?jv_no=<?=$jv_no[$row->payment_no]?>" target="_blank"><?=$row->payment_no;?> </a><?php */?> <?=$row->bank_lc_no;?> </td>
		<td><?php echo date('d-M-Y',strtotime($row->lc_issue_date));?></td>
		<td><?=number_format($row->lc_value,2); $tot_lc_value+=$row->lc_value;?></td>
		<td><?=$lc_type[$row->lc_type];?></td>
		...
		<!--<table class="table table-bordered">
			...
			<tbody>
			<?php 
				$sql='select * from  lc_ltr_loan where lc_no="'.$row->lc_no.'"';
				$query=db_query($sql);
				while($l_row=mysqli_fetch_object($query)){
			?>
				<tr>
					<td><?php echo $l_row->ltr_number;?></td>
					<td><?php echo find_a_field('secondary_journal','cr_amt','ledger_id='.$l_row->ledger_id);?></td>
				</tr>
				<?php } ?>
			</tbody>
		</table>-->
