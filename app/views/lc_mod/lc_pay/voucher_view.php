...
$tr_no_get=$_GET['payment_no'];
$jv_no_get=find_a_field('secondary_journal','jv_no','tr_no="'.$tr_no_get.'"');
 $jv_no= $jv_no_get;
...
<a target="_blank" href="chalan_view2.php?v_no=<?=$pr->po_no?>"></a></div><?  }?>
      ...
      <tr>
        <td class="tabledesign_text"> <strong>Voucher No  :</strong>
			<?php 
			if($jv->manual_voucher_no>0){ echo $jv->manual_voucher_no; } else { echo $jv_no;} ?> </span></td>
...
$sub_ledger = find_a_field('general_sub_ledger','sub_ledger_name','sub_ledger_id="'.$info->sub_ledger.'"');	  
    ...
    <td align="right"><? 
		if($info->dr_amt>0){
		echo number_format($info->dr_amt,2); $ttd = $ttd + $info->dr_amt; } ?></td>
        <td align="right"><? 
			if($info->cr_amt>0){
		echo number_format($info->cr_amt,2); $ttc = $ttc + $info->cr_amt; } ?></td>
        </tr>
<?php }?>
<?
