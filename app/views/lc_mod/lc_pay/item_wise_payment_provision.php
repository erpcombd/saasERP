    ...
    $sqlc = 'select d.*, i.item_name from lc_purchase_master m, lc_purchase_invoice d, item_info i where m.po_no=d.po_no and i.item_id=d.item_id and m.lc_no='.$_POST['lc_no'].'
			...
			while($datac = mysqli_fetch_object($queryc)){
					  ...
					  $assesment_value=$_POST['assesment_value_'.$datac->item_id];
						...
						//$bill_type=$_POST['bill_type_'.$datac->item_id.'_'.$data->id];
					  if($payment_amt>0){
					    $insql='insert into item_wise_payment_provision(payment_id,item_id,lc_no,bill_category,payment_amt,payment_date)values("'.$payment_no.'","'.$datac->item_id.'","'.$lc_no.'","'.$bill_category.'","'.$payment_amt.'","'.$payment_date.'")';
					db_query($insql);
					
					    $ins_invoice = 'INSERT INTO lc_bill_payment_provision (payment_no, payment_date, payment_method, lc_no, lc_number, lc_ledger, bank_pay_id, lc_part, lc_part_no, group_for, bill_type, bill_category, category_view, cr_ledger_id, pay_amt_in, pay_amt_out, remarks, note, r_no, r_date, status, entry_by, entry_at, boe_no, boe_date,assesment_value)
