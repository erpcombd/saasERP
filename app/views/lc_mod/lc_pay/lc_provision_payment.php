...

{
		...
		else if($_POST['cr_ledger_id']>0){
		$ledger_id_cus=$_POST['cr_ledger_id'];
		  $sub_ledger_id=0;
		  	$cr_ledger_id=$ledger_id_cus;
		}
		else {
		//$main_ledger[$_POST['cr_sub_ledger_id']];
		$ledger_id_cus=$_POST['cr_sub_ledger_id'];
		  $sub_ledger_id=0;
		  	$cr_ledger_id=$ledger_id_cus;
		}
...
$dr_ledger=$ledger_id;
      ...
      $ins_invoice = 'INSERT INTO lc_bill_payment_provision (payment_no, payment_date, lc_no, lc_number, lc_ledger, bank_pay_id, lc_part_no, group_for, bill_type, bill_category, category_view, cr_ledger_id, pay_amt_in, pay_amt_out, remarks, note, r_no, r_date, status, entry_by, entry_at, boe_no, boe_date,sub_ledger_id,dr_sub_ledger_id,exchange_rate,loan_ledger)
  
  VALUES("'.$payment_no.'", "'.$payment_date.'", "'.$lc_no.'", "'.$lc_number.'", "'.$dr_ledger.'", "'.$bank_pay_id.'", "'.$lc_part_no.'", "'.$group_for.'",  "'.$bill_type.'", "'.$bill_category.'", "'.$data->category_view.'", "'.$cr_ledger_id.'", "'.$payment_amt.'", "0", "'.$remarks.'", "'.$note.'", "'.$r_no.'", "'.$r_date.'", "CHECKED", "'.$entry_by.'", "'.$entry_at.'", "'.$boe_no.'", "'.$boe_date.'","'.$sub_ledger_id.'","'.$_POST['dr_sub_ledger_id'].'","'.$_POST['exchange_rate'].'","'.$sub_ledger_id.'")';
...
$sql = "SELECT id, bill_type, bill_category FROM lc_bill_category WHERE 1 ".$bill_type_con." and bill_type!=8 order  by bill_type,id ";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
?>
...
<? if(isset($_POST['submit'])){ ?>
									...
									?>
<label>L/C REFERENCE NO  :</label>
<input name="lc_number" type="text" id="lc_number"  readonly="" style="width:220px; height:32px;" value="<?=$lc_data->lc_number;?>"  required tabindex="105" />	
	...
	<label>COMPANY :</label>								
									...
									<label> COMPANY : </label>
 <input name="company" type="text" id="company"  readonly="" style="width:220px; height:32px;" value="<?=find_a_field('user_group','group_name','id='.$lc_data->group_for); ?>"   tabindex="105" />
