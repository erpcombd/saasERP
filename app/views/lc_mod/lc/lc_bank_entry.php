...
createSubLedger($custome_code,$_POST['ledger_name'],'LC',$lc_no_ins_id,$_POST['account_code'] ,'');
		...
		$tr_no = next_transection_no($group_for,$lc_issue_date,'lc_bank_entry','tr_no');
	 	     ...
	 	     $ins_invoice = 'INSERT INTO lc_bank_entry (tr_no, lc_issue_date, pi_reference_no, lc_no, lc_number, po_no, group_for, bank_lc_no, pi_no, lc_expiry_date, tolerance, insurance_cover_note_no, shipment_date, shipment_expiry_date, lc_af_no, bank_name, lc_type, lc_value,ud_amount, mode_of_transport, cover_note_date, ammendment_date, remarks, status, entry_at, entry_by,tenor_days,currency)
  
  VALUES("'.$tr_no.'", "'.$lc_issue_date.'",  "'.$pi_reference.'", "'.$lc_no_ins_id.'", "'.$_POST['ledger_name'].'", "'.$po_no.'", "'.$group_for.'", "'.$bank_lc_no.'", "'.$pi_no.'", "'.$lc_expiry_date.'", "'.$tolerance.'", "'.$insurance_cover_note_no.'", "'.$shipment_date.'", "'.$shipment_expiry_date.'", "'.$lc_af_no.'", "'.$bank_name.'", "'.$lc_type.'", "'.$lc_value.'","'.$ud_amount.'", "'.$mode_of_transport.'", "'.$cover_note_date.'", "'.$ammendment_date.'", "'.$remarks.'", "CHECKED", "'.$entry_at.'", "'.$entry_by.'","'.$tenor_days.'","'.$currency.'")';
...
db_query($ins_invoice);
 ...
 db_query($up_sql);
		  		...
		  		$link = "status.php?po_no=" .$po_no;
$redirect = "pending_pi_for_lc.php"; // page you want current tab to go
...
<div class="box" style="width:100%;">
	...
	//echo 'select * from lc_pi_reference_setup where id="'.$pi_reference_no.'"';
									?>
		  	...
		  	<tr>
				...
				<td><input name="company" type="text" id="company"  readonly="" style="width:220px; height:32px;" value="<?=find_a_field('user_group','group_name','id='.$po_data->group_for); ?>"   tabindex="105" />	</td>
								...
								<tr>
								 ...
								 <td>PI value (<?php echo $currency_name[$po_data->currency];?>):</td>
									...
									<td>UD Amount (<?php echo $currency_name[$po_data->currency];?>):</td>
									<td>
         ...
         <? foreign_relation('currency_type','id','currency_type',$currency, 'id="'.$po_data->currency.'"');?>
        </select>
		 </td>
