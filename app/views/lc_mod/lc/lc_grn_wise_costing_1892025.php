...
$con .= ' and a.id in ('.$_POST['lc_no'].') ';
 		   ...
 		   $sql = "select payment_id, payment_id as payment_no from lc_purchase_closing where 1 group by payment_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $payment_no[$info->payment_id]=$info->payment_no;
		}



    $res="select b.*, c.pi_no, c.bank_lc_no from lc_number_setup a, lc_bank_payment b, lc_bank_entry c  where a.id=b.lc_no and b.tr_no=c.tr_no ".$con."  group by b.id order by a.id, b.id";
