...
do_calander('#payment_date');
 do_calander('#shipped_on_board_date');
...
if ($data_found==0) {
 create_combobox('pay_id');
	...
	$po_data = find_all_field('lc_purchase_master','*','lc_no='.$pay_id); 
...

{
		...
		$container_wise_no_of_pkgs=$_POST['container_wise_no_of_pkgs'];
...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
  ...
  $ins_invoice = 'INSERT INTO lc_bank_payment (payment_no, payment_date, payment_id, order_no, lc_part, lc_no_part, tr_no, po_no, lc_no, lc_number, ledger_id, group_for, lc_bank_ledger, bank_pay_amt, lc_ltr_ledger, loan_amt, lc_margin_adjustment, remarks, status, entry_at, entry_by,bill_of_loading,booking_no,shipping_line,shipped_on_board_date,container_no,vessel_name,container_type,container_wise_no_of_pkgs,net_weight,gross_weight,measurement)
  
  VALUES("'.$payment_no.'", "'.$payment_date.'", "'.$payment_id.'", "'.$pay_id.'", "'.$lc_part.'", "'.$lc_no_part.'", "'.$lc_bank_data->tr_no.'", "'.$po_no.'", "'.$lc_no.'", "'.$lc_number.'", "'.$ledger_id.'", "'.$group_for.'", "'.$bank_ledger_id.'", "'.$bank_pay_amt.'", "'.$lc_ltr_ledger.'", "'.$loan_amt.'", "'.$lc_margin_adjustment.'", "'.$remarks.'", "CHECKED", "'.$entry_at.'", "'.$entry_by.'", "'.$bill_of_loading.'", "'.$booking_no.'", "'.$shipping_line.'", "'.$shipped_on_board_date.'", "'.$container_no.'", "'.$vessel_name.'", "'.$container_type.'", "'.$container_wise_no_of_pkgs.'", "'.$net_weight.'", "'.$gross_weight.'", "'.$measurement.'")';
...
//}
		$link = "shipment_view.php?payment_no=".$payment_no."&pay_id=".$pay_id;
$redirect = "pending_lc_for_payment.php"; // page you want current tab to go
		  	...
		  	<tr>
				<td><input name="company" type="text" id="company"  readonly="" style="width:220px; height:32px;" value="<?=find_a_field('user_group','group_name','id='.$pay_data->group_for); ?>"   tabindex="105" />	</td>
								 ...
								 <td>PI  NO:</td>
									...
									<td>L/C VALUE (<?php echo $currency_name[$po_data->currency];?>):</td>
									<td>
				...
				<input name="lc_value" type="text" id="lc_value"  style="width:220px; height:32px;" readonly=""  value="<?=$pay_data->lc_value?>"  required tabindex="105" />									</td>
								  ...
								  <tr>
									...
									<td><input name="bill_of_loading" type="text" id="bill_of_loading"  style="width:220px; height:32px;"     />	</td>
									<td>Booking No</td>
									<td><input name="booking_no" type="text" id="booking_no"  style="width:220px; height:32px;"     /></td>
										<td>Shipping Line</td>
									<td><select name="shipping_line" id="shipping_line">
									<option></option>
									<? foreign_relation('shipping_line','id','shipping_line_name',$shipping_line,'1');?>
									</select>	</td>
								  </tr>
								  	  <tr>
								  	<td> Shipped On Board Date</td>
									<td><input name="shipped_on_board_date" type="text" id="shipped_on_board_date"  style="width:220px; height:32px;"     />	</td>
									<td>Container No</td>
									<td><input name="container_no" type="text" id="container_no"  style="width:220px; height:32px;"     /></td>
										<td>Vessel Name</td>
									<td><input name="vessel_name" type="text" id="vessel_name"  style="width:220px; height:32px;"     /></td>
								  </tr>
								  		  <tr>
									...
									<option></option>
									<? foreign_relation('container_type','id','container_type',$container_type,'1');?>
									</select>	
									 </td>
									<td>Container Wise No.Of PKGS</td>
									<td><input name="container_wise_no_of_pkgs" type="text" id="container_wise_no_of_pkgs"  style="width:220px; height:32px;"     /></td>
										<td>Net Weight</td>
									<td><input name="net_weight" type="text" id="net_weight"  style="width:220px; height:32px;"     /></td>
								  </tr>
								    	  <tr>
								  	<td> Gross Weight</td>
									<td><input name="gross_weight" type="text" id="gross_weight"  style="width:220px; height:32px;"     />	</td>
									<td>Measurement</td>
									<td><input name="measurement" type="text" id="measurement"  style="width:220px; height:32px;"     /></td>
