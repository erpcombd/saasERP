...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
			...
			$invoice_no = next_transection_no($group_for,$invoice_date,'foreign_currency_purchase','invoice_no');
   ...
   $ins_invoice = 'INSERT INTO foreign_currency_purchase (invoice_no, invoice_date, group_for, transaction_type, cheque_no, cheque_date, reference_by, remarks, cr_ledger_id, ledger_id, purchase_amt, exchange_rate, total_amt_bdt, entry_by, entry_at)
  
  VALUES("'.$invoice_no.'", "'.$invoice_date.'", "'.$group_for.'", "'.$transaction_type.'", "'.$cheque_no.'", "'.$cheque_date.'", "'.$reference_by.'", "'.$remarks.'", "'.$cr_ledger_id.'", "'.$ledger_id.'", "'.$purchase_amt.'", "'.$exchange_rate.'", "'.$total_amt_bdt.'", "'.$entry_by.'", "'.$entry_at.'")';
...
$tot_stock_amt[$data->item_id]=$data->tot_stock_amt;
  ...
  echo $sql = "select item_id, sum(item_in-item_ex) as stock_qty from journal_item where ji_date<='".$_POST['issue_date']."'  group by item_id";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
<input name="item_id_<?=$data->item_id?>" id="item_id_<?=$data->item_id?>" type="hidden"   value="<?=$data->item_id?>"  onkeyup="EXPcalculation(<?=$data->item_id?>)" style="width:120px; height:25px;"  />
 ...
 <input name="issue_qty_<?=$data->item_id?>" id="issue_qty_<?=$data->item_id?>" type="text"  value=""  onkeyup="EXPcalculation(<?=$data->item_id?>)" style="width:120px; height:25px;"  />	</td>
    <td><input name="avg_price_<?=$data->item_id?>" id="avg_price_<?=$data->item_id?>" type="text"  value="<?=$tot_stock_amt[$data->item_id];?>" onkeyup="EXPcalculation(<?=$data->item_id?>)"  style="width:100px; height:25px;"  /></td>
