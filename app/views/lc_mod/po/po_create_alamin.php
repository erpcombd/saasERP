...
$checked_values = isset($_POST['myChec']) ? $_POST['myChec'] : [];
    if (!empty($checked_values)) {
      echo    $po_ids=implode(",", array_map('htmlspecialchars', $checked_values));
    }
  $po_no_get = 	isset($_POST['po_no']) ? $_POST['po_no'] : '' ;
...
<option></option>
	  ...
	  <? $field='supplier_bank_details';?>
		...
		<? }?>
	    	  ...
	    	  <div>
        <label style="width:30%" for="supplier_agent">PO NO:</label>	
       <input  name="po_no" type="text" id="po_no" value="<?=$po_no_get?>" style="width:50%" />	
	  </div>
	  
	      	  <div>
        <label style="width:30%" for="supplier_agent">PO IDS:</label>	
       <input  name="po_ids" type="text" id="po_ids" value="<?=$po_ids?>" style="width:50%" />	
	  </div>
