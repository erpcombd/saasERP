...
if(isset($_POST['confirmm']))
{
		...
		$msg='Successfully Forwarded for Approval.';
	  		...
	  		$link = "status.php?po_no=" .$$unique;
$redirect = "lc_entry_master.php"; // page you want current tab to go
...
<option></option>
	  ...
	  <div>
       ...
       <input  name="trans_of_payment" type="text" id="trans_of_payment" value="<?=$trans_of_payment?>" style="width:50%" />
	  	  ...
	  	  <div>
        <label style="width:30%" for="supplier_agent">PORT OF LOADING:</label>
       <input  name="port_of_loading" type="text" id="port_of_loading" value="<?=$port_of_loading?>" style="width:50%" />	
	  </div>
	  	  	  <div>
        <label style="width:30%" for="supplier_agent">PORT OF DISCHARGE:</label>
       <input  name="port_of_discharge" type="text" id="port_of_discharge" value="<?=$port_of_discharge?>" style="width:50%" />	
	  </div>
	  	  	  <div>
        <label style="width:30%" for="supplier_agent">PLACE OF DELIVERY:</label>
       <input  name="place_of_delivery" type="text" id="place_of_delivery" value="<?=$place_of_delivery?>" style="width:50%" />	
	  </div>
...
<option></option>
	  ...
	  <? $field='supplier_bank_details';?>
		...
		<? }?>
     		...
     		<div>
        <label style="width:30%" for="lc_manual_no">Currency Type:</label>
			<select id="currency" name="currency" required style="width:50%" >
	
         <? foreign_relation('currency_type','id','currency_type',$currency, '1 limit 100 offset 1');?>
        </select>
		</div>
...
<? 
 $group_for = find_a_field('warehouse','group_for','warehouse_id='.$_SESSION['user']['depot'].' ');
...
<? }else{?>
<a href="status_draft.php?po_no=<?=$_SESSION[$unique]?>" target="_blank"><input type="button" class="btn btn-success" value="View PI" /></a>
			<table  width="100%" border="1" align="left"  style="border-collapse:collapse; border:1px solid #caf5a5;" cellpadding="2" cellspacing="2">
                      <tr>
					  <!--<th>Item Group</th>
								<th>Item Sub Group</th>
									<th>Item Sub Sub Group</th>-->
                        <td  align="center" bgcolor="#0099FF"><strong>Item Name 
						
						</strong></td>
                        <td   align="center" bgcolor="#0099FF"><strong>Unit</strong></td>
						  <td   align="center" bgcolor="#0099FF"><strong>Specification</strong></td>
                        <td   align="center" bgcolor="#0099FF"><strong>Quantity</strong></td>
                        <td  align="center" bgcolor="#0099FF"><strong>Rate(<?php echo $currency_name[$currency];?>)</strong></td>
                        <td   align="center" bgcolor="#0099FF"><strong>Amount(<?php echo $currency_name[$currency];?>) </strong></td>
...
<?php /*?><td>
        <select name="group_id_cus" id="group_id_cus"  >
		<?php 
		if($_GET['item_id']>0){
		?>
...
<!--<input  name="item_id" type="text" id="item_id" value="<?=$item_id?>" style="width:90%; height:32px;" required onblur="getData2('po_ajax.php', 'po',this.value,document.getElementById('warehouse_id').value);"/>-->
...
echo "<input list='item_list' id='item_id' name='item_id' placeholder='-- Select Item --' 
       onblur=\"getData2('po_ajax.php', 'po', this.value, document.getElementById('warehouse_id').value);\">";
	  ...
	  <br />
        ...
        <td><div class="tabledesign2">
          <table id="grp" width="100%" cellspacing="0" cellpadding="0"><tbody>
			...
			<th width="8%">Unit  </th>
			<th width="11%" >   Rate(<?php echo $currency_name[$currency];?>) </th>
			<th width="12%" >Amount(<?php echo $currency_name[$currency];?>)</th>
...
$(document).ready(function(){
    ...
    $('#group_id').change(function(){
	    ...
	    // Load item when sub Sub Group is selected
