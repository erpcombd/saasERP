...
$master_data = find_all_field('lc_bill_payment','','payment_no='.$payment_no); 

 $data_found = $payment_no;
...

{
		...
		$del_sql1 = "DELETE from lc_bill_payment where payment_no='".$payment_no."' and lc_no = '".$lc_no."' ";
		db_query($del_sql1);
		
		$del_sql2 = "DELETE from secondary_journal where tr_no='".$payment_no."' and tr_id = '".$lc_no."' and note='LC Journal' ";
		db_query($del_sql2);
		
		$del_sql3 = "DELETE from journal where tr_no='".$payment_no."' and tr_id = '".$lc_no."' and note='LC Journal' ";
		db_query($del_sql3);
 ...
 if($master_data->bill_type!='')
  $bill_type_con=" and bill_type=".$master_data->bill_type;
...
window.location.href = "select_voucher_recheck.php";
</script>
...
<div class="box" style="width:100%;">
			...
			<input style="width:220px; height:32px;"  name="invoice_date" type="text" id="invoice_date"  value="<?=$master_data->payment_date?>"   required />									</td>
									<td width="14%">L/C  NO  :</td>
									<td width="21%">
									
									<?
									 $lc_data = find_all_field('lc_number_setup','','id='.$master_data->lc_no); 
									 $bank_ledger = find_a_field('lc_bank_entry','bank_ledger','lc_no='.$master_data->lc_no); 
									?>
									
									<input name="payment_no" type="hidden" id="payment_no"  readonly="" style="width:220px; height:32px;" value="<?=$payment_no;?>"  required tabindex="105" />
									
					<input name="bill_type" type="hidden" id="bill_type"  readonly="" style="width:220px; height:32px;" value="<?=$master_data->bill_type;?>"  required tabindex="105" />
					...
					<? if ($master_data->bill_type==1) {?>
								 ...
								 <td>&nbsp;</td>
									...
									<select name="cr_ledger_id" id="cr_ledger_id" required="required" style="width:220px;">
									  <option></option>
									  <? foreign_relation('accounts_ledger','ledger_id','ledger_name',$master_data->cr_ledger_id,'1');?>
									</select>
							...
							<? } elseif ($master_data->bill_type==2 || $master_data->bill_type==8) {?>	  
	...
	<select name="payment_method" id="payment_method" required style="width:220px;" >
										...
										<? foreign_relation('payment_method','payment_method','payment_method',$master_data->payment_method,'1 order by payment_method');?>
									</select>									</td>
									<td>CASH/BANK:</td>
									<td>
									...
									<select name="cr_ledger_id" id="cr_ledger_id" required="required" style="width:220px;">
									  <option></option>
									  <? foreign_relation('accounts_ledger','ledger_id','ledger_name',$master_data->cr_ledger_id,'ledger_group_id in (1009,1011)');?>
									</select>
									</span>
									</td>
									<td>REMARKS:</td>
						...
						<? }  elseif ($master_data->bill_type==3) {?>
								  ...
								  <tr>
									...
									<option></option>
										...
										<? foreign_relation('payment_method','payment_method','payment_method',$master_data->payment_method,'1');?>
									</select>									</td>
									<td>CASH/BANK:</td>
									<td>
									...
									<select name="lc_part" id="lc_part" required="required" style="width:220px;">
									  <option></option>
									  <? foreign_relation('lc_part','id','lc_part',$master_data->bank_pay_id,'1');?>
									</select>
									</td>
				 </tr>
				 ...
				 <tr>
								 ...
								 <td>&nbsp;</td>
									...
									<option></option>
									  ...
									  <? foreign_relation('lc_ltr_loan','ledger_id','ltr_number',$master_data->cr_ledger_id,'lc_no="'.$master_data->lc_no.'"');?>
									</select>
						...
						<? } elseif ($master_data->bill_type==4) { ?>
						...
						<? } elseif ($master_data->bill_type==5) { ?>
						...
						<? } elseif ($master_data->bill_type==6) {?>
...
<tr>
								...
								<td>L/C Part:</td>
								 ...
								 <td>BILL OF ENTRY NO: </td>
									...
									<td>
									<input name="boe_no" type="text" id="boe_no"  style="width:220px; height:32px;" value="<?=$master_data->boe_no?>"   tabindex="105" />
									</td>
									...
									<input name="boe_date" type="text" id="boe_date"  style="width:220px; height:32px;" value="<?=$master_data->boe_date?>"   tabindex="105" />
						...
						<tr>
								 ...
								 <td>C&amp;F COMPANY:</td>
									...
									<select name="cr_ledger_id" id="cr_ledger_id" required="required" style="width:220px;">
									  <option></option>
									  <? foreign_relation('accounts_ledger','ledger_id','ledger_name',$master_data->cr_ledger_id,'under_ledger=2018000800000000');?>
									</select>
									</td>
									<td>R. NO: </td>
									<td>
							
									<input name="r_no" type="text" id="r_no"  style="width:220px; height:32px;" value="<?=$master_data->r_no?>"   tabindex="105" />
									
									</td>
									<td>R. DATE :</td>
									<td><input name="r_date" type="text" id="r_date"  style="width:220px; height:32px;" value="<?=$master_data->r_date?>"   tabindex="105" />									</td>
								  </tr>
...
//}
 ...
 $sql = "select sum(pay_amt_in) as pay_amt, bill_category  from lc_bill_payment where payment_no='".$payment_no."'  group by bill_category";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
$pay_amt[$data->bill_category]=$data->pay_amt;
}
...
<? //if ($due_amt=$data->invoice_amt-$payment_amt[$data->ledger_id][$data->tr_no]+$return_amt[$data->ledger_id][$data->tr_no]>0) {?>
 ...
 <input name="payment_amt_<?=$data->id?>" id="payment_amt_<?=$data->id?>" type="text" size="10"  value="<?=$pay_amt[$data->id]; $tot_pay_amt +=$pay_amt[$data->id];?>" onkeyup="SUMcalculation(<?=$data->id?>)"  style="width:120px; height:25px;"  />	</td>
  ...
  <tr>
    ...
    <td><input name="total_amt" id="total_amt" type="text" size="10"  value="<?=$tot_pay_amt?>" style="width:120px; height:25px;"  />	</td>
    <td align="center">&nbsp;</td>
  </tr>
