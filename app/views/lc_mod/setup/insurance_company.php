...
$title='Insurance Company';			// Page Name and Page Title

do_datatable('table_head');

$page="insurance_company.php";		// PHP File Name



$table='insurance_company';		// Database Table Name Mainly related to this page
...
//$custome_codes = find_a_field('insurance_company','max(sub_ledger_id)','1');
//if($custome_codes>0){
//	$custome_code = $custome_codes+1;
//	}
//	else{
//	$custome_code = 90000001;
//	}
...
//createSubLedger($custome_code,$_POST['company_name'],'Insurance_Company',$_POST[$unique],$_POST['account_code'] ,0);
//}
...
$acc_ins_led = 'INSERT INTO accounts_ledger (ledger_sl, ledger_name, ledger_group_id,  balance_type,  proj_id,  acc_class, acc_sub_class, acc_sub_sub_class, group_for, entry_by, entry_at)
  ...
  VALUES("'.$_POST['ledger_sl'].'", "'.$_POST['ledger_name'].'", "'.$_POST['ledger_group_id'].'", "Both", "'.$proj_id.'",  "'.$gl_group->acc_class.'", "'.$gl_group->acc_sub_class.'", "'.$gl_group->acc_sub_sub_class.'", "'.$gl_group->group_for.'", "'.$_POST['entry_by'].'", "'.$_POST['entry_at'].'")';
...

{
	  ...
	  $sql1 = 'update general_sub_ledger set sub_ledger_name="'.$_POST['company_name'].'" 
                                     ...
                                     <td><span class="style1" style="padding-top:5px;">*</span>GL Configuration:</td>
										...
										<? if ($ledger_id==0) {?>
											<select name="ledger_group" required="required" id="ledger_group" style="width:250px; font-size:12px;" tabindex="9">
											  ...
											  <option></option>
                                              <? foreign_relation('ledger_group','group_id','group_name',$ledger_group,'group_id=123001'); //acc_sub_class=203?>
											  <? // foreign_relation('accounts_ledger','ledger_id','ledger_name',$ledger_id,'ledger_id="'.$config_all->lc_insurance_lg.'" and group_for="'.$_SESSION['user']['group'].'"');?>
                                            </select>
											<? }?>
