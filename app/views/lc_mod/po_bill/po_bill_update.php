...
var strURL="po_bill_update_ajax.php?po_no="+po_no+"&bill_id="+bill_id+"&bill_no="+bill_no+"&bill_date="+bill_date+"&purchase_manager="+purchase_manager+"&vendor_id="+vendor_id+"&ledger_id="+ledger_id+"&jv_no="+jv_no+"&flag="+flag;
...
<div class="box" style="width:100%;">
								...
								<input name="bill_no" type="text" id="bill_no" style="width:200px; height:32px;" value="<?=$bill_data->bill_no?>" readonly="" tabindex="105" />									</td>
								 ...
								 <td> Bill Date:</td>
									...
									<input style="width:220px; height:32px;"  name="po_date" type="text" id="po_date"  value="<?=$_POST['po_date'];?>"  />																		</td>
								  </tr>
								</table>
...
//		$do_con .= ' and m.do_no in ('.$_POST['do_no'].') ';
   ...
   from purchase_sp_master m, vendor v 
   where m.vendor_id=v.vendor_id and m.purchase_manager='".$bill_data->purchase_manager."' and m.po_date > '2022-03-31' and m.status!='MANUAL' ".$po_dt_con." group by m.po_no  order by m.po_date, m.po_no";
...
<? if($po_no[$data->po_no]==0) {  ?>
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
	...
	<input name="jv_no_<?=$data->po_no?>" type="hidden" id="jv_no_<?=$data->po_no?>" value="<?= $jv_no[$data->ledger_id][$data->po_no];?>" />
	</td>
    <td>
    ...
    <td><?= number_format($cr_amt[$data->ledger_id][$data->po_no],2);?></td>
    <td>
	<? if($bill_data->status!="COMPLETED") {?>
	<span id="divi_<?=$data->po_no?>">
	<input name="flag_<?=$data->po_no?>" type="hidden" id="flag_<?=$data->po_no?>" value="0" />

	<input type="button" name="Button" value="Approve"  onclick="update_value(<?=$data->po_no?>)" style="width:70px; font-size:12px; height:30px; background-color:#66CC66; font-weight:700;"/>
