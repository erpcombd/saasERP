...
var ledger_problem=(document.getElementById('ledger_problem_'+order_no).value);
var ledger_id=(document.getElementById('ledger_id_'+order_no).value);
var flag=(document.getElementById('flag_'+order_no).value); 

var strURL="item_revise_ajax.php?order_no="+order_no+"&ledger_problem="+ledger_problem+"&ledger_id="+ledger_id+"&flag="+flag;
...
<div class="box" style="width:100%;">
    ...
    <td width="22%" align="right" ><strong> LC No: </strong></td>
	 <td width="40%" >
	 <select name="lc_no"  id="lc_no" style="width:90%; height:32px;"  tabindex="2">
	 
	 <option></option>
			

					 <? foreign_relation('lc_number_setup l, lc_ltr_loan t','t.lc_no','l.lc_number',$_POST['lc_no'],'l.id=t.lc_no and t.ledger_problem>0 group by t.lc_no');?>
			</select>
...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0" style="font-size:12px; text-transform:uppercase;">
  ...
  <tr>
    ...
    <th width="24%">Loan No </th>
    <th width="10%">Loan Ledger </th>
    <th width="10%">Loan Ledger </th>
...
//		$do_con .= ' and m.do_no in ('.$_POST['do_no'].') ';
    ...
    if($_POST['lc_no']!='')
	$lc_no_con=" and t.lc_no='".$_POST['lc_no']."'";
...
//		}
    ...
    $sqlfg = "select t.*, l.lc_number from lc_ltr_loan t, lc_number_setup l where l.id=t.lc_no and t.ledger_problem>0 ".$lc_no_con." group by t.id ";
...
<? //if($po_no[$data->po_no]==0) { } ?>
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
    ...
    <td> <?=$datafg->ltr_number?> </td>
    <td><?=$datafg->ledger_problem?></td>
    <td bgcolor="#99CCFF">
	<input name="ledger_problem__<?=$datafg->id?>" type="hidden" id="ledger_problem_<?=$datafg->id?>" value="<?=$datafg->ledger_problem; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:150px; height:30px; " />
	<input name="ledger_id_<?=$datafg->id?>" type="text" id="ledger_id_<?=$datafg->id?>" value="<?=$datafg->ledger_id; ?>" onkeyup="TRcalculationpo(<?=$data->id?>)" style="width:150px; height:30px; " />
	<input name="order_no_<?=$datafg->id?>" type="hidden" id="order_no_<?=$datafg->id?>" value="<?=$datafg->id?>" />      </td>
    <td>
