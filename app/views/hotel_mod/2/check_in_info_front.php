...
if($_REQUEST['del_room'])
{
	...
	mysql_query($sql);
	$sql="update hms_hotel_room set status='2' , status_detail='Pertial Check Out' where id='".$room_id."'";mysql_query($sql);
}
...
//$sql="select distinct b.id,b.room_no, c.room_type ,a.checked_in,a.checked_out from hms_hotel_room_status a,hms_hotel_room b,hms_room_type c where a.reserve_id=$reserve_id and a.room_id=b.id and b.room_type_id=c.id";
	$query=mysql_query($sql);
	$room_select = mysql_num_rows($query);
	while($data=mysql_fetch_row($query)){
...
?>
  ...
  <div align="center">
    ...
    <input name="" type="text" value="<?=$due?>" style="width:250px; height:40px; font-size:18px;" />
  </div>
<? }?>