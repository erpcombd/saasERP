...
{
		$sql="update `hms_reservation` set checked_in='$now',check_in_by='$user_id' where id='$reserve_id'";
		db_query($sql);
...
if($_REQUEST['check_out']==1)
{
		...
		db_query($sql);
		echo '<script type="text/javascript">
parent.parent.document.location.href = "../transaction/hotel_status_front.php";
</script>';
}
...
if($_REQUEST['del_room'])
{
	...
	db_query($sql);
	$sql="update hms_hotel_room set status='2' , status_detail='Pertial Check Out' where id='".$room_id."'";db_query($sql);
}

if($_REQUEST['pertial_checkout'])
{

	$room_id=$_REQUEST['room_id'];
	$reserve_id=$_REQUEST['reserve_id'];
	$sql="update hms_hotel_room set status='2' , status_detail='Pertial Check Out' where id='".$room_id."'";
db_query($sql);
}
...
if($_REQUEST['card_in'])
{
	...
	$check_in=str_replace('-','',$_REQUEST['check_in']).'0600';
	$check_out=(str_replace('-','',$_REQUEST['check_out'])+1).'1200';
	$file = 'c:/PMTCPClient/card.txt';

	$current='00000I|R'.$room_no.'|T04|D'.$check_in.'|O'.$check_out;
	file_put_contents($file, $current);
	//shell_exec ("C:/PMTCPClient/card.exe");
}
if($_REQUEST['card_out'])
{
$file = 'c:/PMTCPClient/card.txt';
$current='00000B';
file_put_contents($file, $current);
...
if(isset($_POST['room_add']))
{
	...
	$checked_in=$_POST['check_in'].' '.$_POST['in_time'];
	$checked_out=$_POST['check_out'].' '.$_POST['out_time'];
			$start_date = $check_in;
			$end_date = $check_out;
...
checked_in,
checked_out)
VALUES ('$room_no', '9', '$start_date', '$today', '$reserve_id', '$user_id','$checked_in','$checked_out')";
db_query($sql);
				$start_date = date ("Y-m-d", strtotime("+1 day", strtotime($start_date)));
			}
	...
	if(isset($_POST['client']))
	{
			...
			$_POST['create_on'] = $_POST['checked_in'] = $today;
			$_POST['user_id']	= $_SESSION['user']['id'];
			
	if($reserve_id>0)
	{
			$_POST['checked_in'] = strtotime('now');
			$_POST['id'] = $reserve_id;
			$crud->update('id');
	}
	else
...
<table width="98%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#3f590a" bgcolor="#c7eb8e" style="border:1px solid #3f590a; border-collapse:collapse; font-family:Verdana, Arial, Helvetica, sans-serif; font-size:11px; color:#000000;">
  <tr bgcolor="#679435">
    <td colspan="2" align="center" style="padding:3px; font-weight:bold;  font-size:13px; color:#FFFFFF;"><span style="padding:3px;">Guest</span>  Information </td>
  </tr>
  <tr bgcolor="#f8fbc1">
    <td width="39%" align="right" style="padding:3px;">Guest Name : </td>
...
<table width="98%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#3f590a" bgcolor="#c7eb8e" style="border:1px solid #3f590a; border-collapse:collapse; font-family:Verdana, Arial, Helvetica, sans-serif; font-size:11px; color:#000000;">
  ...
  <tr bgcolor="#f8fbc1">
    ...
    <td width="50%" align="left" style="padding:3px;"><select name="room_no" id="room_no">
      <? 
			  $sql="SELECT a.id,concat(a.room_no,' : ',b.room_type) FROM `hms_hotel_room` a,`hms_room_type` b WHERE b.id=a.room_type_id and a.status not in (0) order by a.room_no";
...
<br>
      <table width="98%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#3f590a" bgcolor="#c7eb8e" style="border:1px solid #3f590a; border-collapse:collapse; font-family:Verdana, Arial, Helvetica, sans-serif; font-size:11px; color:#000000;">
        <tr bgcolor="#679435">
          ...
          <td width="6%" align="center" style="padding:3px; font-weight:bold;  font-size:13px; color:#FFFFFF;"><p>Write</p>            </td>
          <td width="6%" align="center" style="padding:3px; font-weight:bold;  font-size:13px; color:#FFFFFF;">Clear</td>
          <td width="7%" align="center" style="padding:3px; font-weight:bold;  font-size:13px; color:#FFFFFF;">Room ID </td>
          <td width="25%" align="center" style="padding:3px; font-weight:bold;  font-size:13px; color:#FFFFFF;">Room Type </td>
          <td width="25%" align="center" style="padding:3px; font-weight:bold;  font-size:13px; color:#FFFFFF;">Day IN </td>
          <td width="25%" align="center" style="padding:3px; font-weight:bold;  font-size:13px; color:#FFFFFF;">Day OUT </td>
          <td width="25%" align="center" style="padding:3px; font-weight:bold;  font-size:13px; color:#FFFFFF;">OUT </td>
          <td width="6%" align="center" style="padding:3px; font-weight:bold;  font-size:13px; color:#FFFFFF;">Reject</td>
        </tr>
...
//$sql="select distinct b.id,b.room_no, c.room_type ,a.checked_in,a.checked_out from hms_hotel_room_status a,hms_hotel_room b,hms_room_type c where a.reserve_id=$reserve_id and a.room_id=b.id and b.room_type_id=c.id";
	$query=db_query($sql);
...
?>
        <tr bgcolor="#f8fbc1">
          <td align="left"><div align="center"><a href="?id=<?=$room_id?>&amp;date=<?=$date?>&amp;card_in=1&amp;room_no=<?=$data[1]?>&amp;check_in=<?=$data[3]?>&amp;check_out=<?=$data[4]?>&amp;room_id=<?=$data[0]?>&amp;reserve_id=<?=$reserve_id?>"><img src="../../images/w.jpg" width="16" height="16" /></a></div></td>
          <td align="left"><div align="center"><a href="?id=<?=$room_id?>&amp;date=<?=$date?>&amp;card_out=1&amp;room_id=<?=$data[0]?>&amp;reserve_id=<?=$reserve_id?>"><img src="../../images/c.jpg" width="16" height="16" /></a></div></td>
          ...
          <td align="left">
          <? if($room_select >1){?>
          <div align="center"><a href="?id=<?=$room_id?>&amp;date=<?=$date?>&amp;pertial_checkout=1&amp;room_id=<?=$data[0]?>&amp;reserve_id=<?=$reserve_id?>"><img src="../../images/del.png" width="16" height="16" /></a></div>
          <? }?>
          </td>
<td align="left">
<? if($room_select >1){?>
<div align="center"><a href="?id=<?=$room_id?>&date=<?=$date?>&del_room=1&room_id=<?=$data[0]?>&reserve_id=<?=$reserve_id?>"><img src="../../images/del.png" width="16" height="16" /></a></div>
<? }?>
</td>
        </tr>
<? }?>  
...
<?
$sql="SELECT a.id,a.service_name FROM `hms_services` a WHERE id in (3,4,5,8)";
advance_foreign_relation($sql);
?>
</select>
                <label>
                <input type="hidden" name="bill_id" value="" />
                <input type="hidden" name="service_room_no" value="<?=$service_room_no?>" />
              </label></td>
