...
$ddd=@mysql_fetch_row(mysql_query('SELECT sum(`paid_amt`) FROM `hms_bill_payment` WHERE `reserve_id`='.$reserve_no.' and `service_group_id`=5'));
$total_discount = $ddd[0];
$total_paid=$total_paid+$info->paid_amt;
$grand_total=$total_paid+$info->paid_amt;
}?>
...
<table width="700" border="0" cellspacing="0" cellpadding="0" align="center">
	  ...
	  <tr>
	    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
	      <tr>
			...
			if(is_file($path)) echo '<img src="'.$path.'" height="95" width="200" />';?>
	          <br /></td>
	        <td valign="bottom"></td>
	        <td align="right" valign="bottom"><p><span style="font-size:44px; font:Tahoma">
  ...
  <tr>
    <td><div class="header2">
	  ...
	  Name of Guest :<strong> <?php echo $reserve_all->client_name;?>.</strong><br>
	  <? if($reserve_all->company_name!=''){?>
	  Company Name : <?php echo $reserve_all->company_name;?><br>
	  <? }?>
	  ...
	  <p>	  Bill No: <?php echo $reserve_all->id;?><br />
	  Bill Date: <?=$date[0]?><br />
...
<table width="100%" class="tabledesign" border="1" bordercolor="#000000" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        <td align="left" valign="top"><? for($i=0;$i<$pi;$i++){?><p><nobr><?=$date[$i]?></nobr></p><? }?></td>
        <td align="center" valign="top"><? for($i=0;$i<$pi;$i++){?><p><nobr><?=$room[$i]?></nobr></p><? }?></td>
        
        <td align="left" valign="top"><? for($i=0;$i<$pi;$i++){?><p><nobr><?=$item[$i]?></nobr></p><? }?></td>
      ...
      <tr>
        <td colspan="8"><div align="right">Advance: </div></td>
        <td align="right"><?php echo number_format($advanced,2);?></td>
        </tr>
      <tr>
        <td colspan="8"><div align="right">Discount:</div></td>
        <td align="right"><?php echo number_format($less,2);?></td>
        </tr>
      <tr>
        <td colspan="8"><div align="right">Grand Total:</div></td>
      ...
      <table width="100%" border="0" bordercolor="#000000" cellspacing="3" cellpadding="3" class="tabledesign1" style="width:700px">
        ...
        <tr>
		<td>In Word(Due): Taka <?=CLASS_Numbertoword::convert($to_pay,'en')?> Only</td>
        </tr>
    </table></td>
  </tr>
  <tr>
