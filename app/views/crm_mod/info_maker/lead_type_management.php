...
$_SESSION['employee_selected'] = find_a_field('user_activity_management','PBI_ID','user_id="'.$_SESSION['user']['id'].'"');
 ...
 if(isset($_POST['insert'])){
     ...
     $crud1->insert();
	 header("Location: lead_type_management.php");
    exit();
 }
...
<nav class="navbar navbar-light bg-light" style="box-shadow: 0 3px #888888;">
  <form class="form-inline">
    ...
    <div style="right;!important;">
                ...
                <a href="lead_type_management.php?update=<?=encrypTS('new')?>" class="btn btn-outline-info btn-sm">Add New <i class="fa fa-plus-square" aria-hidden="true"></i></a>
                <a href="lead_type_management.php" class="btn btn-outline-warning btn-sm">Refresh <i class="fa fa-refresh" aria-hidden="true"></i></a>
        </div>
</nav>
            ...
            <table id="example" class="table">
                ...
                ?>
                    ...
                    <tr>
                        <td><?=$row->id?></td>
                        <td><?=$row->type?></td>
                        <td><a class="btn btn-info btn-sm" href="#"><?=find_a_field('crm_project_lead', 'count(*)', 'lead_type="'.$row->id.'"')?></a></td>
                        <td><?=find_a_field('user_activity_management', 'fname', 'PBI_ID = "'.$row->entry_by.'"')?></td>
                        <td><?=$row->entry_at?></td>
                        <td><a class="btn btn-sm <?php if($row->is_active=='1'){echo 'btn-warning';}else{echo 'btn-danger';}?>" href="lead_type_management.php?update=<?=encrypTS($row->id)?>"><i class="fa-solid fa-pencil"></i></a></td>
                    </tr>
    ...
    <div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        ...
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLongTitle">Lead Type Management</h5>
            ...
            <div class="col-md-12">
                  <table class="table">
                    <tbody>
                      ...
                      <tr>
                        <td>Field Name: </td>
                        <td><input type="text" name="type" class="form-control" value="<?=$datas->type?>" style="border-left:3.5px solid #df5b5b!important;" required></td>
                      </tr>
