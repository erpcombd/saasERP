...
if(isset($_POST['approved'])) {
            ...
            $update_stmt = $conn->prepare("UPDATE bills_details SET  status = ?, approve_by=?, approve_at = ? WHERE bills_id = ?");
            $update_stmt->bind_param("sisi", $status, $approve_by, $approve_at, $id);
...
from bills_details a, personnel_basic_info b,  hrm_od_info c  
where a.status='UNCHECKED' and a.entry_by = b.PBI_ID and a.od_id = c.id";
...
while($data = mysqli_fetch_object($queryd)){
  ...
  ?>
	...
	<a href="tada_details_approval.php?sl=<?=$data->bills_id?>"> <button type="button" class="btn btn-primary"><i class="far fa-eye"></i></button></a>
