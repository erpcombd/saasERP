...
if (isset($_POST['save_leave'])) {
    ...
    $entry_by =$user_id;
  // Ensure $PBI_ID is correctly defined
    $start_date = $_POST['s_date'];
    $s_time = $_POST['start_time'];
    $e_time = $_POST['end_time'];
    $note = isset($_POST['reason']) ? trim($_POST['reason']) : '';
    ...
    foreach ($person_ids as $PBI_ID) {
        ...
        $sql_master = "INSERT INTO hrm_od_info (s_time, e_time, s_date, latitude, longitude, entry_by, entry_at, PBI_ID, project_id, group_od_id , note, od_att_status) 
        VALUES ('$s_time', '$e_time', '$start_date', '$latitude', '$longitude', '$entry_by', '$entry_at', '$PBI_ID', '$project_id', '$new_group_id', '$note','STARTED')";
        ...
        if ($conn->query($sql_master)) {
            ...
            if (count($types) === count($amounts)) {
                foreach ($types as $index => $type) {
                    ...
                    $sql_master_bill = "INSERT INTO bills (emp_code, conveyance_type, amount, od_id, entry_by, entry_at, group_od_id, remarks, conveyance_date) 
                    VALUES ('$PBI_ID', '$type', '$amount', '$od_id', '$user_id', '$entry_at', '$new_group_id', '$note', '$start_date')";
...
$sql_t = "SELECT * FROM hrm_od_info a WHERE PBI_ID = '" . $user_id . "' OR entry_by = '" . $user_id. "'   ORDER BY id DESC LIMIT 1";
    ...
    <input type="text" class="form-control" 
           ...
           readonly required/>
            ...
            <div class="d-flex justify-content-center row">
                <div class="col-6">
                    <input type="submit" name="save_leave" class="btn btn-3d btn-m btn-full mb-2 b-n rounded-xs font-900 shadow-s btn-success w-100">
                </div>
            </div>
        </div>
    </form>
...
// Fetch data from the database
$sql_t = "SELECT * FROM hrm_od_info WHERE PBI_ID='" . $user_id . "' OR entry_by = '" . $user_id . "' ORDER BY id DESC";
$query2 = db_query($sql_t);
...
function showPosition(position) {
	...
	<?php
	$sql_t = "SELECT * FROM hrm_od_info a WHERE PBI_ID='" . $user_id . "' ORDER BY id DESC";
	$query2 = db_query($sql_t);
	$sl= 0;
	while ($data2 = mysqli_fetch_object($query2)) {?>
                    ...
                    if ($('#lead_name').length) {
                        if(data==='' || data == null) {
                          $('#lead_name').html('<option value="0" selected>No Lead</option>');  
                            ...
                            $('#lead_name').html(data + '<option value="0">No Lead</option>');
                        }
