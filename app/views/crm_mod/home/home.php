   ...
   <div class="col-sm-6 pb-2">
            <div class="floatingshadowfahim">
                <div class="card-block">
                    <h6 class="m-b-20 p-2 rounded " style="background-color: #ef476f;">Total Active Lead</h6>
                    <h2 class="text-right" > <span> <i  class="fa-solid fa-users-medical f-left" style="color: #ef476f;"></i></span><span class="textspan"><?=find_a_field('crm_project_lead', 'count(id)', 'status = "1"')?></span></h2>
        ...
        <div class="col-sm-6 pb-2">
            <div class="floatingshadowfahim">
                <div class="card-block">
                    <h6 class="m-b-20 p-2 rounded "  style="background-color: #ffd166;"> Total Generated Lead</h6>
                    <h2 class="text-right"><i class="fa-solid fa-users-gear f-left" style="color: #ffd166;"></i><span class="textspan"><?=find_a_field('crm_project_lead', 'count(id)', '1')?></span></h2>
                </div>
            </div>
        </div>
        ...
        <div class="col-sm-6 pb-2">
            <div class="floatingshadowfahim">
                <div class="card-block">
                    <h6 class="m-b-20 rounded p-2"  style="background-color: #06d6a0;">Running Projects</h6>
                    <h2 class="text-right"><i class="fas fa-chart-pie f-left" style="color: #06d6a0;"></i><span class="textspan"><?=find_a_field('crm_project_lead a, crm_project_org b', 'count(b.id)', 'a.organization=b.id AND a.status="1"')?></span></h2>
                </div>
            </div>
        </div>
        ...
        <div class="col-sm-6 pb-2">
            <div class="floatingshadowfahim">
                <div class="card-block">
                    <h6 class="m-b-20 rounded p-2"  style="background-color: #118ab2;">Total Task</h6>
                    <h2 class="text-right"><i class="fas fa-tasks f-left" style="color: #118ab2;"></i><span class="textspan"><?=find_a_field('crm_task_add_information', 'count(task_id)', '1')?></span></h2>
                </div>
            </div>
        </div>
   </div>
...
<?php
// Assuming $user_id is defined and database connection is established
...
<div class="calendar">
    <div class="cal-header">
        ...
        <p>Red dates are not available</p>
    </div>
    <div class="cal-days">
        <a href="#">SUN</a>
        <a href="#">MON</a>
    ...
    <div class="cal-dates">
        <?php foreach ($calendarDates as $date): ?>
            ...
            if ($date && isset($activitiesByDate[$date])) {
                $statusClass = 'color-green-dark'; 
                $activityDetails = implode(', ', $activitiesByDate[$date]); 
            }
            ?>
            <div class="cal-date-box <?php echo $statusClass; ?>">
                <div class="cal-date-number"><?php echo $day; ?></div>
                <?php if ($activityDetails): ?>
                    <div class="cal-activity-details"><small><?php echo $activityDetails; ?></small></div>
                <?php endif; ?>
            </div>
        <?php endforeach; ?>
...
// Fetch activities from the database
$user_id = $basic->PBI_ID;
$sqlActivities = "SELECT activity_type, date,project_id,assign_person FROM crm_lead_activity WHERE activity_type='Meeting'";
$resultActivities = db_query($sqlActivities);
...
if ($resultActivities) {
    while ($activityRow = mysqli_fetch_object($resultActivities)) {
        ...
        $activitiesByDate[$activityDate][] = $activityRow->activity_type;
		$activitiesProject[$activityDate][] = $activityRow->project_id;
		$activitiesPerson[$activityDate][] = $activityRow->assign_person;
    }
}
...
<div class="calendar">
    <div class="cal-header">
        <h4>Available Metting Schedule Overview</h4>
        <p>Red dates are not available</p>
    </div>
    <div class="cal-days">
        <a href="#">SUN</a>
    ...
    <div class="cal-dates">
        <?php foreach ($calendarDates as $date): ?>
            ...
            if ($date && isset($activitiesByDate[$date])) {
                $statusClass = 'color-green-dark'; 
                $activityDetails = implode(', ', $activitiesByDate[$date]); 
			    $activitiesPersonn = implode(', ', $activitiesPerson[$date]);
				$activitiesProjectt = implode(', ', $activitiesProject[$date]);
            }
            ?>
	...
	$sqlQuery = "SELECT PBI_NAME FROM personnel_basic_info WHERE PBI_ID IN ($activitiesPersonn) ORDER BY `PBI_ID` DESC";
    $result = db_query($sqlQuery);
    if ($result) {
    while ($row = mysqli_fetch_object($result)) { ?> 
	              
	<div class="cal-activity-details">
	<small><span class="badge badge-danger"><?=$row->PBI_NAME;?></span> </small>
...
<div class="col-md-12  m-0 p-0">
    ...
    <table id="example" class="table">
	   ...
	   '<?=$row->contact_person;?>','<?=$row->contact_number;?>','<?=$row->contact_email;?>','<?=$row->latitude?>','<?=$row->longitude?>')"><i class="fa-solid fa-pencil" style="color: #ffffff;" ></i></button>
...
<div class="modal-header">
    ...
    <h5 class="modal-title" id="leadentrymodalLongTitle"><?php if(isset($datas)){echo 'Update';}else{echo 'Create';}?> <?php /*?><?=$CRMleadName?><?php */?> Customer</h5>
  ...
  <h5 class="text-center mb-2"><?php if(isset($datas)){echo 'Update';}else{echo 'New';}?> <?php /*?><?=$CRMleadName?><?php */?> Customer Information</h5>
  </div>
        ...
        <table class="table">
          ...
          <tbody>
			...
			<tr>
              ...
              <td>  
                ...
                <select name="country" id="country" class=" input_required" onChange="ccl()"  >
                  ...
                  <option value=" <?php foreign_relation('crm_country_management','id','country_name',$datas->country,'is_active=1 ORDER BY country_name'); ?>"></option>
            ...
            <tr>
              ...
              <td>
                  <select name="division" id="division" class=" input_general" data-live-search="true">
                    <option value="<?php foreign_relation('division','division_CODE','division_name',$datas->division,'1'); ?>"></option>
			...
			<tr>
              ...
              <td>
                  <select name="district" id="district" class=" input_general" data-live-search="true">
                    <option value=" <?php foreign_relation('district_list','id','district_name',$datas->district,'1'); ?>">--Select One--</option>
			...
			<tr>
              ...
              <td><input type="text" value="" name="longitude" id="longitude" class="form-control" style="border-left: 3.5px solid #aeddf7 !important;"></td>
        ...
        <table class="table">
          ...
          <tbody>
			...
			<tr>
              ...
              <td><input type="text" value="<?=$datas->latitude?>" name="latitude" id="latitude" class="form-control" style="border-left: 3.5px solid #aeddf7 !important;"></td>
...
class="form-control" style="border-left: 3.5px solid #aeddf7 !important;" required>
      ...
      <div class="col-md-6">
    	...
    	<tr>
              ...
              <td><input type="text" value="<?=$datas->contact_number?>" name="contact_number" id="contact_number" class="form-control" style="border-left: 3.5px solid #aeddf7 !important;" required></td>
	  ...
	  class="form-control" style="border-left: 3.5px solid #aeddf7 !important;" required></td>
              ...
              <option id="converttoleadidioption" value=""></option>
						...
						<tr>
							<td>Enter Lead Name</td>
								...
								<td>
								<!--<div >
							<input class="form-control" type="text" name="lead_name" id="lead_name" value="<? foreign_relation('crm_lead_type','id','type','1') ?>;">
								<option></option>
								</div>-->
								<select  name="lead_name"  class=" input_general" required >
						<option></option>
						<? foreign_relation('crm_lead_type','id','type',$lead_type,'1'); ?>
						</select>
...
<script>
	...
	orgzip,orgcountry,orgdivision,orgdescription,orgconperson,orgconnumber,orgconmail, orglatitude, orglongitude) {
      ...
      // description = description.replace(/\n/g, '<br>');
        ...
        document.getElementById('latitude').value = orglatitude;
document.getElementById('longitude').value = orglongitude;
