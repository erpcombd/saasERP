...
if(isset($_POST['approved'])){
            ...
            $update_stmt = $conn->prepare("UPDATE bills_details SET  status = ?, amount = ?,approve_at=?,approve_by=? WHERE bills_id = ?");
            $update_stmt->bind_param("sdssi", $status, $amount, $approve_at, $approve_by, $id);
...
// exit;
            ...
            header("Location: tada_approval.php");
exit;
    
}
if(isset($_POST['return_to_user'])){
            $status = 'MANUAL';
            $id = $_POST['update_id'];
            $amount = $_POST['amount'];
            $hr_remarks = $_POST['hr_remarks'];
            $update_stmt = $conn->prepare("UPDATE bills_details SET  status = ?, amount = ?, hr_remarks = ? WHERE bills_id = ?");
            $update_stmt->bind_param("sdsi", $status, $amount, $hr_remarks, $id);
            if ($update_stmt->execute()) {
                $message = "Record updated successfully";
                $message_type = "success";
            } else {
                ...
                <div class="oe_chatter">
                  <div class="oe_followers oe_form_invisible">
                    ...
                    <div class="card-body">
                          ...
                          <table class="table table-borderless">
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td><input name="hr_remarks" type="text" id="hr_remarks" value="<?=$tada_details->hr_remarks?>" class="form-control" /></td>
                              </tr>
                      ...
                      <div class="text-end mt-4" style="display:flex; justify-content: center;align-itmes:center;">
                              <div class="text-end mt-4">
                              <button type="submit" name="return_to_user" id="return_to_user" class="btn btn-danger">Return to User</button>
