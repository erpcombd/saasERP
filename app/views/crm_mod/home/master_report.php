...
<table width="100%">
	...
	<?php 
		...
		//echo $sql_data1 = 'SELECT * FROM hrm_od_info WHERE  1 '.$date_con.' ';
			$sql_data1 = 'SELECT o.id as od_id, o.*, u.PBI_NAME, p.name as project_name 
			FROM hrm_od_info o, personnel_basic_info u, crm_project_org p 
			WHERE '.$date_con.$emp_con.' AND o.project_id=p.id AND o.PBI_ID=u.PBI_ID';
...
<? }
if($_REQUEST['report']==20250806) {
$report="Conveyance Bill Details Report";
?>
...
<table width="100%">
	...
	<tr height="30">
		<th bgcolor="#FFFACD">SL</th>
		<th bgcolor="#FFFACD">CONVEYANCE ID</th>
		<th bgcolor="#FFFACD">EMPLOYEE CODE</th>
		<th bgcolor="#FFFACD">EMPLOYEE NAME</th>
		<th bgcolor="#FFFACD">CUSTOMER NAME</th>
		<th bgcolor="#FFFACD">CONVEYANCE DATE</th>
		<th bgcolor="#FFFACD">START TIME</th>
		<th bgcolor="#FFFACD">END TIME</th>
		<th bgcolor="#FFFACD">FROM ADDRESS</th>
		<th bgcolor="#FFFACD">TO ADDRESS</th>
		<th bgcolor="#FFFACD">CONVINCE TYPE</th>
		<th bgcolor="#FFFACD">MEANS of CONVEYANCE</th>
		<th bgcolor="#FFFACD">TOTAL AMOUNT</th>
		<th bgcolor="#FFFACD">STATUS</th>
		<th bgcolor="#FFFACD">MAP</th>
	</tr>
	...
	<?php 
		...
		if($_POST['od_id']!=''){
		$od_id_con = ' and d.od_id = '.$_POST['od_id'].'';
		}
		
		if($_POST['type']!=''){
		$type_con = ' and d.conveyance_type = "'.$_POST['type'].'"';
		}
		if($_POST['transport_type']!=''){
		$transport_type_con = ' and d.transport_type = "'.$_POST['transport_type'].'"';
		}
		if($_POST['project_id']!=''){
		$project_id_con = ' and o.project_id = '.$_POST['project_id'].'';
		}
		if($_POST['status']!=''){
		$status_con = ' and d.status = "'.$_POST['status'].'"';
		}
		
		//echo $sql_data1 = 'SELECT * FROM hrm_od_info WHERE  1 '.$date_con.' ';
			$sql_data1 = 'SELECT o.id as od_id, o.*, u.PBI_NAME, p.name as project_name , 
			d.conveyance_type,d.amount,d.od_id,d.from_address,d.to_address,d.status,d.transport_type
			FROM hrm_od_info o, personnel_basic_info u, crm_project_org p ,bills_details d
			WHERE '.$date_con.$emp_con.$od_id_con.$type_con.$transport_type_con.$project_id_con.$status_con.' AND o.project_id=p.id and o.id=d.od_id 
			AND o.PBI_ID=u.PBI_ID';
			$query = db_query($sql_data1);
		$sl=1;
		while($info=mysqli_fetch_object($query)){
	?>
		<tr>
			...
			<td><?=$info->s_time;?></td>
			<td><?=$info->e_time;?></td>
			<td><?=$info->from_address; ?></td>
			<td><?=$info->to_address; ?></td>
			<td><?=$info->conveyance_type;?></td>
			<td><?=$info->transport_type;?></td>
			<td align="right"><strong><?=number_format($info->amount,2); $all_total +=$info->amount; ?></strong></td>
			<td><?=$info->status; ?></td>
			<td><? ?></td>
		</tr>
	<? } ?>
...
<? }
if($_REQUEST['report']==202508061) {
$report="Conveyance Bill Summary Report";
?>
...
<table width="100%">
	...
	<?php 
		if($_POST['f_date']!=''){
		$date_con = ' d.conveyance_date between "'.$_POST['f_date'].'" and "'.$_POST['t_date'].'"';
		}
		
		if($_POST['PBI_ID']!=''){
		$emp_con = ' and  d.emp_code = '.$_POST['PBI_ID'].'';
		}
		...
		//echo $sql_data1 = 'SELECT * FROM hrm_od_info WHERE  1 '.$date_con.' ';
			$sql_data1 = 'SELECT d.emp_code, u.PBI_NAME, 
			d.conveyance_type,sum(d.amount) as amount
			FROM personnel_basic_info u, bills_details d
			WHERE '.$date_con.$emp_con.$od_id_con.$type_con.$transport_type_con.$project_id_con.$status_con.'
			AND d.emp_code=u.PBI_ID group by d.emp_code,d.conveyance_type';
			$query = db_query($sql_data1);
		$sl=1;
		while($info=mysqli_fetch_object($query)){
	?>
