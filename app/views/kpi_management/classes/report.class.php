...
if($s!='') $sl=$s;
	if($sql==NULL) return NULL;
		...
		$str 	.= '</td></tr>';
		$str 	.= '<tr>';
		if(isset($sl))$str .='<th>SL</th>';
		$cross_year=-1;
		$service_length_of_PP=-1;
		$total_service_length=-1;
		$present_age=-1;
		for($i=0;$i<$cols;$i++)
			{
				$name = mysql_field_name($res,$i);
				$str .='<th>'.ucwords(str_replace('_', ' ',$name)).'</th>';
				if($name=='cross_year') $cross_year=$i;
				if($name=='service_length_of_PP') $service_length_of_PP=$i;
				if($name=='total_service_length') $total_service_length=$i;
				if($name=='present_age') $present_age=$i;
			}
		$str .='</tr></thead><tbody>';
...
elseif($i==$cross_year) {
	$show[$i]=1; 
	if(date('m')>substr($row[$i],5,2))	$yyy=(date('Y')-substr($row[$i],0,4));
	else $yyy=(date('Y')-substr($row[$i],0,4))-1;
	$str .='<td>'.$yyy.'</td>';}
elseif($i==$service_length_of_PP) {
	$show[$i]=1; 
	if($row[$i]=='') $str .='<td>&nbsp;</td>';
	else $str .='<td>'.Date2age($row[$i]).'</td>';}
elseif($i==$total_service_length) {
	$show[$i]=1; 
	if($row[$i]=='') $str .='<td>&nbsp;</td>';
	else $str .='<td>'.Date2age($row[$i]).'</td>';}
elseif($i==$present_age) {
	$show[$i]=1; 
	if($row[$i]=='') $str .='<td>&nbsp;</td>';
	else $str .='<td>'.Date2age($row[$i]).'</td>';}
