...
if(isset($_POST['add']))

	{
       ...
       //select week section 
			...
			$in1 = 'INSERT INTO `kpi_task` (`PBI_ID`, `task_name`, `week`, `year`, `mon`, `entry_by`) 
			VALUES ("'.$_GET['id'].'", "'.$_POST['task_name'].'", "'.$_SESSION['week_name'].'", "'.date('Y').'", "'.$_SESSION['month_name'].'" ,  "'.$_SESSION['employee_selected'].'")';
			...
			$in2 = 'INSERT INTO `kpi_task_temporary` (`PBI_ID`, `task_name`, `week`, `year`,`mon`, `entry_by`) 
			VALUES ("'.$_GET['id'].'", "'.$_POST['task_name'].'", "'.$_SESSION['week_name'].'", "'.date('Y').'", "'.$_SESSION['month_name'].'", "'.$_SESSION['employee_selected'].'")';
	   ...
	   //end select week section
			...
			$in = 'INSERT INTO `kpi_log_sheet_temporary` (`PBI_ID`, `task_name`, `week`, `year`, `mon`,`entry_by`) 
			VALUES ("'.$_GET['id'].'", "'.$_POST['task_name'].'", "'.$_SESSION['week_name'].'", "'.date('Y').'", "'.$_SESSION['month_name'].'",  "'.$_SESSION['employee_selected'].'")';
...

	{
		...
		$ssql = 'select * from kpi_log_sheet_temporary where PBI_ID="'.$_GET['id'].'" and week="'.$_SESSION['week_name'].'" and mon="'.$_SESSION['month_name'].'"  and year="'.date('Y').'"';

		$qr = mysql_query($ssql);

		$row = find_a_field('kpi_log_sheet_temporary','count(PBI_ID)','PBI_ID="'.$_GET['id'].'" and week="'.$_SESSION['week_name'].'" and mon="'.$_SESSION['month_name'].'" and year="'.date('Y').'"');
		...
		$img_id = rand();
		 ...
		 $insert = 'INSERT INTO `kpi_log_sheet_details` (`PBI_ID`, `log_id`, `submitted`, `documentart_evidence`,`point`,`entry_by`,`week`,`year`,`mon`,`att_file`) 
		 VALUES ("'.$_GET['id'].'", "'.$task_data->task_id.'", "'.$submitted.'", "'.$check.'","'.$point.'","'.$_SESSION['employee_selected'].'","'.$_SESSION['week_name'].'",
		 "'.date('Y').'","'.$_SESSION['month_name'].'","'.$upload.'")';
...

	{
			...
			$del2 = 'delete from kpi_log_sheet where task_id="'.$_GET['del'].'"';

			mysql_query($del2);
			
			$del3 = 'delete from kpi_task where task_id="'.$_GET['del'].'"';

			mysql_query($del3);
			
			$del4 = 'delete from  kpi_task_temporary where task_id="'.$_GET['del'].'"';
...
<table width="100%" border="1" cellspacing="5" cellpadding="2" align="center" style="padding:5px;">
	      ...
	      <td colspan="9" style="background:#337AB7; color:#FFFFFF;"><div align="center" style="font-size:16px; font-weight:bold;">Weekly Log Sheet  &nbsp;&nbsp;(<?=$_SESSION['week_name'];?>)</div></td>
	  ...
	  <?
	    ...
	    $ssql = 'select * from kpi_log_sheet_temporary where PBI_ID="'.$_GET['id'].'" and week="'.$_SESSION['week_name'].'" and year="'.date('Y').'"';

		$qr = mysql_query($ssql);

		 $count = find_a_field('kpi_log_sheet_temporary','count(PBI_ID)','PBI_ID="'.$_GET['id'].'" and year="'.date('Y').'"');

		 $per_row_column = 25/$count;
