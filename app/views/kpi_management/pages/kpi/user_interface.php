	...
	<?php /*?>	  <?
          ...
          <option></option>
		   ...
		   <option value="12">Dec</option>
	          ...
	          <input type="hidden" name="PBI_ID" id="PBI_ID" value="<?=$_SESSION['employee_selected']?>"/></div></td>
      ...
      <table width="100%" border="1" cellspacing="5" cellpadding="2" align="center" style="padding:5px;">
		...
		<tr height="30" style="background:#00CCFF">
		  ...
		  <td width="60%"><div align="center"><strong>Task Name</strong></div></td>
	  ...
	  <?
	    ...
	    $ssql = 'select * from kpi_log_sheet where PBI_ID="'.$_SESSION['employee_selected'].'" and week="'.$_SESSION['week_name'].'" and mon="'.$_POST['mon'].'" and year="'.date('Y').'"';

		$qr = mysql_query($ssql);

		 $count = find_a_field('kpi_log_sheet','count(PBI_ID)','PBI_ID="'.$_SESSION['employee_selected'].'" and year="'.date('Y').'"');
