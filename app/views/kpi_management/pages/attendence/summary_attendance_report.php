...
$sql="SELECT a.PBI_NAME as name,a.PBI_ID, b.DOMAIN_DESC as company_name,a.PBI_DEPARTMENT,d.DESG_DESC,a.off_day
FROM personnel_basic_info a, domai b, designation d WHERE a.PBI_DESIGNATION=d.DESG_ID and a.PBI_DOMAIN=b.DOMAIN_CODE and a.PBI_DOMAIN='".$PBI_DOMAIN."' ".$con." 
...
$info['access_stamp'][$date]=$access->access_stamp;
...
if($latetime>0) {++$late;$info['status'][$date]='Late';$info['bgcolor'][$date] = '#FFFFCC';} else {++$regular;
$info['status'][$date]='Regular';$info['bgcolor'][$date] = '#EAFFEF';}}
else
...
$department = $data->PBI_DEPARTMENT;
    ...
    <tr bgcolor="<?=$info['bgcolor'][$date]?>">
      ...
      <td><?='IT-'.date('h:i:sA',strtotime($data->access_date.' '.$info['start_time'][$date])).'<Br>OT-'.date('h:i:sA',strtotime($data->access_date.' '.$info['end_time'][$date]));?></td><td><?=($info['access_stamp'][$date]!=0)?date('h:i:sA',$info['access_stamp'][$date]):'';?></td>
