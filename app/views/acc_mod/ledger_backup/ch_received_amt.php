...
<td><input name="do_date_fr" type="text" id="do_date_fr" value="<?=$_POST['do_date_fr']?>" autocomplete="off" style="width:150px;" /></td>
<td>-to-</td>
<td><input name="do_date_to" type="text" id="do_date_to" value="<?=$_POST['do_date_to']?>" autocomplete="off" style="width:150px;"/></td>
              ...
              <tr>
                ...
                <td colspan="3"><div align="left"><span class="oe_form_group_cell" style="padding: 2px 0 2px 2px;">
                  <select name="depot_id" id="depot_id" style="width:250px;">
                    <? foreign_relation('warehouse','warehouse_id','warehouse_name',$depot_id,'use_type in("SD","WH") order by warehouse_name');?>
                  </select>
                </span></div></td>
                </tr>
				              <tr>
...
$sql2='select chalan_no,driver_name from sale_do_chalan where chalan_date between "'.$_POST['do_date_fr'].'" and "'.$_POST['do_date_to'].'"
group by chalan_no';
$query2=db_query($sql2);	
while($info=mysqli_fetch_object($query2)){
$store_no[$info->chalan_no]=$info->driver_name;
}
		...
		$sql="SELECT DISTINCT 
				...
				FROM
				  ...
				  sale_do_master m
				WHERE
				  (j.ledger_id = 3001000100000000 or j.ledger_id =4014000800010000 or j.ledger_id =4014000700030001 or j.ledger_id =4014000700030002 or j.ledger_id =4014000200010000 or j.ledger_id =4014000800020000) and 
      ...
      <td align="center"><?=$store_no[$data->tr_no];?></td>
...
if(($data->checked=='YES')){
?>
<input type="button" name="Button" value="YES"  onclick="window.open('sales_sec_print_view.php?jv_no=<?=$data->jv_no;?>');" style=" font-weight:bold;width:40px; height:20px;background-color:#66CC66;"/>
<?
}elseif(($data->checked=='NO')){
?>
<input type="button" name="Button" value="NO"  onclick="window.open('sales_sec_print_view.php?jv_no=<?=$data->jv_no;?>');" style="font-weight:bold;width:40px; height:20px;background-color:#FF0000;"/>
<? }?>
          </span></td>
      </tr>
	  <? }?>
