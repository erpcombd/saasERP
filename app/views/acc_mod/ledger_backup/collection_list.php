...
a.entry_time
	 ...
	 a.rcv_amt>0 and a.status!='Manual' and b.AREA_CODE=d.area_code and a.dealer_code=d.dealer_code and d.dealer_type='Distributor' ".$con." order by (a.rcv_amt - a.received_amt) desc";
	  ...
	  ?>
				...
				<tr <? if(++$i%2==0) echo ''; else echo ' class="alt"';?>>
					<td><?=$i;?></td>
					<td><? echo $data[0];?></b><br /><? echo $data[12];?></td>
					<td><? echo $data[2];?></td>
					<td><?=$data[15]?></td>
					<td><?=$data[16]?></td>
					<td><?=$data[11]?></td>
					<td><?=find_a_field('accounts_ledger','ledger_name','ledger_id='.$data[9]);?></td>
					<td><? echo $data[4].' - '.$data[14];?></td>
					<td><? echo $data[13];?></td>
					<td><? echo number_format($data[5],2); $rcv_total = $rcv_total +$data[5];?></td>
					<td><? echo number_format($data[6],2); ?></td>
...
Pending<? }?>
				...
				<? }}?>
	        <tr class="alt">
        <td colspan="9" align="center"><div align="right"><strong>Total Received: </strong></div></td>
...
a.entry_time
	 ...
	 a.rcv_amt>0 and a.status!='Manual' and b.AREA_CODE=d.area_code and a.dealer_code=d.dealer_code and d.dealer_type='Distributor' ".$con." order by (a.rcv_amt - a.received_amt) desc";
      ...
      <td align="center"><?=$data[15]?></td>
      <td align="center"><?=$data[16]?></td>
      <td align="center"><?=$data[11]?></td>
      <td align="center"><div align="left"><?=find_a_field('accounts_ledger','ledger_name','ledger_id='.$data[9]);?></div></td>
