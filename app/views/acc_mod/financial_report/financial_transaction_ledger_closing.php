...
and j.group_for='".$_REQUEST['group_for']."' and  j.jv_date<'".$fdate."' group by a.ledger_id";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
$opening_amt[$data->ledger_id]=$data->opening_amt;
}

 $sql = "select a.ledger_id, sum(j.dr_amt) as dr_amt, sum(j.cr_amt) as cr_amt 
 from acc_sub_sub_class s, ledger_group l, accounts_ledger a, journal j
 where s.id=l.acc_sub_sub_class and l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id 
 and j.group_for='".$_REQUEST['group_for']."' and  j.jv_date between '".$fdate."' and '".$tdate."' group by a.ledger_id";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
from acc_sub_sub_class s, ledger_group l, accounts_ledger a, journal j
where s.id=l.acc_sub_sub_class and l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id 
and j.group_for='".$_REQUEST['group_for']."' and  j.jv_date<='".$tdate."' group by a.ledger_id";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
if($closing_amt[$data1->ledger_id]!=0){
$pi++;
$sl=$pi;
?>
...
<a href="../files/transaction_subledger_summary.php?show=show&fdate=<?=$_REQUEST['fdate']?>&tdate=<?=$_REQUEST['tdate']?>&ledger_id=<?=$data1->ledger_id?>&group_for=<?=$_REQUEST['group_for']?>" target="_blank" style="color:#000000;">
<?=$data1->ledger_name;?></a>
