...
if($jv_no>0){	
	...
	$up = 'update secondary_journal set tr_mode="'.$tr_mode.'", branch="'.$branch.'", manual_voucher_no="'.$manual_voucher_no.'" where jv_no="'.$_SESSION['jv_no'].'"';
	db_query($up);
...
if(isset($_POST['limmit'])){
	  ...
	  if($_SESSION['tr_no']>0){
		if($_SESSION['jv_no']>0){
			...
			$up = 'update secondary_journal set checked="NO", tr_mode="'.$tr_mode.'", branch="'.$j_data->branch.'", manual_voucher_no="'.$j_data->manual_voucher_no.'" where jv_no="'.$_SESSION['jv_no'].'"';
				...
				<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6">
					<div class="container n-form2">
						...
						<div class="row pb-1">
							...
							<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 pr-0">
								<div class="form-group row m-0 pb-1">
									<label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text req-input font-size12">Payment Mode:</label>
									<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
							...
							<div class="col-sm-6 col-md-6 col-lg-6 col-xl-6 pl-0">
								<div class="form-group row m-0 pb-1">
									<label class="col-sm-5 col-md-5 col-lg-5 col-xl-5 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text font-size12">Paid Through:</label>
									<div class="col-sm-7 col-md-7 col-lg-7 col-xl-7 p-0 pr-2 ">
	...
	<?php 
						...
						<div id="cash_check" class="form-group row m-0 pb-1">
							...
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2 ">
								...
								<? if($jv->tr_no>0){ ?><input type="text" name="c_id_name" id="cash_disable_id" readonly value="<?=find_a_field('accounts_ledger','ledger_name','ledger_id="'.$jv->relavent_cash_head.'"');?>" style="float:left" tabindex="5"/>
								...
								<? } else{?>
									...
									foreign_relation('accounts_ledger','ledger_id','ledger_name',$c_id,'ledger_group_id="'.$cash_ledg_group_id.'" order by ledger_id');
						...
						<div id="bank_check" class="form-group row display-mamun m-0 pb-1">
							...
							<label class="col-sm-3 col-md-3 col-lg-3 col-xl-3 m-0 p-0 d-flex justify-content-end align-items-center pr-1 font-size12 bg-form-titel-text req-input">Bank A/C Credit:</label>
							<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9 p-0 pr-2">

								<? if($jv->tr_no>0){ ?><input type="text" name="b_id_name" id="bank_disable_id"  tabindex="6" value="<?=find_a_field('accounts_ledger','ledger_name','ledger_id="'.$jv->relavent_cash_head.'"');?>" readonly="readonly" /><input type="hidden" name="b_id" value="<?=$jv->relavent_cash_head?>" /><? }else{?><select name="b_id" id="bank_disable_id"  tabindex="2" <? if($jv->tr_no>0) echo 'readonly'?> onchange="bankData(this.value)" >

									<option value="0"></option>

									<?

									foreign_relation('accounts_ledger','ledger_id','ledger_name',$b_id,'ledger_group_id="'.$bank_ledg_group_id.'"  order by ledger_name asc');
						...
						<?php if($cid == 'golden' || $cid == 'golden-demo' || $cid == 'dailyrice'){?>
						<!--This code for golden only start-->
						...
						<!--this code for golden only end-->
						<?php } ?>
						
						<div id="trMode_check" class="form-group row display-mamun m-0 pb-1">
							...
							if(mysqli_num_rows($data2)>0){
								...
								{$dataa[2]=substr($dataa[2],0,20).'...';
									...
									<tr>
										...
										<? if($cid == 'mamun'){?>
										<td><a href="general_voucher_print_view_for_draft_new.php?jv_no=<?php echo $dataa[4];?>" target="_blank"><img src="../images/print.png" width="16" height="16" border="0"></a></td>
										<? } else {?>
											<td><a href="general_voucher_print_view_for_draft.php?jv_no=<?php echo $dataa[4];?>" target="_blank"><img src="../images/print.png" width="16" height="16" border="0"></a></td>
										<? }?>
									</tr>
								<? }}?>
							</tbody>
