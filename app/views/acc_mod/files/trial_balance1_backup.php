...
<?
 ...
 $t_date=$_REQUEST['tdate'];
	...
	$sql = "select g.acc_sub_class, sum(j.dr_amt-j.cr_amt) as sub_cls_opening_amt from journal j, accounts_ledger a, ledger_group g 
	where j.ledger_id=a.ledger_id and a.ledger_group_id=g.group_id and j.jv_date <'".$f_date."' group by g.acc_sub_class";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		...
		$sql = "select g.acc_sub_class, sum(j.dr_amt) as sub_cls_dr_amt, sum(j.cr_amt) as sub_cls_cr_amt from journal j, accounts_ledger a,  ledger_group g   
		where j.ledger_id=a.ledger_id  and a.ledger_group_id=g.group_id  and j.jv_date between '".$f_date."' and '".$t_date."' group by g.acc_sub_class";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		...
		$sql = "select g.acc_sub_class, sum(j.dr_amt-j.cr_amt) as sub_cls_closing_amt from journal j, accounts_ledger a, ledger_group g  
		where j.ledger_id=a.ledger_id and a.ledger_group_id=g.group_id and j.jv_date <='".$t_date."' group by g.acc_sub_class ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
...
<table border="0" cellspacing="0" cellpadding="0" class="table" style="margin-bottom: 0px;;">
<tr>
<td style="width: 35%; font-weight: 600;" class="sr-ledger-name"><span style="font-size: 14px; font-weight: bold; letter-spacing: 5px;">--</span><?=$row3->group_name?></td>
<td style="width: 15%; text-align: right;">
<? if($grp_opening_amt[$row3->group_id]>0) echo number_format($grp_opening_amt[$row3->group_id],2).' (Dr)'; elseif($grp_opening_amt[$row3->group_id]<0) echo number_format(((-1)*$grp_opening_amt[$row3->group_id]),2).' (Cr)';else echo "0.00"; ?>
</td>
<td style="width: 15%; text-align: right;" class="sr-ledger-dr-amt"><?=$grp_dr_amt[$row3->group_id]?> </td>
<td style="width: 15%; text-align: right;" class="sr-ledger-cr-amt"><?=$grp_cr_amt[$row3->group_id]?> </td> 
<td style="width: 15%; text-align: right;">

<? if($grp_closing_amt[$row3->group_id]>0) echo number_format($grp_closing_amt[$row3->group_id],2).' (Dr)'; elseif($grp_closing_amt[$row3->group_id]<0) echo number_format(((-1)*$grp_closing_amt[$row3->group_id]),2).' (Cr)';else echo "0.00"; ?>
 </td>
<td style="width: 5%;" class="sr-ledger-action"  align="center"><button data-toggle="collapse" data-parent="#faqAccordion" data-target="#t<?=$row3->group_id?>" class="bgc-new-btn">&emsp;</button></td>
</tr>
</table>
</div>
...
?>
<tr>
<td style="width: 35%;" class="sr-ledger-name"><span style="font-size: 14px; font-weight: bold; letter-spacing: 5px;">---</span><?=$row4->ledger_name?></td>
<td style="width: 15%; text-align: right;">

<? if($opening_amt[$row4->ledger_id]>0) echo number_format($opening_amt[$row4->ledger_id],2).' (Dr)'; elseif($opening_amt[$row4->ledger_id]<0) echo number_format(((-1)*$opening_amt[$row4->ledger_id]),2).' (Cr)';else echo "0.00"; ?>

</td>
<td style="width: 15%; text-align: right;" class="sr-ledger-dr-amt"><?=$dr_amt[$row4->ledger_id]?></td>
<td style="width: 15%; text-align: right;" class="sr-ledger-cr-amt"><?=$cr_amt[$row4->ledger_id]?></td> 
<td style="width: 15%; text-align: right;">
<? if($closing_amt[$row4->ledger_id]>0) echo number_format($closing_amt[$row4->ledger_id],2).' (Dr)'; elseif($closing_amt[$row4->ledger_id]<0) echo number_format(((-1)*$closing_amt[$row4->ledger_id]),2).' (Cr)';else echo "0.00"; ?>
</td>
