...
/*}*/
	...
	<div class="form-container_large">
		...
		<form  id="form1" name="form1" method="post" action="">
			...
			foreign_relation('accounts_ledger','ledger_id','ledger_name',$ledger_id,"1 and group_for='".$_REQUEST['group_for']."'  order by ledger_id");
		...
		<input type="hidden" name="group_for" id="group_for" class="form-control" style="float:left" value="<?=$_REQUEST['group_for']?>">
				<input  name="group_for_show" id="group_for_show" class="form-control" style="float:left" 
				value="<?=find_a_field('user_group','group_name','id='.$_REQUEST['group_for']);?>">
     ...
     $fy_start_date = $fy_start_year . "-07-01"; // Fiscal year starts on July 1st
      $fy_end_date = $fy_end_year . "-06-30";
							...
							if($cc_code > 0)
							 ...
							 where a.ledger_id=b.ledger_id and a.group_for='".$_REQUEST['group_for']."' and a.jv_date between '$fdate' AND '$tdate' 
							 and a.ledger_id like '$sledger_id%' and d.sub_ledger_id = a.sub_ledger ".$cc_con.$d_where.$v_type.$sub_ledger_con." 
							 group by a.sub_ledger ";
...
WHERE a.ledger_id like '$sledger_id%' 
	...
	//sub ledger wise opening -> open
		 ...
		 from journal a 
		 left join general_sub_ledger s on s.sub_ledger_id=a.sub_ledger 
		 left join user_group g on g.id=a.group_for,accounts_ledger b".$d_table." 
		 where a.ledger_id=b.ledger_id and a.jv_date < '$fdate'
		 ".$cc_con.$sub_ledger_con." 
		 and a.group_for='".$_REQUEST['group_for']."' and a.ledger_id='$sledger_id' ".$d_where." group by s.sub_ledger_id" ;
 ...
 $p="select a.jv_date,b.ledger_name,sum(a.dr_amt) as dr_amt,sum(a.cr_amt) as cr_amt,
 a.tr_from,a.narration,a.jv_no,a.tr_no,a.jv_no,a.cheq_no,a.cheq_date,a.relavent_cash_head , a.cc_code,a.group_for, s.sub_ledger_name,g.group_name,s.sub_ledger_id,b.ledger_id
		 from journal a 
		 left join general_sub_ledger s on s.sub_ledger_id=a.sub_ledger 
		 left join user_group g on g.id=a.group_for,accounts_ledger b".$d_table." 
		 where a.ledger_id=b.ledger_id and a.group_for='".$_REQUEST['group_for']."' and a.jv_date between '$fdate' AND '$tdate' 
		 ...
		 and a.ledger_id='$sledger_id' ".$d_where." group by s.sub_ledger_id" ;
							...
							while($data=mysqli_fetch_row($sql))
							{
								...
								<tr <?=($xx%2==0)?' bgcolor="#EDEDF4"':'';$xx++;?>>
									...
									<td align="left" ><a href="transaction_listledger_bs.php?show=show&fdate=<?=$_REQUEST['fdate']?>&tdate=<?=$_REQUEST['tdate']?>&ledger_id=<?=$data[17]?>&group_for=<?=$_REQUEST['group_for']?>" target="_blank"><?=$data[1];?></a></td>
									<td align="left" ><a href="transaction_listledger_bs.php?show=show&fdate=<?=$_REQUEST['fdate']?>&tdate=<?=$_REQUEST['tdate']?>&sub_ledger_id=<?=$data[16]?>&ledger_id=<?=$data[17]?>&group_for=<?=$_REQUEST['group_for']?>" target="_blank"><?=$data[14];?></a></td>
