...
if($_GET['tr_no']>0){
 ...
 $sj_val=find_a_field('secondary_journal','count(id)',"tr_no='".$_SESSION['tr_no']."' and tr_from='Receipt' and dr_amt>0");
  ...
  $vou_type=find_a_field('secondary_journal','type',"tr_no='".$_SESSION['tr_no']."' and tr_from='Receipt' and cr_amt>0");
...
$relavent_head = $_POST['b_id'];
 ...
 where jv_no="'.$_SESSION['jv_no'].'" and tr_from="Receipt"';
...
$sum_dr_amt=find_a_field('secondary_journal','sum(cr_amt)','jv_no="'.$_SESSION['jv_no'].'" and cr_amt>0');

$detail = 'Received from '.$j_data->received_from;

add_to_sec_journal($proj_id, $j_data->jv_no, $j_data->jv_date, $j_data->relavent_cash_head,  $detail, $sum_dr_amt, '0', $tr_from, $j_data->tr_no,'','','0','',$j_data->user_id,'',$j_data->received_from,$j_data->bank,$j_data->cheq_no,$j_data->cheq_date,$j_data->relavent_cash_head,$checked,$j_data->type,$j_data->employee_id , 
$j_data->remarks , '0' );
...
var info = ledger_id+"<@>"+reference_id+"<@>"+cc_code+"<@>"+narration+"<@>"+cr_amt+"<@>"+flag;
...
from  secondary_journal a where a.tr_from='Receipt'  and SUBSTRING(a.tr_no,5,1)='0' and  a.dr_amt>0  and a.jv_no='".$_SESSION['jv_no']."' group by a.tr_no   order by a.tr_no desc limit 5";
