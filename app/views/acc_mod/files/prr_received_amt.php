				...
				group by  j.tr_no";
					...
					while($data=mysqli_fetch_object($query)){
						...
						<tr <?=($i%2==0)?'class="alt"':'';?>>
							...
							<!--<td><a target="_blank" href="prr_sec_print_view.php?jv_no=<?=rawurlencode(url_encode($data->jv_no)) ?>"><img src="../images/print_hover.png" width="20" height="20" /></a></td>-->
							...
							<td>
								<span id="divi_<?=$data->tr_no?>">
									...
									if(($data->checked=='YES')){
										...
										<!--<input type="button" name="Button" value="YES"  onclick="window.open('prr_sec_print_view.php?jv_no=<?=rawurlencode(url_encode($data->jv_no));?>');" class="btn1 btn1-bg-submit"/>-->
										
										
   <input type="button" name="Button" value="YES"
  onclick="window.open('prr_sec_print_view.php?c=<?=rawurlencode(url_encode($c_id))?>&v=<?=rawurlencode(url_encode($data->jv_no))?>');"
