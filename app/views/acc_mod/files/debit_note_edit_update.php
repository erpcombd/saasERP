...
if($_GET['tr_no']>0){
 ...
 $sj_val=find_a_field('secondary_journal','count(id)',"tr_no='".$_SESSION['tr_no']."' and tr_from='Payment' and cr_amt>0");
   ...
   $vou_type=find_a_field('secondary_journal','type',"tr_no='".$_SESSION['tr_no']."' and tr_from='Payment' and dr_amt>0");
...
$relavent_head = $_POST['b_id'];
  ...
  $ms_up = 'update secondary_journal set remarks="'.$_POST['remarks'].'", jv_date="'.$_POST['jv_date'].'", relavent_cash_head="'.$relavent_head.'" 
 where jv_no="'.$_SESSION['jv_no'].'" and tr_from="Payment"';
...
if($sj_val>0){
 ...
 db_query($del_sj);
			...
			$sum_cr_amt=find_a_field('secondary_journal','sum(dr_amt)','jv_no="'.$_SESSION['jv_no'].'" and dr_amt>0');

			$detail = 'Paid To '.$j_data->received_from;



			add_to_sec_journal($proj_id, $_SESSION['jv_no'], $j_data->jv_date, $j_data->relavent_cash_head,  $detail, '0',$sum_cr_amt, $tr_from, $_SESSION['tr_no'],'','',$j_data->cc_code,'',$_SESSION['user']['id'],'',$j_data->received_from,$j_data->bank,$j_data->cheq_no,$j_data->cheq_date,$j_data->relavent_cash_head,$checked,
...
var reference_id = (document.getElementById('reference_id_'+id).value);
var cc_code = (document.getElementById('cc_code_'+id).value);
...
var info = ledger_id+"<@>"+reference_id+"<@>"+cc_code+"<@>"+narration+"<@>"+dr_amt+"<@>"+flag;
...
foreign_relation('accounts_ledger','ledger_id','ledger_name',$jv->relavent_cash_head,"ledger_group_id=127001 order by ledger_id");
...
foreign_relation('accounts_ledger','ledger_id','ledger_name',$jv->relavent_cash_head,"ledger_group_id=127002 order by ledger_id");
...
?>
                        ...
                        <tr id="check_no_check">
                          <td><div align="right"><span>Instrument No:</span></div></td>
                        ...
                        <tr id="check_date_check">
                          <td><div align="right"><span>Instrument</span> Date:</div></td>
...
from  secondary_journal a where a.tr_from='Payment' and  SUBSTRING(a.tr_no,5,1)='0' and  a.cr_amt>0 and a.jv_no='".$_SESSION['jv_no']."' group by a.tr_no order by a.tr_no desc limit 5";
