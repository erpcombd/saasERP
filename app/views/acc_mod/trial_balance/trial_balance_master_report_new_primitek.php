...
elseif($_POST['report']==230910001)
{
		...
		<td  width="50%" style="border:0px; border-color:white;">
			<table width="100%"  >
				<tr align="center" >
					<?php /*?><td style="font-size:20px; border:0px; border-color:white;"><strong><?=$_SESSION['company_name'];?></strong></td>	<?php */?>
			<td style="font-size:20px; border:0px; border-color:white;"><strong><?=find_a_field('user_group','group_name','id='.$_POST['group_for']);?></strong></td>	
				</tr>
				<tr align="center" >
...
echo $count_days = round($datediff / (60 * 60 * 24));
		...
		if($_POST['group_for']>0)    {$group_con=' and group_for="'.$_POST['group_for'].'"';}
		if($_POST['group_for']>0)    {$group_journal_con=' and j.group_for="'.$_POST['group_for'].'"';}
		...
		if(isset($item_type)) 					{$item_type_con=' and i.item_type='.$item_type;}
 		 ...
 		 $sql = "select ledger_id, sum(dr_amt-cr_amt) as opening_amt from journal where jv_date <'".$f_date."' ".$group_con." group by ledger_id ";
		 $query = db_query($sql);
		...
		$sql = "select ledger_id, sum(dr_amt) as dr_amt from journal where jv_date between '".$f_date."' and '".$t_date."' ".$group_con."group by ledger_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $dr_amt[$info->ledger_id]=$info->dr_amt;
		}
		
		$sql = "select ledger_id, sum(cr_amt) as cr_amt from journal where jv_date between '".$f_date."' and '".$t_date."' ".$group_con." group by ledger_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		...
		$sql = "select ledger_id, sum(dr_amt-cr_amt) as closing_amt from journal where jv_date <='".$t_date."' ".$group_con." group by ledger_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
		  ...
		  $sql_sub="select g.group_id, g.group_name 
		  from ledger_group g, accounts_ledger a 
		  where g.group_id=a.ledger_group_id and g.acc_class in (1,2) 
		  group by a.ledger_group_id order by g.group_id";
	...
	<td bgcolor="#d6eaf8"><div align="left" style="text-transform:uppercase; font-size:14px; font-weight:700;"><strong><?=$info_sub->group_name; ?></strong></div></td>
		...
		<?
//retined earning calculated from balance sheet
...
from acc_sub_sub_class s, ledger_group l, accounts_ledger a, journal j
 where s.id=l.acc_sub_sub_class and l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id and  j.jv_date<'".$f_date."' and s.acc_sub_class=21 and s.id=215
 ".$group_journal_con." group by s.acc_sub_class";
$query = db_query($sql);
...
$opening_sc_amt[$data->acc_sub_class]=$data->opening_sc_amt;
 ...
 from acc_sub_sub_class s, ledger_group l, accounts_ledger a, journal j 
 where s.id=l.acc_sub_sub_class and l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id and  j.jv_date<'".$f_date."' and a.acc_class=3 
 ".$group_journal_con." group by a.acc_class";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
from acc_sub_sub_class s, ledger_group l, accounts_ledger a, journal j
 where s.id=l.acc_sub_sub_class and l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id and  j.jv_date<'".$f_date."' and a.acc_class=4 
 ".$group_journal_con." group by a.acc_class";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
from acc_sub_sub_class s, ledger_group l, accounts_ledger a, journal j
 where s.id=l.acc_sub_sub_class and l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id and  j.jv_date between '".$f_date."' and '".$t_date."' and s.acc_sub_class=21 and s.id=215 ".$group_journal_con." group by s.acc_sub_class";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
$sc_amt[$data->acc_sub_class]=$data->sc_amt;
 ...
 from acc_sub_sub_class s, ledger_group l, accounts_ledger a, journal j 
 where s.id=l.acc_sub_sub_class and l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id and  j.jv_date between '".$f_date."' and '".$t_date."' and a.acc_class=3 ".$group_journal_con." group by a.acc_class";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
from acc_sub_sub_class s, ledger_group l, accounts_ledger a, journal j
 where s.id=l.acc_sub_sub_class and l.group_id=a.ledger_group_id and a.ledger_id=j.ledger_id and  j.jv_date between '".$f_date."' and '".$t_date."' and a.acc_class=4  ".$group_journal_con." group by a.acc_class";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
$retained_earning_ledger_value_during=find_a_field('journal','sum(dr_amt-cr_amt)','ledger_id=97 and jv_date between "'.$fdate.'" and "'.$tdate.'" 
and group_for="'.$_POST['group_for'].'"');
...
$net_retained_earnings=(($sales_amt[3]+$ret_amt[21])-$exp_amt[4])+$retained_earning_ledger_value_during;

if($net_retained_earnings>0){$pos_net_reatined_earnings=$net_retained_earnings;}else{$pos_net_reatined_earnings=($net_retained_earnings*(-1));}
	 ...
	 //retined earning calculated from balance sheet	
 	 ...
 	 $sql="select distinct ledger_id, ledger_name from accounts_ledger where ledger_group_id='".$info_sub->group_id."'  order by ledger_name";
		...
		<tr>
		<td></td>
		<td>Retained Earning Calculated</td>
		<td style="text-align:right">
		<? 
		if($pos_net_reatined_earnings_ope>0){
		echo number_format($pos_net_reatined_earnings_ope,2);
		$total_retained_earning_op=$pos_net_reatined_earnings_ope;
		$total_retained_earning_op2=$pos_net_reatined_earnings_ope;
		} 
		
		?>
		</td>
		<? if($net_retained_earnings<0){?>
		<td style="text-align:right">
		<?=number_format($pos_net_reatined_earnings,2); $total_net_retained_earnings_dr+=$pos_net_reatined_earnings;
		$total_net_retained_earnings_dr_2+=$pos_net_reatined_earnings;
		?>
		</td>
		<td></td>
		<? } else {?>
		<td></td>
		<td style="text-align:right">
		<?=number_format($pos_net_reatined_earnings,2); $total_net_retained_earnings_cr+=$pos_net_reatined_earnings;
		$total_net_retained_earnings_cr_2+=$pos_net_reatined_earnings;
		?>
		</td>
		<? } ?>
		<td></td>
...
<? } } ?>
		<tr  style="font-size:14px;">
		  ...
		  <? if($total_retained_earning_op>0){?>
		  <td>
		  <div align="right">
		  <span class="style7">
		    <? number_format($sub_total_opening_balance=$sub_total_opening_amt+$total_retained_earning_op,2); 
			$total_sub_total_opening_amt +=$sub_total_opening_balance; 
			if($sub_total_opening_balance>0) echo number_format($sub_total_opening_balance,2).' (Dr)'; 
			elseif($sub_total_opening_balance<0) echo number_format(((-1)*$sub_total_opening_balance),2).' (Cr)';
			else echo "0.00"; ?>
	      </span>
		  </div>
		  </td>
		  <? $total_retained_earning_op=0; } else { ?>
		  <td>
		  <div align="right">
		  <span class="style7">
		  ...
		  <? } ?>
		  <?php /*?><? if($total_net_retained_earnings_dr>0){?>
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_dr_amt+$total_net_retained_earnings_dr,2); $total_sub_total_dr_amt +=$sub_total_dr_amt+$total_net_retained_earnings_dr;?>
	      </span></div></td>
		  <? $total_net_retained_earnings_dr=0; }else{ ?><?php */?>
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_dr_amt,2); $total_sub_total_dr_amt +=$sub_total_dr_amt;?>
	      </span></div></td>
		<?php /*?>  <? } if($total_net_retained_earnings_cr>0){?>
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_cr_amt+$total_net_retained_earnings_cr,2); $total_sub_total_cr_amt +=$sub_total_cr_amt+$total_net_retained_earnings_cr;?>
	      </span></div></td>
		  
		  <? $total_net_retained_earnings_cr=0; } else { }?><?php */?>
		  <td><div align="right"><span class="style7">
		    <?=number_format($sub_total_cr_amt,2); $total_sub_total_cr_amt +=$sub_total_cr_amt;?>
	      </span></div></td>
		  
		  <?  if($total_net_retained_earnings_dr_2>0 || $total_net_retained_earnings_cr_2>0 || $total_retained_earning_op2>0){ ?>
		  <td>
		    <div align="right"><span class="style7">
		      <? number_format($sub_total_closing_balance=$sub_total_closing_amt+$pos_net_reatined_earnings_ope+$total_net_retained_earnings_dr_2-$total_net_retained_earnings_cr_2,2); $total_sub_total_closing_amt +=$sub_total_closing_balance;
			...
			<? $total_net_retained_earnings_dr_2=0; $total_net_retained_earnings_cr_2=0; $total_retained_earning_op2=0;} else {?>
			 <td>
		    <div align="right"><span class="style7">
