...
if(isset($_POST['confirms'])){
		...
		$group = $info->group_for;
		 ...
		 $master_insert = 'insert into vendor_invoice_master set invoice_no="'.$invoice_no.'",referance="'.$referance.'",invoice_date="'.$invoice_date.'",receive_date="'.$receive_date.'",vendor_id="'.$info->vendor_id.'",grn_no="'.$grn_no.'",po_no="'.$info->po_no.'",tds_code="'.$tds_code.'",tds_percent="'.$tds_percent.'",vds_percent="'.$_POST['mushak_no'].'",tds_amount="'.$tds_amount.'",deductible="'.$_POST['deductible'].'", entry_at="'.date('Y-m-d H:i:s').'",entry_by="'.$_SESSION['user']['id'].'", status="PENDING",group_for="'.$info->group_for.'"';
 ...
 $q = 'insert into vendor_invoice_details set system_invoice_no="'.$system_invoice_no.'",grn_no="'.$grn_no.'",pr_id="'.$data->id.'",po_no="'.$info->po_no.'",item_id="'.$data->item_id.'",qty="'.$qty.'",rate="'.$data->rate.'",amount="'.$amount.'",group_for="'.$info->group_for.'",entry_at="'.date('Y-m-d H:i:s').'", entry_by="'.$_SESSION['user']['id'].'"';
db_query($q);
  ...
  <tr>
    ...
    <td colspan="5" valign="top"><table width="40%" border="1" align="center" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
      ...
      <tr>
        ...
        <td rowspan="2" align="center" bgcolor="#CCFF99"><a href="../../../views/purchase_mod/po/po_print_view_store.php?po_no=<?=$po_no?>" target="_blank"><img src="../../../images/print.png" width="26" height="26" /></a></td>
      </tr>
...
<?php /*?>	<? if($ex<time()){?>
	...
	<table width="96%" border="1" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <td bgcolor="#66CCCC"><input style="width:120px;"  name="mushak_no" type="text" id="mushak_no" value="<?=$info->vat?>" readonly/></td>
...
<? if($grn_no>0){
 ...
 $sql='select a.id,a.item_id,a.po_no,b.item_name,b.unit_name,a.qty,a.rate,a.order_no from purchase_receive a,item_info b where b.item_id=a.item_id and a.pr_no='.$grn_no;
$res=db_query($sql);
          ...
          <tr bgcolor="<?=(($bg%2)==1)?'#FFEAFF':'#DDFFF9'?>">
            ...
            <td><?=$row->item_id?>
              ...
              <td>
			  		...
			  		<?=find_a_field('purchase_invoice','item_description','id='.$row->order_no);?>
   ...
   id="amount<?=$row->id?>" value="<?=$totala=$row->qty*$row->rate?>" readonly /></td>  <? $G_total +=$totala;  ?>
          ...
          <? }?>
		    ...
		    <tr> 
			  ...
			  <td bgcolor="#6699FF" style="" colspan="8">Total Amount </td>
