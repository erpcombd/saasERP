...
//var_dump($_POST);
 ...
 // $j_sql="select ledger_id,sum(cr_amt),sum(dr_amt),narration,sub_ledger from journal where tr_from='Opening' and group_for ='".$_SESSION['user']['group']."' group by sub_ledger"; // modified by sakib on 29/6/2025 for primitek CPL. The older version is just below it.
    
 $j_sql="select ledger_id,cr_amt,dr_amt,narration,sub_ledger from journal where tr_from='Opening' and group_for ='".$_POST['group_for']."' ";
...
$ledger_cr[$j_data[4]][$j_data[0]]=$j_data[1];

$ledger_dr[$j_data[4]][$j_data[0]]=$j_data[2];

$narration[$j_data[4]][$j_data[0]]=$j_data[3];
...
var ledger_id=(document.getElementById('ledger_id'+sub_ledger_id).value);
...
var strURL="opening_balance_sub_ledger_id_ajax.php?ledger_id="+ledger_id+"&cr="+cr+"&dr="+dr+"&opdate="+opdate+"&narration="+narration+"&flag="+flag+"&sub_ledger="+sub_ledger_id+"&group_for="+group_for;
...
value="<?=$_POST['op_date']?>"  required autocomplete="off"/>
		...
		<div class="container-fluid bg-form-titel mt-2">
			<div class="row">
				...
				<div class="col-sm-9 col-md-9 col-lg-9 col-xl-9">
					<div class="form-group row m-0">
				<label class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-end align-items-center pr-1 bg-form-titel-text">Acc Ledger</label>
						<div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0">
						
								<input type="text" name="search_ledger_id" id="search_ledger_id" list="ledgeridList" value="<? if(isset($_POST['search_ledger_id'])) echo find_a_field('accounts_ledger','concat(ledger_name,"->",ledger_id)','ledger_id='.end(explode("->",$_POST['search_ledger_id'])))?>" required>
					    <datalist id="ledgeridList">
						...
						foreign_relation('accounts_ledger','concat(ledger_name,"->",ledger_id)','""',$_POST['search_ledger_id'],'1');
			...
			<table class="table1  table-striped table-bordered table-hover table-sm">
				...
				<tbody class="tbody1">
					 <?
						  ...
						  if($_POST['search_ledger_id']!=''){
						  $only_ledger_id_ledger = end(explode("->",$_POST['search_ledger_id']));
						  $ledger_id_con .=' and j.ledger_id = "'.$only_ledger_id_ledger.'"';
						  }
						   if($_POST['group_for']!=''){
						  $group=$_POST['group_for'];
						  $group_con .=' and a.group_for = "'.$group.'"';
						  }
						...
						where a.ledger_id=j.ledger_id ".$ledger_con.$group_con." order by a.sub_ledger_id";
				   ...
				   $dr_amt=$ledger_dr[$data[0]][$only_ledger_id_ledger];
					
					$cr_amt=$ledger_cr[$data[0]][$only_ledger_id_ledger];
					
					$narrations=$narration[$data[0]][$only_ledger_id_ledger];
	...
	<!--  <td ><input type="text" name="ledger_id<?=$data[0]?>" id="ledger_id<?=$data[0]?>" list="subledgerList" value="" >
		...
		<? foreign_relation('accounts_ledger','concat(ledger_name,"->",ledger_id)','""',$_POST['ledger_id'],'ledger_id="'.$_POST['search_ledger_id'].'"');?>
						</datalist>
						</td>-->

	  		 <td >
   <input type="text" name="ledger_id<?=$data[0]?>" id="ledger_id<?=$data[0]?>" 
   value="<?=$_POST['search_ledger_id']?>" >
