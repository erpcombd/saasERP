...
function check()
{
	...
	if (isNaN(document.form1.cc_code.value)) 
	{
		alert('Please Enter only numerical values in CC Code.');
		return false;
	}
	if(document.form1.amount.value<1){
		...
		return false;
			...
			var f=document.getElementById("ledger_status").value;
		...
		$.ajax({
		  ...
		  data: "a="+a+"&b="+b+"&c="+c+"&d="+d+"&e="+e+"&f="+f+"&cc_code="+cc_code+"&count="+count,
		  success: function(data) {						
				$('#tbl').append(data);
				$('#count').val(count);
				count = count + 1;		
