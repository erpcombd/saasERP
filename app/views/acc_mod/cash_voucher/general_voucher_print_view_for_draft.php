...
$jv_config = find_a_field('voucher_config','direct_journal','voucher_type="'.$jv_all->group_for.'"');
...
<!--				<p class="text">Cell: <?=$group_data->mobile?>. Email: <?=$group_data->email?> <br> <?=$group_data->vat_reg?></p>-->
		...
		<td width="17%" align="right" class="qr-code">
              ...
              $print_url = "https://saaserp.ezzy-erp.com/app/views/acc_mod/cash_voucher/general_voucher_print_view_for_draft.php?c=" . rawurlencode($company_id) . "&v=" . rawurlencode($req_no_qr_data);
        ...
        <td align="left"><div align="center">
          <?=++$s;?>
			...
			<? if ($jv_all->tr_from=='Receipt') { ?>
					...
					<td align="left"><a style="text-decoration: none;" target="_blank" href="general_voucher_print_view_for_draft_client.php?c=<?=rawurlencode(url_encode($c_id))?>&v=<?=rawurlencode(url_encode($info->sub_ledger))?>"><?=$info->ledger_id?></a></td>
			<? } else { ?>
					<td align="left" >  <?=$info->ledger_id?>		</td>
			<? } ?>
...
<div class="footer1">
	<table class="footer-table">
        ...
        <tr>
		  ...
		  <p style="font-weight:600; margin: 0;"> <?=find_a_field('user_activity_management','fname','user_id='.$jv->entry_by);?> </p>
		  ...
		  <p style="font-size:11px; margin: 0;">(<?=find_a_field('user_activity_management','designation','user_id='.$jv->om_checked);?>) <br/> <?=$jv->om_checked_at?></p>
		  </td>
