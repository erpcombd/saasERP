			...
			<table id="table_head" class="table1 table-striped table-bordered table-hover table-sm dataTable no-footer" role="grid" aria-describedby="table_head_info" style=" ZOOM: 95%; ">
				...
				?>
					<tr>
						...
						<td>
						<? if($row->status == 'check'){?>
						<button type="button" onclick="custom('<?=rawurlencode(url_encode($row->id));?>');" class="btn2 btn1-bg-update"><i class="fa-solid fa-pen-to-square"></i></button>
						<? }else{ ?>
						<button type="button" onclick="custom1('<?=rawurlencode(url_encode($row->id));?>');" class="btn2 btn1-bg-help"><i class="fa-solid fa-print"></i></button>
						<? } ?>
						</td>
