...
case 2002:
		...
		i.finish_goods_code as fg,i.item_id,i.item_name,i.unit_name as unit,i.sales_item_type ,
		i.item_brand as brand,i.pack_size pkt
...
if(isset($region_id)) 		{$acon.=' and b.BRANCH_ID="'.$region_id.'"';}
		...
		$sql2='select i.item_id,i.pack_size as pkt,sum(a.total_unit) mod i.pack_size as pcs,sum(a.total_unit) qty,sum(a.total_unit*a.unit_price) as sale_price
...
a.unit_price>0 and a.item_id=i.item_id '.$acon.$con.$date_con.$warehouse_con.$item_con.$item_brand_con.$dtype_con.' group by  a.item_id';
		...
		<td  width="50%" style="border:0px; border-color:white;">
			...
			<table width="100%" id="ExportTable">
				...
				<tr align="center" >
					...
					<td style="font-size:18px; border:0px; border-color:white;"><strong>
					<?=find_a_field('user_group','group_name','id='.$_SESSION['user']['group'])?></strong></td>

				</tr>

				<tr align="center" >

					<td style="font-size:14px; border:0px; border-color:white;"><strong><?=$report;?> of: <?php echo date('d-M-Y',strtotime($_REQUEST['t_date']));?></strong></td>
...
<!-- 		 <tr>
		...
		<thead>
			<tr><td colspan="12" style=" border: none; font-weight: 600; text-align: center; font-size: 16px; ">Daily Regular Report</td></tr>
			<tr>
				<td></td>
			</tr>
		<tr height="30">
		...
		<th width="12%" bgcolor="#FFFACD">Emp Code</th>

		<th width="15%" bgcolor="#FFFACD">Name</th>
        <th width="14%" bgcolor="#FFFACD">Department</th>
		<th width="11%" bgcolor="#FFFACD">Emp Type</th>
		

		<th width="9%" bgcolor="#FFFACD">Designation</th>

		<th width="9%" bgcolor="#FFFACD">Date</th>

		<th width="9%" bgcolor="#FFFACD">In Time</th>

		<th width="9%" bgcolor="#FFFACD">Out Time</th>

		<th width="5%" bgcolor="#FFFACD">Schedule</th>
       
        <th class="new_print" width="8%" bgcolor="#FFFACD">Location</th>

		<th width="5%" bgcolor="#FFFACD">Source</th>
   ...
   echo  $sql = "select EMP_CODE,  min(xtime) as min_time,latitude,longitude from hrm_attdump where xdate ='".$t_date."' group by EMP_CODE ";
     ...
     echo $sql = "select EMP_CODE,  max(xtime) as max_time  from hrm_attdump where xdate ='".$t_date."' group by EMP_CODE ";
...
if($_POST['DEPT_ID']!='')
$dept_con .= ' and p.DEPT_ID = "'.$_POST['DEPT_ID'].'" ';

if($_POST['PBI_DESIGNATION']!='')
$desg_con .= ' and p.DESG_ID = "'.$_POST['PBI_DESIGNATION'].'" ';

if($_POST['PBI_SECTION']!='')
$section_con .= ' and p.SECTION_ID = "'.$_POST['PBI_SECTION'].'" ';
...
if($_POST['JOB_STATUS']!='')
$emp_status_con .= ' and p.PBI_JOB_STATUS = "'.$_POST['JOB_STATUS'].'" ';


	$sql="select p.* from personnel_basic_info p where 1 ".$dept_con.$desg_con.$org_con.$emp_status_con.$section_con." order by p.DEPT_ID";
	$res	 = mysql_query($sql);
	while($row=mysql_fetch_object($res))
	{


		 $new_min_time=date("h:i:s a",strtotime($min_time[$row->PBI_ID]));

		 $max_min_time=date("h:i:s a",strtotime($max_time[$row->PBI_ID]));

		 $i = $t_date;	
		 

		 $office_in_time = strtotime($i.' 10:5:59');
		 $office_out_time = strtotime($i.' 18:00:00');

		 $in = strtotime($min_time[$row->PBI_ID]);
		 $out = strtotime($max_time[$row->PBI_ID]);
		...
		<? if($in>0 || $out>0 && $status = 'Present' and $in>$office_in_time!='late') { ?>
		...
		<!-- <td><?=find_a_field('section','SECTION_NAME','SECTION_ID='.$row->SECTION_ID);?></td> -->
       ...
       <td><? if($max_min_time!=$new_min_time) echo $max_min_time;?></td>

	   <td align="center"><?=find_a_field('hrm_attdump','schedule_notes','EMP_CODE="'.$row->PBI_ID.'" and xdate="'.$t_date.'"');?>


   <td class="new_print" align="center"><?  


         if($in_latitute!='' && $in_longitude!=''){?>

              <a href="https://www.latlong.net/c/?lat=<?=$in_latitute?>&long=<?=$in_longitude?>" target="_blank" class="btn btn-warning btn-xs">
  ...
  
{
   ...
   ?>
		...
		<td  width="100%" style="border:0px; border-color:white;">
        ...
        <tr>
        <td colspan="7" align="center" style="font-size:14px;border:0px; border-color:white;"><strong>
		 Group: <?=find_a_field('ledger_group','group_name','group_id='.$_REQUEST['ledger_group'])?></strong></td>
   ...
   <!--      </tr> -->
		...
		<th width="13%" bgcolor="#FFFACD">SL</th>

		<th width="13%" bgcolor="#FFFACD">EmpCode</th>

		<th width="13%" bgcolor="#FFFACD"> Name </th>

		<th width="12%" bgcolor="#FFFACD">Department</th>

		<th width="12%" bgcolor="#FFFACD">Designation</th>

		<th width="12%" bgcolor="#FFFACD">Date</th>
		<th width="12%" bgcolor="#FFFACD">In Time</th>
		<th width="12%" bgcolor="#FFFACD">Out Time</th>
       ...
       $sql = "select xenrollid,  min(xtime) as min_time from hrm_attdump where xdate ='".$t_date."' group by xenrollid ";
       $query = mysql_query($sql);
       while($info=mysql_fetch_object($query)){
       $min_time[$info->xenrollid]=$info->min_time;

		}

		 $sql = "select xenrollid,  max(xtime) as max_time  from hrm_attdump where xdate ='".$t_date."' group by xenrollid ";

		 $query = mysql_query($sql);
		 while($info=mysql_fetch_object($query)){

  		 $max_time[$info->xenrollid]=$info->max_time;

		 $xenrollid[$info->xenrollid]=$info->xenrollid;
		...
		if($_POST['PBI_DOMAIN']!='')
        $section_con .= " and p.PBI_DOMAIN = '".$_POST['PBI_DOMAIN']."'";

		if($_POST['JOB_STATUS']!='')
        $section_con .= " and p.PBI_JOB_STATUS = '".$_POST['JOB_STATUS']."'";

		 $sql="select p.* from personnel_basic_info p where 1 ".$dept_con.$desg_con.$section_con.$desg_con."";
         $res	 = mysql_query($sql);
      ...
      while($row=mysql_fetch_object($res))
		...
		<? if($xenrollid[$row->PBI_ID]<1 || $in>$office_in_time and $in!='' and $out!='') { $status='Late'?>
        <tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
...

{
		...
		<td  width="100%" style="border:0px; border-color:white;">
			...
			<table width="100%"  >
				...
				<tr align="center" >
					...
					<td colspan="7" style="font-size:14px; border:0px; border-color:white;"><strong><?=$report;?></strong></td>
				</tr>

		 <? if ($_REQUEST['ledger_group']>0) {?>

        <tr>
        <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;"><strong>
		 Group: <?=find_a_field('ledger_group','group_name','group_id='.$_REQUEST['ledger_group'])?></strong></td>
   ...
   <!--      </tr> -->
		...
		<th width="14%" bgcolor="#FFFACD">SL</th>

		<th width="15%" bgcolor="#FFFACD">EmpCode</th>

		<th width="16%" bgcolor="#FFFACD">Name </th>

		<th width="15%" bgcolor="#FFFACD" style=" text-align: center; ">Department</th>

		<th width="16%" bgcolor="#FFFACD">Designation</th>

		<th width="16%" bgcolor="#FFFACD" style=" text-align: center; ">Date</th>
       ...
       $min_time[$info->xenrollid]=$info->min_time;
        ...
        $section_con .= " and p.PBI_JOB_STATUS = '".$_POST['JOB_STATUS']."'";
		 ...
		 $sql="select p.* from personnel_basic_info p where 1 ".$dept_con.$desg_con.$section_con.$desg_con." and  PBI_CODE NOT IN (1006,1007)";
         $res	 = mysql_query($sql);

      while($row=mysql_fetch_object($res))

		{
               $leave_id=find_a_field('hrm_att_summary','count(id)','att_date ="'.$t_date.'" and emp_id="'.$row->PBI_ID.'" and leave_id>0');
        
                
		?>

		<? if($xenrollid[$row->PBI_ID]<1 && $leave_id<1) { $status='Absent'?>
        <tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
...

{
   ...
   <!--      </tr> -->
		...
		<th width="15%" bgcolor="#FFFACD">Emp Code</th>

		<th width="16%" bgcolor="#FFFACD">Name </th>

		<th width="15%" bgcolor="#FFFACD">Department</th>

		<th width="16%" bgcolor="#FFFACD">Designation</th>

		<th width="16%" bgcolor="#FFFACD">Date</th>
    ...
    if(isset($item_type)) 					{$item_type_con=' and i.item_type='.$item_type;}
     ...
     $sql = "select EMP_CODE,  min(xtime) as min_time,latitude,longitude from hrm_attdump where xdate ='".$t_date."' group by EMP_CODE ";
     $query = mysql_query($sql);
     while($info=mysql_fetch_object($query)){
      ...
      while($row=mysql_fetch_object($res))
		...
		<? if($leave[$row->PBI_ID]>0) { $status='Leave'?>
        <tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
...

{
   ...
   <!--      </tr> -->
		...
		<th width="16%" bgcolor="#FFFACD">Date</th>
		<th width="16%" bgcolor="#FFFACD">Status</th>	
     ...
     while($info=mysql_fetch_object($query)){
      ...
      $in_longitudee[$info->EMP_CODE]=$info->longitude;
       ...
       $sql = "select EMP_CODE,  max(xtime) as max_time  from hrm_attdump where xdate ='".$t_date."' group by EMP_CODE ";
		...
		$sql = "select id,PBI_ID,incharge_status  from  hrm_leave_info where  dept_head_status='pending'and incharge_status='pending'and s_date ='".$t_date."'  group by PBI_ID ";
		 ...
		 $leave[$info->PBI_ID]=$info->PBI_ID;
		  $incharge_status[$info->PBI_ID]=$info->incharge_status;
      ...
      while($row=mysql_fetch_object($res))
		...
		<td><?php echo date('d-M-Y',strtotime($t_date));?></td>
		<td><?=$incharge_status[$row->PBI_ID];?></td>
...
elseif($_REQUEST['report']==210907001) {
	...
	echo $sql = "select EMP_CODE,  min(xtime) as min_time,latitude,longitude,sch_latitude_point,sch_longitude_point,schedule_notes from hrm_attdump where xdate ='".$t_date."' group by EMP_CODE ";
     $query = db_query($sql);
     while($info=mysqli_fetch_object($query)){
...
$cost_con .= ' and p.cost_center = "'.$_POST['cost_center'].'" ';

if($_POST['class']!='')
$class_con .= ' and p.class = "'.$_POST['class'].'" ';

if($_POST['JOB_LOC_ID']!='')
$location_con .= ' and p.JOB_LOC_ID = "'.$_POST['JOB_LOC_ID'].'" ';

if($_POST['section']!='')
$section_con .= ' and p.section = "'.$_POST['section'].'" ';
...
if($user_id == 10073 || $user_id == 10074 || $user_id == 10108) $JOB_LOC_ID_BLOCK = " and p.JOB_LOC_ID !=3";



	$sql="select p.* from personnel_basic_info p where 1 ".$dept_con.$JOB_LOC_ID_BLOCK.$cost_con.$org_con.$emp_status_con.
	$location_con.$class_con.$section_con." order by p.class, p.cost_center";
	$res	 = db_query($sql);
	while($row=mysqli_fetch_object($res))
	{
		 ...
		 $out = strtotime($max_time[$row->PBI_ID]);
           ...
           $in_latitute =  $in_latitutee[$row->PBI_ID]; 
             ...
             $in_longitude = $in_longitudee[$row->PBI_ID];
           $in_from_latitute =  $in_from_latitutee[$row->PBI_ID]; 
               $in_from_longitude = $in_from_longitudee[$row->PBI_ID];
...
?>
       ...
       <td style="text-align:center"><?=$row->PBI_CODE?></td>
       <td><?=$row->PBI_NAME?></td>
	   <td><?=find_a_field('designation','DESG_DESC','DESG_ID='.$row->DESG_ID);?></td>
	   ...
	   <td><?=find_a_field('hrm_attdump','schedule_notes','xenrollid="'.$row->PBI_ID.'" and xdate ="'.$t_date.'"');?></td>
...
<?  } //} ?>
...

 {
 		...
 		<th width="9%" bgcolor="#FFFACD">EMP CODE</th>
 		...
 		<th width="25%" bgcolor="#FFFACD">EMP NAME</th>

     <th width="10%" bgcolor="#FFFACD">DEPARTMENT</th>

 		<th width="10%" bgcolor="#FFFACD">Employee Type</th>

 		<th width="5%" bgcolor="#FFFACD">SECTION</th>
 		...
 		<th width="25%" bgcolor="#FFFACD">Status</th>
 ...
 $org_con .= ' and p.PBI_ORG = "'.$_POST['PBI_ORG'].'" ';
  ...
  $sql="select p.* from personnel_basic_info p where 1 ".$dept_con.$desg_con.$org_con.$section_con." order by p.DEPT_ID";
  ...
  $office_in_time = strtotime($i.' 09:15:59');
  ...
  $office_out_time = strtotime($i.' 18:00:00');
 		...
 		<? if($xenrollid[$row->PBI_ID]>0 && $max_min_time==$new_min_time) {?>
 ...
 <?  } }?>
   ...
   ?>
		...
		<th width="25%" bgcolor="#FFFACD">EMPLOYEE NAME </th>
		...
		<th width="16%" bgcolor="#FFFACD">DEPARTMENT</th>
		...
		<th width="14%" bgcolor="#FFFACD">DATE</th>
		...
		<th width="16%" bgcolor="#FFFACD"> IN TIME </th>
		...
		<th width="18%" bgcolor="#FFFACD">OUT TIME </th>
...
if($user_id == 10073 || $user_id == 10074 || $user_id == 10108) $JOB_LOC_ID_BLOCK = " and p.JOB_LOC_ID !=3";
		 ...
		 $sql="select p.* from personnel_basic_info p where 1 ".$cost_con.$class_con.$location_con .$section_con.$dept_con .$org_con.$emp_status_con. $JOB_LOC_ID_BLOCK."";
...
//******************        new       ************ */
  elseif($_REQUEST['report']==210907003)
...
?>
	 ...
	 <table width="100%">
		...
		<tr><td width="25%" align="center" style="border:0px; border-color:white;"><img src="<?=SERVER_ROOT?>public/uploads/logo/tecno.png" style="width:220px;"></td>
	 ...
	 <td  width="50%" style="border:0px; border-color:white;">
		 ...
		 <table width="100%">
          ...
          <tr align="center"><td style="font-size:14px; border:0px; border-color:white;"><strong><?=$report;?></strong></td></tr>





	  <tr><td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white;">For the Date of:<?php echo date('d-m-Y',strtotime($_REQUEST['t_date']));?></td></tr>

      </table> </td>



     <td  width="25%" align="center" style="border:0px; border-color:white;">&nbsp;</td> </tr>



	 <tr><td colspan="15" style="font-size:14px;border:0px; border-color:white;">&nbsp;</td></tr> </table>
     ...
     <th width="9%" bgcolor="#FFFACD">DEPARTMENT NAME</th>

	 <th width="16%" bgcolor="#FFFACD">Section Name</th>

	 <th width="16%" bgcolor="#FFFACD">Designation</th>

     <th width="25%" bgcolor="#FFFACD">Total EMP</th>

     <th width="16%" bgcolor="#FFFACD">Attend EMP</th>

     <th width="16%" bgcolor="#FFFACD">Absent EMP</th>
	 ...
	 $dept_con .= ' and d.DEPT_ID = "'.$_POST['DEPT_ID'].'" ';



	 if($_POST['PBI_SECTION']!='')

	 $section_con .= " and p.PBI_SECTION = '".$_POST['PBI_SECTION']."'";
	 ...
	 ////////////
	  ...
	  /* echo $sql = "select a.xenrollid,count(a.xenrollid) as emp_total from hrm_attdump a,personnel_basic_info p

	 where a.xenrollid=p.PBI_ID ".$dept_con.$section_con." and xdate ='".$t_date."' group by a.xenrollid ";
	 ...
	 //$min_time[$info->xenrollid]=$info->min_time;
      ...
      echo $to_emp = $info->emp_total;



	  echo  $sql="select count(a.xenrollid) as active_emp,p.PBI_SECTION,c.DESG_DESC,c.DESG_ID

	     from personnel_basic_info p,designation c,hrm_attdump a



	  where p.PBI_ID=a.xenrollid  and p.DESG_ID=c.DESG_ID and a.xdate ='".$t_date."'".$dept_con.$section_con." group by c.DESG_ID";
...
//////////////////////
		...
		$sql2="select a.xenrollid,a.xdate

		from hrm_attdump a where a.xdate ='".$t_date."' group by a.xenrollid";



		$res2	 = db_query($sql2);

		while($info=mysqli_fetch_object($res2)){

		$emp_id = $info->xenrollid;
	   ...
	   echo $sql="select COUNT(p.PBI_ID) as active_emp, p.PBI_SECTION,c.DESG_DESC,c.DESG_ID from personnel_basic_info p,designation c

	   where p.DESG_ID=c.DESG_ID ".$dept_con.$section_con." group by c.DESG_ID";
	 ...
	 //$attend_count =find_a_field('hrm_attdump h,personnel_basic_info p','count(h.xenrollid)', 'h.xenrollid=p.PBI_ID and p.DESG_ID="'.$row->DESG_ID.'" and xdate ="'.$t_date.'" group by h.xenrollid');





	 //echo $attend_counttt = count($xenrollid[$row->PBI_ID]);





	 $attend_count =find_a_field('personnel_basic_info','count(PBI_ID)', 'DESG_ID="'.$row->DESG_ID.'"');
	 ...
	 <td><?=$row->DEPT_SHORT_NAME?></td>



	 <td><?=$row->PBI_SECTION?></td>
