...
if(isset($_POST['new'])){
		...
		if($_FILES['att_file']['tmp_name']!=''){
						...
						$path='../vehicle_mod/files/expense/';
						$path2 = 'files/expense/';
...
//db_query($conn, "update office_expense set expense_date='".$_POST['expense_date']."' where expense_no='".$_POST['expense_no']."'");		
...
if(isset($_POST['hold'])){
		...
		?><script>window.location.href='office_expense_unfinished.php';</script><?php 
}
...
?><script>window.location.href='office_expense_status.php';</script><?  
} // End confirm
...
<div class="form-group row">
           ...
           <div class="col-4"><label for="expense_no" class="col-form-label">Claim Date:</label></div> <!--sk-->
            <div class="col-8 mb-1"><div class="col-sm-3"><? $field='expense_date';?><input  class="form-control border border-info" name="<?=$field?>" type="date" id="<?=$field?>" value="<?=$$field?>"/></div></div>
			
			<div class="col-4"><label for="expense_no" class="col-form-label">Items:</label></div> <!--sk-->
            <div class="col-8 mb-1"><div class="col-sm-3"><? $field='items';?><input  class="form-control border border-info" name="<?=$field?>" type="text" id="<?=$field?>" value="<?=$$field?>"/>
			</div></div>
			
			
			<div class="col-4"><label for="expense_no" class="col-form-label">Amount:</label></div> <!--sk-->
            <div class="col-8 mb-1"><div class="col-sm-3"><? $field='amount';?><input  class="form-control border border-info" name="<?=$field?>" type="text" id="<?=$field?>" value="<?=$$field?>"/></div></div>
...
<? 
$res='select a.id,a.items as name,a.qty as qty,a.rate as rate,a.amount as amt,a.att_file,"x" 
from '.$table_details.' a
where a.expense_no='.$_SESSION['expense_no'].' order by a.id desc';
