...
<?
$store_name = find1("select warehouse_name from warehouse where warehouse_id='".$_SESSION['user']['depot']."'");
...
/*
$sql='select a.item_id, sum(a.item_in-a.item_ex) as stock
from journal_item a
where  a.warehouse_id="'.$_SESSION['user']['depot'].'" group by a.item_id ';
$qqq=db_query($sql);
while($aaa=mysqli_fetch_object($qqq)){
//if($aaa->tr_no==5)
//$stock[$aaa->item_id] = 1000;
//else
$stock[$aaa->item_id] = $aaa->stock;
} 
*/
...
if(isset($_POST['confirmm'])){
		...
		$chalan_no = find1("select max(chalan_no)+1 from ss_do_chalan where 1");
		...
		$sql = 'select * from ss_do_details where do_no = '.$do_no.' and (item_id!=1096000100010239 and item_id!=1096000100010312 and item_id!=1096000100010967)';
...
$q = "INSERT INTO  ss_do_chalan (order_no, chalan_no, do_no, item_id, dealer_code, unit_price, pkt_size, pkt_unit, dist_unit, total_unit, total_amt, chalan_date, depot_id, driver_name, driver_name_real,shipping_name, vehicle_no,gate_pass_no, delivery_man, entry_by, entry_at,do_date)
VALUES 
('".$data->id."', '".$chalan_no."', '".$do_no."', '".$data->item_id."', '".$dealer_code."', '".$data->unit_price."', '".$data->pkt_size."', '".$chalan_pkt."', '".$chalan_dist."', '".$unit_qty."', '".$total_amt."', '".$chalan_date."', '".$_SESSION['user']['depot']."', '".$driver_name."',
...
journal_item_ss($data->item_id ,$_SESSION['user']['depot'],$_POST['chalan_date'],0,$unit_qty,'Sales',$ch_id,'','',$chalan_no);
			}
...
//                  VALUES 
//                 ('".$data->id."', '".$chalan_no."', '".$do_no."', '".$data->item_id."', '".$dealer_code."', '".$data->unit_price."', '".$data->pkt_size."', '".$chalan_pkt."', '".$chalan_dist."', '".$unit_qty."', '".$total_amt."', '".$chalan_date."', '".$_SESSION['user']['depot']."', '".$driver_name."','".$driver_name_real."', '".$vehicle_no."', '".$gate_pass_no."', '".$delivery_man."', '".$_SESSION['user']['id']."', '".$now."','".$master->do_date."')";
//                 db_query($conn, $q);
//                 	$ch_id = mysqli_insert_id($conn);
...
redirect2("do_list.php");


} // end submit confirm
} // end duplicate confirm
else
{
	$type=0;
...
<div class="row">
        <div class="col-2"><label for="do_no" class="col-form-label">DO No:</label></div> 
        <div class="col-4"><input class="form-control border border-info" name="do_no" type="text" id="do_no" value="<?=$do_no?>" required="required" readonly/></div>
        
        <div class="col-2"><label for="do_date" class="col-form-label">Date:</label></div>
        <div class="col-4"><input class="form-control border border-info" name="do_date" type="text" id="do_date" value="<?=$do_date?>" required="required" readonly/></div>
</div>    


<div class="form-group row mb-2">
    
            <div class="col-2"><label for="shop" class="col-sm-2 col-form-label">Shop:</label></div> 
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td bgcolor="#9966FF"><strong>
			<input style="width:150px;"  name="chalan_date" type="date" id="chalan_date" readonly="readonly" required="required" value="<?=date('Y-m-d');?>"/>
          </strong></td>
...
<? 
$sql='select a.id,a.item_id,b.finish_goods_code,b.item_name,a.pkt_unit,a.unit_price,a.dist_unit,a.total_unit,a.pkt_size,a.gift_on_item from ss_do_details a,item_info b 
...
<td align="center"><button name="confirmm" type="submit" class="btn btn-success">Confirm Order</button></td>
</tr>
