...
<!-- User list items  -->
<div class="row">
<div class="row text-center mb-2"><h4>Fuel Claim Status<br /><?=$_SESSION['msg'];unset($_SESSION['msg']);?></h4></div>    
...
$sql = "select m.* from fuel_expense m where 1 ".$con." order by m.expense_no asc";
        ...
        <!--<a href="do_checking.php?expense_no=<?=$data->expense_no?>">   -->
            <li class="list-group-item border-0">
                <div class="row">
                    ...
                    <div class="col px-0">
                        <p>Bill No: <?=$data->expense_no?><br><small class="text-secondary"><?=find_a_field('user_activity_management','fname','user_id="'.$data->entry_by.'"')?> ,<?=$data->expense_date?><br /><?=$data->status?></small></p>
                    </div>
