...
$t_address = json_decode($_POST["T_address"]);
...
if ($result->num_rows > 0) {
    ...
    echo "Bill number already used by another user.";
		 ...
		 $sql="INSERT INTO bills (emp_code,conveyance_no,conveyance_type, from_address,to_address,amount,remarks,conveyance_date,
		 	status,entry_by,means) 
		 VALUES ('$emp_code[$i]', '$con_no[$i]' , '$bill_type[$i]', '$f_address[$i]' , '$t_address[$i]','$amount[$i]', '$remarks[$i]', 
		 	'$con_date[$i]', 'Pending', '$PBI_ID[$i]','$means_of[$i]')";
		$query=db_query($sql);
