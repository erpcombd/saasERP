...
if($_FILES['att_file']['tmp_name']!=''){
						...
						$uploaded_file = $path.$_SESSION['jv_no'].'.'.$ext;

						

						$file_name = $_SESSION['jv_no'].'.'.$ext;

						unlink($uploaded_file);

						db_query('delete from document_upload where master_id="'.$_SESSION['jv_no'].'" and tr_from="Receipt"');

						move_uploaded_file($file_tmp, $path.$_SESSION['jv_no'].'.'.$ext);

						

						$file_insert = 'insert into document_upload (`master_id`,`tr_from`,`file_name`,`entry_at`,`entry_by`) value("'.$_SESSION['jv_no'].'","Receipt","'.$uploaded_file.'","'.date('Y-m-d H:i:s').'","'.$_SESSION['user']['id'].'")';
...
<td required><div align="right" ><span class="style3">*</span>Bank A/c Debit:</div></td>
...
<td align="right"><span class="style3"></span>Invoice No:</td>
