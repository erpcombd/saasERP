...
if(isset($_POST['submit'])){
  ...
  $up = db_query('update welcome_note set note="'.$note.'" where id=2');
...
move_uploaded_file($file_temp,$uploaded_image);
  ...
  $up2 = db_query('insert into welcome_images(`att_file`,`status`) value("'.$uploaded_image.'","UNPUBLISHED")');
...
$up2 = db_query('insert into policy(`file_name`,`type`,`att_file`) value("'.$att_file_name.'","'.$type.'","'.$uploaded_image.'")');
...
$up2 = db_query('insert into policy(`file_name`,`type`,`att_file`) value("'.$att_file_name2.'","'.$type2.'","'.$uploaded_image.'")');
...
$up2 = db_query('insert into policy(`file_name`,`type`,`att_file`) value("'.$att_file_name3.'","'.$type3.'","'.$uploaded_image.'")');
...
if($_GET['del']>0){
   ...
   db_query('delete from policy where id='.$_GET['del']);
...
if($_GET['delete']>0){
   ...
   db_query('delete from welcome_images where id='.$_GET['delete']);
 
}

if($_GET['update']>0){
   
   db_query('update welcome_images set status="UNPUBLISHED" where 1');
   db_query('update welcome_images set status="PUBLISHED" where 1 and id='.$_GET['update']);
...
<div class="form-group"> 
 ...
 <?
   ...
   $qr2 = db_query($select2);
   while($data2 = mysqli_fetch_object($qr2)){
