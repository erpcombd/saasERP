...
var win = window.location.assign('../kpi/performance_appraisal.php?id='+lk, '');
  win.focus();
}</script>
...
<div class="oe_view_manager oe_view_manager_current">
  ...
  $res = "select p.PBI_ID as Emp_id,p.PBI_NAME,desg.DESG_DESC,(select DEPT_DESC from department where DEPT_ID=p.PBI_DEPARTMENT) as department,(select PROJECT_DESC from project where PROJECT_ID=p.JOB_LOCATION) as project,(select status from performance_appraisal where PBI_ID=p.PBI_ID and year=2021) as status from personnel_basic_info p, designation desg, essential_info e where p.PBI_DESIGNATION=desg.DESG_ID and p.PBI_ID=e.PBI_ID and p.PBI_JOB_STATUS='In Service' and e.ESSENTIAL_REPORTING=".$_SESSION['employee_selected'];
