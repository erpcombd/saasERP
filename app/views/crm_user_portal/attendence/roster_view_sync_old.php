...
if($PBI_ORG>0) $ORG_con = " and p.PBI_ORG='".$PBI_ORG."'";
	...
	WHERE p.PBI_ID=h.emp_id and att_date BETWEEN '".$start_date."' AND '".$end_date."' 
	".$ORG_con.$emp_id_con."";
...
and sc.id=ro.shedule_1 and 
p.PBI_ID=h.xenrollid and 
xdate BETWEEN '".$start_date."' AND '".$end_date."' and 
h.xmechineid=m.mac_id and 
m.mac_type!='Out' ".$ORG_con.$emp_id_con."
...
$sql = "SELECT xenrollid , xdate , max(xtime) out_time  FROM `hrm_attdump` h, hrm_machice_type m,personnel_basic_info p
	WHERE p.PBI_ID=h.xenrollid and xdate BETWEEN '".$start_date."' AND '".$end_date."' and h.xmechineid=m.mac_id and m.mac_type!='In' 
	".$ORG_con.$emp_id_con."
	GROUP BY `xenrollid`,`xdate`";
...
{
$ots = (int)((strtotime($out_time[$emp_ids[$x]][$att_date[$x]]) - (strtotime($att_date[$x].' '.$office_end_time[$x])))/3600);$ot = ($ots>0)?$ots:0;

$sql="INSERT INTO hrm_att_summary2 
	(emp_id, att_date, in_time,out_time, sch_in_time, sch_out_time, ot, dayname)
	VALUES 
('".$emp_ids[$x]."','".$att_date[$x]."', '".$in_time[$x]."','".$out_time[$emp_ids[$x]][$att_date[$x]]."','".$office_start_time[$x]."','".$office_end_time[$x]."','".$ot."', dayname('".$att_date[$x]."'))";
}
	else
{
$ots = (int)((strtotime($out_time2[$emp_ids[$x]][$att_date[$x]]) - strtotime($att_date[$x].' '.$office_end_time[$x])+86400)/3600);$ot = ($ots>0)?$ots:0;
$sql="INSERT INTO hrm_att_summary2 
	(emp_id, att_date, in_time,out_time, sch_in_time, sch_out_time, ot,  dayname)
	VALUES 
('".$emp_ids[$x]."','".$att_date[$x]."', '".$in_time[$x]."','".$out_time2[$emp_ids[$x]][$att_date[$x]]."','".$office_start_time[$x]."','".$office_end_time[$x]."','".$ot."', dayname('".$att_date[$x]."'))";
}
	$query=db_query($sql);
...
<tr>
  ...
  <!--<input type="text" name="emp_id" id="emp_id" value="<?=$_POST['emp_id']?>" />-->
	...
	<?php auto_dropdown("select PBI_ID,concat(PBI_ID,'-',PBI_NAME) from personnel_basic_info 
	where PBI_ORG='".$_SESSION['user']['group']."' and JOB_LOCATION not in('1')"); ?>
	</select>
  </td>
</tr>
<tr>
