...
<div class="form-container_large">
  ...
  <div class="row m-0">
    <div class="col-md-12 p-0 mob">
      <p style=" text-align: center; font-weight: bold; margin-top: 20px; font-size: 25px; text-decoration: underline;">Attendance Statistics For :
        <?=date('Y')?>
        -
        <?=date('F')?>
      </p>
      <table id="example" class="table table-bordered scrollbar table-sm" style="font-size: 12px;">
        <thead style="background: #1ABB9C;">
          <tr>
            ...
            <th style="color: white;">In Time Image</th>
            <th style="color: white;">Out Time</th>
            <th style="color: white;">Out Time Location</th>
            <th style="color: white;">Out Time Image</th>
          </tr>
        </thead>
        <tbody>
          <?
...
?>
            ...
            <td><? $in_time = find_a_field('hrm_attdump','min(ztime)','bizid="'.$PBI_ID.'" and xdate="'.$date.'" group by xdate'); if($in_time!=''){ echo $in_time;}else{ if($off_day=='Fri'){ echo '<span class="btn btn-success btn-xs">Day Off</span>'; }else{ echo '<span class="btn btn-danger btn-xs">Absent</span>'; } }?></td>
...
'bizid="'.$PBI_ID.'" and xdate="'.$date.'" and ztime="'.$in_time.'"'); if($in_latitute!='' && $in_longitude!=''){?>
            ...
            <td><?  $in_id = find_a_field('hrm_attdump','sl','bizid="'.$PBI_ID.'" and xdate="'.$date.'" and ztime="'.$in_time.'"'); $in_file_make = '../../../../attandance_image/'.$in_id.'.png'; if(file_exists($in_file_make)>0){ ?>
              ...
              <? } ?></td>
            <td><? $out_time = find_a_field('hrm_attdump','max(ztime)','bizid="'.$PBI_ID.'" and xdate="'.$date.'" group by xdate'); if($out_time!=''){ echo $out_time;}else{ if($off_day=='Fri'){ echo '<span class="btn btn-success btn-xs">Day Off</span>'; }else{ echo '<span class="btn btn-danger btn-xs">Absent</span>'; } }?></td>
...
'bizid="'.$PBI_ID.'" and xdate="'.$date.'" and ztime="'.$out_time.'"'); if($out_latitute!='' && $out_longitude!=''){?>
            ...
            <td><?  $out_id = find_a_field('hrm_attdump','sl','bizid="'.$PBI_ID.'" and xdate="'.$date.'" and ztime="'.$out_time.'"'); $out_file_make = '../../../../attandance_image/'.$out_id.'.png'; if(file_exists($out_file_make)){ ?>
              <a href="<?=$out_file_make?>"  target="_blank" class="btn btn-info btn-xs">View</a>
              <? } ?></td>
          </tr>
          <? } ?>
        </tbody>
