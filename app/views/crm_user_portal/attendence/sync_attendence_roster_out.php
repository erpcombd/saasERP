...
$sql = "SELECT xenrollid , xdate , min(`xtime`) in_time,max(xtime) out_time, office_start_time sch_in_time,office_end_time sch_out_time FROM `hrm_attdump` h, hrm_machice_type m,personnel_basic_info p, hrm_roster_allocation r, hrm_schedule_info s
	WHERE p.PBI_ID = r.PBI_ID AND r.roster_date = h.xdate AND r.shedule_1=s.id AND p.PBI_ID=h.xenrollid and xdate BETWEEN '".$start_date."' AND '".$end_date."' and h.xmechineid=m.mac_id and m.mac_type!='In' 
	".$ORG_con.$emp_id_con."
...
WHERE p.PBI_ID=h.xenrollid and xtime BETWEEN concat(h.xdate,' 00:00:00') AND  concat(h.xdate,' 10:00:00') 
and h.xmechineid=m.mac_id and m.mac_type!='In' ".$ORG_con.$emp_id_con." 
and xdate BETWEEN '".$start_date."' AND '".$end_date."' GROUP BY `xenrollid` , `xdate`";

$query2 = db_query($sql2);
while($data2 = mysqli_fetch_object($query2)){
$att_date = date('Y-m-d',(strtotime($data2->xdate) - 86400));
$out_time[$data2->xenrollid][$att_date] = $data2->out_time;
}


for($x=1;$x<=$sl;$x++)
{
$out = $out_time[$value[$x]['emp_id']][$value[$x]['att_date']];
$sql='update hrm_att_summary set out_time="'.$out.'"
