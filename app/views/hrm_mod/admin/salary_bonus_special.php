...
$pre_del='delete from salary_bonus where bonus_type='.$_POST['bonus_type'].' and year='.$_POST['year'];
		...
		db_query($pre_del);
		 ...
		 $sql = 'select p.*, s.gross_salary, s.basic_salary, s.cash_bank
		  
		  from personnel_basic_info p, salary_info s
		  
		  where p.PBI_ID=s.PBI_ID and p.PBI_JOB_STATUS="In Service" and s.gross_salary>0  and p.PBI_ORG='.$_POST['PBI_ORG'].$con; // and p.PBI_ID=1867
		...
		$gross_salary=$datas->gross_salary;
		...
		$bonus_percentage=$_POST['bonus_percentage'];
		 ...
		 $bonus_given_by = $datas->cash_bank;
		...
		$bonusAmount=$gross_salary;
		   ...
		   if($bonus_days>=180){
			...
			$bonus_amt=round(($bonusAmount*50)/100);
		...
		$bonus_amt=0;
		 $bonus_percent='';
		  ...
		  // elseif($bonus_days>=30 & $bonus_days<=180){
//
//
//
//		   $bonus_amt=round(($bonusAmount*50)/100);
...
//
//		   if($bonus_given_by=="Cash+Bank"){
//           $cash_bonus = round(($datas->basic_salary_cash*25)/100);
//           $bank_bonus = round(($datas->basic_salary_bank*25)/100);
//
//	       }elseif($bonus_given_by=="Cash"){
//
//		   $cash_bonus = $bonus_amt;
...
//
//
//		   elseif($bonus_days<30 && $bonus_days>=10){
//           $bonus_amt=1000;
//           $bonus_percent='';
//
//		   
...
$insSql='INSERT INTO salary_bonus( bonus_type, year, PBI_ID, pbi_department, pbi_designation, pbi_organization, pbi_job_location, pbi_doj, job_status, basic_salary, bonus_percentage, cut_off_date, bonus_days, bonus_amt, bank_paid, cash_paid,payroll_card_paid,bkash_paid,entry_by, entry_at, job_period, bonus_percent, gross_salary,bank_or_cash,mon,bonus_date) 



VALUES ("'.$bonus_type.'", "'.$year.'", "'.$PBI_ID.'", "'.$pbi_department.'", "'.$pbi_designation.'", "'.$pbi_organization.'", "'.$pbi_job_location.'", "'.$pbi_doj.'", "'.$job_status.'", "'.$basic_salary.'", "'.$bonus_percentage.'", "'.$cut_off_date.'", "'.$bonus_days.'", "'.$bonus_amt.'", "'.$bank_bonus.'", "'.$cash_bonus.'","'.$paroll_card_bonus.'","'.$bkash_bonus.'", "'.$entry_by.'", "'.$entry_at.'", "'.$job_period.'", "'.$bonus_percent.'", "'.$gross_salary.'","'.$bonus_given_by.'","'.$_POST['mon'].'","'.$bonus_date.'")';
...
top: 5px;
                                      ...
                                      <div  class="oe_view_manager_view_list">
                                        <div  class="oe_list oe_view">
                                          <table width="100%" border="0" class="oe_list_content">
                                           <tbody>
                                              <tr>
                                                <td width="10%" align="left" class="alt"><strong>Bonus Type  : </strong></td>
                                                <td width="20%" align="left" class="alt"><strong>
                                                  <select name="bonus_type" required = "required" style="width:160px;">
                                                    <? foreign_relation('bonus_type','id','type',$_POST['bonus_type'],'1');?>
                                                  </select>
                                                  </strong></td>
                                                <td  align="right" class="alt"><strong>Bonus % </strong>: </td>
                                              ...
                                              <tr  class="alt">
                                                <td height="53" align="left">Cut off Date : </td>
                                                <td align="left"><input name="cut_off_date" type="text" id="cut_off_date" value="<?=$_POST['cut_off_date']?>"  style="width:20px;" placeholder="YY-mm-dd"/>
                                                 </td>
    ...
    $ss = "select bonus_type from salary_bonus where year='$yy' and bonus_type='".$_POST['bonus_type']."'";
  ...
  ?>
		...
		if($_POST['bonus_type']!=''){  $con .= " and p.bonus_type = '".$_POST['bonus_type']."'";}
		 ...
		 $sqli = 'select p.year,p.bonus_type,p.PBI_ID,per.PBI_CODE, p.cut_off_date,p.pbi_department,p.pbi_designation,p.pbi_job_location,p.bonus_type,p.basic_salary,d.DESG_DESC,dept.DEPT_DESC,per.PBI_NAME,p.gross_salary,p.job_period,p.bonus_percent,p.remarks ,p.bonus_amt
		 
from salary_bonus p,designation d,department dept,personnel_basic_info per 

where p.pbi_designation=d.DESG_ID AND per.PBI_JOB_STATUS="In Service" and p.pbi_department=dept.DEPT_ID '.$con.' and p.PBI_ID=per.PBI_ID and p.lock_status=0 order by p.id desc';
...
$relieving_date = find_a_field('essential_info','ESSENTIAL_RESIG_DATE','PBI_ID='.$info->PBI_ID);
...
?>
                                              <tr style="font-size:10px; padding:3px; ">
                                                ...
                                                <td><?=find_a_field('project','PROJECT_DESC','PROJECT_ID="'.$info->pbi_job_location.'"');?></td>
                                                <td align="center"><?=$info->job_period?></td>
                                                <td><input type="text" value="<?=$info->bonus_percent?>" name="bonus_percent_<?=$info->PBI_ID?>" id="bonus_percent_<?=$info->PBI_ID?>" style="width:100%;"/>
                                                  ...
                                                  <input type="hidden" value="<?=$info->bonus_type?>" name="bonus_type_<?=$info->PBI_ID?>" id="bonus_type_<?=$info->PBI_ID?>" />
                                                  <input type="hidden" value="<?=$info->year?>" name="year_<?=$info->PBI_ID?>" id="year_<?=$info->PBI_ID?>" />
                                                </td>
												<td><input type="text" value="<?=$info->gross_salary?>" name="gross_salary_<?=$info->PBI_ID?>" id="gross_salary_<?=$info->PBI_ID?>" style="width:100%;"/>
                                                </td>
												<td><input type="text" value="<?=$info->bonus_amt?>" name="bonus_amt_<?=$info->PBI_ID?>" id="bonus_amt_<?=$info->PBI_ID?>" style="width:100%;"/>
                                                </td>
                                                <td align="center"><input type="text" name="remarks_<?=$info->PBI_ID?>" id="remarks_<?=$info->PBI_ID?>" value="<?=$info->remarks?>" style="width:100%;" />
                                                </td>
                                              ...
                                              <? } if($count==0 || $count==''){ ?>
                                              <tr>
                                                <td colspan="9"><div align="center" style="color:red; font-size:16px; font-weight:bold;">Bonus locked or not generated for this period</div></td>
                                              </tr>
                                              <? }?>
                                          </table>
                                          <? } }  ?>
