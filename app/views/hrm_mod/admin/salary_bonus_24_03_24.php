...
$pre_del='delete from salary_bonus where bonus_type='.$_POST['bonus_type'].' and year='.$_POST['year'];
		...
		$bonusAmount=$basic_salary;
		   ...
		   elseif($bonus_days>=30 & $bonus_days<=164){



		   $bonus_amt=round(($bonusAmount*25)/100);
...
top: 5px;
  ...
  ?>
		...
		if($_POST['bonus_type']!=''){  $con .= " and p.bonus_type = '".$_POST['bonus_type']."'";}
		 ...
		 $sqli = 'select p.year,p.bonus_type,p.PBI_ID,per.PBI_CODE, p.cut_off_date,p.pbi_department,p.pbi_designation,p.pbi_job_location,p.bonus_type,p.basic_salary,d.DESG_DESC,dept.DEPT_DESC,per.PBI_NAME,p.job_period,p.bonus_percent,p.remarks 
