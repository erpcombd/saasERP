...
if(isset($_POST['submit'])){
        ...
        $pre_del='delete from salary_bonus where pbi_organization = '.$_POST['PBI_ORG'].' and  bonus_type='.$_POST['bonus_type'].' and   year='.$_POST['year'];
		...
		db_query($pre_del);
		 ...
		 $sql = 'select p.*, s.gross_salary, s.basic_salary, s.cash_bank,s.cash_amt,s.bank_amt
		...
		$bonus_percentage=$_POST['bonus_percentage'];
		 ...
		 $bonus_given_by_cash = $datas->cash_amt;
		 
		 $bonus_given_by_bank = $datas->bank_amt;
		...
		$bonusAmount=$gross_salary;
          ...
          // $bonus_amt=round($bonusAmount);
		   ...
		   if($bonus_given_by_cash > 0 && $bonus_given_by_bank>0 ){

		   $cash_bonus = round($datas->basic_salary_cash);

		   $bank_bonus = round($datas->basic_salary_bank);

	       }elseif($bonus_given_by_cash>0){

		   $cash_bonus = $bonus_amt;

		   $bank_bonus = 0;

	       }elseif($bonus_given_by_bank>0){
...
top: 5px;
                                          ...
                                          <table width="100%" border="0" class="oe_list_content">
                                           <tbody>
                                              <tr>
                                                ...
                                                <td  align="right" class="alt"><strong>Bonus On Gross Salary % </strong>: </td>
                                              ...
                                              <tr  class="alt">
                                                <td height="53" align="left">Up To Date : </td>
                                                <td align="left"><input name="cut_off_date" type="text" id="cut_off_date" value="<?=$_POST['cut_off_date']?>"  style="width:20px;" placeholder="YY-mm-dd" required/>
                                                 </td>
  ...
  ?>
		...
		if($_POST['PBI_ORG']!=''){  $con .= " and per.PBI_ORG = '".$_POST['PBI_ORG']."'";}
		 ...
		 $sqli = 'select p.year,p.bonus_type,p.PBI_ID,per.PBI_CODE, p.cut_off_date,p.pbi_department,p.pbi_designation,p.pbi_job_location,p.bonus_type,p.basic_salary,p.pbi_organization,
		 d.DESG_DESC,dept.DEPT_DESC,per.PBI_NAME,p.gross_salary,p.job_period,p.bonus_percent,p.remarks ,p.bonus_amt
		 
from salary_bonus p,designation d,department dept,personnel_basic_info per 

where p.pbi_designation=d.DESG_ID AND   p.pbi_department=dept.DEPT_ID '.$con.' and p.PBI_ID=per.PBI_ID and p.lock_status=0 order by p.id desc';
...
?>
                                              ...
                                              <? } if($count==0 || $count==''){ ?>
                                              <tr>
                                                <td colspan="10"><div align="center" style="color:red; font-size:16px; font-weight:bold;">Bonus locked or not generated for this period</div></td>
                                              </tr>
                                              <? }?>
                                          </table>
                                          <? } }  ?>
