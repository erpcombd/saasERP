...
var emp_code			=document.getElementById('emp_code_'+id).value;
var emp_name			=document.getElementById('emp_name_'+id).value; 
var emp_mobile			=document.getElementById('emp_mobile_'+id).value; 
...
var strURL="join_ajax.php?dealer_code="+dealer_code+"&qty="+qty+"&purpose="+purpose+"&replacement_code="+replacement_code+"&avg_sales="+avg_sales+"&remarks="+remarks+"&mon="+mon+"&emp_code="+emp_code+"&emp_name="+emp_name+"&emp_mobile="+emp_mobile+"&year="+year+"&status="+sta;
...
var strURL="notjoin_ajax.php?dealer_code="+dealer_code+"&qty="+qty+"&purpose="+purpose+"&replacement_code="+replacement_code+"&avg_sales="+avg_sales+"&remarks="+remarks+"&mon="+mon+"&emp_code="+emp_code+"&emp_name="+emp_name+"&emp_mobile="+emp_mobile+"&year="+year+"&status="+sta;
...
<tr class="oe_list_header_columns">
  <th colspan="6"><span style="text-align: center; font-size:18px; color:#09F">Upcoming Employee </span></th>
  </tr>
<tr class="oe_list_header_columns">
  ...
  <tr >
    ...
    <td colspan="3"><span class="oe_form_group_cell">
      <select required="required" name="PBI_GROUP" id="PBI_GROUP">
        <? 
...
<option value="<?=$month_find;?>"><?=$monthName;?></option>
    ...
    <td><span class="oe_form_group_cell">
      <select  name="PBI_BRANCH" id="PBI_BRANCH"   onchange="getData2('ajax_zone.php', 'loc', this.value,  this.value)">
        <?
...
if($_POST['PBI_BRANCH']){ $region_con = "and region_id='".$_POST['PBI_BRANCH']."'"; }
if($_POST['PBI_ZONE']){ $zone_con = "and zone_id='".$_POST['PBI_ZONE']."'"; }
	
$sql = "select d.*, r.* from dealer_info d,hrm_requisition r
where d.dealer_code=r.dealer_code and d.product_group='".$_POST['PBI_GROUP']."' 
".$region_con.$zone_con."
and d.canceled='Yes' and status in ('send') order by d.dealer_code";
...
value="<?=($data->qty=='')?$att->qty:$data->qty;?>" size="2" maxlength="2" readonly="readonly"/></td>

<td align="center">
<select name="purpose_<?=$info->dealer_code?>" id="purpose_<?=$info->dealer_code?>"  readonly="readonly">
...
style="font-size:10px; width:100px; min-width:10px;" 
value="<?=($data->replacement_code=='')?$att->replacement_code:$data->replacement_code;?>" size="2"  readonly="readonly"/></td>
...
value="<?=($data->avg_sales=='')?$att->avg_sales:$data->avg_sales;?>" size="4"  readonly="readonly"/></td>
...
style="font-size:10px; width:100px; min-width:20px;" 
 value="<?=$att->remarks;?>" size="10"   readonly="readonly"/></td>
 
<td align="center"><input name="emp_code_<?=$info->dealer_code?>" type="text" id="emp_code_<?=$info->dealer_code?>" 

style="font-size:10px; width:100px; min-width:7px;" 
value="<?=($data->emp_code=='')?$att->emp_code:$data->emp_code;?>" size="2"  readonly="readonly"/></td>


<td align="center"><input name="emp_name_<?=$info->dealer_code?>" type="text" id="emp_name_<?=$info->dealer_code?>" 
style="font-size:10px; width:100px; min-width:20px;" 
 value="<?=$att->emp_name;?>" size="10"  readonly="readonly"></td>
 
 
 
<td align="center"><input name="emp_mobile_<?=$info->dealer_code?>" type="text" id="emp_mobile_<?=$info->dealer_code?>" 
style="font-size:10px; width:100px; min-width:20px;" 
 value="<?=$att->emp_mobile;?>" size="10"  readonly="readonly"></td>
...
if($status=='send'){
		...
		<input type="button" name="Button" value="Join"  
		onclick="cal_all(<?=$info->dealer_code?>), update_value(<?=$info->dealer_code?>)" style="font-size:10px; background-color:#00FF00"/>
		
		<input type="button" name="Button" value="Not Join"  onclick="not_join_value(<?=$info->dealer_code?>)" style="font-size:10px;"/>
		<?
		  }
		  else echo 'Saved';
		  }
