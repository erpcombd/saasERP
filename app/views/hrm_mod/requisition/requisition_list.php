...
var strURL="approve_ajax.php?dealer_code="+dealer_code+"&qty="+qty+"&purpose="+purpose+"&replacement_code="+replacement_code+"&avg_sales="+avg_sales+"&remarks="+remarks+"&mon="+mon+"&year="+year+"&status="+sta;
...
".$region_con.$zone_con."
and d.canceled='Yes' order by d.dealer_code";
...
if($att->status=='apply')
$status='Approve';
elseif($att->status=='send') $status='send';
...
<? 
  ...
  if($status=='Approve')
  {
  if($_SESSION['user']['level']==5||$_SESSION['user']['level']==59)
  { ?>
