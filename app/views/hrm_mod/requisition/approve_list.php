...
var strURL="hr_approve_ajax.php?dealer_code="+dealer_code+"&qty="+qty+"&purpose="+purpose+"&replacement_code="+replacement_code+"&avg_sales="+avg_sales+"&remarks="+remarks+"&mon="+mon+"&emp_code="+emp_code+"&emp_name="+emp_name+"&emp_mobile="+emp_mobile+"&year="+year+"&status="+sta;
  ...
  <tr >
    ...
    <td colspan="3"><span class="oe_form_group_cell">
      <select required name="PBI_GROUP" id="PBI_GROUP">
		...
		foreign_relation('product_group','group_name','group_name',$_POST['PBI_GROUP'],' 1 ');
...
$sql = "select d.* from dealer_info d,hrm_requisition r
where d.dealer_code=r.dealer_code and year='".$_POST['year']."'  and mon='".$_POST['mon']."' 
and d.product_group='".$_POST['PBI_GROUP']."' 
".$region_con.$zone_con."
and d.canceled='Yes' and status in ('approve','send') order by d.dealer_code";
...
<input type="button" name="Button" value="Cancel"  onclick="delete_value(<?=$info->dealer_code?>)" style="font-size:10px;"/>
<?
  }
  else echo 'Saved';
  }
