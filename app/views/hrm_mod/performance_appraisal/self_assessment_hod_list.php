...
$title='Upcomming Self Assessment List(HOD)';
...
$_SESSION['notify'][774]=find_a_field('self_assessment_appraisal','count(id)','status="CHECKED" and   employee_id in(select EMP_ID from personnel_basic_info where incharge_id="'.$hod_pbi.'")');
?>
<?php 
  ...
  <form action="" method="post" name="codz" id="codz">
    ...
    <table width="80%" border="0" align="center">
      ...
      <tr>
        ...
        <td width="" bgcolor="#FF9966"><strong>
...
<table class="table table-bordered table-sm" id="sales_list">
	...
	if($_SESSION['user']['id']==1 || $_SESSION['user']['id']==10130){
		  echo $sql="select * from self_assessment_appraisal where status in('CHECKED')    order by id desc ";
	}
	else{
	    $sql="select * from self_assessment_appraisal where status in('CHECKED') and  employee_id='".$cus_emp_id."'  order by id desc ";
	   }
	$query=db_query($sql);
	...
	?>
		<tr>
		 ...
		 <td><?=$data->from_date." To ".$data->to_date?></td>
			<td><a href="self_assessment_view_hod.php?emp_id=<?php echo $pbi_all->PBI_ID;?>&id=<?php echo $data->id;?>"  ><input type="button" value="View" /></a></td>
