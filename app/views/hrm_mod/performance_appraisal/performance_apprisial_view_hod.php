...
$all_edu=find_all_field('hrm_education_detail','*','PBI_ID="'.$all_data->PBI_ID.'" order by EDUCATION_D_ID desc ');
 ...
 if(isset($_POST['submit'])){
 	...
 	$all_sql='select * from pa_criteria';
			...
			echo $ins_sql='insert into performance_appraisal_details(employee_id,pa_id,criteria,outstanding,excellent,good,average,below_average,unsatisfactory,entry_by)values("'.$employee_id.'","'.$pa_id.'","'.$criteria.'","'.$outstanding.'","'.$excellent.'","'.$good.'","'.$average.'","'.$below_average.'","'.$unsatisfactory.'","'.$_SESSION['user']['id'].'")';
			db_query($ins_sql);
			
				echo $ins_sql2='insert into performance_appraisal_details_history(employee_id,pa_id,criteria,outstanding,excellent,good,average,below_average,unsatisfactory,entry_by)values("'.$employee_id.'","'.$pa_id.'","'.$criteria.'","'.$outstanding.'","'.$excellent.'","'.$good.'","'.$average.'","'.$below_average.'","'.$unsatisfactory.'","'.$_SESSION['user']['id'].'")';
			db_query($ins_sql2);
			}
			$_POST['incharge_entry_by']=$_SESSION['user']['id'];
			$ins_sql_two='insert into performance_appraisal_details_two (employee_id,pa_id,group_involvement,increment,promotion,confirmation,extension_of_confirmation,termination,others,other_recommendation,increment_amount,promotion_to,incharge_comments,incharge_entry_by)values("'.$_POST['PBI_ID'].'","'.$_GET['id'].'","'.$_POST['group_involvement'].'","'.$_POST['increment'].'","'.$_POST['promotion'].'","'.$_POST['confirmation'].'","'.$_POST['extension_of_confirmation'].'","'.$_POST['termination'].'","'.$_POST['others'].'","'.$_POST['other_recommendation'].'","'.$_POST['increment_amount'].'","'.$_POST['promotion_to'].'","'.$_POST['incharge_comments'].'","'.$_POST['incharge_entry_by'].'")';
			db_query($ins_sql_two);
			
			
			$up_sql='update performance_appraisal set status="VERIFIED" where id="'.$_GET['id'].'"';
			db_query($up_sql);
			header("Location: performance_apprisial_view_hod_view.php?id=".$_GET['id']);
 }
...
.first_part, .second_part{
  ...
  <body>
    ...
    <section >
	 ...
	 <div class="container"> 
				...
				<div class="row m-0 p-0 " style="padding:10px;">
					<h4 style="padding:4px!important;font-weight:bold; text-align:center; padding-bottom:20px !important;">Necessary Information (During appraisal period):</h4> 
	...
	<!--	<h1 >Total Point <span id="point_gain"></span></h1>-->
		<!-- Your existing HTML/PHP code goes here -->
...
//var totalSum = 0;
//
//// Function to calculate the sum for a specific row
//function calculate_out_sum(rowId) {
//    var rowTotal = 0;
//
//    // Get all checkboxes whose name contains 'outstanding_' for the given row
//    var checkboxes = document.querySelectorAll('input[name^="outstanding_' + rowId + '"]');
//    
//    // Iterate through checkboxes and sum up the values
//    checkboxes.forEach(function (checkbox) {
//        if (checkbox.checked) {
//            // Parse the value as an integer and add it to the rowTotal
//            rowTotal += parseInt(checkbox.value, 10);
//			//alert(parseInt(checkbox.value, 10));
//        }
//    });
//
//    // Update the totalSum with the rowTotal
//    totalSum += rowTotal;
//
//    // Display the totalSum in the console (you can modify this part based on your requirement)
//    //console.log("Total Sum of Outstanding checkboxes: " + totalSum);
//document.getElementById("point_gain").innerText=totalSum;
//    // You can use 'totalSum' variable as needed, for example, updating another input field or displaying it on the page.
//}
//
//// Call the function when the document is ready
//document.addEventListener("DOMContentLoaded", function () {
//    // Add click event listeners to all checkboxes whose id starts with 'outstanding_'
//    var checkboxes = document.querySelectorAll('input[id^="outstanding_"]');
//    
//    checkboxes.forEach(function (checkbox) {
//        checkbox.addEventListener("click", function (event) {
//            // Check if the event is already being processed
//            if (event.detail === 1) {
//                // Extract the row ID from the checkbox id
//                var rowId = checkbox.id.replace("outstanding_", "");
//                
//                // Call the function to calculate the sum for the clicked checkbox's row
//                calculate_out_sum(rowId);
//            }
//        });
//    });
//});
...
//</script>
		...
		<table class="table table-bordered table-sm" style="width:100%;">
			...
			?>
				<tr>
					...
					$check_outstanding=find_a_field('performance_appraisal_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and outstanding=5');
					...
					<?php 
					$check_excellent=find_a_field('performance_appraisal_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and excellent=4');
				    ...
				    <td><input type="checkbox"
					<?php 
					$check_good=find_a_field('performance_appraisal_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and good=3');
				    ...
				    <td><input type="checkbox" 
					<?php 
					$check_average=find_a_field('performance_appraisal_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and average=2');
				    ...
				    <td><input type="checkbox"
						<?php 
					$check_below_average=find_a_field('performance_appraisal_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and below_average=1');
				    ...
				    <td><input type="checkbox"
					<?php 
					$check_unsatisfactory=find_a_field('performance_appraisal_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and unsatisfactory!=""');
		...
		<tr>
			...
			<table>
				<tr>
					<td><input type="checkbox"  id="increment" name="increment" value="1" style="height: 19px;width: 29px;"> <span style="vertical-align:top;">Increment</span></td>
					<td>&nbsp;&nbsp;</td>
					<td><input type="checkbox"  id="extension_of_confirmation" name="extension_of_confirmation" value="1" style="height: 19px;width: 29px;"><span style="vertical-align:top;"> Extension of Confirmation</span></td>
				</tr>
				<tr>
					<td><input type="checkbox"  id="promotion" name="promotion" value="1" style="height: 19px;width: 29px;"> <span style="vertical-align:top;">Promotion</span></td>
					<td>&nbsp;&nbsp;</td>
					<td><input type="checkbox"  id="termination" name="termination" value="1" style="height: 19px;width: 29px;"> <span style="vertical-align:top;">Termination</span></td>
				</tr>
				<tr>
					<td><input type="checkbox"  id="confirmation" name="confirmation" value="1" style="height: 19px;width: 29px;"> <span style="vertical-align:top;">Confirmation</span></td>
					<td>&nbsp;&nbsp;</td>
					<script>
					function set_other(){
    ...
    document.getElementById("other_recommendation").style.display = vis;
					...
					<td><input type="checkbox"  id="others" name="others" onClick="set_other()" value="1" style="height: 19px;width: 29px;"> <span style="vertical-align:top;">Others</span>
					<input type="text" name="other_recommendation" id="other_recommendation" style="display:none;float: right;">
					</td>
				</tr>
			</table>
			...
			<tr>
			<td>a) Pay Increase Amount: TK. <input type="text" name="increment_amount" id="increment_amount" style="background:#a1f9a4;"></td>
		    <td>b) Promotion To- <input type="text" name="promotion_to" id="promotion_to" style="background:#aae9f3;"></td>
		  </tr>
			<tr>
			<td colspan="2">Comments (if any)<br>
			<textarea style="height: 70px;width: 80%;" name="incharge_comments" id="incharge_comments"></textarea>
			</td>
		</tr>
		</tbody>
		</table>
