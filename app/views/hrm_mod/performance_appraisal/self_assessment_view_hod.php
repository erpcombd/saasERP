...
$all_edu=find_all_field('hrm_education_detail','*','PBI_ID="'.$all_data->PBI_ID.'" order by EDUCATION_D_ID desc ');
 ...
 if(isset($_POST['submit'])){
 	...
 	$all_sql='select * from pa_criteria';
			...
			echo $ins_sql='insert into self_assessment_details(employee_id,pa_id,criteria,outstanding,excellent,good,average,below_average,unsatisfactory,entry_by)values("'.$employee_id.'","'.$pa_id.'","'.$criteria.'","'.$outstanding.'","'.$excellent.'","'.$good.'","'.$average.'","'.$below_average.'","'.$unsatisfactory.'","'.$_SESSION['user']['id'].'")';
			db_query($ins_sql);
			
				echo $ins_sql2='insert into self_assessment_details_history(employee_id,pa_id,criteria,outstanding,excellent,good,average,below_average,unsatisfactory,entry_by)values("'.$employee_id.'","'.$pa_id.'","'.$criteria.'","'.$outstanding.'","'.$excellent.'","'.$good.'","'.$average.'","'.$below_average.'","'.$unsatisfactory.'","'.$_SESSION['user']['id'].'")';
			db_query($ins_sql2);
			}
			$_POST['incharge_entry_by']=$_SESSION['user']['id'];
			$ins_sql_two='insert into self_assessment_details_two (employee_id,pa_id,group_involvement,increment,promotion,confirmation,extension_of_confirmation,termination,others,other_recommendation,increment_amount,promotion_to,incharge_comments,incharge_entry_by)values("'.$_POST['emp_id'].'","'.$_GET['id'].'","'.$_POST['group_involvement'].'","'.$_POST['increment'].'","'.$_POST['promotion'].'","'.$_POST['confirmation'].'","'.$_POST['extension_of_confirmation'].'","'.$_POST['termination'].'","'.$_POST['others'].'","'.$_POST['other_recommendation'].'","'.$_POST['increment_amount'].'","'.$_POST['promotion_to'].'","'.$_POST['incharge_comments'].'","'.$_POST['incharge_entry_by'].'")';
			db_query($ins_sql_two);
			
			
			$up_sql='update self_assessment_appraisal set status="VERIFIED" where id="'.$_GET['id'].'"';
			db_query($up_sql);
			header("Location: self_assessment_view_hod_view.php?id=".$_GET['id']);
 }
...
//</script>
		...
		<table class="table table-bordered table-sm" style="width:100%;">
			...
			?>
				<tr>
					...
					$check_outstanding=find_a_field('self_assessment_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and outstanding=5');
					...
					<?php 
					$check_excellent=find_a_field('self_assessment_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and excellent=4');
				    ...
				    <td><input type="checkbox"
					<?php 
					$check_good=find_a_field('self_assessment_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and good=3');
				    ...
				    <td><input type="checkbox" 
					<?php 
					$check_average=find_a_field('self_assessment_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and average=2');
				    ...
				    <td><input type="checkbox"
						<?php 
					$check_below_average=find_a_field('self_assessment_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and below_average=1');
				    ...
				    <td><input type="checkbox"
					<?php 
					$check_unsatisfactory=find_a_field('self_assessment_details','count(id)','pa_id="'.$_GET['id'].'" and criteria="'.$row->id.'" and employee_id="'.$all_data_master->employee_id.'" and unsatisfactory!=""');
    ...
    document.getElementById("other_recommendation").style.display = vis;
			...
			<tr>
			<td>a) Pay Increase Amount: TK. <input type="text" name="increment_amount" id="increment_amount"></td>
		    <td>b) Promotion To- <input type="text" name="promotion_to" id="promotion_to"></td>
		  </tr>
			<tr>
