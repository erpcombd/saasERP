...
$PBI_ID=$_POST['employee_name'];

$all_data=find_all_field('personnel_basic_info','*','PBI_ID='.$PBI_ID);
echo $ins_sql='insert into performance_appraisal(employee_id,concern_name,appraisal_for,from_date,to_date,present_date,status,entry_by)values("'.$_POST['employee_name'].'","'.$all_data->PBI_ORG.'","'.$_POST['appraisal_for'].'","'.$_POST['from_date'].'","'.$_POST['to_date'].'","'.$_POST['present_date'].'","MANUAL","'.$_SESSION['user']['id'].'")';
db_query($ins_sql);
$last_id=db_insert_id();
header("Location: performance_apprisial.php?id=".$last_id);
...
$all_data=find_all_field('personnel_basic_info','*','PBI_ID='.$PBI_ID);
  $up_one_sql='update performance_appraisal set employee_id="'.$_POST['employee_name'].'",concern_name="'.$all_data->PBI_ORG.'",appraisal_for="'.$_POST['appraisal_for'].'",from_date="'.$_POST['from_date'].'",to_date="'.$_POST['to_date'].'" where id="'.$_GET['id'].'" ';
db_query($up_one_sql);
 
header("Location: performance_apprisial.php?id=".$_GET['id']);
...
if(isset($_POST['confirm'])){
  $up_sql='update performance_appraisal set name_of_appraisee="'.$_POST['name_of_appraisee'].'",designation="'.$_POST['designation'].'",department="'.$_POST['department'].'",doj="'.$_POST['doj'].'",qualification="'.$_POST['qualification'].'",aca_sub="'.$_POST['aca_sub'].'",experience="'.$_POST['experience'].'",experience_tpc="'.$_POST['experience_tpc'].'",late_join_date="'.$_POST['late_join_date'].'",absent_pay="'.$_POST['absent_pay'].'",warning="'.$_POST['warning'].'",present_salary="'.$_POST['present_salary'].'" ,last_inc_amt="'.$_POST['last_inc_amt'].'",last_inc_date="'.$_POST['last_inc_date'].'",tot_availed="'.$_POST['tot_availed'].'",status="CHECKED"  where id='.$_GET['id'];
db_query($up_sql);
header("Location: performance_apprisial_view_hr1.php?id=".$_GET['id']);
}
?>
<!doctype html>
<html lang="en">
			   ...
			   <input list="employee_names" name="employee_name" id="employee_name" style="width:370px; margin-left:5px;" value="<?php echo $all_data_master->employee_id;?>" >
					  ...
					  <datalist id="employee_names">
						<?php 
								...
								?>
						<option value="<?php echo $row->PBI_ID;?>"><?php echo $row->PBI_ID,"-> ",$row->PBI_NAME;?></option>
								<?php } ?>
					  </datalist>
			</div>
					...
					<h4 style="text-align:center;">Necessary Information (During appraisal period):</h4> 
					  ...
					  <div class="row m-0 p-0">
						...
						<div class="input-group mb-3 input-group-sm">
							  ...
							  <?php 
							  //echo $all_data_master->from_date;
							  ...
							  $from_id=find_a_field('salary_attendence','id','mon="'.$date_mon_from[1].'" and year="'.$date_mon_from[0].'" and PBI_ID="'.$all_data_master->employee_id.'"');
							  	    $to_id=find_a_field('salary_attendence','id','mon="'.$date_mon_to[1].'" and year="'.$date_mon_to[0].'" and PBI_ID="'.$all_data_master->employee_id.'"');
							 $total_late=find_a_field('salary_attendence','sum(lt)','PBI_ID="'.$all_data_master->employee_id.'" and id between "'.$from_id.'" and "'.$to_id.'"');
							//echo 'select id from salary_attendence where mon="'.$date_mon_from[1].'" and year="'.$date_mon_from[0].'" and PBI_ID="'.$all_data_master->employee_id.'"';
							 		 $total_lwp_absent=find_a_field('salary_attendence','sum(ab+lwp)','PBI_ID="'.$all_data_master->employee_id.'" and id between "'.$from_id.'" and "'.$to_id.'"');
							 // echo 'select sum(ab+lwp) from salary_attendence where PBI_ID="'.$all_data_master->employee_id.'" and id between "'.$from_id.'" and "'.$to_id.'"';
							  ?>
...
$g_e_date=date('Y-12-25');
   ...
   ////////// this year ager prappo leave  END///////
		...
		//echo number_format($this_year_op_cl,2);
							 		   ...
							 		   $total_leave_availed=find_a_field('salary_attendence','sum(lv)','PBI_ID="'.$all_data_master->employee_id.'" and id between "'.$from_id.'" and "'.$to_id.'"');
							// echo 'select sum(lv) from salary_attendence where PBI_ID="'.$all_data_master->employee_id.'" and id between "'.$from_id.'" and "'.$to_id.'"';
							  ?>
