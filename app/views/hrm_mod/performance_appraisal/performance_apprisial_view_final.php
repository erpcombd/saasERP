...
$all_data=find_all_field('personnel_basic_info','*','PBI_ID='.$all_data_master->employee_id); 
$all_edu=find_all_field('hrm_education_detail','*','PBI_ID="'.$all_data->PBI_ID.'" order by EDUCATION_D_ID desc ');
  ...
  $up_sql='update performance_appraisal_details_two set approve_amt="'.$_POST['approve_amt'].'",md_comments="'.$_POST['md_comments'].'",md_entry_by="'.$_SESSION['user']['id'].'" where pa_id='.$_GET['id'];
  db_query($up_sql);
  $up_sql='update performance_appraisal set status="COMPLETED" where id="'.$_GET['id'].'"';
			db_query($up_sql);
		header("Location: performance_apprisial_view_md_view.php?id=".$_GET['id']);
  }
...
div .cus{
  ...
  <body style="width:90%;margin:0 auto;">
    ...
    <section >
	  ...
	  function cus_print(){
	  document.getElementById("cus_print_span").style.display="none";
	  window.print();
	   document.getElementById("cus_print_span").style.display="";
			...
			<span id="cus_print_span">
			<a href="perfor_appri_status_list.php"><input type="button" class="btn btn-primary" value="Go Back" style="margin-bottom:5px;"></a>
			 <input type="button" class="btn btn-primary" value="Print" onClick="cus_print()" style="margin-bottom:5px;"> 
			 </span>
      <div class="col-6">
	 ...
	 <form action="" method="post">
					  ...
					  <div class="col-6">
							...
							<div class="input-group mb-3 input-group-sm">
							  <div class="input-group-prepend" style="width:31%">
							   ...
							   <input type="text" style="font-size:14px; font-weight:bold;" class="form-control" name="employee_id" id="employee_id" value="<?php echo $all_data->PBI_ID;?>">
			...
			<br> 	 
				<table style="width:25%;" style="padding-left: 15px;" >	  
		...
		<table class="table table-bordered table-sm" style="width:100%;">
			...
			<tbody>
			<?php 
			$all_sql='select * from pa_criteria';
			$all_query=db_query($all_sql);
			while($row=mysqli_fetch_object($all_query)){
			...
			?>
				<tr>
					<td><?php echo ++$i;?></td>
					<td><?php echo $row->criteria_name;?>
					<input type="hidden" name="criteria_<?php echo $row->id;?>" id="criteria_<?php echo $row->id;?>" value="<?php echo $row->id;?>">
					<input type="hidden" name="PBI_ID" id="PBI_ID" value="<?php echo $all_data_master->employee_id;?>">
					</td>
					<td>
					<?php 
					$check_outstanding=find_a_field('performance_appraisal_details d,performance_appraisal_details_history h','count(d.id)','d.pa_id="'.$_GET['id'].'" and d.pa_id=h.pa_id and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'" and d.outstanding=h.outstanding and d.outstanding=5');
					$check_outstanding_history=find_a_field('performance_appraisal_details_history d','count(d.id)','d.pa_id="'.$_GET['id'].'"  and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'"  and d.outstanding=5');
					
					?>
					<?php 
					if($check_outstanding>0){
					?>
					<input type="checkbox" checked="checked"  id="outstanding_<?php echo $row->id;?>" name="outstanding_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="5">
					<?php } else if($check_outstanding_history>0){?>
					<img src="gray_check.png" width="31px" height="29px"> 
					<?php }else{ ?>
					<input type="checkbox"   id="outstanding_<?php echo $row->id;?>" name="outstanding_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="5">
					<?php } ?>
					
					</td>
				    <td>
					<?php 
					$check_excellent=find_a_field('performance_appraisal_details d,performance_appraisal_details_history h','count(d.id)','d.pa_id="'.$_GET['id'].'" and d.pa_id=h.pa_id and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'" and d.excellent=h.excellent and d.excellent=4');
					$check_excellent_history=find_a_field('performance_appraisal_details_history d','count(d.id)','d.pa_id="'.$_GET['id'].'"  and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'"  and d.excellent=4');
					
					?>
					<?php 
					if($check_excellent>0){
					?>
					<input type="checkbox" checked="checked"  id="excellent_<?php echo $row->id;?>" name="excellent_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="4">
					<?php } else if($check_excellent_history>0){?>
					<img src="gray_check.png" width="31px" height="29px"> 
					<?php }else{ ?>
					<input type="checkbox" id="excellent_<?php echo $row->id;?>" name="excellent_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="4">
					<?php } ?>	
					
					</td>
				    <td>
							<?php 
					$check_good=find_a_field('performance_appraisal_details d,performance_appraisal_details_history h','count(d.id)','d.pa_id="'.$_GET['id'].'" and d.pa_id=h.pa_id and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'" and d.good=h.good and d.good=3');
					$check_good_history=find_a_field('performance_appraisal_details_history d','count(d.id)','d.pa_id="'.$_GET['id'].'"  and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'"  and d.good=3');
					
					?>
					<?php 
					if($check_good>0){
					?>
					<input type="checkbox" checked="checked"  id="good_<?php echo $row->id;?>" name="good_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="3">
					<?php } else if($check_good_history>0){?>
					<img src="gray_check.png" width="31px" height="29px"> 
					<?php }else{ ?>
					<input type="checkbox"  id="good_<?php echo $row->id;?>" name="good_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="3">
					<?php } ?>		
					...
					<?php 
					$check_average=find_a_field('performance_appraisal_details d,performance_appraisal_details_history h','count(d.id)','d.pa_id="'.$_GET['id'].'" and d.pa_id=h.pa_id and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'" and d.average=h.average and d.average=2');
					$check_average_history=find_a_field('performance_appraisal_details_history d','count(d.id)','d.pa_id="'.$_GET['id'].'"  and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'"  and d.average=2');
					
					?>
					<?php 
					if($check_average>0){
					?>
					<input type="checkbox" checked="checked"  id="average_<?php echo $row->id;?>" name="average_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="2">
					<?php } else if($check_average_history>0){?>
					<img src="gray_check.png" width="31px" height="29px"> 
					<?php }else{ ?>
					<input type="checkbox"   id="average_<?php echo $row->id;?>" name="average_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="2">
					<?php } ?>		
	
				 </td>
				    <td>
					<?php 
					$check_below_average=find_a_field('performance_appraisal_details d,performance_appraisal_details_history h','count(d.id)','d.pa_id="'.$_GET['id'].'" and d.pa_id=h.pa_id and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'" and d.below_average=h.below_average and d.below_average=1');
					$check_below_average_history=find_a_field('performance_appraisal_details_history d','count(d.id)','d.pa_id="'.$_GET['id'].'"  and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'"  and d.below_average=1');
					
					?>
					<?php 
					if($check_below_average>0){
					?>
					<input type="checkbox" checked="checked"  id="below_average_<?php echo $row->id;?>" name="below_average_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="1">
					<?php } else if($check_below_average_history>0){?>
					<img src="gray_check.png" width="31px" height="29px"> 
					<?php }else{ ?>
					<input type="checkbox"   id="below_average_<?php echo $row->id;?>" name="below_average_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="1">
					<?php } ?>		
				    ...
				    <td>
					<?php 
					$check_unsatisfactory=find_a_field('performance_appraisal_details d,performance_appraisal_details_history h','count(d.id)','d.pa_id="'.$_GET['id'].'" and d.pa_id=h.pa_id and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'" and d.unsatisfactory=h.unsatisfactory and d.unsatisfactory!=""');
					$check_unsatisfactory_history=find_a_field('performance_appraisal_details_history d','count(d.id)','d.pa_id="'.$_GET['id'].'"  and d.criteria="'.$row->id.'" and d.employee_id="'.$all_data_master->employee_id.'"  and d.unsatisfactory!=""');
					
					?>
					<?php 
					if($check_unsatisfactory>0){
					?>
					<input type="checkbox" checked="checked"  id="unsatisfactory_<?php echo $row->id;?>" name="unsatisfactory_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="0">
					<?php } else if($check_unsatisfactory_history>0){?>
					<img src="gray_check.png" width="31px" height="29px"> 
					<?php }else{ ?>
					<input type="checkbox"   id="unsatisfactory_<?php echo $row->id;?>" name="unsatisfactory_<?php echo $row->id;?>" style="height: 29px;width: 39px;" value="0">
					<?php } ?>		
				...
				<tr>
					...
					<td colspan="6" style="background-color:darkseagreen;">
						<select name="group_involvement" id="group_involvement" class="form-control" style="margin: 0 auto;width:131px;">
    ...
    document.getElementById("other_recommendation").style.display = vis;
			...
			<tr>
			<td>a) Pay Increase Amount: TK. <input type="text" name="increment_amount" value="<?php echo $all_data_details_two->increment_amount;?>" id="increment_amount" ></td>
		    <td>b) Promotion To- <input type="text" name="promotion_to" id="promotion_to" value="<?php echo $all_data_details_two->promotion_to;?>"></td>
		  </tr>
			<tr>
		...
		<tbody>
				<tr>
				<td  >Approve Amount<br>
			<input type="text" name="approve_amt" id="approve_amt" value="<?php echo $all_data_details_two->autho_increment_amount;?> ">
			</td>
		</tr>
		<tr>
				<td  >Comments<br>
		...
		<br> 	 
				<table style="width:25%;" >	  
					...
					<tr style="border:0px solid;">
						<td style="text-align:center;" ><?php 
						echo find_a_field('user_activity_management','fname','user_id='.$all_data_details_two->authorize_by);
						?></td>
					</tr>
						<tr style="border-top:1px solid black;">
						<td style="text-align:center;font-weight:bold;">Signature of the Authorizer </td>
					</tr>		  
              </table>
		<br> 	<br> 
			<span style="font-weight:bold;">3. HR Comments / Development Plan(if any)</span>
			<table class="table table-bordered table-condensed">
		<tbody>
		<tr>
				<td  >Remarks:<br>
			<textarea style="height: 70px;width: 80%;" name="hr_remarks" id="hr_remarks"><?php echo $all_data_details_two->hr_remarks;?></textarea>
			</td>
		</tr>
		<tr>
				<td  >Development Plan:<br>
			<textarea style="height: 70px;width: 80%;" name="hr_development_plan" id="hr_development_plan"><?php echo $all_data_details_two->hr_development_plan;?></textarea>
			</td>
		</tr>
		</tbody>
		</table>
		<br>  
				<table style="width:25%;" >	  
					...
					<tr style="border:0px solid;">
						<td style="text-align:center;" ><?php 
						echo find_a_field('user_activity_management','fname','user_id='.$all_data_details_two->hr_entry_by);
						?></td>
					</tr>
						<tr style="border-top:1px solid black;">
						<td style="text-align:center;font-weight:bold;">Signature of the HRD </td>
					</tr>		  
              </table>
		<br> 
		<span style="font-weight:bold;">4. Final Approval of Higher Authority</span>
			<table class="table table-bordered table-condensed">
		<tbody>
		<tr>
				<td  >Approve Amount<br>
			<input type="text" name="approve_amt" id="approve_amt" value="<?php echo $all_data_details_two->approve_amt;?> ">
			</td>
		</tr>
		<tr>
				<td  >Comments<br>
			<textarea style="height: 70px;width: 80%;" name="md_comments" id="md_comments"><?php echo $all_data_details_two->md_comments;?></textarea>
			</td>
		</tr>
		</tbody>
		</table>
		<br> 	
		 <div class="row">
			...
			<table style="width:100%;" >	  
					...
					<tr style="border:0px solid;">
						<td style="text-align:center;" ><?php 
						echo find_a_field('user_activity_management','fname','user_id='.$all_data_details_two->md_entry_by);
						?></td>
					</tr>
						<tr style="border-top:1px solid black;">
						<td style="text-align:center;font-weight:bold;">President /  Managing Director</td>
					</tr>		  
              </table>
