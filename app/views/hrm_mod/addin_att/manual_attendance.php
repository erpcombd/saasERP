...
$sch = find_all_field_sql('select p.off_day,s.office_start_time,s.office_end_time from personnel_basic_info p, hrm_schedule_info s where p.PBI_ID="'.$emp_id.'" and p.office_time=s.id');
	$office_start_time = number_format(($sch->office_start_time+.10),2,'.','');
	
	$date = date('Ymd',$access_stamp);
	if(date('N',$access_stamp)==$sch->off_day) $info['status'][$date]=0;
	else{
	if($office_start_time == '')		$info['status'][$date]=1;
	else								{$info['late'][$date] = (int)(($access_stamp - strtotime($access_date.' '.$office_start_time))/60);
...
VALUES ('$emp_id', '$data[3]', '$access_date','$access_time', '$access_stamp', '$user1', '".$info['status'][$date]."','$sch->off_day', '$office_start_time', '$sch->office_end_time')";
$query=db_query($sql);
		}
?>
          ...
          <div  class="oe_view_manager_view_list"><div  class="oe_list oe_view">
            <table width="80%" border="1" align="center">
              ...
              <tr>
                ...
                <td colspan="3"><input name="PBI_ID"  type="text" required id="PBI_ID" size="10" onblur="" tabindex="1" style="width:400px;" /></td>
              </tr>
              <tr>
                <td width="20%"><div align="right">Access Date-Time:</div></td>
                <td><input type="text" name="m_date" id="m_date" required style="width:100px;" /></td>
                <td><select name="m_hr" id="m_hr">
                    <? for($i=7;$i<20;$i++){ if($i>11) {$cut = ' PM'; $ix = $i-12;} else {$cut = ' AM'; $ix = $i;}
	  ?>
                  <option value="<?=$i?>">
                    <?=sprintf('%02d'.$cut, $ix);?>
                    </option>
