...
$office_start_time = number_format(($sch->office_start_time+.10),2,'.','');
	...
	else								{$info['late'][$date] = (int)(($time_stamp - strtotime($day.' '.$office_start_time))/60);
	
	if($info['late'][$date]>0) 	$info['status'][$date]=2;
	else 						$info['status'][$date]=1;
	}}

$sql="INSERT INTO hrm_inout (employee_id, access_date, access_time, access_stamp, access_timestamp,off_day,start_time,end_time,status) VALUES ('$employee_id', '$day', '$daytime', '$time_stamp', CURRENT_TIMESTAMP ,'$sch->off_day', '$office_start_time', '$sch->office_end_time', '".$info['status'][$date]."' )";
db_query($sql);}
}
}
