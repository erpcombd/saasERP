...
<tr class="oe_list_header_columns">
  ...
  <tr  class="alt">
    ...
    <td width="9%" align="left"><input type="text" name="start_date" id="start_date" style="width:100px;" required value="<?=$_POST['start_date']?>" /></td>
    <td width="16%" align="right"><strong>End Date   :</strong></td>
    <td width="38%"><input type="text" name="end_date" id="end_date" style="width:100px;" required value="<?=$_POST['end_date']?>" /></td>
  </tr>
<tr class="oe_list_header_columns">
...
?>
<div style="text-align:center"><div class="print_box">	
...
<tr class="oe_list_header_columns">
  <th colspan="4" style="text-align:center"><table width="100%" border="0" cellspacing="0" cellpadding="0" id="grp">
   <thead> <tr bgcolor="#CCCCCC">
      ...
      <th><div align="center" class="style2">Office Time</div></th>
      <th><div align="center" class="style2">IN</div></th>
      <th><div align="center" class="style2">Late(min)</div></th>
      <th><span class="style2">OUT</span></th>
      <th><span class="style2">Early(min)</span></th>
      <th><div align="center" class="style2">Off Day </div></th>
      </tr></thead>
	  <? 
...
$s = 'select access_date,id,access_time,access_stamp,end_time from hrm_inout where id!="'.$data->id.'" and employee_id="'.$PBI_ID.'" and  access_date = "'.$data->access_date.'" order by access_stamp desc limit 1';
...
$this_date = $dt->format( "Ymd" );
	...
	if($info['access_stamp'][$this_date]=='')
	{
...
?>
    <tr bgcolor="<?=$info['bgcolor'][$this_date]?>">
      ...
      <td><?=$dt->format("l");?></td>
      <td><?=$info['status'][$this_date];?></td>
      <td><? 
	  $ddd = strtotime($data->access_date.' '.$info['start_time'][$date]);
      $ddd1 = (int)$ddd-600;
	  echo 'IT-&nbsp;'.date('h:i A',($ddd1)).'
	  <Br>OT-'.date('h:i A',strtotime($data->access_date.' '.$info['end_time'][$date]));?></td>
      <td><?=($info['access_stamp'][$this_date]!=0)?date('h:i:s A',$info['access_stamp'][$this_date]):'';?></td>
      <td><?=($info['late'][$this_date]>0)?(($info['late'][$this_date])):'';?></td>
      <td><?=($info['out_stamp'][$this_date]!=0)?date('h:i:s A',$info['out_stamp'][$this_date]):'';?></td>
...
<? }?>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <th bgcolor="#336699"><div align="center" class="style2">Extra</span></th>
      </tr>
