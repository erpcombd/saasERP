...
function auto_calculation($PBI_ID,$mon,$year,$search_PBI_ID='',$lv){
$_REQUEST['lv'] = $lv;
...
$_REQUEST['edit_at']=date('Y-m-d H:i:s');  $crud->update($unique);
}
else
{
$_REQUEST['entry_by']=$_SESSION['user']['id'];
$_REQUEST['entry_at']=date('Y-m-d H:i:s');  $crud->insert();
}
}
...
//$in_time = date('H:i:s',$data->access_stamp);
//if(date('N',$info['access_stamp'][$date])==$datas->off_day)
//	{echo $info['status'][$date] ='Off Day';$info['bgcolor'][$date] = '#FFF';}
//	elseif($data->start_time == '')	{$info['status'][$date]='Regular'; $info['bgcolor'][$date] = '#EAFFEF'; ++$regular; }
//	
//
//else{
...
//
//
//db_query("insert into hrm_attendence_final 
...
$year = $_REQUEST['year'];
		...
		{
		auto_calculation($info->PBI_ID,$mon,$year,$search_PBI_ID,$_REQUEST['lv_'.$info->PBI_ID]);
		}
}
...
$mon = $_REQUEST['mon'];
$year = $_REQUEST['year'];
$startTime = $days1=mktime(1,1,1,($mon-1),26,$year);
$endTime = $days2=mktime(1,1,1,$mon,25,$year);
...
$days_mon=$days_mon=(($endTime - $startTime)/(3600*24))+1;
for ($i = $startTime; $i <= $endTime; $i = $i + 86400) {
$day   = date('l',$i);
...
$$day .= '"'.date('Y-m-d', $i).'"';
		...
		$holy_day=find_a_field('salary_holy_day','count(holy_day)','holy_day between "'.$year.'-'.($mon-1).'-'.'26'.'" and "'.$year.'-'.$mon.'-25"');
		if($_POST['search_PBI_ID']>0) $con .= ' and a.PBI_ID='.$_POST['search_PBI_ID'];
		$sql = "select a.PBI_NAME,a.PBI_ID,a.PBI_ID,c.DESG_DESC,d.DEPT_DESC,a.off_day from personnel_basic_info a,designation c, department d where a.PBI_DESIGNATION=c.DESG_ID and a.PBI_DEPARTMENT=d.DEPT_ID and a.PBI_DOMAIN='".$_POST['area']."'".$con." and a.PBI_JOB_STATUS='1' order by a.PBI_ID ";
		$query = db_query($sql);
		while($info=mysqli_fetch_object($query))
		{

		$r_count=${'day'.$info->off_day};
