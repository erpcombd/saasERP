...
FROM hrm_roster_allocation r, hrm_schedule_info s
WHERE r.shedule_1 = s.id and s.schedule_type = 'Offday' and r.roster_date BETWEEN '".$start_date."' AND '".$end_date."' 
group by r.PBI_ID";
...
$query = db_query($sql);
	...
	$sql = "SELECT r.PBI_ID,count(1) off_day
FROM hrm_roster_allocation r, hrm_schedule_info s
WHERE r.shedule_1 = s.id and s.schedule_type = 'Holiday' and r.roster_date BETWEEN '".$start_date."' AND '".$end_date."' 
group by r.PBI_ID";
	
$query = db_query($sql);
	while($data = mysqli_fetch_object($query))
	{
      $holiday_in_month[$data->PBI_ID] = $data->off_day;
	}
...
FROM hrm_att_summary2 h,personnel_basic_info p, hrm_roster_allocation r,hrm_schedule_info s
WHERE 
r.shedule_1 = s.id 
...
$query = db_query($sql);
	...
	{
	$pi++;
	if($fiscal_start>$data->PBI_DOC2){
	$lv_start = $fiscal_start;}
	else{
	$lv_start = $data->PBI_DOC2;}
	
	
	
	if($lv_start<$start_date&&$data->PBI_DOC2!=''&&$data->PBI_DOC2!='0000-00-00'){
   $dStart = new DateTime($data->PBI_DOC2);
   $dEnd  = new DateTime($fiscal_end);
   $dDiff = $dStart->diff($dEnd);
   $avail_days[$data->PBI_ID] = $dDiff->days;
   $leave_avail_days[$data->PBI_ID] = (30/365)*$avail_days[$data->PBI_ID];
   }
   else{
   $leave_avail_days[$data->PBI_ID] = 0;}
...
$leave_in_month[$data->PBI_ID] = 0;
	...
	$values[$pi]['hd'] = $holiday_in_month[$data->PBI_ID];
	$values[$pi]['lt'] = $late_panelty*.5;

	if($leave_avail_days[$data->PBI_ID]>=$values[$pi]['lv'])
	{
	$values[$pi]['lwp'] = $absent_in_month[$data->PBI_ID];
	}
	else 
	{
	$values[$pi]['lv']  = $leave_avail_days[$data->PBI_ID];
	$values[$pi]['lwp'] = $absent_in_month[$data->PBI_ID] + ($values[$pi]['lv'] - $leave_avail_days[$data->PBI_ID]); 
	}
	
	$values[$pi]['pre'] = $data->pre + $iom_in_month[$data->PBI_ID] ;

	$values[$pi]['pay'] = $values[$pi]['pre'] + $values[$pi]['lv'] + $values[$pi]['hd'] + $values[$pi]['od'] - $values[$pi]['lt'] - $leave_days_lv;
	$values[$pi]['ab']  = (($values[$pi]['td'])-($values[$pi]['pre'] + $values[$pi]['lv'] + $values[$pi]['lwp'] + $values[$pi]['hd'] + $values[$pi]['od'])) + $leave_days_lv;
	
	
}
for($y=1;$y<=$pi;$y++)
{
$found = find_a_field('hrm_attendence_final2','1','PBI_ID="'.$values[$y]['emp_id'].'" and mon="'.$mon.'" and year="'.$year.'"');


if($found==0)
			{
 $sql = "INSERT INTO hrm_attendence_final2 
(mon, year, PBI_ID, 
td, od, hd, lt, ab, lv,lwp, pre, pay, entry_at, entry_by) values
('".$mon."','".$year."','".$values[$y]['emp_id']."',
'".$values[$y]['td']."','".$offday_in_month[$values[$y]['emp_id']]."', '".$holiday_in_month[$values[$y]['emp_id']]."','".$values[$y]['lt']."','".$values[$y]['ab']."','".$values[$y]['lv']."','".$values[$y]['lwp']."','".$values[$y]['pre']."','".$values[$y]['pay']."','".date('Y-m-d H:i:s')."','".$_SESSION['user']['id']."')";
db_query($sql);
			}
			else
			{
$sql = "Update hrm_attendence_final2 set 

td='".$values[$y]['td']."', od='".$offday_in_month[$values[$y]['emp_id']]."',hd='".$holiday_in_month[$values[$y]['emp_id']]."', lt='".$values[$y]['lt']."', ab='".$values[$y]['ab']."', lv='".$values[$y]['lv']."', lwp='".$values[$y]['lwp']."', pre='".$values[$y]['pre']."',pay='".$values[$y]['pay']."', entry_at='".date('Y-m-d H:i:s')."', entry_by='".$_SESSION['user']['id']."'
          ...
          <div  class="oe_view_manager_view_list"><div  class="oe_list oe_view">
            ...
            <table width="80%" border="1" align="center">
              ...
              <tr>
                ...
                <td height="40" colspan="4" bgcolor="#00FF00"><div align="center" class="style1">Salary Process - hrm_attendence_final2 (Roster) </div></td>
                </tr>
