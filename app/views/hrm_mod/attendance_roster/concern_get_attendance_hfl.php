...
$start_date = $syear.'-'.sprintf("%02d", $smon).'-25';


$startTime = $days1 = strtotime($start_date);
$days_mon = date('t',$startTime);

$end_date   = $year.'-'.sprintf("%02d", $mon).'-27';
...
(emp_id, in_time, out_time, att_date, sch_in_time, sch_out_time, sch_off_day, iom_type, iom_sl_no, iom_reason, iom_approved_by, iom_entry_at, iom_entry_by, leave_id, leave_type, leave_reason, leave_duration, panalty_leave_duration, leave_approved_by, leave_entry_at, leave_entry_by, dayname, late_min, grace_no, final_late_min, final_late_status, process_time, holyday, final_status, deleted,ot)
select
h.emp_id, h.in_time, h.out_time, h.att_date, h.sch_in_time, h.sch_out_time, h.sch_off_day, h.iom_type, h.iom_sl_no, h.iom_reason, h.iom_approved_by, h.iom_entry_at, h.iom_entry_by, h.leave_id, h.leave_type, h.leave_reason, h.leave_duration, h.panalty_leave_duration, h.leave_approved_by, h.leave_entry_at, h.leave_entry_by, h.dayname, h.late_min, h.grace_no, h.final_late_min, h.final_late_status, h.process_time, h.holyday, h.final_status, h.deleted , (TIMESTAMPDIFF(SECOND,h.in_time, h.out_time) DIV 3600)
...
and (p.employee_type = 'Roster' or p.employee_type = 'General Roster')
	...
	// out time of before next in time
...
for($x=1;$x<=$sl;$x++)
{
	...
	else
	{
		...
		if($sch_ou[$x]>$exit_time[$x]) {$deleted[$x] = 1;}
	}
...
$hours = round((strtotime($exit_time[$x]) - strtotime($in_time[$x]))/3600, 1);
$hours= $hours-8;
$ot = floor($hours);

	$sql="INSERT INTO hrm_att_summary2 (emp_id, att_date, in_time,out_time, sch_in_time, sch_out_time,  dayname, 
	panalty_leave_duration, deleted, late_min, grace_no, final_late_min, final_late_status, process_time,ot)
	VALUES 
	('".$emp_ids[$x]."','".$att_date[$x]."', '".$in_time[$x]."','".$exit_time[$x]."','".$office_start_time[$x]."','".$office_end_time[$x]."', dayname('".$att_date[$x]."'), '".$panalty_leave_duration[$x]."','".$deleted[$x]."','".$late_min[$x]."','".$grace_no[$x]."','".$final_late_min[$x]."','".$final_late_status[$x]."','".$process_time[$x]."','".$ot."')";
