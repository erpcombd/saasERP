...
$user_data=find_all_field('user_activity_management','','user_id='.$_SESSION['user']['id']);
?>
...
var strURL="monthly_attendence_ajax_rsm.php?PBI_ID="+PBI_ID+"&td="+td+"&fd="+fd+"&od="+od+"&hd="+hd+"&lt="+lt+"&ab="+ab+"&lv="+lv+"&lwp="+lwp+"&pre="+pre+"&pay="+pay+"&ot="+ot+"&deduction="+deduction+"&mon="+mon+"&year="+year+"&bonus="+bonus+"&benefits="+benefits;
  ...
  <tr  class="alt">
    ...
    <td colspan="3"><span class="oe_form_group_cell">
      <select name="dept" style="width:160px;" id="dept">
        <?=foreign_relation('department','DEPT_SHORT_NAME','DEPT_DESC',$_POST['dept']);?>
      </select>
    </span></td>
  </tr>
  <tr >
  ...
  <tr >
    ...
    <td><span class="oe_form_group_cell">
      <select name="PBI_BRANCH" id="PBI_BRANCH">
        <?  if($user_data->region_id>0){
		foreign_relation('branch','BRANCH_ID','BRANCH_NAME',$user_data->region_id,' BRANCH_ID='.$user_data->region_id.' order by BRANCH_NAME');
		}
		else{
		foreign_relation('branch','BRANCH_ID','BRANCH_NAME',$_POST['PBI_BRANCH'],' BRANCH_ID='.$user_data->region_id.' order by BRANCH_NAME');}?>
      </select>
    </span></td>
    <td><div align="right"><strong>Zone: </strong></div></td>
    <td><span class="oe_form_group_cell">
      <select name="PBI_ZONE" id="PBI_ZONE">
        <? if($rgn_data->PBI_BRANCH>0){
		foreign_relation('zon','ZONE_CODE','ZONE_NAME',$_POST['PBI_ZONE'],' REGION_ID='.$rgn_data->PBI_BRANCH.' order by ZONE_NAME');
		}else{
  ...
  <tr >
    ...
    <td><span class="oe_form_group_cell">
      <select name="PBI_GROUP" id="PBI_GROUP">
        <? foreign_relation('product_group','group_name','group_name',$user_data->product_group);?>
      </select>
    </span></td>
  </tr>
...
<?		
		...
		<tr bgcolor="#CC99FF" style="font-size:10px; padding:3px; ">
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
		  <td>&nbsp;</td>
...
value="<? if($att->td>0) echo $att->td; else {if($new_emp_days>0) echo $new_emp_days; else echo $days_mon;}?>" size="2" maxlength="2" readonly="readonly" /></td>
		  ...
		  <td align="center"><input name="lwp_<?=$info->PBI_ID?>" type="text" id="lwp_<?=$info->PBI_ID?>" style="font-size:10px; width:28px; min-width:28px;" value="<?=($data->lwp_=='')?$leave_days_lwp:$data->lwp;?>" size="2" maxlength="2"  onchange="cal_all(<?=$info->PBI_ID?>)"/></td>
		  <td align="center"><input name="lv_<?=$info->PBI_ID?>" type="text" id="lv_<?=$info->PBI_ID?>" style="font-size:10px; width:28px; min-width:28px;" value="<?=($data->lv=='')?$leave_days_lv:$data->lv;?>" size="2" maxlength="2"  onchange="cal_all(<?=$info->PBI_ID?>)"/></td>
		  ...
		  <td align="center"><? 
			  ...
			  {?>
		    <input type="button" name="Dutton" value="<?=$status?>"  onclick="cal_all(<?=$info->PBI_ID?>), update_value(<?=$info->PBI_ID?>)" style="font-size:10px;"/>
		    <?
			  }
			  else echo 'Saved';
			  }
...
&nbsp;</td>
		...
		<? echo $z_sql='select p.*, z.ZONE_NAME, z.ZONE_CODE from zon z, personnel_basic_info p where p.PBI_DESIGNATION="ZI" and p.PBI_ZONE=z.ZONE_CODE and p.PBI_BRANCH='.$user_data->region_id.' and p.PBI_GROUP="'.$user_data->product_group.'" ';
		$z_query=db_query($z_sql);
		while($z_data=mysqli_fetch_object($z_query)){
...
$leave_days_lv = 0;}
		...
		<tr bgcolor="#99CCFF" style="font-size:10px; padding:3px; ">
          <td><?php echo ++$z;?></td>
          <td><?php echo $z_data->PBI_ID;?></td>
          <td><?php echo $z_data->PBI_NAME;?></td>
          <td><?php echo $z_data->PBI_DESIGNATION;?></td>
          <td colspan="2"><?php echo $z_data->ZONE_NAME;?></td>
          <td align="center"><input name="td_<?=$z_data->PBI_ID?>" type="text" id="td_<?=$z_data->PBI_ID?>" style="font-size:10px; width:20px; min-width:20px;" 
value="<? if($att->td>0) echo $att->td; else {if($new_emp_days>0) echo $new_emp_days; else echo $days_mon;}?>" size="2" maxlength="2" readonly="readonly" /></td>
          <td align="center"><input name="od_<?=$z_data->PBI_ID?>" type="text" id="od_<?=$z_data->PBI_ID?>" style="font-size:10px; width:20px; min-width:20px;" size="2" maxlength="2" readonly="readonly" value="<? if($att->od>0) echo $att->od; else {if($new_emp_off>0) echo $new_emp_off; else echo $r_count;}?>" /></td>
          <td align="center"><input name="hd_<?=$z_data->PBI_ID?>" type="text" id="hd_<?=$z_data->PBI_ID?>" style="font-size:10px; width:20px; min-width:20px;" size="2" maxlength="2" readonly="readonly" value="<? if($att->hd>0) echo $att->hd; else {if($new_emp_holy_day>0) echo $new_emp_holy_day; else echo $holy_day;}?>" /></td>
          <td align="center"><input name="lt_<?=$z_data->PBI_ID?>" type="text" id="lt_<?=$z_data->PBI_ID?>" style="font-size:10px; width:20px; min-width:20px;" value="<?=($data->lt=='')?$att->lt:$data->lt;?>" size="2" maxlength="2" onchange="cal_all(<?=$z_data->PBI_ID?>)" /></td>
          <td align="center"><input name="ab_<?=$z_data->PBI_ID?>" type="text" id="ab_<?=$z_data->PBI_ID?>" style="font-size:10px; width:20px; min-width:20px;" value="<?=($data->ab=='')?$att->ab:$data->ab;?>" size="2" maxlength="2"  onchange="cal_all(<?=$z_data->PBI_ID?>)"/></td>
          <td align="center"><input name="lwp_<?=$z_data->PBI_ID?>" type="text" id="lwp_<?=$z_data->PBI_ID?>" style="font-size:10px; width:28px; min-width:28px;" value="<?=($data->lwp_=='')?$leave_days_lwp:$data->lwp;?>" size="2" maxlength="2"  onchange="cal_all(<?=$z_data->PBI_ID?>)"/></td>
          <td align="center"><input name="lv_<?=$z_data->PBI_ID?>" type="text" id="lv_<?=$z_data->PBI_ID?>" style="font-size:10px; width:28px; min-width:28px;" value="<?=($data->lv=='')?$leave_days_lv:$data->lv;?>" size="2" maxlength="2"  onchange="cal_all(<?=$z_data->PBI_ID?>)"/></td>
          <td align="center"><input name="pre_<?=$z_data->PBI_ID?>" type="text" id="pre_<?=$z_data->PBI_ID?>" style="font-size:10px; width:25px; min-width:20px;" onchange="cal_all(<?=$z_data->PBI_ID?>)" value="<?=($data->pre=='')?$att->pre:$data->pre;?>" size="2" maxlength="2" readonly="readonly" /></td>
          <td align="center"><input name="pay_<?=$z_data->PBI_ID?>" type="text" id="pay_<?=$z_data->PBI_ID?>" style="font-size:10px; width:25px; min-width:20px;" value="<?=($data->pay=='')?$att->pay:$data->pay;?>" size="2" maxlength="2" readonly="readonly" /></td>
          <td align="center"><input name="ot_<?=$z_data->PBI_ID?>" type="text" id="ot_<?=$z_data->PBI_ID?>" style="font-size:10px; width:25px; min-width:20px;" value="<?=$data->ot?>" size="2" maxlength="2" /></td>
          <td align="center"><? 
			  if($status=='Edit')
			  {
			  if($_SESSION['user']['level']==5)
			  {?>
            <input type="button" name="Cutton" value="<?=$status?>"  onclick="cal_all(<?=$z_data->PBI_ID?>), update_value(<?=$z_data->PBI_ID?>)" style="font-size:10px;"/>
            <?
			  }
			  else echo 'Saved';
			  }
...
<?		
		$sql = "select p.*, a.AREA_NAME, d.dealer_name_e from personnel_basic_info p, salary_info s, area a, dealer_info d where p.PBI_AREA=d.area_code and p.PBI_AREA=a.AREA_CODE and p.PBI_ID=s.PBI_ID and p.PBI_JOB_STATUS='In Service' and p.PBI_DEPARTMENT='".$_POST['dept']."' and p.PBI_ORG='".$_SESSION['user']['group']."' and p.PBI_ZONE=".$z_data->ZONE_CODE.$con." and p.PBI_DESIGNATION not in('ASM','ZI','RSM') order by (s.basic_salary+s.consolidated_salary) desc";
...
$leave_days_lv = 0;}
        ...
        <tr style="font-size:10px; padding:3px; "><td><?=++$S?></td>
          ...
          <td><?=$info->PBI_NAME?></td><td><?=$info->PBI_DESIGNATION?></td>
...
<td align="center"><input name="pre_<?=$info->PBI_ID?>" type="text" id="pre_<?=$info->PBI_ID?>" style="font-size:10px; width:25px; min-width:20px;" onchange="cal_all(<?=$info->PBI_ID?>)" value="<?=($data->pre=='')?$att->pre:$data->pre;?>" size="2" maxlength="2" readonly="readonly" /></td>
<td align="center"><input name="pay_<?=$info->PBI_ID?>" type="text" id="pay_<?=$info->PBI_ID?>" style="font-size:10px; width:25px; min-width:20px;" value="<?=($data->pay=='')?$att->pay:$data->pay;?>" size="2" maxlength="2" readonly="readonly" /></td>
<td align="center"><input name="ot_<?=$info->PBI_ID?>" type="text" id="ot_<?=$info->PBI_ID?>" style="font-size:10px; width:25px; min-width:20px;" value="<?=$data->ot?>" size="2" maxlength="2" />
  <input name="deduction_<?=$info->PBI_ID?>" type="hidden" id="deduction_<?=$info->PBI_ID?>" style=" width:50px; min-width:20px;" value="<?=$data->deduction?>" size="8" maxlength="8" />
  <input name="benefits_<?=$info->PBI_ID?>" type="hidden" id="benefits_<?=$info->PBI_ID?>" style=" width:50px; min-width:20px;" value="<?=$data->benefits?>" size="8" maxlength="8" /></td>
