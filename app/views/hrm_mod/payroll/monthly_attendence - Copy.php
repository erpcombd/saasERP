...
if(@$att->od=='') @$att->od = $r_count;
		...
		else
		{
			...
			$att = find_all_field('hrm_attendence_final','','PBI_ID="'.$info->PBI_ID.'" and mon="'.$mon.'" and year="'.$year.'" ');
				...
				//var pre=td - (od + hd + ab + lv + lwp );
				//var pay=td - ab - ltdd - lwp;
				$att->td = ($new_emp_days>0)?$new_emp_days:$days_mon;
				$att->od = ($new_emp_off>0)?$new_emp_off:$r_count;
				$att->hd = ($new_emp_holy_day>0)?$new_emp_holy_day:$holy_day;
				$att->ab = 
				$att->lt = 0;
				$att->pre = $att->pre;
				$att->pay = $att->pre + $att->od + $att->hd + $leave_days_lv;
				}
			}
			else
			{
...
<td align="center"><input name="ot_<?=$info->PBI_ID?>" type="text" id="ot_<?=$info->PBI_ID?>" style="font-size:10px; width:45px; min-width:45px;" value="<?=$data->dealer_code?>" size="2" maxlength="2" /></td>
