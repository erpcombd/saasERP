...
$pr = find_all_field('purchase_receive','pr_no','id='.$jv->tr_id);
...
<input type="button" name="Submit" value="EDIT VOUCHER"  onclick="DoNav('<?php echo '&v_type=Purchase&v_no='.$jv_no.'&view=Show' ?>');" /></td>
...
<a target="_blank" href="chalan_view2.php?v_no=<?=$pr->pr_no?>"></a></div><? }?>
      ...
      <tr>
        <td class="tabledesign_text">GR Date :
        ...
        <td class="tabledesign_text">Voucher Date :
          <strong><?=date('d-m-Y',$jv->jv_date);?></strong></td>
      </tr>
      <tr>
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#000000" class="tabledesign">
      <tr>
        <td rowspan="2" align="center"><div align="center">Control Head</div></td>
        <td rowspan="2" align="center">A/C Ledger Head</td>
	  ...
	  <?
$sql2="SELECT a.ledger_id,a.ledger_name,sum(dr_amt) as dr_amt,narration,ledger_group_id FROM accounts_ledger a, secondary_journal b where b.jv_no='$jv_no' and a.ledger_id=b.ledger_id and jv_no=$jv_no and dr_amt>0 group by b.ledger_id desc";
$data2=db_query($sql2);
while($info=mysqli_fetch_object($data2)){		  
      ...
      <tr>
		...
		if($ggrp[1]!='') echo $ggrp[1]; else echo $grp_name; 
		?>
        </div></td>
        <td align="left"><?=$info->ledger_name?>
:<br>
  <?=$info->ledger_id?></td>
...
<?php }?>
<?
$sql2="SELECT a.ledger_id,a.ledger_name,sum(cr_amt) as cr_amt,narration,ledger_group_id FROM accounts_ledger a, secondary_journal b where b.jv_no='$jv_no' and a.ledger_id=b.ledger_id and jv_no=$jv_no and cr_amt>0 group by b.ledger_id desc";
$data2=db_query($sql2);
while($info=mysqli_fetch_object($data2)){		  
	  ?>
      <tr>
