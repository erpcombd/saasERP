...
$_POST['joining_deduction']=((($salary->gross_salary)/($days_mon))*($_POST['joining_ab']));
...
//____________ ATTENDANCE BONUS _________
if($_POST['td']==$days_mon && $_POST['lv']==0){

		   $tot_late = find_a_field('hrm_attendence_final','lt','PBI_ID="'.$_GET['PBI_ID'].'" and mon ="'.$_GET['mon'].'" and  year ="'.$_GET['year'].'"');
		   $tot_early = find_a_field('hrm_attendence_final','lt','PBI_ID="'.$_GET['PBI_ID'].'" and mon ="'.$_GET['mon'].'" and  year ="'.$_GET['year'].'"');
		   
		  if($tot_late+$tot_early==0){
		  $_POST['attendence_bonus'] = 500;
		  }elseif($tot_late+$tot_early==1){
		   $_POST['attendence_bonus'] = 350;
		  }elseif($tot_late+$tot_early == 2){
		   $_POST['attendence_bonus'] = 250;
		  }else{
		   $_POST['attendence_bonus'] = 0;
		  }
...
//__________ LATE DEDUCTION ________
...
$late_tot_min =  find_a_field('hrm_attendence_final','total_late_min','PBI_ID="'.$_GET['PBI_ID'].'" and mon ="'.$_GET['mon'].'" and  year ="'.$_GET['year'].'"');
if($late_tot_min>30)
$_POST['late_deduction']=round(((($salary->basic_salary)/(30*8*60))*($late_tot_min)));

$_POST['over_time_amount']= round(((($salary->gross_salary/208)*2)*($_GET['ot'])));  //round(((($salary->gross_salary)/240)*($_GET['ot']))); 

$_POST['absent_deduction']=round(((($salary->basic_salary)/($days_mon))*(($_GET['lwp']+$_GET['ab']))));
...
$_POST['total_deduction'] = $_POST['advance_install']+$_POST['other_install']+$_POST['deduction']+$_POST['income_tax']+$_POST['late_deduction']+$_POST['absent_deduction']+$_POST['pf']+$_POST['joining_deduction'];
$_POST['total_benefits'] = $_POST['salary_arrear']+$_POST['over_time_amount']+$_POST['attendence_bonus'];
