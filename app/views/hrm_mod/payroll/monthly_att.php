...
<?		
		...
		$sql = "select p.* from personnel_basic_info p, salary_info s where p.PBI_ID=s.PBI_ID and p.PBI_JOB_STATUS='In Service' and p.PBI_DEPARTMENT like '".$_POST['dept']."' and p.PBI_ORG='".$_POST['PBI_ORG']."' ".$con." and p.PBI_DOJ<'".$j_date."' order by (s.basic_salary+s.consolidated_salary) desc";
...
$leave_days_lv = 0;}
		...
		?>
        <? if($att->od>0) echo $att->od; else {if($new_emp_off>0) $new_emp_off; else $r_count;}?>
        <tr style="font-size:10px; padding:3px; "><td><?=++$S?></td>
          ...
          <td><?=$info->PBI_NAME?></td><td><?=$info->PBI_DESIGNATION?></td><td><?=$info->PBI_DEPARTMENT?></td>
...
<td align="center"><input name="od_<?=$info->PBI_ID?>" type="text" id="od_<?=$info->PBI_ID?>" style="font-size:10px; width:20px; min-width:20px;" size="2" maxlength="2" value="<?=($data->od=='')?'4':$data->od;?>" /></td>
<td align="center"><input name="hd_<?=$info->PBI_ID?>" type="text" id="hd_<?=$info->PBI_ID?>" style="font-size:10px; width:20px; min-width:20px;" size="2" maxlength="2" value="<? if($data->hd>0) echo $data->hd; else {if($new_emp_holy_day>0) echo $new_emp_holy_day; else echo $holy_day;}?>" /></td>
...
<td align="center"><input name="lwp_<?=$info->PBI_ID?>" type="text" id="lwp_<?=$info->PBI_ID?>" style="font-size:10px; width:28px; min-width:28px;" value="<?=($data->lwp=='')?$leave_days_lwp:$data->lwp;?>" size="4" maxlength="4"  onchange="cal_all(<?=$info->PBI_ID?>)"/></td>
<td align="center"><input name="lv_<?=$info->PBI_ID?>" type="text" id="lv_<?=$info->PBI_ID?>" style="font-size:10px; width:28px; min-width:28px;" value="<?=($data->lv=='')?$leave_days_lv:$data->lv;?>" size="4" maxlength="4"  onchange="cal_all(<?=$info->PBI_ID?>)"/></td>
...
<td align="center"><input name="deduction_<?=$info->PBI_ID?>" type="text" id="deduction_<?=$info->PBI_ID?>" style="font-size:10px; width:35px; min-width:20px;" value="<?=$data->deduction?>" size="8" maxlength="8" /></td>
