...
// Conditions based on POST data
/*if (!empty($_POST['PBI_BRANCH'])) { 
    $con .= " and p.PBI_BRANCH = '" . $_POST['PBI_BRANCH'] . "'";
}

if (!empty($_POST['PBI_ZONE'])) {
    $con .= " and PBI_ZONE = '" . $_POST['PBI_ZONE'] . "'";
}

if (!empty($_POST['PBI_GROUP'])) {
    $group .= " and p.PBI_GROUP = '" . $_POST['PBI_GROUP'] . "'";
}else{
    $group .= " and p.PBI_GROUP = ''";
}



if (!empty($_POST['PBI_DOMAIN'])) {
    $con .= " and PBI_DOMAIN = '" . $_POST['PBI_DOMAIN'] . "'";
}

if (!empty($_POST['JOB_LOCATION'])) {
    $con .= " and JOB_LOCATION = '" . $_POST['JOB_LOCATION'] . "'";
}

if (!empty($_POST['pbi_id_in'])) {
    $con .= " and p.PBI_ID in (" . $_POST['pbi_id_in'] . ")";
}
*/
// Initialize $con_dep
$con_dep = '';

// Check conditions and build the query part
if ($_POST['dept'] > 0 && !empty($_POST['dept'])) {
    $con_dep .= " and p.DEPT_ID = '" . $_POST['dept'] . "'";
}

// SQL query
    $sql = "SELECT p.* FROM personnel_basic_info p, salary_info s 
    WHERE p.PBI_ID = s.PBI_ID 
    AND p.PBI_JOB_STATUS = 'In Service' 
    AND p.PBI_ORG = '" . $_POST['PBI_ORG'] . "' " . $con_dep . " 
    ORDER BY p.PBI_CODE";
...
//echo $info->PBI_DUE_DOJ;
					...
					<tr style="font-size:10px; padding:3px; "><td><?=++$S?></td>
						...
						<?php if ((isset($att) && $att->ab > '6') || (isset($data) && $data->ab > '6')) { ?>
