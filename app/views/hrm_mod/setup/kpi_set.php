...
//   $data = mysqli_fetch_object($conn);
//   foreach($data as $key =>$value) {
//     $$key = $value;
//   }
// }
if (!isset($$unique)) $$unique = mysqli_insert_id($conn);
?>
<style type="text/css">
  .MATERNITY_LEAVE {
    display: none;
                  ...
                  <div class="x_content">
                    ...
                    <script type="text/javascript">
                      function DoNav(lk) {
                        ...
                        return GB_show('ggg',F '<?=SERVER_ROOT?>app/views/hrm_mod/<?=$root?>/<?=$input_page?>?<?=$unique?>='+lk,600,940)
                      }
                    </script>
                                      ...
                                      <div class="oe_form_sheet oe_form_sheet_width">
                                        ...
                                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                          ...
                                          $res = 'select k.id,
                                            ...
                                            (select PBI_NAME from personnel_basic_info where PBI_ID=k.PBI_ID) as KPI_FOR,
                                            (select PBI_NAME from personnel_basic_info where PBI_ID=k.LINE_MANAGER) as KPI_AUTHORITY,
                                            (select PBI_NAME from personnel_basic_info where PBI_ID=k.DEPT_HEAD) as Department_head,
                                        ...
                                        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-bordered table-sm">
                                          ...
                                          $res = 'select k.id,k.PBI_ID,
                                                      ...
                                                      (select PBI_NAME from personnel_basic_info where PBI_ID=k.LINE_MANAGER) as KPI_AUTHORITY,
                                                      (select PBI_NAME from personnel_basic_info where PBI_ID=k.DEPT_HEAD) as Department_head,
                                                      k.status from hrm_kpi_set k where 1 and status="Active"';
                                          ...
                                          ?>
                                            ...
                                            <td nowrap="nowrap"><?= $data->KPI_AUTHORITY ?></td>
                                            <td nowrap="nowrap"><?= $data->Department_head ?></td>
