...
$str.='<table width="100%" border="1" cellspacing="1" cellpadding="1">
  ...
  // $ss = mysqli_query("select a.*,p.PBI_NAME,p.PBI_DOJ from performance_appraisal a, personnel_basic_info p where a.PBI_ID=p.PBI_ID and a.id='".$_GET['asign_id']."'");

	//  $data = mysqli_fetch_object($ss);
	 
  // Prepare the statement
$stmt = $conn->prepare("SELECT a.*, p.PBI_NAME, p.PBI_DOJ FROM performance_appraisal a JOIN personnel_basic_info p ON a.PBI_ID = p.PBI_ID WHERE a.id = ?");
if ($stmt === false) {
    die("Prepare failed: " . $conn->error);
}

// Bind the parameter
$asign_id = $_GET['asign_id'];
$stmt->bind_param("i", $asign_id);
