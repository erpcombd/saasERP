...
$re_date = date('Y-m-d',strtotime($r_date)+(6*86400));
		...
		$entry_by = $_SESSION['user']['id'];
						...
						db_query($del_sql);
						$insSql = 
	'INSERT INTO hrm_roster_allocation( PBI_ID, roster_date, point_1, shedule_1, job_location,group_for, entry_by) 
	VALUES 
	("'.$info->PBI_ID.'", "'.$rp3_date.'", "'.$point.'", "'.$shedule.'", "'.$_POST['job_location'].'", "'.$_POST['group_for'].'", "'.$entry_by.'")';
						db_query($insSql);
				 		$rp3_date = date ("Y-m-d", strtotime("+1 day", strtotime($rp3_date)));
				 } 
}
  header('Location:roster_day_set_all.php');
} // end save
...
<select name="s_<?=$info->PBI_ID?>_<?=$rp2_date?>" id="s_<?=$info->PBI_ID?>_<?=$rp2_date?>" style="width:70px; font-size:12px">
<? foreign_relation('hrm_schedule_info','id','schedule_name',$shedule[$info->PBI_ID][$rp2_date],'group_for = "'.$_POST['group_for'].'"');?>
</select></td>
