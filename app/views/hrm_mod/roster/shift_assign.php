...
//roster_start_date < "' . $s_date . '" AND
 $sSql = 'SELECT id, PBI_ID FROM hrm_roster_assign WHERE PBI_ID="' . $key . '" AND  roster_start_date <"' . $s_date . '" AND shedule_1="' . $roster_shedule . '"';
$sQuery = db_query($sSql);
$sData = mysqli_fetch_object($sQuery);

if ($sData->id > 0) {

     $upSql = 'UPDATE hrm_roster_assign 
    SET  roster_end_date = "' . $new_end_date . '",  edit_at="' . $edit_at . '", edit_by = "' . $edit_by . '" WHERE id=' . $sData->id;
    db_query($upSql);
    ...
    // Check the condition if you want to insert data or not
     $checkSql = 'SELECT id FROM hrm_roster_assign WHERE PBI_ID="' . $key . '" AND roster_start_date < "' . $s_date . '" AND shedule_1="' . $roster_shedule . '"';
    $checkQuery = db_query($checkSql);
    $checkData = mysqli_fetch_object($checkQuery);

    if ($checkData->id > 0) {
        // Do nothing or handle the condition where shedule_1 is equal to roster_shedule
    } else {
	
	    $last_id  = find_a_field('hrm_roster_assign','id','PBI_ID="'.$key.'" and shedule_1!="'.$roster_shedule.'" order by id desc');
	   
	     $upSql = 'UPDATE hrm_roster_assign 
    SET  roster_end_date = "' . $new_end_date . '",  edit_at="' . $edit_at . '", edit_by = "' . $edit_by . '" WHERE id=' . $last_id;
    db_query($upSql);
...
//___________________ END _______________
