...
$tdate = date("d");
if($tdate>26 && $tdate<32){
$limit_date = date('Y-').(date('m')).'-25';
}else {
$limit_date = date('Y-').(date('m')-1).'-25';
}

if(isset($_POST['save'])){

if(strtotime($_POST['roster_date'])>strtotime($limit_date)){
...
db_query($del_sql);
}else { echo 'Sorry Your Session Time Out!..';}
...
?>
<center><h1 style="color: #66CC66">Update Successful</h1> </center>
<?
} else { ?>
<center><h1 style="color: #FF0000">You can't Edit Old Roster Information</h1> </center>
<? 
}
}
?>
    ...
    <div class="oe_view_manager_body">
      ...
      <div class="oe_view_manager_view_form">
        <div style="opacity: 1; text-align: right;" class="oe_formview oe_view oe_form_editable">
                  ...
                  <div class="oe_form_sheet oe_form_sheet_width">
                    <div  class="oe_view_manager_view_list">
                      <div  class="oe_list oe_view">
                        <table width="100%" border="0" class="oe_list_content" style="text-align:center">
                          <thead>
                            <tr class="oe_list_header_columns">
                              <th colspan="2"><span style="text-align: center; font-size:18px; color:#09F">Roster Day Set For 7 Days </span></th>
                            </tr>
                            <tr class="oe_list_header_columns">
                              <th height="23" colspan="2"><span style="text-align: center; font-size:16px; color:#C00">Select Options</span></th>
                            </tr>
                          </thead>
                          <tfoot>
                          </tfoot>
                          <tbody>
                            ...
                            <tr >
                              ...
                              <td align="right"><div align="left"><span class="oe_form_group_cell"> </span><span class="oe_form_group_cell">
                                <input name="roster_date" type="text" id="roster_date" 
								value="<? if($_POST['roster_date']>0){echo $_POST['roster_date'];}else{echo date("Y-m-d");}?>" required  class="form-control"/>
                              </span></div></td>
                            </tr>
                            
                            <tr>
                              <td align="center" style="text-align: right"><div align="right"><strong>Company : </strong></div></td>
                              <td><select name="group_for" id="group_for"   onchange="getData2('ajax_location.php', 'loc', this.value,  this.value)" required class="form-control">
		  ...
		  <td><span id="loc"><select name="job_location" id="job_location" class="form-control" >
<? foreign_relation('office_location', 'ID', 'LOCATION_NAME',$_POST['job_location'],'1 and ID !=15')?>
                ...
                <input type='hidden' name='area' id='area' value='1' /></td>
                             ...
                             <tr>
                               <td align="center" style="text-align: right"><strong>Department :</strong></td>
                               <td><span class="oe_form_group_cell">
                     <input name="department"id="department" value="<?=$dept_id; ?>" readonly="readonly" class="form-control">
                              
							   </span></td>
                             </tr>
                             <tr>
                               <td align="center" style="text-align: right"><strong>Section:</strong></td>
                               <td><strong>
								 <input name="PBI_DOMAIN" id="PBI_DOMAIN" value="<?=$sec_name; ?>"class="form-control">
                               </strong></td>
                             </tr>
                             <tr>
...
$re_date = date('Y-m-d',strtotime($r_date)+(6*86400));
		...
		$sql = "select a.PBI_NAME,a.PBI_ID,a.PBI_DESIGNATION,a.PBI_DEPARTMENT from personnel_basic_info a
		where  1 ".$con."  and a.PBI_JOB_STATUS='In Service' order by a.PBI_ID ";
...
<td><select name="s_<?=$info->PBI_ID?>_<?=$rp2_date?>" id="s_<?=$info->PBI_ID?>_<?=$rp2_date?>" style="width:70px; font-size:12px">
<? foreign_relation('hrm_schedule_info','id','schedule_name',$shedule[$info->PBI_ID][$rp2_date],'group_for = "'.$_POST['group_for'].'" and schedule_type not in("IOM")');?></select></td>
                                ...
                                <th colspan="4"><table width="100%" border="0" cellpadding="5" cellspacing="0" bgcolor="#FF6633">
                                    <tr>
                                      <td align="center"><div align="center">
                                        <input name="save" type="submit" id="save" value="SET SCHEDULE" />
                                      </div></td>
                                    </tr>
                                </table></th>
                              </tr>
