...
if(isset($_POST['save']))
{
	...
	echo $sql = "select PBI_ID from 
		personnel_basic_info
		where  employee_type LIKE '%roster%'";
		$query = db_query($sql);
		...
		while($info=mysqli_fetch_object($query))
		{
			...
			for ( $i = $startTime; $i <= $endTime; $i = $i + 86400 ) {
			  $s_date = date( 'Y-m-d', $i );
			  $del_sql='delete from roster_date_wise where s_date="'.$s_date.'" and PBI_ID='.$info->PBI_ID;
			  db_query($del_sql);

			//db_query('update personnel_basic_info set office_time="'.$schedule_id.'",off_day="'.$_POST['off_day'].'" where PBI_ID='.$info->PBI_ID);
			db_query('insert into roster_date_wise (PBI_ID,s_date,e_date,sch_time,off_day,area) 
			values("'.$info->PBI_ID.'","'.$s_date.'","'.$s_date.'","'.$_POST['schedule_id'].'","'.$_POST['off_day'].'","'.$_POST['area'].'")');
			}
...
<tr class="oe_list_header_columns">
  <th colspan="3"><span style="text-align: center; font-size:18px; color:#09F">Roster Day Set Date Wise</span></th>
  </tr>
<tr class="oe_list_header_columns">
...
<table width="100%" class="oe_list_content">
  <thead>
<tr class="oe_list_header_columns">
  <th colspan="3"><table width="100%" border="0" cellpadding="5" cellspacing="0" bgcolor="#FF6633">
    <tr>
      <td width="45%"><div align="" class="style1">Chose Date: </div></td>
      <td width="27%"><input name="s_date" type="text" id="s_date" value="<?php echo date('Y-m-d')?>" /></td>
	  <td width="27%"><input name="e_date" type="text" id="e_date" value="<?php echo date('Y-m-d')?>" /></td>
    </tr>
	
	<tr>
      <td width="45%"><div align="" class="style1">New Schedule: </div></td>
...
<div class="oe_form_sheetbg">
		...
		echo $sql = "select a.PBI_NAME,a.PBI_ID,c.DESG_DESC,d.DEPT_DESC,
		(select s.schedule_name from hrm_schedule_info s where s.id=a.office_time) as schedule_name, 
		a.off_day 
		from 
		personnel_basic_info a,designation c, department d
		where a.DESG_ID=c.DESG_ID and a.DEPT_ID=d.DEPT_ID ".$con."  and a.PBI_JOB_STATUS='In Service' order by a.PBI_ID ";
		$query = db_query($sql);
		while($info=mysqli_fetch_object($query))
		{
		?>
