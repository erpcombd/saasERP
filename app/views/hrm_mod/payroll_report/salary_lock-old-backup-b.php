...
if(isset($_REQUEST['lock'])){
  ...
  //$updatee1 = 'update salary_journal set total_deduction="'.$dataa1->total_ded.'",total_payable="'.$dataa1->total_pay.'",cheked_by="'.$_SESSION['user']['id'].'" where mon="'.$_POST['mon'].'" and year="'.$_POST['year'].'" and department="'.$dataa1->department.'"';
   ...
   $sss = 'select proj.PROJECT_DESC,proj.account_ledger ,a.job_location,sum(a.gross_salary+a.food_allowance+a.transport_allowance+a.other_allowance+a.adjustment_amount+a.offday_allowance+a.sitevisit_allowance) as total_pay,sum(a.absent_deduction+a.late_deduction+a.lwp_deduction+a.hr_action_amt+a.other_deduction+a.other_deductions) as total_deduction from salary_attendence a, personnel_basic_info b, project proj where a.PBI_ID = b.PBI_ID and mon='.$_POST['mon'].' and year='.$_POST['year'].' and b.PBI_JOB_STATUS="In Service"  and  a.job_location = proj.PROJECT_ID and a.pay>0 GROUP BY proj.PROJECT_ID';
   ...
   add_to_sec_journal('Aksid', $jv_no, $jv_date, $ledger_id_dr, $narration_dr, $total_payable, '0', $tr_from, $tr_no,0,0,22,2);
   
   //add_to_journal('Aksid', $jv_no1, $jv_date, $ledger_id_dr, $narration_dr,  $total_payable, '0', $tr_from, $tr_no,0,0,22,2);
   
   add_to_journal('Aksid', $jv_no1, $jv_date, $ledger_id_dr, $narration_dr, $total_payable, '0', $tr_from, $tr_no,$sub_ledger='',$tr_id='',$cc_code='22',$c_no='',$c_date='',$receive_ledger='');
   }
...
//department brief salary to secondary journal
		...
		$sqli11 = 'select dept.DEPT_DESC,dept.account_ledger,a.department, sum(a.gross_salary+a.food_allowance+a.transport_allowance+a.other_allowance+a.adjustment_amount+a.offday_allowance+a.sitevisit_allowance) as total_pay,
		sum(a.absent_deduction+a.late_deduction+a.lwp_deduction+a.hr_action_amt+a.other_deduction+a.other_deductions) as total_deduction from salary_attendence a, personnel_basic_info b, department dept where a.PBI_ID = b.PBI_ID and mon='.$_POST['mon'].' and year='.$_POST['year'].'  and b.PBI_JOB_STATUS="In Service"   and a.department = dept.DEPT_ID and  a.pay>0 and dept.DEPT_ID not in (13)  GROUP BY dept.DEPT_ID  ';
   ...
   $row = db_query($insert11);
     ...
     add_to_sec_journal('Aksid', $jv_no, $jv_date, $ledger_id_dr, $narration_dr, $total_payable1, '0', $tr_from, $tr_no,0,0,22,2);
   //add_to_journal('Aksid', $jv_no1, $jv_date, $ledger_id_dr, $narration_dr,  $total_payable1, '0', $tr_from, $tr_no,0,0,22,2);
    
   add_to_journal('Aksid', $jv_no1, $jv_date, $ledger_id_dr, $narration_dr, $total_payable1, '0', $tr_from, $tr_no,$sub_ledger='',$tr_id='',$cc_code='22',$c_no='',$c_date='',$receive_ledger='');
...
if(isset($_REQUEST['m_lock'])){
   ...
   $sss = 'select proj.PROJECT_DESC,proj.acc_ledger_for_mobile_bill ,a.job_location, sum(a.mobile_bill_amt) as total_amt from salary_attendence a, personnel_basic_info b, project proj where a.PBI_ID = b.PBI_ID and mon='.$_POST['mon'].' and year='.$_POST['year'].' and b.PBI_JOB_STATUS="In Service"  and  a.job_location = proj.PROJECT_ID  GROUP BY proj.PROJECT_ID';
		$query12 = db_query($sss);
   ...
   add_to_sec_journal('Aksid', $jv_no, $jv_date, $ledger_id_dr, $narration_dr, $mdata->total_amt, '0', $tr_from, $tr_no);
   
   add_to_journal('Aksid', $jv_no1, $jv_date, $ledger_id_dr, $narration_dr,  $mdata->total_amt, '0', $tr_from, $tr_no);
   }
...
//department brief salary to secondary journal
		...
		$sqli11 = 'select dept.DEPT_DESC,dept.acc_ledger_for_mobile_bill,a.department, sum(a.mobile_bill_amt) as total_amt from salary_attendence a, personnel_basic_info b, department dept where a.PBI_ID = b.PBI_ID and mon='.$_POST['mon'].' and year='.$_POST['year'].'  and b.PBI_JOB_STATUS="In Service"   and a.department = dept.DEPT_ID  and dept.DEPT_ID not in (13)  GROUP BY dept.DEPT_ID  ';
		$query13 = db_query($sqli11);
    while($dataa1 = mysqli_fetch_object($query13)){
...
if(isset($_REQUEST['b_lock'])){
   ...
   $insert2 = 'INSERT INTO `salary_bonus_lock`(`bonus_type`, `year`, `PBI_ID`, `pbi_department`, `pbi_designation`, `pbi_organization`, `pbi_job_location`, `pbi_doj`, `job_status`, `gross_salary`, `basic_salary`, `bonus_percentage`, `cut_off_date`, `job_period`, `bonus_days`, `bonus_percent`, `bonus_amt`, `bank_paid`, `cash_paid`, `entry_by`, `entry_at`, `remarks`) VALUES ("'.$data2->bonus_type.'","'.$data2->year.'","'.$data2->PBI_ID.'","'.$data2->pbi_department.'","'.$data2->pbi_designation.'","'.$data2->pbi_organization.'","'.$data2->pbi_job_location.'","'.$data2->pbi_doj.'","'.$data2->job_status.'","'.$data2->gross_salary.'","'.$data2->basic_salary.'","'.$data2->bonus_percentage.'","'.$data2->cut_off_date.'","'.$data2->job_period.'","'.$data2->bonus_days.'","'.$data2->bonus_percent.'","'.$data2->bonus_amt.'","'.$data2->bank_paid.'","'.$data2->cash_paid.'","'.$data2->entry_by.'","'.$data2->entry_at.'","'.$data2->remarks.'")';
   ...
   $sss = 'select proj.PROJECT_DESC ,proj.account_ledger, sum(a.bonus_amt) as total_amt from salary_bonus a, personnel_basic_info b, project proj where a.PBI_ID = b.PBI_ID and bonus_type='.$_POST['bonus_type'].' and year='.$_POST['year'].' and b.PBI_JOB_STATUS="In Service" and a.pbi_job_location = proj.PROJECT_ID  GROUP BY proj.PROJECT_ID';
		$query12 = db_query($sss);
   ...
   add_to_sec_journal('Aksid', $jv_no, $jv_date, $ledger_id_dr, $narration_dr, $total_payable, '0', $tr_from, $tr_no);
   
   add_to_journal('Aksid', $jv_no1, $jv_date, $ledger_id_dr, $narration_dr,  $total_payable, '0', $tr_from, $tr_no);
   }
...
//department brief salary to secondary journal and journal
   ...
   add_to_sec_journal('Aksid', $jv_no, $jv_date, $ledger_id_dr, $narration_dr, $total_payable1, '0', $tr_from, $tr_no);
   add_to_journal('Aksid', $jv_no1, $jv_date, $ledger_id_dr, $narration_dr,  $total_payable1, '0', $tr_from, $tr_no);
        ...
        <tr style="font-size:10px; padding:3px; ">
		 ...
		 <tr>
		   <? $found = find_a_field('salary_attendence','lock_status','mon="'.$_POST['mon'].'" and year="'.$_POST['year'].'"');
		    if($found == 0){
		  ?>
		   <td colspan="6"><div align="center"><input type="submit" name="lock" value="Lock" style="width:100px; height:30px" /></div></td>
		   <? } else{?>
...
?>
        <tr style="font-size:10px; padding:3px; ">
		 ...
		 <? $found = find_a_field('salary_attendence','m_lock','mon="'.$_POST['mon'].'" and year="'.$_POST['year'].'"');
		    if($found == 0){
		  ?>
		   <td colspan="6"><div align="center"><input type="submit" name="m_lock" value="Lock" style="width:100px; height:30px" /></div></td>
		   <? } else{?>
		    <td colspan="6"><div align="center"><strong>Already Locked</strong></div></td>
		   <? } ?>
		 </tr>
...
?>
        <tr style="font-size:10px; padding:3px; ">
		 ...
		 <? $found = find_a_field('salary_bonus','lock_status','bonus_type="'.$_POST['bonus_type'].'" and year="'.$_POST['year'].'"');
		    if($found == 0){
		  ?>
		   <td colspan="6"><div align="center"><input type="submit" name="b_lock" value="Lock" style="width:100px; height:30px" /></div></td>
		   <? } else{?>
		    <td colspan="6"><div align="center"><strong>Already Locked</strong></div></td>
		   <? } ?>
		 </tr>
