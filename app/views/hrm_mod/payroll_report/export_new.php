  ...
  <table width="100%" cellspacing="0" cellpadding="2" border="0">
    ...
    <thead>
      ...
      <tr>
        ...
        <td style="border:0px;"><div align="center" style="margin-left: -20%;"><?=$str?></div></td>
...
$found = find_a_field('salary_attendence','lock_status','mon="'.$_POST['mon'].'" and year="'.$_POST['year'].'"');
  ...
  if($found==0){  if($_POST['PBI_ORG']!='')	$salaryCon =' and a.PBI_ORG = "'.$_POST['PBI_ORG'].'"';

if ($_POST['JOB_LOCATION'] !='')	$salaryCon .= ' and t.job_location='.$_POST['JOB_LOCATION'];

if ($_POST['department'] !='')	$salaryCon .= ' and t.department='.$_POST['department'];

if ($_POST['job_status'] !='')$salaryCon .=' and a.PBI_JOB_STATUS="'.$_POST['job_status'].'"'; 

     $sqld = 'select t.*, a.PBI_ID, a.PBI_NAME, a.PBI_DOJ, d.DESG_SHORT_NAME as Designation
 ...
 if($_POST['PBI_ORG']!='')	$salaryConn =' and a.PBI_ORG = "'.$_POST['PBI_ORG'].'"';

if ($_POST['JOB_LOCATION'] !='')	$salaryConn .= ' and t.job_location='.$_POST['JOB_LOCATION'];

if ($_POST['department'] !='')	$salaryConn .= ' and t.department='.$_POST['department'];

            $m_s_date = $_POST['year'].'-'.$_POST['mon'].'-'.'01';		   $m_e_date = $_POST['year'].'-'.$_POST['mon'].'-'.'31';      $sqld = 'select t.*, a.PBI_ID, a.PBI_NAME, a.PBI_DOJ, d.DESG_SHORT_NAME as Designation
...
$m_s_date = $_POST['year'].'-'.$_POST['mon'].'-'.'01';$m_e_date = $_POST['year'].'-'.$_POST['mon'].'-'.'31';$entry_by=$data->entry_by;  //$slq = 'select sum(total_days) from hrm_leave_info where PBI_ID="'.$data->PBI_ID.'" and type=1 and s_date>="'.$m_s_date.'" and e_date<="'.$m_e_date.'" and  leave_status="GRANTED"';

 $tot_ded = $data->other_deduction+$data->other_deductions;//for joining  deduction
 //$join_date_diff = date('Y-m-d', strtotime($data->PBI_DOJ. ' -2 day'));

 $join_date_org = date('Y-m-d', strtotime($data->PBI_DOJ));

 $join_date_diff = date('Y-m-d',(strtotime ( '-1 day' , strtotime ($data->PBI_DOJ) ) ));$joindate = date_parse_from_format("Y-m-d", $data->PBI_DOJ);
...
?>
      ...
      <tr>
        ...
        <td><?  $dd = 'select s_date,e_date from hrm_leave_info where s_date>="'.$m_s_date.'" and PBI_ID="'.$data->PBI_ID.'" and leave_status="GRANTED" and type in (1)';				 				$dumping = db_query($dd);				$day_count = '';				$last_date = $_POST['year'].'-'.$_POST['mon'].'-'.'31';				while($leave_d = mysqli_fetch_object($dumping)){								 $d = date_parse_from_format("Y-m-d", $leave_d->e_date);                  $next_m =  $d["month"];				  				   if($_POST['mon']<$next_m){				   				   $e_date = $last_date;				   				   }else{				   $e_date = $leave_d->e_date;				   }				$s_date = $leave_d->s_date;				$e_date = date('Y-m-d H:i:s', strtotime($e_date . ' +1 day'));						 $begin = new DateTime($s_date);        $end = new DateTime($e_date);

      $interval = DateInterval::createFromDateString('1 day');      $period = new DatePeriod($begin, $interval, $end);	

    foreach ($period as $dt) {	        $dt->format("l Y-m-d H:i:s\n");    $today = $dt->format("Y-m-d");    if($dt->format("l")!='Friday')    {       $found = 0;       $found = find_a_field('salary_holy_day','1',' holy_day="'.$today.'" ');       if($found==0)       $day_count++;	         }}  }  echo $day_count;?>

        </td>

        <td><?  $dd = 'select s_date,e_date from hrm_leave_info where s_date>="'.$m_s_date.'" and PBI_ID="'.$data->PBI_ID.'" and leave_status="GRANTED" and type in (2)';				 				$dumping = db_query($dd);				$day_count = '';				$last_date = $_POST['year'].'-'.$_POST['mon'].'-'.'31';				while($leave_d = mysqli_fetch_object($dumping)){								 $d = date_parse_from_format("Y-m-d", $leave_d->e_date);                  $next_m =  $d["month"];				  				   if($_POST['mon']<$next_m){				   				   $e_date = $last_date;				   				   }else{				   $e_date = $leave_d->e_date;				   }				   				   				   				 				$s_date = $leave_d->s_date;																				$e_date = date('Y-m-d H:i:s', strtotime($e_date . ' +1 day'));						 $begin = new DateTime($s_date);        $end = new DateTime($e_date);

      $interval = DateInterval::createFromDateString('1 day');      $period = new DatePeriod($begin, $interval, $end);	

    foreach ($period as $dt) {	        $dt->format("l Y-m-d H:i:s\n");    $today = $dt->format("Y-m-d");    if($dt->format("l")!='Friday')    {       $found = 0;       $found = find_a_field('salary_holy_day','1',' holy_day="'.$today.'" ');       if($found==0)       $day_count++;	         }}  }  echo $day_count;?></td>

        <td><?  $dd = 'select s_date,e_date from hrm_leave_info where s_date>="'.$m_s_date.'" and PBI_ID="'.$data->PBI_ID.'" and leave_status="GRANTED" and type in (3)';				 				$dumping = db_query($dd);				$day_count = '';				$last_date = $_POST['year'].'-'.$_POST['mon'].'-'.'31';				while($leave_d = mysqli_fetch_object($dumping)){								 $d = date_parse_from_format("Y-m-d", $leave_d->e_date);                  $next_m =  $d["month"];				  				   if($_POST['mon']<$next_m){				   				   $e_date = $last_date;				   				   }else{				   $e_date = $leave_d->e_date;				   }     
				$s_date = $leave_d->s_date;         $e_date = date('Y-m-d H:i:s', strtotime($e_date . ' +1 day'));						 $begin = new DateTime($s_date);        $end = new DateTime($e_date);
    ...
    foreach ($period as $dt) {	        $dt->format("l Y-m-d H:i:s\n");    $today = $dt->format("Y-m-d");    if($dt->format("l")!='Friday')    {       $found = 0;       $found = find_a_field('salary_holy_day','1',' holy_day="'.$today.'" ');       if($found==0)       $day_count++;	         }}  }  echo $day_count;?></td>
        ...
        <td align="right"><? if($data->offday_allowance+$data->sitevisit_allowance+$data->food_allowance+$data->transport_allowance+$data->other_allowance>0){echo $data->offday_allowance+$data->sitevisit_allowance+$data->food_allowance+$data->transport_allowance+$data->other_allowance;	 $totAllowance +=$data->offday_allowance+$data->sitevisit_allowance+$data->food_allowance+$data->transport_allowance+$data->other_allowance;	}	?></td>
        ...
        <td align="right"><?=($data->late_deduction>0)? $data->late_deduction : '';           $totLateDeduct += $data->late_deduction?></td>
         ...
         <td align="right"><?=($data->joining_deduction>0)? $data->joining_deduction : '';           $deduction_total_amount += $data->joining_deduction?></td>                <td align="right"><?=($tot_ded>0)? $tot_ded : '';       $totOtherDeduct += $tot_ded?></td>
...
?>
      ...
      <tr>
        ...
        <td align="right"><strong>          <?=($totOther>0)? number_format($totOther,0) : '';?>
        ...
        <td align="right"><strong>
          ...
          <td align="right"><strong><?=($deduction_total_amount>0)? number_format($deduction_total_amount,0) : '';?></strong></td>
        <td align="right"><strong>
...
<form method="post" action="export.php">
 <input type="submit" name="export" class="btn btn-success" value="Export" />
...
</div>  <div style="width:100%; margin:60px auto">
    ...
    <div style="float:left; width:25%; text-align:center">-------------------<br>      Managing Director</div>
