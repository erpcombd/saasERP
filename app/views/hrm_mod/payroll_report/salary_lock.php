...
"'.$data->total_earning_cash.'")';
  ...
  //$updatee1 = 'update salary_journal set total_deduction="'.$dataa1->total_ded.'",total_payable="'.$dataa1->total_pay.'",cheked_by="'.$_SESSION['user']['id'].'" where mon="'.$_POST['mon'].'" and year="'.$_POST['year'].'" and department="'.$dataa1->department.'"';
   ...
   $daynamic_days=cal_days_in_month(CAL_GREGORIAN,$_POST['mon'],$_POST['year']);
   
   $date = $_POST['year'].'-'.$_POST['mon'].'-'.$daynamic_days;
...
sum(a.absent_deduction+a.joining_deduction+a.late_deduction+a.lwp_deduction+a.hr_action_amt+a.other_deduction+a.other_deductions) as total_deduction,
sum(a.income_tax+a.food_deduction+a.mobile_deduction) as credit_total_deduction from salary_attendence a, personnel_basic_info b, department dept where a.PBI_ID = b.PBI_ID and mon='.$_POST['mon'].' and year='.$_POST['year'].'  and b.PBI_JOB_STATUS="In Service"   and a.department = dept.DEPT_ID and  a.pay>0 and dept.DEPT_ID not in (13,10,25)  GROUP BY dept.DEPT_ID  ';
  ...
  // ################################ START    Casual Staff Salary Automation  // Management salary only  ################################################
...
sum(a.absent_deduction+a.joining_deduction+a.late_deduction+a.lwp_deduction+a.hr_action_amt+a.other_deduction+a.other_deductions) as total_deduction,
sum(a.income_tax+a.food_deduction+a.mobile_deduction) as credit_total_deduction,e.ESS_DEPARTMENT 

from salary_attendence a,personnel_basic_info b,essential_info e
where a.PBI_ID = b.PBI_ID and a.PBI_ID=e.PBI_ID and mon='.$_POST['mon'].' and year='.$_POST['year'].' and e.ESS_DEPARTMENT=25 and  e.acledger_for_salary_casual_staff>0 and 
b.PBI_JOB_STATUS="In Service"  and  a.pay>0 GROUP BY a.PBI_ID';
	...
	$narration_dr_management = 'Management Salary expense for the month of ' .$_POST['mon']. ' and year of ' .$_POST['year'];
    $narration_cr_management = 'Management Salary Payable for the month of ' .$_POST['mon']. ' and year of ' .$_POST['year'];
    ...
    $ledger_id_crc = 2063000200000000;
     ...
     add_to_sec_journal('Aksid', $jv_no, $jv_date, $ledger_id_drc, $narration_dr_management, $total_payable1c, '0', $tr_from, $tr_no,0,0,$dep_cc_code_drc,2);
	 add_to_sec_journal('Aksid', $jv_no, $jv_date, $ledger_id_crc, $narration_cr_management, '0', $total_payable_credit1c, $tr_from, $tr_no,0,0,$dep_cc_code_drc,2);

   //add_to_journal('Aksid', $jv_no1, $jv_date, $ledger_id_dr, $narration_dr,  $total_payable1, '0', $tr_from, $tr_no,0,0,22,2);

   add_to_journal('Aksid', $jv_no1, $jv_date, $ledger_id_drc, $narration_dr_management, $total_payable1c, '0', $tr_from, $tr_no,$sub_ledger='',$tr_id='',$dep_cc_code_drc,$c_no='',$c_date='',$receive_ledger='');
   add_to_journal('Aksid', $jv_no1, $jv_date, $ledger_id_crc, $narration_cr_management,'0',$total_payable_credit1c, $tr_from, $tr_no,$sub_ledger='',$tr_id='',$dep_cc_code_drc,$c_no='',$c_date='',$receive_ledger='');
...
// START ================ ````````````````````````````````  department brief mobile bill to secondary journal
		...
		$sqli11 = 'select dept.DEPT_DESC,dept.acc_ledger_for_mobile_bill,dept.cc_code,a.department, sum(a.mobile_bill_amt) as total_amt,sum(a.mobile_deduction) as deduction_amt  
		from salary_attendence a, personnel_basic_info b, department dept 
		where a.PBI_ID = b.PBI_ID and mon='.$_POST['mon'].' and year='.$_POST['year'].'  and b.PBI_JOB_STATUS="In Service"   and a.department = dept.DEPT_ID  
		and dept.DEPT_ID not in (13)  GROUP BY dept.DEPT_ID  ';
