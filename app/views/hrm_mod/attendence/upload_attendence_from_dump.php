...
echo $sql="SELECT EMP_CODE,a.roster_date,min(d.xtime) in_time,max(d.xtime) out_time,a.shedule_1,p.schedule_type,p.PBI_ID

FROM hrm_attdump d, hrm_roster_allocation a , hrm_schedule_info s,personnel_basic_info p

WHERE  a.PBI_ID=p.PBI_ID and d.type!=2 and  d.EMP_CODE=a.PBI_ID and  d.xdate in (d.xdate,DATE_ADD(d.xdate, INTERVAL 1 DAY)) and  

a.roster_date between '".$start_date."' and '".$end_date."' and shedule_1=s.id and  

d.xtime between   DATE_ADD(concat(a.roster_date,' ',s.office_start_time), INTERVAL ".min_in." HOUR) and 

DATE_ADD(concat(a.roster_date,' ',s.office_end_time), INTERVAL ".max_out." HOUR) ".$job_location_con.$emp_id_con.$department_con." group by a.PBI_ID,a.roster_date";
