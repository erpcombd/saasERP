...
if($_POST['JOB_LOCATION']>0) $job_location_con2 = " and p.JOB_LOC_ID='".$_POST['JOB_LOCATION']."'";
...
and iom_id = 0 and leave_id = 0
and h.att_date BETWEEN '".$start_date."' AND '".$end_date."' 
".$emp_id_con.$job_location_con.$ORG_con.$department_con;
$query = db_query($sql);



	
  $sql = " SELECT EMP_CODE,a.roster_date,min(d.xtime) in_time,max(d.xtime) out_time,a.shedule_1,p.schedule_type
...
('".$value[$x]['emp_id']."','".$value[$x]['att_date']."', '".$value[$x]['sch_id']."','".$value[$x]['in_time']."','".$value[$x]['out_time']."',

dayname('".$value[$x]['att_date']."'), '".$value[$x]['sch_in_time']."','".$value[$x]['sch_mid_time']."','".$value[$x]['sch_out_time']."')";
...
in_time="'.$value[$x]['in_time'].'", out_time="'.$value[$x]['out_time'].'", sch_in_time="'.$value[$x]['sch_in_time'].'",sch_mid_time="'.$value[$x]['sch_mid_time'].'",sch_out_time="'.$value[$x]['sch_out_time'].'" where  emp_id="'.$value[$x]['emp_id'].'" and att_date="'.$value[$x]['att_date'].'" ';
