...

{
   ...
   <!--      </tr> -->
      ...
      while($row=mysqli_fetch_object($res))
		...
		{
              $leave_id = find_a_field('hrm_leave_info', 'id', 'PBI_ID = "' . $row->PBI_ID . '" AND "' . $t_date . '" BETWEEN s_date AND e_date');
        
                
		?>

		<? if($xenrollid[$row->PBI_ID]<1 && $leave_id ==0) { $status='Absent'?>
        <tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
     ...
     while($info=mysqli_fetch_object($query)){
      ...
      $in_longitudee[$info->EMP_CODE]=$info->longitude;
       ...
       $sql = "select EMP_CODE,  max(xtime) as max_time  from hrm_attdump where xdate ='".$t_date."' group by EMP_CODE ";
		...
		//dept_head_status='pending'and incharge_status='pending'and
		$sql = "select id,PBI_ID,incharge_status,leave_status  from  hrm_leave_info where   '" . $t_date . "' BETWEEN s_date AND e_date  group by PBI_ID ";
		 ...
		 $leave[$info->PBI_ID]=$info->PBI_ID;
		  $incharge_status[$info->PBI_ID]=$info->incharge_status;
                $leave_status[$info->PBI_ID]=$info->leave_status;
          }
...
elseif($_REQUEST['report']==210907001) {
    ...
    @page {
        size: A4;
        margin: 10mm; /* reduce top/bottom margins to avoid extra blank page */
    }
    ...
    table {
        page-break-inside: auto; /* allow table to split naturally */
    }

    tr {
        page-break-inside: avoid; /* prevent a row from breaking across pages */
...
if($user_id == 10073 || $user_id == 10074 || $user_id == 10108) {$JOB_LOC_ID_BLOCK = " and p.JOB_LOC_ID !=3";}
	...
	$sql = "SELECT p.* 
        ...
        WHERE EXISTS (
            SELECT 1 FROM hrm_attdump h 
            WHERE h.EMP_CODE = p.PBI_ID 
              AND h.xdate = '".$t_date."'
        ) 
        ".$dept_con.$JOB_LOC_ID_BLOCK.$cost_con.$org_con.$emp_status_con.$location_con.$class_con.$section_con."
        ORDER BY p.class, p.cost_center";
	$res	 = db_query($sql);
	while($row=mysqli_fetch_object($res))
	{
