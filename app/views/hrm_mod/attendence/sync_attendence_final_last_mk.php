...
// Main Data Fetch
   ...
   $sql = "SELECT h.*,p.define_schedule,p.define_offday,p.punch_type
...
if($late_min>7) $final_late_status = 1;
...
if(($sch_outs_datetime[$data->emp_id][$data->att_date]>$outs_datetime[$data->emp_id][$data->att_date]) && ($sch_off_day[$data->emp_id][$data->att_date] == 0) && ($ins_datetime[$data->emp_id][$data->att_date]>0))
...
if($ot_applicable[$data->emp_id]=='YES' && $data->leave_type != 'Last Half' &&  $data->iom_type!='Last Half') {	



if(($outs_datetime[$data->emp_id][$data->att_date]>0)  && ($sch_off_day[$data->emp_id][$data->att_date] == 0) ){
...
$ot_min = (int)(($outs_datetime[$data->emp_id][$data->att_date] - $sch_outs_datetime[$data->emp_id][$data->att_date]) / 60);
	...
	$ot_hour = (int)($ot_final_min/60);
	 ...
	 $ot_f_min = ($ot_final_min / 60); //($ot_final_min%60);
...
//	if($sch_break[$data->sch_id]=='YES' &&  $ot_min > 60){
...
//		$ot_min =$ot_min-$ot_hour_adjust[$data->emp_id];
...
//  	$ot_final_min = ($ot_min+$ot_extra[$data->att_date][$data->emp_id]);

//

//	$ot_final_hour = (int)($ot_final_min/60);
