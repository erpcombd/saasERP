...
<tr class="oe_list_header_columns" style="text-align:center">
  ...
  <tr >
    ...
    <td colspan="3"><span class="oe_form_group_cell">
      <select required name="PBI_GROUP" id="PBI_GROUP" style="" class="form-control">
        <? 
...
$month_find = find_a_field('hrm_portal_setup','mon','1 and level=59');
...
<option value="<?=$month_find;?>"><?=$monthName;?></option>
    ...
    <td><span class="oe_form_group_cell">
      <select required="required" name="PBI_BRANCH" id="PBI_BRANCH"   onchange="getData2('ajax_zone.php', 'loc', this.value,  this.value)" class="form-control">
        <? 
  ...
  <tr >
    ...
    <td><span id="loc">
      <select name="PBI_ZONE" id="PBI_ZONE" class="form-control">
        <?  //and REGION_ID="'.$_POST['PBI_BRANCH'].'"
...
$leave_days_lv = 0;}
	...
	?>
        ...
        <tr style="font-size:10px; padding:3px; "><td><?=++$S?></td>
          ...
          <td><?=$info->PBI_NAME?></td><td><?=find_a_field('designation','DESG_SHORT_NAME','DESG_ID='.$info->DESG_ID)?></td><td><?=find_a_field('area','AREA_NAME','AREA_CODE="'.$info->PBI_AREA.'"');?></td>
...
<td align="center" style="color: tomato"><?=$att->do_amt?>&nbsp;</td>
