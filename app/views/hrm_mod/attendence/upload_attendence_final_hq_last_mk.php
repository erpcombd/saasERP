...
'holy_day between "' . $start_date . '" and "' . $end_date . '" and job_loc_id in (1,3) '
...
$ORG_con = " and p.PBI_ORG='" . $PBI_ORG . "'";
 ...
 $sql ="SELECT h.emp_id,(count(1)-sum(h.leave_duration)) pre, sum(h.final_late_min) l_min,sum(h.final_late_status) l_status,sum(h.ot_final_hour) as total_over_time,p.PBI_DOJ,p.PBI_ID,sum(h.final_early_status) as early_out,sum(h.final_late_status) as total_late,sum(final_late_min) as total_late_min,p.DEPT_ID FROM `hrm_att_summary` h,personnel_basic_info p WHERE 1 and h.present=1  and p.PBI_ID=h.emp_id and h.att_date BETWEEN '" .$start_date ."' AND '" .$end_date ."' " .$emp_con .$ORG_con .$job_location_con.
...
while ($data = mysqli_fetch_object($query)) {
 ...
 $sqll ="SELECT h.*,p.PBI_DOJ FROM `hrm_att_summary` h,personnel_basic_info p WHERE 1 and p.PBI_ID=h.emp_id and  (h.sch_off_day=1 or h.holyday=1)  and h.att_date BETWEEN '" .$start_date ."' AND '" .$end_date ."' and h.emp_id=".$data->emp_id."  ";
...
$values[$pi]["early_out"] = $data->early_out;
 ...
 $hollyday_office = find_a_field('hrm_att_summary','sum(holyday)','dayname!="Friday" and att_date between "'.$start_date.'" and "'.$end_date.'" and emp_id="'.$data->PBI_ID.'" and leave_id=0 and iom_id=0');
...
$values[$pi]['pay'] = ($values[$pi]['pre'] + $values[$pi]['hd'] + $values[$pi]['od'])-($values[$pi]["lwp"]+$values[$pi]['day_deduct']);



$values[$pi]['ab'] = (($values[$pi]['td']+$values[$pi]['day_deduct'])-($values[$pi]['pre'] + $values[$pi]['hd'] + $values[$pi]['od']));
