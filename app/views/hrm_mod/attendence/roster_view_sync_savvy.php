...
//START 1 Deleting All Previous Data
$sql = "update hrm_att_summary h, personnel_basic_info p set h.deleted=0, h.panalty_leave_duration=0, h.holyday=0
WHERE p.PBI_ID=h.emp_id 
and p.employee_type = 'Roster'
and h.att_date BETWEEN '".$start_date."' AND '".$end_date."' 
...
//	echo $sql = "SELECT xenrollid , xdate , max(xtime) out_time  
//	FROM `hrm_attdump` h, hrm_machice_type m,personnel_basic_info p
//	
//	WHERE p.PBI_ID=h.xenrollid 
//	and out_time<concat(xdate,' 10:00:00')
//	and p.employee_type = 'Roster'
//	and xdate BETWEEN '".$start_date."' AND '".$end_date."' 
//	and h.xmechineid=m.mac_id and m.mac_type!='In' 
//	".$ORG_con.$emp_id_con."
...
//	
////	if($data->out_time<($date.' 10:00:00'))
//	echo $out_time[$data->xenrollid][$date] = $data->out_time;
//	
//	}
	
	
// out time of a non regular day
...
//elseif($office_start_time[$x]=='0000-00-00 00:00:00')
//{
...
//		if($sch_out>$exit_time) {$deleted[$x] = 1;}
//		
//	echo $sql="INSERT INTO hrm_att_summary 
//	(emp_id, att_date, in_time,out_time, sch_in_time, sch_out_time,  dayname, 
//	panalty_leave_duration, deleted, late_min, grace_no, final_late_min, final_late_status, process_time)
//	VALUES 
...
<tr><td height="40" colspan="4" bgcolor="#00FF00"><div align="center" class="style1">Collect Attendance Data For Savvy </div></td></tr>
...
<select name="PBI_ORG" style="width:160px;" id="PBI_ORG">
  ...
  <option value="12">Savvy Foods</option>
</select>
</span></td>
</tr>
<tr>
