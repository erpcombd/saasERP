...
/*	if( $_GET['asign_id']>0 ){
		 ...
		 $emp_id = $all_info->PBI_ID;

		 
          $attendance_date = $_GET['asign_id'];
				   
      
				   
		       echo  $sql = "UPDATE hrm_att_summary SET in_time = '0000-00-00 00:00:00', out_time = '0000-00-00 00:00:00'
				  WHERE  emp_id = '$emp_id' AND att_date = '$attendance_date'";
				 $update_result =  db_query($sql);
				  
				echo  $sql = "UPDATE hrm_attdump SET type=2 ,entry_by = '$user_id'
			...
			if ($update_result) {
				...
				echo "<script>
					$(document).ready(function() {
						swal({
							title: 'Attendance Successfully Clear.',
		 ...
		 $emp= $all_info->PBI_ID;  
          ...
          foreach ( $display as $key  ) {
		         ...
		         $sql = "UPDATE hrm_att_summary SET in_time = '0000-00-00 00:00:00', out_time = '0000-00-00 00:00:00'
				  WHERE  emp_id = '$emp' AND att_date = '$key'";
				 $update_result =  db_query($sql);
				  
				  $sql = "UPDATE hrm_attdump SET type=2 ,entry_by = '$user_id'
				  WHERE  EMP_CODE = '$emp' AND xdate = '$key'";
				 $update_result =  db_query($sql);
...
<!-- <td bgcolor="<?=$bgcolor?>">
<button type="submit" name="clear" class="btn btn-success">
<a style="color:white" href="time_adjustment_absent.php?att_date=<?=$day_date;?>&emp_code=<?=$val[$day_date]['emp_id'];?>">Absent</a></button>
