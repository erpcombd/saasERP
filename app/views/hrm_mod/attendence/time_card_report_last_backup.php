...
<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["save"])) {
		 ...
		 $emp= $all_info->PBI_ID;  //$_POST['PBI_IDD'];
		...
		$selectedoutTime = $_POST['punch_out'];
				 ...
				 $att_sql = "INSERT INTO hrm_attdump ( ztime, bizid, xenrollid, xdate, xtime,EMP_CODE,time,entry_by,type) 
					...
					echo $att_sql1 = "INSERT INTO hrm_attdump ( ztime, bizid, xenrollid, xdate, xtime,EMP_CODE,time,entry_by,type) 
  ...
  ?>
      ...
      <?  if (isset($_POST['submit'])) {
              ...
              for ($i = 0; $i < count($_POST['emp_id']); $i++) {
				...
				$dump_emp = $emp_id_no;  //$_POST['PBI_IDD']; 
         ...
         echo $found = find_a_field('hrm_att_summary','1','in_time ="'.$access_time_in.'" and out_time ="'.$access_time_out.'" and id="'.$id.'" ');
          
         // $system_inTime = find_a_field('hrm_att_summary','in_time','id="'.$id.'" ');
          //$system_outTime = find_a_field('hrm_att_summary','out_time','id="'.$id.'" ');
		 ...
		 if($found==0) {
               ...
               if($in_time>0 || $out_time>0){
                 ...
                 echo $sql = "UPDATE hrm_att_summary SET in_time = '$access_time_in' , out_time='$access_time_out' WHERE id = $id";
                db_query($sql);
                	}
			 ...
			 $att_sql1 = "INSERT INTO hrm_attdump ( ztime, bizid, xenrollid, xdate, xtime, EMP_CODE,time,entry_by,type) 
				VALUES ('$access_time_in', '$dump_emp', '$dump_emp', '$date_att', '$access_time_in','$dump_emp','$in_time','$user_id',1)";
				$att_query1=db_query($att_sql1);
               ...
               if($out_time>0){
				$access_time = $i . ' ' . $selectedoutTime;
			 echo $att_sql1 = "INSERT INTO hrm_attdump ( ztime, bizid, xenrollid, xdate, xtime, EMP_CODE,time,entry_by,type) 
				VALUES ('$access_time_out', '$dump_emp', '$dump_emp', '$date_att', '$access_time_out','$dump_emp','$in_time','$user_id',1)";
				$att_query1=db_query($att_sql1);
...
?>
	...
	value="<?php echo ( date("H:i" , strtotime($val[$day_date]['in_time'])) > '00:00:00')  ? date("H:i", strtotime($val[$day_date]['in_time'])) : '00:00 pm'; ?>">
...
value="<?php echo ( date("H:i", strtotime($val[$day_date]['out_time'])) > '00:00:00' ) ? date("H:i", strtotime($val[$day_date]['out_time'])) : '00:00 pm'; ?>">
