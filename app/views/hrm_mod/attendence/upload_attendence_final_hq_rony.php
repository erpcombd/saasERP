...
$m_s_date = $_POST['s_date'];

$m_e_date = $_POST['e_date'];
...
$days_in_month = round($diff / (60 * 60 * 24))+1;

$days_mon = $days_in_month;
...
"' ".$job_location_con."  And leave_id>'0'
...
//$sql ="SELECT h.emp_id,(count(1)-sum(h.leave_duration)) pre, sum(h.final_late_min) l_min,sum(h.final_late_status) l_status,sum(h.ot_final_min) as total_over_time,p.PBI_DOJ,p.PBI_ID,sum(h.final_early_status) as early_out,sum(h.final_late_status) as total_late,sum(final_late_min) as total_late_min,p.DEPT_ID FROM `hrm_att_summary` h,personnel_basic_info p WHERE 1 and p.PBI_ID=h.emp_id and h.att_date BETWEEN '" .$start_date ."' AND '" .$end_date ."' " .$emp_con .$ORG_con .$job_location_con." and if(p.DEPT_ID!=52,h.dayname!='Friday',h.dayname!='') group by h.emp_id";



 $sql ="SELECT h.emp_id,(count(1)-sum(h.leave_duration)) pre, sum(h.final_late_min) l_min,sum(h.final_late_status) l_status,sum(h.ot_final_min+h.ot_extra_min) as total_over_time,p.PBI_DOJ,p.PBI_ID,sum(h.final_early_status) as early_out,sum(h.final_late_status) as total_late,sum(final_late_min) as total_late_min,p.DEPT_ID FROM `hrm_att_summary` h,personnel_basic_info p WHERE 1 and h.present=1  and p.PBI_ID=h.emp_id and h.att_date BETWEEN '" .$start_date ."' AND '" .$end_date ."' " .$emp_con .$ORG_con .$job_location_con.
"  group by h.emp_id";
...
while ($data = mysqli_fetch_object($query)) {
 ...
 $sqll ="SELECT h.* FROM `hrm_att_summary` h,personnel_basic_info p WHERE 1 and p.PBI_ID=h.emp_id and  (h.sch_off_day=1 or h.holyday=1)  and h.att_date BETWEEN '" .$start_date ."' AND '" .$end_date ."' and h.emp_id=".$data->emp_id."  ";
...
'" and job_loc_id in ('.$_POST['JOB_LOCATION'].',3)  ');
...
//if ($data->PBI_DOJ > $current_mon_date) {
...
$holy_sql = db_query('select * from salary_holy_day where holy_day between "'.$start_date.'" and "'.$end_date.'"');
...
$check_lv_iom_in_hd = find_a_field('hrm_att_summary','id','att_date="'.$holy_data->holy_day.'" and emp_id="'.$data->PBI_ID.'"');
...
$values[$pi]['od'] = $values[$pi]['od']-($iom_in_friday+$leave_in_hd_od);



$values[$pi]['pre'] = $values[$pi]['pre']-$friday_office;



$values[$pi]['day_deduct']= $day_deduct[$data->PBI_ID];

$values[$pi]['pay'] = ($values[$pi]['pre'] + $values[$pi]['lv'] + $values[$pi]['hd'] + $values[$pi]['od'])-($values[$pi]["early_out"]+$values[$pi]["lwp"]+$values[$pi]['day_deduct']);

$values[$pi]['ab'] = (($values[$pi]['td']+$values[$pi]["early_out"]+$values[$pi]['day_deduct'])-($values[$pi]['pre'] + $values[$pi]['lv'] + $values[$pi]['hd'] + $values[$pi]['od']));
...
(`mon`, `year`, `PBI_ID`, `td`, `od`, `hd`, `lt`, `total_late_min`, `eo`,`ab`, `lv`,`iom`,`lwp`, `pre`, `pay`,`ot`,`hourly_leave`, `entry_at`, `entry_by`,`day_deduct`) values



('" .$mon."','".$year."','" .$values[$y]["emp_id"] ."','" .$values[$y]["td"] ."','" .$values[$y]["od"] ."', '" .$values[$y]["hd"] ."','" .$values[$y]["lt"] ."','" .$values[$y]["total_late_min"] ."', '" .$values[$y]["early_out"] ."', '" .$values[$y]["ab"] ."','" .$values[$y]["lv"] ."','" .$values[$y]["iom"] ."','" .$values[$y]["lwp"] ."','" .$values[$y]["pre"] ."','" .$values[$y]["pay"] ."','" .$values[$y]["total_over_time"] ."','" .$values[$y]["hourly_leave"] ."','" .date("Y-m-d H:i:s")."','".$_SESSION["user"]["id"]."','".$values[$y]['day_deduct']."')";
...
"',day_deduct='".$values[$y]['day_deduct']."' where mon='" .
