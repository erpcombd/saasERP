...
$m_e_date = $year . "-" . $mon . "-" . $days_mon;
 ...
 $sql ="SELECT h.*,s.*,p.DESG_ID,p.DEPT_ID,p.PBI_ORG,p.JOB_LOC_ID,p.class,p.PBI_JOB_STATUS,p.cost_center,p.section
...
$values[$pi]["convenience"] = $data->convenience;
$values[$pi]["food_allowance"] = $data->food_allowance;
$values[$pi]["ta_da_data"] = $data->ta;
$values[$pi]["special_allowance"] = $data->special_allowance;
$values[$pi]['mobile_allowance'] = $data->mobile_allowance;
$values[$pi]["bank_or_cash"] = $data->cash_bank;
$values[$pi]["over_time_amount"] = round(((($data->basic_salary)/104)*($data->ot)));
...
if($data->total_late_min>0){
	...
	if($data->class==3){
	$late_min_deduction = round(((($data->basic_salary)/(14400))*($data->total_late_min)));
	
	 }else{
	if($data->total_late_min>10)
	$late_min_deduction = round(((($data->gross_salary)/(14400))*($data->total_late_min)));
	}
...
$values[$pi]["joining_deduction"] = ((($data->gross_salary)/($days_mon))*($joining_ab_days));
...
$values[$pi]["resign_deduction"] = ((($data->gross_salary)/($days_mon))*($resign_ab_days));
}
...
//$values[$pi]["late_deduction"] = round(((($data->basic_salary)/(30))*($late_deduct_days)));


//________ Leave Balance Check 
$total_late_count = $data->lt+$data->eo;
$late_deduct_day = ((int) $total_late_count/3);
$late_deduct_days = floor($late_deduct_day);


if(($data->class!=3) && ($total_late_count>3)){

$g_s_date = date($year . '-01-01');
$g_e_date = date($year . '-12-31');


$joiningdate = find_a_field('personnel_basic_info', 'PBI_DOJ', 'PBI_ID='.$data->PBI_ID);
...
$annual_leave_opening = find_a_field('annual_leave_balance', 'BALANCE', 'PBI_ID="' .$data->PBI_ID. '"');
$annual_leave_earn = find_a_field('hrm_att_summary', 'sum(annual_leave)', 'emp_id="' .$data->PBI_ID. '" and att_date BETWEEN  "' . $g_s_date . '" 
 and "' . $g_e_date . '" ');
...
if ($joiningYear < $currentYear && $days_between_pre >= 365) {
      ...
      $al_entitlement = $total_annual_allocated; 
       ...
       $cl_entitlement = roundToQuarter(10 / 360 * $days_between_pre);
       $al_entitlement = 0;
...
$cl_balance = ($cl_entitlement-$cl_availd);	
  ...
  if($total_late_count>3 || $total_late_count<6){
     ...
     if ($al_balance > 0) {
		...
		VALUES ('".$end_date."','Approve', 'Full' , '" .$data->PBI_ID."',3, '" .$end_date. "' , '" .$end_date. "'  ,'1', 'Late Beyond 4', 'GRANTED' , 1)";
	   ...
	   $cl_full_day_query = "INSERT INTO hrm_leave_info( leave_apply_date , incharge_status,half_or_full , PBI_ID, type, s_date,e_date,total_days,reason,
		...
		VALUES ('".$end_date."','Approve', 'Full' ,'" .$data->PBI_ID."',1, '" .$end_date. "' , '" .$end_date. "' ,'1', 'Late Beyond 4', 'GRANTED' , 1)";
	   ...
	   $lwp_query = "INSERT INTO hrm_leave_info( leave_apply_date , incharge_status , half_or_full , PBI_ID, type, s_date,e_date,total_days,reason,
		...
		VALUES ('".$end_date."','Approve', 'Full' ,'" .$data->PBI_ID."', 9 , '" .$end_date. "' , '" .$end_date. "' ,'1','Late Beyond 4', 'GRANTED' , 1)";
		$result = db_query($lwp_query);
		
	  }
	
   
 }elseif($total_late_count>6){
   
   $values[$pi]["late_deduction"] = round(((($data->basic_salary)/(30))*($late_deduct_days)));
   
 }else{
 
   $values[$pi]["late_deduction"] = 0;
...
//Late Beyound 4 tretment end 

if($data->class==3){
$values[$pi]["absent_deduction"] = round(((($data->basic_salary)/(30))*(($data->lwp+$data->ab))));
}else{
$values[$pi]["absent_deduction"] = round(((($data->gross_salary)/(30))*(($data->lwp+$data->ab))));
}
...
$values[$pi]["total_earning"] = round((($data->gross_salary) - $values[$pi]["absent_deduction"]-$values[$pi]['late_min_deduction']-
$values[$pi]["joining_deduction"]-$values[$pi]["resign_deduction"]))+$values[$pi]["total_benefits"]+$values[$pi]["adjustment_amount"];




$payable_before_stamp = round((($data->gross_salary) - $total_deduction_befor_stamp))+$values[$pi]["total_benefits"]+$values[$pi]["adjustment_amount"];
...
$values[$pi]["total_payable"]  = round((($data->gross_salary) - $values[$pi]["total_deduction"]))+$values[$pi]["total_benefits"]+$values[$pi]["adjustment_amount"];
...
'" .$values[$y]["mtd"] ."', '" .$values[$y]["td"] ."','" .$values[$y]["od"] ."', 
...
gross_salary='" .$values[$y]["gross_salary"] ."', basic_salary='" .$values[$y]["basic_salary"] ."', house_rent='" .$values[$y]["house_rent"] ."' ,

convenience='" .$values[$y]["convenience"] ."', food_allowance='" .$values[$y]["food_allowance"] ."', special_allowance='" .$values[$y]["special_allowance"] ."',
