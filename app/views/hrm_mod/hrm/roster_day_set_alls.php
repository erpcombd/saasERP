  ...
  <div class="oe_view_manager oe_view_manager_current">
    <div class="oe_view_manager_body">
      ...
      <div class="oe_view_manager_view_form">
                          ...
                          <tbody>
                            ...
                            <tr >
                              ...
                              <td align="right"><div align="left"><span class="oe_form_group_cell"> </span><span class="oe_form_group_cell">
                                <input name="roster_date" type="text" id="roster_date" value="<?=$_POST['roster_date']?>" />
                              </span></div></td>
                            </tr>
                            <tr >
                              <td align="right"><div align="right"><strong>Designation :</strong></div></td>
                            ...
                            <tr>
                            <td align="right"><div align="right"><strong>Job Location : </strong></div></td><td><select name="job_location" id="job_location" >
                                  ...
                                  <table width="100%" class="oe_list_content">
                                    <thead>
                                      <tr class="oe_list_header_columns" style="font-size:10px;padding:3px;">
                                        ...
                                        <th>LOC - <?=$rp1_date?></th>
                                        <th>SCH - <?=$rp1_date?></th>
										<? $date = date ("Y-m-d", strtotime("+1 day", strtotime($rp1_date)));} ?>
                                      </tr>
                                    </thead>
                                    <tbody>
                                      <?
		...
		echo $ros = "select * from hrm_roster_allocation where PBI_ID='".$info->PBI_ID."' and roster_date between '".$r_date."' and '".$re_date."' ";
		$ros_r = db_query($ros);
		while($roster = mysqli_fetch_object($ros_r)){
		$point[$roster->PBI_ID][$roster->roster_date]=$roster->point_1;
...
<td><select name="p_<?=$info->PBI_ID?>_<?=$rp2_date?>" id="p_<?=$info->PBI_ID?>_<?=$rp2_date?>" style="width:100px;"><? foreign_relation('hrm_roster_point','id','point_short_name',$point[$info->PBI_ID][$rp2_date],'group_for = "'.$_POST['group_for'].'"');?></select></td>
<td><select name="s_<?=$info->PBI_ID?>_<?=$rp2_date?>" id="s_<?=$info->PBI_ID?>_<?=$rp2_date?>" style="width:100px;"><? foreign_relation('hrm_schedule_info','id','schedule_name',$shedule[$info->PBI_ID][$rp2_date],'group_for = "'.$_POST['group_for'].'"');?></select></td>
<? // $date = date ("Y-m-d", strtotime("+1 day", strtotime($rp2_date)));} ?>                           </tr>
<?	}?>
                                    </tbody>
                                    <tfoot>
                                      <tr>
