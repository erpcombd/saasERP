...
header("Location:../report/do_view.php?v_no=".$_REQUEST['do_no']);
	break;
		...
		where m.do_no=o.do_no and m.dealer_code=d.dealer_code and i.item_id=o.item_id  and m.status in ('CHECKED','COMPLETED') ".$date_con.$item_con.$item_brand_con.$pg_con.$depot_con.' 
...
{
$con.=' and d.group_for="'.$_SESSION['user']['group'].'"';
echo $sql="select m.do_no,m.do_date,m.entry_at ,m.entry_time,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,w.warehouse_name as depot,d.product_group as grp, m.rcv_amt,concat(m.payment_by,':',m.bank,':',m.remarks) as Payment_Details from 
sale_other_master m,dealer_info d  , warehouse w
where m.status in ('CHECKED','COMPLETED')  and m.dealer_code=d.dealer_code and w.warehouse_id=d.depot".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con.$con." order by m.do_date,m.do_no";
$query = db_query($sql);
...
where w.group_for='".$_SESSION['user']['group']."' and dl.do_no=m.do_no and m.status in ('CHECKED','COMPLETED')  and m.dealer_code=d.dealer_code and m.dealer_code not in (30005,30019) and w.warehouse_id=d.depot".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con." group by d.dealer_code order by d.dealer_name_e";
$query = db_query($sql);
?>
<table width="100%" cellspacing="0" cellpadding="2" border="0">
...
$sql="select m.do_no,m.do_date,concat(d.dealer_code,'- ',d.dealer_name_e) as dealer_name,w.warehouse_name as depot,d.product_group as grp, m.rcv_amt,concat(m.payment_by,':',m.bank,':',m.remarks) as Payment_Details,d.dealer_code,sum(dl.total_amt) as dp_t,sum(dl.t_price*dl.total_unit) as tp_t from 
sale_other_master m,dealer_info d  , warehouse w, sale_other_details dl
where w.group_for='".$_SESSION['user']['group']."' and dl.do_no=m.do_no and m.status in ('CHECKED','COMPLETED')  and m.dealer_code=d.dealer_code and m.dealer_code in (30005,30019) and w.warehouse_id=d.depot".$depot_con.$date_con.$pg_con.$dealer_con.$dtype_con." group by d.dealer_code order by d.dealer_name_e";
$query = db_query($sql);
?>

<tr><th>S/L</th><th>Dealer Name</th><th>Area</th><th>Depot</th><th>Grp</th><th>Rcv Amt</th><th>DP Total</th><th>TP Total</th></tr>
...
$tp_t = $tp_t+$data->tp_t;
?>
<tr><td><?=++$s2?></td><td><?=$data->dealer_name?></td><td><?=$data->area?></td><td><?=$data->depot?></td><td><?=$data->grp?></td><td style="text-align:right"><?=$data->rcv_amt?></td><td><?=number_format($data->dp_t,2)?></td><td><?=number_format($data->tp_t,2)?></td></tr>
<?
}
?><tr class="footer"><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td style="text-align:right"><?=number_format($rcv_t,2)?></td><td>&nbsp;</td><td><?=number_format($dp_t,2)?></td><td><?=number_format($tp_t,2)?></td></tr></tbody></table>
<? 
}
...
{
?>
<table width="100%" cellspacing="0" cellpadding="2" border="0"><thead><tr><td style="border:0px;" colspan="7"><?=$str?><div class="left"></div><div class="right"></div><div class="date">Reporting Time: <?=date("h:i A d-m-Y")?>222</div></td></tr>
...
and i.finish_goods_code not between 4999 and 6000
and i.finish_goods_code not between 2000 and 2010
and w.group_for=2
and a.tr_from in ("Bulk Sales","Staff Sales","Other Sales","Gift Issue","Entertainment Issue","Sample Issue")
'.$date_con.'
group by a.item_id';

$query = db_query($sql);
while($info = mysqli_fetch_object($query)){
$all_sales_amt[$info->item_id] = $info->amount;
$all_sales_pcs[$info->item_id] = $info->qty;
}
...
FROM  journal_item a,item_info i , sales_do_chalan c
where
c.chalan_type = "Delivery"
and c.chalan_no = a.sr_no
...
and i.finish_goods_code not between 4999 and 6000
and i.finish_goods_code not between 2000 and 2010 ".$item_con.$item_brand_con.$pg_con.' 
		group by i.finish_goods_code';
?>
<table width="100%" cellspacing="0" cellpadding="2" border="0"><thead><tr><td colspan="7" style="border:0px;"><?=$str?></td></tr><tr><th height="20">S/L</th><th>Item Id</th><th>Code</th><th>Item Name</th><th>CTN</th><th>Pcs</th><th>Amount</th></tr></thead><tbody>
...
$total_qty_pcs = (int)($total_qty/$info->pack_size);
$total_qty_ctn = (int)($total_qty%$info->pack_size);
...
<tr>
    ...
    <td><?=$info->code;?></td>
    <td><?=$info->item_name;?></td>
    <td style="text-align:right"><?=$total_qty_pcs?></td>
    <td style="text-align:right"><?=$total_qty_ctn?></td>
    <td style="text-align:right"><?=number_format($total_amt,0)?></td>
</tr>
<?
}}
?>
<tr>
    ...
    <td style="text-align:right"><?=$t_total_qty_pcs?></td>
    <td style="text-align:right"><?=$t_total_qty_ctn?></td>
    <td style="text-align:right"><?=number_format($t_total_amt,0)?></td>
</tr>
</tbody></table>
<?
		$str = '';
