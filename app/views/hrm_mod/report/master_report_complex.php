...
break;
	...
	$report="Employee Information";
        ...
        $sql.="select a.PBI_ID as Emp_ID";
		
		if($_POST['PBI_NAME']==1) $sql.= ",a.PBI_NAME as Name";
		if($_POST['PBI_SEX']==1) $sql.= ",a.PBI_SEX as Gender";
		if($_POST['PBI_DESIGNATION']==1) $sql.= ",a.PBI_DESIGNATION as designation , a.PBI_DESG_GRADE as grade";
		
		
		if($_POST['PBI_FATHER_NAME']==1) $sql.= ",a.PBI_FATHER_NAME as Father_Name";
		if($_POST['PBI_MOTHER_NAME']==1) $sql.= ",a.PBI_MOTHER_NAME as Mother_Name";
		if($_POST['PBI_GARDIAN']==1) $sql.= ",(select concat('Name:',GUARDIAN_NAME,', Relation:',GUARDIAN_RELATION,', PRE-ADD:',GUARDIAN_PRESENT_ADDRESS,', PER-ADD-',GUARDIAN_PERMANENT_ADDRESS,', Mobile-',GUARDIAN_MOBILE) from guardian where PBI_ID=a.PBI_ID limit 1) as Gardian";
		...
		if($_POST['ESSENTIAL_BLOOD_GROUP']==1) $sql.= ",a.ESSENTIAL_BLOOD_GROUP as Blood_grp";
		if($_POST['ESSENTIAL_VOTER_ID']==1) $sql.= ",a.ESSENTIAL_VOTER_ID as National_ID";
		
		if($_POST['JOB_STATUS']==1) $sql.= ",a.PBI_JOB_STATUS as Job_status";
		
		if($_POST['PBI_DOMAIN']==1) $sql.= ",a.PBI_DOMAIN as Domain";
		if($_POST['PBI_ZONE']==1) $sql.= ",a.PBI_ZONE as Zone";
		if($_POST['PBI_BRANCH']==1) $sql.= ",a.PBI_BRANCH as Branch";
		if($_POST['PBI_AREA']==1) $sql.= ",a.PBI_AREA as Area";
		if($_POST['PBI_PROJECT']==1) $sql.= ",a.PBI_PROJECT as Project";
		
		if($_POST['PBI_REGION']==1) $sql.= ",(select region_name from region where region_id=a.PBI_REGION) as Region";
		if($_POST['PBI_DEPARTMENT']==1) $sql.= ",a.PBI_DEPARTMENT as Department";
		if($_POST['PBI_EDU_QUALIFICATION']==1) $sql.= ",a.PBI_EDU_QUALIFICATION as Edu_Qualification";
		
		
		
		if($_POST['resign_date']==1) $sql.= ",a.resign_date as resign_date";
		if($_POST['PBI_separation_type']==1) $sql.= ",a.PBI_separation_type as separation_type";
		if($_POST['PBI_SPECIALTYs']==1) $sql.= ",a.PBI_SPECIALTY as expertise";
		if($_POST['PBI_DOJ']==1) $sql.= ",a.PBI_DOJ as Initial_Joinning_date";
		if($_POST['PBI_SERVICE_LENGTH']==1) $sql.= ",concat(TIMESTAMPDIFF(YEAR,a.PBI_DOJ,now()),' Year & ', 
		( ((TIMESTAMPDIFF(MONTH,a.PBI_DOJ,now()))%12) ),' Month') as service_length";
		
		if($_POST['PBI_DOJ_PP']==1) $sql.= ",a.PBI_DOJ_PP as Joining_date_PP";
		if($_POST['service_length_pp']==1) $sql.= ",concat(TIMESTAMPDIFF(YEAR,a.PBI_DOJ_PP,now()),' Year & ', 
		( ((TIMESTAMPDIFF(MONTH,a.PBI_DOJ_PP,now()))%12) ),' Month') as service_length_PP";
		//if($_POST['PBI_SERVICE_LENGTH_PP']==1) $sql.= ",a.PBI_SERVICE_LENGTH_PP as service_length_PP";
		//if($_POST['present_age']==1) $sql.= ",a.present_age as present_age";
		if($_POST['PBI_DOC']==1) $sql.= ",a.PBI_DOC as confirmation_date";
		if($_POST['PBI_NATIONALITY']==1) $sql.= ",a.PBI_NATIONALITY as PBI_NATIONALITY";
		if($_POST['JOB_LOCATION']==1) $sql.= ",a.JOB_LOCATION as JOB_LOCATION";
		if($_POST['PBI_PRIMARY_JOB_STATUS']==1) $sql.= ",a.PBI_PRIMARY_JOB_STATUS as Initial_job_status";
		if($_POST['personal_file_status']==1) $sql.= ",a.personal_file_status as file_status";
		if($_POST['PBI_POB']==1) $sql.= ",a.PBI_POB as birth_place";
