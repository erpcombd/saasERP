...
order by (t.consolidated_salary+t.basic_salary) desc';
		...
		if ($_POST['report'] == 61) {
			...
			$full_sql = "select concat(date_format(a.att_date,'%Y%m')) as month ,a.emp_id,sum(a.leave_duration) as full from hrm_att_summary a,hrm_leave_info l where a.leave_id>0 and l.id=a.leave_id and l.type !='Adjust Leave' and l.half_or_full='Full' group by a.emp_id,concat(date_format(a.att_date,'%Y%m'))";
			...
			<table width="100%" cellspacing="0" cellpadding="2" border="0">
				<thead>
					...
					<tr>
						...
						<th>
							<div align="center">Half Or Full</div>
						</th>
						<th>
							<div align="center">Leave Type</div>
						</th>
						<th>
							<div align="center">Leave Days</div>
						</th>
						<th>
							<div align="center">Entry At</div>
						</th>
						<th>
				...
				<tbody>
					...
					$leave_sql = "SELECT l.*,a.PBI_CODE,a.PBI_NAME as Employee_Name,(select DESG_DESC from designation where DESG_ID=a.DESG_ID) as designation,
(select DEPT_DESC from department where DEPT_ID=a.DEPT_ID) as dept

FROM hrm_leave_info l, personnel_basic_info a

where a.PBI_ID=l.PBI_ID and l.half_or_full='Full' " . $con . $pbi_con . $date_con . $job_loc_con . "  ";
					...
					<tr>
						<td colspan="9">
							<div align="right"><b> Total Days</b></div>
						</td>
						<td><b><?= $grand_tot_leave ?></b></td>
			...
			<table style="width: 80%; margin: 0 auto;text-align:center;" cellpadding="0" cellspacing="0" class="oe_list_content">
				<thead>
					<tr class="oe_list_header_columns" style=" text-align:center; font-size:12px;" align="center">
						...
						<th align="center">
							<div align="center">Job Location/Project</div>
						</th>
						<th>
							<div align="center">Absent in days</div>
						</th>
					</tr>
				</thead>
				<tbody>
...
FROM hrm_att_summary l, personnel_basic_info a where   a.PBI_ID=l.emp_id " . $pbi_con . $org_con . $date_con . $con . " group by l.emp_id order by a.DEPT_ID";
					...
					<tr>
						<td colspan="6">
							<div align="right"><b> Total Minutes/Hours</b></div>
						</td>
						<td align="right"><b>
								<?= $grand_tot_leave ?>
							</b></td>
...
FROM hrm_att_summary l, personnel_basic_info a where   a.PBI_ID=l.emp_id " . $pbi_con . $org_con . $date_con . $con . " group by l.emp_id order by a.DEPT_ID";
 ...
 att_date between "' . $_POST['fdate'] . '" and  "' . $_POST['tdate'] . '"');
					...
					<tr>
						<td colspan="6">
							<div align="right"><b> Total Out Punch Missing Days</b></div>
						</td>


						<td align="right"><b><?= $grand_tot_leave_days ?></b></td>
					</tr>
				</tbody>
			</table>
			<style type="text/css">
		...
		?>
			...
			<table style="width: auto; margin: 0 auto; font-size: 20px;text-align:center;" border="1" bordercolor="#FFFFFF">
				<tr>
					<td style="border:0px solid white;"><strong><?= $str ?></strong></td>
				</tr>
			...
			<table style="width:auto;margin:0 auto;" cellpadding="0" cellspacing="0" border="1" id="ExportTable">
				...
				if ($_POST['job_status'] != '')
					$basicConn = ' and a.PBI_JOB_STATUS = "' . $_POST['job_status'] . '"';




				$basic_sql = "SELECT a.PBI_ID,a.PBI_NAME,a.DEPT_ID,a.PBI_DESIGNATION,a.PBI_DOJ

   FROM personnel_basic_info a  where 1  " . $con_pbi . $salaryConn . $basicConn . " 

   group by a.PBI_ID   order by a.DEPT_ID";
				...
				while ($r = mysqli_fetch_object($basic_query)) {
					...
					$present = find_a_field('hrm_att_summary', 'count(id)', 'emp_id="' . $r->PBI_ID . '" and dayname != "Friday" and att_date between "' . $_POST['fdate'] . '" and  "' . $_POST['tdate'] . '"');

					$leave = find_a_field('hrm_att_summary', 'count(leave_id)', 'emp_id="' . $r->PBI_ID . '" and leave_id>0 and att_date between "' . $_POST['fdate'] . '" and  "' . $_POST['tdate'] . '"');
				...
				?>
					...
					<tr>
						...
						<td><?= $attedance->hd ?></td>
						<td><?= $attedance->lt ?></td>
						<td><?= $absent_days - $friday_count ?></td>
						<td><?= $leave ?></td>
						<td><?= $present_days ?></td>
						<td><?= $payble_days ?></td>
						<td><?= $attedance->overtime_hour ?></td>
		...
		?>
			<table width="70%" cellspacing="0" id="ExportTable" cellpadding="2" border="0" style="margin:auto;">
				...
				<tbody>
					...
					if ($_POST['PBI_CODE'] !="") {
						$pbi_con = " and a.PBI_CODE='" . $_POST['PBI_CODE'] . "' ";
					}
					
						if ($_POST['incharge_id'] >0 ) {
						$pbi_con = " and a.incharge_id='" . $_POST['incharge_id'] . "' ";
					}



					if ($_POST['JOB_LOC_ID']>0) {
						$job_con = " and a.JOB_LOC_ID='" . $_POST['JOB_LOC_ID'] . "' ";
					}
					
					
					if ($_POST['cost_center']>0) {
						$job_con = " and a.cost_center='" . $_POST['cost_center'] . "' ";
					}
					
						if ($_POST['section']>0) {
						$job_con = " and a.section='" . $_POST['section'] . "' ";
					}
					...
					a.section,a.incharge_id,i.entry_at
                	 ...
                	 i.e_date<='" . $_POST['tdate'] . "' " . $con.$job_con. $pbi_con . " order by i.id desc";
...
FROM hrm_attdump l, personnel_basic_info a where   a.PBI_ID=l.EMP_CODE " . $pbi_con . $date_con . $con . " order by l.xenrollid,l.xtime asc";
			...
			<table class="new_table" cellspacing="0" cellpadding="2" border="0" style="margin:0 auto;">
				...
				<tr>
					<td colspan="4" style="border: 0px; padding: 0px;">
						<table width="100%" cellspacing="0" cellpadding="2" border="0" style="margin:0 auto;">
							<tr>
								<td width="15%" style=" border: 0px; ">
									<img src="<?=SERVER_ROOT?>public/uploads/logo/<?= $_SESSION['proj_id'] ?>.png" width="100%" class="logo-img" />
								</td>
								<td width="70%" style=" border: 0px; ">
									<h2 class="panel-title" align="center"><b><?= find_a_field('user_group', 'group_name', 'id=' . $basic_idd->PBI_ORG); ?></b></h2>
									<h3 class="panel-title" align="center"><b>Time Card</b></h3>
									<h3 class="panel-title" align="center">
						<b>For the month of <?=$monthName = date('F', mktime(0, 0, 0, $_POST["mon"], 1));?> <?=$_POST["year"];?></b></h3>
								</td>
								<td width="15%" style=" border: 0px; ">&nbsp;
						...
						<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
							<tbody>
								...
								<tr class="titel-font">
									...
									<td width="33%" align="left" style="border: 0px; padding: 5px 0px;">Employee ID: <strong><?= $basic_idd->PBI_CODE; ?></strong></td>
									<td width="40%" align="left" style="border: 0px; padding: 5px 0px;">Employee Name: <strong><?= find_a_field('personnel_basic_info', 'PBI_NAME', 'PBI_ID="' . $basic_idd->PBI_ID . '"'); ?></strong></td>
									<td width="27%" align="left" style="border: 0px; padding: 5px 0px;">Designation: <strong><?= find_a_field('designation', 'DESG_DESC', 'DESG_ID=' . $basic_idd->DESG_ID); ?></strong></td>

									<!--<td width="25%"  align="left" style="border: 0px; padding: 5px 0px;" >Employee ID: <strong><?= $basic_idd->PBI_CODE; ?></strong></td>-->
									<!--<td width="30%"  align="left" style="border: 0px; padding: 5px 0px;" >Employee Name: <strong><?= find_a_field('personnel_basic_info', 'PBI_NAME', 'PBI_ID="' . $basic_idd->PBI_ID . '"'); ?></strong></td>-->
									<!--<td width="20%"  align="left" style="border: 0px; padding: 5px 0px;" >Designation: <strong><?= find_a_field('designation', 'DESG_DESC', 'DESG_ID=' . $basic_idd->DESG_ID); ?></strong></td>-->
									<!--<td width="25%"  align="right" style="border: 0px; padding: 5px 0px;" >DOJ: <strong> <?= find_a_field('personnel_basic_info', 'PBI_DOJ', 'PBI_ID="' . $basic_idd->PBI_ID . '"'); ?> </strong></td>-->
								...
								<tr class="titel-font">
									...
									<td width="33%" align="left" style="border: 0px; padding: 5px 0px;">Cost Center: <strong>
											<?= find_a_field('hrm_cost_center', 'center_name', 'id=' . $basic_idd->cost_center); ?> </strong>
									</td>

									<td width="40%" align="left" style="border: 0px; padding: 5px 0px;">Depertment: <strong>
											<?= find_a_field('department', 'DEPT_DESC', 'DEPT_ID="' . $basic_idd->DEPT_ID . '"'); ?></strong>
									</td>
								...
								<tr class="titel-font">
									<td width="33%" align="left" style="border: 0px; padding: 5px 0px;"> Section: <strong>
											<?= find_a_field('PBI_Section', 'sec_name', 'sec_id=' . $basic_idd->section); ?> </strong></td>
									<td width="40%" align="left" style="border: 0px; padding: 5px 0px;">
										DOJ: <strong> <?= find_a_field('personnel_basic_info', 'PBI_DOJ', 'PBI_ID="' . $basic_idd->PBI_ID . '"'); ?> </strong>
									</td>
									<td width="27%" align="left" style="border: 0px; padding: 5px 0px;">DOL:<strong> </strong></td>
								</tr>


								<!--<tr>-->
								<!--<td align="left" style="border: 0px; padding: 0px;" > Work Station: <strong><?= find_a_field('hrm_workstation', 'work_station_name', 'station_id=' . $basic_idd->PBI_WORK_STATION); ?></strong></td>-->
								<!--<td align="left" style="border: 0px; padding: 0px;" > Class: <strong> <?= find_a_field('hrm_class', 'class_name', 'id=' . $basic_idd->class); ?></strong></td>-->
								<!--<td align="left" style="border: 0px; padding: 0px;" >Section: <strong> <?= find_a_field('hrm_workstation', 'work_station_name', 'station_id=' . $basic_idd->PBI_WORK_STATION); ?> </strong></td>-->
				...
				<tr>
					<td colspan="4" style="border: 0px; padding: 0px;">
						<table width="100%" cellspacing="0" cellpadding="2" border="0" style="margin:auto;" class="sr-table">
							<thead>
								<tr class="table-font">
									<!--<th>SL </th>-->
									<th>Date </th>
									...
									<th>OT Hours</th>
									<th align="center">
										<p style="margin: 0px; line-height: 15px; padding: 0px;font-weight: 600; text-align: center;" class="table-font">In Late</p>
										<p style="margin: 0px; line-height: 15px; text-align: center; padding: 0px;font-weight: 600;" class="table-font-sub">(Minute)</p>
									</th>
									<th align="center">
										<p style="margin: 0px; line-height: 15px; padding: 0px;font-weight: 600; text-align: center;" class="table-font">Early out</p>
										<p style="margin: 0px; line-height: 15px; text-align: center; padding: 0px;font-weight: 600;" class="table-font-sub">(Minute)</p>
									</th>
									<th>Status</th>
								</tr>
							...
							<tbody class="table-font">
								...
								$leave_sql = "SELECT l.*,a.PBI_NAME as Employee_Name,a.PBI_CODE,(select DEPT_DESC from department where DEPT_ID=a.DEPT_ID) as department
 FROM hrm_att_summary l, personnel_basic_info a where a.PBI_ID=l.emp_id " . $date_con . $NameConn . $IDConn . $codeConn . $con . " order by l.emp_id,l.att_date asc";
								...
								while ($data = mysqli_fetch_object($leave_query)) {
                                   ...
                                   $lave_types = find_a_field('hrm_leave_type','leave_type_name','id='.$lave_type_id);
								...
								?>
									<tr style="color:<?= $bgcolor ?>">
										...
										<td><? //= $val[$day_date]['final_status']; ?>
											...
											<? if($val[$day_date]['final_status']=='LEAVE') echo $val[$day_date]['leave_type']; 
				else echo $val[$day_date]['final_status'];?>
								...
								<tr class="table-font">
									...
									<td style=" border: 0px; "><!--<span>Absent Amendment: <?= $amendment; ?></span>--><span>Day off: 0</span> </td>
								</tr>
							</tbody>
						</table>
					</td>
				...
				<tr>
					<td colspan="4" style="padding: 0px;">
						<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
							<tr class="table-font">
								<td style=" border: 0px;">Present: <strong><?= $regular; ?></strong></td>
								<td style=" border: 0px;">Absent: <strong><?= $absent; ?></strong></td>
								<td style=" border: 0px;">Holiday: <strong><?= $off_days + $public_holy; ?> </strong></td>
								<td style=" border: 0px;">Leave: <strong><?= $leave; ?></strong></td>
								<td style=" border: 0px;">Absent Amendment: <strong> <?= $amendment; ?> </strong></td>
								<td style=" border: 0px;">Present In Holiday: <strong>0</strong></td>
								<td style=" border: 0px;">In Late: <strong><?= $late; ?></strong></td>
								<td style=" border: 0px;">Early Out: <strong><?= $early; ?></strong></td>
							</tr>
						</table>
					</td>
				</tr>
			...
			<table width="70%" cellspacing="0" cellpadding="2" border="0" style="margin:auto;">
				<thead>
					...
					<tr>
						...
						<th colspan="3">
							<div align="center">Leave Availed</div>
						</th>
						<th colspan="7">
							<div align="center">Leave Types (AVAIL)</div>
						</th>
						<th colspan="6">
							<div align="center">Leave Summary (BALANCE)</div>
						</th>
						<th rowspan="2">Total Leave</th>
						<th colspan="3">
							<div align="center">Leave Status</div>
						</th>
					</tr>
					<tr>
						<th>Full</th>
						<th>Half</th>
						<th>Leave For Late</th>
						<th>(CL)</th>
						<th>(SL)</th>
						<th>(AL)</th>
						<th>(ML)</th>
				...
				<tbody>
					...
					if ($_POST['year'] > 0) {
						...
						$leave_sql = "SELECT a.PBI_ID,a.PBI_CODE,a.PBI_NAME as Employee_Name,(select DEPT_DESC from department where DEPT_ID=a.DEPT_ID) as department,a.PBI_DOJ as joining_date  FROM personnel_basic_info a where 1 " . $con . $orgcon . " order by a.DEPT_ID";
					...
					?>
							...
							<tr>
								...
								<td><?= $data_leave->Employee_Name ?></td>
								<td><?= $data_leave->department ?></td>
								<td><?= $total_full_leave = find_a_field('hrm_leave_info', 'sum(total_days)', 'half_or_full="Full" and type!="Adjust Leave" and PBI_ID="' . $data_leave->PBI_ID . '" and s_date>="' . $startDate . '" and e_date<="' . $endDate . '" ') ?></td>

								<td><?= $total_half_leave = find_a_field('hrm_leave_info', 'sum(total_days)', 'half_or_full!="Full" and type!="Adjust Leave" and PBI_ID="' . $data_leave->PBI_ID . '" and s_date>="' . $startDate . '" and e_date<="' . $endDate . '" ') ?></td>
								...
								<td><? $cl_bl = find_a_field('hrm_leave_rull_manage', 'sum(CL)', '1');
									echo $cl_bl - $cl ?></td>

								<td><? $sl_bl = find_a_field('hrm_leave_rull_manage', 'sum(SL)', '1');
									echo $sl_bl - $sl ?></td>

								<td><? $al_bl = find_a_field('hrm_leave_rull_manage', 'sum(AL)', '1');
									echo $al_bl - $al ?></td>

								<td><? $ml_bl = find_a_field('hrm_leave_rull_manage', 'sum(ML)', '1');
									echo $ml_bl - $ml ?></td>

								<td><? $mlv_bl = find_a_field('hrm_leave_rull_manage', 'sum(MLV)', '1');
									echo $mlv_bl - $mlv ?></td>

								<td><? $pl_bl = find_a_field('hrm_leave_rull_manage', 'sum(MLV)', '1');
									echo $pl_bl - $pl ?></td>

								<td><?= $total_leave = $total_full_leave + $total_half_leave + $leave_late; ?></td>

								<td><?= $entitlement = find_a_field('hrm_leave_rull_manage', 'sum(CL+SL+AL+ML+MLV+PL)', '1'); ?></td>

								<td><?= $availed = $total_leave; ?></td>

								<td><?= $balance = $entitlement - $availed; ?></td>
 ...
 att_date between "' . $_POST['fdate'] . '" and  "' . $_POST['tdate'] . '"');
	 ...
	 att_date between "' . $_POST['fdate'] . '" and  "' . $_POST['tdate'] . '" and att_date!="' . $holy . '"');
					...
					<tr>
						<td colspan="6">
							<div align="right"><b> Total Absent Days</b></div>
						</td>
						<td align="right"><b><?= $grand_tot_leave_days ?></b></td>
					</tr>
				</tbody>
					...
					<td colspan="10">
						<table width="100%" border="1">
							...
							<tr>
								...
								<td rowspan="8">
									<div align="center">Production</div>
								</td>
								<td rowspan="3">
							...
							<tr class="totaldig">
								...
								<td>
									<div align="center"><strong>76</strong></div>
								</td>
								<td>
									<div align="center"><strong>62</strong></div>
								</td>
								<td>
						...
						<table style="margin-top:10px;" width="100%" border="1">
							<tr>
								<td width="16%" rowspan="8">
									<div align="center">POLY</div>
								</td>
								<td width="13%" rowspan="8">
									<div align="center">Production</div>
								</td>
								<td width="11%" rowspan="3">
									<div align="center">Cutting</div>
								</td>
								<td width="11%">
									<div align="center">Officer</div>
								</td>
					...
					<td colspan="10">
						<table style="margin-top:10px;" width="100%" border="1">
							<tr class="totaldig">
								...
								<td width="9%">
									<div align="center"><strong>124</strong></div>
								</td>
								<td width="8%">
			...
			<table width="100%" cellspacing="0" cellpadding="2" border="0">
				<thead>
					...
					<tr>
						...
						<th rowspan="2">
							<div align="center">January<br>
								<?= $_POST['year'] ?>
							</div>
						</th>
						<th rowspan="2">
							<div align="center">February<br>
								<?= $_POST['year'] ?>
							</div>
						</th>
						<th rowspan="2">
							<div align="center">March<br>
								<?= $_POST['year'] ?>
							</div>
						</th>
						<th rowspan="2">
							<div align="center">April<br>
								<?= $_POST['year'] ?>
							</div>
						</th>
						<th rowspan="2">
							<div align="center">May<br>
								<?= $_POST['year'] ?>
							</div>
						</th>
						<th rowspan="2">
							<div align="center">June<br>
								<?= $_POST['year'] ?>
							</div>
						</th>
						<th rowspan="2">
							<div align="center">July<br>
								<?= $_POST['year'] ?>
							</div>
						</th>
						<th rowspan="2">
						...
						<th rowspan="2">
							<div align="center">September<br>
								<?= $_POST['year'] ?>
							</div>
						</th>
						<th rowspan="2">
							<div align="center">October<br>
								<?= $_POST['year'] ?>
							</div>
						</th>
						<th rowspan="2">
							<div align="center">November<br>
								<?= $_POST['year'] ?>
							</div>
						</th>
						<th rowspan="2">
							<div align="center">December<br>
								<?= $_POST['year'] ?>
							</div>
						</th>
						<th rowspan="2">
			...
			<table width="100%" cellspacing="0" cellpadding="2" border="1" id="example">
				<thead>
					<tr>
						...
						<th rowspan="2">
							<div align="center">Salary Given By</div>
						</th>
					</tr>
					<tr>
						<th>
						...
						<th>
							<div align="center">Salary Advanced</div>
						</th>
						<th>
						...
						<tbody>
							<tr style="height:130px;">
								<td align="center" style="text-align:center">
									<p><img src="../../pic/staff/<?= $data->PBI_ID ?>.jpg" alt="" width="98" height="101" border="1" /></p>
								</td>
...
where s.PBI_ID=p.PBI_ID and s.basic_salary=b.basic_salary ' . $pay_con . ' ';
			...
			?>
				<table width="600" border="0" cellpadding="0" cellspacing="0" align="center" style="font-size:16px;" class="tabledesign">
					<tr>
						<td style="border:0px;">&nbsp;
							<button type="button" id="print" onClick="hide();window.print();">Print</button>
						</td>
					</tr>
				...
				?>
					...
					<table width="600" border="1" cellpadding="2" cellspacing="0" align="center" style="font-size:normal 12px  thoma;">
						...
						<!-- <?
								...
								$bonus = find_a_field('salary_bonus', 'bonus_amt', 'cut_off_date between "' . $start . '" and "' . $end . '" and PBI_ID=' . $paySlip->PBI_ID);
					...
					<table width="100%" cellspacing="0" cellpadding="2" border="0">
						<thead>
							...
							<tr>
								...
								<th colspan="2">
									<div align="center">Driver Mobile No</div>
								</th>
					...
					<table width="70%" cellspacing="0" cellpadding="2" border="0" style="margin:auto;">
						<thead>
							...
							<tr>
								...
								<th rowspan="2">
									<div align="center">Name of Employee</div>
								</th>
								<th rowspan="2">
									<div align="center">Accounts Number</div>
								</th>
								<th rowspan="2">
...
where 1 " . $con;
  ...
  from department where DEPT_ID=a.PBI_DEPARTMENT) as department,(select PROJECT_DESC from project where PROJECT_ID=a.JOB_LOCATION) as project_name,d.bizid,d.xdate,d.late_in,d.ztime,d.xtime
   ...
   where 1 " . $con . " group by a.PBI_ID,a.DEPT_ID order by a.DEPT_ID asc";
					...
					?>
						<tr>
							<td style="border:0px; font-size:16px; font-weight:bold;" colspan="10">
								<div align="left">
									<?= $dept_data->DEPT_DESC ?>
								</div>
							</td>
   ...
   where p.DEPT_ID='" . $dept_data->DEPT_ID . "'  group by p.PBI_ID";
			...
			?>
				<table style="width: 100%; margin: 0 auto; font-size: 20px;text-align:center;" border="1" bordercolor="#FFFFFF">
					<tr>
						<td style="border:0px solid white;"><strong><?= find_a_field('user_group', 'group_name', 'id=' . $_SESSION['user']['group']) ?></strong></td>
					</tr>
					...
					<tr>
						<td style="border:0px solid white;">Period : <?= date('M-Y', strtotime($fdate)); ?> - <?= date('M-Y', strtotime($tdate)); ?> </td>
					</tr>
				</table>
				<table id="ExportTable" style="width:100%;margin:0 auto;" cellpadding="0" cellspacing="0" border="1">
					...
					<tr>
						...
						<td><strong>Department</strong></td>
						<? for ($i = $fdate; $i <= $tdate; $i = date('Y-m-d', strtotime($i . " +1 days"))) { ?>
							<td style="width: 2.5%"><strong><?= date('F j D', strtotime($i)); ?></strong></td>
						<? } ?>
					</tr>
...
between "' . $fdate . '" and "' . $tdate . '" and emp_id="' . $r->PBI_ID . '"');
			...
			<?
				...
				if ($_POST['job_status'] != '') {
					$job_status_con .= " and p.PBI_JOB_STATUS='" . $_POST['job_status'] . "' ";
				}
				if ($_POST['PBI_IDD'] > 0) $IDConn = " and s.emp_id='" . $_POST['PBI_IDD'] . "'";
   ...
   s.att_date='" . $_POST['fdate'] . "' " 
   .$job_con.$IDConn.$codeConn.$NameConn.$org.$cost_center.$dept.$section.$job_loc.$gender.$grade.$work.$level.$hrm_class.$job_status_con.$incharge. 
   ...
   $basic_sql = "select s.*,p.PBI_CODE,p.PBI_NAME,p.DEPT_ID,p.section,p.PBI_DESIGNATION,p.EMPLOYMENT_TYPE,
  p.PBI_DOJ
				...
				<table style="width: 80%; margin: 0 auto;text-align:center;" cellpadding="0" cellspacing="0" class="oe_list_content">
					<thead>
						<tr class="oe_list_header_columns" style=" text-align:center; font-size:12px;" align="center">
							<th>S/L</th>
							<th style="">
								<div align="center">Attendance Date</div>
							</th>
							<th>
								<div align="center">Employee ID</div>
							</th>
							...
							<th>
								<div align="center">Total Working Time</div>
							</th>
							<th>
								<div align="center">Late in days</div>
							</th>
							<th>
								<div align="center">OD</div>
							</th>
							<th>
								<div align="center">SHL</div>
							</th>
							<th>
