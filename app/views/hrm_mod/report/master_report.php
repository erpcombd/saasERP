...
where a.PBI_ID=b.PBI_ID and  a.PBI_DOJ <= '" .$joining_date_check. "' and b.pay>0  
...
order by b.PBI_ID";
            ...
            ?>
              <tr>
                ...
                <td <?php 
				//if ($data->Total_Late_Min > 0) echo 'style="background-color: #f90a0a;"'; ?>><? //if ($data->Total_Late_Min > 0) echo $data->Total_Late_Min; ?></td>
        ...
        <table width="100%" cellspacing="0" cellpadding="2" border="1" id="ExportTable">
          <thead>
            ...
            <tr>
              ...
              <th>
                <div align="center">Total od Days </div>
              </th>
...
// Dynamic joining date check
$days = cal_days_in_month(CAL_GREGORIAN, $month, $year);
$joining_date_check = $year . '-' . $month . '-' . $days;

// Refactored SQL Query

 $sqll = "SELECT 
    a.PBI_CODE AS Emp_ID,
    a.PBI_NAME AS Name,
    (SELECT DESG_DESC FROM designation WHERE DESG_ID = a.DESG_ID) AS designation,
    (SELECT sec_name FROM PBI_Section WHERE sec_id = a.section) AS Section,
    ...
    a.line AS Line, 
    a.PBI_DOJ AS DOJ, 
    COUNT(b.s_date) AS oddate

FROM 
    personnel_basic_info a
    
LEFT JOIN 
    hrm_od_info b ON a.PBI_ID = b.PBI_ID 

WHERE 
    a.PBI_DOJ <= '$joining_date_check'  
    AND b.s_date BETWEEN '$start_date' AND '$end_date'"
...
LEFT JOIN 
    hrm_iom_info b ON a.PBI_ID = b.PBI_ID 

WHERE 
    a.PBI_DOJ <= '$joining_date_check'   and iom_status='GRANTED'
    AND b.s_date BETWEEN '$start_date' AND '$end_date'"
...
?>
    <tr>
      ...
      <?
       ...
       if ($_POST['report'] == 2000) {
        $report = "IOM Detail Report";
      ?>
        <style>
		@media print {
		@page {
          ...
          <thead>
            ...
            <tr>
              ...
              <th>
			    ...
			    <th>
                <div align="center">Entry Date </div>
              </th>
              <th>
			  ...
			  <th>
                <div align="center">Punch Time </div>
              </th>
			   <th>
                <div align="center">Late min </div>
              </th>
...
$sqll = "SELECT 
    ...
    (
        SELECT MIN(c.xtime)
        FROM hrm_attdump c
        WHERE c.EMP_CODE = a.PBI_ID 
        AND c.xdate = b.s_date
    ) AS earliest_xtime,
    od.od_s_date,
    od.project_id
...
LEFT JOIN 
    (
        SELECT PBI_ID, s_date AS od_s_date, project_id
        FROM hrm_od_info
        GROUP BY PBI_ID, s_date  -- one OD record per person per day
    ) od ON a.PBI_ID = od.PBI_ID AND od.od_s_date = b.s_date
...
?>
    <tr>
        ...
        <td><?= $data->department; ?></td>
         ...
         <td><?= $data->s_date; ?></td>
          <td><?= $data->e_date; ?></td>
		 <td><?= date('h:i A', strtotime($data->earliest_xtime)); ?></td>
		 <td> <?php
...
order by b.PBI_ID";
 ...
 "'.$m_s_date.'" and "'.$m_e_date.'" and ot_time_to_decimal >=4  and emp_id='.$data->PBI_ID);
    ...
    .container {
        ...
        <table width="100%" cellspacing="0" cellpadding="2" border="1" id="ExportTable" >
                  ...
                  <tr>
                    ...
                    <td style="border:0px;" width="70%" align="center">
                         ...
                         <?php if($location ==3){ ?>
                                <h1 style=" font-weight: 600; padding-bottom: 10px; ">Mahir Group </h1>
                          <? } else{?>
                                <h1 style=" font-weight: 400; padding-bottom: 10px; "><?=find_a_field('user_group','group_name','id =' .$company);?></h1>
                          <? } ?>
                          
           <h1 style=" font-weight: 500; ">
		   <?//= $report ?>  <?php if($_POST['PBI_JOB_STATUS']=='Not In Service'){ ?> Dropout <? } ?> Eid-Ul-Fitre Festival Bonus Sheet, <?= date('F-Y', mktime(1, 1, 1, $_POST['mon'], 1, $_POST['year'])) ?> </h1>
          ...
          <tbody class="new-body">
           ...
           $sqld = 'select a.*, a.PBI_ID,a.PBI_CODE, a.PBI_NAME, a.PBI_DOJ, a.grade , b.job_period, b.cut_off_date,
            (select DESG_DESC from designation where DESG_ID=a.DESG_ID) as Designation,
            (select DEPT_DESC from department where DEPT_ID=a.DEPT_ID) as department ,b.bonus_type,i.gross_salary,b.bonus_amt,i.basic_salary
            from salary_info i, personnel_basic_info a,salary_bonus b 
            where  b.bonus_amt>0 and   b.mon=' . $_POST['mon'] . ' and b.year=' . $_POST['year'] . ' and b.PBI_ID=a.PBI_ID and i.PBI_ID=a.PBI_ID
            ' . $salaryConn .$codeConn.$idConn.$NameConn.$desgConn.$JOB_LOC_ID_BLOCK.$depConn.$work_station.$OrgConn.$job_statusConn.$secConn.$JoblocConn.$CostConn.$classConn.' 
  ...
  $date1 = strtotime($data->PBI_DOJ); 
  $date2 = strtotime($data->cut_off_date); 
  ...
  $days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
        ...
        <td rowspan="2" align="right"><?=number_format($data->gross_salary,2);        $totGross += $data->gross_salary ?></td>
        <td rowspan="2" align="right"><?=number_format($data->basic_salary,2);        $totBasic += $data->basic_salary ?></td>
       
          <td ><?=$years." Years ".$months." Months ".$days." Days"; //$data->PBI_SERVICE_LENGTH ?></td>
          <td nowrap="nowrap" align="right"><?= number_format($data->bonus_amt,2);$tot_bonus+=$data->bonus_amt; ?></td>
          <td nowrap="nowrap"> <?=$duration?></td>
            ...
            <tr>
              ...
              <td align="right"> </td>
              <td align="right"> </td>
              <td align="right"><strong><?=number_format($totGross,2); ?></strong></td>
              <td align="right"><strong><?=number_format($totBasic,2); ?></strong></td>
              <td align="right"><strong></strong></td>
             <td align="right"><strong><?=number_format($tot_bonus,2); ?></strong></td>
             <td align="right"><strong></strong></td>
            </tr>
          ...
          <tr >
              <td colspan="28" style=" border: none;   ">
                ...
                <div id="main_sarwar" " style=" display: flex;" >
...
$signature_a = '';
    ...
    @media print {
		   ...
		   <?//= $report ?>  <?php if($_POST['PBI_JOB_STATUS']=='Not In Service'){ ?> Dropout <? } ?> Eid-Ul-Fitre Festival Bonus Sheet, <?= date('F-Y', mktime(1, 1, 1, $_POST['mon'], 1, $_POST['year'])) ?> </h1>
            ...
            <tr class="row">
              ...
              <th rowspan="2">
               ...
               <th rowspan="5" style=" width: 201px; ">
                <div align="center">Signature </div>
              </th>
         </tr>
        ...
        if($_POST['PBI_CODE']!="") $codeConn = " and a.PBI_CODE='".$_POST['PBI_CODE']."'";
         ...
         $sqld = 'select a.*, a.PBI_ID,a.PBI_CODE, a.PBI_NAME, a.PBI_DOJ, a.grade , a.PBI_SERVICE_LENGTH,
            (select DESG_DESC from designation where DESG_ID=a.DESG_ID) as Designation,
            (select DEPT_DESC from department where DEPT_ID=a.DEPT_ID) as department ,b.bonus_type,i.gross_salary,
            b.bonus_amt,i.basic_salary,b.cut_off_date
            from salary_info i, personnel_basic_info a,salary_bonus b 
            where  b.bonus_amt>0 and   b.mon=' . $_POST['mon'] . ' and b.year=' . $_POST['year'] . ' and b.PBI_ID=a.PBI_ID and i.PBI_ID=a.PBI_ID
            ' . $salaryConn .$codeConn.$idConn.$NameConn.$desgConn.$JOB_LOC_ID_BLOCK.$depConn.$work_station.$OrgConn.$job_statusConn.$secConn.$JoblocConn.$CostConn.$classConn.' 
            ...
            while ($data = mysqli_fetch_object($queryd)) {
              ...
              $grade_name = find_a_field('hrm_grade','grade_name','id="'.$data->grade.'"');
                ...
                $dateJoin = new DateTime($data->PBI_DOJ);
                $dateCutofDate = new DateTime($data->cut_off_date);
                
                $interval = $dateJoin->diff($dateCutofDate);
        ...
        <td rowspan="2" align="right"><?=number_format($data->basic_salary,2);        $totBasic += $data->basic_salary ?></td>
          ...
          <td  align="center"><?="$totalYears years, $totalMonths months, and $totalDays days.";?></td>
          <td nowrap="nowrap" align="right"><?= number_format($data->bonus_amt,2);$tot_bonus+=$data->bonus_amt; ?></td>
          <td nowrap="nowrap"></td>
              </tr>
...
$signature_a = '';
      ...
      <table width="100%" cellspacing="0" cellpadding="2" border="0">
		   ...
		   <?//= $report ?>Eid-Ul-Fitre Bonus Summary For <?= date('F-Y', mktime(1, 1, 1, $_POST['mon'], 1, $_POST['year']))." (".$job_status.")"; ?></h1>
              ...
              <table width="100%" border="2">
                ...
                <tr>
                  ...
                  <th><div align="center"><strong>Cost Center</strong></div></th>
                  <th><div align="center"><strong>Total Employee </strong></div></th>
                  <th><div align="center"><strong>Gross </strong></div></th>
                  <th><div align="center"><strong>Basic </strong></div></th>
                  <th scope="col"><div align="center"><strong>Bonus Amount </strong></div></th>
                ...
                <?
                            ...
                            if($_POST['PBI_NAME']!="") $NameConn = " and a.PBI_NAME='".$_POST['PBI_NAME']."'";
                            if($_POST['DESG_ID']>0) $desgConn = " and a.DESG_ID='".$_POST['DESG_ID']."'";
                            if($_POST['DEPT_ID']>0) $depConn = " and a.department='".$_POST['DEPT_ID']."'";
                            if($_POST['PBI_SEX']!="") $genderConn = " and a.PBI_SEX='".$_POST['PBI_SEX']."'";
                            if($_POST['grade']>0) $gradeConn = " and a.grade='".$_POST['grade']."'";  
                            if($_POST['work_station']>0) $work_station = " and a.PBI_WORK_STATION='".$_POST['work_station']."'";  
                ...
                $m_e_date = $_POST['year'] . '-' . $_POST['mon'] . '-' . '31';
                 ...
                 $sqld = 'select  h.center_name , a.cost_center , sum(s.gross_salary) as gross_salary, 
                 sum(t.bank_paid) as bank,sum(t.cash_paid) as cash,
                sum(s.basic_salary) as basic_salary,
		
			     sum(t.bonus_amt) as bonus_amt,
				 count(t.PBI_ID) as total_employee
				
				 

                from salary_bonus t, personnel_basic_info a, hrm_cost_center h,salary_info s

                where t.mon=' . $_POST['mon'] . ' and t.year=' . $_POST['year'] . '

                and t.PBI_ID=a.PBI_ID and a.cost_center=h.id and s.PBI_ID=a.PBI_ID and t.bonus_amt>0
				
				' . $desgConn.$depConn.$work_station.$OrgConn.$job_statusConn.$secConn.$JoblocConn.$CostConn.$classConn.$JOB_LOC_ID_BLOCK.' 

                group by a.cost_center order by (h.center_name) asc';
                ...
                ?>
                  ...
                  <tr>
                    <td width="1%"  ><?=++$s?> </td>
                    <td width="13%"><?= $data->center_name;?> </td>
                    <td width="6%"><div align="right"><?=$data->total_employee; $tot_emp +=$data->total_employee;?></div></td>
                    <td width="5%"><div align="right"><?=number_format($data->gross_salary); $tot_gross +=$data->gross_salary;?></div></td>
				    <td width="5%"><div align="right"><?=$data->basic_salary; $tot_basic +=$data->basic_salary; ?></div></td>
                    <td width="8%"><div align="right"><?=number_format($data->bonus_amt); $tot_bonus +=$data->bonus_amt; ?></div></td>
                  </tr>
                <? 
                ...
                <tr class="all_table">
                  ...
                  <td width="5%" class="all_table"><strong>Grand Total </strong></td>
                  <td width="13%" class="all_table"><strong></strong></td>
                  <td width="5%" class="all_table"><div align="center"><strong><?=$tot_emp;?></strong></div></td>
                  <td width="10%" class="all_table"><div align="center"><strong><?=number_format($tot_gross);?></strong></div></td>
				  
				 <td width="5%" class="all_table"><div align="center"><strong><?=number_format($tot_basic);?></strong></div></td>
                  <td width="5%" class="all_table"><div align="center"><strong><?=number_format($tot_bonus);?></strong></div></td>
              ...
              <td width="10%" class="all_table"><div align="left"><strong>Total Cash Amount:<?=number_format($tot_cash);?></strong></div></td>
				<br>
				 <td width="10%" class="all_table"><div align="left"><strong>Total Bank Amount:<?=number_format($tot_bank);?></strong></div></td>   
				</tr>
        ...
        <table width="100%" class="all_table">
                ...
                <tbody>
                  <tr>
					...
					echo "All";
                      ...
                      <h1>  <strong> Eid-Ul-Fitre Bonus Sheet For The Month of : <?= date('F-Y', mktime(1, 1, 1, $_POST['mon'], 1, $_POST['year'])) ?></strong>
          ...
          <tr>
            <td class="all_table">
				  ...
				  <th width="3%" rowspan="2" scope="col">
                    <div align="center"><strong>Basic</strong>
					</div>
                  </th>
                  ...
                  <th width="5%" rowspan="2" scope="col">
                    <div align="center"><strong> Bonus Amount </strong></div>
                  </th>
                ...
                <tr>
				 ...
				 $dept_sql = "select  p.cost_center, dept.center_name , sum(s.gross_salary) as gross_salary,   sum(s.basic_salary) as basic_salary,sum(t.bonus_amt) as bonus_amt,
				...
				from personnel_basic_info p, hrm_cost_center dept ,salary_bonus t, salary_info s

				where p.cost_center=dept.id  
				
				and  t.mon=".$_POST['mon']." and t.year=".$_POST['year']." 

                and t.PBI_ID=p.PBI_ID and s.PBI_ID=p.PBI_ID  and t.bonus_amt>0
              ...
              $sqld = 'select h.*, h.class_name,p.class, sum(s.gross_salary) as gross_salary,   sum(s.basic_salary) as basic_salary, sum(t.bonus_amt) as bonus_amt,
				
		
			
				count(t.PBI_ID) as total_employee

                from salary_bonus t, personnel_basic_info p, hrm_class h,  salary_info s

                where  t.mon=' . $_POST['mon'] . ' and t.year=' . $_POST['year'] . ' and 
				
				p.cost_center="'.$dept_data->cost_center.'"

                and t.PBI_ID=p.PBI_ID  and p.class=h.id  and s.PBI_ID=p.PBI_ID and t.bonus_amt>0
                ...
                ?>
					...
					<td width="5%"><div align="right"><?=number_format($data->bonus_amt); $tot_bonus +=$data->bonus_amt; ?></div></td>
				...
				<?  } ?>
                 ...
                 <tr style="border:none; background-color: #99fffa;height: 29px; ">
                  ...
                  <td><div align="right"><strong><?=number_format($dept_data->bonus_amt);?></strong></div></td>
                </tr>
...
<?  } ?>							  
                  ...
                  <td><div align="right"><strong style="font-size: 15px;"> <?=number_format($tot_bonus);?></strong></div></td>
...
from personnel_basic_info a , salary_info s
where	1 " .$idConn.$codeConn.$NameConn.$desgConn.$genderConn.$genderConn.$work_station.$ReligionConn.$OrgConn.$job_statusConn.$secConn.$JoblocConn.$CostConn.$classConn.$lineConn.$inchargeConn.$DOJConn.$shiftConn. " and s.PBI_ID=a.PBI_ID order by a.PBI_ID asc";
