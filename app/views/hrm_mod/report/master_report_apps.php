...
if($_REQUEST['report']==20241031) {
$report="OD Details Report";
?>
...
<table width="100%">
	...
	<?php 
		...
		//echo $sql_data1 = 'SELECT * FROM hrm_od_info WHERE  1 '.$date_con.' ';
			$sql_data1 = 'SELECT o.id as od_id, o.*, u.PBI_NAME,u.PBI_CODE ,p.name  as project_name FROM hrm_od_info o, personnel_basic_info u, crm_project_org p WHERE '.$date_con.$emp_con.' AND o.project_id=p.id AND o.PBI_ID=u.PBI_ID order by PBI_ID ASC ';
			$query = db_query($sql_data1);
		$sl=1;
		while($info=mysqli_fetch_object($query)){
	?>
