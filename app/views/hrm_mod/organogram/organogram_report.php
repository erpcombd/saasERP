...
<?php
  ...
  echo 'Department : '.find_a_field('department','DEPT_DESC','DEPT_ID='.$dept).'';
    ...
    $sql = 'select p.* from personnel_basic_info p  where  1 and p.PBI_JOB_STATUS="In Service" and p.DEPT_ID="'.$dept.'" and p.PBI_ID="'.$dept_head.'"  order by PBI_ID asc limit 0, 1';	
...
function demo_recursive($parent_id, $under_id){
	...
	if($parent_id!='' && $under_id == 0){
	 $sql2 = "select p.* from personnel_basic_info p where  1 and p.PBI_JOB_STATUS='In Service' and  p.incharge_id = '".$parent_id."' ";						
...
echo '<ul>';
	 while($row = mysqli_fetch_object($result2)){
		...
		$under_id =  $row->incharge_id;
		echo '<li>';
		echo '<div align="center">';
		echo '<img src="../../../assets/support/upload_view.php?name='.$row->PBI_PICTURE_ATT_PATH.'&folder=hrm_emp_pic&proj_id='.$_SESSION["proj_id"].'&mod=hrm_mod" style="width:70px; height:65px; border-radius:25%; margin-top:2px;"> <br>';
...
<li class="company"><? 
 echo '<img src="../../../assets/support/upload_view.php?name='.$data->PBI_PICTURE_ATT_PATH.'&folder=hrm_emp_pic&proj_id='.$_SESSION["proj_id"].'&mod=hrm_mod" style="width:70px; height:65px; border-radius:25%; margin-top:2px;"> <br>';
