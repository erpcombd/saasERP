 ...
 <p class="new-header"><strong> Please fill the information to continue </strong>  </p> 
      <div class="card-body">
	    ...
	    <input type="hidden" id="designation" name="designation" value="<?=find_a_field('personnel_basic_info','DESG_ID','PBI_ID='.$_SESSION['employee_selected']);?>">
                ...
                <input type="text" id="grossSalary" name="gross_salary" oninput="calculateSalary();calculateSalaryTax();" value="<?=$gross_salary?>">
                </div></div>
...
<div class="card-body n-form2">         
        ...
        <p class="new-header"> Bank Iinformation Section : </p>
		  ...
		  <div class="form-group row m-0 pb-1">
                <label for="basic_salary" class="col-sm-4 col-md-4 col-lg-4 col-xl-4 m-0 p-0 d-flex justify-content-start align-items-center pr-1 
				bg-form-titel-text"> Monthly Income Tax : </label>
                <div class="col-sm-8 col-md-8 col-lg-8 col-xl-8 p-0 pr-2 ">
              <input name="income_tax" type="text" id="monthly_tax"  value="<?= $income_tax; ?>" class="form-control console" />
...
function calculateSalaryTax() {
    ...
    // Ensure gross salary, basic salary, and gender are available
    if (!isNaN(grossSalary) && !isNaN(basicSalary) && gender) {
        // Call server to fetch tax info
        fetch(`get_taxinfo_ajx.php?gross_salary=${grossSalary}&basic_salary=${basicSalary}&gender=${gender}`)
            ...
            .then(data => {
                ...
                if (data.success && !isNaN(data.tax)) {
                    document.getElementById('monthly_tax').value = parseFloat(data.tax).toFixed(2);  // Set the calculated tax
                } else {
                    document.getElementById('monthly_tax').value = data.message || 'Error calculating tax';
                }
            })
            .catch(error => {
                console.error('Error:', error);
                document.getElementById('monthly_tax').value = 'Error calculating tax';  // Handle errors from the fetch call
            });
    } else {
        // Clear tax field if inputs are incomplete or invalid
