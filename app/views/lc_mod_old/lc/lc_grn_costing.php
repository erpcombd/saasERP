...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
		...
		$closing_no = next_transection_no($group_for,$closing_date,'lc_purchase_closing','closing_no');


		$sql = "SELECT p.*, i.item_name FROM lc_purchase_receive p, item_info i WHERE p.item_id=i.item_id and p.payment_id=".$payment_id." order  by  p.id ";
   ...
   $ins_invoice = 'INSERT INTO lc_purchase_closing (closing_no, closing_date, payment_id, payment_no, po_no, order_no, group_for, lc_no, lc_number, ledger_id, vendor_id, item_id, unit_name, rec_qty, rec_rate_usd, rec_amt_usd, cost_price_bdt, cost_amt_bdt, exchange_rate, total_pay_amt, entry_by, entry_at,sub_ledger_id)
  
  VALUES("'.$closing_no.'", "'.$closing_date.'", "'.$payment_id.'", "'.$payment_no.'",  "'.$data->po_no.'", "'.$data->id.'", "'.$group_for.'", "'.$lc_no.'", "'.$lc_number.'", "'.$ledger_id.'", "'.$data->vendor_id.'", "'.$data->item_id.'", "'.$data->unit_name.'", "'.$rec_qty.'", "'.$rec_rate_usd.'", "'.$rec_amt_usd.'", "'.$cost_price.'", "'.$cost_amt.'", "'.$exchange_rate.'", "'.$total_pay_amt.'", "'.$entry_by.'", "'.$entry_at.'", "'.$lc_data->sub_ledger_id.'")';
...
journal_item_control($data->item_id, $_SESSION['user']['depot'], $closing_date,  $rec_qty, 0, 'LC Purchase', $xid, $cost_price, $_SESSION['user']['depot'], $closing_no);
		...
		$up_sql3 = "update lc_bank_payment set status='COMPLETED' where id='".$payment_id."'";
		db_query($up_sql3);
...
<div class="box" style="width:100%;">
			...
			<input style="width:220px; height:32px;"  name="invoice_date" type="text" id="invoice_date"  value="<?=($invoice_date!='')?$invoice_date:date('Y-m-d')?>"   required />									</td>
									...
									$lc_data = find_all_field('lc_number_setup','','id='.$bnk_pay_data->lc_no); 
									$bnk_data = find_all_field('lc_bank_entry','','lc_no='.$bnk_pay_data->lc_no); 
									$po_data = find_all_field('lc_purchase_master','','lc_no='.$bnk_pay_data->lc_no);
									?>
...
<tr>
	...
	WHERE a.id=b.bill_type and b.id=c.bill_category and c.lc_no='".$bnk_pay_data->lc_no."' and b.id not in (7,29,30) and c.bank_pay_id=0 group by b.bill_type order by c.bill_type ";
  ...
  $sql = "select m.lc_no,  r.payment_id, sum(r.amount_usd) as tot_rec_amt_usd  from lc_purchase_master m, lc_purchase_receive r where m.po_no=r.po_no and r.payment_id=".$payment_id."   group by m.lc_no ";
$query = db_query($sql);
while($data=mysqli_fetch_object($query)){
...
<? //if ($due_amt=$data->invoice_amt-$payment_amt[$data->ledger_id][$data->tr_no]+$return_amt[$data->ledger_id][$data->tr_no]>0) {?>
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>" style="border:1px solid #000000; ">
   ...
   &nbsp;&nbsp;  <?=$data->unit_name;?>    </td>
    ...
    <td>
 <input name="cost_price_<?=$data->id?>" id="cost_price_<?=$data->id?>" type="text" size="10"  value="<?=$cost_price=($data->rate_usd*$exchange_rate);?>" style="width:100px; height:25px;"  />	</td>
