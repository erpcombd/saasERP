	...
	$data=mysqli_fetch_object($query);
		...
		//$tr_from = 'LC Journal';
			...
			if($data->bill_type==1){
				...
				if($bank_cash_group==10311 || $bank_cash_group==10312){
					$tr_from = 'Payment';
				}else{
					$tr_from = 'Journal';
				}	
...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
  ...
  VALUES("'.$payment_no.'", "'.$payment_date.'", "'.$payment_method.'", "'.$lc_no.'", "'.$lc_number.'", "'.$ledger_id.'", "'.$lc_part.'", "'.$lc_part_view.'", "'.$lc_no_part.'", "'.$group_for.'",  "'.$bill_type.'", "'.$bill_category.'", "'.$data->bill_category.'", "'.$cr_ledger_id.'", "'.$payment_amt.'", "0", "'.$remarks.'", "'.$note.'", "'.$r_no.'", "'.$r_date.'", "CHECKED", "'.$entry_by.'", "'.$entry_at.'", "'.$boe_no.'", "'.$boe_date.'")';
...
<div class="box" style="width:100%;">
	...
	<select name="payment_method" id="payment_method" required style="width:220px;" >
									...
									<?php //echo 'select * from accounts_ledger where ledger_group_id in (125001)';?>
									<select name="cr_ledger_id" id="cr_ledger_id" required="required" style="width:220px;">
									  <option></option>
									  <? foreign_relation('accounts_ledger','ledger_id','ledger_name',$cr_ledger_id,'ledger_group_id in (125001)');?>
									</select>
									</span>
									</td>
									<td align="right">REMARKS:</td>
						...
						<? }  elseif ($_POST['bill_type']==3) {?>
								  ...
								  <tr>
									...
									<select name="cr_ledger_id" id="cr_ledger_id" style="width:220px;">
									  <option></option>
									  <? foreign_relation('accounts_ledger','ledger_id','ledger_name',$_POST['cr_ledger_id'],'ledger_group_id in (125001)');?>
									</select>
									</span>
									</td>
