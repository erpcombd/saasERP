...
while($data=mysqli_fetch_object($query)){
	...
	$pre_costing_rate[$data->bill_category][$data->order_no]= $data->rate;
}
...
<table width="100%" border="0" align="center" id="grp" cellpadding="0" cellspacing="0" style="font-size:12px; text-transform:uppercase;">
 ...
 // $sql = "SELECT id, bill_type, bill_category FROM lc_bill_category WHERE 1 ".$bill_type_con." order  by bill_type,id ";
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
	...
	<? for($k=1;$k<=$c;$k++){ ?>
	<td><input name="pre_cost_<?=$cat_id[$k]."_".$data->order_no;?>" id="pre_cost_<?=$cat_id[$k]."_".$data->order_no;?>" type="text" size="10"  value="<?=$pre_costing_rate[$cat_id[$k]][$data->order_no]?>" style="width:80px;" readonly /></td>
