...
//   		$chalan_no=''.$yer.''.$month.''.$cy_id;
   ...
   $ins_invoice = 'INSERT INTO lc_bill_payment (payment_no, payment_date, payment_method, lc_no, lc_number, lc_ledger, bank_pay_id, lc_part_no, group_for, bill_type, bill_category, category_view, cr_ledger_id, pay_amt_in, pay_amt_out, remarks, note, r_no, r_date, status, entry_by, entry_at, boe_no, boe_date)
  
  VALUES("'.$payment_no.'", "'.$payment_date.'", "'.$payment_method.'", "'.$lc_no.'", "'.$lc_number.'", "'.$ledger_id.'", "'.$bank_pay_id.'", "'.$lc_part_no.'", "'.$group_for.'",  "'.$bill_type.'", "'.$bill_category.'", "'.$data->category_view.'", "'.$cr_ledger_id.'",  "0", "'.$payment_amt.'", "'.$remarks.'", "'.$note.'", "'.$r_no.'", "'.$r_date.'", "CHECKED", "'.$entry_by.'", "'.$entry_at.'", "'.$boe_no.'", "'.$boe_date.'")';
...
if($payment_no>0)
{
auto_insert_lc_payment_adjustment_secoundary($payment_no);
}
				...
				<td><input name="company" type="text" id="company"  readonly="" style="width:220px; height:32px;" value="<?=find_a_field('user_group','group_name','id='.$lc_data->group_for); ?>"   tabindex="105" />	</td>
								 ...
								 <td>&nbsp;</td>
									...
									<select name="cr_ledger_id" id="cr_ledger_id" required="required" style="width:220px;">
									  <option></option>
									  <? foreign_relation('accounts_ledger','ledger_id','ledger_name',$_POST['cr_ledger_id'],'1');?>
									</select>
...
<? //if ($due_amt=$data->invoice_amt-$payment_amt[$data->ledger_id][$data->tr_no]+$return_amt[$data->ledger_id][$data->tr_no]>0) {?>
  ...
  <tr bgcolor="<?=($i%2)?'#E8F3FF':'#fff';?>">
    ...
    <td>
     <?=find_a_field('lc_bill_type','bill_type','id='.$data->bill_type);?>
    </td>
 ...
 <input name="payment_amt_<?=$data->id?>" id="payment_amt_<?=$data->id?>" type="text" size="10"  value="" style="width:120px; height:25px;"  />	</td>
