...
if(isset($_POST['submit'])&&isset($_POST['report'])&&$_POST['report']>0)
{
	...
	if($_POST['item_type']>0) 					$item_type=$_POST['item_type'];
...
<input name="print" type="hidden" onclick="hide();window.print();" value="Print" />
</div>
<div class="main">
<?
...
if(isset($t_date)) 
		...
		<th width="3%" rowspan="2">S/L Up</th>
		...
		<th colspan="4" bgcolor="#339999" style="text-align:center">Item In </th>
		...
		<tr style="font-size:14px; height:22px;">
		  ...
		  <th width="7%" bgcolor="#339999" style="text-align:center">Production Receive</th>
		...
		$sql = "select j.item_id, sum(j.item_in) as semi_production_qty from journal_item j where  j.ji_date between '".$f_date."' and '".$t_date."' and j.tr_from='Receive' 
		".$warehouse_con." group by j.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $semi_production_qty[$info->item_id]=$info->semi_production_qty;
...
//		}
		...
		<? }?>		</td>
		<td>
			<? if ($semi_production_qty[$row->item_id]>0) {?>
		<a href="stock_master_report.php?report=200602022205&f_date=<?=$_POST['f_date']?>&t_date=<?=$_POST['t_date']?>&item_id=<?=$row->item_id?>&group_id=<?=$_POST['group_id']?>&sub_group_id=<?=$_POST['sub_group_id']?>&warehouse_id=<?=$_POST['warehouse_id']?>" target="_blank">
		<?=number_format($semi_production_qty[$row->item_id],2); $total_semi_production_qty +=$semi_production_qty[$row->item_id];?>
		</a>
		<? }?>	
		</td>
		<td>
		...
		<? }?>		</td>
		<td><?= number_format($total_item_in = ($purchase_qty[$row->item_id]+$semi_production_qty[$row->item_id]+$transfer_rec_qty[$row->item_id]),2); $total_total_item_in +=$total_item_in;?></td>
		<td>
...
<? } }?>
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
		  ...
		  <td><span class="style7">
		    <?=number_format($total_semi_production_qty,2);?>
		  </span></td>
		  <td><span class="style7">
...
elseif($_REQUEST['report']==200602022205)
...
$report="Warehouse Stock IN Semi-Finished Goods Receive";
...
item_info i,  item_sub_group s, item_group g where  s.sub_group_id=i.sub_group_id and s.group_id=g.group_id and a.item_in!="" and a.tr_from="Receive" and
...
<?	 if($data->tr_type =="Receive") { ?>


<td><a href="../../../production_mod/pages/rm_consumption/production_issue_report.php?v_no= <?=$data->sr_no?>" target="_blank">
...
//$warehouse_con =  ' and a.warehouse_id="'.$_SESSION['user']['depot'].'"';
  ...
  $res='select a.ji_date,i.item_id,i.finish_goods_code as fg_code, a.tr_no, a.sr_no, a.po_no, a.group_for, i.item_name, s.sub_group_id,s.sub_group_name as Category,i.unit_name as unit,a.item_ex as `OUT`, a.item_price as rate,(a.item_in*a.item_price) as amount,a.tr_from as tr_type,(select warehouse_name from warehouse where warehouse_id=a.warehouse_id) as warehouse_name, (select warehouse_name from warehouse where warehouse_id=a.relevant_warehouse) as relevant_warehouse,a.sr_no,a.entry_at
...
<td>
<? if($data->group_for==1) { ?>
<a href="../rmc_ffw/rmc_view_ffw_extrusion.php?log_no= <?=$data->sr_no?>" target="_blank"><?=$data->sr_no?></a>
<? }?>

<? if($data->group_for==3 || $data->group_for==4) { ?>

<? if($data->sub_group_id==1700010000) { ?>
<a href="../../../production_mod/pages/rm_consumption/production_issue_report.php?v_no= <?=$data->sr_no?>" target="_blank"><?=$data->sr_no?></a>
<?  }?>

<? if($data->sub_group_id==1700040000) { ?>
<a href="../production_receive_net/production_consumption_report.php?v_no= <?=$data->sr_no?>" target="_blank"><?=$data->sr_no?></a>
<?  }?>
...
echo $data->warehouse_name;
 ...
 $machine_name = find_a_field('machine_info','machine_short_name','machine_id='.$machine_id);
?></td>
</tr>
...
elseif($_POST['report']==210709001)
{
		$report="Finished Goods Present Stock";		
		if(isset($warehouse_id)){$warehouse_con.=' and j.warehouse_id="'.$warehouse_id.'"';}
		...
		echo '<h1>'.find_a_field('warehouse','warehouse_name','warehouse_id='.$group_for).'</h1>';
...
if(isset($t_date)) 
		...
		<tr style="font-size:14px; height:22px; text-transform: uppercase;">
		<th width="3%">S/L</th>
		<th width="11%">Sub Group </th>
		<th width="21%">Item Name </th>
		<th width="3%">Unit  </th>
		<th width="6%" bgcolor="#99CC99" style="text-align:center">OPENING Stock </th>
		<th width="7%" bgcolor="#339999" style="text-align:center">Stock In </th>
		<th width="6%" bgcolor="#99FFFF" style="text-align:center">Stock Out </th>
		<th width="12%" bgcolor="#FF3300" style="text-align:center">CLOSING STOCK</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		if(isset($group_for)){$group_for_stk_con.=' and j.group_for="'.$group_for.'"';}
		
		
		
		  $sql = "select j.item_id, sum(item_in-item_ex) as pre_stock from journal_item j where  j.warehouse_id not in (6,7,8,9,10,11,12) and ji_date<'".$f_date."' ".$warehouse_con.$group_for_stk_con."  group by j.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $opening_stock[$info->item_id]=$info->pre_stock;
		...
		$sql = "select j.item_id, sum(j.item_in) as stock_in_qty from journal_item j where j.warehouse_id not in (6,7,8,9,10,11,12) and  j.ji_date between '".$f_date."' and '".$t_date."'  
		".$warehouse_con.$group_for_stk_con." group by j.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $stock_in_qty[$info->item_id]=$info->stock_in_qty;
  		 
		}
		
	
		
		 $sql = "select j.item_id, sum(j.item_ex) as stock_out_qty from journal_item j where j.warehouse_id not in (6,7,8,9,10,11,12) and j.ji_date between '".$f_date."' and '".$t_date."' 
		 ".$warehouse_con.$group_for_stk_con." group by j.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $stock_out_qty[$info->item_id]=$info->stock_out_qty;
  		 
		}
		
		

		$sql = "select j.item_id, sum(item_in-item_ex) as closing_stock from journal_item j where j.warehouse_id not in (6,7,8,9,10,11,12) and  ji_date<='".$t_date."' 
		".$warehouse_con.$group_for_stk_con." group by j.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $closing_stock[$info->item_id]=$info->closing_stock;
		...
		if(isset($item_type)){$item_type_con.=' and i.item_type="'.$item_type.'"';}
		  ...
		  $sql="select  i.item_id, i.item_name, i.unit_name, s.sub_group_name from item_info i, item_sub_group s where i.sub_group_id=s.sub_group_id  and
		 i.product_nature='Salable' ".$sub_group_id_con.$group_id_con.$group_for_con.$item_id_con.$item_type_con." group by i.item_id order by i.sub_group_id,  i.item_type, i.item_name";
		...
		if($opening_stock[$row->item_id]<>0 || $stock_in_qty[$row->item_id]<>0  || $stock_out_qty[$row->item_id]<>0 || $closing_stock[$row->item_id]<>0){
		?>
		...
		<td><?=number_format($opening_stock[$row->item_id],2);  $total_opening_stock +=$opening_stock[$row->item_id];?> </td>
		<td>
		<? if ($stock_in_qty[$row->item_id]>0) {?>
		<a href="stock_master_report.php?report=210709001001&f_date=<?=$_POST['f_date']?>&t_date=<?=$_POST['t_date']?>&item_id=<?=$row->item_id?>&group_id=<?=$_POST['group_id']?>&sub_group_id=<?=$_POST['sub_group_id']?>&warehouse_id=<?=$_POST['warehouse_id']?>&group_for=<?=$_POST['group_for']?>" target="_blank">
		<?=number_format($stock_in_qty[$row->item_id],2); $total_stock_in_qty +=$stock_in_qty[$row->item_id];?>
		</a>
		<? }?>		</td>
		<td>
		<? if ($stock_out_qty[$row->item_id]>0) {?>
		<a href="stock_master_report.php?report=210709001002&f_date=<?=$_POST['f_date']?>&t_date=<?=$_POST['t_date']?>&item_id=<?=$row->item_id?>&group_id=<?=$_POST['group_id']?>&sub_group_id=<?=$_POST['sub_group_id']?>&warehouse_id=<?=$_POST['warehouse_id']?>&group_for=<?=$_POST['group_for']?>" target="_blank">
		<?=number_format($stock_out_qty[$row->item_id],2); $total_stock_out_qty +=$stock_out_qty[$row->item_id];?>
		</a>
		<? }?>		</td>
		<td><?=number_format($closing_stock[$row->item_id],2); $total_closing_stock +=$closing_stock[$row->item_id];?></td>
		</tr>
<? } }?>
		<tr <?=($xx%2==0)?' bgcolor="#CCCCCC"':'';$xx++;?>>
		  ...
		  <td><span class="style7">
		    <?=number_format($total_stock_in_qty,2);?>
		  </span></td>
		  <td><span class="style7">
	      <?=number_format($total_stock_out_qty,2);?>
		  </span></td>
		  <td><span class="style7">
	      <?=number_format($total_closing_stock,2);?>
		  </span></td>
...
if(isset($t_date)) 
		...
		<th width="6%" rowspan="2" bgcolor="#99CC99" style="text-align:center">OPENING  </th>
		<th colspan="4" bgcolor="#339999" style="text-align:center">Item In </th>
		<th colspan="4" bgcolor="#99FFFF" style="text-align:center">Item Out </th>
		<th width="12%" rowspan="2" bgcolor="#FF3300" style="text-align:center">CLOSING STOCK</th>
		</tr>
		<tr style="font-size:14px; height:22px;">
		  <th width="7%" bgcolor="#339999" style="text-align:center">Production Receive</th>
		  <th width="7%" bgcolor="#339999" style="text-align:center">Transfer Receive</th>
		  <th width="6%" bgcolor="#339999" style="text-align:center">Sales Return</th>
		  <th width="5%" bgcolor="#339999" style="text-align:center">Total</th>
		  <th width="6%" bgcolor="#99FFFF" style="text-align:center">Sales</th>
		  <th width="6%" bgcolor="#99FFFF" style="text-align:center">Product Transfer</th>
		  <th width="6%" bgcolor="#99FFFF" style="text-align:center">Other Issue </th>
		  <th width="7%" bgcolor="#99FFFF" style="text-align:center">Total</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		$sql = "select j.item_id, sum(j.item_in) as production_qty from journal_item j where  j.ji_date between '".$f_date."' and '".$t_date."' and j.tr_from='Receive' 
		".$warehouse_con." group by j.item_id ";
		 $query = db_query($sql);
		 while($info=mysqli_fetch_object($query)){
  		 $production_qty[$info->item_id]=$info->production_qty;
		...
		if($opening_stock[$row->item_id]<>0 || $production_qty[$row->item_id]<>0  || $transfer_rec_qty[$row->item_id]<>0 || $sales_return_qty[$row->item_id]<>0
		...
		<td><?=number_format($opening_stock[$row->item_id],2);  $total_opening_stock +=$opening_stock[$row->item_id];?> </td>
		<td>
		<? if ($production_qty[$row->item_id]>0) {?>
		...
		<?=number_format($production_qty[$row->item_id],2); $total_production_qty +=$production_qty[$row->item_id];?>
		</a>
		<? }?>		</td>
		<td>
		<? if ($transfer_rec_qty[$row->item_id]>0) {?>
		...
		<? }?>		</td>
		<td><?= number_format($total_item_in = ($production_qty[$row->item_id]+$transfer_rec_qty[$row->item_id]+$sales_return_qty[$row->item_id]),2); $total_total_item_in +=$total_item_in;?></td>
		<td>
...
elseif($_REQUEST['report']==210709001001)
...
$report="Finished Goods Stock In Report";
...
<th width="6%" bgcolor="#82D8CF">Barcode</th>
...
<th width="8%" bgcolor="#82D8CF">TR Type</th>
<th width="19%" bgcolor="#82D8CF">Warehouse</th>
</tr>
...
from journal_item a,
item_info i,  item_sub_group s, item_group g where  s.sub_group_id=i.sub_group_id and s.group_id=g.group_id and a.item_in!="" and a.warehouse_id not in (6,7,8,9,10,11,12) and

a.item_id=i.item_id '.$group_con.$date_con.$warehouse_con.$item_con.$status_con.$item_sub_con.$tr_con.$warehouse_id_con.$group_for_con.' order by a.ji_date asc';
...
<?php /*?><?	 if($data->tr_type =="Receive") { ?><? }  ?><?php */?>


<td><?	 if($data->barcode>0) { ?><a href="../../../inventory_automation/pages/production_receive_fn/barcode_print_view.php?v_no=<?=$data->barcode?>" target="_blank">
...
elseif($_REQUEST['report']==210709001002)
...
$report="Finished Goods Stock Out Report";
...
from journal_item a,
item_info i,  item_sub_group s, item_group g where  s.sub_group_id=i.sub_group_id and s.group_id=g.group_id and a.item_ex!="" and a.warehouse_id not in (6,7,8,9,10,11,12) and
...
$report="Warehouse Stock IN Production Receive";
...
<td><a href="../production_receive_net/production_receive_report.php?v_no= <?=$data->sr_no?>" target="_blank">
...
<th width="7%" bgcolor="#82D8CF">TR Type</th>
<th width="14%" bgcolor="#82D8CF">To Warehouse</th>
<th width="15%" bgcolor="#82D8CF">From Warehouse</th>
</tr>
...
<th width="7%" bgcolor="#82D8CF">TR Type</th>
<th width="14%" bgcolor="#82D8CF">Warehouse</th>
<th width="15%" bgcolor="#82D8CF">Dealer Name </th>
</tr>
...
<td><a href="../../../sales_mod/pages/report/invoice_view_acc_new.php?v_no=<?=$data->sr_no?>" target="_blank">
...
<th width="7%" bgcolor="#82D8CF">TR Type</th>
<th width="14%" bgcolor="#82D8CF">From Warehouse</th>
<th width="15%" bgcolor="#82D8CF">To Warehouse</th>
</tr>
