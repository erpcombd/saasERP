...
elseif($_POST['report']==220307005)
{
  ...
  <? if ($item_mother_group>0) {?>
		...
		<tr>
		<th width="2%" rowspan="2" bgcolor="#9999FF">SL</th>
		<th width="27%" rowspan="2" bgcolor="#9999FF">Item Name </th>
		<th colspan="3" bgcolor="#99FFFF" style="text-align:center">SALES  </th>
		<th colspan="3" bgcolor="#33CCCC" style="text-align:center">SALES RETURN</th>
		<th colspan="3" bgcolor="#FF0000" style="text-align:center">NET SALES </th>
		</tr>
		<tr>
		  <th width="7%" bgcolor="#99FFFF" style="text-align:center">BAG</th>
		  <th width="8%" bgcolor="#99FFFF" style="text-align:center">QUANTITY</th>
		  <th width="9%" bgcolor="#99FFFF" style="text-align:center">AMOUNT</th>
		  <th width="7%" bgcolor="#33CCCC" style="text-align:center">BAG</th>
		  <th width="8%" bgcolor="#33CCCC" style="text-align:center">QUANTITY</th>
		  <th width="8%" bgcolor="#33CCCC" style="text-align:center">AMOUNT</th>
		  <th width="6%" bgcolor="#FF0000" style="text-align:center">BAG</th>
		  <th width="9%" bgcolor="#FF0000" style="text-align:center">QUANTITY</th>
		...
		<td><?=$row->item_name?></td>
		<td><?=$bag_unit[$row->item_id]; $total_bag_unit +=$bag_unit[$row->item_id]; ?></td>
		<td> <a href="master_report.php?f_date=<?=$f_date;?>&t_date=<?=$t_date;?>&item_id=<?=$row->item_id;?>&dealer_code=<?=$dealer[0];?>&report=211100707&submit=1" target="_blank" 
		style="color:#000000;" > 
...
elseif($_REQUEST['report']==211100707)
{
        ...
        <tr>
          <td colspan="15" align="center" style="font-size:14px;border:0px; border-color:white; text-transform:uppercase;"><strong> Customer Name: <?=$_REQUEST['ledger_id']?> - <?=find_a_field('dealer_info','dealer_name_e','dealer_code='.$_REQUEST['dealer_code'])?></strong></td>
         </tr>
		 <? }?>
		...
		<th width="8%" bgcolor="#33CCCC">BARCODE</th>
		<th width="10%" bgcolor="#33CCCC">INVOICE</th>
		<th width="10%" bgcolor="#33CCCC"><span style="text-align:center">Chalan Date </span></th>
		<th width="10%" bgcolor="#33CCCC">TYPE</th>
		<th width="21%" bgcolor="#33CCCC">ITEM NAME </th>
		<th width="7%" bgcolor="#33CCCC" style="text-align:center">Unit </th>
		<th width="8%" bgcolor="#33CCCC" style="text-align:center">Unit Price </th>
		<th width="10%" bgcolor="#33CCCC" style="text-align:center">Quantity</th>
		<th width="15%" bgcolor="#33CCCC" style="text-align:center">Amount</th>
		</tr>
		</thead><tbody>
		<? $sl=1;
		...
		<?
 		 ...
 		 $sql="select i.item_name, i.unit_name, m.do_type as so_type,  c.* from sale_do_master m, sale_do_chalan c, item_info i
		 where c.item_id=i.item_id and c.dealer_code='".$_REQUEST['dealer_code']."' and c.item_id='".$_REQUEST['item_id']."' and m.do_no=c.do_no and 
		 c.chalan_date between '".$_REQUEST['f_date']."' and '".$_REQUEST['t_date']."' order by c.chalan_date";
		...
		<td><?=$sl++;?></td>
		<td><? if($row->barcode>0) {?> 
		<a href="../../../inventory_automation_dynamic_menu/pages/production_receive_fn/barcode_sales_print_view.php?v_no=<?=$row->barcode;?>" target="_blank"><?=$row->barcode;?></a> <? }?></td>
		<td><a href="../../../sales_mod/pages/report/invoice_view_acc_new.php?v_no=<?=$row->chalan_no;?>" target="_blank"><?=$row->chalan_no;?></a></td>
		<td><?php echo date('d-m-Y',strtotime($row->chalan_date));?></td>
		<td><?=$row->so_type;?></td>
		<td><?=$row->item_name;?></td>
		<td><?=$row->unit_name;?></td>
		<td><?=number_format($row->unit_price,2);?></td>
		<td><?=number_format($row->total_unit,2); $total_total_unit +=$row->total_unit; ?></td>
...
echo $count_days = round($datediff / (60 * 60 * 24));
		...
		<tr  <? if ($percentage_if<5.1){ echo 'bgcolor="#64E986"';}elseif($percentage_if>15) {echo 'bgcolor="#E0B0FF"';}?> style="font-size:14px;">
		<td><?=$sl++;?></td>
		...
		<td><? 
		$opening_balance=$opening_amt[$row->ledger_id];
		$sub_total_opening_amt +=$opening_amt[$row->ledger_id]; 
		if ($opening_balance>0) { echo  number_format($opening_balance,2). " (DR)"; } else { echo number_format($opening_balance*(-1),2). " (CR)"; }  ?></td>
		...
		<td><? $closing_balance=$closing_amt[$row->ledger_id]; $sub_total_closing_amt +=$closing_balance; 
		if ($closing_balance>0) { echo  number_format($closing_balance,2). " (DR)"; } else { echo number_format($closing_balance*(-1),2). " (CR)"; } 
		 ?></td>
...
?>
      <tr>
        ...
        <td style="text-align:left"><? $gm_100=find_a_field('sale_do_chalan','sum(total_unit)','item_id=1096000900010003 and chalan_date between "'.$from_date.'" and  "'.$to_date.'"'.$depot_conn.' group by item_id');  $tot_gm_100+=$gm_100;  echo number_format($gm_100,2)/</td>
