...
<?
 $link="quotation_create.php?mhafuz=2";
  $btn_name="CREATE New Quotation";
...
while($row=mysqli_fetch_object($query)){
	$item_name_get[$row->item_id]=$row->item_name;
				...
				$con2 .= ' and a.quotation_date between "'.$_POST['fdate'].'" and "'.$_POST['tdate'].'"';
    ...
    $res='select   a.quotation_no,a.quotation_date,a.req_no,a.status,a.entry_by,b.warehouse_name as warehouse, c.fname as entry_by, a.entry_at 
 
 from import_quotation_master a,warehouse b,user_activity_management c
 
 where   a.warehouse_id=b.warehouse_id  and a.status in ("CHECKED")  and a.entry_by=c.user_id '.$con.$con2.'  order by a.quotation_no desc';
                        ...
                        <tr>
                          ...
                          <td><?= $row->req_no?></td>
                            ...
                            <td>
							<button type="button" onclick="custom('<?=url_encode($row->quotation_no);?>')" class="btn2 btn1-bg-submit"><i class="fa-solid fa-eye"></i></button>
