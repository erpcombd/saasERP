            ...
            <div class="cal-footer">
                ...
                <!--<span class="cal-message mt-3 mb-3">
				  ...
				  <?  
				  	...
				  	//___________All ___________
                $sqlRemainder= "SELECT a.*,b.lead_name FROM crm_lead_activity a,crm_project_lead b 
				WHERE a.lead_id=b.id and a.lead_id IN ($leadIdsString) AND a.status!=2 order by a.activity_id DESC";
 ...
 <div class="card card-style mt-5">
            <div class="content ">
                <h3>Personal RoadMap </h3>
				  	...
				  	//___________All ___________
                $personalsql= "SELECT * FROM hrm_roster_allocation WHERE MONTH(roster_date) = MONTH(CURDATE()) AND YEAR(roster_date) = YEAR(CURDATE())";
			
				$resultpersonal = db_query($personalsql);
				if ($resultpersonal) {
				while ($personalRow = mysqli_fetch_object($resultpersonal)) {
					// Format the date
					$formattedDate = date('d M', strtotime($personalRow->roster_date));

					// Format the time
					$formattedTime = date('h:i A', strtotime($personalRow->time));
					// Format the time
          					$point_1 = $personalRow->point_1;

							$shedule_1 = $personalRow->shedule_1;
    		  				$pid = $personalRow->PBI_ID;
				  ?>
		
        <div class="timeline-body mt-0">
            <div class="timeline-deco mt-5"></div>
<!--            <div class="timeline-item">
                <span class="timeline-icon bg-red-dark font-14">2020</span>
                <div class="timeline-item-content rounded-s">
                    <h5>Style 1</h5>
                    <p class="mb-0">
                        Enabled is the best selling Mobile Author on Envato's with over 35k Customers 
                    </p>
                    <a href="#" class="pt-3 d-block font-12">Read More</a>
                </div>
            </div>-->		            
            <div class="timeline-item">
                <span class="timeline-icon bg-blue-dark font-13"><?=$formattedDate?></span>
                <div class="timeline-item-content rounded-s">
				<button class="fa fa-edit color-brown-dark float-end m-2" data-menu="editschedulemeeting" onclick="openModalupdatetask('<?=$row->activity_id;?>')"></button>
				<h4><i class="color-red-dark fa fa-map-marker-alt"></i> <?=find_a_field('hrm_roster_point', 'point_short_name', 'id = "'.$point_1.'"')?></h4>
				
				<p class="mb-0"><i class="color-yellow-dark fa fa-user"></i> <?=find_a_field('personnel_basic_info', 'PBI_NAME', 'PBI_ID = "'.$pid.'"')?></p>
				
				<p class="mb-0">
					<i class="color-green-dark fa fa-calendar-alt"></i> <?=find_a_field('hrm_schedule_info', 'schedule_name', 'id = "'.$shedule_1.'"')?>
				</p>

   
                    <!--<a href="#" class="bg-highlight btn btn-full rounded-sm font-12 mt-4 text-uppercase font-700">Read More</a>-->
                </div>
            </div>										
            		<?     } }	 ?>			
        </div>
 		...
 		<div class="card card-style mt-5">
            <div class="content ">
				...
				while ($remainderRow = mysqli_fetch_object($resultRemainder)) {
					// Format the date
					$formattedDate = date('d M', strtotime($remainderRow->date));
...
<!--            <div class="timeline-item">
            ...
            <div class="timeline-item">
                <span class="timeline-icon bg-green-dark font-13"><?=$formattedDate?></span>
                <div class="timeline-item-content rounded-s">
				<button class="fa fa-edit color-brown-dark float-end m-2" data-menu="editschedulemeeting" onclick="openModalupdatetask('<?=$row->activity_id;?>')"></button>
					<h4><?=$formattedTime?></h4>
                    
									<div class="d-flex justify-content-between align-items-center">
											<h5><?=$remainderRow->activity_type?></h5>
                                            ...
                                            <span class=" modal-icon badge bg-green-dark float-end m-2 color-white font-10 ">COMPLETE</span>
												<? }elseif($remainderRow->status =='1'){ ?>
														   
											<span class="modal-icon badge bg-red-dark float-end m-2 color-white font-10 ">PENDING</span>
													<? }else{ ?>
														   
											<span class="modal-icon badge bg-blue-dark float-end m-2 color-white font-10  ">CANCELLED</span>
														<?  } ?>
							
						</div>
                    <p class="mb-0">
                        <?=$remainderRow->activity_type?> with <?=$remainderRow->lead_name?> .
                    </p>
					<?  $ids = explode(',', $remainderRow->assign_person);
					    foreach ($ids as $id) {  ?>
						<p class="font-11 font-800 text-uppercase color-theme mb-0 opacity-50">Assigned TEAM</p>
						<span class="badge bg-blue-dark color-white font-10 mt-2"><?=find_a_field('personnel_basic_info', 'PBI_NAME', 'PBI_ID = "'.$id.'"')?></span>	
...
<!--            <div class="timeline-item">
                <span class="timeline-icon bg-blue-dark font-14">2018</span>
                <div class="timeline-item-content rounded-s">
                    <h5>Style 3</h5>
                    <p class="mb-0">
                        Enabled starts releasing Atomic / Component design elements on Envato.
                    </p>
                    <a href="#" class="bg-highlight btn btn-sm btn-full rounded-sm font-12 mt-4 text-uppercase font-700">Read More</a>
                </div>
            </div>-->					
            		<?     } }	 ?>			
        </div>
