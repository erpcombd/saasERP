...
<style>
    ...
    <div class="page-content header-clear-medium">
		...
		<div class="content d-flex justify-content-center align-items-center">
			<span class="font-32 mb-0 color-black">Leads</span>
			<div class="ms-auto">
				<a href="../main/lead_details_entry.php" class="icon icon-xxl bg-theme gradient-green color-white shadow-l rounded-m">
...
//		  $leadsQry = "SELECT a.*,o.name FROM $table1 a,crm_project_org o WHERE a.organization=o.id and a.assign_person=$PBI_ID ORDER BY a.id DESC";
		  $leadsQry = "SELECT * FROM $table1 where lead_status!=9";
		  ...
		  $status = 'Cancel';
		  }elseif($row->lead_status ==5){
		  $status = 'No Bid';
		  }elseif($row->lead_status ==6){
		  $status = 'Proposal';
		  }elseif($row->status ==7){
		  $status = 'Qualified';
		  }elseif($row->lead_status ==8){
		  $status = 'Negotiation';
		  }elseif($row->lead_status ==9){
		  $status = 'Closed';
		  }else{
		  $status = 'Junk';
		  }
        ...
        <?php /*?><a href="../info_maker/lead_details_show.php?view=<?=encrypTS($row->id)?>&tp='<?=encrypTS('lead')?>'"><?php */?>
		 <a href="../info_maker/lead_details_manage_.php?view=<?=encrypTS($row->lead_id)?>&tp='<?=encrypTS('lead')?>'">
...
<!--                        <div class="align-self-center">
                        ...
                        <div class="align-self-center ps-4">
                            <h1 class="font-23 mb-0"><?=$row->company_name?></h1>
                            <p class="font-13 opacity-70 mt-2 mb-0"><?=$row->contact_email?></p>
							<p class="font-11 opacity-50 mt-n2 mb-0"><?=$formattedDate?> <?=$formattedTime?></p>
                        </div>

                        <div class="aling-self-center ms-auto">
							<a href="../info_maker/lead_details_manage_.php?view=<?=encrypTS($row->lead_id)?>&tp='<?=encrypTS('lead')?>'" class="btn btn-m float-end rounded-xl shadow-xl text-uppercase font-800 <?=$class?> fa-2x mt-1 modal-icon bg-highlight"><?=find_a_field('crm_lead_status_manage', 'status', 'id = "'.$row->lead_status.'"')?> </a>
                        </div>
                    </div>
                </div>
