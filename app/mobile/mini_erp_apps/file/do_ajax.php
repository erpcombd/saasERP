...
from sale_do_chalan 
where chalan_date>='".$opening_date."' and chalan_date<='".date('Y-m-d')."' and dealer_code='".$dealer_code."' ".$date_con." and item_id='".$item_id."' ");

// pri return
$pri_return=find1("select sum(d.total_unit) as qty 
from sale_return_master m, sale_return_details d 
where m.do_no=d.do_no and m.dealer_code='".$dealer_code."' and m.do_date>='".$opening_date."' and d.item_id='".$item_id."' and m.status in ('CHECKED') ");
