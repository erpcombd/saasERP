...
//var_dump($_SESSION);
$title = "Sales Return";
$page = "return_sales_rate_entry.php";
...
?><script>
		window.location.href = 'return_sales_rate_entry.php?pal=2';
	</script><?php
			}
...
//	}
	...
	function getData() {
		...
		jQuery.ajax({
			url: 'return_sales_rate_entry_ajax.php',
			type: 'post',
			data: 'id=' + id,
			success: function(result) {
				var json_data = jQuery.parseJSON(result);
...
<script type="text/javascript">
	...
	function FetchAllItemList(id) {
		$.ajax({
			type: 'post',
			url: 'return_item_ajax_with_rate.php',
			data: {
				subcategory_id: id
			},
			success: function(data) {
