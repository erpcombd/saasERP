...
$dealer     = findall('select * from ss_shop where dealer_code="'.$order->vendor_id.'"');
$mdealer    = findall('select * from dealer_info where dealer_code="'.$order->warehouse_id.'"');
...
<!--<a href="other_receive_status.php" class="back_button"><button class="btn btn-print" style="background-color:#00BCD4 !important; margin-bottom: 3px; color: #fff !important;">Back </button></a>-->
<!--<button id="printButton" onClick="window.print()" class="btn btn-print">Print</button>-->


	<!-- Alert Section -->
	<?php /*?><div class="card card-style">
		<div class="alert alert-success	 text-center font-16 font-weight-bold" role="alert">
			<b>Replace Return Number :</b> <?= htmlspecialchars($or_no) ?></span><br>
			<b>Date :</b> <?= $order->or_date ?>
		</div>
	</div><?php */?>
	<div class="row justify-content-between align-items-center mb-3 " style="width:100% !important;">
						<div class="col-6">
						<a href="#" class=" btn-m btn-full mb-3 invoice-header text-uppercase text-center font-900   ">Sales Return View</a>
	...
	<div class="content m-0">
		<!--<div class="card-header m-0 p-0">-->
			...
			<div class="col-6 table-responsive mb-2 table2 ">
					<table  cellspacing="0" cellpadding="2" border="0" class="hhs  mb-2">
						<thead>
							...
							<tr>
							    ...
							    <td width="30%">Sales Return Number </td>
								<td width="3%">:</td>
				...
				<!-- Chalan Details -->
			<div class="col-6 table-responsive justify-content-end d-flex mb-2 table2 ">
					<table cellspacing="0" cellpadding="2" border="0" class="hhs mb-2">
						<thead>
	...
	<!--<div class="card card-style" style=" zoom: 45%; ">
		<div class="content">-->
				...
				<tbody >
					...
					?>
						<tr>
							<td><?= $sl++ ?></td>
							<td ><?=($row->item_id) ?></td>
							<td align="left" class="text-left"><?= htmlspecialchars($row->item_name) ?></td>
							<td><?= number_format($row->rate, 2, '.', '') ?></td>
							<td><?= (int)$row->qty; $tqty+=$row->qty;?></td>
							<td><?= number_format($row->amount, 2, '.', '');$final_amount += $row->amount; ?></td>
					...
					<tr class="footer">
						...
						<td style="text-align:center"><?= number_format($tqty, 2, '.', '') ?></td>
						<td style="text-align:center"><?= number_format($final_amount, 2, '.', '') ?></td>
					</tr>
				</tbody>
			</table>
		</div>
