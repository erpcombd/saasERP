...
$dealer_code = $_SESSION['user']['username'];
$emp_code = $username;
$today 		= date('Y-m-d');
...
<div class="card card-style mb-0">
				<div class="content m-0">
                    ...
                    <select type="text" name="dealer_code" id="dealer_code" autocomplete="off" class="form-control-round">
					 <option>Select a Shop</option>
                        ...
                        where master_dealer_code='".$dealer_code."'
		...
		<div class="d-flex justify-content-center row m-0 mt-3">
			...
			<div class="card card-style"> 
				...
				<div class="table-responsive " style="zoom: 70%;height: 700px; overflow-x: auto; overflow-y: auto;  ">
					
					<table class="table table-borderless text-left table-scroll table_new_border" >
					...
					if(isset($_POST['submitit'])){
						...
						$res='select  m.do_no,m.do_no as do_no,m.do_date,m.remarks, s.shop_name as party, s.dealer_code as party_code, FORMAT(sum(d.total_amt),2) as Total,sum(d.pkt_unit) as qty from ss_do_master m, ss_do_details d , ss_shop s where m.do_no=d.do_no and m.dealer_code=s.dealer_code and m.status in("Checked","COMPLETED") '.$con. $shop_con.' and m.depot_id="'.$emp_code.'" group by m.do_no order by m.do_no desc';
							...
							<tr>
								...
								<!--<td><?=$data->remarks;?></td>-->
								<td>
								<div class="d-flex gap-2 justify-content-left align-items-center h-100 w-100">
