...
function process_invoice_order($do_no) {

$do_info = findall("select * from ss_do_details where do_no='".$do_no."' limit 1");


    $item_list = find1("select item_list from ss_gift_offer_invoice where year='2025' AND mon='5' limit 1");
    
    if (empty($item_list)) {
    return; // No items to check, skip processing
    }

    
    // Get total amount of the whole order for this do_no
   $sql_total = "SELECT SUM(total_amt) AS total_order_amt FROM ss_do_details WHERE do_no = '" .$do_no. "' and item_id in (".$item_list.")";

    $result_total = db_query($sql_total);
    $data_total = mysqli_fetch_assoc($result_total);
    $total_order_amt = isset($data_total['total_order_amt']) ? (float)$data_total['total_order_amt'] : 0;

    // Fetch gift rule that matches the total amount
    $sql = "SELECT item_list, min_taka, max_taka, gift_item_id FROM ss_gift_offer_invoice WHERE year='2025' AND mon='5' AND $total_order_amt BETWEEN min_taka AND max_taka ORDER BY min_taka DESC LIMIT 1"; // pick the most suitable highest tier
    $result = db_query($sql);

        // Delete previous gift item if exists
        $del_sql = "DELETE FROM ss_do_details WHERE do_no = '" .$do_no. "' AND gift_id = 1";
        db_query($del_sql);
    
    if ($row = mysqli_fetch_assoc($result)) {
        $gift_item_id = trim($row['gift_item_id']);



        if($gift_item_id>0){
            // Insert the matched gift item
            
            $insert_sql = "INSERT IGNORE INTO ss_do_details (do_no,do_date,dealer_code,depot_id ,item_id, total_unit, gift_id) 
            VALUES ('" .$do_no. "','" .$do_info->do_date."','" .$do_info->dealer_code."','" .$do_info->depot_id."', '" .$gift_item_id. "', 1, 1)";
            db_query($insert_sql);
        }
    }
...
function process_invoice_chalan($chalan_no) {

    $item_list = find1("select item_list from ss_gift_offer_invoice where year='2025' AND mon='5' limit 1");

    $sql_total = "SELECT SUM(total_amt) AS total_order_amt FROM ss_do_chalan WHERE chalan_no = '" .$chalan_no. "' and item_id in (".$item_list.")";
    $result_total = db_query($sql_total);
    $data_total = mysqli_fetch_assoc($result_total);
    $total_order_amt = isset($data_total['total_order_amt']) ? (float)$data_total['total_order_amt'] : 0;
    ...
    $result = db_query($sql);
        ...
        // Delete previous gift item if exists
        $del_sql = "DELETE FROM ss_do_chalan WHERE chalan_no = '" .$chalan_no. "' AND gift_id = 1";
        db_query($del_sql);    
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        ...
        if($gift_item_id>0){
            // Insert the matched gift item
            $ch_info = findall("select * from ss_do_chalan where chalan_no='".$chalan_no."' limit 1");
            
            $insert_sql = "INSERT IGNORE INTO ss_do_chalan (chalan_no,chalan_date,do_no,dealer_code,depot_id,item_id, total_unit) 
            VALUES ('" .$chalan_no. "','" .$ch_info->chalan_date."' , '" .$ch_info->do_no."', '" .$ch_info->dealer_code."', '" .$ch_info->depot_id."', '".$gift_item_id."', 1)";
            db_query($insert_sql);
        }
    }
...
function process_trade_offer($do_no) {

    // Fetch all trade commission rules
    $sql = "SELECT item_list, min_qty, max_qty, trade_com_per FROM ss_trade_comission where year='2025' and mon='2' ";
    $result = db_query($sql);

    while ($row = mysqli_fetch_assoc($result)) {
        $item_list = explode(',', $row['item_list']); // Convert item_list CSV into an array
        $min_qty = (int)$row['min_qty']; // Ensure integer type
        $max_qty = (int)$row['max_qty'];
        $trade_com_per = (float)$row['trade_com_per']; // Ensure decimal type

        // Sanitize and format item list for SQL
        $item_list_escaped = [];
        foreach ($item_list as $item) {
            $trimmed_item = trim($item);
            if (!empty($trimmed_item)) {
                //$item_list_escaped[] = "'" . mysqli_real_escape_string($con, $trimmed_item) . "'";
                $item_list_escaped[] = "'" . $trimmed_item. "'";
            }
        }

        if (empty($item_list_escaped)) {
            continue; // Skip processing if no valid items
        }

        $item_list_str = implode(',', $item_list_escaped); // Convert to SQL string

        // Get total quantity sold for these items in this do_no
        $sql2 = "SELECT SUM(total_unit) AS total_sold FROM ss_do_details 
                 WHERE do_no = $do_no 
                 AND item_id IN ($item_list_str)";
        ...
        $total_sold = isset($data['total_sold']) ? (int)$data['total_sold'] : 0;

        // Apply trade_com_per if total_sold is within the defined range
        if ($total_sold >= $min_qty && $total_sold <= $max_qty) {
            
            // Update nsp_per for these items in ss_do_details
            $update_sql = "UPDATE ss_do_details 
                           SET nsp_per = $trade_com_per 
                           WHERE do_no = '".$do_no."' 
                           AND item_id IN ($item_list_str)";
            db_query($update_sql);               
                           
        // update all rate and amt    
            $update_amt = "update ss_do_details 
            set unit_price = t_price-(t_price*(nsp_per/100)), total_amt = (total_unit * (t_price-(t_price*(nsp_per/100))) )
            WHERE do_no = '".$do_no."' ";  
            db_query($update_amt);
...
function process_trade_offer_chalan($ch_no) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // Get total quantity sold for these items in this do_no
        $sql2 = "SELECT SUM(total_unit) AS total_sold FROM ss_do_chalan WHERE chalan_no = '".$ch_no."' AND item_id IN ($item_list_str)";
        $result2 = db_query($sql2);
        ...
        if ($total_sold >= $min_qty && $total_sold <= $max_qty) {
            ...
            // Update nsp_per for these items in ss_do_details
            $update_sql = "UPDATE ss_do_chalan SET nsp_per = $trade_com_per WHERE chalan_no = '".$ch_no."' AND item_id IN ($item_list_str)";
            db_query($update_sql);               
           
                           
        // update all rate and amt   
            $update_amt = "update ss_do_chalan set unit_price = t_price-(t_price*(nsp_per/100)), total_amt = (total_unit * (t_price-(t_price*(nsp_per/100))) ) WHERE chalan_no = '".$ch_no."' ";  
            db_query($update_amt);
            
        // update journal item   
            $update_amt = "update ss_do_chalan c, journal_item j set j.item_price=c.unit_price WHERE c.chalan_no=j.sr_no and c.item_id=j.item_id and c.chalan_no = '".$ch_no."'  ";  
            db_query($update_amt); 
		  ...
		  while ($row=mysqli_fetch_row($res))
			...
			return ($row[0]);
				...
				function getScheduleRoute(){
					...
					$pbi_id = find_a_field('ss_user','user_id','username="'.$_SESSION['user']['username'].'"');
