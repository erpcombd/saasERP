		...
		while($data=mysqli_fetch_object($query)){
			...
			if(($_POST['chalan_'.$data->id]>0)||($_POST['chalan2_'.$data->id]>0)){
               ...
               $q = "INSERT INTO  ss_do_chalan (order_no, chalan_no, do_no, item_id, dealer_code,t_price,nsp_per, unit_price, pkt_size, pkt_unit, dist_unit, total_unit, total_amt,total_tp, chalan_date, depot_id, driver_name, driver_name_real,shipping_name, vehicle_no,gate_pass_no, delivery_man, entry_by, entry_at,do_date,offer_ignore,do_type,gift_id)
                ...
                '".$now."','".$master->do_date."','".$data->offer_ignore."','".$master->do_type."','".$data->gift_id."')";
...
// update trade commission rate
process_trade_offer_chalan($chalan_no);
...
// end dashboard	
 ...
 process_invoice_chalan($chalan_no);
...
<!-- start of Page Content-->  
   <div class="page-content header-clear-medium">
   	<a href="do_list.php"><button class="btn btn-print text-center" style="background-color:#00BCD4 !important; margin-left: 5px; color: #fff !important;">Back </button></a>
        ...
        <div class="card card-style mb-0">
				<div class="content mt-0 ms-0 me-0">
					...
					<select name="<?=$field?>" id="<?=$field?>" required readonly/>
						<option value="<?=$$field?>"><?php echo $party_name=find1('select shop_name from ss_shop where dealer_code="'.$$field.'"');?> (<?=$$field?>)</option>
					</select>
						</div>
						<div class="col-lg-6">
		...
		$pr="select d.item_id,sum(d.total_unit) as qty 
			...
			where m.do_no=d.do_no and m.dealer_code='".$dealer_code."' and m.sr_date>='".$opening_date."' 
			group by d.item_id";
			$query3 = db_query($pr);
...
<?php /*?><td><?=number_format($row->unit_price, 2, '.', '');?><?php */?>
...
?>		  
          ...
          <? } ?>
				...
				// 		db_update($table_master, $do_no, $vars, 'do_no');
				    $ssqld="update ss_do_master set status='COMPLETED' where do_no='".$do_no."' and do_type='Booking'";
                    db_query($ssqld);
						?>
						<!--<div class="col-12">-->
