...
if(isset($_POST['new'])){
	...
	// $_POST['longitude']=$_POST['longitude_do'];
		...
		$_POST['status']='MANUAL';
		$_POST['depot_id']=$dealer->master_dealer_code;
...
if(isset($_POST['no_order'])){
    ...
    if($_POST['flag'] == 0){
        ...
        unset($$unique);
        $type = 1;
        $msg = 'No Order status saved successfully.';
...
if(isset($_POST['close_order'])){
    ...
    if($_POST['flag'] == 0){
        ...
        unset($$unique);
        $type = 1;
        $msg = 'Shop marked as Close successfully.';
        header("Location: ../main/home.php");
    } else {
        $crud = new crud($table_master);
        $crud->update($unique_master);
    ...
    <div class="card card-style m-0">
        <form action="" method="post" name="codz2" id="codz2">
            ...
            <div class="content">
                ...
                <label for="dealer_code">Route Wise Shop</label>
                <select name="dealer_code" required="required" id="dealer_code" class="select2-container select2-results__options" style="overflow-y: auto !important;" onchange="FetchShopit(this.value)">
                    <option></option>
                    <? 
                    optionlist('select s.dealer_code, concat(s.dealer_code,"--",s.shop_name)
                ...
                <div class="row">
                    <div class="col-6">
                        ...
                        <input name="<?=$field?>"  id="<?=$field?>" value="<?=$$field?>" required class=" form-control" readonly/>
                    </div>	
...
<textarea name="remarks" id="remarks" placeholder="Note" class="form-select form-control"><?=$remarks?></textarea>
    ...
    <div class="col-4 p-0 pe-1">
        ...
        <? } else { ?>
            <input name="new" type="submit" value="Get Order" class="b-n btn btn-success btn-3d btn-block text-light w-100 py-3" onclick="return validateGetOrder()" />
            <input name="flag" id="flag" type="hidden" value="0" />
        <? } ?>
    </div>
    <div class="col-4 p-0 pe-1">
        <input type="submit" name="close_order" value="Close" class="b-n btn btn-primary btn-3d btn-block text-light w-100 py-3" onclick="return validateWithRemarks()" />
    </div>
    <div class="col-4 p-0">
        <input name="no_order" type="submit" value="No Order" class="b-n btn btn-danger btn-3d btn-block text-light w-100 py-3" onclick="return validateWithRemarks()" />
    </div>
</div>
            </div>
    ...
    <!-- Popup Modal -->
    <!--<div class="modal fade" id="popupModal" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 bg-transparent">
                <div class="circular-popup">
                    <button type="button" id="closePopupBtn" class="btn-close" aria-label="Close"></button>
...
<?php 
// Any PHP code should go here
// The selected_two function should be defined or included before use

 require_once '../assets/template/inc.footer.php';
?>

<script>
// Function for initializing Select2 on different elements
function selected_two(selector) {
    $(selector).select2({
        placeholder: 'Select',
        allowClear: true,
...
$(document).ready(function() {
    ...
    // Initialize Select2 for dealer_code
    $('#dealer_code').select2({
        placeholder: 'Select',
        allowClear: true,
        dropdownAutoWidth: true,
    ...
    // Initialize and show the popup modal
    setTimeout(function() {
...
// Define the showPosition function to use with geolocation
function showPosition(position) {
    // Handle the position data
    console.log("Latitude: " + position.coords.latitude + 
    ", Longitude: " + position.coords.longitude);
    // Add your code here to use the coordinates
}
...
// Get geolocation when page loads
window.onload = function() {
    getLocation();
};
</script>
<script>
function validateWithRemarks() {
    if (document.getElementById('remarks').value.trim() === '') {
        alert('Note is required for this action!');
...
function validateGetOrder() {
    // No validation for remarks is needed for Get Order
    return true;
}
</script>
