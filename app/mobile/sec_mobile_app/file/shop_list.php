...
//echo $res = "SELECT s.*, r.route_name FROM ss_shop s JOIN ss_route r ON s.route_id = r.route_id  WHERE s.status = 1 ORDER BY s.dealer_code DESC;";

$res="SELECT s.dealer_code, s.route_id, r.route_name, s.shop_name, s.shop_address, s.emp_code, s.shop_owner_name, s.mobile, s.shop_class, s.shop_type, s.shop_channel, s.shop_route_type, s.shop_identity
FROM ss_shop s
INNER JOIN ss_route r ON s.route_id = r.route_id
WHERE s.status = 1  and emp_code ='".$username."'
