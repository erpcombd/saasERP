...
// Define maximum allowed distance (800 meters)
define('MAX_ATTENDANCE_DISTANCE', 800);
...
if (isset($_POST['punch_in_'.$data2->id]) || isset($_POST['punch_out_'.$data2->id])) {
    ...
    if (!empty($punch_LATITUDE) && !empty($punch_LONGITUDE)) {
        // Calculate distance
        ...
        if ($distance <= MAX_ATTENDANCE_DISTANCE && $schedule_LATITUDE>0) {
            //  Normal attendance
            ...
            $sql_att = 'INSERT INTO hrm_attdump (`ztime`, `bizid`, `EMP_CODE`, `xenrollid`, `time`, `xtime`, `xdate`, `xlocationid`, 
                ...
                "'.date('Y-m-d H:i:s').'", "'.date('Y-m-d').'", "999", "'.$punch_LATITUDE.'", 
                "'.$punch_LONGITUDE.'", "'.$_POST['schedule_notes_'.$data2->id].'", "'.$schedule_LATITUDE.'", 
                "'.$schedule_LONGITUDE.'")';
            $conn->query($sql_att);
            ...
            echo "<script>alert('Attendance recorded successfully.'); window.location.reload();</script>";

        } else {
            //  Out of allowed range - Show popup
            $msg2 = "You are outside the allowed attendance range. Distance: " . round($distance, 2) . " meters.";
            ?>
            <script>
            if (confirm("<?php echo $msg2; ?>\nDo you want to force attendance?")) {
                <?php
                // Force attendance insert
                $status = isset($_POST['punch_in_'.$data2->id]) ? 'PROCESSING' : 'COMPLETED';
                $sql_att = 'INSERT INTO hrm_attdump_apps (`ztime`, `bizid`, `EMP_CODE`, `xenrollid`, `time`, `xtime`, `xdate`, `xlocationid`, 
                    ...
                    VALUES ("'.date('Y-m-d H:i:s').'", "'.$PBI_ID.'", "'.$PBI_ID.'", "'.$PBI_ID.'", "'.$time.'", 
                    "'.date('Y-m-d H:i:s').'", "'.date('Y-m-d').'", "999", "'.$punch_LATITUDE.'", 
                    "'.$punch_LONGITUDE.'", "'.$_POST['schedule_notes_'.$data2->id].'", "'.$punch_LATITUDE.'", 
                    "'.$punch_LONGITUDE.'", "'.$incharge.'")';
                $conn->query($sql_att);

                $sql_in = "UPDATE `hrm_od_info` SET `od_att_status` = '".$status."' WHERE id = '".$att_punch_in_id."'";
                $conn->query($sql_in);
                ?>
                alert("Force attendance recorded successfully.");
        ...
        echo "<script>alert('Location data is missing. Please ensure location services are enabled.');</script>";
    }
}
...
<div class="card card-style">
    <form action="od_entry.php" method="post">
		...
		<input name="PBI_ID" type="hidden" id="PBI_ID" class="form-control validate-text" value="<?=$PBI_ID?>" placeholder="Employee Code" readonly required/>
				 ...
				 <div class="col-12">
					...
					<div class="col-6"> 
					  ...
					  <input type="date" name="s_date" class="form-control" id="roster_date" required>
					</div>
					...
					<div class="col-6">
					 ...
					 <select class="form-control req" name="project_id" id="point_1" required>
					  <option value="">Select Project Name</option>
					  <? foreign_relation('crm_project_org','id','name',$_POST['point_1'],'1'); ?>
					</select>
...
SELECT * 
FROM hrm_od_info a 
WHERE a.PBI_ID = '$PBI_ID'
AND (
    (YEAR(a.s_date) = YEAR(CURDATE()) AND MONTH(a.s_date) = MONTH(CURDATE()))  
    OR
    (YEAR(a.s_date) = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 MONTH)) AND MONTH(a.s_date) = MONTH(DATE_SUB(CURDATE(), INTERVAL 1 MONTH)))
)
ORDER BY a.id DESC
";
...
<script>
$('#roster_date').on('change', function () {
   
    const date = $(this).val();
    const PBI_ID = $('#PBI_ID').val();

    if (date && PBI_ID) {
        $.ajax({
            url: 'get_project_by_roster.php',
            type: 'POST',
            data: {
                roster_date: date,
                PBI_ID: PBI_ID
            ...
            success: function (response) {
                ...
                try {
                    const data = response;  // Try parsing JSON
                    console.log(data);

                    if (data.point_1) {
                        console.log('fahimmmmmmmmmmmmmm' + data.point_1);
                        $('#point_1').val(data.point_1);
                    } else {
                        $('#point_1').val('');
                        alert(data.message || 'No project found.');
                    }
                } catch (err) {
