...
function auto_insert_sales_return_cogs_secoundary_toriqul($chalan_no,$jv_noo)
{
...
auto_insert_sales_return_cogs_secoundary_toriqul($chalan_no,$jv_no);
...
function has_permission($fileName, $userLevel, $conn) {
  $role_sql = "SELECT allowed_page FROM user_role WHERE id = '$userLevel' LIMIT 1";
  $role_result = mysqli_query($conn, $role_sql);
  
  if (mysqli_num_rows($role_result) > 0) {
      $role_data = mysqli_fetch_assoc($role_result);
      
      $allowed_pages = json_decode($role_data['allowed_page'], true);
