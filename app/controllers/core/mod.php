...
add_to_sec_journal($proj_id, $jv_no, $jv_date, $srow->item_ledger, $narration, '0', ($srow->tot_gift_amt), $tr_from, $tr_no,$srow->sub_ledger_id,$tr_id,$cc_code,$group_for);
}
}
else{
...
function auto_insert_sales_cogs_secoundary($chalan_no,$jv_noo)
...
function auto_insert_lc_grn_secoundary_cus($pr_no)

{
			...
			$cc_code = 0;
			$tr_from = 'Journal';
			$proj_id="Jamuna Paper";
			$pr_all = find_all_field('lc_purchase_receive','*',"pr_no=".$pr_no);
			$config = find_all_field('config_group_class','',"group_for=".$group_for);
			$jv_no=next_journal_sec_voucher_id('','Journal',$_SESSION['user']['depot']);	
			$tr_no = $pr_no;
			$tr_id =$pr_no;
			$lc_data = find_all_field('lc_number_setup','','id='.$pr_all->lc_no); 
			
			
	$jv_date = $pr_all->rec_date;		
			////////////manual vou start////
	
	// Convert $jv_date to parts
$year  = date('y', strtotime($jv_date)); // last 2 digits of year
$month = date('m', strtotime($jv_date)); // 2-digit month
$day   = date('d', strtotime($jv_date)); // 2-digit day

$prefix = "JV";
$date_prefix = $year.$month.$day;

// Get the max serial number for today
  $query = "
    SELECT MAX(CAST(SUBSTRING(manual_voucher_no, 10, 3) AS UNSIGNED)) AS max_serial
    FROM secondary_journal
    WHERE manual_voucher_no LIKE 'JV$date_prefix%'
      AND LENGTH(manual_voucher_no) = 11
";
$result = db_query($query);
$row = mysqli_fetch_object($result);
 
if ($row->max_serial && $row->max_serial < 999) {
 
    $serial = (int)$row->max_serial + 1;
} else {
 
    $serial = 1;
}
// Pad serial to 3 digits
$serial_padded = str_pad($serial, 3, "0", STR_PAD_LEFT);
// Build final voucher number
$voucher_no = $prefix . $date_prefix . $serial_padded;
	$manual_voucher_no = $voucher_no;
		...
		$sql2 = 'select sum(l.amount) as cost_amt_bdt, i.ledger_id as item_ledger, s.item_ledger as item_group_ledger ,l.rec_date
		from lc_purchase_receive l, item_info i, item_sub_group s 
		where l.item_id=i.item_id and i.sub_group_id=s.sub_group_id and l.pr_no="'.$pr_no.'" group by i.item_id order by i.sub_group_id';
		$query2 = db_query($sql2);
		while($data2=mysqli_fetch_object($query2)){
	...
	add_to_sec_journal($proj_id,$jv_no, $data2->rec_date, $item_ledger, $narration_dr, $data2->cost_amt_bdt, 0,  $tr_from, $tr_no, $data2->sub_ledger_id, $tr_id, $cc_code, $group_for,'','', '', '', '', '',$data2->sub_ledger_id, 'NO', $data2->payment_method,'','','','LC Journal', '' );

$total_cr_amt_get+=$data2->cost_amt_bdt;
		}


			$narration_dr = $data->lc_number;		
			$narration_cr = 'L/C GRN';

add_to_sec_journal($proj_id,$jv_no, $jv_date, $lc_data->ledger_id, $narration_cr,  0, $total_cr_amt_get, $tr_from, $tr_no, $lc_data->sub_ledger_id, $tr_id, $cc_code, $group_for,'','', '', '', '', '',$lc_data->sub_ledger_id, 'NO', $data->payment_method,'','','','LC Journal', '' );

$up = 'update secondary_journal set  manual_voucher_no="'.$manual_voucher_no.'" where jv_no="'.$jv_no.'"';
	db_query($up);
$jv_config = find_a_field('voucher_config','direct_journal','voucher_type="'.$tr_from.'"');
...
function auto_insert_lc_closing_secoundary_jamuna($closing_no)

{
	...
	$sql = 'select closing_no, closing_date, group_for, lc_no, lc_number,  ledger_id,sub_ledger_id, sum(total_pay_amt) as gr_total_pay_amt,sum(cost_amt_bdt) as tot_actual_cost_amt,sum(difference_amt) as tot_difference_amt from lc_purchase_closing
	...
	$data=mysqli_fetch_object($query);
			...
			$jv_no=next_journal_sec_voucher_id('','Journal',$_SESSION['user']['depot']);
			 	...
			 	$difference_amt_get=$data->tot_difference_amt;
		if($difference_amt_get>0){
	 
		$debit_ledger=$data->ledger_id;
		$credit_ledger=$config->lc_closing_adjust_ledger;
		$diff_amt=$difference_amt_get;
		}
		else{
		 
			$debit_ledger=$config->lc_closing_adjust_ledger;
		$credit_ledger=$data->ledger_id;
		$diff_amt=($difference_amt_get*(-1));
		}
...
add_to_sec_journal($proj_id,$jv_no, $jv_date, $debit_ledger, $narration_cr,$diff_amt,0, $tr_from, $tr_no, $data->sub_ledger_id, $tr_id, $cc_code, $group_for,'','', '', '', '', '',$data->sub_ledger_id, 'NO', $data->payment_method,'','','','LC Journal', '' );


add_to_sec_journal($proj_id,$jv_no, $jv_date, $credit_ledger, $narration_cr,0,$diff_amt, $tr_from, $tr_no, $data->sub_ledger_id, $tr_id, $cc_code, $group_for,'','', '', '', '', '',$data->sub_ledger_id, 'NO', $data->payment_method,'','','','LC Journal', '' );
...
function next_transection_no_closing($depot_id,$chalan_date,$table,$tr_no)

		{

			$sdate = mysqli_format2stamp($chalan_date);

			$e_ch = date('y',$sdate).date('m',$sdate).date('d',$sdate).sprintf('%03s', $depot_id).'999';

			$s_ch = date('y',$sdate).date('m',$sdate).date('d',$sdate).sprintf('%03s', $depot_id).'000';
...
function auto_insert_lc_closing_secoundary_old_28092025($closing_no)
...

{
	...
	db_delete_all('secondary_journal','tr_from = "Purchase" AND `tr_no` = "'.$pr_no.'"');

	db_delete_all('journal','tr_from = "Purchase" AND `tr_no` = "'.$pr_no.'"');
...
$d_sql = "select r.id as tr_id,r.with_vat_rate,r.with_vat_amt as amount,r.vat_amt,r.deduction_qty,r.detection_amount,s.item_ledger,i.sub_ledger_id  from purchase_receive r,item_info i,item_sub_group s  where  r.pr_no=".$pr_no." and i.item_id=r.item_id and i.sub_group_id=s.sub_group_id";
...
$qc_deduct_qty_amount += $data->with_vat_rate*$data->deduction_qty;
...
function next_journal_voucher_id($date='',$tr_from = '',$depot_id='')
	...
	elseif($tr_from == 'Import') 			$tr = '33';	
	elseif($tr_from == 'Batch Closing') 			$tr = '34';	
	...
	elseif($tr_from == 'Import') 			$tr = '33';
	elseif($tr_from == 'Goods Return') 			$tr = '34';
...

{
	...
	elseif($tr_from == 'Other Issue') $tr = '21';
	
	elseif($tr_from == 'Other Receive') $tr = '22';
...
function journal_item_control($item_id, $warehouse_id,$ji_date,$item_in,$item_ex,$tr_from,$tr_no,$rate='',$r_warehouse='',$sr_no='',$bag_size='',$bag_unit='',$group_for='',$final_price='',$barcode='',$serial_no='',$lot_no='')

{

	$item_in =(float) $item_in;
	$item_ex =(float) $item_ex;

$pre_stock=find_all_field('journal_item','final_stock',' item_id = "'.$item_id .'"  order by id desc');

$pre_stock_final_stock = find_a_field('journal_item','sum(item_in-item_ex)','item_id = "'.$item_id.'"');
...
if($item_in>0){
	...
	elseif($tr_from=='Issue'){

			$item_price =find_a_field('journal_item','item_price',' item_id = "'.$item_id .'" and  tr_from="Issue" and  item_ex>0 and tr_no="'.$tr_no.'" 
			order by id desc');

			if($item_price>0){$item_price=$item_price;}else{$item_price=$rate;}


		$final_price = ((( (float)$pre_stock->final_price * (float)$pre_stock_final_stock )+ ( (float)$item_price * ((float)$item_in )) ) / ( (float)$final_stock ));

		$c_price=$final_price;

		}
		elseif($tr_from=='fg_transfer'){

			$item_price =find_a_field('journal_item','item_price',' item_id = "'.$item_id .'" and  tr_from="fg_transfer" and  item_ex>0 and tr_no="'.$tr_no.'" 
			order by id desc');
	...
	if(($tr_from=='Purchase Return') || ($tr_from == 'Batch issue')  || ($tr_from == 'Consumption') || ($tr_from == 'Batch Closing')){
	
		$item_price =$rate; 
		
	}elseif($tr_from =='Issue'){

		$item_price =find_a_field('journal_item','final_price',' item_id = "'.$item_id .'" and  tr_from="Issue" and  item_ex>0 and tr_no="'.$tr_no.'" 
			order by id desc');
	...
	if($final_stock>0){
		$final_price = ((( (float)$pre_stock->final_price * (float)$pre_stock_final_stock )+ ( (float)$item_price * ((float)$item_in - (float)$item_ex)) ) / ( (float)$final_stock ));
	}
  ...
  (ji_date, item_id, warehouse_id, pre_stock, pre_price, item_in, item_ex, item_price, final_stock, final_price,tr_from, tr_no, entry_by, entry_at,relevant_warehouse,sr_no,c_price,bag_size,bag_unit,group_for, barcode,pre_id,serial_no,lot_no) 

  VALUES 

  ('".$ji_date."', '".$item_id."', '".$warehouse_id."', '".$pre_stock_final_stock."', '".$pre_stock->final_price."', '".$item_in."', '".$item_ex."', '".$item_price."', '".$final_stock."', '".$final_price."', '".$tr_from."', '".$tr_no."', '".$_SESSION['user']['id']."', '".date('Y-m-d H:i:s')."', '".$r_warehouse."', '".$sr_no."','".$c_price."','".$bag_size."', '".$bag_unit."', '".$group_for."', '".$barcode."','".$pre_id."','".$serial_no."','".$lot_no."')";
