...
$vat_on_sales = ($amount_after_discount*$do_master->vat)/100;
...
add_to_sec_journal($proj_id, $jv_no, $jv_date, $config_ledger->sales_ledger, $narration, '0', ($sales_amt), $tr_from, $tr_no,$dealer->sub_ledger_id,$tr_id,$cc_code,$group_for);


$sql='select d.item_id,d.unit_price,d.total_unit, sum(d.unit_price_gift_item*d.total_unit) as tot_gift_amt,i.item_id,i.sub_group_id,s.sub_group_id,s.item_ledger from sale_do_chalan d,item_info i,item_sub_group s  where d.item_id=i.item_id and i.sub_group_id=s.sub_group_id and d.chalan_no="'.$chalan_no.'" group by i.sub_group_id';
$query=db_query($sql);
...
add_to_sec_journal($pbroj_id, $jv_no, $jv_date, $srow->item_ledger, $narration, '0', ($srow->tot_gift_amt), $tr_from, $tr_no,$dealer->sub_ledger_id,$tr_id,$cc_code,$group_for);
}
}
else{
...

{
	...
	//add_to_sec_journal($proj_id, $jv_no, $jv_date, $config->cogs_ledger, $narration,   $data->total_amt, '0',  $tr_from, $tr_no,'',$tr_id, $cc_code, $group_for);
		 ...
		 $sql2 = 'select s.item_ledger as item_ledger,s.cogs_ledger as cogs_ledger, sum(c.cost_amt) as item_amt from sale_do_chalan c, item_info i, item_sub_group s 

		where c.item_id=i.item_id and i.sub_group_id=s.sub_group_id and c.chalan_no="'.$chalan_no.'" group by s.sub_group_id order by s.sub_group_id';
		...
		add_to_sec_journal($proj_id, $jv_no, $jv_date, $data2->cogs_ledger, $narration,($data2->item_amt), '0',$tr_from, $tr_no,'',$tr_id, $cc_code, $group_for);
...

{
	...
	$sql = 'select receipt_no, receipt_date, dr_ledger_id,sub_ledger_id,sum(receipt_amt) as receipt_amt from receipt_from_customer where receipt_no="'.$receipt_no.'" GROUP by receipt_no';
...
add_to_sec_journal($proj_id, $jv_no, $jv_date, $data2->ledger_id, $narration_cr, '0', ($data2->receipt_amt),  $tr_from, $tr_no,$data2->sub_ledger_code, $data2->chalan_no,$cc_code,$group_for);
...
add_to_sec_journal($proj_id, $jv_no, $jv_date, $data->cr_ledger_id, $narration_cr,  '0', ($data->tot_pay_amt), $tr_from, $tr_no,$data->lc_ledger,$tr_id, $cc_code, $group_for,'','', '', '', '', '',$data->lc_ledger, 'NO', $data->payment_method,'','','','LC Journal', '' );
...
//		}	
	...
	add_to_sec_journal($proj_id, $jv_no, $jv_date, $data2->lc_ledger, $data2->category_view, ($data2->pay_amt_in), '0', $tr_from, $tr_no, $data2->cr_ledger_id, $tr_id, $cc_code, $group_for,'','', '', '', '', '', $data2->cr_ledger_id, 'NO', $data->payment_method,'','','','LC Journal', $data2->bill_category );
...

{
	...
	$sql = 'select closing_no, closing_date, group_for, lc_no, lc_number,  ledger_id, total_pay_amt from lc_purchase_closing
...
add_to_sec_journal($proj_id,$jv_no, $jv_date, $data->ledger_id, $narration_cr,  0, $data->total_pay_amt, $tr_from, $tr_no, $data->lc_ledger, $tr_id, $cc_code, $group_for,'','', '', '', '', '',$data->lc_ledger, 'NO', $data->payment_method,'','','','LC Journal', '' );
	...
	add_to_sec_journal($proj_id,$jv_no, $jv_date, $item_ledger, $narration_dr, $data2->cost_amt_bdt, 0,  $tr_from, $tr_no, $data2->lc_ledger, $tr_id, $cc_code, $group_for,'','', '', '', '', '',$data2->lc_ledger, 'NO', $data2->payment_method,'','','','LC Journal', '' );
...
$d_sql = "select r.id as tr_id,r.amount,s.item_ledger from purchase_receive r,item_info i,item_sub_group s  where  r.pr_no=".$pr_no." and i.item_id=r.item_id and i.sub_group_id=s.sub_group_id";
...
add_to_sec_journal($proj_id, $jv_no, $jv_date, $data->item_ledger, $narration,($data->amount+$vat_on_purchase), '0', $tr_from, $pr_no,'',$data->tr_id,$cc_code,$group_for);
...
add_to_sec_journal($proj_id, $jv_no, $jv_date,$config_ledger->purchase_vat, $narration,($total_rebate_amt), '0', $tr_from, $pr_no,$data->sub_ledger_id,$data->id,$cc_code,$data->group_for);
}
...
if(	$ledger_type	==	'journal' ) $balance_type = " and 1  ";

if(	$ledger_type	==	'contra' ) $balance_type  = "  and ledger_group_id IN (126001,126002)  ";
...
function add_to_sec_journal($proj_id, $jv_no, $jv_date, $ledger_id, $narration, $dr_amt, $cr_amt, $tr_from, $tr_no,$sub_ledger='',$tr_id='',$cc_code='',$group='',$entry_by='',$entry_at='',$received_from='', $bank='', $cheq_no='',$cheq_date='',$relavent_cash_head='',$checked='',$type='',$employee='',$remarks='',$file_upload='',$reference_id='',$note='',$pay_id='') 
...

{
   ...
   $journal="INSERT INTO `secondary_journal` (
	...
	,'$bank','$cheq_no','$cheq_date','$relavent_cash_head','$checked','$type','$employee','$remarks','$file_upload','$reference_id','$note','$pay_id')";
   ...
   $sql = 'select * from secondary_journal where jv_no = "'.$sec_jv_no.'" and tr_from = "'.$tr_froms.'"';
    ...
    $journal="INSERT INTO `journal` (
    	...
    	cc_code
            ...
            '$data->tr_id','$data->sub_ledger','".date('Y-m-d H:i:s')."', '$data->group_for','".$data->entry_by."','".$data->cc_code."')";
...
function journal_item_control($item_id, $warehouse_id,$ji_date,$item_in,$item_ex,$tr_from,$tr_no,$rate='',$r_warehouse='',$sr_no='',$bag_size='',$bag_unit='',$group_for='',$final_price='',$barcode='' )
...
$item_price = $pre_stock->final_price;

$final_price =$pre_stock->final_price;
...
$c_price=$final_price;
  ...
  (ji_date, item_id, warehouse_id, pre_stock, pre_price, item_in, item_ex, item_price, final_stock, final_price,tr_from, tr_no, entry_by, entry_at,relevant_warehouse,sr_no,c_price,bag_size,bag_unit,group_for, barcode) 

  VALUES 

  ('".$ji_date."', '".$item_id."', '".$warehouse_id."', '".$pre_stock->final_stock."', '".$pre_stock->final_price."', '".$item_in."', '".$item_ex."', '".$item_price."', '".$final_stock."', '".$final_price."', '".$tr_from."', '".$tr_no."', '".$_SESSION['user']['id']."', '".date('Y-m-d H:i:s')."', '".$r_warehouse."', '".$sr_no."','".$c_price."','".$bag_size."', '".$bag_unit."', '".$group_for."', '".$barcode."')";
