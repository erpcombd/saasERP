...
?>
		.logo-menu-cid-class{
			width: <?=$allCss->menu_width;?> !important;
			height: <?=$allCss->menu_height;?> !important;
		}
                        .page_title{
                            border-radius: 5px;
                        ...
                        * {
                        ...
                        /*THIS CSS IS FOR SEARCHING MODULE*/
                        ...
                        text-align: left; /* Ensure text stays on the left */
                        }
                        .submenu-suggestions a:hover {
                        background-color: #4F87C7;
                        color: white;
                        }
                        /* Scrollbar styling for Chrome, Edge, and Safari */
                        .submenu-suggestions::-webkit-scrollbar {
                            width: 2px; /* Scrollbar width */
                        }

                        .submenu-suggestions::-webkit-scrollbar-track {
                            background: #f1f1f1; /* Track background */
                            border-radius: 10px;
                        }

                        .submenu-suggestions::-webkit-scrollbar-thumb {
                            background: #888; /* Scrollbar thumb color */
                            border-radius: 10px;
                        }

                        /*END OF THIS CSS IS FOR SEARCHING MODULE*/
...
$module_sql='select m.module_name,m.id,m.module_link,m.module_icon_img,m.module_menu_icon,d.module_id,d.user_id from user_module_define d,user_module_manage m where d.module_id=m.id and m.status="Yes" and d.status="enable" and d.user_id="'.$_SESSION['user']['id'].'" and m.id!='.$mod_id.' ';
                        ...
                        <div class="right1" >
                         ...
                         <div class="sidemenu-top">
							...
							<a href="<?=SERVER_VIEW?>auth/masters/home.php">
								...
								if($company_logo !=''){
								?>
									<img alt="this is img" src="<?=SERVER_UPLOAD?>logo/<?=$_SESSION['proj_id']?>.png" class="logo-menu-cid-class" >
                                 <? } else{?>
								 	<img alt="this is img" src="<?=$default_logo;?>" style=" width: 70%; ">
								 <? } ?>
								</div>
							...
							<div class="position-relative ml-1 mr-1">
                                <!-- Search Input -->
                                <input type="search" id="submenuSearch" onkeyup="showSuggestions()" 
                                    class="submenu-search-input pr-4 pl-3" 
                                    placeholder="Type to menu search..." oninput="toggleSearchIcon()" autocomplete="off">
                                ...
                                <div id="suggestions" class="submenu-suggestions"></div>
                            </div>
...
function showSuggestions() {
    let input = document.getElementById("submenuSearch");
    let filter = input.value.toUpperCase();
    let suggestionsDiv = document.getElementById("suggestions");
    let items = document.querySelectorAll(".dashboard1-nav-dropdown-item");
    
    suggestionsDiv.innerHTML = ''; // Clear previous suggestions
    toggleSearchIcon(); // Ensure icons update while typing
    ...
    items.forEach(function(item) {
        ...
        if (txtValue.toUpperCase().indexOf(filter) > -1 && matches < 10) {
            // Create a new suggestion element
            let suggestion = document.createElement("a");
            suggestion.href = item.href;
            suggestion.classList.add("submenu-suggestion-item");
            suggestion.innerHTML = item.innerHTML;

            // Add suggestion to dropdown
            suggestionsDiv.appendChild(suggestion);
            matches++;
        }
    });

    // Show/hide suggestions dropdown based on matches
    suggestionsDiv.style.display = matches > 0 ? "block" : "none";
}
...
document.getElementById("suggestions").addEventListener("click", function(e) {
    if (e.target.tagName === "A") {
        let input = document.getElementById("submenuSearch");
        input.value = e.target.textContent; // Set input value to selected text
...
// Toggle search & clear icons dynamically
...
// Clear search box & reset suggestions
...
//     // Check if there's any submenu with the 'active1' class
//     const activeSubmenu = document.querySelector('.dashboard1-nav-dropdown .active1');

//     if (activeSubmenu) {
//         // Get the #menu_bg container
//         const menuBg = document.getElementById('menu_bg');
        
//         // Scroll the menu to the top of the active submenu
//         menuBg.scrollTop = activeSubmenu.offsetTop;
//     }
// });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (activeSubmenu) {
        ...
        // Calculate the position of the active submenu relative to the #menu_bg container
        const submenuTop = activeSubmenu.offsetTop;
        const submenuHeight = activeSubmenu.offsetHeight;
        const menuBgHeight = menuBg.offsetHeight;
        
        // Scroll the menu to make the submenu appear in the middle of the container
        const scrollPosition = submenuTop - (menuBgHeight / 2) + (submenuHeight / 2);
        
        // Apply the scroll position
        menuBg.scrollTop = scrollPosition;
    }
});
                        ...
                        <div class="right1" >
                          ...
                          <div class="sidemenu-top">
							...
							<!--This code for search menu  start-->
                                ...
                                <input type="search" id="submenuSearch" onkeyup="showSuggestions()" 
                                    class="submenu-search-input pr-4 pl-3" 
                                    placeholder="Type to menu search..." oninput="toggleSearchIcon()">

                                <!-- Search Icon -->
                                <span id="searchIcon" class="position-absolute menusearch-span"
                                    style="right: 35px; top: 50%; transform: translateY(-50%); color: gray; cursor: pointer;">
                                    <i class="fas fa-search"></i>
                                </span>
