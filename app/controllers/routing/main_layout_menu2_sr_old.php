...
?>
                        ...
                        <div class="right1" >
							...
							<div class="position-relative ml-1 mr-1">                                
								<input type="text" id="menuSearch" onkeyup="filterMenu()" class="menusearch pr-4 pl-3" placeholder="Type to menu search..." oninput="toggleSearchIcon()">
							...
							<p id="noMenuMessage" style="display: none; color: red; font-weight: bold;">No menu found in this name</p>
...
function filterMenu() {
    ...
    let anyVisible = false; // Track if any menu is visible
    ...
    menus.forEach(menu => {
        ...
        submenus.forEach(submenu => {
            ...
            if (submenuText.includes(filter) || menuTitle.includes(filter)) {
        ...
        if (submenuVisible || menuTitle.includes(filter)) {
            ...
            submenuContainer.style.display = "block"; // Open submenu if there's a match
            anyVisible = true;
        } else {
            menu.style.display = "none";
            submenuContainer.style.display = "none"; // Ensure submenu stays closed when no match
        }
    });

    // Show "No menu found" message if no menu is visible
    noMenuMessage.style.display = anyVisible ? "none" : "block";

    // Reset all dropdowns when search is cleared
    if (filter === "") {
        noMenuMessage.style.display = "none";
        menus.forEach(menu => {
            menu.style.display = "block"; // Show all menus again
            menu.querySelector(".dashboard1-nav-dropdown-menu").style.display = "none"; // Collapse submenus
        });
    }
}


// Ensure dropdown toggles only open when clicked, regardless of search
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".dashboard1-nav-dropdown-toggle").forEach(toggle => {
...
function clearSearch() {
    ...
    input.value = "";
    toggleSearchIcon();
    filterMenu(); // Re-run the filtering function to reset the menu
}
